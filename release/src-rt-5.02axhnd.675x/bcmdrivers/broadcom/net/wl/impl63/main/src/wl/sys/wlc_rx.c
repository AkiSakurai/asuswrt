 ...
 *
 * $Id: wlc_rx.c 781787 2019-11-28 14:31:43Z $
 *
 */
#include <wlc_cfg.h>
#include <typedefs.h>
	...
	int len, bool update_roam)
{
	...
	if (SCAN_IN_PROGRESS(wlc->scan))
		wlc_recv_scan_parse(wlc, wrxh, plcp, hdr, body, body_len);
#ifdef WL_OCE
	/* TODO: can we avoid this overhead on every beacon processing, and put in scan time */
	if (OCE_ENAB(wlc->pub)) {
		if (CHSPEC_IS2G(wlc->cfg->current_bss->chanspec)) {
	...
	struct dot11_management_header *hdr, uint8 *body, int bcn_len, bool *cur_bss_bcn)
{
	...
	*cur_bss_bcn = FALSE;
	...
	if (AP_ACTIVE(wlc) || wlc->ibss_bsscfgs > 0) {
		 ...
		 */
		/* TODO:6GHZ: add functionality for handling 6GHz bcn_channel as well */
		chspec20mhz = (WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(bcn_channel) | bcn_channel);
		...
		if (AP_ACTIVE(wlc)) {
			...
			ftpparm.bcn.scb = scb;
			ftpparm.bcn.chan = bcn_channel; /* TODO:6GHZ: use chspec instead */
			ftpparm.bcn.cap = cap;
			...
			pparm.rxchan = CHSPEC_CHANNEL(rx_chanspec); /* TODO:6GHZ: use chspec */
	...
	*cur_bss_bcn = TRUE;
	...
	pparm.hdr = (uint8 *)hdr;
	pparm.body = (uint8 *)bcn;
	pparm.rxchan = CHSPEC_CHANNEL(rx_chanspec);
	pparm.ht = N_ENAB(wlc->pub);
#ifdef WL11AC
	pparm.vht = VHT_ENAB(wlc->pub);
#endif // endif
        ...
        struct dot11_management_header *hdr, uint8 *body, int bcn_len)
{
	...
	*/
	...
	if (add_ie) {
		...
		(ds_params->len == 1)) {
		/* TODO:6GHZ: extend the code below for 6GHz channels */
		uint ch = ds_params->data[0];
		/* use channel info from DS Params IE if present */
		rx_chanspec = (WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(ch) | ch);
	}
	else {
...
{
	/* TODO:6GHZ: this function needs to be changed to return an (optionally 6GHz) 20MHz
	 * chspec.
	 */
	chanspec_t rxchan;
	uint16 channel;
...
wlc_recv_prep_event_rx_frame_data(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh, uint8 *plcp,
                             wl_event_rx_frame_data_t *rxframe_data)
{
	/* TODO:6GHZ: this function should be able to deal with 6ghz as well */
	chanspec_t chspec20mhz;
	uint32 tsf, rate;

	rxframe_data->version = hton16(BCM_RX_FRAME_DATA_VERSION);
	chspec20mhz = wlc_recv_mgmt_rx_chspec_get(wlc, wrxh);
	rxframe_data->channel = hton16(chspec20mhz);
