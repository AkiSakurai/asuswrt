 ...
 *
 * $Id: wlc_obss.c 781624 2019-11-26 09:22:59Z $
 */
...
wlc_write_obss_intol_chanlist_ie(wlc_info_t *wlc, uint8* cp, uint8 len, uint8 *coex_map)
{
	...
	if ((wlc_japan(wlc) == TRUE) && coex_map[14] && len) {
		...
		chanlist_ie->regclass = wlc_get_regclass(wlc->cmi,
		                                         CH20MHZ_CHSPEC2(14, WLC_BAND_2G));
	...
	chanlist_ie->regclass = wlc_get_regclass(wlc->cmi, CH20MHZ_CHSPEC2(1, WLC_BAND_2G));
...
wlc_ht_ap_coex_scan_parse_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	/* TODO:6GHZ: how should chan (below) be interpreted ? */
	rxchspec = (data->pparm->ft->bcn.chan | WL_CHANSPEC_BW_20);
	if (data->pparm->ft->bcn.chan <= CH_MAX_2G_CHANNEL) {
		rxchspec |= WL_CHANSPEC_BAND_2G;
	} else {
		rxchspec |= WL_CHANSPEC_BAND_5G;
	}
