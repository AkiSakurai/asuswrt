 ...
 *
 * $Id: wlc_ltecx.h 690843 2017-03-18 00:11:03Z $
 */
...
typedef struct {
	//used to store prev watchdog ecounter value for comparison
	uint32	txframe_prev;
	uint32	txretrans_prev;
	uint32	rxframe_prev;
	uint32	rxrtry_prev;
	uint32	txnocts_prev;
	uint32	txrts_prev;
	uint32	txdeauth_prev;
	uint32	txassocreq_prev;
	uint32	txassocrsp_prev;
	uint32	txreassocreq_prev;
	uint32	txreassocrsp_prev;
} wlc_ltecoex_stats_stored_t;
...
struct wlc_ltecx_info {
	...
	bool            ocl_iovar_set;
	wlc_ltecoex_stats_t *	stats;
	wlc_ltecoex_stats_stored_t *ltecx_bins_prev;
    };
