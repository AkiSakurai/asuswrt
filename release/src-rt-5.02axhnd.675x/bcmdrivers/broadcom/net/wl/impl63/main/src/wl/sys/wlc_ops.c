...
static const bcm_iovar_t ops_iovars[] = {
	{"ops_status", IOV_OPS_STATUS, (0), IOVF2_RSDB_CORE_OVERRIDE,
...
/*
 * common module state variabled
 */
typedef struct wlc_ops_cmn_info {
	uint32 ops_cfg;
...
#define WL_OPS_INFO(args)   \
    EVENT_LOG_COMPACT_CAST_PAREN_ARGS(EVENT_LOG_TAG_OPS_INFO, args)
#else
...
BCMATTACHFN(wlc_ops_attach)(wlc_info_t *wlc)
{
	...
	/* Allocate common ops info if not already done so via obj registry */
	ops_info->cmn = (wlc_ops_cmn_info_t *)obj_registry_get(wlc->objr, OBJR_OPS_CMN_INFO);
	if (ops_info->cmn == NULL) {
		/* Allocate ops common data since not found */
		if (!(ops_info->cmn =
		    (wlc_ops_cmn_info_t *) MALLOCZ(wlc->osh, sizeof(wlc_ops_cmn_info_t)))) {
			WL_ERROR(("wl%d: %s: ops_cmn_info: MALLOCZ failed, size = %d\n",
			    WLCWLUNIT(wlc), __FUNCTION__, sizeof(wlc_ops_cmn_info_t)));
			goto fail;
		}
		/* init of ops_cfg to default */
		ops_info->cmn->ops_cfg = WL_OPS_DEFAULT_CFG;
		ops_info->cmn->ops_max_sleep_dur = WL_OPS_MAX_SLEEP_DUR;

		/* Update obj registry after allocations */
		obj_registry_set(wlc->objr, OBJR_OPS_CMN_INFO, ops_info->cmn);
	}
	(void)obj_registry_ref(wlc->objr, OBJR_OPS_CMN_INFO);
	...
	ops_info->slice_mask = ((1 << MAX_RSDB_MAC_NUM) - 1);
...
BCMATTACHFN(wlc_ops_detach)(wlc_ops_info_t *ops_info)
{
	...
	if (ops_info->cmn) {
		if (obj_registry_unref(wlc->objr, OBJR_OPS_CMN_INFO) == 0) {
			obj_registry_set(wlc->objr, OBJR_OPS_CMN_INFO, NULL);
			MFREE(wlc->osh, ops_info->cmn, sizeof(wlc_ops_cmn_info_t));
			ops_info->cmn = NULL;
		}
	}
            ...
            void *arg, uint len)
{
	...
	case WL_OPS_CFG_SUBCMD_ENABLE:
	{
		...
		if (len < ops_cfg_out->len)
			return BCME_BUFTOOSHORT;
		enable_cfg->bits = (ops_info->cmn->ops_cfg |
	...
	case WL_OPS_CFG_SUBCMD_MAX_SLEEP_DUR:
	{
		...
		if (len < ops_cfg_out->len)
			return BCME_BUFTOOSHORT;
		max_sleep->val = ops_info->cmn->ops_max_sleep_dur;
		break;
	}
	default:
		err = BCME_UNSUPPORTED;
...
ops_cfg_set(wlc_ops_info_t *ops_info, void *arg, uint len)
{
	...
	case WL_OPS_CFG_SUBCMD_ENABLE:
	{
		...
		if (ops_cfg->len >= cfg_hdr_len + sizeof(*enable_cfg)) {
			...
			ops_info->cmn->ops_cfg = enable_cfg->bits;
			bool_val = (enable_cfg->bits != 0) ? TRUE : FALSE;
			FOREACH_WLC(wlc->cmn, idx, wlc) {
				wlc_ops_update_disable_reqs(wlc->ops_info,
				                            OPS_DISABLED_HOST, !bool_val);
			}
		} else {
			err = BCME_BADLEN;
		}
	...
	case WL_OPS_CFG_SUBCMD_MAX_SLEEP_DUR:
	{
		...
		if (ops_cfg->len >= cfg_hdr_len + sizeof(*max_sleep)) {
			...
			wlc->ops_info->cmn->ops_max_sleep_dur = max_sleep->val;
			FOREACH_WLC(wlc->cmn, idx, wlc) {
				if ((wlc->clk && wlc->pub->hw_up)) {
					wlc_bmac_write_shm(wlc->hw, (M_OPS_MAX_LMT(wlc)),
					                   max_sleep->val);
				}
			}
		} else {
		...
		if (ops_cfg->len >= cfg_hdr_len + sizeof(*reset_stats)) {
			...
			FOREACH_WLC(wlc->cmn, idx, wlc) {
				if (reset_stats->val & (1 << wlc_get_wlcindex(wlc))) {
					wlc_ops_stats_snapshot(wlc);
					memset(wlc->ops_info->curr_stats, 0,
						sizeof(*ops_info->curr_stats));
				}
			}
		} else {
			err = BCME_BADLEN;
...
wlc_ops_handle_up_state(void *ctx)
{
	...
	WL_OPS_INFO(("wl%d: ops : up state apply ops_cfg 0x%x",
	            WLCWLUNIT(wlc),
	            ops_info->cmn->ops_cfg));

	wlc_bmac_write_shm(wlc->hw, (M_OPS_MAX_LMT(wlc)), ops_info->cmn->ops_max_sleep_dur);
...
wlc_ops_configure_shmem(wlc_ops_info_t *ops_info)
{
	...
	if (!(wlc->clk && wlc->pub->hw_up)) {
		WL_OPS_INFO(("ops_cfg_shmem: wl%d: noclk, skip applying ops_cfg 0x%x: "
		            "disable_res 0x%x \n", WLCWLUNIT(wlc),
		            ops_info->cmn->ops_cfg, ops_info->disable_reqs));
		return;
	}
	if (!ops_info->disable_reqs) {
		ops_info->applied_ops_config = ops_info->cmn->ops_cfg;
	} else {
	...
	WL_OPS_INFO(("wl%d, applied_cfg/cmn_ops_cfg 0x%x/0x%x disable_req 0x%x disable_obss %d\n",
	            ...
	            ops_info->cmn->ops_cfg, ops_info->disable_reqs, ops_info->disable_obss));
}
