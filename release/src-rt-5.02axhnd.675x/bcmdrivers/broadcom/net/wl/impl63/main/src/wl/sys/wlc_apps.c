	...
	*/
...
#ifdef WL_PS_STATS
	{
		...
		if (wlc->datablock_starttime != 0) {
			...
			wlc->datablock_avgtime += datablock_delta;
...
#ifdef WLTAF
	/* Update TAF state */
	wlc_taf_scb_state_update(wlc->taf_handle, scb, NULL, TAF_SCBSTATE_TWT_SP_EXIT);
#endif /* WLTAF */
...
#ifdef WLTAF
	/* Update TAF state */
	wlc_taf_scb_state_update(wlc->taf_handle, scb, NULL, TAF_SCBSTATE_TWT_SP_ENTER);
#endif /* WLTAF */
...
wlc_apps_print_scb_psinfo_txstuck(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "WLC discards:%d, ps_deferred:%d Supr discard:%d\n",
		wlc->psinfo->ps_discard,
		wlc->psinfo->ps_deferred,
		wlc->psinfo->supr_drop);
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		bcm_bprintf(b, "tx_block %d ext_qos_null %d psp_pending %d discards %d "
			"Supr drop %d\n",
			scb_psinfo->tx_block, scb_psinfo->ext_qos_null, scb_psinfo->psp_pending,
			scb_psinfo->ps_discard, scb_psinfo->supr_drop);
		...
		bss_info = APPS_BSSCFG_CUBBY(wlc->psinfo, bsscfg);
#ifdef WL_USE_SUPR_PSQ
		bcm_bprintf(b, "Pkt Q %p. Que len %d. Max %d. Avail %d. SuprQ Max %d. "
			"Seen %d. Disc %d\n",
			OSL_OBFUSCATE_BUF(pktq),
			pktq_n_pkts_tot(pktq), pktq_max(pktq), pktq_avail(pktq),
			spktq_max(&scb_psinfo->supr_psq[0]),
			WLCNTVAL(bss_info->bcmc_pkts_seen), WLCNTVAL(bss_info->bcmc_discards));
#else
...
wlc_apps_stats_dump(void *ctx, struct bcmstrbuf *b)
{
	...
	if (datablock_cnts > 0) {
		bcm_bprintf(b, "data_block_ps %d cnt %d dur avg:%u (min:%u max:%u) usec, "
			...
			wlc->datablock_avgtime/(uint32)datablock_cnts,
			wlc->datablock_mintime, wlc->datablock_maxtime,
			wlc->pktpend_avg/(uint32)datablock_cnts,
	...
	FOREACH_BSS(wlc, i, bsscfg) {
		...
		FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
			...
			if (ps_cnt > 0) {
				bcm_bprintf(b, " %2d.%3d "MACF"%s ps %d ST %02x ps_on %d "
					"dur avg:%u (min:%u max:%u) msec, supr %u",
					i, j, ETHER_TO_MACF(scb->ea), (scb->permanent ? "*":" "),
					scb->PS, scb->state, scb->ps_on_cnt,
					scb->ps_avgtime/(uint32)ps_cnt,
					scb->ps_mintime, scb->ps_maxtime, scb->suprps_cnt);
			} else {
				bcm_bprintf(b, " %2d.%3d "MACF"%s ps %d ST %02x ps_on 0 "
					"dur avg:0 (min:0 max:0) msec, supr %u",
					i, j, ETHER_TO_MACF(scb->ea), (scb->permanent ? "*":" "),
					scb->PS, scb->state, scb->suprps_cnt);
			}
