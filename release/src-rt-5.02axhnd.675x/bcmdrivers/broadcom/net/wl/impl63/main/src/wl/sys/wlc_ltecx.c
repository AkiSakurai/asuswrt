 ...
 *
 * $Id: wlc_ltecx.c 772932 2019-03-07 09:10:45Z $
 */
...
#include <ecounters.h>
static int
ltecx_ecounters_get_stats(uint16 stats_type, void *context,
...
static void wlc_ltecx_update_ecounter_bin(wlc_ltecx_info_t *ltecx, int noise_lte_delta);
static void wlc_ltecx_update_ecounters(wlc_info_t *wlc);
#endif /* ECOUNTERS */
...
static const bcm_iovar_t ltecx_iovars[] = {
	...
	(IOVF_RSDB_SET | IOVF_SET_DOWN), 0, IOVT_UINT8, 0
	},
	...
	(IOVF_RSDB_SET), 0, IOVT_BUFFER, sizeof(mws_ant_map_t)
	},
	...
	(IOVF_RSDB_SET), 0, IOVT_BUFFER, sizeof(wl_mws_ocl_override_t)
	},
	{NULL, 0, 0, 0, 0, 0}
};
...
#ifdef BCMULP
static int wlc_ulp_ltecx_enter_cb(void *handle, ulp_ext_info_t *einfo);
 ...
 * by wowl/ulp-ucode.
 */
static const ulp_p1_module_pubctx_t wlc_ulp_ltecx_p1_ctx = {
	MODCBFL_CTYPE_STATIC,
	NULL,
	NULL,
	NULL,
	NULL,
	wlc_ulp_ltecx_enter_cb
};
...
BCMATTACHFN(wlc_ltecx_attach)(wlc_info_t *wlc)
{
	...
	if ((ltecx = (wlc_ltecx_info_t *)
		...
		ltecx->cmn = obj_registry_get(wlc->objr, OBJR_LTECX_CMN_INFO);
		...
		if (ltecx->cmn == NULL) {
			...
			obj_registry_set(wlc->objr, OBJR_LTECX_CMN_INFO, ltecx->cmn);
		}
	}
	(void)obj_registry_ref(wlc->objr, OBJR_LTECX_CMN_INFO);
	...
	if ((ulp_p1_module_register(ULP_MODULE_ID_WLC_ULP_LTECX,
			&wlc_ulp_ltecx_p1_ctx, (void*)ltecx)) != BCME_OK) {
...
#ifdef ECOUNTERS
	/* LTECX ECOUNTERS only for AUX core */
	if (ECOUNTERS_ENAB() &&
		((RSDB_ENAB(wlc->pub) &&
		(wlc->pub->unit == MAC_CORE_UNIT_1)) ||
		!RSDB_ENAB(wlc->pub))) {
		if ((ltecx->stats = (wlc_ltecoex_stats_t *)
			MALLOCZ(wlc->osh, sizeof(wlc_ltecoex_stats_t))) == NULL) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			goto fail;
		}
		if ((ltecx->ltecx_bins_prev = (wlc_ltecoex_stats_stored_t *)
				MALLOCZ(wlc->osh, sizeof(wlc_ltecoex_stats_stored_t))) == NULL) {
				WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
				goto fail;
		}
		wl_ecounters_register_source(
			WL_IFSTATS_XTLV_WL_SLICE_LTECOEX, ltecx_ecounters_get_stats, (void*)wlc);
	} /* ECOUNTERS_ENAB */
#endif /* ECOUNTERS */
...
BCMATTACHFN(wlc_ltecx_detach)(wlc_ltecx_info_t *ltecx)
{
	...
	!RSDB_ENAB(wlc->pub))) {
		if (ltecx->stats) {
			MFREE(wlc->osh, ltecx->stats, sizeof(ltecx->stats));
		}
		if (ltecx->ltecx_bins_prev) {
			MFREE(wlc->osh, ltecx->ltecx_bins_prev, sizeof(ltecx->ltecx_bins_prev));
		}
	}
#endif /* ECOUNTERS */

	if (obj_registry_unref(ltecx->wlc->objr, OBJR_LTECX_CMN_INFO) == 0) {
		if (ltecx->cmn) {
        ...
        void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
#if WLRSDB
					/* Disable txpwrcap feature on other cores
					  * when wci2 loopback test is going on one PSM
					  */
					if (WLC_DUALMAC_RSDB(wlc->cmn)) {
						FOREACH_WLC(wlc_cmn, idx, other_wlc) {
							if (wlc != other_wlc) {
								wlc_ltecx_interface_disable(
										other_wlc->hw);
							}
						}
					}
...
#if WLRSDB
				/* loopback disabled, enable back txpwrcap */
				if (WLC_DUALMAC_RSDB(wlc->cmn)) {
					FOREACH_WLC(wlc_cmn, idx, other_wlc) {
						if (wlc != other_wlc) {
							wlc_ltecx_interface_active(other_wlc->hw);
						}
					}
				}
#endif // endif
	...
	case IOV_GVAL(IOV_LTECX_MWS_OCLMAP):
		...
		if (!BCMLTECOEX_ENAB(wlc->pub)) {
			...
			wlc_ltecx_update_scanreq_bm_channel(ltecx, WLC_BAND_PI_RADIO_CHANSPEC);
		}
		else
			err = BCME_BUFTOOSHORT;
		break;
...
wlc_ltecx_watchdog(void *arg)
{
	...
	if (BCMLTECOEX_ENAB(wlc->pub)) {
		...
		if (wlc->pub->up && wlc->clk) {
			...
			(wlc->pub->unit == MAC_CORE_UNIT_1)) ||
			!RSDB_ENAB(wlc->pub))) {
				wlc_ltecx_update_ecounter_bin(ltecx, noise_delta);
			}
		}
#endif // endif
...
wlc_lte_ocl_update(wlc_ltecx_info_t *ltecx, int cellstatus)
{
	...
	if ((cellstatus & LTECX_CELLSTATUS_UKNOWN) || (!(cellstatus & LTECX_CELLSTATUS_ON))) {
		...
		if (BSSCFG_INFRA_STA(ltecx->wlc->cfg)) {
			if (ltecx->wlc->cfg->associated) {
				disable_current = wlc_ltecx_process_oclmap(ltecx,
					ltecx->wlc->cfg->current_bss);
 ...
 * wlc_watchdog() will then update ltecx stats.
 */
static int
ltecx_ecounters_get_stats(uint16 stats_type, void *context,
	const ecounters_stats_types_report_req_t *req,
	...
	uint16 *attempted_write_len)
{
	...
	/* Should not happen */
	if (stats_type != WL_IFSTATS_XTLV_WL_SLICE_LTECOEX) {
		return BCME_ERROR;
	}
	/* Get LTE Coex specific ecounters if core is Up */
	if (!wlc->pub->up || !wlc->clk) {
		return BCME_NOTUP;
	}
	wlc_ltecx_update_ecounters(wlc);
	wlc->ltecx->stats->version = LTECOEX_STATS_VER;
	wlc->ltecx->stats->slice_index = RSDB_ENAB(wlc->pub) ? wlc->pub->unit : 0;
	write_status = bcm_xtlv_put_data(xtlvbuf,
		stats_type,
		(uint8 *)wlc->ltecx->stats, sizeof(wlc_ltecoex_stats_t));

	/* If there is enough size. */
	if (write_status == BCME_NOMEM) {
		*attempted_write_len = BCM_XTLV_HDR_SIZE + sizeof(wlc_ltecoex_stats_t);
...
static void
wlc_ltecx_update_ecounter_bin(wlc_ltecx_info_t *ltecx, int noise_lte_delta)
{
	/* Diff based cumulative counter */
	uint32 txframe_diff =
		ltecx->wlc->pub->_cnt->txframe >= ltecx->ltecx_bins_prev->txframe_prev ?
		(ltecx->wlc->pub->_cnt->txframe - ltecx->ltecx_bins_prev->txframe_prev):
		~(ltecx->ltecx_bins_prev->txframe_prev - ltecx->wlc->pub->_cnt->txframe);
	ltecx->ltecx_bins_prev->txframe_prev = ltecx->wlc->pub->_cnt->txframe;
	uint32 txretrans_diff =
		ltecx->wlc->pub->_cnt->txretrans >= ltecx->ltecx_bins_prev->txretrans_prev ?
		(ltecx->wlc->pub->_cnt->txretrans - ltecx->ltecx_bins_prev->txretrans_prev):
		~(ltecx->ltecx_bins_prev->txretrans_prev - ltecx->wlc->pub->_cnt->txretrans);
	ltecx->ltecx_bins_prev->txretrans_prev = ltecx->wlc->pub->_cnt->txretrans;
	uint32 rxframe_diff =
		ltecx->wlc->pub->_cnt->rxframe >= ltecx->ltecx_bins_prev->rxframe_prev ?
		(ltecx->wlc->pub->_cnt->rxframe - ltecx->ltecx_bins_prev->rxframe_prev):
		~(ltecx->ltecx_bins_prev->rxframe_prev-ltecx->wlc->pub->_cnt->rxframe);
	ltecx->ltecx_bins_prev->rxframe_prev = ltecx->wlc->pub->_cnt->rxframe;
	uint32 rxrtry_diff =
		ltecx->wlc->pub->_cnt->rxrtry >= ltecx->ltecx_bins_prev->rxrtry_prev ?
		(ltecx->wlc->pub->_cnt->rxrtry - ltecx->ltecx_bins_prev->rxrtry_prev):
		~(ltecx->ltecx_bins_prev->rxrtry_prev - ltecx->wlc->pub->_cnt->rxrtry);
	ltecx->ltecx_bins_prev->rxrtry_prev = ltecx->wlc->pub->_cnt->rxrtry;
	uint32 txnocts_diff =
		ltecx->wlc->pub->_cnt->txnocts >= ltecx->ltecx_bins_prev->txnocts_prev ?
		(ltecx->wlc->pub->_cnt->txnocts - ltecx->ltecx_bins_prev->txnocts_prev):
		~(ltecx->ltecx_bins_prev->txnocts_prev - ltecx->wlc->pub->_cnt->txnocts);
	ltecx->ltecx_bins_prev->txnocts_prev = ltecx->wlc->pub->_cnt->txnocts;
	uint32 txrts_diff =
		ltecx->wlc->pub->_cnt->txrts >= ltecx->ltecx_bins_prev->txrts_prev ?
		(ltecx->wlc->pub->_cnt->txrts - ltecx->ltecx_bins_prev->txrts_prev):
		~(ltecx->ltecx_bins_prev->txrts_prev - ltecx->wlc->pub->_cnt->txrts);
	ltecx->ltecx_bins_prev->txrts_prev = ltecx->wlc->pub->_cnt->txrts;
	uint32 txdeauth_diff =
		ltecx->wlc->pub->_cnt->txdeauth >= ltecx->ltecx_bins_prev->txdeauth_prev ?
		(ltecx->wlc->pub->_cnt->txdeauth - ltecx->ltecx_bins_prev->txdeauth_prev):
		~(ltecx->ltecx_bins_prev->txdeauth_prev - ltecx->wlc->pub->_cnt->txdeauth);
	ltecx->ltecx_bins_prev->txdeauth_prev = ltecx->wlc->pub->_cnt->txdeauth;
	uint32 txassocreq_diff =
		ltecx->wlc->pub->_cnt->txassocreq >= ltecx->ltecx_bins_prev->txassocreq_prev ?
		(ltecx->wlc->pub->_cnt->txassocreq - ltecx->ltecx_bins_prev->txassocreq_prev):
		~(ltecx->ltecx_bins_prev->txassocreq_prev - ltecx->wlc->pub->_cnt->txassocreq);
	ltecx->ltecx_bins_prev->txassocreq_prev = ltecx->wlc->pub->_cnt->txassocreq;
	uint32 txassocrsp_diff =
		ltecx->wlc->pub->_cnt->txassocrsp >= ltecx->ltecx_bins_prev->txassocrsp_prev ?
		(ltecx->wlc->pub->_cnt->txassocrsp - ltecx->ltecx_bins_prev->txassocrsp_prev):
		~(ltecx->ltecx_bins_prev->txassocrsp_prev - ltecx->wlc->pub->_cnt->txassocrsp);
	ltecx->ltecx_bins_prev->txassocrsp_prev = ltecx->wlc->pub->_cnt->txassocrsp;
	uint32 txreassocreq_diff =
		ltecx->wlc->pub->_cnt->txreassocreq >= ltecx->ltecx_bins_prev->txreassocreq_prev ?
		(ltecx->wlc->pub->_cnt->txreassocreq - ltecx->ltecx_bins_prev->txreassocreq_prev):
		~(ltecx->ltecx_bins_prev->txreassocreq_prev - ltecx->wlc->pub->_cnt->txreassocreq);
	ltecx->ltecx_bins_prev->txreassocreq_prev = ltecx->wlc->pub->_cnt->txreassocreq;
	uint32 txreassocrsp_diff =
		ltecx->wlc->pub->_cnt->txreassocrsp >= ltecx->ltecx_bins_prev->txreassocrsp_prev ?
		(ltecx->wlc->pub->_cnt->txreassocrsp - ltecx->ltecx_bins_prev->txreassocrsp_prev):
		~(ltecx->ltecx_bins_prev->txreassocrsp_prev - ltecx->wlc->pub->_cnt->txreassocrsp);
	ltecx->ltecx_bins_prev->txreassocrsp_prev = ltecx->wlc->pub->_cnt->txreassocrsp;

	if (noise_lte_delta <= 0) {
		ltecx->stats->txframe_no_LTE += txframe_diff;
		ltecx->stats->txretrans_no_LTE += txretrans_diff;
		ltecx->stats->rxframe_no_LTE += rxframe_diff;
		ltecx->stats->rxrtry_no_LTE += rxrtry_diff;
		ltecx->stats->txnocts_no_LTE += txnocts_diff;
		ltecx->stats->txrts_no_LTE += txrts_diff;
		ltecx->stats->txdeauth_no_LTE += txdeauth_diff;
		ltecx->stats->txassocreq_no_LTE += txassocreq_diff;
		ltecx->stats->txassocrsp_no_LTE += txassocrsp_diff;
		ltecx->stats->txreassocreq_no_LTE += txreassocreq_diff;
		ltecx->stats->txreassocrsp_no_LTE += txreassocrsp_diff;
	} else if (noise_lte_delta < 10) {
		ltecx->stats->txframe_light_LTE += txframe_diff;
		ltecx->stats->txretrans_light_LTE += txretrans_diff;
		ltecx->stats->rxframe_light_LTE += rxframe_diff;
		ltecx->stats->rxrtry_light_LTE += rxrtry_diff;
		ltecx->stats->txnocts_light_LTE += txnocts_diff;
		ltecx->stats->txrts_light_LTE += txrts_diff;
		ltecx->stats->txdeauth_light_LTE += txdeauth_diff;
		ltecx->stats->txassocreq_light_LTE += txassocreq_diff;
		ltecx->stats->txassocrsp_light_LTE += txassocrsp_diff;
		ltecx->stats->txreassocreq_light_LTE += txreassocreq_diff;
		ltecx->stats->txreassocrsp_light_LTE += txreassocrsp_diff;
	} else {
		ltecx->stats->txframe_heavy_LTE += txframe_diff;
		ltecx->stats->txretrans_heavy_LTE += txretrans_diff;
		ltecx->stats->rxframe_heavy_LTE += rxframe_diff;
		ltecx->stats->rxrtry_heavy_LTE += rxrtry_diff;
		ltecx->stats->txnocts_heavy_LTE += txnocts_diff;
		ltecx->stats->txrts_heavy_LTE += txrts_diff;
		ltecx->stats->txdeauth_heavy_LTE += txdeauth_diff;
		ltecx->stats->txassocreq_heavy_LTE += txassocreq_diff;
		ltecx->stats->txassocrsp_heavy_LTE += txassocrsp_diff;
		ltecx->stats->txreassocreq_heavy_LTE += txreassocreq_diff;
		ltecx->stats->txreassocrsp_heavy_LTE += txreassocrsp_diff;
	}
}

static void
wlc_ltecx_update_ecounters(wlc_info_t *wlc)
{
	wlc_hw_info_t *wlc_hw = wlc->hw;
	wlc_ltecx_info_t *ltecx;
	ltecx = wlc->ltecx;
	...
	if (ltecx->ltecx_shm_addr) {
		ltecx->stats->type4_txinhi_dur = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TYPE4_TXINHIBIT_DURATION(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TYPE4_TXINHIBIT_DURATION(wlc_hw), 0);
		ltecx->stats->type4_nonzero_cnt = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TYPE4_NONE_ZERO_CNT(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TYPE4_NONE_ZERO_CNT(wlc_hw), 0);
		ltecx->stats->type4_timeout_cnt = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TYPE4_TIMEOUT_CNT(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TYPE4_TIMEOUT_CNT(wlc_hw), 0);
		ltecx->stats->rx_pri_dur = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_RXPRI_DURATION(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_RXPRI_DURATION(wlc_hw), 0);
		ltecx->stats->rx_pri_cnt = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_RXPRI_CNT(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_RXPRI_CNT(wlc_hw), 0);
		ltecx->stats->type6_dur = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TYP6_DURATION(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TYP6_DURATION(wlc_hw), 0);
		ltecx->stats->type6_cnt = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TYP6_CNT(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TYP6_CNT(wlc_hw), 0);
		ltecx->stats->ts_prot_frm_cnt = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TS_PROT_FRAME_CNT(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TS_PROT_FRAME_CNT(wlc_hw), 0);
		ltecx->stats->ts_gr_cnt = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TS_GRANT_CNT(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TS_GRANT_CNT(wlc_hw), 0);
		ltecx->stats->ts_gr_dur = wlc_bmac_read_shm(wlc_hw,
			M_LTECX_TS_GRANT_TIME_DUR(wlc_hw));
		wlc_bmac_write_shm(wlc_hw,
			M_LTECX_TS_GRANT_TIME_DUR(wlc_hw), 0);
	}
}
#endif /* ECOUNTERS */
...
static int
wlc_ulp_ltecx_enter_cb(void *handle, ulp_ext_info_t *einfo)
{
	wlc_ltecx_info_t *ltecx = (wlc_ltecx_info_t *)handle;
	wlc_info_t *wlc = ltecx->wlc;
	bool ltecx_en = 0;
	...
	/* Check if ltecx needs to be enabled */
	if (CHSPEC_IS2G(wlc->chanspec)) {
		ltecx_en = 1;
	}

	/* Update ucode ltecx flags for DS1 state */
	wlc_ulp_configure_ulp_features(wlc->ulp,
		(C_LTE_COEX | C_LTECX_PSPOLL_PRIO_EN), (ltecx_en)? TRUE: FALSE);
	ltecx->ltecx_enabled = ltecx_en;
	return BCME_OK;
}
#endif /* BCMULP */
