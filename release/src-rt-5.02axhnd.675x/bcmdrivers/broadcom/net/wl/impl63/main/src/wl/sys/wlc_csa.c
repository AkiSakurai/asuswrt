	...
	*/
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, csa->csa.chspec)) {
		...
		csa->csa.chspec = wlc_next_chanspec(
					wlc->cmi,
					CH20MHZ_CHSPEC2(CH_MAX_2G_CHANNEL + 1, WLC_BAND_2G),
					CHAN_TYPE_CHATTY, 0);
...
wlc_csa_calc_mcst_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (WL11H_ENAB(wlc) && BSSCFG_AP(cfg) && wlc_csa_get_csa_count(csam, cfg) > 0) {
		...
		uint8 *radarch_len, uint8 **radarch)
{
	...
	for (i = 0; i < *radarch_len; i++) {
		...
		chan_info.chanspec = (WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(ch) | ch);
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	/* TODO:6GHZ: add capability of handling 6GHz channels advertised in CSA IE */
	if (extch == DOT11_EXT_CH_NONE) {
		chspec = (WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(csa_ie->channel) | csa_ie->channel);
	} else {
