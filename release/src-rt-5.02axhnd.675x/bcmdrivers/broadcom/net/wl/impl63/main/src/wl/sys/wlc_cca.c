 ...
 *
 * $Id: wlc_cca.c 780846 2019-11-05 04:23:36Z $
 */
...
static const bcm_iovar_t wlc_cca_iovars[] = {
	...
	(IOVF_RSDB_SET), 0, IOVT_BUFFER, sizeof(wl_chan_qual_event_t)
	},
#ifdef WL_CCA_STATS_MESH
        ...
        void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_CCA_STATS_MESH):
		if ((p_len < sizeof(cca_congest_channel_req_t)) ||
			...
			/* Feature is not available on correv prior to 42, 56, 65,
			 * and revisions greater than 128 in KUDU
			 */
			if (D11REV_IS(wlc->pub->corerev, 42) ||
					D11REV_IS(wlc->pub->corerev, 56) ||
	...
	wlc_congest_channel_req_t *stats_results, int buflen)
{
	...
	if (RSDB_ENAB(wlc->pub)) {
		...
		if (other_wlc) {
			other_cca = other_wlc->cca_info;
			other_second = other_cca->cca_second;
		}
	}
#endif /* WLRSDB */
...
#ifdef WLRSDB
		/* Accumulate with CCA statistics from the other radio */
		if (RSDB_ENAB(wlc->pub) && other_cca) {
			other_second = MODDEC(other_second, other_cca->cca_second_max);
			congest = CCA_POOL_DATA(other_cca, ch_idx, other_second);
...
cca_stats_mesh(cca_info_t *cca, void *input)
{
	 ...
	 * and revisions greater than 128 in KUDU
	 */
	if (!(D11REV_IS(wlc->pub->corerev, 42) || D11REV_IS(wlc->pub->corerev, 56) ||
