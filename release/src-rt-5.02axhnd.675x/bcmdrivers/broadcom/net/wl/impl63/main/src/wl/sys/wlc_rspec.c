 ...
 *
 * $Id: wlc_rspec.c 779772 2019-10-07 12:54:13Z $
 */
...
static int
actionid2bandtype(uint32 actionid)
{
	int band_type;
	...
	void *params, uint p_len, void *arg, uint a_len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_6G_RATE): {
		ratespec_t wlrspec;
		ratespec_t ratespec;
		enum wlc_bandunit band_unit = actionid2bandunit(actionid);

		ratespec = wlc->bandstate[band_unit]->rspec_override;
	...
	case IOV_SVAL(IOV_6G_RATE):
	{
		int band_type = actionid2bandtype(actionid);

		err = wlc_set_iovar_ratespec_override(wlc, bsscfg, band_type,
	...
	case IOV_GVAL(IOV_6G_MRATE):
	{
		ratespec_t wlrspec;
		enum wlc_bandunit band_unit = actionid2bandunit(actionid);
		ratespec_t ratespec = wlc->bandstate[band_unit]->mrspec_override;
...
wlc_set_ratespec_override(wlc_info_t *wlc, int band_id, ratespec_t rspec, bool mcast)
{
	...
	Nsts = 1;
	band = wlc->bandstate[wlc_bandtype2bandunit(band_id)];
