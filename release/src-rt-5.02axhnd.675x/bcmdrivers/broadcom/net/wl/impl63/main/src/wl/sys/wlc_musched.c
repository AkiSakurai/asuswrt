...
#define MUSCHED_DLOFDMA_MAX_NSS			3	/* exclude 4x4 and up */
...
struct wlc_muscheduler_info {
	...
	bool   dsmps_war;	/* = TRUE dynamic SMPS enabled STAs excluded from DL-OFDMA */
...
wlc_musched_dump_policy(wlc_muscheduler_info_t *musched, bcmstrbuf_t *b)
{
	...
	bcm_bprintf(b, "           RU alloc mode: %s (%d), mu-rts: %d, "
		...
		musched->rualloc, musched->use_murts, musched->mix_ackp,
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (musched_scb && musched_scb->scb_ru_stats) {
			...
			bcm_bprintf(b, ""MACF" (aid 0x%x) is_dlofdma: %d\n",
				ETHER_TO_MACF(scb->ea), scb->aid, SCB_DLOFDMA(scb));
...
{
	wlc_info_t *wlc = musched->wlc;
	if (D11REV_GE(wlc->pub->corerev, 129) && D11REV_LT(wlc->pub->corerev, 131)) {
