 ...
 * XTLV callback function. This could change when ecounters is implemented
 */
typedef struct {
	wlc_info_t *wlc;
	struct wlc_if *wlcif;
...
/* Note that this function is called from ecounters as well as from iovar
 * handling code.
 */
int
	...
	bcm_xtlvbuf_t *xtlv_context, void *param, uint p_len)
{
	 ...
	 * If coming from ecounters, there is no such container so
	 * simply proceed
	 */
