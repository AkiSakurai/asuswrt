...
#ifdef PHYCAL_CACHING
	phy_cache_register_cb(wlc->pi, wlc_chanctxt_chk_calcache_cb, (void *)wlc);
	...
	if (bsscfg != NULL) {
		...
		if (cbi->on_channel) {
			WL_CHANCTXT_WARN(("[WARNING]wl%d.%d: wlc_txqueue_start: "
				...
				wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), chanctxt->count,
	...
	if (bsscfg != NULL) {
		...
		if (!cbi->on_channel || !chanctxt) {
			WL_CHANCTXT_WARN(("[WARNING]wl%d.%d: wlc_txqueue_end: "
				...
				WLC_BSSCFG_IDX(bsscfg), chanctxt? chanctxt->count : -100,
				chanctxt_info->excursion_count));
			return NULL;
		}
...
wlc_txqueue_end(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_chanctxt_backup_fn_t backup_fn)
{
	...
	WL_CHANCTXT_TRACE(("[ENTRY]wl%d.%d: wlc_txqueue_end: call by %p\n",
	...
	if (backup_fn && cfg != NULL && !pktq_empty(WLC_GET_CQ(qi))) {
...
#ifdef WLRSDB
	wlc_chanctxt_chk_calcache(wlc_other);
#endif /* WLRSDB */
...
wlc_chanctxt_chk_calcache(wlc_info_t *wlc)
{
	...
	for (idx = 0; idx < PREALLOCATE_CHANCTXT_TOTAL; idx++) {
		if (chanlist[idx] &&
			...
			phy_chanmgr_destroy_ctx((phy_info_t *)WLC_PI(wlc), chanlist[idx]);
	}
}
