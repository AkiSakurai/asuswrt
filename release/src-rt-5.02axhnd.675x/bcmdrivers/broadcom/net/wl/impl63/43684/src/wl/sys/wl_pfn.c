 ...
 *
 * $Id: wl_pfn.c 779930 2019-10-10 08:07:06Z $
 *
 */
...
static bool is_channel_in_list(chanspec_t *chanspec_list,
                     uint16 channel, uint32 n_channels);
...
/* TODO:6GHZ: PFN IOCTL wl_pfn_cfg_t needs to be 6g capable, after which this function needs to be
 * extended with 6ghz.
 */
static int
	...
	for (i = 0; i < gscan_cfg->count_of_channel_buckets; i++) {
		...
		if ((gscan_cfg->cur_ch_bucket_active & (1 << i))) {
			if (bssid_channel == UNSPECIFIED_CHANNEL) {
				...
				n_channels = channel_bucket->bucket_end_index -
				               start_idx + 1;
				if (is_channel_in_list(&chan_orig_list[start_idx],
				   bssid_channel, n_channels)) {
...
static bool
is_channel_in_list(chanspec_t *chanspec_list, uint16 channel, uint32 n_channels)
{
	uint32 i;
	bool ret = FALSE;
