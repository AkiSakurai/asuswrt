 ...
 *
 * $Id: rte_isr.c 779823 2019-10-08 11:09:11Z $
 */
...
/**
 * Return the currently executing DPC.
 *
 * @return		Pointer to current DPC, or NULL if not in DPC context.
 */

hnd_dpc_t*
hnd_dpc_get_current(void)
{
	if (hnd_in_isr() == FALSE) {
		osl_ext_task_t *thread = osl_ext_task_current();
		ASSERT(thread != NULL);

		return thread->current_dpc;
	}
...
/**
 * Get statistics context of the current DPC.
 *
 * @return			Statistics context, or NULL if not in DPC context.
 */

hnd_stats_t*
hnd_isr_stats_for_dpc(void)
{
	hnd_dpc_t *dpc = hnd_dpc_get_current();
	return dpc != NULL ? dpc->stats : NULL;
}
		...
		for (stats = hnd_isr->stats_list; stats; stats = stats->next) {
			if ((flags & INTSTATS_DUMP_ALL) || stats->count != 0) {
				if (stats->type == OBJ_TYPE_USR) {
					hnd_isr_dump_stats_single(stats, 0, flags,
