 ...
 *
 * $Id: wl_p2po.c 778871 2019-09-12 09:05:12Z $
 *
 */
#include <wlc_cfg.h>
#include <typedefs.h>
	...
	uint32 event_type, wl_event_msg_t *wl_event_msg, uint8 *data, uint32 length)
{
	...
	if (event_type == WLC_E_ESCAN_RESULT && wl_event_msg->status == WLC_E_STATUS_PARTIAL) {
		wl_escan_result_t *escan_data = (wl_escan_result_t *)data;

		if (length >= sizeof(*escan_data)) {
			wlc_info_t *wlc = p2po->wlc;
			wl_bss_info_t *bi = (wl_bss_info_t *)&escan_data->bss_info;
			...
			instance->gas = bcm_gas_create((struct bcm_gas_wl_drv_hdl *)wlc,
				bsscfgIndex, NULL, channel, addr, FALSE);
			if (instance->gas == 0) {
...
exit1:
	...
	bcm_p2p_disc_process_wlan_event(0, event_type, wl_event_msg, data, length);
}
#endif /* P2POELO */
/*
