 ...
 *
 * $Id: wlc_sta.c 779645 2019-10-03 16:27:57Z $
 */
		...
		case CFG_CLONE_END: {
			if (cfg->ibss_up_pending) {
				...
				if (!to_wlc->bandlocked) {
					...
					FOREACH_WLC_BAND(from_wlc, bandunit) {
						if (CHSPEC_BANDUNIT(bi->chanspec) == bandunit) {
							continue; // skip current band
						}
						if (wlc_rsdb_change_band_allowed(from_wlc,
							bandunit)) {
							wlc_change_band(from_wlc, bandunit);
						}
					}
				}
				...
				if (bi) {
					...
					if (!to_wlc->bandlocked) {
						enum wlc_bandunit bandunit;
						FOREACH_WLC_BAND(from_wlc, bandunit) {
							if (CHSPEC_BANDUNIT(bi->chanspec) ==
							    bandunit) {
								continue; // skip current band
							}
							if (wlc_rsdb_change_band_allowed(from_wlc,
