...
#ifdef BCMDBG
#define FTM_DEBUG_OPT(_ftm, _opt) ((_ftm->ftm_cmn)->config->debug & (_opt))
#ifdef EVENT_LOG_COMPILE
#define FTM_LOG_OPT(_ftm, _opt, _args) do { \
	if (FTM_DEBUG_OPT(_ftm, WL_PROXD_DEBUG_EVENTLOG)) { \
		if (FTM_DEBUG_OPT(_ftm, _opt) && EVENT_LOG_IS_LOG_ON(EVENT_LOG_TAG_PROXD_TRACE)) \
			EVENT_LOG_COMPACT_CAST_PAREN_ARGS(EVENT_LOG_TAG_PROXD_TRACE, \
			                                  EVENT_LOG_REMOVE_PAREN(_args)); \
	else \
		if (FTM_DEBUG_OPT(_ftm, _opt)) WL_APSTA _args; \
	} \
} while (0)
...
#define FTM_LOGNAN(_ftm, _args) FTM_LOG_OPT(_ftm, WL_PROXD_DEBUG_NAN, _args)
