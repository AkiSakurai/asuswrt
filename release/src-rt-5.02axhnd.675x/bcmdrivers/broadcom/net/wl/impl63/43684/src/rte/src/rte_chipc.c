 ...
 *
 * $Id: rte_chipc.c 771484 2019-01-29 09:54:08Z $
 */
 ...
 * @param dpc_fn	DPC callback function, or NULL.
 * @param intmask	Interrupt mask.
 * @param cbdata	Context to be passed to ISR and DPC functions.
 * @param id		Secondary handler id.
 * @return		TRUE on success.
 */

bool
BCMATTACHFN(si_cc_register_isr_internal)(si_t *sih, cc_isr_fn isr_fn, cc_dpc_fn dpc_fn,
	uint32 intmask, void *cbdata, const char *isr_id, const char *dpc_id)
{
	chipcregs_t *regs = hnd_ccr;

	/* Register secondary ISR and/or DPC function */
	if (hnd_sec_isr_register_internal(cc_isr, intmask, (hnd_sec_isr_fn_t)isr_fn,
		(hnd_sec_dpc_fn_t)dpc_fn, cbdata, isr_id, dpc_id) == FALSE) {
		return FALSE;
	}
...
hnd_chipc_isr(void *cbdata)
{
	...
	/* Run secondary ISRs */
	hnd_sec_isr_run(cc_isr, cc_intstatus);
}
...
hnd_chipc_dpc(void *cbdata)
{
	...
	/* Run secondary DPC functions */
	hnd_sec_dpc_run(cc_isr, cc_intstatus);
...
#else
	cc_isr = hnd_isr_register(0, CC_CORE_ID, 0,
	    hnd_chipc_isr, NULL, hnd_chipc_dpc, NULL, NULL, SI_BUS);
	if (cc_isr == NULL) {
		HND_DIE();
	}
#endif	/* !RTE_POLL */
...
static void
rte_gci_dpc(void* cbdata, uint32 ccintst)
{
...
BCMATTACHFN(rte_gci_init)(si_t *sih)
{
	...
	si_cc_register_isr(sih, NULL, rte_gci_dpc, CI_ECI, (void *)sih);
}
