 ...
 *
 *   $Id: wlc_pm.c 778492 2019-09-03 22:21:48Z $
 */
...
static const bcm_iovar_t pm_iovars[] = {
	...
	{"pm2_radio_shutoff_dly", IOV_PM2_RADIO_SHUTOFF_DLY, IOVF_RSDB_SET, 0, IOVT_UINT16, 0},
#ifdef WL_EXCESS_PMWAKE
...
wlc_pm_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	...
	if (BSSCFG_STA(cfg)) {
		const wlc_pm_copy_t *cp = (const wlc_pm_copy_t *)data;
		cfg->pm->PM = cp->PM;
