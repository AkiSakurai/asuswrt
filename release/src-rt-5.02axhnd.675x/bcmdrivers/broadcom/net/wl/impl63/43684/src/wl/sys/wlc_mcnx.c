 ...
 *
 * $Id: wlc_mcnx.c 776422 2019-06-27 22:27:11Z $
 */
...
#define WL_MCNX_TS(wlc)		(wlc->clk ? R_REG(wlc->osh, D11_TSFTimerLow(wlc)) : 0xDEADDAED)
#endif // endif
...
typedef struct {
	...
	int		rcmta_ra_idx;		/**< RCMTA idx for RA to be used for P2P */
	int		rcmta_bssid_idx;	/**< RCMTA idx for BSSID to be used for P2P */
	...
	int		rcmta_alt_ra_idx;       /** RSDB, RA index which is used by other wlc. */
...
#define INVALID_RCMTA_IDX (-2) /* Default value of rcmta_alt_ra_idx.
				       * used only for corerev > 40
				       */
...
int
wlc_mcnx_rcmta_alloc(wlc_mcnx_info_t *mcnx, int *idx)
{
	int i;
	...
	uint32 ltsf_h, uint32 ltsf_l, uint32 *ltbtt_h, uint32 *ltbtt_l)
{
	...
	if (bmi->unaligned_tbtt_tsfo > 0) {
		...
		/* offset to last tbtt */
		offset = wlc_calc_tbtt_offset(bcnint, rtbtt_h, rtbtt_l);
		/* last tbtt */
		...
		if (bmi->unaligned_tbtt_tsfo < offset) {
			wlc_uint64_add(&rtbtt_h, &rtbtt_l, 0, bcnint<<10);
		}
	} else {
	...
	uint32 ltsf_h, uint32 ltsf_l, uint32 *ltbtt_h, uint32 *ltbtt_l)
{
	...
	/* immediate next tbtt */
	wlc_mcnx_l2r_tsf64(mcnx, cfg, ltsf_h, ltsf_l, &rtbtt_h, &rtbtt_l);
	offset = wlc_calc_tbtt_offset(bmi->bcn_prd, rtbtt_h, rtbtt_l);
	wlc_tsf64_to_next_tbtt64(bmi->bcn_prd, &rtbtt_h, &rtbtt_l);
	/* last tbtt */
	wlc_uint64_sub(&rtbtt_h, &rtbtt_l, 0, bmi->bcn_prd << 10);

	if (bmi->unaligned_tbtt_tsfo > 0) {
		/* sub bcnint from last tbtt if not passed unaligned_tbtt_tsfo */
		if (offset < bmi->unaligned_tbtt_tsfo) {
			wlc_uint64_sub(&rtbtt_h, &rtbtt_l, 0,
				bmi->bcn_prd << 10);
		}
		wlc_uint64_add(&rtbtt_h, &rtbtt_l, 0,
			bmi->unaligned_tbtt_tsfo);
	}
	wlc_mcnx_r2l_tsf64(mcnx, cfg, rtbtt_h, rtbtt_l, ltbtt_h, ltbtt_l);
}
                         ...
                         wlc_d11rxhdr_t *wrxh, uint8 *plcp, wlc_bsscfg_t *cfg)
{
	...
	if (((bcn_offset >> 10) > MAX_P2P_BCN_OFFSET_MS)) {
		bool short_preamble = ((D11PHYSTSBUF_ACCESS_VAL(&wrxh->rxhdr,
			wlc->pub->corerev, PhyRxStatus_0) & PRXS_SHORTH(wlc->pub->corerev)) != 0);
...
/* enable BSS PowerSave via M_P2P_BSS_ST word */
/* XXX ucode make per-BSS PowerSave feature depending on BSS type being P2P GO.
 * To workaround this limitation, M_P2P_BSS_ST_GO is added via this API
 */
static void
_wlc_mcnx_ap_ps_enab(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab)
{
	wlc_info_t *wlc = mcnx->wlc;
	uint16 state;
	int bss;
...
void
wlc_mcnx_ap_ps_enab(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab)
{
	wlc_info_t *wlc = mcnx->wlc;
	...
	wlc_mcnx_mac_suspend(mcnx);
	_wlc_mcnx_ap_ps_enab(mcnx, cfg, enab);
	wlc_mcnx_mac_resume(mcnx);
...
uint32
wlc_mcnx_unaligned_tbtt_get(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	bss_mcnx_info_t *bmi;
	...
	return bmi->unaligned_tbtt_tsfo;
}
...
wlc_mcnx_info_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	*pbmi = bmi;
	...
	bmi->rcmta_ra_idx = wlc->pub->max_addrma_idx;
	bmi->rcmta_bssid_idx = wlc->pub->max_addrma_idx;
	bmi->bcn_prd = cfg->current_bss->beacon_period;
...
wlc_mcnx_bssid_set(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	if (D11REV_GE(mcnx->wlc->pub->corerev, 40)) {
		mask = NBITVAL(C_ADDR_BSSID_NBIT);
		val = NBITVAL(C_ADDR_BSSID_NBIT);
	} else {
		mask = ADDR_BMP_BSSID;
		val = ADDR_BMP_BSSID;
	}
...
wlc_mcnx_alt_ra_set(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	...
	if (bmi->rcmta_alt_ra_idx == INVALID_RCMTA_IDX) {
...
static void write_dts_shm(wlc_bsscfg_t *cfg, uint32 dts)
{
	...
	wlc_mcnx_write_shm(mcnx,  M_P2P_TX_STOP_TS(cfg->wlc, bss, 1), (uint16)(dts >> 16));
	wlc_mcnx_write_shm(mcnx,  M_P2P_TX_STOP_TS(cfg->wlc, bss, 0), (uint16)(dts));

	// readback
	DTS_INFO(("\t\t wrote dts_shm(bss_idx:%d):%08x, readback:%04x:%04x\n\n",
		wlc_mcnx_BSS_idx(mcnx, cfg), dts,
		wlc_mcnx_read_shm(mcnx, M_P2P_TX_STOP_TS(cfg->wlc, bss, 1)),
		wlc_mcnx_read_shm(mcnx, M_P2P_TX_STOP_TS(cfg->wlc, bss, 0))));
}
...
/*	--- smart ampdu TX dts (Dedline Time Stamp) utility function ---
	...
	for Fix-Fix timeslots (NAN) -from SLOT_START type callback
