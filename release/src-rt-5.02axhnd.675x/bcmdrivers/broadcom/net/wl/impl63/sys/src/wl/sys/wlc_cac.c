 ...
 *
 * $Id: wlc_cac.c 778769 2019-09-10 09:43:50Z $
 */
...
wlc_cac_cubby_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	...
	/* to make sure source ptr is not freed in deinit */
	bss_info->cp_scb_cubby = NULL;
...
wlc_cac_cubby_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	wlc_cac_t *cac = (wlc_cac_t *)ctx;
	cac_bss_info_t *bss_info = NULL;
	struct wl_timer *bss_timer;

	bss_info = CAC_BSSCFG_CUBBY(cac, cfg);
	ASSERT(cfg != NULL);
	ASSERT(data != NULL);

	/* backup timer overwrite in memcpy below */
	bss_timer = bss_info->addts_timer;
	...
	bss_info->addts_timer = bss_timer;
