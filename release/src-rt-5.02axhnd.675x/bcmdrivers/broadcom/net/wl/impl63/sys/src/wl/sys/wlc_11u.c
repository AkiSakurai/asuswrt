 ...
 *
 * $Id: wlc_11u.c 771216 2019-01-18 14:11:18Z $
 */
...
#endif // endif
#ifdef WLRSDB
static int wlc_11u_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len);
static int wlc_11u_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len);
#endif /* WLRSDB */
...
#define BSS_11U_CUBBY_COPY_LEN	sizeof(wlc_11u_bsscfg_cubby_copy_t) + \
					UP_TABLE_MAX
...
#ifdef WLRSDB
	cubby_params.fn_get = wlc_11u_bss_get;
	cubby_params.fn_set = wlc_11u_bss_set;
	cubby_params.config_size = BSS_11U_CUBBY_COPY_LEN;
#endif /* WLRSDB */
...
#ifdef WLRSDB
static int
wlc_11u_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	wlc_11u_info_t *m11u = (wlc_11u_info_t *)ctx;
	wlc_11u_bsscfg_cubby_t *bss_11u = WLC_11U_BSSCFG_CUBBY(m11u, cfg);
	wlc_11u_bsscfg_cubby_copy_t *cp = (wlc_11u_bsscfg_cubby_copy_t *)data;
	int err = BCME_OK;
	ASSERT(cfg);
	ASSERT(data);
	if ((BSS_11U_CUBBY_COPY_LEN) > *len) {
		err = BCME_BUFTOOSHORT;
		goto exit;
	}
	if (bss_11u->up_table) {
		cp->up_table = (uint8*)cp + sizeof(*cp);
		memcpy(cp->up_table, bss_11u->up_table, UP_TABLE_MAX);
	} else {
		cp->up_table = NULL;
	}
	cp->interworking = bss_11u->interworking;
exit:
	*len = BSS_11U_CUBBY_COPY_LEN;
	return err;

}

static int
wlc_11u_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	int err = BCME_OK;
	wlc_11u_info_t *m11u = (wlc_11u_info_t *)ctx;
	wlc_11u_bsscfg_cubby_t *bss_11u = WLC_11U_BSSCFG_CUBBY(m11u, cfg);
	const wlc_11u_bsscfg_cubby_copy_t *cp = (const wlc_11u_bsscfg_cubby_copy_t *)data;
	/* the vendor IE's (data) has already been copied across in bsscfg_configure() API
	* below code snippet updates the pointers on this bsscfg.
	*/
	wlc_11u_upd_ie_ptr(m11u, cfg, DOT11_MNG_INTERWORKING_ID);
	wlc_11u_upd_ie_ptr(m11u, cfg, DOT11_MNG_QOS_MAP_ID);
	if (cp->up_table) {
		if (!(bss_11u->up_table = MALLOC(m11u->wlc->pub->osh, UP_TABLE_MAX))) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				m11u->wlc->pub->unit, __FUNCTION__,
				MALLOCED(m11u->wlc->pub->osh)));
			err = BCME_NOMEM;
			return err;
		}
		memcpy(bss_11u->up_table, cp->up_table, UP_TABLE_MAX);
	}
	bss_11u->interworking = cp->interworking;
	return err;
}
#endif /* WLRSDB */
