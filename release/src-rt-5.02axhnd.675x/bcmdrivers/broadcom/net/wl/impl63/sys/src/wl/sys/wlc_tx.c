 ...
 *
 * $Id: wlc_tx.c 781837 2019-12-03 02:16:10Z $
 *
 */
...
wlc_txstall_dump(void *ctx, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "wl%d %d TX stalls. Total stalled time: %d seconds (AVG: %d)\n",
	...
	for (i = 0; i < TXSTALL_HISTORY_SZ; i++) {
		...
		if (stall->time > 0) {
			cnt++;
			bcm_bprintf(b, "%d sec ago: stalled for %d sec; block bitmap=0x%x FIFO's:",
...
wlc_txq_stall_watchdog(wlc_info_t *wlc)
{
	 ...
	 */
	if (wlc->block_time && wlc->block_timeout) {
		if (wlc->pub->now > wlc->block_time) {
			dt = wlc->pub->now - wlc->block_time;
			if (dt > wlc->block_timeout && wlc->block_datafifo) {
				stall_detect = TRUE;
			}
		}
	}
	...
	WL_ERROR(("wl%d TX Stall: block_datafifo set for %d sec\n",
		wlc->pub->unit, dt));
	 ...
	 */
	first_pass = dt <= (wlc->block_timeout + 1);
...
#ifdef WLTAF
			{
				...
				for (tid = 0; tid < NUMPRIO; tid++) {
					...
					wlc_taf_sched_state(taf_handle, NULL, tid,
						mutx_ac_mask_in->mask & 1 << (WL_MU_VHTMU - 1),
						NULL, TAF_SCHED_STATE_MU_DL_VHMIMO);
					wlc_taf_sched_state(taf_handle, NULL, tid,
						mutx_ac_mask_in->mask & 1 << (WL_MU_HEMMU - 1),
						NULL, TAF_SCHED_STATE_MU_DL_HEMIMO);
					wlc_taf_sched_state(taf_handle, NULL, tid,
						mutx_ac_mask_in->mask & 1 << (WL_MU_DLOFDMA - 1),
						NULL, TAF_SCHED_STATE_MU_DL_OFDMA);
				}
			}
#endif /* WLTAF */
		} else {
...
wlc_tx_open_datafifo(wlc_info_t *wlc)
{
	...
	if (!wlc->pub->up || wlc->hw->reinit || wlc->txfifo_detach_pending) {
		...
		if (wlc->block_datafifo && wlc->block_time && wlc->block_timeout &&
			(wlc->pub->now - wlc->block_time) > wlc->block_timeout) {
			WL_ERROR(("wl%d STALL: up=%d reinit=%d detach=%d datablock=0x%x "
				...
				wlc->block_time ? wlc->pub->now - wlc->block_time : 0));
		}
#endif /* BCMDBG_TXSTALL */
		return;
	}
...
#ifdef BCMDBG_TXSTALL
	if (block_datafifo_last && wlc->block_datafifo && wlc->block_time == 0) {
		/* still blocked */
		wlc->block_time = wlc->pub->now;
	}
...
wlc_recover_pkt_scb(wlc_info_t *wlc, void *pkt)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (scb == wlc->bandstate[bandunit]->hwrs_scb) {
	...
	if (bsscfg == NULL) {
		...
		bandunit = wlc_txhinfo2bandunit(wlc, &txh_info);
