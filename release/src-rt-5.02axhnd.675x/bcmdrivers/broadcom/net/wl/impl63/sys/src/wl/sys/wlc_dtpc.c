...
dtpc_power_context_set_channel(wlc_dtpc_info_t *dtpci, chanspec_t chanspec)
{
	...
	/* board limit */
	wlc_phy_txpower_sromlimit(dtpci->wlc->pi, chanspec, NULL, power_context->board_min, 0);
	...
	if (cubby == NULL || pwr == NULL) {
		...
		if (pwr->clm_max == NULL) {
			if (!dtpc_power_context_set_channel(dtpci,
				phy_utils_get_chanspec((phy_info_t *)WLC_PI(dtpci->wlc)))) {
				return BCME_NOMEM;
			}
		}
		if (pwr->clm_max) {
