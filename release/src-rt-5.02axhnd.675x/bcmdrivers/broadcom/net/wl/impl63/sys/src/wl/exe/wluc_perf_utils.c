	...
	bool percent)
{
	enum {
		...
		PLSCB_QUEUE = 1 << 12, PLX_SQSS = 1 << 13, PL_AQM = 1 << 14
	...
	const prec_log_definitions_t counter_defs[NUM_PREC_LOGS] = {
		...
		PLS5 | PLS6 | PLL       | PL6 | PLNORM | PLPCENT | PL_AQM },
		/* PREC_LOG_SGI */
		{ ", %sgi", " %4s",           pdump_aqm5,
		PLS6 | PLL       | PL6 | PLNORM | PLPCENT | PLSCB_QUEUE | PL_AQM },
		/* PREC_LOG_NSS */
	...
	if ((iov_pktq_log->params.addr_type[index] & 0x7F) == 'C' && (iov_pktq_log->version >= 6)) {
		usage_exclude |= PLSCB_QUEUE;
	}
	...
	bool percent)
{
	...
	const mutx_log_definitions_t counter_defs[NUM_MUTX_LOGS] = {
		...
		{ ",%RU 2 /   4 /   8 /  20 /  40 /  80 /  160",  "  %4s, %4s, %4s, %4s, %4s, %4s, "
		"%4s" },
		/* MUTX_MIMO_GROUP */
