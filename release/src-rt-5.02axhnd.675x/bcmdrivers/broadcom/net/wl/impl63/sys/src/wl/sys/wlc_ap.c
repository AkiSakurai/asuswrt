 ...
 *
 * $Id: wlc_ap.c 781849 2019-12-03 09:16:21Z $
 */
...
typedef struct
{
	...
	ratespec_t	force_bcn_rspec;	/**< force beacon ratespec (in unit of 500kbps) */
...
wlc_ap_up(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg)
{
	...
	if (CHSPEC_IS40(target_bss->chanspec) &&
		...
		target_bss->chanspec = CH20MHZ_CHSPEC2(channel,
		                                       CHSPEC_BANDTYPE(target_bss->chanspec));
	...
	void *p, bool short_preamble, d11rxhdr_t *rxh)
{
	...
	if (scb && (bsscfg->WPA_auth & (WPA3_AUTH_SAE_PSK | WPA3_AUTH_SAE_FBT)) &&
			(auth_alg == DOT11_SAE)) {
...
send_result:
	...
	if (scb && (status == DOT11_SC_SUCCESS) && (auth_alg == DOT11_FAST_BSS) &&
	...
	if (scb != NULL) {
		...
		if (mac_mode_deny_client && !scb) {
			...
			wlc_sendauth(bsscfg, &hdr->sa, &bsscfg->BSSID, scb, auth_alg, auth_seq + 1,
				status, NULL, short_preamble, wlc_ap_auth_tx_complete, scb);
	...
	wlc_rx_data_desc_t *rx_data_desc, struct scb *scb, bool short_preamble)
{
	...
	bool free_scb = FALSE;
	wlc_assoc_oui_t *assoc_oui_cubby;
	...
	if (assoc_oui_cubby) {
		bzero(assoc_oui_cubby, sizeof(*assoc_oui_cubby));
		bcopy(&assoc_oui, assoc_oui_cubby, sizeof(*assoc_oui_cubby));
	}
...
#ifdef WL_OCE_AP
	if (OCE_ENAB(wlc->pub) &&
		(wlc_oce_if_valid_assoc(wlc->oce, &wrxh->rssi, &reject_data) ==
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_MAP):
	{
		...
		if ((isset(&int_val, 0) || isset(&int_val, 1)) && !BSSCFG_AP(bsscfg)) {
			WL_ERROR(("wl%d: Error. Trying to set Multi-AP Fronthaul or Backhaul AP"
		...
		if (isset(&int_val, 2) && !BSSCFG_STA(bsscfg)) {
			WL_ERROR(("wl%d: Error. Trying to set Multi-AP Backhaul STA"
...
wlc_ap_aid_new(wlc_ap_info_pvt_t *appvt, wlc_bsscfg_t *cfg)
{
	 ...
	 */
	for (aid = AP_MAX_BSSCFG_AID; aid < AP_MAX_AID; aid++) {
		/* AID still in use? */
