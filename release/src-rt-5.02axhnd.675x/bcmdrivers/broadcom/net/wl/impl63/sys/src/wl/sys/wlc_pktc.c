 ...
 *
 * $Id: wlc_pktc.c 773974 2019-04-04 09:07:47Z $
 */
...
wlc_scb_pktc_disable(wlc_pktc_info_t *pktc, struct scb *scb)
{
	...
	if (RSDB_ENAB(wlc->pub)) {
		...
		FOREACH_WLC(wlc_cmn, idx, wlc_iter) {
			if (wlc_iter->pktc_info->rfc[cidx].scb == scb) {
				WL_NONE(("wl%d: %s: Invalidate rfc %d before"
					"freeing scb %p\n", wlc_iter->pub->unit,
					__FUNCTION__, cidx, OSL_OBFUSCATE_BUF(scb)));
				wlc_iter->pktc_info->rfc[cidx].scb = NULL;
			}
		}
	} else
#endif /* WLRSDB */
