 ...
 *
 * $Id: wlc_assoc.c 781849 2019-12-03 09:16:21Z $
 */
 ...
 */
static int
wlc_assoc_pruned_roamscan_prep(wlc_bsscfg_t *cfg, chanspec_t *chanspec_list, int n_channels)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_roam_t *roam = cfg->roam;
	int ret = n_channels;
	...
	if (roam->prune_type == ROAM_PRUNE_NONE) {
		...
		ret = wlc_scan_filter_channels(wlc->scan, chanspec_list, n_channels);
...
/** chanspec_list being NULL/n_channels being 0 means all available chanspecs */
static int
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int n_channels)
{
	...
	if (assoc) {
		 ...
		 */
			...
			/* TODO:6GHZ: below code fragment does not add 6GHz chspecs yet */
			for (i = 0; i < sizeof(chanvec_t); i++) {
				chn_list = roam->roam_chn_cache.vec[i];
				idx = i * NBBY; /* idx represents a channel number */
				for (j = 0; j < NBBY; j++, idx++, chn_list >>= 1) {
					chanspec_t chspec20mhz =
					          (WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(idx) | idx);
					if (!wlc_valid_chanspec_db(wlc->cmi, chspec20mhz))
						continue;

					/* Home channel is already included */
					if (chanspecs[0] == chspec20mhz)
						continue;
		...
		*/
		...
		if (RSDB_APSCAN_ENAB(wlc->pub)) {
			...
			if (WLRCC_ENAB(wlc->pub) && roam->roam_scan_started &&
				n_channels != 0 && chanspec_list != NULL) {
				int channel_num_pruned;
				memcpy(chanspec_pruned, chanspec_list,
					n_channels * sizeof(*chanspec_pruned));
				channel_num_pruned = wlc_assoc_pruned_roamscan_prep(cfg,
					chanspec_pruned, n_channels);
				if (channel_num_pruned != n_channels) {
					n_channels = channel_num_pruned;
		...
		err = wlc_scan(wlc->scan, bss_type, bssid, 1, &ssid, scan_type, nprobes,
			active_time, passive_time, home_time, chanspec_list, n_channels, 0, TRUE,
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_ROAM_CHANNELS_IN_HOTLIST): {
		...
		list->count = 0;
		/* TODO:6GHZ: make the below for loop capable of returning 6g channels as well */
		for (i = 0; i < sizeof(chanvec_t); i++) {
			...
			while (chn_list) {
				if (chn_list & 1) {
					list->element[list->count++] =
						(WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(ch_idx) |
					         ch_idx);
				}
				ch_idx++;
				chn_list >>= 1;
