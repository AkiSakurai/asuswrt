 ...
 *
 * $Id: wlc_ht.c 781667 2019-11-26 22:31:28Z $
 */
...
void BCMATTACHFN(wlc_ht_init_defaults)(wlc_ht_info_t *pub)
{
	...
	wlc_update_bwcap(pub, WLC_BAND_2G, WLC_BW_CAP_20MHZ, FALSE);
	wlc_update_bwcap(pub, WLC_BAND_5G, WLC_BW_CAP_UNRESTRICTED, TRUE);
#if BAND6G
	wlc_update_bwcap(pub, WLC_BAND_6G, WLC_BW_CAP_UNRESTRICTED, TRUE);
#endif // endif
	...
	void *params, uint p_len, void *arg, uint len, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		{
			int cur_band_type = wlc->band->bandtype;

			if (bsscfg->associated) {
				cur_band_type = CHSPEC_BANDTYPE(current_bss->chanspec);
			}

			err = wlc_set_iovar_ratespec_override(wlc, bsscfg, cur_band_type,
...
bool
wlc_is_scb_dynamic_smps(wlc_ht_info_t *pub, struct scb *scb)
{
	...
	if (scb_pub->ht_mimops_enabled && scb_pub->ht_mimops_rtsmode) {
		return TRUE;
	} else {
		return FALSE;
	}
		...
		if (!(scb->flags2 & SCB2_IGN_SMPS) || SCB_IS_BRCM(scb)) {
			...
			if ((old_mimops != scb_pub->ht_mimops_enabled) ||
				...
				WL_INFORM(("wl%d:%s,"MACF"changd,mimops:[%d %d],rtsmde[%d %d]\n",
					...
					old_mimops, scb_pub->ht_mimops_enabled, old_mimops_rtsmode,
					scb_pub->ht_mimops_rtsmode));
			}
		}
