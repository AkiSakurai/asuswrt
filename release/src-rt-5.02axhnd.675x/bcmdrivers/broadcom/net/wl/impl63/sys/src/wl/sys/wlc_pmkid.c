 ...
 *
 * $Id: wlc_pmkid.c 771216 2019-01-18 14:11:18Z $
 */
...
#ifdef WLRSDB
#define PMKID_CUBBY_CONFIG_SIZE	sizeof(bss_pmkid_info_t)
static int
wlc_pmkid_bss_get(void *ctx, wlc_bsscfg_t *bsscfg, uint8 *data, int *len)
{
	bss_pmkid_info_t *bss_pmkid;
	ASSERT(ctx);
	ASSERT(data);
	ASSERT(len);
	bss_pmkid = PMKID_BSSCFG_CUBBY((wlc_pmkid_info_t *)ctx, bsscfg);
	...
	if (*len < PMKID_CUBBY_CONFIG_SIZE) {
		*len = PMKID_CUBBY_CONFIG_SIZE;
		return BCME_BUFTOOSHORT;
	}
	*len = PMKID_CUBBY_CONFIG_SIZE;
	memcpy(data, bss_pmkid, PMKID_CUBBY_CONFIG_SIZE);

	return BCME_OK;
}

static int
wlc_pmkid_bss_set(void *ctx, wlc_bsscfg_t *bsscfg, const uint8 *data, int len)
{
	bss_pmkid_info_t *bss_pmkid;
	ASSERT(ctx);
	ASSERT(data);
	ASSERT(len == PMKID_CUBBY_CONFIG_SIZE);
	bss_pmkid = PMKID_BSSCFG_CUBBY((wlc_pmkid_info_t *)ctx, bsscfg);
	...
	memcpy(bss_pmkid, data, PMKID_CUBBY_CONFIG_SIZE);
...
#ifdef WLRSDB
	bsscfg_cubby_params.fn_get = wlc_pmkid_bss_get;
	bsscfg_cubby_params.fn_set = wlc_pmkid_bss_set;
	bsscfg_cubby_params.config_size = PMKID_CUBBY_CONFIG_SIZE;
#endif /* WLRSDB */
