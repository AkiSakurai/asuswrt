 ...
 *
 * $Id: wlc_he.c 780724 2019-11-01 16:19:24Z $
 */
...
/** called on './wl he partialbsscolor', this command is no longer supported, due to limitation
 * it puts on AID usage.
 */
static int
...
wlc_he_get_omi_tx_nsts(wlc_he_info_t *hei, scb_t *scb)
{
	scb_he_t *sh;
	sh = SCB_HE(hei, scb);
	return (HTC_OM_CONTROL_TX_NSTS(sh->omi_htc) + 1);
}
...
#ifdef TESTBED_AP_11AX
		update_pkt_eng_ulnss(wlc, scb->aid, HTC_OM_CONTROL_TX_NSTS(omi_data));
#endif /* TESTBED_AP_11AX */
	}
