 ...
 *
 * $Id: wlc_bmac.c 781829 2019-12-02 14:52:28Z $
 */
	...
	wl_txpwrcap_tbl_t *txpwrcap_tbl, int* cellstatus)
{
	...
	if (!wlc_hw->up) {
		...
		if ((wlc_hw->deviceid == BCM4360_D11AC_ID) ||
		    ...
		    (wlc_hw->deviceid == BCM43692_D11AX_ID) ||
...
wlc_bmac_revinfo_get(wlc_hw_info_t *wlc_hw, wlc_bmac_revinfo_t *revinfo)
{
	...
	revinfo->nbands = NBANDS(wlc_hw->wlc);
...
no_devid_override:
	...
	WL_ERROR(("wlc_bmac_attach, deviceid 0x%x bands %x\n", wlc_hw->deviceid,
		wlc_hw->_bandmask));
...
BCMATTACHFN(wlc_bmac_info_init)(wlc_hw_info_t *wlc_hw)
{
	...
	wlc_hw->chanspec = CH20MHZ_CHSPEC2(1, WLC_BAND_2G);
...
/* the "#else" below is intentional */
	...
	/* Enable Defragment feature */
	if (D11REV_GE(wlc_hw->corerev, 128)) {
		setbit(&mac_cap_l, MSWCAP_DEFRAG_ON_NBIT);
	}
	...
	if (D11REV_IS(wlc_hw->corerev, 132)) {
		...
		if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
			if (BTCX_ENAB(wlc_hw)) {
				...
				wlc_ucode_write(wlc_hw, d11ucode_mu129_2,
						d11ucode_mu129_2sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_mu129_2,
						d11ucodex_mu129_2sz);
			}
		} else if (D11MINORREV_IS(wlc_hw->corerev_minor, 1)) {
			if (BTCX_ENAB(wlc_hw)) {
...
wlc_bmac_dump_phy(wlc_hw_info_t *wlc_hw, const char *name, struct bcmstrbuf *b)
{
...
#endif // endif
	{
		...
		FOREACH_WLC_BAND(wlc_hw->wlc, bandunit) {
			...
			pi = band->pi;
			ret = phy_dbg_dump((phy_info_t *)pi, name, b);
		}
	}
...
wlc_bmac_cca_stats_read(wlc_hw_info_t *wlc_hw, cca_ucode_counts_t *cca_counts)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 42) || D11REV_IS(wlc_hw->corerev, 56) ||
...
wlc_bmac_hwa_txfifo_commit(wlc_info_t *wlc, uint fifo, const char *from)
{
	...
	if (hwa_txpkt->head == NULL) {
		...
		WL_INFORM(("wl%d %s: local pktc is empty fifo=%d from \"%s\"\n",
			wlc->pub->unit, __FUNCTION__, fifo, from));
		return BCME_OK;
	}
...
chain_hwa_txpkt:
		if (hwa_txpkt->head) {
			...
			if (hwa_txpkt->fifo != fifo) {
				WL_ERROR(("wl%d: Commit current fifo-%d, new fifo-%d coming.\n",
	...
	if (BCM_DMA_CT_ENAB(wlc)) {
		...
		if (!(*wlc_hw->txavail_aqm[fifo])) {
			WL_INFORM(("wl%d %s: NO AQM descriptors available, fifo=%d \n",
				wlc->pub->unit, __FUNCTION__, fifo));
			return BCME_BUSY;
		}
		...
		txd_range_t range, uint16 ncons, bool nonAMPDU)
{
	...
	for (npkt = 0; npkt < ncons; npkt++)
	{
		 ...
		 * the errror handling
		 */
		WL_INFORM(("wl%d: %s: AQM-fifo %d reclaim err=%u, AMPDU=%u nc=%u np=%u\n",
