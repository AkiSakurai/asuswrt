 ...
 *
 * $Id: wluc_rpsnoa.c 674525 2016-12-09 04:17:19Z $
 */
#include <bcmendian.h>
...
wl_rpsnoa_sub_cmd_enable(void *wl, const wl_rpsnoa_sub_cmd_t *cmd, char **argv)
{
	...
	if (*argv && (!stricmp(*argv, "b") || !stricmp(*argv, "2g"))) {
		iov_in.data->band = WLC_BAND_2G;
	} else if (*argv && (!stricmp(*argv, "a") || !stricmp(*argv, "5g"))) {
		iov_in.data->band = WLC_BAND_5G;
	} else if (*argv && !stricmp(*argv, "all")) {
		iov_in.data->band = WLC_BAND_ALL;
	} else {
		ret = BCME_BADARG;
		goto fail;
	}
	...
	if (*++argv) {
		...
		for (i = 0; i < iov_resp->hdr.cnt; i++)
			printf("band : %s, enable %d\n",
				...
				((iov_resp->data[i].band == WLC_BAND_5G) ? "5G" : "BAND ALL")),
...
wl_rpsnoa_sub_cmd_params(void *wl, const wl_rpsnoa_sub_cmd_t *cmd, char **argv)
{
	...
	if (*argv && (!stricmp(*argv, "b") || !stricmp(*argv, "2g"))) {
		iov_in.param->band = WLC_BAND_2G;
	} else if (*argv && (!stricmp(*argv, "a") || !stricmp(*argv, "5g"))) {
		iov_in.param->band = WLC_BAND_5G;
	} else if (*argv && !stricmp(*argv, "all")) {
		iov_in.param->band = WLC_BAND_ALL;
	} else {
		ret = BCME_BADARG;
		goto fail;
	}
	...
	if (*++argv) {
		...
		for (i = 0; i < iov_resp->hdr.cnt; i++)
			printf("band : %s, pps %d, quiet_time %d, level %d, stas_assoc_check %d\n",
				...
				((iov_resp->param[i].band == WLC_BAND_5G) ? "5G" : "BAND ALL")),
