 ...
 *
 * $Id: wlc_bssload.c 766978 2018-08-23 11:08:21Z $
 */
...
static const bcm_iovar_t bssload_iovars[] = {
	...
	(0), 0, IOVT_BUFFER, sizeof(bss_load_t)
	},
	{NULL, 0, 0, 0, 0, 0},
};
...
struct wlc_bssload_info {
	...
	struct ether_addr steering_sta;		/* steering_sta address */
	bool fake_bssload;			/* fake bssload  */
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_BSSLOAD_FAKE_CHAN_UTIL):
		if (BSSCFG_AP(bsscfg)) {
			bss_load_t *bss_load  = (struct bss_load *)arg;
			if (!ETHER_ISNULLADDR(&mbssload->steering_sta)) {
				bcopy((char *)(&mbssload->steering_sta),
					(char *)(bss_load->maclist.ea),	ETHER_ADDR_LEN);
				bss_load->maclist.count = 1;
				bss_load->chan_util = mbssload->fake_chan_util;
			}
		}
		break;
	...
	case IOV_SVAL(IOV_BSSLOAD_FAKE_CHAN_UTIL):
		if (BSSCFG_AP(bsscfg)) {
			bss_load_t *bss_load  = (struct bss_load *)arg;
			if (bss_load->maclist.count) {
				mbssload->fake_chan_util = bss_load->chan_util;
				bcopy(&bss_load->maclist.ea,  &mbssload->steering_sta,
					ETHER_ADDR_LEN);
			}
		}
		break;
...
int
wlc_bssload_set_fake_bssload_flag(struct ether_addr *da,  wlc_info_t *wlc)
{
	wlc_bssload_info_t *mbssload = (wlc_bssload_info_t *)wlc->mbssload;

	if ((!ETHER_ISNULLADDR(&mbssload->steering_sta)) && (!ETHER_ISNULLADDR(da))) {
		if (!bcmp(da, &mbssload->steering_sta, ETHER_ADDR_LEN)) {
			mbssload->fake_bssload = TRUE;
		}
	}
	return 0;
}
...
wlc_bssload_write_qbss_load_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (mbssload->fake_bssload && (data->ft == FC_PROBE_RESP)) {
		chan_util = mbssload->fake_chan_util;
		mbssload->fake_bssload = FALSE;
	}
