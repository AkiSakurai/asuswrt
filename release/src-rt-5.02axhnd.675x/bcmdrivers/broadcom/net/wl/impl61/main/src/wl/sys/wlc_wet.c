 ...
 *
 * $Id: wlc_wet.c 766336 2018-07-31 03:38:15Z $
 */
...
static int wet_sta_update_all(wlc_wet_info_t *weth, wlc_bsscfg_t *cfg,
	...
	uint8 *eh, uint8 *iph, int length, int send)
{
	...
	if (send) {
		...
		*/
		...
		if (wet_table_upd && wet_sta_update_all(weth, cfg, iaddr,
	...
	/* no action for received bcast/mcast ethernet frame */
		...
		if (IP_ISMULTI(iaddr_dest)) {
			...
			if (wet_sta_find_ip(weth, iaddr, &sta) < 0) {
				WL_ERROR(("wl%d:: unable to find STA %u.%u.%u.%u\n",
	...
	uint8 *eh, uint8 *arph, int length, int send)
{
	...
	if (send) {
		...
		if (wet_sta_update_all(weth, cfg, iaddr,
		(struct ether_addr*)(eh + ETHER_SRC_OFFSET), &sta) < 0) {
...
/* Returns 0 if succeeded; < 0 otherwise. */
static int
wet_sta_update_all(wlc_wet_info_t *weth, wlc_bsscfg_t *cfg, uint8 *iaddr,
	struct ether_addr *eaddr, wet_sta_t **saddr)
{
	wet_sta_t *sta;
	int i;
