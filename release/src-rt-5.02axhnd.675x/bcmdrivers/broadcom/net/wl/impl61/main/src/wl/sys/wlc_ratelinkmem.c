...
#define D11_LINK_MEM_ENTRY_ADDR_BME_VIEW(wlc, index) (SI_ENUM_BASE_BP(wlc->pub->sih) + \
	(D11REV_IS(wlc->pub->corerev, 128) ? LINK_OFFSET_REV128(index) : LINK_OFFSET(index)))
...
struct wlc_ratelinkmem_info {
	...
	bme_info_t	*bme;		/* reference to BME context */
...
BCMATTACHFN(wlc_ratelinkmem_attach)(wlc_info_t *wlc)
{
	...
	rlmi->bme = bme_attach(wlc->pub->sih, BME_CHANNEL_0);
	if (rlmi->bme == NULL) {
		WL_ERROR(("wl%d: %s: Unable to register BME channel: %d\n",
			wlc->pub->unit, __FUNCTION__, BME_CHANNEL_0));
		ASSERT(0);
		return rlmi;
	}
	...
	if (BUSTYPE(wlc->pub->sih->bustype) == SI_BUS) {
		...
		/* src may be cached, dst never is */
		bme_flags |= BME_CONFIG_FLAG_SRC_COHERENT;
#endif // endif
	} else if (BUSTYPE(wlc->pub->sih->bustype) == PCI_BUS) {
		/* NIC driver use host memory. */
		bme_flags |= BME_CONFIG_FLAG_SRC_HOSTMEM;
	}

	bme_config(rlmi->bme, hi_src, hi_dst, bme_flags, burst_len);
...
wlc_ratelinkmem_hw_init(wlc_info_t *wlc)
{
	...
	ASSERT(RATELINKMEM_ENAB(wlc->pub));
	ASSERT(wlc->rlmi->bme != NULL);

	/* Inform HW of entry sizes in DWORD units */
	W_REG(wlc->osh, D11_PSM_RATEXFER_SIZE(wlc), (WLC_RLM_RATE_ENTRY_SIZE / 4));
	W_REG(wlc->osh, D11_PSM_LINKXFER_SIZE(wlc), (link_size / 4));
...
wlc_ratelinkmem_write_rucfg(wlc_info_t *wlc, uint8 *buf, uint sz, uint16 rmem_idx)
{
	...
	if (sz != WLC_RLM_RATE_ENTRY_SIZE) {
		W_REG(wlc->osh, D11_PSM_RATEXFER_SIZE(wlc), (sz/ 4));
	}

	/* ensure previous transfer (if any) completed before we overwrite memory */
	if (!PHYSADDRISZERO(rlmi->rate_dma_pa)) {
		bme_sync(rlmi->bme); /* spin until previous transfer completed */
#ifndef BCM_SECURE_DMA
...
wlc_ratelinkmem_upd_rmem(wlc_ratelinkmem_info_t *rlmi, uint8 *buf, uint sz, uint16 rmem_idx)
{
	...
	bme_cpy64(rlmi->bme, paddr,
		(uintptr)D11_RATE_MEM_ENTRY_ADDR_BME_VIEW(wlc, rmem_idx),
		sz);
#else
	bme_cpy(rlmi->bme, (const uint8*) rlmi->rate_dma_pa,
	...
	bool clr_txbf_stats)
{
	...
	bme_cpy64(rlmi->bme, paddr,
	        (uintptr)D11_LINK_MEM_ENTRY_ADDR_BME_VIEW(wlc, scbh->rate_link_index),
		size);
#else
	bme_cpy(rlmi->bme, (const uint8*) pa,
	...
	if (!D11REV_IS(wlc->pub->corerev, 128) && 0 &&
		...
		PHYSADDRTOULONG(pa, paddr);
		bme_cpy64(rlmi->bme, paddr,
		        (uintptr)D11_LINK_MEM_ENTRY_ADDR_BME_VIEW(wlc,
...
#ifdef BCMDMA64OSL
	PHYSADDRTOULONG(pa, paddr);
	bme_cpy(rlmi->bme, (const uint8*) (uintptr)paddr,
		(uint8 *)(uintptr)D11_LINK_MEM_ENTRY_ADDR_BME_VIEW(wlc,
