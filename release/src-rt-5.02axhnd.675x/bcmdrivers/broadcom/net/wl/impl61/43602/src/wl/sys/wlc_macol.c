...
wlc_macol_d11hdrs(macol_info_t *macol, void *p, ratespec_t rspec, uint16 fifo)
{
	...
	frameid = ((seq << TXFID_SEQ_SHIFT) & TXFID_SEQ_MASK) | (fifo & TXFID_QUEUE_MASK);
	macol->counter++;
	...
	if (!fc_mgmt && macol_txinfo->key.algo) {
		txh->CacheInfo.BssIdEncAlg = macol_txinfo->key.algo_hw << D11AC_ENCRYPT_ALG_SHIFT;
		txh->CacheInfo.KeyIdx = macol_txinfo->key.idx;
...
wlc_macol_acphy_txctl1_calc(wlc_hw_info_t *wlc_hw, ratespec_t rspec)
{
	...
	*/
	...
	if (WLPROPRIETARY_11N_RATES_ENAB(wlc_hw->wlc->pub)) {
		if (RSPEC_ISHT(rspec) && (rspec & RSPEC_RATE_MASK) >= WLC_11N_FIRST_PROP_MCS) {
...
wlc_recv_mgmt_rx_channel_get(wlc_hw_info_t *wlc_hw, wlc_d11rxhdr_t *wrxh)
{
	...
	if (CHSPEC_IS40(wrxh->rxhdr.RxChan)) {
		...
		if (WLCISACPHY(wlc_hw->band)) {
			uint16 subband = (wrxh->rxhdr.PhyRxStatus_0 & PRXS0_ACPHY_SUBBAND_MASK);
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	if ((scan->bss_type == DOT11_BSSTYPE_ANY ||
	     (scan->bss_type == DOT11_BSSTYPE_INFRASTRUCTURE && (cap & DOT11_CAP_ESS)) ||
...
wlc_macol_dequeue_dma_pkt(wlc_hw_info_t *wlc_hw, tx_status_t *txs)
{
	...
	p = dma_getnexttxp(wlc_hw->di[queue], HNDDMA_RANGE_TRANSMITTED);
	ASSERT(p);
	if (p) {
...
{
	d11actxh_rate_t *ri = &acHdrPtr->RateInfo[rateIdx];
...
static void
wlc_macol_print_per_pkt_cache_ac(d11actxh_t* acHdrPtr)
{
	printf("TxD Pkt Cache Info:\n");
	wlc_macol_print_byte(" BssIdEncAlg", acHdrPtr->CacheInfo.BssIdEncAlg);
	wlc_macol_print_byte(" KeyIdx", acHdrPtr->CacheInfo.KeyIdx);
	wlc_macol_print_byte(" PrimeMpduMax", acHdrPtr->CacheInfo.PrimeMpduMax);
	wlc_macol_print_byte(" FallbackMpduMax", acHdrPtr->CacheInfo.FallbackMpduMax);
	wlc_macol_print_word(" AmpduDur", ltoh16(acHdrPtr->CacheInfo.AmpduDur));
	wlc_macol_print_byte(" BAWin", acHdrPtr->CacheInfo.BAWin);
	wlc_macol_print_byte(" MaxAggLen", acHdrPtr->CacheInfo.MaxAggLen);
	prhex(" TkipPH1Key", (uchar *)acHdrPtr->CacheInfo.TkipPH1Key, 10);
	prhex(" TSCPN", (uchar *)acHdrPtr->CacheInfo.TSCPN, 6);
}

static void
wlc_macol_print_txdesc_ac(void* hdrsBegin)
{
	/* tso header */
	d11actxh_t* acHdrPtr;
	uint len, rate_count;
	...
	if (len == D11AC_TXH_LEN) {
		...
		wlc_macol_print_per_pkt_cache_ac(acHdrPtr);
	}
}

void
wlc_macol_print_txdesc(wlc_txd_t *txd)
{
	wlc_macol_print_txdesc_ac(txd);
}
