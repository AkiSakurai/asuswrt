 ...
 *
 * $Id: wlc_ampdu.c 769206 2018-11-06 10:38:14Z $
 */
...
_wlc_sendampdu_noaqm(ampdu_tx_info_t *ampdu_tx, wlc_txq_info_t *qi, void **pdu, int prec)
{
	...
	uint32 scb_flags;
	uint32 pktlen = 0;
	uint32 tx_pkts = 0; /* accumulate stats locally, then flush to scb_stats */
	uint32 tx_ucast_bytes = 0;
#endif /* PKTC */
	...
	while (p) {
		...
		if (err == BCME_OK) {
			WLCNTSCBINCR(tx_pkts);
			WLCNTSCBADD(tx_ucast_bytes, pktlen);
		}
...
#ifdef PKTC
	/* Transfer prep_sdu_fast success stats to scb_stats */
	WLCNTSCBADD(scb->scb_stats.tx_pkts, tx_pkts);
	WLCNTSCBADD(scb->scb_stats.tx_ucast_bytes, tx_ucast_bytes);
#endif /* PKTC */
...
_wlc_sendampdu_aqm(ampdu_tx_info_t *ampdu_tx, wlc_txq_info_t *qi, void **pdu, int prec)
{
	...
	/* Transfer prep_sdu_fast success stats into scb_stats */
	WLCNTSCBADD(scb->scb_stats.tx_pkts, tx_pkts);
	WLCNTSCBADD(scb->scb_stats.tx_ucast_bytes, tx_ucast_bytes);
