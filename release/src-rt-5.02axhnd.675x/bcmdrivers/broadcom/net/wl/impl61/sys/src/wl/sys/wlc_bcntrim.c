 ...
 *
 * $Id: wlc_bcntrim.c 771216 2019-01-18 14:11:18Z $
 */
...
#define WL_BCNTRIM_INFO(args)   \
    EVENT_LOG_COMPACT_CAST_PAREN_ARGS(EVENT_LOG_TAG_BCNTRIM_INFO, args)
#define WL_BCNTRIM_TRACE(args)   \
    EVENT_LOG_COMPACT_CAST_PAREN_ARGS(EVENT_LOG_TAG_BCNTRIM_TRACE, args)
#else
...
wlc_bcntrim_report_stats(wlc_bcntrim_info_t *bcntrim, uint32 *stats, uint8 reset)
{
	...
	FOREACH_WLC(wlc_cmn, idx, wlc_iter) {
		if (!wlc_iter->clk) {
			...
			wlc_bcntrim_stats_snapshot(wlc_iter);
		}
		for (i = 0; i < BCNTRIM_STATS_MAX; i++) {
			stats[i] += wlc_iter->bcntrim->curr_stats[i];
		}
		/* reset stats */
		if (reset) {
			wlc_bcntrim_reset_stats(wlc_iter, wlc_iter->clk);
		}
	}
}
...
/* bsscfg cubby copy */
static int
wlc_bcntrim_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	wlc_bcntrim_info_t *bcntrim_info = (wlc_bcntrim_info_t *)ctx;
	bcntrim_bsscfg_cubby_t *bcntrim_bsscfg = NULL;
	bcntrim_bsscfg_cubby_copy_t *cp = (bcntrim_bsscfg_cubby_copy_t *)data;
	int config_len = sizeof(bcntrim_bsscfg_cubby_copy_t);
	uint32 now = OSL_SYSUPTIME();

	ASSERT(cfg != NULL);
	ASSERT(data != NULL);
	bcntrim_bsscfg = BCNTRIM_BSSCFG_CUBBY(bcntrim_info, cfg);
	ASSERT(bcntrim_bsscfg != NULL);
	...
	cp->bcntrim_enabled = bcntrim_bsscfg->bcntrim_enabled;
	cp->bcntrim_threshold = bcntrim_bsscfg->bcntrim_threshold;
	cp->phyrate_threshold =  bcntrim_bsscfg->phyrate_threshold;
	cp->tsf_drift = bcntrim_bsscfg->tsf_drift;
	cp->override_disable_mask = bcntrim_bsscfg->override_disable_mask;
	/* disable duration needs to be updated before copy */
	if (bcntrim_bsscfg->disable_reqs & ~bcntrim_bsscfg->override_disable_mask) {
		bcntrim_bsscfg->disable_duration += (now -
		                                     bcntrim_bsscfg->disable_start_time);
		bcntrim_bsscfg->disable_start_time = now;
	}
	cp->disable_start_time = bcntrim_bsscfg->disable_start_time;
	cp->disable_duration = bcntrim_bsscfg->disable_duration;
...
static int
wlc_bcntrim_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	wlc_bcntrim_info_t *bcntrim_info = (wlc_bcntrim_info_t *)ctx;
	bcntrim_bsscfg_cubby_t *bcntrim_bsscfg = BCNTRIM_BSSCFG_CUBBY(bcntrim_info, cfg);
	const bcntrim_bsscfg_cubby_copy_t *cp = (const bcntrim_bsscfg_cubby_copy_t *)data;

	bcntrim_bsscfg->bcntrim_enabled = cp->bcntrim_enabled;
	bcntrim_bsscfg->bcntrim_threshold = cp->bcntrim_threshold;
	bcntrim_bsscfg->phyrate_threshold =  cp->phyrate_threshold;
	bcntrim_bsscfg->tsf_drift = cp->tsf_drift;
	bcntrim_bsscfg->disable_duration = cp->disable_duration;
	bcntrim_bsscfg->disable_start_time = cp->disable_start_time;
	bcntrim_bsscfg->override_disable_mask = cp->override_disable_mask;

	wlc_bcntrim_configure_shmem(bcntrim_info->wlc);
	/* XXX: AS_RECREATE => we are cloning a live STA,
	* from one slice to another
	* (could be in a future use-case in a chip like 4364: 3x3 => 1x1)
	*/
	if (cfg->assoc->type == AS_RECREATE) {
...
#ifdef WLRSDB
	cubby_params.fn_get = wlc_bcntrim_bss_get;
	cubby_params.fn_set = wlc_bcntrim_bss_set;
	cubby_params.config_size = sizeof(bcntrim_bsscfg_cubby_t);
#endif /* WLRSDB */
...
wlc_bcntrim_recv_process_partial_beacon(wlc_bcntrim_info_t *bcntrim_info)
{
	...
	if (BSSCFG_STA(bsscfg) && bsscfg->BSS) {
		...
		EVENT_LOG(EVENT_LOG_TAG_BEACON_LOG, "Beacon(trim): RSSI=%d", rssi);
#endif /* EVENT_LOG_COMPILE */
