 ...
 *
 * $Id: wlc_rsdb.c 767268 2018-09-03 08:40:07Z $
 */
...
wlc_rsdb_set_core_band_lock(wlc_rsdb_info_t *rsdbinfo, bool is_bandswitch)
{
	wlc_info_t *wlc = rsdbinfo->wlc;
	rsdb_core_cap_band_t band;
	uint bandunit, idx = WLCWLUNIT(wlc);
...
wlc_rsdb_bmc_smac_template(void *hdl, int tplbuf, uint32 doublebufsize)
{
	...
	for (i = 0; i < tplbuf; i ++) {
		...
		if (end_idx >= 2 * tplbuf)
			end_idx = (2 * tplbuf) - 1;
		W_REG(osh, D11_MSDUEntryStartIdx_ALTBASE(sregs, wlc->regoffsets), tplbuf + i);
		W_REG(osh, D11_MSDUEntryEndIdx_ALTBASE(sregs, wlc->regoffsets), end_idx);
