 ...
 *
 * $Id: wlc_btcx.c 776077 2019-06-18 11:43:47Z $
 */
...
BCMATTACHFN(wlc_btc_attach)(wlc_info_t *wlc)
{
	...
	btc->sw_coex = BTCX_ENAB(wlc_hw) && SWBTCX_ENAB(wlc_hw);
	btc->eci_coex = BTCX_ENAB(wlc_hw) && !SWBTCX_ENAB(wlc_hw);
...
wlc_btc_wlc_up(void *ctx)
{
	...
	if ((btc->btc_bt_gpio && btc->btc_stat_gpio && btc->btc_wlan_gpio)) {
		...
		/* program coex gpio mask bit scratch registers; lower 16 GPIOs for now */
	...
	phy_btc_config.sw_coex = btc->sw_coex;
	phy_btc_config.eci_coex = btc->eci_coex;
