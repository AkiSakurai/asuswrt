 ...
 */
extern uint32 wlc_offload_m2m_intstatus(wlc_offload_t *wlc_offl, bool in_isr);

/** Enable or disable BME txstatus interrupts */
extern void wlc_offload_set_txs_intmask(wlc_offload_t *wlc_offl, bool enable);

#ifdef BCMQT
/**
 * The 2x2 ax core in the 63178 has two irq lines towards the ARM host processor. This could not
 * be easily modeled in Veloce. Thefore, the Veloce testbench implemented a shortcut: the two irq
 * lines are OR'ed together and result in one interrupt on the (emulated) PCIe endpoint. XXX.
 */
extern bool wlc_offload_is_m2m_irq(wlc_offload_t *wlc_offl);
#endif /* BCMQT */
 ...
 */
extern int wlc_offload_wl_dump(wlc_offload_t *wlc_offl, struct bcmstrbuf *b);
#endif // endif
