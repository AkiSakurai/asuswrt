 ...
 *
 * $Id: wl_mdns_common.c 617664 2016-02-06 13:09:19Z $
 *
 */
...
u8 *GetLargeResourceRecord(DNSMessage *msg, u8 *ptr, u8 *end, u8 RecordType, LargeCacheRecord *largecr)
{
	...
	if (ptr + 10 > end) {
		WL_ERROR(("%s: Record len < 10 bytes, ptr 0x%p, end 0x%p\n", __FUNCTION__,
			OSL_OBFUSCATE_BUF(ptr), OSL_OBFUSCATE_BUF(end)));
		return (mDNSNULL);
	}
	if (IS_WL_TRACE_ON()) {
...
{
	int len = DomainNameLength(src);
	memcpy(dst->c, src->c, len > MAX_DOMAIN_NAME ? MAX_DOMAIN_NAME : len);
}
