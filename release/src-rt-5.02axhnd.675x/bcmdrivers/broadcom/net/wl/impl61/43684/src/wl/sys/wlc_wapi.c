 ...
 *
 * $Id: wlc_wapi.c 774133 2019-04-11 09:15:54Z $
 */
...
static const bcm_iovar_t wlc_wapi_iovars[] = {
	{"wai_restrict", IOV_WAI_RESTRICT, (0), 0, IOVT_BOOL, 0},
	{"wai_rekey", IOV_WAI_REKEY, (0), 0, IOVT_BUFFER, ETHER_ADDR_LEN},
	{"wapiie", IOV_WAPIIE, (0), 0, IOVT_BUFFER, 0},
	{NULL, 0, 0, 0, 0, 0}
};
#define BSS_WAPI_PRIV_CUBBY_LEN(wapi_ie_len) wapi_ie_len + \
	sizeof(wlc_wapi_bsscfg_cubby_priv_t)

static int wlc_wapi_bsscfg_init(void *ctx, wlc_bsscfg_t *cfg);
static void wlc_wapi_bsscfg_deinit(void *ctx, wlc_bsscfg_t *cfg);
#ifdef WLRSDB
static int wlc_wapi_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len);
static int wlc_wapi_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len);
#endif /* WLRSDB */
static int wlc_wapi_doiovar(void *ctx, uint32 actionid,
...
BCMATTACHFN(wlc_wapi_attach)(wlc_info_t *wlc)
{
	...
	cubby_params.context = wapi;
	cubby_params.fn_init = wlc_wapi_bsscfg_init;
	cubby_params.fn_deinit = wlc_wapi_bsscfg_deinit;
	cubby_params.fn_dump = wlc_wapi_bsscfg_dump;
#ifdef WLRSDB
	cubby_params.fn_get = wlc_wapi_bss_get;
	cubby_params.fn_set = wlc_wapi_bss_set;
	cubby_params.config_size = sizeof(wlc_wapi_bsscfg_cubby_priv_t);
#endif /* WLRSDB */

	/* reserve cubby in the bsscfg container for per-bsscfg private data */
	wlc->wapi_cfgh = wlc_bsscfg_cubby_reserve_ext(wlc, priv_offset +
		sizeof(wlc_wapi_bsscfg_cubby_priv_t),
		&cubby_params);
	if (wlc->wapi_cfgh < 0) {
...
#ifdef BCMWAPI_WAI
static int
wlc_wapi_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_WAPIIE):
		if ((int)len < cfg_priv->wapi_ie_len)
...
#ifdef WLRSDB
static int
wlc_wapi_bss_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	wlc_wapi_info_t *wapi = (wlc_wapi_info_t *)ctx;
	wlc_wapi_bsscfg_cubby_priv_t *bss_wapi_priv = WAPI_BSSCFG_PRIV(wapi, cfg);
	wlc_wapi_bsscfg_cubby_priv_t *cp = (wlc_wapi_bsscfg_cubby_priv_t *)data;
	int err = BCME_OK;
	ASSERT(cfg);
	ASSERT(data);

	if ((int)(BSS_WAPI_PRIV_CUBBY_LEN(bss_wapi_priv->wapi_ie_len)) > *len) {
		err = BCME_BUFTOOSHORT;
		goto exit;
	}
	cp->wai_preauth = bss_wapi_priv->wai_preauth;
	if (bss_wapi_priv->wapi_ie) {
		cp->wapi_ie_len = bss_wapi_priv->wapi_ie_len;
		cp->wapi_ie = (uint8*)cp + (sizeof(*cp));
		memcpy(cp->wapi_ie, bss_wapi_priv->wapi_ie, bss_wapi_priv->wapi_ie_len);
	} else {
		cp->wapi_ie = NULL;
	}
exit:
	*len = BSS_WAPI_PRIV_CUBBY_LEN(cp->wapi_ie_len);
	return err;

}

static int
wlc_wapi_bss_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	wlc_wapi_info_t *wapi = (wlc_wapi_info_t *)ctx;
	wlc_wapi_bsscfg_cubby_priv_t *bss_wapi_priv = WAPI_BSSCFG_PRIV(wapi, cfg);
	const wlc_wapi_bsscfg_cubby_priv_t *cp = (const wlc_wapi_bsscfg_cubby_priv_t *)data;
	bss_wapi_priv->wai_preauth = cp->wai_preauth;
	if (cp->wapi_ie) {
		if (!(bss_wapi_priv->wapi_ie = MALLOC(wapi->pub->osh, cp->wapi_ie_len))) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				wapi->pub->unit, __FUNCTION__,
				MALLOCED(wapi->pub->osh)));
			return BCME_NOMEM;
		}
		bss_wapi_priv->wapi_ie_len = cp->wapi_ie_len;
		memcpy(bss_wapi_priv->wapi_ie, cp->wapi_ie, cp->wapi_ie_len);
	}
	return BCME_OK;
}
#endif /* WLRSDB */
	...
	uint8 *data, uint32 len)
{
	...
	e = wlc_event_alloc(wapi->wlc->eventq, WLC_E_WAI_MSG);
	if (e == NULL) {
	...
	e->event.datalen = len;
	e->data = wlc_event_data_alloc(wapi->wlc->eventq,
	                               e->event.datalen, WLC_E_WAI_MSG);
	if (e->data == NULL) {
	...
	memcpy(e->data, data, e->event.datalen);
...
wlc_wapi_wai_req_ind(wlc_wapi_info_t *wapi, wlc_bsscfg_t *bsscfg, struct scb *scb)
{
	...
	wlc_ap_find_wpaie(wapi->wlc->ap, scb, &wpaie, &wpaie_len);
	wlc_wapi_station_event(wapi, bsscfg, &scb->ea, wpaie,
...
#if !defined(WLNOEIND)
void
wlc_wai_event_indicate(wlc_info_t *wlc, void *p, struct scb *scb, wlc_bsscfg_t *bsscfg,
	struct ether_header *eh)
{
	...
	ASSERT(ntoh16(eh->ether_type) == ETHER_TYPE_WAI);

	if (BSSCFG_AP(bsscfg)) {
		WL_APSTA_RX(("wl%d: %s: gen WAI event for AP %s\n",
		             wlc->pub->unit, __FUNCTION__,
		             bcm_ether_ntoa(&scb->ea, eabuf)));
	}
	if (bsscfg->BSS || SCB_IS_IBSS_PEER(scb)) {
		wlc_wapi_bss_wai_event(wlc->wapi, bsscfg, &scb->ea,
				PKTDATA(wlc->osh, p) + ETHER_HDR_LEN,
				PKTLEN(wlc->osh, p) - ETHER_HDR_LEN);
	}
} /* wlc_wai_event_indicate */
#endif /* !WLNOEIND */
#endif /* BCMWAPI_WAI */
