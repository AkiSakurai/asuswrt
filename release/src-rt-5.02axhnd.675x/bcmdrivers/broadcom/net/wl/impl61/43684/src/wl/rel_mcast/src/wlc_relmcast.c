...
#define ERMC_DFLT_TXRATE_VHT	OFDM_RSPEC(WLC_RATE_24M)
...
static const bcm_iovar_t wlc_rmc_iovars[] = {
	...
	(IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, sizeof(uint8)
	},
	{"rmc_txack", IOV_RELMCAST_TXACK,
	(0), 0, IOVT_BOOL, 0
	},
	{"rmc_ackmac", IOV_RELMCAST_ACKMAC,
	(IOVF_OPEN_ALLOW), 0, IOVT_BUFFER,
	...
	(0), 0, IOVT_BUFFER, sizeof(wl_relmcast_status_t)
	},
	...
	(IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, sizeof(wl_rmc_cnts_t)
	},
...
static void wl_ermc_xmitter_start(wlc_rmc_info_t* ermc);
...
/* beacon process call back function */
static void wlc_rmc_beacon_process_cb(wlc_rmc_info_t *rmcp,
...
static uint8 wlc_rmc_stats(wlc_rmc_info_t * const rmcp, wl_rmc_cnts_t *cnts);
static void wlc_rmc_status(wlc_rmc_info_t * const rmcp, wl_relmcast_status_t *statusp);
static int wlc_rmc_down(void *context);
...
static inline void wlc_rmc_insert_tr(wlc_rmc_trans_info_t **head_tr,
...
static int wlc_rmc_scb_init(void *context, struct scb *scb);
static void wlc_rmc_scb_deinit(void *context, struct scb *scb);
...
struct wlc_rmc_info {
	...
	bool			mc_age_timer_started;	/* age timer startred */
...
static uint8 wlc_rmc_send_ctrlframe(wlc_rmc_info_t *rmcp, uint8 count)
{
	...
	bsscfg = wlc_bsscfg_find_by_hwaddr(rmcp->wlc, &rmcp->self_mac);
	ASSERT(bsscfg != NULL);
...
static void wlc_rmc_sendnull(wlc_rmc_info_t * const rmcp, wlc_bsscfg_t *bsscfg)
{
	...
	if (wlc_sendnulldata(rmcp->wlc, bsscfg, da, 0, 0, PKTPRIO_NON_QOS_DEFAULT, NULL, NULL)) {
	...
	uint8 *body, int body_len, wlc_d11rxhdr_t *wrxh)
{
	...
	bcopy((rmc_notify_af_t *)body, &afdata, MIN(body_len, sizeof(rmc_notify_af_t)));
	if (body_len < sizeof(rmc_notify_af_t)) {
		/* fill the default value */
		afdata.artmo = WL_RMC_DEFAULT_ARTMO;
	}
	...
	is_brcm_action = (body_len >= sizeof(rmc_notify_af_t)) &&
		!bcmp(afdata.bcm_oui, BRCM_PROP_OUI, sizeof(afdata.bcm_oui));
	if (is_brcm_action) {
	/* translate the action type to subtype */
...
#endif // endif
		if (BSSCFG_IBSS(wlc->cfg) && is_brcm_action)
...
static uint8 wlc_rmc_handle_start_msg(wlc_rmc_info_t *ermc, rmc_notify_af_t *afdata)
{
	...
	if (bcmp(&afdata->leader_mac, &ermc->self_mac, ETHER_ADDR_LEN) == 0) {
		...
		ermc->mc_txack = TRUE;
		/* starting mc_age_out timer for active receiver */
		if (!ermc->mc_age_timer_started) {
			wl_add_timer(ermc->wlc->wl, ermc->mc_age_timer,
				WL_ERMC_AFTX_PERIOD * 2, TRUE);
			ermc->mc_age_timer_started = TRUE;
		}
...
static uint8 wlc_rmc_stats(wlc_rmc_info_t * const rmcp,	wl_rmc_cnts_t *cnts)
{
	...
	bcopy((char*)rmcp->ermc_cnts, cnts, sizeof(*rmcp->ermc_cnts));
	return err;
}
	...
	if (rmcp->mc_process_fn) {
		if (rmcp->mc_ackerscb &&
			...
			if ((1000*(rmcp->wlc->pub->now - rmcp->mc_ackerscb->used)) >
				...
				rmcp->mc_ackerscb->flags3 |= SCB3_RELMCAST_NOACK;
				wlc_lq_rssi_ma_reset(rmcp->wlc, SCB_BSSCFG(rmcp->mc_ackerscb),
				                     rmcp->mc_ackerscb, WLC_RSSI_INVALID);
			}
			else {
				/* Current acker is still acking, else use rssi 10000 */
				rssi = wlc_lq_rssi_get(rmcp->wlc, SCB_BSSCFG(rmcp->mc_ackerscb),
				                       rmcp->mc_ackerscb);
			}
		}
		...
		FOREACHSCB(rmcp->wlc->scbstate, &scbiter, scb) {
			if ((((SCB_ASSOCIATED(scb) &&
				...
				if (!(scb->flags3 & SCB3_RELMCAST_NOACK) &&
					rmcp->auto_ar_select == TRUE) {
					rssi1 = wlc_lq_rssi_get(rmcp->wlc, SCB_BSSCFG(scb), scb);
	...
	ratespec_t *rspecp)
{
	...
	if (type == FC_TYPE_DATA && ETHER_ISMULTI(&h->a1) && !ETHER_ISBCAST(&h->a1)) {
...
static void wl_ermc_xmitter_start(wlc_rmc_info_t* rmcp)
{
	...
	if (rmcp->mc_process_fn) {
		FOREACHSCB(rmcp->wlc->scbstate, &scbiter, scb) {
			if ((SCB_ASSOCIATED(scb) && BSSCFG_AP(scb->bsscfg)) ||
				...
				clientp->rssi = wlc_lq_rssi_get(rmcp->wlc, SCB_BSSCFG(scb), scb);
	...
	ratespec_t rate_override, pkcb_fn_t fn)
{
	...
	if ((pkt = wlc_frame_get_action(wlc, &af->da,
...
static int wlc_rmc_doiovar(void *ctx,
	uint32 actionid, void *params, uint p_len, void *arg,
...
wlc_rmc_info_t * BCMATTACHFN(wlc_rmc_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_scb_cubby_reserve(wlc, 0,
		wlc_rmc_scb_init, wlc_rmc_scb_deinit, NULL, rmcp) < 0) {
	...
	if (wlc_module_register(wlc->pub,
	                        ...
	                        "rmc",
	                         (void *)rmcp,
	                         wlc_rmc_doiovar,
	                         NULL, NULL, wlc_rmc_down) != BCME_OK) {
	...
	if (wlc_scb_state_upd_register(wlc, (scb_state_upd_cb_t)wlc_rmc_check_cb,
	                               rmcp) != BCME_OK) {
	...
	if (wlc_bss_rx_bcn_register(wlc,
			(bss_rx_bcn_notif_fn_t)wlc_rmc_beacon_process_cb, rmcp) != BCME_OK) {
		WL_ERROR(("wl%d: %s: wlc_bss_rx_bcn_register failed\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
...
#endif // endif
	rmcp->auto_ar_select = TRUE;
	rmcp->mc_age_timer_started = FALSE;
...
void BCMATTACHFN(wlc_rmc_detach)(wlc_rmc_info_t * rmcp)
{
	...
	wlc_bss_rx_bcn_unregister(wlc,
		(bss_rx_bcn_notif_fn_t)wlc_rmc_beacon_process_cb, rmcp);
...
static int
wlc_rmc_down(void *ctx)
{
	wlc_rmc_info_t *rmcp = (wlc_rmc_info_t *)ctx;

	if (!rmcp) {
		WL_ERROR(("%s wlc_rmc_info is NULL\n", __FUNCTION__));
		return BCME_NOMEM;
	}

	if (rmcp->mc_age_timer_started) {
		wl_del_timer(rmcp->wlc->wl, rmcp->mc_age_timer);
		rmcp->mc_age_timer_started = FALSE;
	}
	...
	uint8 *tag_params, int tag_params_len)
{
	...
	if (rmcp->mc_process_fn) {
		...
		if (rmc_brcm_prop_ie) {
			...
			wlc_lq_sample_req_enab(scb, RX_LQ_SAMP_REQ_RMC, TRUE);
	...
	if (rmcp && rmcp->mc_process_fn && scb) {
		if (SCB_ASSOCIATED(scb) || BSSCFG_IBSS(scb->bsscfg)) {
			if (scb != rmcp->mc_ackerscb && (scb->flags3 & SCB3_RELMCAST) &&
				!(scb->flags3 & SCB3_RELMCAST_NOACK) &&
			    (rssi = wlc_lq_rssi_get(rmcp->wlc, SCB_BSSCFG(scb), scb)) >=
					rmcp->rmc_lowrssi) {
				/* New station authenticated */
				if (rmcp->mc_ackerscb)
					arrssi = wlc_lq_rssi_get(rmcp->wlc,
						SCB_BSSCFG(rmcp->mc_ackerscb), rmcp->mc_ackerscb);
...
/* beacon process call back function */
static void wlc_rmc_beacon_process_cb(wlc_rmc_info_t *rmcp, bss_rx_bcn_notif_data_t *data)
{
	ASSERT(data != NULL);
	ASSERT(rmcp != NULL);

	if (BSSCFG_IBSS(data->cfg)) {
		struct dot11_bcn_prb *bcn = (struct dot11_bcn_prb *)data->body;
		uint16 cap = ltoh16(bcn->capability);
		int rx_bandunit = CHSPEC_BANDUNIT(data->wrxh->rxhdr.lt80.RxChan);
		struct scb *scb = wlc_scbfindband(rmcp->wlc, data->cfg,
			&data->hdr->sa, rx_bandunit);
		if (scb != NULL) {
			/* Check privacy mode to differentiate secured devices
			 * in the same BSSID
			 */
			if (cap & DOT11_CAP_PRIVACY) {
				scb->WPA_auth = WPA_AUTH_NONE;
			} else {
 ...
 */
static inline void wlc_rmc_insert_tr(wlc_rmc_trans_info_t **head_tr, wlc_rmc_trans_info_t* tr_info)
{
	ASSERT(tr_info != NULL);
	ASSERT(head_tr != NULL);
...
{
	bool flag = FALSE;
	uint min_body_len = OFFSETOF(rmc_notify_af_t, leader_mac) + sizeof(struct ether_addr);
...
static void wlc_rmc_age_timer(void *arg)
{
	...
	if ((rmcp->tr_cnt == 0) && rmcp->mc_age_timer_started) {
		wl_del_timer(rmcp->wlc->wl, rmcp->mc_age_timer);
		rmcp->mc_age_timer_started = FALSE;
	}
}
...
static int
wlc_rmc_scb_init(void *context, struct scb *scb)
{
	return BCME_OK;
}

static void
wlc_rmc_scb_deinit(void *context, struct scb *scb)
{
	wlc_rmc_info_t *rmcp = (wlc_rmc_info_t *)context;

	if (rmcp && (scb == rmcp->mc_ackerscb)) {
		rmcp->mc_ackerscb->flags3 |= SCB3_RELMCAST_NOACK;
		wlc_rmc_find_acker(rmcp);
	}
	return;
