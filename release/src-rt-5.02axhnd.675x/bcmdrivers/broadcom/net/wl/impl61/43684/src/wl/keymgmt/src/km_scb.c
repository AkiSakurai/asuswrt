...
#ifdef BCMWAPI_WPI
	for (id = WLC_KEY_ID_PAIRWISE; id <= (WLC_KEY_ID_PAIRWISE + 1); id++)
#endif // endif
	{
...
#ifdef BCMWAPI_WPI
		/* for wapi primary key could have a key id of 1 */
		if (key_info && (key_info->key_idx != scb_km->key_idx))
			key = wlc_keymgmt_get_scb_key(km, scb, WLC_KEY_ID_PAIRWISE + 1,
				WLC_KEY_FLAG_NONE, key_info);
#endif /* BCMWAPI_WPI */
	} else {
