 ...
 *
 * $Id: rte_arm.c 772360 2019-02-21 10:33:20Z $
 */
...
hnd_fiqtrap_handler(uint32 epc, uint32 lr, uint32 sp, uint32 cpsr)
{
	...
	hnd_cpu_loadavg_timer(hnd_sih, loadavg_time);
#endif /* BCMDBG_LOADAVG */
...
#if !(defined(BCMDBG_LOADAVG) && (defined(__ARM_ARCH_7R__) || \
	defined(__ARM_ARCH_7A__))) && !defined(THREADX_WATCHDOG)
...
#elif !defined(BCM_BOOTLOADER) && !defined(THREADX_WATCHDOG)
#ifndef WLTEST
...
#if defined(RTE_CONS) && !defined(BCM_BOOTLOADER)
#ifdef BCMDBG_LOADAVG
	if (!hnd_cons_add_cmd("lat", hnd_loadavg_test, 0))
		return BCME_NORESOURCE;
#endif // endif
#ifdef BCMDBG_CPU
