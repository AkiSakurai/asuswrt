...
static uint16 txbfcfg_def_vals[TXBFCFG_MAX_ENTRIES] = {
	1, 1, 1,			/* host flags */
	0xc8,				/* musnd_timer */
	1, 1, 4, 4, 64,			/* thresholds for vht, hemm, hests, hesus, cqi */
	0,				/* ntxvnew */
	0,				/* musnd2grp_disable */
...
#define C_BFECAP_HE	0xe21b // default
...
#define C_BFECAP_VHT	0xc41b // default
...
wlc_txbf_mu_cap_stas(wlc_txbf_info_t *txbf)
{
	...
	uint8 link_bw;
	uint32 bw_policy;
	bool exclude_he = HE_DLMU_ENAB(wlc->pub);
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (!wlc_mutx_sta_ac_check(wlc->mutx, scb))
			continue;
		if (SCB_HE_CAP(scb) && exclude_he) {
			/* exclude HE stations from MU MIMO in case DL OFDMA is enabled */
			continue;
		}
