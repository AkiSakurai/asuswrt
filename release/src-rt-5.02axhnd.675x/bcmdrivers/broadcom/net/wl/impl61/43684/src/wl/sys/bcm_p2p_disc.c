 ...
 *
 * $Id: bcm_p2p_disc.c 690360 2017-03-15 23:28:25Z $
 */
...
struct bcm_p2p_disc_s
{
	...
	struct ether_addr addr;		/* BCM_P2P_DISC_ADD_DELETE_IES */
	/* channel */
	...
	uint16 extListenCount;
	uint16 extListenCurrentCount;
	bcm_p2p_disc_cb_t ext_listen_count_cb;
	void	*ext_listen_count_ctx;
...
bcm_p2p_disc_t *bcm_p2p_disc_create(struct bcm_p2p_wl_drv_hdl *drv, uint16 listenChannel)
{
	...
	disc->ext_listen_count_cb = NULL;
	disc->ext_listen_count_ctx = NULL;
...
{
	disc->ext_listen_count_cb = fn;
	disc->ext_listen_count_ctx = p2po;
}

bool bcm_p2p_is_listening(bcm_p2p_disc_t *disc)
{
	if (disc->state == STATE_EXT_LISTEN_ON) {
		return TRUE;
	} else {
		return FALSE;
	}
	...
	uint16 count)
{
	...
	WL_TRACE(("bcm_p2p_disc_start_ext_listen %d %d %d\n",
		listenOnTimeout,
		listenOffTimeout,
		count));
	disc->extListenOnTimeout = listenOnTimeout;
	disc->extListenOffTimeout = listenOffTimeout;
	disc->extListenCount = count;
	disc->extListenCurrentCount = 0;
...
{
	wlc_info_t *wlc = (wlc_info_t *)disc->drv;
	wlc_bsscfg_t *bsscfg = wl_disc_get_p2p_devcfg(wlc);

	BCM_REFERENCE(bsscfg);

	if (disc->extListenCount != 0) {
		disc->extListenCurrentCount++;
	}
...
static state_func_t *
BCMRAMFN(p2p_disc_get_table)(void)
{
	return g_table;
}
...
static void fsm(bcm_p2p_disc_t *disc, eventT event)
{
	...
	if (disc->state < STATE_LAST && event < EVENT_LAST) {
		...
		(p2p_disc_get_table()[disc->state])(disc, event);
	...
	if (disc->ext_listen_count_cb && (disc->extListenCount != 0) &&
			(disc->extListenCurrentCount > disc->extListenCount)) {
		disc->ext_listen_count_cb(disc->ext_listen_count_ctx);
	}
