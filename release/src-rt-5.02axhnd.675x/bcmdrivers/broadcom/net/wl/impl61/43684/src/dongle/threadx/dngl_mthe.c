 ...
 *
 * $Id: dngl_mthe.c 774076 2019-04-09 12:13:18Z $
 */
...
#ifndef MAIN_THREAD_EVENT_DEPTH
#define MAIN_THREAD_EVENT_DEPTH		OSL_DEFAULT_EVENT_DEPTH
#endif /* MAIN_THREAD_EVENT_DEPTH */
...
main_thread_entry(osl_ext_task_arg_t arg)
{
	...
	thread->stats = (void*)0xFFFFFFFF; /* Special value for hnd_isr_stats_fixup. */

	/* Initialize system. */
	sih = _c_main();

	/* Enable event handling (DPCs scheduled from ISRs or timers) for this thread */
	hnd_thread_init_event_handling(thread, MAIN_THREAD_EVENT_DEPTH);
...
#ifdef HND_INTSTATS
	/* Create main thread statistics object and fix references of child objects */
	thread->stats = hnd_isr_stats_create(OBJ_TYPE_THREAD, "main_thread",
		(void*)main_thread_entry, NULL);
	hnd_isr_stats_fixup(thread->stats);
...
BCMATTACHFN(tx_application_define)(void *first_unused_memory)
{
	 ...
	 */
	if (osl_ext_task_create("main_thread",
	    get_main_thread_stack(), HND_STACK_SIZE,
	    OSL_EXT_TASK_TIME_CRITICAL_PRIORITY,
	    main_thread_entry, NULL, get_main_thread()) != OSL_EXT_SUCCESS) {
		THREADX_STAT_UPDATE(THREADX_STAT_ERROR);
		return;
	}
}
