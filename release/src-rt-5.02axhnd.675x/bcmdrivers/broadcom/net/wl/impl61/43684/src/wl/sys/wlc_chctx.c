 ...
 *
 * $Id: wlc_chctx.c 758241 2018-04-18 07:43:37Z $
 */
...
BCMATTACHFN(wlc_chctx_pre_attach)(wlc_info_t *wlc)
{
	...
	/* Support shared chctx module among all radio slices */
	if ((chctx = (wlc_chctx_reg_t *)obj_registry_get(wlc->objr, OBJR_SHARED_CHCTX)) != NULL) {
		(void)obj_registry_ref(wlc->objr, OBJR_SHARED_CHCTX);
		info->chctx = chctx;
		return info;
	}
	...
	obj_registry_set(wlc->objr, OBJR_SHARED_CHCTX, chctx);
	(void)obj_registry_ref(wlc->objr, OBJR_SHARED_CHCTX);
	info->chctx = chctx;
...
BCMATTACHFN(wlc_chctx_post_detach)(wlc_chctx_info_t *info)
{
	...
	/* Support shared chctx module among all radio slices */
	wlc = info->wlc;
	if (obj_registry_unref(wlc->objr, OBJR_SHARED_CHCTX) == 0) {
		obj_registry_set(wlc->objr, OBJR_SHARED_CHCTX, NULL);
