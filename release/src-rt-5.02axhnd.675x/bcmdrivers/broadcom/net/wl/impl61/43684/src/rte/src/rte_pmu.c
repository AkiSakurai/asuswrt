 ...
 *
 * $Id: rte_pmu.c 772360 2019-02-21 10:33:20Z $
 */
...
struct pmu_extwake_ctx {
	...
	pmu_extwake_event_dpc_cb_t event_dpc_cb;
	struct pmu_extwake_ctx *next;
};

struct pmu_info {
	si_t *sih;
	uint32 pmu_int_status; /* preserves pmu_isr_status for dpc */
	uint32 pmu_extwake_status; /* preserves pmu_extwake_status for dpc */
...
BCMATTACHFN(hnd_pmu_init)(si_t *sih)
{
	 ...
	 */
	si_cc_register_isr(sih, hnd_pmu_cc_isr, hnd_pmu_cc_dpc, CI_PMU, pmu_i);
...
hnd_pmu_cc_isr(void* cbdata, uint32 ccintst)
{
	if (PMUREV(hnd_sih->pmurev) >= 26) {
		...
		intstatus = R_REG(hnd_osh, PMUREGADDR(hnd_sih, hnd_pmur, hnd_ccr, pmuintstatus));

		pmu_i->pmu_int_status |= intstatus;

		if (intstatus & RSRC_INTR_MASK_TIMER_INT_0) {
			hnd_pmu_timer_int0_isr(cbdata);
		}

		if (PMUREV(hnd_sih->pmurev) > 30 &&
			intstatus & PMU_INTR_MASK_EXTWAKE_REQ_ACTIVE_0) {
			hnd_pmu_extwake_req_active_0_isr(cbdata);
		}

#ifdef BCMPMU_STATS
		if (PMU_STATS_ENAB() &&
			intstatus & PMU_INT_STAT_TIMER_INT_MASK) {
...
static void
hnd_pmu_timer_int0_dpc(void* cbdata)
{
...
static void
hnd_pmu_extwake_req_active_0_dpc(void* cbdata)
{
	pmu_info_t *pmu_i = cbdata;
	uint32 processed_events = 0;
	pmu_extwake_ctx_t *extwk_ctx = pmu_i->pmu_extwake_ctx_list;

	/* event specific dpc handling */
	while (extwk_ctx) {

		if (pmu_i->pmu_extwake_status
			& PMU_EXTWAKE_EVT_MASK(extwk_ctx->event)) {
			if (extwk_ctx->event_dpc_cb) {
				extwk_ctx->event_dpc_cb(extwk_ctx->ctx);
...
#ifdef BCMPMU_STATS
static void
hnd_pmu_statstimer_int_dpc(void* cbdata)
{
	if (PMUREV(hnd_sih->pmurev) >= 33) {
...
hnd_pmu_set_gtimer_period()
{
	...
	if (hnd_sih->pmurev >= 34) {
		/* Setup AlpPeriod 1MHz toggle as PMU Precision usecTimer clock input */
		/* ROUND(2^26/ALP in Mhz, 0) */
		alp = 0x8012f685;
...
hnd_pmu_extwake_register_cb(si_t *sih,
	...
	pmu_extwake_event_isr_cb_t isr_cb, pmu_extwake_event_dpc_cb_t dpc_cb)
{
	osl_t *osh = si_osh(sih);
	...
	extwk_item->event_dpc_cb = dpc_cb;
