 ...
 *
 * $Id: pdftmevt.c 690872 2017-03-18 01:55:00Z $
 */
...
pdftm_event_signal(pdftm_t *ftm, wl_proxd_event_type_t event_type, pdftm_session_t *sn)
{
	...
	(void)bcm_notif_signal(ftm_cmn->h_notif, &evd);
}
...
ftm_event_get_session_data_len(pdftm_session_t *sn)
{
	...
	rtt_results_len = sizeof(wl_proxd_rtt_result_t) +
		((num_rtt + 1) * sizeof(wl_proxd_rtt_sample_t));
...
ftm_send_event(pdftm_t *ftm,  wl_proxd_event_type_t event_type,
	...
	wl_proxd_status_t status, int event_len, wl_proxd_event_t *event)
{
	int err = BCME_OK;
	...
	err = pdftm_iov_pack(ftm, event_len, event->tlvs, tlv_ids, num_tlv_ids, dig, &event_len);
	if (err != BCME_OK) {
	...
	proxd_send_event(ftm->pd, bsscfg, status, addr, event, event_len);
	...
	pdftm_notify_info_t *info, wl_proxd_event_t **out_event)
{
	...
	/* data is already in XTLV format */
	len = FTM_EVENT_ALLOC_SIZE(info->data_len, 0); /* xtlv hdr included */
...
ftm_event(pdftm_t *ftm, wlc_bsscfg_t *bsscfg, pdftm_notify_info_t *info)
{
	...
	if (!WL_PROXD_EVENT_ENABLED(ftm_cmn->config->event_mask, info->event_type)) {
		FTM_LOG(ftm, (("wl%d: %s: masked event %d, mask 0x%08x\n", FTM_UNIT(ftm),
			__FUNCTION__, info->event_type, ftm_cmn->config->event_mask)));
		goto done;
	}
	...
	pdftm_notify_t notif, pdftm_notify_info_t *info)
{
	...
	case PDFTM_NOTIF_BSS_DOWN:
		...
		if (notif == PDFTM_NOTIF_BSS_DOWN) {
			...
			if (ETHER_ISNULLADDR(&ftm_cmn->config->dev_addr) &&
				wlc_bsscfg_primary(ftm->wlc)) {
				memcpy(&ftm_cmn->config->dev_addr,
	...
	pdftm_event_callback_t cb, void *cb_ctx)
{
	...
	err = bcm_notif_add_interest(ftm_cmn->h_notif, (bcm_notif_client_callback)cb,
		cb_ctx);
	...
	pdftm_event_callback_t cb, void *cb_ctx)
{
	...
	pdftm_cmn_t *ftm_cmn = ftm->ftm_cmn;
	ASSERT(FTM_VALID(ftm));
	err = bcm_notif_remove_interest(ftm_cmn->h_notif, (bcm_notif_client_callback)cb,
		cb_ctx);
