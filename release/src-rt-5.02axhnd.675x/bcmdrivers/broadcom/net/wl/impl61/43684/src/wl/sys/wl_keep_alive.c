 ...
 *
 * $Id: wl_keep_alive.c 774483 2019-04-26 03:52:54Z $
 *
 */
...
#ifdef WLRSDB
static int wlc_mkeepalive_config_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len);
static int wlc_mkeepalive_config_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len);
#endif // endif
...
BCMATTACHFN(wl_keep_alive_attach)(wlc_info_t *wlc)
{
	...
	if (RSDB_ENAB(wlc->pub)) {
		...
		cubby_params.context = info;
		cubby_params.fn_get = wlc_mkeepalive_config_get;
		cubby_params.fn_set = wlc_mkeepalive_config_set;
		cubby_params.config_size = MKEEPALIVE_CUBBY_CONFIG_SIZE;
		if (wlc_bsscfg_cubby_reserve_ext(wlc, NULL, &cubby_params) < 0) {
			goto fail;
		}
	}
#endif // endif
	ka_cmn = info->ka_cmn = (keep_alive_cmn_t *)
		obj_registry_get(wlc->objr, OBJR_KEEPALIVE_CMN_INFO);
	if (ka_cmn != NULL) {
		/* Found previous instance, reuse  */
		goto done;
	}
	ka_cmn = info->ka_cmn = (keep_alive_cmn_t *)
		MALLOCZ(wlc->pub->osh, sizeof(*ka_cmn));
	if (ka_cmn == NULL) {
		WL_ERROR(("wl%d: %s: MALLOC failed, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto fail;
	}
	obj_registry_set(wlc->objr, OBJR_KEEPALIVE_CMN_INFO, info->ka_cmn);
	...
	(void)obj_registry_ref(wlc->objr, OBJR_KEEPALIVE_CMN_INFO);
...
BCMATTACHFN(wl_keep_alive_detach)(wl_keep_alive_info_t *info)
{
	...
	ka_cmn = info->ka_cmn;
	if (ka_cmn &&
			(obj_registry_unref(info->wlc->objr, OBJR_KEEPALIVE_CMN_INFO) == 0)) {

		obj_registry_set(info->wlc->objr, OBJR_KEEPALIVE_CMN_INFO, NULL);
...
#ifdef WLRSDB
static int
wlc_mkeepalive_config_get(void *ctx, wlc_bsscfg_t *cfg, uint8 *data, int *len)
{
	wl_keep_alive_info_t *info = (wl_keep_alive_info_t *)ctx;
	int i;
	int config_len = MKEEPALIVE_CUBBY_CONFIG_SIZE;
	...
	/* Collect the index of mkeep_alive_indiv structure for which cfg update is required */
	if (data) {
		for (i = 0; i < config_len; i++) {
			data[i] = FALSE;
			if (info->ka_cmn->mkeep_alive[i].timer &&
				(info->ka_cmn->mkeep_alive[i].bsscfg == cfg)) {
				data[i] = TRUE;
			}
		}
	}
...
static int
wlc_mkeepalive_config_set(void *ctx, wlc_bsscfg_t *cfg, const uint8 *data, int len)
{
	wl_keep_alive_info_t *info = (wl_keep_alive_info_t *)ctx;
	int i;
	...
	/* Update the cfg of mkeep_alive_indiv structure */
	for (i = 0; i < config_len; i++) {
		if (info->ka_cmn->mkeep_alive[i].timer && data[i]) {
			info->ka_cmn->mkeep_alive[i].bsscfg = cfg;
		}
	}
	return BCME_OK;
...
keep_alive_timer_update(mkeep_alive_indiv_t *indiv, int period_msec, bool enable)
{
	...
	void *wl = wlc->wl;
/* timers are always schedule from PRIMARY_WLC */
#ifdef WLRSDB
	if (RSDB_ENAB(wlc->pub)) {
		wlc = WLC_RSDB_GET_PRIMARY_WLC(wlc);
		wl = wlc->wl;
