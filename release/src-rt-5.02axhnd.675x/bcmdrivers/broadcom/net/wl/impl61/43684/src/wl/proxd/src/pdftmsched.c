 ...
 *
 * $Id: pdftmsched.c 690111 2017-03-14 18:50:03Z $
 */
...
pdftm_setup_burst(pdftm_t *ftm, pdftm_session_t *sn,
	const uint8 *req, int req_len, const wlc_d11rxhdr_t *req_wrxh, ratespec_t req_rspec)
{
	int err = BCME_OK;
	pdburst_t *burst;
	...
	sst->result_flags = 0;
	memset(&sst->avg_rtt, 0, sizeof(sst->avg_rtt));
	sst->avg_dist = 0;
	sst->num_rtt = 0;	/* note: max_rtt must not change */
	if (sst->rtt)
		memset(sst->rtt, 0, sst->max_rtt * sizeof(*sst->rtt));
		...
		const struct ether_addr *peer)
{
	...
	/* check if the provided duration is sufficient for the burst */
	if (duration_ms < burst_duration) {
...
#ifdef WLAWDL
	if (AWDL_ENAB(ftm->wlc->pub)) {
		err = wlc_awdl_st_notif_peer_add(ftm->wlc->awdl_info, peer_mac,
				(void *)sn, (void *)&sn->ext_sched->req_hdl);
		/* ext sched registration fails for the requested peer_mac */
		if (err != BCME_OK) {
			FTM_LOGSCHED(ftm, (("wl%d: %s awdl timeslot request failed"
				"for peer %s with err %d\n", FTM_UNIT(ftm), __FUNCTION__,
				bcm_ether_ntoa(peer_mac, eabuf), err)));
			return err;
		}
		else {
			FTM_LOGSCHED(ftm, (("wl%d: %s timeslot requested for peer %s \n",
				FTM_UNIT(ftm), __FUNCTION__, bcm_ether_ntoa(peer_mac, eabuf))));
			return err;
		}
	}
#endif /* WLAWDL */
