...
static void pciedev_dpc(void *cbdata);
#endif /* !RTE_POLL */
...
#ifndef RTE_POLL
	/* PCIE Mailbox ISR */
	if (hnd_isr_register(0, coreid, unit,
	    pciedev_isr, rtedev, pciedev_dpc, rtedev, NULL, bus) == NULL) {
...
pciedev_run(hnd_dev_t *rtedev)
{
	...
	if (pciedev_dispatch(drv->pciedev)) {
		if (pciedev_handle_interrupts(drv->pciedev)) {
			...
			if (hnd_dpc_reschedule()) {
				return;
			}
			DBG_BUS_INC(drv->pciedev, pciedev_run);
		}
	}
