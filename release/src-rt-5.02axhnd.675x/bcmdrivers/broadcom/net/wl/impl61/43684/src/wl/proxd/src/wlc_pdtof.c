 ...
 *
 * $Id: wlc_pdtof.c 728770 2017-10-27 10:13:57Z $
 */
...
const char *tofevent_str[] =  {
	...
	"TOF_EVENT_NOACK",
	"TOF_EVENT_WAITMTMO",
	"TOF_EVENT_COLLECT_REQ",
	"TOF_EVENT_LAST"
};

const char *tofstate_str[] =  {
	"TOF_STATE_IDLE",
	"TOF_STATE_IWAITM",
	"TOF_STATE_TWAITM",
	"TOF_STATE_ILEGACY",
...
const char *tofmode_str[] =  {
	"WL_PROXD_MODE_DISABLE",
	"WL_PROXD_MODE_NEUTRAL",
	"WL_PROXD_MODE_INITIATOR",
	"WL_PROXD_MODE_TARGET"
};
...
static uint32 wlc_pdtof_get_kval(pdtof_obj_t *tofobj, bool initiator, bool seq_en)
{
	...
	if (!tofobj->tunep->vhtack ||
		!RSPEC_ISVHT(tofobj->paramp->vht_rate << 16)) {
		/* without vhtack */
		ratespec = (tofobj->paramp->tx_rate |
	...
	if (initiator) {
		...
		k = tofobj->tunep->Ki;
		if (!k) {
			wlc_phy_kvalue(WLC_PI(wlc), tofobj->paramp->chanspec,
		...
		k = tofobj->tunep->Kt;
		if (!k) {
			if (tofobj->sm->tof_mode == WL_PROXD_MODE_TARGET) {
				if (CHSPEC_BW(tofobj->paramp->chanspec) !=
					...
					if (CHSPEC_IS80(WLC_BAND_PI_RADIO_CHANSPEC)) {
						...
						if (CHSPEC_IS20(tofobj->paramp->chanspec))
...
static void wlc_pdtof_hw(pdtof_obj_t *tofobj, bool enter, bool tx)
{
	...
	if (enter) {
		if (tofobj->seq_en) {
			wlc_phy_tof_seq_params(WLC_PI(wlc));
			phy_tof_seq_upd_dly(WLC_PI(wlc), tx, (int)tofobj->tunep->core, FALSE);
		}
	}
#endif // endif
	...
	int32 *gd, int32 *adj, bool initiator)
{
	...
	int adj_err, nfft, nbuffer, nlen, nlen_1;
#ifdef RSSI_REFINE
	int32 *T = NULL;
#endif // endif
#ifdef TOF_COLLECT
	...
	if (tofobj->paramp->chanspec == WLC_BAND_PI_RADIO_CHANSPEC)
	...
	*gd = *adj = 0;
	...
	if (seq_en) {
		if ((frame_bw == TOF_BW_20MHZ_INDEX) &&
			...
			params.thresh_scale[0] = tofobj->tunep->seq_5g20.N_tx_scale;
			params.thresh_log2[0] = tofobj->tunep->seq_5g20.N_tx_log2;
			params.thresh_scale[1] = tofobj->tunep->seq_5g20.N_rx_scale;
			params.thresh_log2[1] = tofobj->tunep->seq_5g20.N_rx_log2;
			params.w_len = tofobj->tunep->seq_5g20.w_len;
			params.w_offset = tofobj->tunep->seq_5g20.w_offset;
		} else {
			int i;
			for (i = 0; i < 2; i++) {
		...
		if (frame_bw == TOF_BW_20MHZ_INDEX &&
			!(tofobj->paramp->vht_rate & (WL_RSPEC_ENCODE_VHT >> 16)) &&
			CHSPEC_CHANNEL(WLC_BAND_PI_RADIO_CHANSPEC) <= 14) {
			/* 2g/20M channels without VHT rate */
			params.thresh_log2[1] = tofobj->tunep->N_log2_2g;
			params.thresh_scale[1] = tofobj->tunep->N_scale_2g;
		} else {
	...
	if (!hw_adj_en && sw_adj_en) {
		adj_err = phy_tof_chan_freq_response(WLC_PI(wlc), nlen,
			CORE0_K_TOF_H_BITS, params.H, NULL, p_collect_data, 1);
		if (adj_err == BCME_OK) {
#ifdef TOF_COLLECT
			n_out = nlen;
#endif // endif
	...
	int32 *gd, int32 *adj, uint16 id, bool acked, int8 *rssip, bool *discard, bool initiator)
{
	...
	if ((rspcmd & TOF_RSP_MASK) == TOF_SUCCESS) {
		...
		if (wlc_phy_tof_info(WLC_PI(wlc), &tof_info, tof_info_mask) == BCME_OK) {
			...
			if (rssip) {
				if (tof_info->info_mask & WLC_PHY_TOF_INFO_TYPE_RSSI)
					*rssip = (int8)tof_info.rssi;
				else
					*rssip = 0;
			}
			if (tof_info->info_mask & WLC_PHY_TOF_INFO_TYPE_FRAME_TYPE)
				tofobj->frame_type_cnt[tof_info.frame_type]++;
...
#ifdef TOF_COLLECT
			if (tofobj->collect) {
				tofobj->collect_info.tof_frame_type = (uint8)(tof_info.frame_type);
				tofobj->collect_info.tof_frame_bw = (uint8)(tof_info.frame_bw);
				tofobj->collect_info.tof_rssi = (int8)(tof_info.rssi);
				tofobj->collect_info.tof_cfo = (int32)(tof_info.cfo);
			}
#endif // endif
		} else {
		...
		if ((tof_info.frame_type < 1) || (tof_info.frame_bw != expected_frame_bw)) {
			TOF_PRINTF(("id 0x%x frame_type %d frame_bw %d expected_frame_bw %d\n",
				id, tof_info.frame_type, tof_info.frame_bw, expected_frame_bw));
			if (discard)
				*discard = TRUE;
			wlc_pdtof_measure(tofobj, initiator? TOF_RX : TOF_RESET);
			return BCME_ERROR;
	...
	wl_proxd_status_iovar_t *iop)
{
	...
	if (tofobj->sm) {
#ifdef TOF_DEBUG
		printf("(%s:%d): smstoped = %d and state = %d \n", __FUNCTION__, __LINE__,
			tofobj->smstoped, tofobj->sm->tof_state);
#endif // endif
		if (is_active != NULL) {
...
static int wlc_pdtof_collect_mem(wlc_info_t *wlc, pdtof_obj_t* tofobj, bool alloc)
{
	if (alloc) {
	  ...
	  tofobj->collect_buf = MALLOC(wlc->osh, K_TOF_COLLECT_HRAW_SIZE_80MHZ *
	                               sizeof(uint32));
	...
	if (!alloc || !tofobj->collect || !tofobj->collect_buf || !tofobj->collect_header) {
	  ...
	  if (tofobj->collect_buf)
	    MFREE(wlc->osh, tofobj->collect_buf, K_TOF_COLLECT_HRAW_SIZE_80MHZ*sizeof(uint32));
...
static int wlc_pdtof_start(pdmthd_if_t* svcif, bool start)
{
	...
	if (start) {
		...
		if (sm->tof_mode == WL_PROXD_MODE_INITIATOR) {
			...
			if (!tofobj->tunep->vhtack || !RSPEC_ISVHT(tofobj->paramp->vht_rate << 16))
				proxd_update_tunep_values(tofobj->tunep,
					tofobj->paramp->chanspec, FALSE);
			else
				proxd_update_tunep_values(tofobj->tunep,
					tofobj->paramp->chanspec, TRUE);
			MDBG(("TS:%d Started\n", get_usts(tofobj->wlc)));
		} else {
			tofobj->tx_t1 = 0;
			tofobj->tx_t4 = 0;
...
static void update_tof_params_from_ftm(pdtof_obj_t* tofobj, tof_trig_frm_t *ftm_rq)
{
	...
	tofobj->tunep->vhtack = params->vhtack;
	proxd_update_tunep_values(tofobj->tunep, tofobj->paramp->chanspec, params->vhtack);
}
	...
	struct ether_addr *da, wlc_d11rxhdr_t *wrxh, uint8 *body, int body_len, uint32 rspec)
{
	...
	PROXD_TRACE(("%s: Received TOF action frame ", __FUNCTION__));
	ASSERT(tofobj != NULL);
	ASSERT(tofobj->sm != NULL);
	ASSERT(tofobj->wlc != NULL);
	...
	if (tofobj->smstoped) {
		PROXD_TRACE(("%s: Discarding since state machine is disabled ", __FUNCTION__));
		return 0;
	}
#ifdef	TOF_DEBUG
	...
	if (*(body+1) == DOT11_WNM_ACTION_TMNG_MEASUR_REQ)
	{
		...
		if (*(body+2) == TOF_TYPE_REQ_END)
			...
			return BCME_ERROR;
	           ...
	           (rxh->lt80.RxStatus1 &  htol16(RXS_TOFINFO))) {
	...
	if (ret == TOF_RET_ALIVE) {
		...
		if (tofobj->scanestarted) {
			wlc_scan_abort_ex(wlc->scan, tofobj->bsscfg, WLC_E_STATUS_ABORT);
			tofobj->scanestarted = FALSE;
		}
	}
...
static void tof_ftm_req_init(pdtof_obj_t* tofobj, tof_trig_frm_t *tof_hdr, uint8 type)
{
	...
	/* T period during which intr's phy will be blind, depends on b/w */
		...
		if (!tofobj->tunep->vhtack || !RSPEC_ISVHT(tofobj->paramp->vht_rate << 16))
			tof_hdr->ftm_ie.params.rqparams.vhtack = 0;
		else
			tof_hdr->ftm_ie.params.rqparams.vhtack = 1;
...
static int wlc_pdtof_send(pdtof_sm_t *sm, struct ether_addr	*da, uint8 type)
{
	...
	af->packetId = tofobj->paramp->chanspec;
	...
	if (type == TOF_TYPE_REQ_START || type == TOF_TYPE_REQ_END)
	{
		...
		durid = wlc_compute_frame_dur(wlc, wlc->band->bandtype,
		...
		if (type == TOF_TYPE_MEASURE) {
			...
			af->packetId |= (PROXD_FTM_PACKET_TAG | PROXD_MEASUREMENT_PKTID);
		}
		else {
			/* mark measurement pkt with special packet ID to identify it later */
...
#endif // endif
#ifdef WL_PROXD_AVB_TS
	wl_proxd_event_ts_results_t *tsp;
#endif /* WL_PROXD_AVB_TS */
...
#ifdef WL_PROXD_AVB_TS
		int tslen = OFFSETOF(wl_proxd_event_ts_results_t, ts_buff);
#endif /* WL_PROXD_AVB_TS */
...
#ifdef WL_PROXD_AVB_TS
		if (TOF_VER == 2 && listp->tscnt > 0 &&
			(reason == TOF_REASON_OK ||
			reason == TOF_REASON_TIMEOUT ||
			reason == TOF_REASON_NOACK)) {
			ts_sample_t *samplep;
			tslen += sizeof(ts_sample_t) * listp->tscnt;

			tsp = (wl_proxd_event_ts_results_t *)MALLOC(tofobj->wlc->osh, tslen);
			if (tsp == NULL) {
				WL_ERROR(("TOF confirm alloc memory failed\n"));
				return tofret;
			}
			tsp->ver = hton16(TOF_VER);
			tsp->mode = hton16(sm->tof_mode);
			tsp->method = hton16(PROXD_TOF_METHOD);
			tsp->err_code = reason;
			tsp->TOF_type = ((sm->tof_legacypeer == TOF_LEGACY_AP) ?
				TOF_TYPE_ONE_WAY : TOF_TYPE_TWO_WAY);
			tsp->ts_cnt = hton16(listp->tscnt);
			samplep = &tsp->ts_buff[0];
			for (i = 0; i < listp->tscnt; i++) {
				samplep[i].t1 = hton32(listp->tslist[i].t1);
				samplep[i].t2 = hton32(listp->tslist[i].t2);
				samplep[i].t3 = hton32(listp->tslist[i].t3);
				samplep[i].t4 = hton32(listp->tslist[i].t4);
			}
			if (reason == TOF_REASON_OK)
				ret = (*tofobj->svc_funcs->notify)(tofobj->svc_funcs->notifyptr,
					&sm->tof_peerea, WLC_E_STATUS_SUCCESS,
					WLC_E_PROXD_TS_RESULTS, (void *)tsp, tslen);
			else
				ret = (*tofobj->svc_funcs->notify)(tofobj->svc_funcs->notifyptr,
...
#ifdef TOF_DEBUG_TIME
			printf("%s (%d)T1 %u T2 %u	T3 %u T4 %u	Delta %d "
...
#ifdef WL_PROXD_AVB_TS
		if (tsp) {
			MFREE(tofobj->wlc->osh, tsp, tslen);
		}
#endif /* WL_PROXD_AVB_TS */
	}
#ifdef TOF_PROFILE_BUNDLE
...
static void wlc_pdtof_collect_data(pdtof_obj_t* tofobj, int index)
{
	if (tofobj->collect) {
		...
		// (id may be same if frame is received multiple times)
		...
		while (p_collect < p_collect_end) {
			...
			if (tofobj->collect_info.nfft > K_TOF_COLLECT_H_SIZE_80MHZ)
				tofobj->collect_info.nfft = K_TOF_COLLECT_H_SIZE_80MHZ;
			bcopy((void*)&tofobj->collect_info, (void*)&p_collect->info,
					sizeof(wl_proxd_collect_info_t));
			bcopy((void*)(p_buf + n), (void*)&p_collect->H,
					K_TOF_COLLECT_H_SIZE_80MHZ*sizeof(uint32));
	...
	switch (query->request) {
		...
		case PROXD_COLLECT_QUERY_HEADER:
		{
			...
			*reqLen = sizeof(wl_proxd_collect_header_t);
			...
			if (tofobj->remote_collect && tofobj->collect_header) {
				...
				reply->clkdivisor &= 0xff; //PMU1_PLL0_PC1_M1DIV_MASK;
				reply->chipnum = tofobj->chipnum;
				reply->chiprev = tofobj->chiprev;
				reply->phyver = wlc->band->phyrev;

				bcopy(&sm->tof_selfea, &reply->localMacAddr, ETHER_ADDR_LEN);
		...
		case PROXD_COLLECT_QUERY_DATA:
		{
			...
			size = sizeof(wl_proxd_collect_data_t) -
				(K_TOF_COLLECT_H_SIZE_80MHZ - collect->info.nfft)* sizeof(uint32);
	...
	wlc_pdtof_data_t *datap)
{
	...
	TOF_PRINTF(("pdtof_sm event = %s and state = %s mode = %s \n",
		(event <= 7 ? tofevent_str[event] : NULL),
		(sm->tof_state <= 7 ? tofstate_str[sm->tof_state] : NULL),
		(sm->tof_mode <= 3 ? tofmode_str[sm->tof_mode] : NULL)));
	...
	printf("<7> %s: Mode %d	Event %d State %d Peer %s Pktcnt %d	%d\n",
		...
		sm->tof_rxcnt);
#endif // endif
	TOF_PRINTF(("pdtof_sm End event = %s and state = %s mode = %s \n", tofevent_str[event],
		tofstate_str[sm->tof_state], tofmode_str[sm->tof_mode]));
	...
	struct ether_addr *selfmac, pdsvc_payload_t *payload)
{
	...
	if (tofobj != NULL) {
...
#endif /* TOF_COLLECT */
		tofobj->tofif.offload_result = NULL;
		...
		tunep->seq_5g20.w_len = 10;
		tunep->seq_5g20.w_offset = 8;
...
#ifdef TOF_DBG
int wlc_tof_dbg_seq_iov(pdmthd_if_t* svcif, uint32 flags, int* p_result)
{
	pdtof_obj_t* tofobj = (pdtof_obj_t*)svcif;
	wlc_info_t *wlc = tofobj->wlc;
	...
	if (p_result) {
		*p_result = 0;
		...
		return wlc_phy_tof_dbg(WLC_PI(wlc), (int)flags);
	}
}
#endif // endif
...
#ifdef WL_PROXDETECT_OL

int wlc_pdtof_ol_measure_results(uint64 *tx, uint64 *rx,
	int32 *gd, int32 *adj, int8 *rssip, bool *discard, uint16 id,
	bool initiator, bool acked, uint16 shmemptr, wlc_info_t *wlc,
	uint32 oldavbrx, uint32 oldavbtx, uint64 tq, uint8 tof_legacypeer,
	wl_proxd_params_tof_tune_t *tunep, wl_proxd_params_tof_method_t *paramp)
{
	pdtof_obj_t tofobj;
	pdtof_sm_t  sm;

	memset(&tofobj, 0, sizeof(pdtof_obj_t));
	memset(&sm, 0, sizeof(pdtof_sm_t));

	/* populate the temporary tofobj structure to do the pdtof measurements */
	tofobj.wlc = wlc;
	tofobj.shmemptr = shmemptr;
	tofobj.oldavbrx = oldavbrx;
	tofobj.oldavbtx = oldavbtx;

	tofobj.tunep = tunep;
	tofobj.paramp = paramp;
	tofobj.Tq = tq;

	tofobj.chipnum = CHIPID(wlc->pub->sih->chip);
	tofobj.chiprev = wlc->pub->sih->chiprev;

	sm.tof_legacypeer = tof_legacypeer;
	tofobj.sm = &sm;

	return wlc_pdtof_measure_results(&tofobj, tx, rx, gd, adj, id,
		acked, rssip, discard, initiator);
}
