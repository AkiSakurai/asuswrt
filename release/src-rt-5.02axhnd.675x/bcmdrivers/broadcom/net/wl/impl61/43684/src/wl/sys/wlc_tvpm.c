 ...
 *
 * $Id: wlc_tvpm.c 692406 2017-03-28 01:09:40Z $
 */
...
/* TVPM_ Ecounters wl_tvpm_ecounters_t version */
#define TVPM_ECOUNTERS_VERSION_V1	1
...
#if defined(ECOUNTERS)
static int
wlc_ecounters_tvpm_cb(uint16 stats_type, void *context,
...
#if defined(BCMDBG_DUMP) || defined(TVPM_DEBUG)
static void
...
static const bcm_iovar_t tvpm_iovars[] = {
	{"tvpm", IOV_TVPM,
	IOVF_RSDB_SET, IOVF2_RSDB_CORE_OVERRIDE, IOVT_BUFFER, 0
	},
	{NULL, 0, 0, 0, 0, 0}
};
...
#ifdef TXPWRBACKOFF
		if (!phy_tpc_txbackoff_enable(WLC_PI(tvpm->wlc),
			tvpm->txpwrbackoff_config)) {
...
#if defined(EVENT_LOG_COMPILE)
#if defined(ECOUNTERS)
	/* register TVPM ecounters' callback if ecounters enabled.
	* When RSDB enabled do it only once - for the MAC-0.
	*/
	if (ECOUNTERS_ENAB() &&
		((RSDB_ENAB(wlc->pub) && wlc->pub->unit == MAC_CORE_UNIT_0) ||
		(RSDB_ENAB(wlc->pub) == FALSE))) {
		if ((wl_ecounters_register_source(WL_IFSTATS_XTLV_WL_SLICE_TVPM,
			wlc_ecounters_tvpm_cb, (void*)wlc)) != BCME_OK) {
			WL_ERROR(("wl%d: %s: wl_ecounters_register_source failed\n",
				wlc->pub->unit, __FUNCTION__));
			goto fail;
		}
	}
		...
		const bcm_xtlv_t *tlv, uint16 *attempted_write_len)
{
	...
	int rc = BCME_OK;
	wl_tvpm_ecounters_t tvpm_ecnts;
	uint16 slice_index;

	/* Check that stats_type is valid */
	if (stats_type != WL_IFSTATS_XTLV_WL_SLICE_TVPM) {
		return BCME_ERROR;
	}

	/* If there is no space for at least one sta_info plus XTLV header,
	* specify sta_info size plus BCM_XTLV_HDR_SIZE in the *attempted_write_len and
	* return BCME_BUFTOOSHORT
	*/
	if (bcm_xtlv_buf_rlen(xtlvbuf) < (BCM_XTLV_HDR_SIZE + sizeof(tvpm_ecnts))) {
		*attempted_write_len = BCM_XTLV_HDR_SIZE + sizeof(tvpm_ecnts);
		return (BCME_BUFTOOSHORT);
	}

	/* set default values */
	memset(&tvpm_ecnts, 0, sizeof(tvpm_ecnts));
	tvpm_ecnts.version = TVPM_ECOUNTERS_VERSION_V1;
	tvpm_ecnts.length = sizeof(tvpm_ecnts);
	...
	/* read TVPM parameters from the current wlc and
	* write them to the wl_tvpm_status_t struct
	*/
	tvpm_ecnts.num_active_chains = wlc->tvpm->tx_active_chains;
	tvpm_ecnts.tx_dutycycle = wlc->tvpm->txduty_cycle;
	tvpm_ecnts.tx_power_backoff = wlc->tvpm->txpwr_backoff;
	tvpm_ecnts.temp = wlc->tvpm->input->temp;
	tvpm_ecnts.vbat = wlc->tvpm->input->vbat;
	tvpm_ecnts.cltm = wlc->tvpm->input->cltm;
	tvpm_ecnts.ppm = wlc->tvpm->input->ppm;

	rc = bcm_xtlv_put_data(xtlvbuf,
			stats_type,
			(const uint8 *) &tvpm_ecnts, sizeof(tvpm_ecnts));
	/* The following code needs to be modified if bcm_xtlv_put_data() fails to
	* write TVPM to the XLTV buffer (rc == BCME_NOMEM or BCME_BUFTOOSHORT),
	* to store a pointer to the current TVPM in the *cookie, which can be
	* used to re-try on the next call. In this case sta_info size plus
	* XTLV header would need to be stored in the *attempted_write_len:
	* *attempted_write_len = BCM_XTLV_HDR_SIZE + sizeof(sta_info);
	*/
	if (rc != BCME_OK) {
		WL_ERROR(("wl%d: %s: Not enough XLTV memory bcm_xtlv_put_data()"
			"failed with rc %d\n", wlc->pub->unit, __FUNCTION__, rc));
		/* Return BCME_OK to be consitent
		* with the previous implementation for now.
		*/
		return (BCME_OK);
	}
	return rc;
