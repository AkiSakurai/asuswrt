 ...
 *
 * $Id: pdftmsn.c 691086 2017-03-20 20:17:59Z $
 */
...
#ifdef WLAWDL
			(void)pdftm_extsched_awdl_unschedule(ftm, sn);
#endif /* WLAWDL */
		}
...
#ifdef WL_FTM_11K
	if (sn->lci_req) {
		MFREE(FTM_OSH(ftm), sn->lci_req, sn->lci_req->len);
		sn->lci_req = NULL;
	}
	if (sn->civic_req) {
		MFREE(FTM_OSH(ftm), sn->civic_req, sn->civic_req->len);
		sn->civic_req = NULL;
	}
	if (sn->lci_rep) {
...
ftm_session_tsf_scan_complete(void *ctx, int wlc_status, wlc_bsscfg_t *bsscfg)
{
	...
	case FTM_TSF_SCAN_STATE_PASSIVE_SCAN:
		...
		break;
	default:
		ASSERT(sn->tsf_scan_state == FTM_TSF_SCAN_STATE_ACTIVE_SCAN ||
