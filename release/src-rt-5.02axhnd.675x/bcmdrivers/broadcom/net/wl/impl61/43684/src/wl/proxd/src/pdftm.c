 ...
 *
 * $Id: pdftm.c 681973 2017-01-30 22:43:59Z $
 */
...
BCMATTACHFN(pdftm_attach)(wlc_info_t *wlc, pdsvc_t *pd)
{
	...
	ftm->scb_cubby_h = wlc_scb_cubby_reserve(wlc, sizeof(ftm_scb_t *),
		pdftm_scb_init, pdftm_scb_deinit, PDFTM_SCB_DUMP, ftm);
	if (ftm->scb_cubby_h < 0) {
		err = BCME_NORESOURCE;
		goto fail;
	}
...
int pdftm_ext_sched_cb(void *ctx, slotted_bss_notif_data_t *cb_data)
{
	...
	if ((cb_data->state == SB_RNG_START)) {
		...
		FTM_LOGSCHED(ftm, (("wl%d: %s: unhandled state %d for peer %s\n",
			FTM_UNIT(ftm), __FUNCTION__, cb_data->state,
			bcm_ether_ntoa(&cb_data->peer_mac_addr, eabuf))));
	}
