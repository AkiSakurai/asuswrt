 ...
 *
 * $Id: pdftmproto.c 690155 2017-03-14 23:38:37Z $
 */
...
ftm_proto_get_chaninfo(pdftm_t *ftm, pdftm_session_t *sn)
{
	...
	if (CHSPEC_IS10(chspec) && !RSPEC_ISHT(rspec)) {
		ci = FTM_PARAMS_CHAN_INFO_NON_HT_10;
	...
	uint8 *body, int body_len)
{
	...
	memset(&unknown_lci, 0, sizeof(unknown_lci));
	...
	uint8 *body, int body_len)
{
	...
	memset(&unknown_civic, 0, sizeof(unknown_civic));
	malloc_len = sizeof(*rrm_config) + TLV_BODY_LEN_MAX - DOT11_MNG_IE_MREP_FIXED_LEN;
 ...
 */
static void
ftm_proto_fill_lci(pdftm_t *ftm, pdftm_session_t *sn, uint8 **body, int *body_len)
{
 ...
 */
static void
ftm_proto_fill_civic(pdftm_t *ftm, pdftm_session_t *sn,	uint8 **body, int *body_len)
{
	dot11_rm_ie_t *rmrep_ie;
 ...
 */
static void
ftm_proto_fill_lci_req(pdftm_session_t *sn,	uint8 **body, int *body_len)
{
 ...
 */
static void
ftm_proto_fill_civic_req(pdftm_session_t *sn, uint8 **body, int *body_len)
{
	...
	const uint8 **body, int *body_len)
{
	...
	if (tlv) {
		int tlv_size = TLV_HDR_LEN + tlv->len;
	...
	const dot11_ftm_params_t *params, ratespec_t rx_rspec, pdftm_session_t *sn)
{
	...
	sncfg->burst_config->chanspec = wrxh->rxhdr.lt80.RxChan;
	...
	do {
		uint8 sn_chaninfo;
		err = ftm_proto_resolve_chanspec(ftm, sn,  wrxh->rxhdr.lt80.RxChan,
		                                 chaninfo, &chanspec);
		if (err != BCME_OK)
			break;
...
do_sep:
	 ...
	 */
	req_dur = FTM_PARAMS_BURSTTMO_USEC(FTM_PARAMS_BURSTTMO(params));
	...
	if (FTM_PARAMS_BURSTTMO(params) != FTM_PARAMS_BURSTTMO_NOPREF) {
	...
	const uint8 *body, int body_len, const wlc_d11rxhdr_t *wrxh, ratespec_t rspec)
{
	...
	FTM_LOGDECL(char eabuf[ETHER_ADDR_STR_LEN]);

	FTM_LOGPROTO(ftm, (("wl%d.%d: %s: handling request from %s\n",
	...
	if (sn != NULL) {
		...
		if (!req->trigger || (params != NULL)) {
			FTM_LOGPROTO(ftm,
				(("wl%d.%d: %s: canceling sn for %s, trig=%d; params %spresent\n",
				FTM_UNIT(ftm), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
				bcm_ether_ntoa(&hdr->sa, eabuf), req->trigger,
				((params != NULL) ? "" : "not "))));
			...
			if (!(sn->flags & FTM_SESSION_DELETE_ON_STOP)) {
				...
				FTM_INIT_SESSION_NOTIFY_INFO(&notify_info, NULL, &hdr->sa,
					WL_PROXD_EVENT_FTM_REQ, wrxh->rxhdr.lt80.RxChan,
		...
		if (params == NULL) { /* continue session */
			...
			if (!(ftm->ftm_cmn->config->flags &  WL_PROXD_FLAG_RX_AUTO_BURST)) {
				...
				FTM_INIT_SESSION_NOTIFY_INFO(&notify_info, NULL /* new session */,
					&hdr->sa, WL_PROXD_EVENT_FTM_REQ, wrxh->rxhdr.lt80.RxChan,
					rspec, (const uint8 *)req, req_len);
	...
	if (!sn) {
		if (!(ftm->ftm_cmn->config->flags & WL_PROXD_FLAG_RX_AUTO_BURST) && params) {
			pdftm_notify_info_t notify_info;
			FTM_INIT_SESSION_NOTIFY_INFO(&notify_info, NULL /* new session */,
				&hdr->sa, WL_PROXD_EVENT_FTM_REQ, wrxh->rxhdr.lt80.RxChan,
				rspec, (const uint8 *)req, req_len);
	...
	if (FTM_TSF_SYNC_SESSION(sn) && sn->ftm_state) {
	...
	do {
		...
		if (FTM_INITIATOR_ALLOW_CIOVRD(ftm, bsscfg)) {
			...
			err = ftm_proto_resolve_chanspec(ftm, sn,
				wrxh->rxhdr.lt80.RxChan, ci, &cspec);
		...
		dur = FTM_PARAMS_BURSTTMO_USEC(FTM_PARAMS_BURSTTMO(params));
		...
		if (FTM_INITIATOR_ALLOW_BPEROVRD(ftm, bsscfg)) {
			...
			if (bp != FTM_INTVL2MSEC(&sncfg->burst_period)) {
...
ftm_proto_process_meas_tlvs(pdftm_t *ftm, pdftm_session_t *sn,
	const uint8 **out_body, int *out_body_len, const dot11_ftm_params_t **params)
{
	const uint8 *body;
	int body_len;
	int err = BCME_OK;
	...
	const uint8 *body, int body_len, const wlc_d11rxhdr_t *wrxh, ratespec_t rspec)
{
	...
	FTM_LOGPROTO(ftm, (("wl%d.%d: %s: handling measurement from %s\n",
		FTM_UNIT(ftm), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
		bcm_ether_ntoa(&hdr->sa, eabuf))));
...
#define FTM_TOD_NOT_CONT 1
	...
	if (tsi != NULL) {
		...
		if (rsp->follow_up) {
			...
			memset(&rsp->tod_err, 0, sizeof(rsp->tod_err));
			memset(&rsp->toa_err, 0, sizeof(rsp->toa_err));
		}
		...
		memset(&rsp->dialog, 0, sizeof(dot11_ftm_t) - OFFSETOF(dot11_ftm_t, dialog));
	}
...
ftm_proto_prep_tx(pdburst_method_ctx_t ctx, pdburst_frame_type_t type, void *pkt, uint8 *body,
    int body_max, int *body_len, const pdburst_tsinfo_t *tsinfo)
{
	int err = BCME_OK;
	pdftm_session_t *sn = (pdftm_session_t *)ctx;
...
pdftm_proto_notify(pdftm_t *ftm, pdftm_session_t *sn, wl_proxd_event_type_t event_type)
{
	...
	if ((event_type == WL_PROXD_EVENT_SESSION_END) &&
		...
		/* notify peer if session ended via local cancel */
		if (FTM_SESSION_IS_INITIATOR(sn) && (sn->flags & FTM_SESSION_FTM1_SENT)) {
