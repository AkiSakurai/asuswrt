		...
		if ((err = wlc_keymgmt_recvdata(wlc->keymgmt, f)) != BCME_OK) {
			...
			if (err == BCME_REPLAY) {
				WLCNTSCB_COND_INCR((f->rx_wep),
					scb->rx_succ_replay_failures);
				if (BSSCFG_AP(bsscfg) &&
					(WLCNTSCBVAL(scb->rx_succ_replay_failures) >=
	...
	wlc_d11rxhdr_t *wrxh, struct scb **pscb, int len)
{
	...
	if (class == 4) {
		...
		else if (!(SCB_A4_DATA(scb)) && !((SCB_DWDS_CAP(scb) || SCB_MAP_CAP(scb)) && SCB_ASSOCIATED(scb))) {
