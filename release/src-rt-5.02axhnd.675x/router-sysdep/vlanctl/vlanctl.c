...
static void usage(void)
{
    ...
    printf("\t--show-table Lists all Tagging Rules stored in the specified Tagging Rule Table. Dependencies: --if, --rx or --tx, and"
           "\n\t--tags.\n\n");
    printf("\t--rule-dump-all Lists all Tagging Rules \n\n");
    ...
    printf("\t--set-iptv Set flag to indicate IPTV traffic will go through the mcast vlan instead of WAN vlan.\n\n");
...
int main(int argc, char **argv)
{
    ...
    int isMulticast = 0;
    int isSwOnly = 0;
    vlanCtl_createParams_t createParams;
    ...
    while(!ret && argc)
    {
        ...
        else if(!strcmp(*argv, "--if-create"))
        {
            ...
            if(!ret)
            {
                createParams.isRouted = isRouted;
                createParams.isMulticast = isMulticast;
                createParams.isSwOnly = isSwOnly;
                ret = vlanCtl_createVlanInterfaceExt((char *)arg[0], arg[1],
                  &createParams);
            }
        }
        else if(!strcmp(*argv, "--if-create-name"))
        {
            ...
            if(!ret)
            {
                ...
                createParams.isMulticast = isMulticast;
                createParams.isSwOnly = isSwOnly;
                ret = vlanCtl_createVlanInterfaceByNameExt((char *)arg[0], (char *)arg[1], 
                  &createParams);
            }
        }
        ...
        else if(!strcmp(*argv, "--rule-dump-all"))
        {
            commandCount++;
            ret = vlanCtl_dumpAllRules();
        }
