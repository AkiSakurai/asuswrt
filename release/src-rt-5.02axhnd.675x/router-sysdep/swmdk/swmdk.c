...
#if defined(CHIP_4908) || defined(CHIP_63158) || defined(CHIP_63178)
...
static phy_driver_t *phy_drv_list[] = {
#if BRCM_EXT_SWITCH_TYPE == 63100 || defined(CHIP_63178)
    &bcm63100_drv,
#endif
...
/* CONFIG_MDK_BCA_BEGIN */
    ...
    for(unit = 0; unit < BMD_CONFIG_MAX_UNITS; unit++) {
        ...
        // For 63168 and 63381, vendor_id is set to 0x6300 in enet_ioctl_ethsw_info 
        if ((unit == 0) && (vendor_id == 0x6300)) {
            ...
            for (port = 0; port < MAX_SWITCH_PORTS; port++) {
                ...
                if (phypbmp & mask) {
                    ...
                    if(link) {
                        ...
                        if  ((duplex != prev_dplx) || (speed != (int) prev_speed[port])) {
                            phy_link_changed = 1;
                        }
                    }
                    ...
                    if (phy_link_changed) {
                        ...
                        mdk_kernel_poll.link_change = ETHSW_LINK_CHANGED;
                    }
