...
"               [{-t|--type} <type>] [{-v|--value} <val>] [-d <ques_per_port>] [-n <unit>] [-p <port>]\n"
...
"          type = 0-read all;"
                  ...
                  #endif
                  "\n"
"                 5-total_xon_hyst 256B unit; 6-queue_xon_hyst 256B unit; 7-pessimistic_mode;\n"
...
"                 12-total_xoff_thresh 256B unit; 13-xoff_thresh 256Bytes unit;"
                  ...
                  #endif
                  "\n"
"                 100-dump peak usages for all queues\n"
                  #if defined(ACB_ALGORITHM2)
"                 Setting 4-Algorithm will set multiple fields to default values for that algorithm.\n"
                  #endif
                   ,
"    ethswctl {-c|--command} maclmt [-n <unit>] -p <port> [-t <type>] [-v <val>]\n"
"        type = 0-portLimitEnable, 1-portLimit, 2-portAction, 3-portlearnedCnt, 4-portOvrLimitPktCnt, 5-portRstOvrLimitPktCnt\n"
"               6-globalLimit, 7-globalLearnedCnt, 8-globalRstOvrLimitPktCnt\n"
"             * no parameter - dump mac limit config/stat for all ports\n"
"             * type 2-portAction after exceeding limit: 0-normal forward, 1-drop, 2-copy to cpu, 3-forward to cpu only\n",
#if defined(HAS_SF2_CFP)
...
"    ethswctl {-c|--command} untaggedprio [-n <unit>] -p <port> [-v <val>]\n"
"          Set/Get packet default untagged priority in Rx Port <port> with value <val>\n",
#if !defined(HAS_SF2)
"    ethswctl {-c|--command} cosqpriomethod [-n <unit>] -p port [-v <val>]\n"
"          val = 0-PORT_QOS, 1-MAC_QOS, 2-8021P_QOS, 3-DIFFSERV_QOS\n",
"    ethswctl {-c|--command} cosqsched -v [val] -t <sp_endq> -w <weight1> -x <weight2>\n"
"               -y <weight3> -z <weight4>\n"
"          val = 0-SP, 1-WRR, 2-COMBO\n"
"          sp_endq: Queue where SP scheduling ends. Valid only for COMBO\n",
#else
"    ethswctl {-c|--command} cosqpriomethod [-n <unit>] [-p port] -t <pkt_type_mask> [-v <val>] [-b]\n"
"          pkt_type_mask = 0x4-Static DA MAC, 0x2-Priority Tagged, 0x1-ipv4/6, 8-All Types\n"
"             Note: 0x7 means a frame with Da in ARL, Tagged and IPv4, but not All Type\n"
"          val = 0-PORT_QOS, 1-MAC_QOS, 2-8021P_QOS, 3-DIFFSERV_QOS\n"
"              When pkt_type_mask is not 8(All Type), <val>'s bit 17 is used to enable/disable this QoS\n"
"              When pkt_type_mask is 8(All Type), val's low 16 bits contains 8 TC Select for 8 packet\n"
"                  type combination, bit16-24 contains 8 enable bits to enable/disable the QoS\n"
"          -b: Disable specifc QoS of a port(For 8021P_QOS and DIFFSERV_QOS)\n",
"    ethswctl {-c|--command} cosqsched [-n <unit>] -p <port> [-v <val>] [-t <sp_queues>] [ -T <deficit_rr>]\n"
...
"    ethswctl {-c|--command} portloopback [-n <unit>] {-p|--port} <port[.sub_port]> [-v <val>] [-s <speed>]\n"
"          val:  1-Enable, 0-Disable\n"
"          speed: speed in the Mbps; If omitted, maximum interface speed will be used\n"
"          sub_port for -p: needed if PHY is behind a crossbar with multiple member ports\n",
...
"    ethswctl {-c|--command} pid2prio [-n <unit>] -p <port> [-q <prio>]\n"
...
"    ethswctl {-c|--command} ifstp [-i] <if_name> [-o <disabled|blocking|listening|learning|forwarding>]\n",
...
"                         Zero iterations will cause keepalive to run in monitor mode\n"
"                         In monitor mode, keepalive will run every <sleep> seconds (minimum 60).\n",
#endif
"    ethswctl {-c|--command} createport -p <port> -w <is_wan> -i <name>\n",
"    ethswctl {-c|--command} deleteport -p <port>\n",
"    ethswctl {-c|--command} gponmcastgemset -p <mcast_gem_idx>\n",
};
...
int ethswctl_control_priority_op(int unit, int port, int queue, int type, unsigned int *val, 
    int *thrMode, int *maxStr, int *actQue, int get);
#define ethswctl_control_priority_get(unit, port, queue, type, val, thrMod, maxStre, actQue) \
    ethswctl_control_priority_op(unit, port, queue, val, manMode, maxStr, actQue, 1)
#define ethswctl_control_priority_set(unit, port, queue, type, val, thrMode, maxStr, actQue) \
    ethswctl_control_priority_op(unit, port, queue, val, manMode, maxStr, actQue, 0)
