...
#if defined(R8000P)
	nvram_set_int("switch_led_gpio", 0xff);
#endif
...
//		if (!nvram_get("ct_max"))
	...
	case MODEL_GTAC5300:
		...
		nvram_set("wired_ifnames", "eth1 eth2 eth3 eth4 bond0");
#endif

		nvram_set("1:ledbh9", "0x1");
		nvram_set("2:ledbh9", "0x1");
		nvram_set("3:ledbh9", "0x1");

		nvram_set_int("led_pwr_gpio", 8|GPIO_ACTIVE_LOW);
		nvram_set_int("led_pwr_red_gpio", 9|GPIO_ACTIVE_LOW);
		//nvram_set_int("led_wan_gpio", 12|GPIO_ACTIVE_LOW);//12/13
		nvram_set_int("led_2g_gpio", 14|GPIO_ACTIVE_LOW);
		nvram_set_int("led_5g_gpio", 15|GPIO_ACTIVE_LOW);
		nvram_set_int("led_5g2_gpio", 16|GPIO_ACTIVE_LOW);
		nvram_set_int("led_usb3_gpio", 18|GPIO_ACTIVE_LOW);		//usb
		nvram_set_int("led_usb_gpio", 17|GPIO_ACTIVE_LOW);//r8000p usb2.0
		nvram_set_int("led_guest_gpio", 17|GPIO_ACTIVE_LOW);//r7900p guest wifi

		nvram_set_int("led_wifi_gpio", 56|GPIO_ACTIVE_LOW);		//wifi on/off
		nvram_set_int("led_wps_gpio", 10|GPIO_ACTIVE_LOW);	
		nvram_set_int("led_wan_normal_gpio", 12|GPIO_ACTIVE_LOW);	
		nvram_set_int("led_wan_gpio", 13|GPIO_ACTIVE_LOW);	


		nvram_set_int("switch_led_gpio", 52|GPIO_ACTIVE_LOW);	//led on/off
		nvram_set_int("btn_rst_gpio", 53|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_wps_gpio", 54|GPIO_ACTIVE_LOW);
		nvram_set_int("btn_wltog_gpio", 55|GPIO_ACTIVE_LOW);	//wifi on/off
...
//		nvram_set("bcm43684_nic", "1");	// bring up bcm43684 in nic mode
	...
	case MODEL_RPAX56:
		...
		if(!nvram_match("no_atf", "1")) {
			_dprintf("prefer to enable atf\n");
			nvram_set("wl_atf", "1");
			nvram_set("wl1_atf", "1");
			nvram_set("wl1_atf", "1");
		}
...
#if defined(RTCONFIG_BCM_7114) || (defined(HND_ROUTER) && !defined(RTCONFIG_HND_ROUTER_AX) && !defined(R8000P))
			if(!factory_debug()) {
				check_4366_dummy();
				sleep(1);
				check_4366_fabid();
