...
init_switch_misc()
{
	...
	int mode = nvram_get_int("ethctl_flag");
#endif
	...
	foreach(word, ifnames, next) {
		if (!strcmp(word, WAN_IF_ETH)) {
			if (mode & ETHCTL_PHYRESET)
				doSystem("ethctl %s phy-reset", WAN_IF_ETH);
			else {
				if (mode & ETHCTL_EEE_OFF)
					doSystem("ethctl %s eee off", WAN_IF_ETH);
				if (mode & ETHCTL_APD_OFF)
					doSystem("ethctl %s apd off", WAN_IF_ETH);
				if (!(mode & ETHCTL_WIRESPEED_OFF))
					doSystem("ethctl %s ethernet@wirespeed enable", WAN_IF_ETH);
			}
		} else
