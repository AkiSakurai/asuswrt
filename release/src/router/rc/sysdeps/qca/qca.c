...
int create_tmp_sta(int unit, const char *sta)
{
	char conf[64];
	...
	if(status < 0) {
		...
		if(nvram_get_int("x_Setting") && (sw_mode()==SW_MODE_AP && !nvram_match("cfg_master", "1")))
			_dprintf("==>wps enrollee: using original %s vap\n",sta);
		else
#endif
		{
			_dprintf("==>wps enrollee: create %s vap\n",sta);
			doSystem("wlanconfig %s create wlandev %s wlanmode sta nosbeacon", sta, get_vphyifname(unit));
			sleep(1);
		}
		ifconfig(sta, IFUP, NULL, NULL);
		eval("/usr/bin/wpa_supplicant", "-B", "-P", pid_file, "-D", get_wsup_drvname(unit), "-i", sta, "-b", lan_if, "-c", conf);
	}
	return 0;
}

void destroy_tmp_sta(const char *sta)
{
	char fpath[64];
...
void amas_save_wifi_para(void)
{
	...
	char ssid[33];
	char key_mgmt[16];
	char tmp[128], prefix[] = "wlcXXXXXXXXX_";
	char word[256], *next;
	char ifnames[128];
	int i;
...
void acs_ch_weight_param()
{
	...
	for (; acs_chweight_p->model!=0; acs_chweight_p++) {
		if (!strncmp(country, acs_chweight_p->country, sizeof(country))) {
