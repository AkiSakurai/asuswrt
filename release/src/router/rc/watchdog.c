...
*/
	...
	if (!btn_pressed && !btn_pressed_setup) {
		if (val) {
			...
			if (btn_led_pressed == 1)
			{
				int ledg_mode = nvram_get_int("ledg_mode");
				int ledg_color = nvram_get_int("ledg_color");

				char nvname[32];
				memset(nvname, 0, sizeof(nvname));
				sprintf(nvname, "ledg_rgb%d_valid", ledg_mode + 1);
				ledg_color = (ledg_color + 1) % (nvram_get_int(nvname) ? 5 : 4);
				nvram_set_int("ledg_color", ledg_color);
				nvram_commit();

				dbg("switch color\n");
				notify_rc("restart_ledg");
			}
			else if (btn_led_pressed == 2)
			{
				int ledg_mode = nvram_get_int("ledg_mode");

				ledg_mode = (ledg_mode + 1) % LEDG_MODE_MAX;
				nvram_set_int("ledg_color", 0);
				nvram_set_int("ledg_mode", ledg_mode);
				nvram_commit();

				if (ledg_mode == LEDG_STEADY_MODE || ledg_mode == LEDG_OFF)
					LED_status = 1;
