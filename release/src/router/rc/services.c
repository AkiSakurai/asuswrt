...
start_acsd()
{
	...
	int i, allon=1, gueston=0, MAX = 0;	// if MAX = 1: single band, 2: dual band
	MAX = num_of_wl_if();
	for(i = 0; i < MAX; i++) {
		snprintf(prefix, sizeof(prefix), "wl%d_", i);
		if (0==nvram_get_int(strcat_r(prefix, "radio", tmp))){
			set_gpio(14+i,1);
			allon=0;	
		}
		else {
			...
			if(nvram_get_int("wlc_psta")==0){
				if(strncmp(nvram_safe_get(strcat_r(prefix, "vifs", tmp)), "wl", 2) == 0)
				gueston=1;
			}else if(nvram_get_int("wlc_psta")==2){
				if(i==(nvram_get_int("wlc_band"))) continue;
				if(strncmp(nvram_safe_get(strcat_r(prefix, "vifs", tmp)), "wl", 2) == 0) gueston=1;
			}					
		}
	}
	if (allon)
