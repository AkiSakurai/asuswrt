 ...
 *
 * $Id: wpa_auth_ft.c 672244 2016-11-25 08:41:22Z $
 */
                 ...
                 unsigned char *buf, size_t len)
{
	...
	if (ntohs(ethhdr->h_proto) == ETH_P_RRB) {
		...
		wpa_ft_rrb_rx(wpa, ethhdr->h_source, ethhdr->h_dest, buf, len);
		return;
	}
}
...
int setup_rrb_socket(wpa_t *wpa, char *ifname)
{
	...
	l2_data = l2_packet_init(ifname, ETH_P_RRB,
		broadcom_handle_data, wpa);
	if (l2_data == NULL) {
		dbg(nas, "Could not setup RRB socket, interface name=%s\n", ifname);
		return -1;
	}

	nas->l2_rrb = l2_data;
	nas->l2_rrb_fd = l2_data->fd;

	dbg(nas, "Setup RRB socket, interface name=%s\n", ifname);
...
static int wpa_ft_rrb_send(wpa_t *wpa, uint8 *dst, uint8 *data, size_t data_len)
{
	...
	if (wpa->nas->l2_rrb != NULL) {
		res = l2_packet_send(wpa->nas->l2_rrb, (uint8 *) eth, len);
	} else {
...
struct wpa_ft_pmk_cache {
	struct wpa_ft_pmk_r0_sa *pmk_r0;	/* PMKR0 for all the STA's */
	struct wpa_ft_pmk_r1_sa *pmk_r1;	/* PMKR1 for all the STA's */
};
	...
	size_t r0kh_id_len, const uint8 *pmk_r0_name)
{
	...
	memcpy(f.nonce, wpa->global_key_counter, sizeof(f.nonce));
	wpa_incr_gkc(wpa);
	...
	const char *pmk, unsigned char *ptk, size_t ptk_len)
{
	...
	wpa_calc_pmkR0((uchar *)ssid, ssid_len, md_id, (uint8 *)r0kh, r0kh_len, &sta->ea,
		(uint8 *)pmk, strlen((char *)pmk)-1, pmk_r0, pmk_r0_name);
	...
	int *pairwise, struct wpa_ft_ies *parse)
{
	...
	if (wpa_ft_fetch_pmk_r1(wpa, sta->ea.octet, pmk_r1_name, pmk_r1,
		pairwise) < 0) {
...
static int32 wpa_ft_prepare_auth_resp(wpa_t *wpa, nas_sta_t *sta, uint8 *pmk_r1,
	struct wpa_ft_ies parse,  int pairwise, uint8 **resp_ies, uint32 *resp_ies_len)
{
	uint8 *pos, *end;
...
static int wpa_ft_process_otd_auth(wpa_t *wpa, nas_sta_t *sta, const uint8 *current_ap,
	uint8 *resp_ies, uint32 resp_ies_len, uint16 status)
{
...
static int wpa_ft_rrb_rx_request(wpa_t *wpa,
	const uint8 *current_ap, const uint8 *sta_addr,
	const uint8 *body, size_t len)
{
	...
	err = wpa_ft_process_otd_auth(wpa, nas_sta, current_ap, resp_ies, resp_ies_len, status);
	free(resp_ies);
	resp_ies = NULL;
		...
		if (pend_auth_req[pend_auth_iter].valid == TRUE &&
			...
			if (pend_auth_req[pend_auth_iter].overds == TRUE) {
				...
				wpa_ft_process_otd_auth(wpa, pend_auth_req[pend_auth_iter].sta,
					pend_auth_req[pend_auth_iter].current_ap, resp_ies,
					resp_ies_len, status);
				free(resp_ies);
				resp_ies = NULL;
			} else {
	...
	const uint8 *data, size_t data_len)
{
	...
	if (memcmp(src_addr, own_addr, ETH_ALEN) == 0) {
		dbg(wpa->nas, "FT: RRB received frame from remote AP %s matching"
			"own address dropping", eabuf_tmp1);
		return -1;
	}
	...
	if (frame->packet_type == FT_PACKET_R0KH_R1KH_PULL)
		return wpa_ft_rrb_rx_pull(wpa, (uint8 *)src_addr, (uint8 *)data, data_len);
	...
	if (frame->packet_type == FT_PACKET_REQUEST) {
		...
		if (wpa_ft_rrb_rx_request(wpa, src_addr,
