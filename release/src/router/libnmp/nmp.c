...
int get_ipv6_routing_info(ipv6_routing_info_t *ipv6_routing_info)
{
	...
	while ((str = fgets(buf, sizeof(buf), fp)) != NULL) {
		...
		snprintf(ipv6_routing_info[count].dest,
			 sizeof(ipv6_routing_info[count].dest), "%s", sdest);

		/* Parse nexthop, reuse buf */
		if (inet_raddr6_pton(snexthop, &nexthop, str) < 1)
			break;
		inet_ntop(AF_INET6, &nexthop, snexthop, sizeof(snexthop));
		snprintf(ipv6_routing_info[count].next_hop,
			 sizeof(ipv6_routing_info[count].next_hop), "%s", snexthop);
		...
		*str++ = '\0';
		snprintf(ipv6_routing_info[count].flags,
			 sizeof(ipv6_routing_info[count].flags), "%s", sflags);

		ipv6_routing_info[count].metric = metric;
		ipv6_routing_info[count].ref = ref;
		ipv6_routing_info[count].use = use;

		snprintf(ipv6_routing_info[count].iface,
			 sizeof(ipv6_routing_info[count].iface), "%s", ifname);
