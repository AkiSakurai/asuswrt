...
static void show_speed_setting(int skfd, struct ifreq *ifr, int phy_id, int sub_port)
{
    ...
    if (ifdata->phycfg & PHY_CFG_AUTO_NEGO)
        printf("    Auto-%s enabled, with capabilities: ", 
            (ETHCTL_GET_FLAG_FROM_PHYID(phy_id) & ETHCTL_FLAG_ACCESS_SERDES)? "Detection of Serdes": "Negotiation");
    else
        printf("    Auto-%s disabled, with fixed speed: ",
            (ETHCTL_GET_FLAG_FROM_PHYID(phy_id) & ETHCTL_FLAG_ACCESS_SERDES)? "Detection of Serdes": "Negotiation");

    if (ifdata->phycfg & PHY_CFG_2500FD) printf("%s2.5GFD", middle++? "|":"");
    if (ifdata->phycfg & PHY_CFG_1000FD) printf("%s1GFD", middle++? "|":"");
    if (ifdata->phycfg & PHY_CFG_1000HD) printf("%s1GHD", middle++?"|":"");
    if (ifdata->phycfg & PHY_CFG_100FD) printf("%s100MFD", middle++?"|":"");
    if (ifdata->phycfg & PHY_CFG_100HD) printf("%s100MHD", middle++?"|":"");
    if (ifdata->phycfg & PHY_CFG_10FD) printf("%s10MFD", middle++?"|":"");
    if (ifdata->phycfg & PHY_CFG_10HD) printf("%s10MHD", middle++?"|":"");
    printf("\n");
