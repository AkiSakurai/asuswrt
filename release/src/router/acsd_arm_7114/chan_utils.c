...
chanspec_t
select_80M_chspec(acs_chaninfo_t *c_info, chanspec_t chanspec)
{
	...
	if (CHSPEC_IS5G(chanspec))
	{
		...
		for (i = 0; i < count; i++) {
			...
			if (CHSPEC_IS80(c) &&
				wf_chspec_ctlchan(c) == wf_chspec_ctlchan(chanspec))
				return c;
		}
	}
...
chanspec_t
select_40M_chspec(acs_chaninfo_t *c_info, chanspec_t chanspec)
{
	wl_uint32_list_t *list;
	chanspec_t c = 0;
	int ret = 0, i, count;
...
int
CTLCH_IS_BAND1(chanspec_t chspec)
{               
	uint ctl_ch;
	const uint8 *ctl_chs = wf_5g_80m_ctl_chans_band1;
	int num_ch = WF_NUM_5G_80M_CTL_CHANS_BAND1;
	int i, match = 0;
