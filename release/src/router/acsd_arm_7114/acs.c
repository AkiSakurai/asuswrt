 ...
 *
 * $Id: acs.c 654154 2016-08-11 08:59:21Z $
 */
...
txpwr_2gstrategy_t txpr_2gscore_88u[TXPS_88U_2GMAX] = {
{{ 15, 30, 30, 30, 30, 30, 30, 30, 30, 30, 20,  0,  0 }},      /* TXPS_88U_2A */
{{ 15, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 25, 15 }},      /* TXPS_88U_2B */
{{ 15, 30, 30, 30, 30, 30, 30, 30, 30, 30, 20,  0,  0 }},      /* TXPS_88U_2C */
};

/* band */
/* 1   2   3    4  */
txpwr_5gstrategy_t txpr_5gscore_88u[TXPS_88U_5GMAX] = {
{{ 20,  0,  0,  30 }}, /* TXPS_88U_5A */ /* band5grp : 9 , 1/8 */
{{ 20, 18, 30,   0 }}, /* TXPS_88U_5B */ /*         : 7 , 3/4 */
{{ 15, 14, 22,  30 }}, /* TXPS_88U_5C */ /*         : f , 3,c */
{{  0,  0,  0,  30 }}, /* TXPS_88U_5D */ /*         : 8 ,     */
{{ 28,  0,  0,  32 }}, /* TXPS_88U_5E */ /*         : 9 , 1/8 */
};
...
acs_dump_map(void)
{
	...
	for (i = 0; i < ACS_MAX_IF_NUM; i++) {
		...
		if (cur_map->in_use) {
			ACSD_DEBUG("i: %d, name: %s, idx: %d, in_use: %d\n",
...
acs_dump_chan_bss(acs_chan_bssinfo_t* bssinfo, int ncis)
{
	...
	ACSD_INFO("channel nCtrl nExt20 nExt40 nExt80\n");
	for (c = 0; c < ncis; c++) {
		cur = &bssinfo[c];
		ACSD_INFO("%3d  %5d%6d%7d%7d\n", cur->channel, cur->nCtrl,
...
display_scan_entry_local(acs_bss_info_sm_t * bsm)
{
	...
	ACSD_INFO("SSID: \"%s\"\n", ssidbuf);
	ACSD_INFO("BSSID: %s\t", wl_ether_etoa(&bsm->BSSID));
	ACSD_INFO("chanspec: 0x%x (%s)\n", bsm->chanspec, wf_chspec_ntoa(bsm->chanspec, chanspecbuf));
	ACSD_INFO("RSSI: %d dBm\t", (int16)bsm->RSSI);
	ACSD_INFO("Type: %s", ((bsm->type == ACS_BSS_TYPE_11A) ? "802.11A" :
...
acs_pick_chanspec(acs_chaninfo_t* c_info, int bw)
{
	...
	for (i = 0; i < c_info->c_count[bw]; i++) {
		...
		if (index < 0) { /* No previous candi, avoid comparing against random memory */
			index = i; /* Select first valid candidate as a starting point */
			ACSD_INFO("[%d] Default: %s channel #%d (0x%x) (%s) with score %d\n",
				i, (candi[i].is_dfs) ? "DFS" : "non-DFS",
		...
		ACSD_INFO("[%d] Checking %s channel #%d (0x%x) (%s) with score %d\n",
			i, (candi[i].is_dfs) ? "DFS" : "non-DFS",
			CHSPEC_CHANNEL(candi[i].chspec), candi[i].chspec, wf_chspec_ntoa(candi[i].chspec, chanspecbuf),
			candi[i].chscore[score_type].score);
	...
	if (index < 0) {
		...
		chspec = candi[index].chspec;
		ACSD_INFO("The highest score: %d, chspec: 0x%x (%s)\n",
			candi[index].chscore[score_type].score,
...
{
	ACSD_INFO("ACS Policy:\n");
	ACSD_INFO("Bg Noise threshold: %d\n", a_pol->bgnoise_thres);
	ACSD_INFO("Interference threshold: %d\n", a_pol->intf_threshold);
	ACSD_INFO("Channel Scoring Weights: \n");
...
static int
get_txp_strategy(int band, int model, unsigned int band5grp)
{
        char *ccode = nvram_safe_get("0:ccode");
        char *reg = nvram_safe_get("0:regrev");

        switch(model) {
                case MODEL_RTAC88U:
                case MODEL_RTAC3100:
                        if(band == 2) {
                                if((!strncmp(ccode, "Q2", 2)&&!strncmp(reg, "992", 3)) || !strncmp(ccode, "CA", 2) || !strncmp(ccode, "TW", 2) || !strncmp(ccode, "AU", 2))
                                        return TXPS_88U_2A;
                                else if(!strncmp(ccode, "E0", 2) || !strncmp(ccode, "JP", 2) || !strncmp(ccode, "CN", 2) || !strncmp(ccode, "KR", 2))
                                        return TXPS_88U_2B;

                                else if(!strncmp(ccode, "SG", 2) || !strncmp(ccode, "US", 2) || (!strncmp(ccode, "Q1", 2)&&!strncmp(reg, "947", 3)))
                                        return TXPS_88U_2C;
                                else
                                        return TXPS_88U_2A;
                        } else if (band == 5) {
                                ...
                                case 8:  /* Band 4 */
                                        return TXPS_88U_5D;
                                case 9:  /* Band 1, 4 */
                                        if(!strncmp(ccode, "US", 2) || (!strncmp(ccode, "Q1", 2)&&!strncmp(reg, "947", 3)))
                                                return TXPS_88U_5E;
                                        else
                                                return TXPS_88U_5A;
                                case 0xf:/* Band 1, 2, 3 */
                ...
                case MODEL_RTAC5300:
                        if(band == 2) {
                                if((!strncmp(ccode, "Q1", 2)&&!strncmp(reg, "984", 3)) || !strncmp(ccode, "CA", 2) || !strncmp(ccode, "TW", 2) || !strncmp(ccode, "AU", 2))
                                        return TXPS_88U_2A;
                                else if(!strncmp(ccode, "E0", 2) || !strncmp(ccode, "CN", 2))
                                        return TXPS_88U_2B;
...
acs_candidate_score_txpwr(ch_candidate_t *candi, acs_chaninfo_t* c_info)
{
        chanspec_t chspec = candi->chspec;
        acs_channel_t chan;
        int score = 0, model, band;
        int unit = -1;
        char prefix[PREFIX_LEN], tmp[100];
        unsigned int band5_grp = 0;
        ...
        switch (model) {
                ...
                case MODEL_RTAC5300:
                        if (BAND_2G(c_info->rs_info.band_type))
                        {
                                score += txpr_2gscore_88u[get_txp_strategy(2, model, 0)].txpwr_score[chan.control - 1];
                                if (chan.control != chan.ext20)
                                        score += txpr_2gscore_88u[get_txp_strategy(2, model, 0)].txpwr_score[chan.ext20 - 1];
                        }
                        else
                        {
                                ...
                                if(unit < 0 || unit > 2)
                                        unit = 1;
                                snprintf(prefix, sizeof(prefix), "wl%d_", unit);
                                band5_grp = atoi(nvram_safe_get(strcat_r(prefix, "band5grp", tmp)));
                                band = CHAN5G_BAND_NUM(chan.control);

                                score += txpr_5gscore_88u[get_txp_strategy(5, model, band5_grp)].txpwr_score[band - 1];
                        }
                        ACSD_DEBUG("txpwr score ++ %d w/ chanspec 0x%x (%s)\n", score, chspec, wf_chspec_ntoa(chspec, chanspecbuf));
                        break;
        }

        candi->chscore[CH_SCORE_TXPWR].score = score;

        ACSD_DEBUG("txpwr score: %d for chanspec 0x%x (%s)\n", score, chspec, wf_chspec_ntoa(chspec, chanspecbuf));
}
	...
	for (i = 0; i < c_info->c_count[bw]; i++) {
		...
		if (ACS_FCS_MODE(c_info)) {
			...
			if (!acs_is_initial_selection(c_info)) {
				...
				if (!candi[i].is_dfs && c_info->acs_fcs.acs_dfs != ACS_DFS_DISABLED) {
					/* Need to pick DFS channels on bootup - Invalidate Non-DFS channels
					*/
					candi[i].reason |= ACS_INVALID_NONDFS;
				}
...
recheck:
	...
	if (acs_has_valid_candidate(c_info, bw)) {
		...
		if ((bw > ACS_BW_80) || (need_coex_check && (bw == ACS_BW_40))) {
...
acs_get_dfs_forced_chspec(acs_chaninfo_t *c_info, char smbuf[WLC_IOCTL_SMLEN])
{
	...
	if (BAND_2G(rsi->band_type) || !ACS_FCS_MODE(c_info) ||
		(c_info->acs_fcs.acs_dfs == ACS_DFS_DISABLED)) {
		return -1;
	}
...
acs_init_run(acs_info_t ** acs_info_p)
{
	...
	strncpy(acs_ifnames, nvram_safe_get("acs_ifnames"), sizeof(acs_ifnames) - 1);
