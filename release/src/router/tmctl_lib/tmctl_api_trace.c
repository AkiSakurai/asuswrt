...
{
    __print("--qid %d --priority %d --qsize %d --weight %d --schedmode %d ",
            pQueueCfg->qid, pQueueCfg->priority, pQueueCfg->qsize,
            pQueueCfg->weight, pQueueCfg->schedMode);
...
static void __queueDropAlgExtTrace(tmctl_dropAlg_e dropAlgorithm,
                                   ...
                                   tmctl_queueDropAlgExt_t *dropAlgHi_p)
{
    __print("--dropalg %d --loredminthr %d --loredmaxthr %d --loredpct %d "
            ...
            dropAlgorithm, dropAlgLo_p->redMinThreshold, dropAlgLo_p->redMaxThreshold,
                                  ...
                                  tmctl_if_t *if_p, uint32_t cfgFlags)
{
    ...
    __trace(porttminit, "--devtype %d --if %s --flag %d\n",
            devType, ifName, cfgFlags);
                                          ...
                                          tmctl_queueDropAlgExt_t *dropAlgHi_p)
{
    ...
    __queueDropAlgExtTrace(dropAlgorithm, dropAlgLo_p, dropAlgHi_p);
