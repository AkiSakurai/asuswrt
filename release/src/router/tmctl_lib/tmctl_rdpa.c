...
#define getDir(devType, rdpaIf) \
  ((((devType == RDPA_IOCTL_DEV_PORT) && (rdpaIf > rdpa_if_wan1)) ||\
  (devType == RDPA_IOCTL_DEV_NONE))? rdpa_dir_ds : rdpa_dir_us)
#if defined(CHIP_6838) || defined(CHIP_6848)
...
#endif /* defined(CHIP_6838) || defined(CHIP_6848) */
...
static tmctl_ret_e addQueue(int devType, int rdpaIf, rdpa_traffic_dir dir,
                            ...
                            int minRate, int shapingRate, int burst, BOOL qClean);
                     ...
                     int minRate, int shapingRate, int burst, BOOL qClean)
{
   ...
   if (tmMode == rdpa_tm_sched_disabled)
   {
#if defined(CONFIG_BCM96858) && defined(EPON_HGU) && defined(SUPPORT_RDPA)
      ...
      if (rdpaCtl_get_wan_type(&wan_type) == 0 && (wan_type == rdpa_wan_epon || wan_type == rdpa_wan_xepon))
      {
          return addQueueHandleEponCtcCase(devType, rdpaIf, dir, parentTmId, qid,
              qsize, tmMode, prioIndex, weight, minRate, shapingRate, burst, qClean);
      }
#endif      
                        ...
                        int shapingRate, int burst)
{
   ...
   rc = rdpaCtl_TmConfig(RDPA_IOCTL_DEV_NONE, 0, parentTmId,
 ...
 *    rdpaIf_p (OUT) rdpa interface ID Array.
 *    arraySz (IN) rdpa interface ID Array size *
 * Return:
 *    tmctl_ret_e enum value
 * ----------------------------------------------------------------------------
 */
tmctl_ret_e tmctlRdpa_getRdpaIfByIfname(const char* ifname, rdpa_if* rdpaIf_p, int arraySz)
{
   int rc;
   int unit;
   bcm_port_t port;
   ...
   if ((strstr(ifname, GPON_IFC_STR)) || (strstr(ifname, EPON_IFC_STR)))
   {
      rdpaIf_p[0] = rdpa_if_wan0;
      return TMCTL_SUCCESS;
   }

#if defined(CHIP_6838) || defined(CHIP_6848) || defined(CHIP_6858)
   /* This would not work with CONFIG_TRUNK because only a single ->rdpa_port per net_device */
   ...
   rc = bcm_enet_map_ifname_to_unit_portmap((char *)ifname, &unit, &portmap);
   if (rc == 0)
   {
      port = INVALID_ID;
      while (portmap)
      {
         ...
         rc = bcm_enet_map_phys_port_to_rdpa_if(unit, port);
         if (idx < arraySz && rc >= 0)
         {
            rdpaIf_p[idx++] = rc;
            tmctl_debug("ifname %s unit=%d port=%d rdpaIf=%d", ifname, unit, port, *rdpaIf_p);
         }
         else
         {
            break;
      ...
      if (idx != 0) /* Filled atleast one rdpa_if*/
      {
         return TMCTL_SUCCESS;
      }
   }

   tmctl_error("Unsupported ifname %s, rc=%d", ifname, rc);
   return TMCTL_ERROR;
                             ...
                             uint32_t             cfgFlags)
{
   ...
   tmctl_debug("Enter: rdpaIf=%d schedType=0x%x maxQueues=%d portShaper=%d queueShaper=%d",
   ...
   tmctl_debug("rdpaCtl_RootTmConfig SUCCESS! rootTmId=%d", rootTmId);

   if (schedType == TMCTL_SCHED_TYPE_SP_WRR)
   {
   ...
   {
      tmctl_debug("Configure default runner queues.");
      ...
      for (qid = 0; qid < tmParms_p->maxQueues; qid++)
      {
         ...
         prioIndex = rdpaCtl_getQueuePrioIndex(qid, dir, tmParms_p->maxQueues, INVALID_ID);
                                  ...
                                  tmctl_queueCfg_t* qcfg_p)
{
   ...
   tmctl_debug("Enter: devType=%d rdpaIf=%d qid=%d priority=%d schedMode=%d qsize=%d "
               "wt=%d shapingRate=%d burstSize=%d minRate=%d",
                devType, rdpaIf, qcfg_p->qid, qcfg_p->priority, qcfg_p->schedMode,
                qcfg_p->qsize, qcfg_p->weight, qcfg_p->shaper.shapingRate,
...
#endif /* defined(GPON_HGU) || defined(EPON_HGU) */
   {
       ...
       */
      tmctl_debug("qcfg:    qid=%d schedMode=%d qsize=%d wt=%d shapingRate=%d burstSize=%d minRate=%d",
                   qcfg_p->qid, qcfg_p->schedMode, qcfg_p->qsize, weight,
                   shapingRate, qcfg_p->shaper.shapingBurstSize,
                   qcfg_p->shaper.minRate);
      tmctl_debug("qcfgSav: qid=%d schedMode=%d qsize=%d wt=%d shapingRate=%d burstSize=%d minRate=%d tmId=%d",
                   qcfgSav.qid, qcfgSav.schedMode, qcfgSav.qsize, qcfgSav.weight,
                   qcfgSav.shaper.shapingRate, qcfgSav.shaper.shapingBurstSize,
                   qcfgSav.shaper.minRate, tmIdSav);
   ...
   tmctl_debug("Set the new qcfg: rdpaIf=%d parentTmId=%d qid=%d qsize=%d sched=%d wt=%d shapingRate=%d",
                rdpaIf, parentTmId, qcfg_p->qid, qcfg_p->qsize, schedMode, weight, shapingRate);
...
#else 
#if defined(EPON_HGU) && defined(SUPPORT_RDPA)
    {
    rdpa_wan_type wan_type = rdpa_wan_none;
   ...
   if (ret == TMCTL_ERROR)
   {
      tmctl_error("addQueue ERROR! rdpaIf=%d parentTmId=%d qid=%d qsize=%d sched=%d wt=%d shapingRate=%d minRate=%d",
                   rdpaIf, parentTmId, qcfg_p->qid, qcfg_p->qsize, schedMode, weight, shapingRate, minRate);
                                      ...
                                      tmctl_queueDropAlgExt_t* dropAlgHi_p)
{
   ...
   rdpaDropAlg = convertDropAlg(dropAlgorithm);
 ...
 * ----------------------------------------------------------------------------
 */
tmctl_ret_e tmctlRdpa_getDscpToPbit(tmctl_dscpToPbitCfg_t* cfg_p)
{
   tmctl_ret_e ret = TMCTL_SUCCESS;
   int  rc;
   BOOL found;
   ...
   found = FALSE;
   if ((rc = rdpaCtl_DscpToPitGet(&found, cfg_p)))
   {
      tmctl_error("rdpaCtl_DscpToPitGet ERROR! rc=%d", rc);
      return TMCTL_ERROR;
   }
...
}  /* End of tmctlRdpa_getDscpToPbit() */
 ...
 * ----------------------------------------------------------------------------
 */
tmctl_ret_e tmctlRdpa_setDscpToPbit(tmctl_dscpToPbitCfg_t* cfg_p)
{
    tmctl_ret_e ret = TMCTL_SUCCESS;
    int  rc;

    if ((rc = rdpaCtl_DscpToPitSet(*cfg_p)))
    {
        tmctl_error("rdpaCtl_DscpToPitSet ERROR! rc=%d", rc);
        return TMCTL_ERROR;
    }

    return ret;

}  /* End of tmctlRdpa_setDscpToPbit() */
