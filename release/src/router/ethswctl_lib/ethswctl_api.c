...
int ethswctl_mibdump_x(int unit, int port, int priority, int type)
{
    ...
    if (type < 0 || type > 1) {
        printf("Invalid type. Valid values are 0 and 1. \n");
        err = -1;
        goto out;
    }
    if (unit < 0 || unit > 1) {
                           ...
                           int *weights)
{
    ...
    *weights = e->weights[0];
    *(weights+1) = e->weights[1];
    *(weights+2) = e->weights[2];
    *(weights+3) = e->weights[3];
...
int bcm_phy_mode_get(int unit, int port, int *speed, int *duplex)
{
    ...
    if (e->ret_val != -1) {
        *speed = (e->phycfg & PHY_CFG_AUTO_NEGO)? 0: e->phycfg;
        *duplex = e->duplex;
    }
out:
    close(skfd);
