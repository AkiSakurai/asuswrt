 ...
 *
 * $Id: acsd.h 558199 2015-05-21 11:00:59Z $
 */
...
#include "acsd_ioctl.h"
#define nvram_get acsd_config_get
#define nvram_match acsd_config_match
#define nvram_safe_get acsd_config_safe_get
#endif
...
#define ACSD_FCS(fmt, arg...) \
		do { \
			if (acsd_debug_level & ACSD_DEBUG_FCS) { \
				acsddbg(fmt, ##arg); \
				if (acsd_debug_level & ACSD_DEBUG_SYSLOG) \
					logmessage("acsd", fmt, ##arg); \
			} \
...
#define ACSD_DFSR(fmt, arg...) \
		do { \
			if (acsd_debug_level & ACSD_DEBUG_DFSR) { \
				acsddbg("DFSR:%20s(%d): "fmt, __FUNCTION__, __LINE__, ##arg); \
				if (acsd_debug_level & ACSD_DEBUG_SYSLOG) \
