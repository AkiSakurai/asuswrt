...
static inline int __rdpa_ioctl(int code, uint32_t ctx)
{
        ...
        BOOL qClean)
{
    ...
    rdpaCtl_debug("dev_type %u, dev_id %u, tmId %u, dir %d, qid %d, qsize %d, wt %d",
        devType, devId, tmId, dir, qid, qsize, weight);
...
{
    return __rdpa_ioctl(RDPA_IOC_IPTV, (uint32_t)iptv_p);
}
        ...
        rdpa_drv_ioctl_iptv_lookup_method_t method)
{
    ...
    if (method >= RDPA_IOCTL_IPTV_METHOD_MAX)
    {
        ...
        rdpa_drv_ioctl_iptv_filter_method_t method)
{
    ...
    if (method >= RDPA_IOCTL_IPTV_FILTER_MAX)
    {
...
int rdpaCtl_get_wan_type(rdpa_wan_type *wanType)
{
    int rc;
...
int rdpaCtl_set_port_sa_limit(uint16_t port_idx, BOOL is_wan_side, uint16_t max_limit)
{
    ...
    port.is_wan_side = is_wan_side;

    rc = __sendPortCommand(&port);
    if (rc)
    {
        rdpaCtl_debug("set sa limit to %d failed", max_limit);
    }
...
int rdpaCtl_get_port_sa_limit(uint16_t port_idx, BOOL is_wan_side, uint16_t *max_limit)
{
    ...
    if (rc)
    {
        rdpaCtl_debug("get sa limit failed");
    }
    else
    {
...
int rdpaCtl_set_port_sal_miss_action(uint16_t port_idx, BOOL is_wan_side, rdpa_forward_action act)
{
    ...
    memset(&port, 0, sizeof(rdpa_drv_ioctl_port_t));
    port.cmd = RDPA_IOCTL_PORT_CMD_SAL_MISS_ACTION_SET;
    port.port_idx = port_idx;
    ...
    if (rc)
    {
        rdpaCtl_debug("set port sal miss action to 0x%04x failed", act);
    }
...
int rdpaCtl_get_port_sal_miss_action(uint16_t port_idx, BOOL is_wan_side, rdpa_forward_action *act)
{
    ...
    memset(&port, 0, sizeof(rdpa_drv_ioctl_port_t));
    port.cmd = RDPA_IOCTL_PORT_CMD_SAL_MISS_ACTION_GET;
    port.port_idx = port_idx;
    port.is_wan_side = is_wan_side;
    ...
    if (rc)
    {
        rdpaCtl_debug("get port sal miss action failed");
    }
    else
    {
...
int rdpaCtl_set_port_dal_miss_action(uint16_t port_idx, BOOL is_wan_side, rdpa_forward_action act)
{
    ...
    memset(&port, 0, sizeof(rdpa_drv_ioctl_port_t));
    port.cmd = RDPA_IOCTL_PORT_CMD_DAL_MISS_ACTION_SET;
    port.port_idx = port_idx;
    port.param.sal_miss_action = act;
    port.is_wan_side = is_wan_side;
...
int rdpaCtl_get_port_dal_miss_action(uint16_t port_idx, BOOL is_wan_side, rdpa_forward_action *act)
{
    ...
    memset(&port, 0, sizeof(rdpa_drv_ioctl_port_t));
    port.cmd = RDPA_IOCTL_PORT_CMD_DAL_MISS_ACTION_GET;
    port.port_idx = port_idx;
    port.is_wan_side = is_wan_side;
...
{
    return __rdpa_ioctl(RDPA_IOC_RDPA_MW, 0);
}
        ...
        *found = dscp_to_pbit.found;
        if (dscp_to_pbit.found)
        {
            for (i = 0; i < TOTAL_DSCP_NUM; i++)
            {
                ...
                rdpaCtl_debug(
                    "get dscp %d to pbit map %u", i, dscp_to_pbit.pbit_q_map[i]);
            }
        }
    }
