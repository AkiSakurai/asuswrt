...
static int procComboImgHdr(imgutil_ctx_t *ctxP, UINT8 *dataP, int len,
  int *offsetP);
static int getEraseSize(UINT32 *eraseSizeP);
...
IMGUTIL_HANDLE img_util_open(imgutil_open_parms_t *openParmsP)
{
    ...
    if ((openParmsP->maxBufSize != 0) && (openParmsP->maxBufSize > flashSize))
    {
        imgutil_error("Image size %d does not fit flash size %d.",
          openParmsP->maxBufSize, flashSize);
        return IMGUTIL_HANDLE_INVALID;
    }
    ...
    ctxP->flashSize = flashSize;
    if (ctxP->flashType == FLASH_INFO_FLAG_NAND)
    {
        getEraseSize(&ctxP->eraseSize);
    }
  ...
  UINT32 bitmask, imgutil_img_info_t *imgInfoP)
{
    ...
    else if (src == IMGINFO_SRC_BLTIN)
    {
        if (bitmask & IMGUTIL_IMGINFO_BIT_VERSION)
        {
            ...
            {
                imgutil_error("Get image version in incremental flashing "
                  "not supported.");
            }
        }
    }
...
/* This function works for NAND only. */
static int getEraseSize(UINT32 *eraseSizeP)
{
    mtd_info_t *mtdP;
    int mtdFd;
    ...
    if (mtdP == 0)
    {
        imgutil_error("get_mtd_device_nm(%s) failed", MTD_NAME_IMAGE);
        return -1;
    }
    ...
    if (mtdP == NULL)
    {
        imgutil_error("get_mtd_device_nm(%s) failed", MTD_NAME_IMAGE_UPDATE);
        return -1;
    }

    *eraseSizeP = mtdP->erasesize;
    put_mtd_device(mtdP, mtdFd);
