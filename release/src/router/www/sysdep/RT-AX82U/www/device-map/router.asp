...
function getVariable(){
	...
	if(system.band2gSupport){
		...
		_element = ['wl0_ssid', 'wl0_nmode_x', 'wl0_auth_mode_x', 'wl0_crypto', 'wl0_wpa_psk', 'wl0_mfp', 'wl0_wep_x', 'wl0_key', 'wl0_key1', 'wl0_key2', 'wl0_key3', 'wl0_key4'];
		_array.push.apply(_array, _element);
	}

	if(system.band5gSupport){
		var _element = new Array();
		_element = ['wl1_ssid', 'wl1_nmode_x', 'wl1_auth_mode_x', 'wl1_crypto', 'wl1_wpa_psk', 'wl1_mfp', 'wl1_wep_x', 'wl1_key', 'wl1_key1', 'wl1_key2', 'wl1_key3', 'wl1_key4'];
		_array.push.apply(_array, _element);
	}

	if(system.band5g2Support){
		var _element = new Array();
		_element = ['wl2_ssid', 'wl2_nmode_x', 'wl2_auth_mode_x', 'wl2_crypto', 'wl2_wpa_psk', 'wl2_mfp', 'wl2_wep_x', 'wl2_key', 'wl2_key1', 'wl2_key2', 'wl2_key3', 'wl2_key4'];
		_array.push.apply(_array, _element);
	}
...
function getInterface(){
	...
	if(system.smartConnectSupport && variable.smart_connect_x != '0'){		// Smart Connect
		if(variable.smart_connect_x == '1'){	// Tri/Dual-Band Smart Connect		
			if(system.band5g2Support){
				if(dwb_info.mode == '1'){
					_temp = ['0', 'Dual-Band Smart Connect'];
					wlInterface.push(_temp);
					_temp = ['2', '5 GHz-2'];
					wlInterface.push(_temp);
				}
				else{
					_temp = ['0', 'Tri-Band Smart Connect'];
					wlInterface.push(_temp);
				}	
			}
			else{
...
function genElement(){
	...
	for(var i=0; i<wlInterface.length; i++){	
		...
		if(!system.lyraHideSupport){
			...
			code += '<span>為確保您的 AiMesh 系統與終端裝置順利連線，再切換到 WPA3-Personal之前，請先閱讀 <a id="wl'+ unit +'_wpa3FaqLink" class="faq-link" target="_blank" href="https://www.asus.com/support/FAQ/1042500">FAQ</a>。</span>';
			code += '</div>';
		}
...
function genAuthMethod(unit, id, nmode_x, auth_mode_x){
	...
	if(sw_mode == '2'){
		auth_array = [['Open System', 'open'], ['WPA2-Personal', 'psk2'], ['WPA-Auto-Personal', 'pskpsk2']];
	}
	else if(unit == '3'){
		auth_array = [['Open System", "open'], ['WPA2-Personal', 'psk2']];
	}
	else if(system.modelName == 'RT-AC87U' && unit == '1'){
		auth_array = [['Open System', 'open'], ['WPA2-Personal', 'psk2'], ['WPA-Auto-Personal', 'pskpsk2']];
	}
	else if(nmode_x == '2'){	// Legacy mode
		if(system.modelName == 'BLUECAVE'){
			auth_array = [["Open System", "open"], ["WPA-Personal", "psk"], ["WPA2-Personal", "psk2"], ["WPA-Auto-Personal", "pskpsk2"], ["WPA-Enterprise", "wpa"], ["WPA2-Enterprise", "wpa2"], ["WPA-Auto-Enterprise", "wpawpa2"]];
		}
		else{
			if(system.wifiLogoSupport){
				auth_array = [['Open System', 'open'], ['WPA2-Personal', 'psk2'], ['WPA-Auto-Personal', 'pskpsk2'], ['WPA-Enterprise', 'wpa'], ['WPA2-Enterprise', 'wpa2'], ['WPA-Auto-Enterprise', 'wpawpa2']];
			}
			else if(system.wpa3Support){
				if(system.newWiFiCertSupport){
					auth_array = [['Open System', 'open'], ['Shared Key', 'shared'], ['WPA2-Personal', 'psk2'], ['WPA3-Personal', 'sae'], ['WPA/WPA2-Personal', 'pskpsk2'], ['WPA2/WPA3-Personal', 'psk2sae'], ['WPA2-Enterprise', 'wpa2'], ['WPA/WPA2-Enterprise', 'wpawpa2'], ['Radius with 802.1x', 'radius']];
				}
				else{
					auth_array = [['Open System', 'open'], ['Shared Key', 'shared'], ['WPA-Personal', 'psk'], ['WPA2-Personal', 'psk2'], ['WPA3-Personal', 'sae'], ['WPA/WPA2-Personal', 'pskpsk2'], ['WPA2/WPA3-Personal', 'psk2sae'], ['WPA-Enterprise', 'wpa'], ['WPA2-Enterprise', 'wpa2'], ['WPA/WPA2-Enterprise', 'wpawpa2'], ['Radius with 802.1x', 'radius']];
				}
			}
			else{
				if(system.newWiFiCertSupport){
					auth_array = [['Open System', 'open'], ['Shared Key', 'shared'], ['WPA2-Personal', 'psk2'], ['WPA-Auto-Personal', 'pskpsk2'], ['WPA2-Enterprise', 'wpa2'], ['WPA-Auto-Enterprise', 'wpawpa2'], ['Radius with 802.1x', 'radius']];
				}
				else{
					auth_array = [['Open System', 'open'], ['Shared Key', 'shared'], ['WPA-Personal', 'psk'], ['WPA2-Personal', 'psk2'], ['WPA-Auto-Personal', 'pskpsk2'], ['WPA-Enterprise', 'wpa'], ['WPA2-Enterprise', 'wpa2'], ['WPA-Auto-Enterprise', 'wpawpa2'], ['Radius with 802.1x', 'radius']];
				}
			}
		}
	}
		...
		if(system.wpa3Support){
			auth_array = [['Open System', 'open'], ['WPA2-Personal', 'psk2'], ['WPA3-Personal', 'sae'], ['WPA/WPA2-Personal', 'pskpsk2'], ['WPA2/WPA3-Personal', 'psk2sae'], ['WPA2-Enterprise', 'wpa2'], ['WPA/WPA2-Enterprise', 'wpawpa2']];
		}
		else{
			auth_array = [['Open System', 'open'], ['WPA2-Personal', 'psk2'], ['WPA-Auto-Personal', 'pskpsk2'], ['WPA2-Enterprise', 'wpa2'], ['WPA-Auto-Enterprise', 'wpawpa2']];
		}
	}
...
function genWEPEncryption(unit, id, auth_mode){
	...
	wepEncryptionArray = [['WEP-64bits', '1'], ['WEP-128bits', '2']];
	if(auth_mode == 'open'){
		wepEncryptionArray = [['None', '0'], ['WEP-64bits', '1'], ['WEP-128bits', '2']];
	}
...
function genWEPKeyIndex(unit, id){
	var wepKeyIndexArray = new Array();
	wepKeyIndexArray = [['1', '1'], ['2', '2'], ['3', '3'], ['4', '4']];
	var code = '';
...
function switchTab(id){
	...
	else if(id == 'information_tab'){
		location.href = 'information.asp';
	}
	else if(id == 'light_effect_tab'){
		location.href = 'router_light_effect.asp';
	}
	else{
		location.href = 'router.asp';
	}
}
...
function validateInput(){
	...
	<div class="main-block">
		<div class="display-flex flex-a-center">
			...
			<!--<div id="information_tab" class="tab-block" onclick="switchTab(this.id)"><#diskUtility_information#></div>-->
			<div id="light_effect_tab" class="tab-block"style="display:none;" onclick="switchTab(this.id)">Light Effect</div><!-- untranslated -->
		</div>
