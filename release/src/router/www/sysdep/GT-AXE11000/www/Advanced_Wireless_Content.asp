...
function initial(){
	...
	auth_method_change('0', '<% nvram_get("wl0_auth_mode_x"); %>');
	auth_method_change('1', '<% nvram_get("wl1_auth_mode_x"); %>');
	auth_method_change('2', '<% nvram_get("wl2_auth_mode_x"); %>');

	handle_smart_connect('<% nvram_get("smart_connect_x"); %>');
	...
	if(_smart_connect_enable == '0'){
		...
		if(document.form.smart_connect_t.value == '1'){
			...
			if(document.form.band01_ssid.value === document.form.band2_ssid.value){
				variable["wl2_ssid"] = document.form.band2_ssid.value + '_6G';
			}
...
function auth_method_change(unit, value){
	var _temp = '';
	var _temp_value = '';
...
function handle_smart_connect(value){
	//var _smart_connect_x_ori = '<% nvram_get("smart_connect_x"); %>';
	if(value == '0'){
		document.getElementById("smart_connect_field").style.display = "none";
		document.getElementById("smartcon_rule_link").style.display = "none";
		...
		if(value == '1'){
			...
			/*document.form.band01_auth_mode_x.value = 'psk2sae';
			document.form.band01_crypto.value = 'aes';
			document.form.band01_mfp.value = '1';
			document.form.band2_auth_mode_x.value = 'sae';
			document.form.band2_crypto.value = 'aes';
			document.form.band2_mfp.value = '2';*/
	...
	if(dwb_info.mode == "1"){
		...
		if(get_cfg_clientlist[0] != undefined){
			...
			if(select_node_capability.fronthaul_ap_option_on)
				$selectObj.append($('<option>', {value: "2", text: "Use as both backhaul and fronthaul connection"}));/* untranslated */
			...
			if(select_node_capability.fronthaul_ap_option_off)
				$selectObj.append($('<option>', {value: "0", text: "Use as a dedicated wireless backhaul"}));/* untranslated */
			if($selectObj.find("option").length > 0){
				var $trObj = $("<tr>").addClass("fronthaul_ap");
				var $thObj = $("<th>").html("AiMesh Wireless Backhaul");/* untranslated */
				var $tdObj = $("<td>");
		...
		if(_smart_connect_enable == "1" && document.form.smart_connect_t.value == "1"){
			$(".fronthaul_ap").show();
			if(document.form.smart_connect_t.value != document.form.smart_connect_x.value)
				$selectObj.val(httpApi.nvramDefaultGet(["fh_ap_enabled"]).fh_ap_enabled);
		}
		else{
			$(".fronthaul_ap").hide();
			$selectObj.val("0");
  ...
  <tr>
	<td align="left" valign="top" >
      ...
      <div class="formfontdesc"><#adv_wl_desc#></div>
			...
			<table id="band_separate" width="99%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" style="margin-top:10px">
				<thead>
				...
				<tr id="band01_auth_method_field">
					...
					<td>
						<select name="band01_auth_mode_x" class="input_option" onChange="auth_method_change('01', this.value);">
				...
				<tr id="band01_psk_field">
					...
					<td>
				  		<input type="text" name="band01_wpa_psk" maxlength="64" class="input_32_table" value="<% nvram_get("wl0_wpa_psk"); %>" autocorrect="off" autocapitalize="off">
					</td>
				</tr>
				  
				<tr id="band01_radius_ip_field">
					<th>
						<a class="hintstyle" href="javascript:void(0);"  onClick="openHint(2,1);"><#WLANAuthentication11a_ExAuthDBIPAddr_itemname#></a>
					</th>
					<td>
				...
				<tr id="band01_radius_port_field">
					<th>
				...
				<tr id="band0_auth_method_field">
					...
					<td>
						<select name="band0_auth_mode_x" class="input_option" onChange="auth_method_change('0', this.value);">
							<option value="open"    <% nvram_match("wl0_auth_mode_x", "open",   "selected"); %>>Open System</option>
							<option value="psk2"    <% nvram_match("wl0_auth_mode_x", "psk2",   "selected"); %>>WPA2-Personal</option>
							<option value="sae"    <% nvram_match("wl0_auth_mode_x", "sae",   "selected"); %>>WPA3-Personal</option>
							<option value="pskpsk2" <% nvram_match("wl0_auth_mode_x", "pskpsk2","selected"); %>>WPA/WPA2-Personal</option>
				...
				<tr>
					...
					<td>
						...
						<span id="band1_160_field" style=""><input id="band1_160" type="checkbox" onClick="separateEnable_160MHz(this);" <% nvram_match("wl1_bw_160", "1" , "checked" ); %>><#WLANConfig11b_ChannelBW_Enable160M#></span>
					</td>
				</tr>
				<tr>
					<th><#WLANConfig11b_Channel_itemname#></th>
				...
				<tr id="band1_auth_method_field">
					...
					<td>
						<select name="band1_auth_mode_x" class="input_option" onChange="auth_method_change('1', this.value);">
				...
				<tr id="band2_channel_field" style="">
					...
					<td>
						...
						<span id="band2_psc6g" style="">
							<input id="band2_psc6g_checkbox" type="checkbox" onclick="separateGenChannel('2', document.form.band2_channel.value, document.form.band2_bw.value);" <% nvram_match("psc6g", "1" , "checked" ); %>>enable PSC (Preferred Scanning Channel) to ensure the 6GHz devices connectivity. Please check <a href="https://www.asus.com/support/FAQ/1044625" target="_blank" style="color:#FC0;text-decoration: underline;">FAQ</a>.
						</span>
					</td>
				</tr>
				...
				<tr id="band2_auth_method_field">
					...
					<td>
						<select name="band2_auth_mode_x" class="input_option" onChange="auth_method_change('2', this.value);">
							<option value="owe" <% nvram_match("wl2_auth_mode_x", "owe",   "selected"); %>>Opportunistic Wireless Encryption</option>
				...
				<tr id="band2_psk_field">
					<th><a class="hintstyle" href="javascript:void(0);" onClick="openHint(0, 7);"><#WLANConfig11b_x_PSKKey_itemname#></a></th>
					<td>
				  		<input type="text" name="band2_wpa_psk" maxlength="64" class="input_32_table" value="<% nvram_get("wl2_wpa_psk"); %>" autocorrect="off" autocapitalize="off">
					</td>
			  	</tr>
