...
static t_WLCSM_MSG_HDR    *_netlink_recv_mesg(int fromNotifier,char *to_buf)
{
    ...
    if((len=recvmsg(fromsocket, &msg, 0)) >= sizeof(t_WLCSM_MSG_HDR)) {
...
static int  _wlcsm_create_nl_socket(int *sock,int pid,int mlcast_id)
{
    ...
    *sock = socket(PF_NETLINK, SOCK_RAW, m_NETLINK_PORT);
    ...
    if(bind(*sock, (struct sockaddr*)&src_addr,
            sizeof(src_addr)) < 0) {
        WLCSM_TRACE(WLCSM_TRACE_LOG, "bind failed \n");
        return WLCSM_GEN_ERR;
    }
...
static int _wlcsm_init(int isdaemon)
{
    ...
    if(WLCSM_NL_NOT_INITED) {
        ...
        char tempStr[64];
        unsigned int num=0;
        if(_wlcsm_create_nl_socket(&wlcsm_agent.sock_nl,getpid(),0)!=WLCSM_SUCCESS) {
...
int _wlcsm_netlink_send_mesg(int msg_type, char *buf, int len,unsigned int to_pid)
{
     ...
     */
    if(to_pid)
        nlh->nlmsg_pid = wlcsm_agent.uni_pid+g_max_pid;
    else
...
#if 0
int wlcsm_mngr_update_stalist(unsigned int radio_idx,unsigned int source )
{
    char temp[16];
    sprintf(temp,"updatesta");
    return wlcsm_mngr_run_cmd(radio_idx,0,source,WLCSM_MNGR_CMD_UPDATE_STALIST,temp);
}
#endif
...
int wlcsm_register_process (char *process_name)
{
    ...
    reg=malloc(len);
    reg->pid=wlcsm_agent.uni_pid+g_max_pid;
    ...
    if(wlcsm_netlink_send_mesg(WLCSM_MSG_REGISTER,(char *)reg,len)== WLCSM_SUCCESS) {
        ...
        if(hdr!=NULL && hdr->type==WLCSM_MSG_REGISTER) return wlcsm_agent.mngr_sock_nl;
    }
...
int wlcsm_mngr_dm_pwr_reset(void) {
    unsigned int radio_idx=0;
    WLCSM_MNGR_CMD_SET_CMD(radio_idx,WLCSM_MNGR_CMD_PWRRESET);
    return _wlcsm_mngr_set(radio_idx,0,"pwrreset","",1);
}
