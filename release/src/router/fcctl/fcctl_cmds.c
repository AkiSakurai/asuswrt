...
int fcCtlFlushHandler(POPTION_INFO pOptions, int nNumOptions)
{
    ...
    if ( nNumOptions == 0 )
    {
        ...
        if ( nRet == FCACHE_ERROR )
        {
            fprintf( stderr, "%s: failed to flush Flow Cache\n", g_PgmName );
        }
    }
    ...
    else if ( !strcmp( pOptions->pOptName, "--if") ) 
    {
        ...
        ifindex = if_nametoindex(pOptions->pParms[0]);
        if (ifindex == 0)
        {
        ...
        nRet = fcCtlFlush(ifindex);
        if ( nRet == FCACHE_ERROR )
        {
            fprintf( stderr, "%s: failed to flush Flow Cache entires for if %s\n", 
...
int fcCtlFlwStatsDump(int num_query)
{
...

	{
		...
		{		
		    /* Dump statistics */		
		    printf("  %8d:%12lu:%20llu:%13lu:%4d: ", flwStDumpEntry->queryIdx, 
		...
		{
		    printf("  prevSt[%d].rx_packets: %lu prevSt[%d].rx_bytes: %llu prevSt[%d].rx_rtp_packets_lost: %lu \n", i, 
