...
static void dbgPrintCmd(uint32_t *arg)
{
...
static int getNumericArg(int *argc, char ***argv, uint32_t *arg)
{
    if(--(*argc))
    {
        (*argv)++;
...
//            VLANCTL_LOG_ERROR("Not a numeric argument");
        ...
        *arg = strtol(**argv, NULL, 0);
        if(errno)
        {
            VLANCTL_LOG_ERROR("%s: %d", strerror(errno), *arg);
            return -1;
        }
...
static int getStringArg(int *argc, char ***argv, uint32_t *arg)
{
    if(--(*argc))
    {
        (*argv)++;

        *arg = (unsigned int)**argv;
...
static int getCmdArgs(int *argc, char ***argv, uint32_t *arg, const char *cmdMask)
{
    int ret = 0;
    int done = 0;
    int i;
...
int main(int argc, char **argv)
{
    int ret;
    int commandCount = 0;
    uint32_t arg[VLANCTL_MAX_COMMAND_ARGS];
    ...
    while(!ret && argc)
    {
        ...
        else if(!strcmp(*argv, "--cfg-tpid"))
        {
            ...
            if(!ret)
            {
                ret = vlanCtl_setTpidTable(devName, arg);
            }
        }
