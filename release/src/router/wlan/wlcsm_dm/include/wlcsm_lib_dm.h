...
#define WL_BSSID_IFNAME(idx,sub) (gp_adapter_objs[(idx)].bssids[(sub)].name)
...
#define WL_AP_STAS(idx,sub) (gp_adapter_objs[(idx)].ssids[(sub)].assocailated_stas)

#define MIMO_PHYY(idx) ((!strcmp(gp_adapter_objs[(idx)].radio.wlPhyType, WL_PHY_TYPE_N)) || (!strcmp(gp_adapter_objs[(idx)].radio.wlPhyType, WL_PHY_TYPE_AC)))

/*************************************************************//**
 * @brief	reload from data model to populate the mngr structure
 *
 * This API is used to reload configuraiton from DM and populate the global
 * wlmngr structure,it also depends on the current selected DM, using
 * WLCSM_DM_SELECT(name) to select the DM which has also been registered.
 *
 * @param	idx- 0 for all interfaces,>0 for each single interface
 * 		from -  this is the source of the resason of reloading
 *
 * @return  	int 0- successful other- failure
 ***************************************************************/
 ...
 * @return  void
 ***************************************************************/
int  wlcsm_dm_save_config(int idx,int from);
...
/*************************************************************//**
 * @brief has dm to setup pwrctl
 *
 * @return int
 ***************************************************************/
int wlcsm_dm_pwrctl(int anyassociated);
...
int wlcsm_dm_select_dm(char *name,unsigned int idx, int from);
...
#define WLCSM_DM_REGISTER(name) \
	wlcsm_dm_register(&(wlcsm_dm_##name##_handler))
 ...
 *
 * */
#define WLCSM_DM_SELECT(name,idx,ret) \
	ret=wlcsm_dm_select_dm(#name,idx,ret)
