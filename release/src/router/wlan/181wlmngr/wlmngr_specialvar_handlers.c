...
int wlmngr_update_possibble_channels(unsigned int idx,SYNC_DIRECTION direction)
{
    ...
    band = WL_RADIO_WLBAND(idx) == BAND_A ? '5' : '2';
...
void wlmngr_nv_adjust_chanspec(unsigned int idx,SYNC_DIRECTION direction)
{
    char value[20]= {0};
    char name[20]= {0};
    snprintf(name, sizeof(name), "%s_chanspec",WL_BSSID_NAME(idx,0));
    snprintf(value, sizeof(value), "%d",WL_RADIO_WLCHANNEL(idx));

    if (MIMO_PHYY(idx) && WL_RADIO_WLCHANNEL(idx)) {
        if (WL_BW_CAP_80MHZ(WL_RADIO_WLNBWCAP(idx)))
            strcat(value, "/80");
...
static int _wlmngr_wps_wlforecewps_handler(t_WLCSM_MNGR_VARHDR *hdr,char *name,char *varValue, t_WLMNGR_VAR_OPER_ENUM op)
{
    ...
    if(op==WLMNGR_VAR_OPER_GET) {
        ...
        if (nvram_match("wps_version2", "enabled")) {
            ...
            if (!wpsDisable) {
                if ((strcmp(WL_AP_WLFLTMACMODE(radio_idx,sub_idx), "allow") == 0) && WL_AP_WLFLTMACLIST(radio_idx,sub_idx)==NULL)
                    wpsDisable = 1;
            }
...
static int _wlmngr_channel_handler(t_WLCSM_MNGR_VARHDR *hdr, char *name, char *varValue, t_WLMNGR_VAR_OPER_ENUM op)
{
    ...
    switch( op ) {
    case WLMNGR_VAR_OPER_GET:
        if (!strcmp(name, "channelsInUse") || !strcmp(name, "wlCurrentChannel")) {
        ...
        break;
    case WLMNGR_VAR_OPER_SET:
        if (!strcmp(name, "wlChannel") || !strcmp(name, "wlCurrentChannel")) {
            wlcsm_dm_mngr_set_all_value(idx, hdr->sub_idx, "wlChannel", varValue);
            if (!strcmp(varValue, "0")) {   //Auto Channel Enabled
                char buf[4] = "1";
                wlcsm_dm_mngr_set_all_value(idx, hdr->sub_idx, "autoChannelEnable", buf);
            }

        } else if (!strcmp(name, "autoChannelEnable")) {
            char buf[4]= {0};
...
int wlmngr_special_nvram_handler(char *name,char *value)
{
    ...
    int i=0,br=0;
    char *nv_name;
    if(!strncmp(name,"lan_wps_oob",11)||sscanf(name,"lan%d_wps_oob",&idx)) {
        for (i=0; i<WL_WIFI_RADIO_NUMBER;  i++ ) {
        ...
        if((idx<WL_WIFI_RADIO_NUMBER) && (ssid_idx<WL_RADIO_WLNUMBSS(idx)) && (nv_name)) {
            ...
            } else if(!strncmp(nv_name,"akm",3)&& value) {
                /* when wlx_akm is set, wlAuthMode will be the same and exclusive
                * wps have the akm as "psk2 ", with space at the end, trim it off*/
                return wlcsm_dm_mngr_set_all_value(idx,ssid_idx,"wlAuthMode",wlcsm_trim_str(value));
            } else if(!strncmp(nv_name,"auth_mode",9)&& value && (!strncmp(value,"none",4))) {
                return wlcsm_dm_mngr_set_all_value(idx,ssid_idx,"wlAuthMode",WL_APSEC_WLAUTHAKM(idx,ssid_idx));
            }
        }
    }
    return 0;
...
void wlmngr_sign_chan_by_chanspec( uint idx,uint ssid_idx,char *value)
{
    ...
    if(value) {
        ...
        if(channel) {
            ...
            fprintf(stderr,"JXUJXU:%s:%d band:%d,nctrlsb:%d,channel:%d,nbw:%d  \r\n",__FUNCTION__,__LINE__,
