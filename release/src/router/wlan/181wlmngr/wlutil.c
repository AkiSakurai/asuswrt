...
#ifdef SUPPORT_MIMO
#define MIMO_ENABLED   ((!strcmp(WL_PHYTYPE(idx), WL_PHY_TYPE_N) || !strcmp(WL_PHYTYPE(idx), WL_PHY_TYPE_AC)) && strcmp(WL_RADIO_NMODE(idx), WL_OFF))
#endif
#define MIMO_PHY ((!strcmp(WL_PHYTYPE(idx), WL_PHY_TYPE_N)) || (!strcmp(WL_PHYTYPE(idx), WL_PHY_TYPE_AC)))
...
int wlmngr_getCoreRev(const unsigned int idx)
{
    ...
    if (fp != NULL) {
        while ( !feof(fp) ) {
            if(fscanf(fp, "%s 0x%x\n", cmd, &corerev)==2 && (!strcmp(match_str,cmd)))  {
                found=1;
                break;
            }
        }
...
void wlmngr_getCountryList(const unsigned int idx, int argc, char **argv, char *list)
{
    ...
    if ( fp != NULL ) {
        ...
        for (i=0; iloc<eloc; i++) {
            ...
            if (!strcmp(WL_RADIO_WLCOUNTRY(idx), abbrv)) {
                cn =1;
            }
        }
...
char * wlmngr_getChannelList(const unsigned int idx, int argc, char **argv, char *list)
{
    ...
    *cmd = 0;
    if ((!strcmp(type, "b")) && (WL_RADIO_WLMBANDS(idx) & BAND_B)) {
    ...
    } else if ((!strcmp(type, "a")) && (WL_RADIO_WLMBANDS(idx) & BAND_A)) {
        snprintf(cmd, sizeof(cmd), "wlctl -i wl%d chanspecs -b %d -w %d -c %s > /var/wl%dchspeclist", \
                 idx, WL_CHANSPEC_5G, atoi(bw), WL_RADIO_WLCOUNTRY(idx), idx);
    } else if ((!strcmp(type, "n")) && *band == 'a' && (WL_RADIO_WLMBANDS(idx) & BAND_A) && (*curPhyType == 'n')) {
        snprintf(cmd, sizeof(cmd), "wlctl -i wl%d chanspecs -b %d -w %d -c %s > /var/wl%dchspeclist", \
                 idx, WL_CHANSPEC_5G, atoi(bw), WL_RADIO_WLCOUNTRY(idx), idx);
    } else if ((!strcmp(type, "n")) && *band == 'b' && (WL_RADIO_WLMBANDS(idx) & BAND_B) && (*curPhyType == 'n')) {
        snprintf(cmd, sizeof(cmd), "wlctl -i wl%d chanspecs -b %d -w %d -c %s > /var/wl%dchspeclist", \
                 idx, WL_CHANSPEC_2G, atoi(bw), WL_RADIO_WLCOUNTRY(idx), idx);
    } else if ((!strcmp(type, "v")) && *band == 'a' && (WL_RADIO_WLMBANDS(idx) & BAND_A) && (*curPhyType == 'v')) {
        snprintf(cmd, sizeof(cmd), "wlctl -i wl%d chanspecs -b %d -w %d -c %s > /var/wl%dchspeclist", \
                 idx, WL_CHANSPEC_5G, atoi(bw), WL_RADIO_WLCOUNTRY(idx), idx);
    } else if ((!strcmp(type, "v")) && *band == 'b' && (WL_RADIO_WLMBANDS(idx) & BAND_B) && (*curPhyType == 'v')) {
        snprintf(cmd, sizeof(cmd), "wlctl -i wl%d chanspecs -b %d -w %d -c %s > /var/wl%dchspeclist", \
                 idx, WL_CHANSPEC_2G, atoi(bw), WL_RADIO_WLCOUNTRY(idx), idx);
    }
...
static inline void wlmngr_togglePowerSave_aspm(const unsigned int idx, char assoc, char *cmd)
{
    ...
    if (!strcmp(WL_PHYTYPE(idx), WL_PHY_TYPE_AC)) {
        if (assoc) {
...
// Returns      : None.
//**************************************************************************
void wlmngr_doQoS(const unsigned int idx)
{
...
void wlmngr_doWlConf(const unsigned int idx)
{
    ...
    if (MIMO_PHY) {
        ...
        if(strcmp(WL_RADIO_NMODE(idx), WL_OFF)) {
...
static int get_event_pid(int wl_unit)
{
    ...
    if (fd > 0) {
        ...
        close(fd);
        if(rc) cpu = atoi(tmpBuf);
        WLCSM_TRACE(WLCSM_TRACE_DBG,"read of output: rc=%d cpu=%d buf=%s", rc, cpu, tmpBuf);
    }
    ...
    if (name != NULL) {
        pid = wlmngr_getPidByName(name);
    }

    WLCSM_TRACE(WLCSM_TRACE_DBG,"returning pid %d", pid);
    return pid;
}
...
static void wlmngr_setpriority(int wl_unit)
{
    ...
    if (wl_unit == 0) {
        ...
        pid = wlmngr_getPidByName("wl0-kthrd");
        if (pid <= 0) {
            /* get worker pid from Linux 2.6.30 kernel */
            pid = get_event_pid(wl_unit);
        }
        pid_wfd = wlmngr_getPidByName("wfd0-thrd");
    } else if (wl_unit == 1) {
        pid = wlmngr_getPidByName("wl1-kthrd");
        if (pid <= 0) {
            pid = get_event_pid(wl_unit);
        }
        pid_wfd = wlmngr_getPidByName("wfd1-thrd");
    } else if (wl_unit == 2) {
        pid = wlmngr_getPidByName("wl2-kthrd");
        if (pid <= 0) {
            pid = get_event_pid(wl_unit);
        }
        pid_wfd = wlmngr_getPidByName("wfd2-thrd");
    }

    if (pid > 0) {
        if (sched_setscheduler(pid, SCHED_RR, &sp) == -1)
            WLCSM_TRACE(WLCSM_TRACE_ERR,"Failed to make wlan processing on unit %d realtime", wl_unit);
        //else
...
void wlmngr_doWlconfAddi(const unsigned int idx)
{
    ...
    // basic rate
    if ( strcmp(WL_RADIO(idx).wlBasicRate, WL_BASIC_RATE_ALL) == 0 ) {
        rateptr = rs_allbasic;
    } else {
        if (WL_RADIO(idx).wlBand == BAND_B) { //options for 11b/g
            if(strcmp(WL_PHYTYPE(idx), WL_PHY_TYPE_B) == 0) {
                if ( strcmp(WL_RADIO(idx).wlBasicRate, WL_BASIC_RATE_1_2) == 0 ) {
                    rateptr = rs_11b_1_2;
                }
            } else { //for all non-11b
                if ( strcmp(WL_RADIO(idx).wlBasicRate, WL_BASIC_RATE_1_2) == 0 ) {
                    rateptr = rs_11g_1_2;
                } else if ( strcmp(WL_RADIO(idx).wlBasicRate, WL_BASIC_RATE_WIFI_2) == 0 ) {
    ...
    if(rateptr != rs_default) {
        snprintf(cmd, sizeof(cmd), "wlctl -i %s rateset %s", WL_BSSID_IFNAME(idx,0), rateptr);
        BCMWL_WLCTL_CMD(cmd);
    }
