...
/*cleanup the old wds list first! */
   ...
   while ( (ret = cmsObj_getNextInSubTree(MDMOID_WL_WDS_CFG, 
         ...
         if ( entry != NULL ) {
             strncpy(entry->macAddress, wdsCfgObj->wlMacAddr, sizeof(entry->macAddress));
#if 0
/*No need to read. */ 
             strncpy(entry->ssid, wdsCfgObj->wlSsid, sizeof(entry->ssid));
	         strncpy(entry->ifcName, wdsCfgObj->wlIfcname, sizeof(entry->ifcName));
#endif
...
CmsRet wlSendCmsMsgToWlmngrByHandle(void *msgHandle, char *message_text)
{
   ...
   snprintf((char *)(msg + 1), sizeof(buf), "%s:%d", message_text, unit + 1);
...
static void getAdapterListFromMdm(int *wl0_present, int *wl1_present, int *wl2_present)
{
   ...
   ret = cmsLck_acquireLockWithTimeout( LOCK_WAIT );
   if ( ret != CMSRET_SUCCESS ) {
      cmsLog_error("Could not get lock after %d ms", LOCK_WAIT);
      return;
   }

   while ((ret = cmsObj_getNextFlags(MDMOID_WL_BASE_CFG, &iidStack,
                                     OGF_NO_VALUE_UPDATE,
                                 (void **)&wlBaseCfgObj)) == CMSRET_SUCCESS)
   {
      instanceId = PEEK_INSTANCE_ID(&iidStack);
      if (instanceId == 1)  // instance 1 == wl0
      {
         cmsLog_debug("detected wl0");
         *wl0_present = 1;
      }
      else if (instanceId == 2)
      {
         cmsLog_debug("detected wl1");
         *wl1_present = 1;
      }
      else if (instanceId == 3)
      {
         cmsLog_debug("detected wl2");
         *wl2_present = 1;
      }
      else
      {
         cmsLog_error("instanceId=%d is not supported! (only 1 or 2 or 3)", instanceId);
      }
...
#endif /* SUPPORT_DM_LEGACY98 || SUPPORT_DM_HYBRID) || SUPPORT_DM_DETECT */


void cmsMdm_detect_adapters(int *wl0_present __attribute__((unused)),
                            int *wl1_present __attribute__((unused)),
                            int *wl2_present __attribute__((unused)))
{

#if defined(SUPPORT_DM_LEGACY98) || defined(SUPPORT_DM_HYBRID)

   /* scan MDM for wlan objects and call wlan_restart for each adapter */
   getAdapterListFromMdm(wl0_present, wl1_present,wl2_present);

#elif defined(SUPPORT_DM_PURE181)

   /* In Pure TR181 mode, wlmngr does not read MDM directly */

#elif defined(SUPPORT_DM_DETECT)

   if (cmsUtil_isDataModelDevice2() == 0)
   {
      /* we are in hybrid mode, so still need to scan MDM and call wlan_restart */
      getAdapterListFromMdm(wl0_present, wl1_present,wl2_present);
   }
                           ...
                           void *msgHandle  __attribute__((unused)))
{
   ...
   /* In Pure TR181 mode, do not initialize the MDM */
