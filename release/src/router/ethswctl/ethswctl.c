...
#define SF2_QOS_TC_SRC_SEL_VAL_MASK 3
    ...
    if (strcmp(name, "hw-switching") == 0) {
        ...
        ret_val = ethswctl_mibdump_x(unit, port, priority, opt_a>0);
    ...
    if (opt_v) {
        ...
        if (opt_p && !opt_y && !opt_z) {
            ...
            if (param1 == 0)
                ...
                if (param1 & PHY_CFG_2500FD) printf("2.5GFD");
                if (param1 & PHY_CFG_1000FD) printf("1GFD");
                if (param1 & PHY_CFG_1000HD) printf("1GHD");
                if (param1 & PHY_CFG_100FD) printf("100FD");
                if (param1 & PHY_CFG_100HD) printf("100HD");
                if (param1 & PHY_CFG_10FD) printf("10FD");
                if (param1 & PHY_CFG_10HD) printf("10HD");
            }
        }
        else if (opt_p &&
                ( (opt_y && param2 == 0 && !opt_z) || /* auto */
                   (opt_y && (param2 == 1000 || param2 == 100 || param2 == 10) &&
    ...
    else if (strcmp(name, "bondingports") == 0) {
        if (!opt_v) {
            printf(usage_msgs[BONDING_PORTS_MSG]);
            return 0;
        }
        if (opt_v)
        {
            bcm_switch_bonding_ports_set(val);
        } else {
            printf(usage_msgs[BONDING_PORTS_MSG]);
        }
    }
...
int ethswctl_wrr_param_get(int unit)
{
    ...
    if (ret_val == 0) {
        printf("Maximum Packets per round = %04d\n", max_pkts_per_iter);
        printf("Weights: %02d %02d %02d %02d\n", weights[0], weights[1], weights[2], weights[3]);
    }
