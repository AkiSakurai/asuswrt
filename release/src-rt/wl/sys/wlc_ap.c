...
wlc_dfs_timer_init(wlc_ap_info_t *ap)
{
	...
	appvt->dfs_cac.cactime_pre_ism =
	appvt->dfs_cac.cactime_post_ism = WLC_DFS_CAC_TIME_USE_DEFAULTS;   /* use default values */
...
wlc_dfs_cacstate_idle_set(wlc_ap_info_t *ap)
{
	...
	appvt->dfs_cac.duration =  wlc_dfs_ism_cactime(wlc, appvt->dfs_cac.cactime_post_ism);
}
	...
	if (wlc_radar_detected(ap) == TRUE) {
		...
		if (wlc_radar_chanspec(wlc->cmi, WLC_BAND_PI_RADIO_CHANSPEC) == TRUE) {
			...
			WL_DFS(("wl%d: dfs : state to %s channel %d at %dms\n",
				...
				appvt->dfs_cac.duration)*WLC_DFS_RADAR_CHECK_INTERVAL));
			/*
			 * Set up for pre-ISM on new channel, which may or may not be a
			 * european weather channel and thus require different timers.
			 */
			appvt->dfs_cac.duration =
				appvt->dfs_cac.cactime =
				wlc_dfs_ism_cactime(wlc, appvt->dfs_cac.cactime_pre_ism);
			return;
		}
		else {
...
wlc_dfs_cacstate_ism(wlc_ap_info_t *ap)
{
	...
	appvt->dfs_cac.duration =
	appvt->dfs_cac.cactime = wlc_dfs_ism_cactime(wlc, appvt->dfs_cac.cactime_post_ism);
}
...
wlc_dfs_cacstate_csa(wlc_ap_info_t *ap)
{
	...
	if (wlc_radar_chanspec(wlc->cmi, WLC_BAND_PI_RADIO_CHANSPEC) == TRUE) {
		if (appvt->dfs_cac.cactime_post_ism) {
			...
			appvt->dfs_cac.duration =
			appvt->dfs_cac.cactime =
				wlc_dfs_ism_cactime(wlc, appvt->dfs_cac.cactime_post_ism);
	        }
	        else {
	                wlc_dfs_cacstate_ism_set(ap);
	        }
...
wlc_dfs_cacstate_init(wlc_ap_info_t *ap)
{
	...
	if (wlc_radar_chanspec(wlc->cmi, WLC_BAND_PI_RADIO_CHANSPEC) == TRUE) {
		...
		/* unit of cactime is WLC_DFS_RADAR_CHECK_INTERVAL */
		appvt->dfs_cac.cactime = wlc_dfs_ism_cactime(wlc, appvt->dfs_cac.cactime_pre_ism);
		if (appvt->dfs_cac.cactime) {
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_DFS_PREISM):
		*ret_int_ptr = appvt->dfs_cac.cactime_pre_ism;
		break;

	case IOV_SVAL(IOV_DFS_PREISM):
		/*
		 * Range check, allow -1..max. Value -1 resets defaults.
		 */
		if ((int_val < -1) || (int_val >= WLC_DFS_CAC_TIME_SEC_MAX)) {
			err = BCME_RANGE;
			break;
		}
		appvt->dfs_cac.cactime_pre_ism = int_val;
		break;

	case IOV_GVAL(IOV_DFS_POSTISM):
		*ret_int_ptr = appvt->dfs_cac.cactime_post_ism;
		break;
	...
	case IOV_SVAL(IOV_DFS_POSTISM):
		...
		appvt->dfs_cac.cactime_post_ism = int_val;
		break;
