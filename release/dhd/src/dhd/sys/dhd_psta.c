	...
	if (!tx && dhd_get_psta_mode((dhd_pub_t *)dhdp) == DHD_MODE_PSR &&
		*(dhcph + DHCP_TYPE_OFFSET) == DHCP_TYPE_REPLY &&
		...
		if (ETHER_IS_LOCALADDR(chaddr)) {
			/* Based on chaddr to get original cli_mac */
                    ...
                    uint8 *icmph, uint16 icmplen, uint8 **shost, bool tx, bool is_bcmc)
{
	...
	/* get mod_mac for dhcp processing */
	if (is_bcmc || dhd_get_psta_mac(dhdp, cli_mac, mod_mac, TRUE)) {
		memcpy(mod_mac, cli_mac, ETHER_ADDR_LEN);
		ETHER_SET_LOCALADDR(mod_mac);
	}
...
dhd_psta_proto_proc(dhd_pub_t *dhdp, uint32 ifidx, void **p, uint8 *eh, uint8 **shost, bool tx)
{
	...
	case ETHER_TYPE_IPV6:
		...
		break;
	case ETHER_TYPE_ARP:
		/* If these skb is cloned, we need to use arpbuf as skb->data to replace original one
		 * before modify ARP header, because original skb->data is shared by other skb of other dev. 
		 * __kfree_skb is free skb->head, not free skb->data.
		 */
		if (tx) {
			bzero(dhdp->arpbuf, sizeof(dhdp->arpbuf));
			memcpy(ALIGN_ADDR(dhdp->arpbuf, sizeof(uint16)), eh, (ARP_DATA_LEN+pull));
			((struct sk_buff *)(*p))->data  = ALIGN_ADDR(dhdp->arpbuf, sizeof(uint16));
			ih = 	(uint8 *)PKTDATA(dhdp->osh, *p) + pull;
		}
