 ...
 *
 * $Id: dhd_linux.c 516977 2014-11-21 18:58:08Z $
 */
 ...
 * BCMEXTRAHDROOM=32 (no need for D11 headers)                          32
 * RXBUFSZ = Buffer to hold ethernet frame = 2048 - 352 - 32        = 1664
 */
#define	RXBUFSZ			(1696 - BCMEXTRAHDROOM)
#else  /* ! BCM_GMAC3 */
...
dhd_get_psta_mac(dhd_pub_t *dhdp, uint8 *cli_mac, uint8 *mod_mac, bool get_mod)
{
	...
	if (ifp) {
		...
		DHD_ERROR(("%s: fail to get %s_mac for %s\n",
			__func__,
			get_mod ? "mod" : "cli",
			bcm_ether_ntoa((struct ether_addr *)mac, eabuf)));
		...
		if (ETHER_ISUCAST(data)) { /* lookup station in forwarder's WOFA */
			...
			{	/* Locate the destination dhd_sta using WOFA */
				...
				if (wofa == FWDER_WOFA_INVALID) {
					struct net_device *def;

					/* PSTA: Use default dev if we have. */
					def = fwder_default(fwder);
					if (def) {
						skb->dev = def;
...
#if 1   /* xxx bh->brcp==NULL error */
	if (dhd->brc_hot) {
		ctf_brc_t *brcp;
		bool hot = FALSE;

		/* use ctf_brc_lkup to protect bridge cache table */
		ctf_brc_acquire(dhd->cih);
		brcp = ctf_brc_lkup(dhd->cih, eh, TRUE);
		if (brcp)
			hot = CTF_HOTBRC_CMP(dhd->brc_hot, (eh), (void *)(ifp->net));

		ctf_brc_release(dhd->cih);
		return hot;
	} else {
#if 0
		DHD_ERROR(("%s: %s [dhd->brc_hot  == NULL] case found!\n",
...
#ifdef DHD_PSTA
		if (PSR_ENABLED(dhdp) && !(ifp->primsta_dwds) &&
...
#ifdef CONFIG_RTAC3200
	if (dhd_found < 2)
		dhd->unit = (1 - dhd_found) + instance_base;
	else
#endif
...
int dhd_set_dev_def(dhd_pub_t *dhdp, uint32 idx, int val)
{
	...
	ifp = dhd->iflist[idx];
	if (ifp && ifp->fwdh)
		fwder_register(dhd->fwdh, val ? ifp->net : NULL);
