 ...
 *
 * $Id: dhd_msgbuf.c 514461 2014-11-11 05:14:26Z $
 */
#include <typedefs.h>
#include <osl.h>
...
#define RX_BUF_BURST				32 /* Rx buffers which will contain MSDU Data */
...
#ifdef NDIS
/* windows dhd cannot allocate a packet bigger than LBDATASZ.
 * windows has a fixed size pool to allocate packets.
 */
#define DHD_FLOWRING_IOCTL_BUFPOST_PKTSZ		LBDATASZ
#else
#ifdef MACOSX
#define DHD_FLOWRING_IOCTL_BUFPOST_PKTSZ		2048
#else
...
static void* dhd_alloc_ring_space(dhd_pub_t *dhd, msgbuf_ring_t *ring,
	uint16 msglen, uint16 *alloced, bool exactly_nitems);
...
#define MAX_PKTID_ITEMS     (32768) /* Maximum number of pktids supported */
...
#define DHD_NATIVE_TO_PKTID_INIT(osh, items) dhd_pktid_map_init((osh), (items))
#define DHD_NATIVE_TO_PKTID_FINI(map)        dhd_pktid_map_fini(map)

#define DHD_NATIVE_TO_PKTID_RSV(map, pkt)    dhd_pktid_map_reserve((map), (pkt))
#define DHD_NATIVE_TO_PKTID_SAVE(map, pkt, nkey, pa, len, dma) \
	dhd_pktid_map_save((map), (void *)(pkt), (nkey), (pa), (uint32)(len), (uint8)dma)
#define DHD_NATIVE_TO_PKTID(map, pkt, pa, len, dma) \
	dhd_pktid_map_alloc((map), (void *)(pkt), (pa), (uint32)(len), (uint8)dma)

#define DHD_PKTID_TO_NATIVE(map, pktid, pa, len) \
...
static INLINE uint32 dhd_native_to_pktid(void *pkt, dmaaddr_t pa, uint32 len);
static INLINE void *dhd_pktid_to_native(uint32 pktid, dmaaddr_t *pa, uint32 *len);

/* Save dma parameters into the packet's pkttag and convert a pktptr to pktid */
static INLINE uint32
dhd_native_to_pktid(void *pktptr32, dmaaddr_t physaddr, uint32 dma_len)
{
	ASSERT(pktptr32 != NULL);
	DHD_PKT_SET_DMA_LEN(pktptr32, dma_len);
	DHD_PKT_SET_PHYSADDR(pktptr32, physaddr);
...
static INLINE void *
dhd_pktid_to_native(uint32 pktid32, dmaaddr_t *physaddr, uint32 *dma_len)
{
	void *pktptr32;
	...
	*physaddr = DHD_PKT_GET_PHYSADDR(pktptr32);

	return pktptr32;
}

#define DHD_NATIVE_TO_PKTID_RSV(map, pkt)  DHD_PKTID32(pkt)

#define DHD_NATIVE_TO_PKTID_SAVE(map, pkt, nkey, pa, len, dma) \
	dhd_native_to_pktid((pkt), (pa), (len))

#define DHD_NATIVE_TO_PKTID(map, pkt, pa, len, dma) \
	dhd_native_to_pktid((pkt), (pa), (len))

#define DHD_PKTID_TO_NATIVE(map, pktid, pa, len) \
	dhd_pktid_to_native((uint32)(pktid), (dmaaddr_t *)&(pa), (uint32 *)&(len))
...
#if defined(DHD_PCIE_PKTID)
	/* PKTID handle INIT */
	prot->pktid_map_handle = DHD_NATIVE_TO_PKTID_INIT(dhd->osh, MAX_PKTID_ITEMS);
...
int dhd_sync_with_dongle(dhd_pub_t *dhd)
{
	...
	bcm_print_bytes("mac", buf, ETHER_ADDR_LEN);
#endif /* OEM_ANDROID */
...
dhd_prot_packet_free(dhd_pub_t *dhd, uint32 pktid)
{
	...
	PKTBUF = DHD_PKTID_TO_NATIVE(dhd->prot->pktid_map_handle, pktid, pa, pa_len);

	if (PKTBUF) {
#if defined(BCM_ROUTER_DHD) && defined(BCM_GMAC3)
		if (!PKTISFWDERBUF(dhd->osh, PKTBUF))
...
dhd_prot_rxbufpost(dhd_pub_t *dhd, uint16 count)
{
	...
	msg_start = (void *)dhd_alloc_ring_space(dhd, ring, count, &alloced, TRUE);
	...
	for (i = 0; i < alloced; i++) {
...
#ifdef BCM_ROUTER_DHD
		/* Reserve extra headroom for router builds because packets can
		 * be forwarded to NIC driver through CTF and we can use this
		 * headroom there in the Tx path while preparing headers.
		 */
		...
		rxbuf_post->cmn_hdr.request_id =
			htol32(DHD_NATIVE_TO_PKTID(dhd->prot->pktid_map_handle, p, physaddr,
			pktlen, DMA_RX));
...
dhd_prot_rxbufpost_ctrl(dhd_pub_t *dhd, bool event_buf)
{
	...
	rxbuf_post = (ioctl_resp_evt_buf_post_msg_t *)dhd_alloc_ring_space(dhd,
		prot->h2dring_ctrl_subn, DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D, &alloced, FALSE);
	if (rxbuf_post == NULL) {
		DHD_GENERAL_UNLOCK(dhd, flags);
...
dhd_prot_ioctcmplt_process(dhd_pub_t *dhd, void * buf, uint16 msglen)
{
	...
	status = ltoh16(ioct_resp->compl_hdr.status);
 ...
 * is determine. The flowring is then queried for space (reserving the space in
...
dhd_prot_txqueue_flush(dhd_pub_t *dhd, void *flow_ring)
{
	...
	ASSERT(!dhd_bus_is_txmode_push(dhd->bus));

	flow_ring_node = (flow_ring_node_t *)flow_ring;
	flow_queue = &flow_ring_node->queue;
	msgbuf_ring = (msgbuf_ring_t*)(flow_ring_node->prot_info);
...
retry_ring_space:
	start_addr = dhd_alloc_ring_space(dhd, msgbuf_ring, ring_avail, &alloced, FALSE);
	if (start_addr == NULL) {
	...
	for (nitems = 0; nitems < alloced; nitems++, txdesc++) {
		...
		if (PKTISFWDERBUF(dhd->osh, pkt)) {
			...
			physaddr = DMA_MAP(dhd->osh, pktdata, FWDER_PKTMAPSZ, DMA_RX, pkt, 0);
		} else {
			...
			physaddr = DMA_MAP(dhd->osh, pktdata, pktlen, DMA_TX, pkt, 0);
		}
...
done_queue_processing:
	...
	if (nitems) { /* at least one pkt was posted */
		prot_ring_write_complete(dhd, msgbuf_ring, start_addr, nitems);
...
dhd_prot_txdata(dhd_pub_t *dhd, void *PKTBUF, uint8 ifidx)
{
	...
	txdesc = (host_txbuf_post_t *)dhd_alloc_ring_space(dhd,
		msg_ring, DHD_FLOWRING_DEFAULT_NITEMS_POSTED_H2D, &alloced, FALSE);
	if (txdesc == NULL) {
		...
		DHD_PKTID_TO_NATIVE(dhd->prot->pktid_map_handle, pktid, physaddr, pktlen);
#endif /* DHD_PCIE_PKTID */
		goto err_no_res_pktfree;
	}
	...
	/* Adjust the data pointer back to original value */
	pktdata = PKTPUSH(dhd->osh, PKTBUF, ETHER_HDR_LEN);
	...
	bool exactly_nitems)
{
	...
	ret_buf = prot_get_ring_space(ring, nitems, alloced, exactly_nitems);
...
/* If exactly_nitems is false, this function will allocate space for nitems or less */
static void *BCMFASTPATH
prot_get_ring_space(msgbuf_ring_t *ring, uint16 nitems, uint16 * alloced, bool exactly_nitems)
{
	void *ret_ptr = NULL;
...
dhd_set_dmaed_index(dhd_pub_t *dhd, uint8 type, uint16 ringid, uint16 new_index)
{
	...
	switch (type) {
		...
		case D2H_DMA_READINDX:
			...
			/* H2D Common Rings start at Id BCMPCIE_H2D_COMMON_MSGRINGS */
...
/* D2H direction: get next space to read from */
static uint8*
