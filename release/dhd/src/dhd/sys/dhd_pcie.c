 ...
 *
 * $Id: dhd_pcie.c 516067 2014-11-18 14:22:33Z $
 */
...
static void  __init select_fd_image(
	...
	if (image_index != -1) {
		...
		*p_dlarray     = 0;
		DHD_ERROR(("%s: Dongle image not available for chipid = 0x%x"
		    "  chiprev = %d  chippkg = %d\n", __FUNCTION__,
		    bus->sih->chip, bus->sih->chiprev, bus->sih->chippkg));
	}
}
#endif /* defined(BCMEMBEDIMAGE) && defined (BCM_ROUTER_DHD) */
 ...
 * and the flowring queue is placed into the pcie bus's txqueue_pend list. The
...
#ifdef BCM_ROUTER_DHD
		/* Enforcing kernel panic, in order to reboot
		 * the router based on the watchdog value when
		 * TRAP is encountered. Rebooting is done to
		 * recover the system from hung state.
		 */
		val = nvram_get("watchdog");
		if (val && bcm_atoi(val)) {
			DHD_ERROR(("Enforcing kernel panic"));
			*(uint*)0 = 0;
		}
#endif
	}
