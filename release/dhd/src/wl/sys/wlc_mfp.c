 ...
 *
 * $Id: wlc_mfp.c 467328 2014-04-03 01:23:40Z $
 *
	...
	eapol_header_t *eapol, uint16 *data_len)
{
	...
	wlc_key_get_seq(igtk, igtk_encap->ipn, sizeof(igtk_encap->ipn), 0, TRUE);
	wlc_key_get_data(igtk, igtk_encap->key, sizeof(igtk_encap->key), NULL);
...
wlc_mfp_write_to_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (WLC_MFP_ENAB(wlc->pub) &&
		...
		pi->value = WLC_MFP_COMEBACK_TIE_TU;
...
bool
wlc_mfp_check_rsn_caps(const wlc_mfp_info_t* mfp, uint32 wsec, uint8 rsn,
	bool *enable)
{
	bool ret = TRUE;

	*enable = FALSE; /* MFP does not apply */
	if ((rsn & RSN_CAP_MFPR) && !(rsn & RSN_CAP_MFPC))
		ret = FALSE;
	else if ((wsec & MFP_CAPABLE) && (rsn & RSN_CAP_MFPC))
