 ...
 *
 * $Id: wlc_rate_sel.c 470212 2014-04-14 17:41:11Z $
 */
...
static void
wlc_ratesel_filter(rcb_t *state, wlc_rateset_t *rateset,
	uint32 max_rate, uint32 min_rate, uint start_rate)
{
	 ...
	 */
	state->rsi->nss_lmt = state->rsi->wlc->stf->txstreams;
	...
	uint8 tx_mcs, uint8 antselid, bool fbr)
{
	...
	/* for fast drop */
	state->ncfails = acked ? 0 : state->ncfails + 1;
	...
	uint8 fbr, uint8 fbr_succ, bool tx_error, uint8 tx_mcs, uint8 antselid)
{
	...
	/* for fast drop:
	 * If PSR @ either MRT or FBR are > 50% reset..
	 * If PSR on both MRT and FBR < 50% and FBR is used, increment
	 */
