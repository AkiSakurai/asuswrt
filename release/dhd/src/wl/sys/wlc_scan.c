 ...
 *
 * $Id: wlc_scan.c 467328 2014-04-03 01:23:40Z $
 */
...
#endif
#if defined(BCMDBG) && !defined(SCANOL)
...
static const bcm_iovar_t wlc_scan_iovars[] = {
	...
	{"scan_ps", IOV_SCAN_TX_PWRSAVE, (IOVF_WHL), IOVT_UINT8, 0},
#endif
	{NULL, 0, 0, 0, 0 }
};
#endif /* WLC_SCAN_IOVARS */
...
wlc_scan_ps_config_cores(scan_info_t *scan_info, bool flag)
{
	...
	if (flag) {
		...
		if (wlc_stf_txchain_ishwdef(scan_info->wlc) &&
		    ...
		    scan_info->scan_tx_pwrsave == TRUE) {
		...
		if (wlc_stf_rxchain_ishwdef(scan_info->wlc) &&
			...
			scan_info->scan_rx_pwrsave == TRUE) {
			...
			if (scan_info->scan_tx_pwrsave == TRUE &&
			    scan_info->scan_ps_txchain) {
			...
			if (scan_info->scan_rx_pwrsave == TRUE &&
				scan_info->scan_ps_rxchain) {
