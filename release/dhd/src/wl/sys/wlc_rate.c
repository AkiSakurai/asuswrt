 ...
 *
 * $Id: wlc_rate.c 467328 2014-04-03 01:23:40Z $
 */
...
#ifdef WL11N
/** MCS'es that use more than the caller supplied number of spatial streams cannot be used. */
void
	...
	uint8 mcsallow, uint8 bw, uint8 txstreams)
{
	...
	/** MCS'es that require more than the number of spatial txstreams cannot be used. */
 ...
 */
uint16
wlc_get_valid_vht_mcsmap(uint8 mcscode, uint8 bw, bool ldpc, uint8 nss, uint8 ratemask)
{
	uint16 peermcsmap;
	uint16 mcsmap;
	...
	WL_RATE(("%s() map=0x%x code=0x%x peermap=0x%x rm=0x%x bw=%d nss=%d \n",
		__FUNCTION__, mcsmap, mcscode, peermcsmap, ratemask, bw, nss));
	return (mcsmap);
}
...
wlc_valid_vht_mcs(uint8 mcs, uint8 nss, uint8 bw, uint8 ratemask, bool ldpc, uint8 mcscode)
{
	if (((bw >= BW_MINMHZ) && (bw <= BW_MAXMHZ)) &&
		...
		uint16 mcsmap = wlc_get_valid_vht_mcsmap(mcscode, bw, ldpc, nss, ratemask);
		if (mcsmap & (1 << mcs))
			return TRUE;
	}
...
wlc_get_highest_mcs_index(uint8 mcs_code, uint8 bw, uint8 nss, bool ldpc, uint8 ratemask)
{
	...
	for (; mcs_indx > 0; mcs_indx--) {
		if (wlc_valid_vht_mcs(mcs_indx, nss, bw, ratemask, ldpc, mcs_code))
			break;
	}
	...
	bool vht_ldpc, uint8 vht_ratemask, uint8 txstreams)
{
	...
	if (rateset->vht_mcsmap != VHT_CAP_MCS_MAP_NONE_ALL) {
		...
		for (nss = 1; nss <= txstreams; nss++) {
			...
			mcs_indx = wlc_get_highest_mcs_index(mcs_code, bw,
