 ...
 *
 * $Id: wl_rte.c 473076 2014-04-27 07:28:33Z $
 */
...
#if defined(CONFIG_WLU) || defined(ATE_BUILD)
/* forward prototype */
void do_wl_cmd(void *arg, int argc, char *argv[]);
#endif /* CONFIG_WLU || ATE_BUILD */
#ifdef BCMDBG
...
hndrte_wlfc_info_dump(void *arg, int argc, char *argv[])
{
	...
	printf("packets: (from_host-nost-sig,status_back,stats_other,credit_back,creditin) = "
		...
		wlfc->stats.nost_from_host,
		wlfc->stats.sig_from_host,
...
wlfc_hostpkt_callback(wlc_info_t *wlc, void *p, uint txstatus)
{
	...
	if (WL_TXSTATUS_GET_FLAGS(pkttag->wl_hdr_information) & WLFC_PKTFLAG_PKTFROMHOST) {
		...
		WL_TXSTATUS_SET_FIFO(statusdata,
			WL_TXSTATUS_GET_FIFO(pkttag->wl_hdr_information));
		WL_TXSTATUS_SET_FLAGS(statusdata, WLFC_CTL_PKTFLAG_DISCARD);
		memcpy(&results[TLV_BODY_OFFSET], &statusdata, sizeof(uint32));

		if (WLFC_GET_REUSESEQ(wl->wlfc_mode)) {
			results[TLV_LEN_OFF] += WLFC_CTL_VALUE_LEN_SEQ;
		}
		wlfc_push_signal_data(wlc->wl, results, results[TLV_LEN_OFF] + TLV_HDR_LEN, FALSE);
...
static const char BCMATTACHDATA(rstr_fmt_hello)[] =
	"wl%d: Broadcom BCM%d 802.11 Wireless Controller %s FWID 01-%x\n";
#else
static const char BCMATTACHDATA(rstr_fmt_hello)[] =
	"wl%d: Broadcom BCM%04x 802.11 Wireless Controller %s FWID 01-%x\n";
#endif
#else
#if (BCMCHIPID > 0x9999)
static const char BCMATTACHDATA(rstr_fmt_hello)[] =
                      ...
                      uint coreid, uint unit)
{
	...
	*/
...
#ifdef FWID
	/* print hello string */
	printf(rstr_fmt_hello, unit, wlc->hw->sih->chip, EPI_VERSION_STR, gFWID);
#else
	/* print hello string */
	...
	if (hnd_isr_register(0, coreid, unit, wl_isr, dev, bus) ||
#ifdef THREAD_SUPPORT
	    hnd_dpc_register(0, coreid, unit, wl_dpc_thread, dev, bus) ||
#endif
	    FALSE) {
...
#ifdef	CONFIG_WLU
	hnd_cons_add_cmd("wl", do_wl_cmd, wl);
#endif /* CONFIG_WLU */
	...
	logtrace_start();
		...
		/* For RSDB devices, there is only single wl_devpwrstchg_notify. The function
		 *  wlc_devpwrstchg_change run through each wlc for the same.
		 */
		if (hndrte_register_devpwrstchg_callback(wl_devpwrstchg_notify, wl) && unit == 0) {
...
#if defined(PROP_TXSTATUS) || defined(BCMPCIEDEV_ENABLED)
	...
	if ((type == WLFC_CTL_TYPE_TXSTATUS) && ((wlc_info_t *)wl->wlc)->comp_stat_enab) {
		...
		while (cur_pos < wl->wlfc_info->pending_datalen) {
			if ((wl->wlfc_info->data[cur_pos] == WLFC_CTL_TYPE_COMP_TXSTATUS)) {
				...
				if ((cnt == xcnt) && (hslot == xhslot) && (status == xstatus)) {
					if (!WLFC_GET_REUSESEQ(wl->wlfc_mode) ||
...
#if defined(PROP_TXSTATUS)
	if (BCMPCIEDEV_ENAB(((wlc_info_t*)(wl->wlc))->pub)) {
		if (wlfc_push_signal_bus_data(wl, data, len) == BCME_OK)
			return rc;
	}
#endif
...
wl_sendup_txstatus(wl_info_t *wl, void **pp)
{
	...
	datalen_min = rssi_space + ampdu_reorder_info_space + seqnumber_space;
...
wl_pkt_header_push(wl_info_t *wl, void *p, uint8 *wl_hdr_words)
{
	...
	*wl_hdr_words = WL_HEADER_LEN/HNDRTE_WORD_SIZE;
	return p;
}
...
wl_send_txstatus(wl_info_t *wl, void *p)
{
	...
	if (wlhdrlen != 0) {
		...
		while (processed < wlhdrlen) {
			if (wlhdrtodev[processed] == WLFC_CTL_TYPE_PKTTAG) {
				...
				if (!(WL_TXSTATUS_GET_FLAGS(pkttag->wl_hdr_information) &
					...
					uint8 ac = WL_TXSTATUS_GET_FIFO(pkttag->wl_hdr_information);
		...
		if (BCMPCIEDEV_ENAB(((wlc_info_t*)(wl->wlc))->pub)) {
			/* We do not expect host to set BDC and wl header on PCIEDEV path,
			* So set it now
			*/
...
#ifdef PROP_TXSTATUS_DEBUG
		((wlfc_info_state_t*)wl->wlfc_info)->stats.sig_from_host++;
#endif
		PKTFREE(wl->pub->osh, p, TRUE);
		return TRUE;
	}
	...
	if ((WL_TXSTATUS_GET_FLAGS(pkttag->wl_hdr_information) & WLFC_PKTFLAG_PKTFROMHOST) &&
		...
		((wlfc_info_state_t*)wl->wlfc_info)->stats.nost_from_host++;
	}
...
wl_txframe_chainable(wl_info_t *wl, wlc_bsscfg_t *bsscfg, void *p, void *head)
{
	...
	if ((WLEXTSTA_ENAB(wl->pub) && BSSCFG_SAFEMODE(bsscfg)) ||
	    ...
	    (PSTA_ENAB(wl->pub) && BSSCFG_STA(bsscfg)) ||
	    CAC_ENAB(wl->pub) ||
	    BSSCFG_AWDL(bsscfg)) {
		goto exit;
	}
...
void
do_wl_cmd(void *arg, int argc, char *argv[])
{
...
{
	memcpy(PKTDATA(wl->pub->osh, p) + BCMPCIE_D2H_METADATA_HDRLEN, &status, sizeof(status));
	PKTSETLEN(wl->pub->osh, p, BCMPCIE_D2H_METADATA_HDRLEN + sizeof(status));
	PKTSETDATAOFFSET(p, BCMPCIE_D2H_METADATA_HDRLEN + sizeof(status));
}
#endif
#endif /* PROP_TXSTATUS */
...
{
#ifdef BCMPCIEDEV_ENABLED
	/* need to clean this up, rte does the cache check,
