 ...
 *
 * $Id: wlc_bmac.c 472911 2014-04-25 19:06:48Z $
 */
...
/* CRWLDOT11M-1160: Revs 48,49, ASM is located at txfifo bank1 */
#define SRASM_TXFIFO_OFFSET_REV48	0x20000
...
/* For 4349 core revision 50 */
#define D11MAC_BMC_SR_BYTES				4096	/* 4K SR bytes */
#define D11MAC_BMC_TPL_NUMBUFS_PERCORE	\
(D11MAC_BMC_TPL_BYTES_PERCORE/(1<<(8 + D11MAC_BMC_BUFSIZE_256BLOCK)))

#define D11MAC_BMC_SR_NUMBUFS			\
(D11MAC_BMC_SR_BYTES/(1<<(8 + D11MAC_BMC_BUFSIZE_256BLOCK)))

/* CRWLDOT11M-1160, impacts both revs 48, 49. Don't change these defines */
/* Specified in units of 256B => 80KB */
#define D11MAC_BMC_STARTADDR_CRWLDOT11M_1160		320
#define D11MAC_BMC_BUFSIZE_512BLOCK_CRWLDOT11M_1160	1	/* 1 = 512B, 0 = 256B */
/* 52KB, to allow for 4KB at start of bank1 for SR ASM */
#define D11MAC_BMC_TPL_BYTES_CRWLDOT11M_1160		53248
#define D11MAC_BMC_TPL_NUMBUFS_CRWLDOT11M_1160	\
	(D11MAC_BMC_TPL_BYTES_CRWLDOT11M_1160/(1<<(8+D11MAC_BMC_BUFSIZE_512BLOCK_CRWLDOT11M_1160)))
...
BCMATTACHFN(wlc_bmac_attach_dmapio)(wlc_hw_info_t *wlc_hw, bool wme)
{
	...
	if (PIO_ENAB_HW(wlc_hw)) {
		...
		if ((BCMLFRAG_ENAB()) && (BCMSPLITRX_ENAB())) {
			...
			di = dma_attach(osh, name, wlc_hw->sih,
				DMAREG(wlc_hw, DMA_TX, 1), NULL, ntxd,
				0, 0, -1, 0, 0, &wl_msg_level);
		}
		if (di == NULL)
			goto dma_attach_fail;
...
#if defined(MBSS) || defined(BCM_OL_DEV)
		{
...
BCMPREATTACHFN(wlc_bmac_sr_params_get)(wlc_hw_info_t *wlc_hw, uint32 *offset, uint32 *srfwsz)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 48) || D11REV_IS(wlc_hw->corerev, 49))
		*offset += SRASM_TXFIFO_OFFSET_REV48 -
			(D11MAC_BMC_STARTADDR_CRWLDOT11M_1160 << 8);
...
wlc_bmac_sr_verify(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *b)
{
	 ...
	 */
	if (D11REV_IS(wlc_hw->corerev, 48) || D11REV_IS(wlc_hw->corerev, 49))
		offset += (D11MAC_BMC_STARTADDR_CRWLDOT11M_1160 << 8);
	...
	bool piomode, osl_t *osh, void *regsva, uint bustype, void *btparam)
{
	...
	for (j = 0; j < NBANDS_HW(wlc_hw); j++) {
		...
		if ((wlc_hw->band->pi = (wlc_phy_t *)
		     phy_attach(wlc_hw->phy_sh, (void *)(uintptr)regs,
		                wlc_hw->band->bandtype, vars)) == NULL) {
			WL_ERROR(("wl%d: %s: phy_attach failed\n", unit, __FUNCTION__));
			err = 17;
			goto fail;
		}
		...
		wlc_hw->band->abgphy_encore = phy_utils_get_encore((phy_info_t *)wlc_hw->band->pi);
#ifdef WLC_HIGH
		/* Monolithic driver gets wlc->band and band members
		 * initialized in wlc_bmac_attach()
		 */
		wlc->band->abgphy_encore = wlc_hw->band->abgphy_encore;
#endif
...
good_phy:
	...
	{
		char *var;
		if ((var = getvar(wlc_hw->vars, "wowl_gpio")) != NULL) {
			wlc_hw->wowl_gpio =  (uint8)bcm_strtoul(var, NULL, 0);
		}
		if ((var = getvar(wlc_hw->vars, "wowl_gpiopol")) != NULL) {
...
BCMINITFN(wlc_coreinit)(wlc_hw_info_t *wlc_hw)
{
	...
	if (D11REV_GE(wlc_hw->corerev, 40)) {
		...
		if (wlc_hw->macunit == 0) {
			wlc_bmac_bmc_init(wlc_hw, 0, bufsize, 0, 1);
		}
	}
...
wlc_write_inits(wlc_hw_info_t *wlc_hw, const d11init_t *inits)
{
	...
	for (i = 0; inits[i].addr != 0xffff; i++) {
		...
		if (inits[i].addr == D11CORE_TEMPLATE_REG_OFFSET) {
			/* D11CORE_TEMPLATE_REGION_START is dont care for non RSDB chips.
			 * This needs update as required for RSDB chips.
			 */
			offset_val = D11CORE_TEMPLATE_REGION_START * wlc_hw->macunit;
		}
		if (inits[i].size == 2)
...
bool
wlc_bmac_wowlpf_config_hw(wlc_hw_info_t *wlc_hw, uint8 wowl_gpio, bool polarity)
{
	...
	/* set gpio to logic low */
	si_gpioout(wlc_hw->sih, 1 <<  wowl_gpio, polarity << wowl_gpio,
		GPIO_DRV_PRIORITY);
...
bool
wlc_bmac_wowlpf_set_hw(wlc_hw_info_t *wlc_hw, bool logic, uint8 wowl_gpio, bool polarity)
{
...
wlc_bmac_txstatus(wlc_hw_info_t *wlc_hw, bool bound, bool *fatal)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 4)) {
		...
		*fatal = wlc_bmac_txstatus_corerev4(wlc->hw);
		...
		mhf2_val = wlc_bmac_mhf_get(wlc_hw, MHF2, WLC_BAND_AUTO);
		if (D11REV_LT(wlc_hw->corerev, 26) ||
			(D11REV_GE(wlc_hw->corerev, 26) && ((mhf2_val & MHF2_TX_TMSTMP) == 0))) {
		...
		while (!(*fatal) && (s1 = R_REG(osh, &regs->frmtxstatus)) & TXS_V) {
			...
			if (D11REV_GE(wlc_hw->corerev, 26) && ((mhf2_val & MHF2_TX_TMSTMP) != 0)) {
	...
	uint8 bufsize_in_256_blocks, uint8 reset_stats, uint8 init)
{
	...
	int bmc_fifo_list[D11AC_MAX_FIFO_NUM] = {7, 0, 1, 2, 3, 4, 5, 6, 8};
	...
	if (D11REV_IS(wlc_hw->corerev, 48) || D11REV_IS(wlc_hw->corerev, 49)) {
		...
		bufsize = D11MAC_BMC_BUFSIZE_512BLOCK_CRWLDOT11M_1160;

		/* start at 80KB, there are fewer buffers available for BMC use */
		bmc_startaddr = D11MAC_BMC_STARTADDR_CRWLDOT11M_1160;

		/* Set this up to be 52KB */
		tplbuf = D11MAC_BMC_TPL_NUMBUFS_CRWLDOT11M_1160;
	}
...
wlc_bmac_rsdb_cap(wlc_hw_info_t *wlc_hw)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 50)) {
		if ((R_REG(wlc_hw->osh, &wlc_hw->regs->machwcap1) &
			MCAP1_NUMMACCHAINS) > 0) {
			return TRUE;
		}
	}
...
wlc_bmac_islast_core(wlc_hw_info_t *wlc_hw)
{
	...
	return (wlc_hw->macunit ==
		(si_numcoreunits(wlc_hw->sih, D11_CORE_ID) - 1));
}
