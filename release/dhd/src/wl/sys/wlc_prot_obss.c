 ...
 */
static bool
wlc_prot_obss_interference_detected(wlc_prot_obss_info_t *prot)
{
	...
	limit = (priv->config->obss_dur_thres * sample_dur) / 100;
...
wlc_prot_obss_detection(wlc_prot_obss_info_t *prot, chanspec_t chanspec)
{
	...
	if (wlc_prot_obss_interference_detected(prot)) {
		if (!WLC_PROT_OBSS_PROTECTION(prot)) {
