 ...
 *
 * $Id: wlc_stf.c 472354 2014-04-23 20:02:26Z $
 */
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		case IOV_SVAL(IOV_STF_OPER_MODE):
		{
			...
			if (VHT_OPER_MODE_STATE_PENDING(bsscfg) != VHT_OP_MODE_NOT_PENDING) {
				err = BCME_BUSY;
				break;
			}
			...
			err = wlc_vht_handle_oper_mode_notif_request(wlc->vhti,
				bsscfg, oper_mode, enabled);
			break;
		}
#endif /* WL11AC */
