 ...
 *
 * $Id: wlc_bsscfg.c 467328 2014-04-03 01:23:40Z $
 */
...
#ifdef WL11AC
	/* operating mode info */
	if ((cfg->opermode_info = (wlc_bsscfg_opermode_info_t*)
	     MALLOCZ(osh, sizeof(wlc_bsscfg_opermode_info_t))) == NULL)
		goto fail;
#endif

	/* bss load reporting */
	if (BSSLOAD_REPORT_ENAB(wlc->pub) && !BSSCFG_AWDL(cfg) && !BSSCFG_AP(cfg)) {
...
#ifdef WL11AC
		/* operating mode info */
		if (cfg->opermode_info != NULL) {
			if (cfg->opermode_info->oper_mode_timer != NULL) {
				wl_del_timer(wlc->wl, cfg->opermode_info->oper_mode_timer);
				wl_free_timer(wlc->wl, cfg->opermode_info->oper_mode_timer);
				cfg->opermode_info->oper_mode_timer = NULL;
			}
			if (cfg->opermode_info->op_ctx != NULL) {
				MFREE(wlc->osh, cfg->opermode_info->op_ctx,
					sizeof(wlc_bsscfg_opermode_ctx_t));
				cfg->opermode_info->op_ctx = NULL;
			}
			MFREE(osh, cfg->opermode_info,
				sizeof(wlc_bsscfg_opermode_info_t));
			cfg->opermode_info = NULL;
		}
#endif /* WL11AC */
	...
	uint flags, bool ap)
{
	...
	if (L2_FILTER_ENAB(wlc->pub)) {
		if (wlc_l2_filter_proxy_arp_enab(wlc) && BSSCFG_AP(bsscfg)) {
...
wlc_bsscfg_tx_stop(wlc_bsscfg_t *bsscfg)
{
	...
	WL_P2P(("wl%d.%d: %s: pending %d packets %d\n",
	        wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
	        TXPKTPENDTOT(wlc), pktq_len(&bsscfg->wlcif->qi->q)));
	...
	while ((pkt = pktq_deq_tail(bsscfg->psq, &prec))) {
		if (!wlc_prec_enq_head(wlc, txq, pkt, prec, TRUE)) {
			WL_ERROR(("wl%d: wlc_bsscfg_tx_start: txq full, frame discarded\n",
			          wlc->pub->unit));
#ifdef WLAMPDU
			/* move the window for the packets freed */
	...
	*highest_used_ratespec = 0x0;
	...
	for (i = 0; i < NTXRATE; i++) {
		...
		if (RSPEC_ISVHT(rspec)) {
			...
			rhist->vht[mcs][nss] += bsscfg->txrspec[i][1];
			...
			/* ASSERT(mcs < WL_RATESET_SZ_HT_MCS * WL_TX_CHAINS_MAX); */
			if (mcs >= WL_RATESET_SZ_HT_MCS * WL_TX_CHAINS_MAX)
