 ...
 *
 * $Id: wlc_channel.c 470995 2014-04-17 05:07:48Z $
 */
...
static int wlc_channels_init(wlc_cm_info_t *wlc_cmi, clm_country_t country);
...
int
wlc_handle_clm_dload(wlc_cm_info_t *wlc_cmi, void* data_chunk, int clm_offset, int chunk_len,
	int clm_total_len, int ds_id)
{
	...
	if (clm_offset == 0) {
		...
		if (download_dataptr != NULL) {
			MFREE(pub->osh, download_dataptr, download_data_len);
			download_dataptr = NULL;
			download_data_len = 0;
		}

		if ((clm_total_len != 0) &&
			(download_dataptr = (struct clm_data_header*)MALLOC(pub->osh,
			clm_total_len)) == NULL) {
	...
	if ((clm_offset + chunk_len) <= download_data_len) {
		...
		if ((clm_offset + chunk_len) == download_data_len) {
			...
			if (ds_id != 0) {
				...
				if (clm_set_inc_data(download_dataptr) != CLM_RESULT_OK) {
					...
					wlc_cm->clm_inc_dataptr = NULL;
					wlc_cm->clm_inc_headerptr = NULL;
					MFREE(pub->osh, download_dataptr,
						download_data_len);
				} else {
				...
				if (wlc_cm->clm_inc_dataptr != NULL) {
					clm_set_inc_data(NULL);
					MFREE(pub->osh, wlc_cm->clm_inc_dataptr,
				...
				if (download_dataptr != NULL) {
					WL_NONE(("wl%d: Pointing API at new base data: v%s\n",
						pub->unit, download_dataptr->clm_version));
			...
			result = wlc_channel_init_ccode(wlc_cmi, country_abbrev,
				sizeof(country_abbrev));
		...
		WL_ERROR(("wl%d: %s: incremental download too big for allocated memory"
			...
			pub->unit, __FUNCTION__, clm_offset, clm_offset + chunk_len,
			download_data_len));
		MFREE(pub->osh, download_dataptr, download_data_len);
		download_dataptr = NULL;
		download_data_len = 0;
...
void
wlc_channel_set_chanspec(wlc_cm_info_t *wlc_cmi, chanspec_t chanspec, uint8 local_constraint_qdbm)
{
	wlc_info_t *wlc = wlc_cmi->wlc;
	ppr_t* txpwr;
...
#define CHSPEC_TO_TX_BW(c)	(\
	(CHSPEC_IS8080(c) || CHSPEC_IS160(c)) ? WL_TX_BW_160 : \
