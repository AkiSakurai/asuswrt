 ...
 *
 * $Id: wlc_tpc.c 468499 2014-04-08 00:00:45Z $
 */
...
wlc_tpc_get_current(wlc_tpc_info_t *tpc, void *pwr, uint len, wlc_bsscfg_t *bsscfg)
{
	...
	clm_power_limits_t limits;		/* 20, 40, 80, 160, 8080 MHz limits */
	clm_power_limits_t limits_subchan1;	/* 20in40, 40in80, 80in160, 80in8080 limits */
#ifdef WL11AC
	clm_power_limits_t limits_subchan2;	/* 20in80, 40in160, 40in8080 limits */
	clm_power_limits_t limits_subchan3; /* 20in160, 20in8080 limits */
	clm_power_limits_t limits_subchan4; /* 8080chan2 limits */
#endif
	clm_limits_type_t sband, ctrl_sband;
	...
	memset(&limits_subchan3, (unsigned char)WL_RATE_DISABLED, sizeof(clm_power_limits_t));
	memset(&limits_subchan4, (unsigned char)WL_RATE_DISABLED, sizeof(clm_power_limits_t));
#endif
	...
	ctrl_sband = clm_chanspec_to_limits_type(chanspec);

	if ((result == CLM_RESULT_OK) && (bw != CLM_BW_20)) {
#ifdef WL11AC
		/* Convert control subchannel to a 1st level subchan (if necessary)
		 * i.e. WL_CHANSPEC_CTL_SB_LLU to WL_CHANSPEC_CTL_SB_L
		 */
		sband = clm_get_enclosing_subchan(ctrl_sband, 1);
#else
		sband = ctrl_sband;
#endif /* WL11AC */

		/* limits_subchan1 - 20in40, 40in80, 80in160, 80in8080 */
		result = clm_limits(&locale, bandtype, chan, ant_gain, sband,
			&lim_params, &limits_subchan1);

		if (result != CLM_RESULT_OK) {
			WL_ERROR(("wlc_tpc_get_current: clm_limits_subchan1 failed\n"));
...
#ifdef WL11AC
	if ((result == CLM_RESULT_OK) &&
		(bw == CLM_BW_80 || bw == CLM_BW_160 || bw == CLM_BW_80_80)) {
		/* Convert control subchannel to a 2nd level subchan (if necessary)
		 * i.e. WL_CHANSPEC_CTL_SB_LLU to WL_CHANSPEC_CTL_SB_LL
		 */
		sband = clm_get_enclosing_subchan(ctrl_sband, 2);

		/* limits_subchan2 - 20in80, 40in160, 40in8080 */
		result = clm_limits(&locale, bandtype, chan, ant_gain, sband,
			&lim_params, &limits_subchan2);

		if (result != CLM_RESULT_OK) {
			WL_ERROR(("wlc_tpc_get_current: clm_limits_subchan2 failed\n"));
			memset(&limits_subchan2, (unsigned char)WL_RATE_DISABLED,
				sizeof(clm_power_limits_t));
		}
	}

	if ((result == CLM_RESULT_OK) && (bw == CLM_BW_160 || bw == CLM_BW_80_80)) {
		/* limits_subchan3 - 20in160, 20in8080 */
		result = clm_limits(&locale, bandtype, chan, ant_gain, ctrl_sband,
			&lim_params, &limits_subchan3);

		if (result != CLM_RESULT_OK) {
			WL_ERROR(("wlc_tpc_get_current: clm_limits_subchan3 failed\n"));
			memset(&limits_subchan3, (unsigned char)WL_RATE_DISABLED,
				sizeof(clm_power_limits_t));
		}
	}

	if (result == CLM_RESULT_OK && bw == CLM_BW_80_80) {
		/* limits_subchan4 - 8080chan2:
		 * Hack: Swap chan1 and chan2 and get 8080 channel CLM limit
		 */
		lim_params.other_80_80_chan = chan;
		chan = wf_chspec_secondary80_channel(chanspec);

		result = clm_limits(&locale, bandtype, chan, ant_gain, CLM_LIMITS_TYPE_CHANNEL,
			&lim_params, &limits_subchan4);

		if (result != CLM_RESULT_OK) {
			WL_ERROR(("wlc_tpc_get_current: clm_limits_subchan4 failed\n"));
			memset(&limits_subchan4, (unsigned char)WL_RATE_DISABLED,
				sizeof(clm_power_limits_t));
		}
	}
#endif /* WL11AC */
	...
	memcpy(&pwr_to_wl->clm_limits_subchan3, &limits_subchan3, WL_NUMRATES);
	memcpy(&pwr_to_wl->clm_limits_subchan4, &limits_subchan4, WL_NUMRATES);
#endif
