 ...
 *
 * $Id: wlc_assoc.c 473062 2014-04-26 22:28:40Z $
 */
...
wlc_cook_join_targets(wlc_bsscfg_t *cfg, bool for_roam, int cur_rssi)
{
	...
	uint32 join_pref_score_size = wlc->as_info->join_targets->count * sizeof(uint32);
	...
	WL_ROAM(("ROAM: RSSI %d, %d roaming targets\n",
		cur_rssi, wlc->as_info->join_targets->count));
...
wlc_join_attempt(wlc_bsscfg_t *cfg)
{
	...
	if (RSDB_ENAB(wlc->pub)) {
		...
		if (to_wlc != from_wlc) {
			WL_ASSOC(("RSDB:wlc mismatch,making join attempt to wl%d.%d from wl%d.%d\n",
				to_wlc->pub->unit, cfg->_idx, wlc->pub->unit, cfg->_idx));

			cfg =  wlc_rsdb_cfg_for_chanspec(from_wlc, cfg, bi->chanspec);
			wlc = to_wlc;
			ASSERT(wlc == cfg->wlc);
			as = cfg->assoc;
			...
			WL_ASSOC(("wl:%d.%d Current Assoc State:%s.. BSS:%s\n",
				wlc->pub->unit, cfg->_idx, wlc_as_st_name(as->state),
				bcm_ether_ntoa(&bi->BSSID, eabuf)));
			wlc_rsdb_join_prep_wlc(wlc, cfg, cfg->SSID, cfg->SSID_len,
				cfg->scan_params, cfg->assoc_params,
	...
	uint assoc_status, bool reassoc, uint bss_type)
{
	...
	if (status == WLC_E_STATUS_SUCCESS) {
		 ...
		 */
		wlc_psta_mode_update(wlc->psta, cfg, addr,
