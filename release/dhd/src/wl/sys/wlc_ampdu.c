 ...
 *
 * $Id: wlc_ampdu.c 472507 2014-04-24 06:30:13Z $
 */
...
BCMATTACHFN(wlc_ampdu_tx_detach)(ampdu_tx_info_t *ampdu_tx)
{
	...
	for (i = 0; i < AMPDU_INI_FREE; i++) {
		if (ampdu_tx->ini_free[i]) {
			MFREE(wlc->osh, ampdu_tx->ini_free[i],
	...
	scb_ampdu_tid_ini_t *ini, uint16 release)
{
				...
				if ((plen <= TCPACKSZSDU) && (prev_p  != NULL)) {
					...
					if (new_pkt) {
						...
						if (phdr) {
						   ...
						   wlc_pkttag_info_move(ampdu_tx->wlc->pub,
						      p, new_pkt);
	...
	for (; release != 0; release--) {
		...
		pktbytes = PKTLEN(ampdu_tx->wlc->osh, p);
		totbytes += pktbytes;
...
wlc_ampdu_calc_maxlen(ampdu_tx_info_t *ampdu_tx, uint8 plcp0, uint plcp3, uint32 txop)
{
	...
	if (txop) {
		...
		maxlen = MIN((txoplen),
			(ampdu_tx_cfg->max_txlen[mcs][is40][sgi] *
			ampdu_tx_cfg->ba_tx_wsize));
	}
...
#endif /* NEW_TXQ */
{
	...
	if (!ini || (ini->ba_state == AMPDU_TID_STATE_BA_PENDING_ON)) {
		...
		WLCNTINCR(ampdu_tx->cnt->orphan);
		if (ini) {
			wlc_ampdu_update_bytes_in_flight(ini, p);
			ini->tx_in_transit--;
		}
		PKTFREE(osh, p, TRUE);
		*pdu = NULL;
...
/** PSPRETEND related feature */
INLINE static bool
wlc_ampdu_cs_retry(ampdu_tx_info_t *ampdu_tx, tx_status_t *txs)
{
...
#define wlc_ampdu_cs_retry(ampdu_tx, txs) FALSE
...
#endif /* NEW_TXQ */
{
	...
	while (TRUE) {
		...
		if (regmpdu_pkt) {
			 ...
			 * not the epoch.
			 */
			if (!change && (hagg->prev_ft == AMPDU_NONE)) {
				change = TRUE;
	...
	void *p, tx_status_t *txs)
{
	...
	if (txs->status.was_acked) {
		ini->acked_seq = seq;
		wlc_ampdu_ini_move_window(ampdu_tx,
			SCB_AMPDU_TX_CUBBY(ampdu_tx, scb), ini);
#if defined(WLAIBSS)
		if (AIBSS_ENAB(ampdu_tx->wlc->pub) && ini->scb && !SCB_ISMULTI(ini->scb)) {
			wlc_bsscfg_t *cfg = SCB_BSSCFG(ini->scb);
			if (cfg && BSSCFG_IBSS(cfg)) {
				aibss_tx_monitor_t	*aibss_scb_tx_stats = SCB_CUBBY((scb),
					WLC_AIBSS_INFO_SCB_HDL(ampdu_tx->wlc->aibss_info));
		...
		if (!(MCHAN_ACTIVE(ampdu_tx->wlc->pub) &&
			...
			if ((AP_ENAB(ampdu_tx->wlc->pub) &&
				...
				/* N.B.: we'll keep this packet to be tx'd when
				 * coming out of absence period but in case we
				 * drop it wlc_ampdu_ini_adjust() will adjust
				 * tx_in_transit...
				 */
				ini->tx_in_transit++;
...
#endif
#if defined(WLAIBSS)
		if (AIBSS_ENAB(ampdu_tx->wlc->pub)) {
			struct dot11_header *h = wlc_pkt_get_d11_hdr(ampdu_tx->wlc, p);
			uint16 fc = ltoh16(h->fc);

			if (ini->scb && !SCB_ISMULTI(ini->scb)) {
				wlc_bsscfg_t *cfg = SCB_BSSCFG(ini->scb);
				if (cfg && BSSCFG_IBSS(cfg) && (FC_TYPE(fc) == FC_TYPE_DATA)) {
					aibss_tx_monitor_t	*aibss_scb_tx_stats =
						SCB_CUBBY((scb),
						WLC_AIBSS_INFO_SCB_HDL(ampdu_tx->wlc->aibss_info));
					aibss_scb_tx_stats->tx_noack_count++;
					wlc_aibss_check_txfail(ampdu_tx->wlc->aibss_info, cfg, scb);
				}
			}
		}
#endif /* WLAIBSS */
	...
	void *p, tx_status_t *txs)
{
	...
	if (txs->status.was_acked) {
		...
		wlc_ampdu_update_bytes_in_flight(ini, p);
		ini->tx_in_transit--;
		wlc_ampdu_ini_move_window(ampdu_tx, SCB_AMPDU_TX_CUBBY(ampdu_tx, scb), ini);
#if defined(WLAIBSS)
		if (AIBSS_ENAB(ampdu_tx->wlc->pub) && ini->scb && !SCB_ISMULTI(ini->scb)) {
			wlc_bsscfg_t *cfg = SCB_BSSCFG(ini->scb);
			if (cfg && BSSCFG_IBSS(cfg)) {
...
#if !defined(WLAMPDU_MAC) && !defined(USING_MUX)
	...
	*/
	...
	defined(BCMDBG_AMPDU))
		...
		*/
		...
		if (acked) {
#if defined(WLAIBSS)
			if (AIBSS_ENAB(ampdu_tx->wlc->pub) && scb && !SCB_ISMULTI(scb)) {
		...
		if (!acked && !pps_retry && !cs_retry) {
			...
			if (AIBSS_ENAB(ampdu_tx->wlc->pub)) {
				struct dot11_header *h =
					(struct dot11_header *)(txh_info->d11HdrPtr);
				uint16 fc = ltoh16(h->fc);
			...
			if (retry) {
				if (AP_ENAB(wlc->pub) &&
					...
					WL_ERROR(("%s: condition not handled\n", __FUNCTION__));
					free_pkt = TRUE;
				}
				if (free_pkt)
					goto not_retry;
	...
	if (nlost) {
		...
		WL_AMPDU_ERR(("wl%d aqm_txs: nlost %d send_bar %d vht %d mcs[0-1] %02x %02x\n",
			wlc->pub->unit, nlost, send_bar, is_vht[0], mcs[0], mcs[1]));
...
ampdu_cleanup_tid_ini(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid, bool force)
{
	...
	if (!ini->bar_ackpending)
		...
		ampdu_tx->ini_free_index =
			MODINC(ampdu_tx->ini_free_index, AMPDU_INI_FREE);
		ini->free_me = TRUE;
	}
...
ampdu_update_max_txlen(ampdu_tx_config_t *ampdu_tx_cfg, uint16 dur)
{
	...
	for (mcs = 0; mcs < MCS_TABLE_SIZE; mcs++) {
		...
		ampdu_tx_cfg->max_txlen[mcs][0][0] = (rate * dur) >> 3;
		/* 40 MHz, No SGI */
		rate = MCS_RATE(mcs, TRUE, FALSE);
		ampdu_tx_cfg->max_txlen[mcs][1][0] = (rate * dur) >> 3;
		/* 20MHz, SGI */
		rate = MCS_RATE(mcs, FALSE, TRUE);
		ampdu_tx_cfg->max_txlen[mcs][0][1] = (rate * dur) >> 3;
		/* 40 MHz, SGI */
		rate = MCS_RATE(mcs, TRUE, TRUE);
		ampdu_tx_cfg->max_txlen[mcs][1][1] = (rate * dur) >> 3;
	}
}
 ...
 */
void wlc_ampdu_ini_adjust(ampdu_tx_info_t *ampdu_tx, struct scb *scb, void *p)
{
	scb_ampdu_tx_t *scb_ampdu_tx;
	scb_ampdu_tid_ini_t *ini;
