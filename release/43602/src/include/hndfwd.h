 ...
 * are maintained, indexed by the GMAC and WLAN radio unit number.
 ...
 * tied to the appropriate CPU core using their respective unit number and they
 * access the per CPU core forwarder object by using the unit number to
 * retrieve the per CPU instance.
 *
 * MultiBSS Radio:
...
#define FWDER_ENABLED_NVAR     "gmac3_enable" /* fwder_attach() */
#define FWDER_WLIFS_NVAR       "wlandevs"     /* fwder_bind() */
 ...
 * forwarder, if it is HW switching capable (nvram "wlandevs").
 */
