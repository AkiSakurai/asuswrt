 ...
 *
 * $Id: dngl_rte.c 488558 2014-07-01 16:05:15Z $
 */
...
static void dngl_sendup_cb(void *lbuf, void *lfrag, void *ctx, bool cancelled);
#endif
...
#ifdef BCMPCIEDEV_ENABLED
static void
dngl_sendup_cb(void *lbuf, void *lfrag, void *ctx, bool cancelled)
{
	struct dngl *dngl = (struct dngl *)ctx;

	if (cancelled) {
		PKTFREE(dngl->osh, lfrag, TRUE);
		return;
	}

	PKTSETNEXT(dngl->osh, lfrag, lbuf);
	PKTSETFRAGTOTLEN(dngl->osh, lfrag, 0);
	PKTSETFRAGLEN(dngl->osh, lfrag, 1, 0);
	PKTSETFRAGTOTNUM(dngl->osh, lfrag, 0);

	/* Bzero the pkttag as it was typecasted into pktfetch_info */
	bzero(PKTTAG(lfrag), sizeof(struct pktfetch_info));

	dngl_sendup(dngl, lfrag);
}
...
dngl_sendup(struct dngl *dngl, void *p)
{
	...
	eh = (struct ether_header*)PKTDATA(dngl->osh, p);

	/* WAR for EAPOL frames pushed to dongle by ext-Sup.
	   NDIS uses exempt bits in packet tag instead of checking ether_type
	   so skip WAR if exempt bits are set.
	*/
	if (PKTISTXFRAG(dngl->osh, p) && PKTFRAGTOTLEN(dngl->osh, p) > 0 &&
		ntoh16(eh->ether_type) == ETHER_TYPE_802_1X &&
		((PKTTAG_GET_VALUE(p) >> 16) & BCMPCIE_PKT_FLAGS_FRAME_EXEMPT_MASK) == 0) {

		struct pktfetch_info *pinfo = (struct pktfetch_info *)PKTTAG(p);
		pinfo->lfrag = p;
		pinfo->headroom = 256;
		pinfo->host_offset = 0;
		pinfo->cb = dngl_sendup_cb;
		pinfo->ctx = (void*)dngl;
		if (hndrte_pktfetch(pinfo) == BCME_OK)
			return;

		err("%s: pktfetch request rejected\n", __FUNCTION__);
	}
#endif /* BCMPCIEDEV_ENABLED */
	...
	while (p != NULL) {
		...
		if (n != NULL)
			n_ifindex = PKTIFINDEX(dngl->osh, n);
#endif /* BCMMSGBUF */
		...
		*/
		if (head != NULL &&
		    (n == NULL || p_drop || p_ifindex != n_ifindex)) {
