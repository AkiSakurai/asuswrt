 ...
 *
 * $Id: rtecdc.c 456127 2014-02-17 23:17:49Z $
 */
...
#ifdef BCMET
extern hndrte_dev_t bcmet;	/* Ethernet device struct */
#else
extern hndrte_dev_t bcmwl;	/* WL device struct */
#endif

extern hndrte_dev_t sdpcmd_dev;
extern hndrte_dev_t usbdev_dev;
extern hndrte_dev_t pciedngl_dev;
extern hndrte_dev_t pciedev_dev;
extern hndrte_dev_t m2md_dev;
...
extern void hnd_patch_init(void *socram_regs);

void __attribute__ ((weak))
BCMATTACHFN(hnd_patch_init)(void *socram_regs)
{
	/* Avoid patch bits execution in roml builds */
	/* hnd_tcam_disablepatch(socram_regs); */
}
#endif /* BCMROMOFFLOAD */
...
#if defined(HNDRTE_CRC32BIN) && defined(BCMSDIODEV_ENABLED) && !defined(mips)
...
BCMATTACHFN(chk_image)(void)
{
	...
	uint8 *crcadr = (uint8 *)hndrte_get_rambase();
	...
	extern uint32 arm_wrap, chiptype, socram_rev;
	extern void *socram_regs, *arm_regs;
	...
	newvals.socram_regs = socram_regs;
	newvals.socram_rev = socram_rev;
#ifdef __ARM_ARCH_7R__
	...
	*(uint32*)0 = newvals.rstvec;
	...
	socram_regs = newvals.socram_regs;
	socram_rev = newvals.socram_rev;
#ifdef __ARM_ARCH_7R__
...
#endif /* HNDRTE_CRC32BIN && BCMSDIODEV_ENABLED && !mips */
...
BCMATTACHFN(c_init)(void)
{
	...
	si_t *sih;
	char chn[8];
	hndrte_dev_t *netdev = NULL;
	hndrte_dev_t *busdev = NULL;
	...
	event_log_set_init(sih, EVENT_LOG_SET_PSM, EVENT_LOG_PSM_BLOCK);
	event_log_set_expand(sih, EVENT_LOG_SET_PSM, EVENT_LOG_PSM_BLOCK);

	event_log_tag_start(EVENT_LOG_TAG_UCODE_WATCHDOG, EVENT_LOG_SET_PSM,
		EVENT_LOG_TAG_FLAG_LOG|EVENT_LOG_TAG_FLAG_PRINT);
	event_log_tag_start(EVENT_LOG_TAG_UCODE_FIFO, EVENT_LOG_SET_WL,
		EVENT_LOG_TAG_FLAG_LOG|EVENT_LOG_TAG_FLAG_PRINT);
	  /* BUS LOG */
	event_log_set_init(sih, EVENT_LOG_SET_BUS, 0x200);
	event_log_set_expand(sih, EVENT_LOG_SET_BUS, 0x200);
	event_log_set_init(sih, EVENT_LOG_SET_DBG, 0x100);
	event_log_set_expand(sih, EVENT_LOG_SET_DBG, 0x100);
...
#endif
	       EPI_VERSION_STR,
	       bcm_chipname(sih->chip, chn, sizeof(chn)), sih->chiprev,
...
#error "Bus type undefined"
	...
	if (hndrte_add_device(&usbdev_dev, USB_CORE_ID, BCM47XX_USBD_ID) == 0 ||
#ifdef USB_XDCI
		((usbdev_dev.flags & (1 << RTEDEVFLAG_USB30)) &&
		(hndrte_add_device(&usbdev_dev, USB30D_CORE_ID, BCM47XX_USB30D_ID) == 0)) ||
#endif
	    hndrte_add_device(&usbdev_dev, USB20D_CORE_ID, BCM47XX_USB20D_ID) == 0 ||
	    hndrte_add_device(&usbdev_dev, USB11D_CORE_ID, BCM47XX_USBD_ID) == 0) {
		busdev = &usbdev_dev;
	...
	if (hndrte_add_device(&sdpcmd_dev, PCMCIA_CORE_ID, SDIOD_FPGA_ID) == 0 ||
	    hndrte_add_device(&sdpcmd_dev, SDIOD_CORE_ID, SDIOD_FPGA_ID) == 0) {
		busdev = &sdpcmd_dev;
	...
	if (hndrte_add_device(&pciedev_dev, PCIE2_CORE_ID, 0x43ff) == 0) {
		busdev = &pciedev_dev;
	...
	if (hndrte_add_device(&m2md_dev, M2MDMA_CORE_ID, 0x4999) == 0) {
		busdev = &m2md_dev;
	...
	if (hndrte_add_device(&pciedngl_dev, PCIE2_CORE_ID, 0x4999) == 0) {
		busdev = &pciedngl_dev;
		bus_ops = &pciedngl_bus_ops;
	}
#endif /* BCM_OL_DEV */

	/* Add the ET or WL device, they are exclusive */
#ifdef BCMET
	trace("add ET device");
	if (hndrte_add_device(&bcmet, ENET_CORE_ID, BCM47XX_ENET_ID) == 0)
		netdev = &bcmet;
#else
	if ((id = si_d11_devid(sih)) == 0xffff)
		id = BCM4318_D11G_ID;
	trace("add WL device 0x%x", id);
	if (hndrte_add_device(&bcmwl, D11_CORE_ID, id) == 0)
		netdev = &bcmwl;
#endif	/* BCMET */
#else
	id = 0;
...
wifi_reflector_init(void)
{
	...
	for (i = 0; i < 8; ++i) {
		...
		bcmwl.funcs->ioctl(&bcmwl, wifi_init_cmds[i].cmd,
		                   buf, wifi_init_cmds[i].len, NULL, NULL, FALSE);
	}
}
#endif /* WIFI_REFLECTOR */

/* c_main is non-reclaimable, as it is the one calling hndrte_reclaim */
void
c_main(void)
{
	si_t *sih;
