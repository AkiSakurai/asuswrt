 ...
 *
 * $Id: hndrte.c 495275 2014-08-06 08:00:24Z $
 */
...
hndrte_rx_reorder_qeueue_flush_cb_info_t rx_reorderqueue_flush;
...
hndrte_pktfetch_dispatch(struct pktfetch_info *pinfo)
{
	...
	if ((lbuf == NULL) && (OSL_MEM_AVAIL() > PKTFETCH_MIN_MEM_FOR_HEAPALLOC)) {
		lbuf = PKTALLOC(hndrte_osh, MAXPKTDATABUFSZ, lbuf_basic);
	}
