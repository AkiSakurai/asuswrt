...
static hndrte_timer_t *timer;

extern uint32 logstrs_size;

static void timer_handle(hndrte_timer_t *timer);


#if defined(EVENT_LOG_ROM_LOG_NUMS_FIX)
	#define FIX_ROM_FMT_NUM(fmt_num) \
		do { \
			if ((fmt_num) > EVENT_LOG_ROM_LOG_NUMS_ADDR) { \
				(fmt_num) -= EVENT_LOG_ROM_LOG_NUMS_ADDR; \
			} \
		} while (0)
#else /* !EVENT_LOG_ROM_LOG_NUMS_FIX */
	#define FIX_ROM_FMT_NUM(fmt_num)
#endif /* EVENT_LOG_ROM_LOG_NUMS_FIX */
...
BCMATTACHFN(event_log_init)(si_t *sih)
{
	...
	/* Start a timer to update to sync the time before the
	 * cycles wraps.
	*/
	if ((timer = hndrte_init_timer(NULL, NULL, timer_handle, NULL)) == NULL) {
		goto err3;
	}

	/* Set the pointer in the debug area */
	hndrte_debug_info.event_log_top = event_log_top;

	/* Put a dummy in the logstrs so that it isn't empty */
	EVENT_LOG(EVENT_LOG_TAG_MAX, " ");

	hndrte_add_timer(timer, 1000, 1);	/* One second */

	return 0;

err3:
	MFREE(si_osh(sih), event_log_tag_sets,
	      sizeof(uint8) * (EVENT_LOG_TAG_MAX + 1));
...
event_log_buffer(int tag, uint8 *buf, int size)
{
	...
	if (size > EVENT_LOG_PSM_BLOCK)
		return;

	num_args = (size + 3) / 4;

	if (flag & EVENT_LOG_TAG_FLAG_PRINT) {
		printf("EL: %x %x", tag & EVENT_LOG_TAG_FLAG_MASK, 0xffff);
		while (i < num_args) {
...
void event_log_time_sync(void)
{
	event_log_top->timestamp = hndrte_time();
	event_log_top->cyclecount = get_arm_cyclecount();
}

void
timer_handle(hndrte_timer_t *timer)
{
	/* Put a periodic timestamp in the log */
	event_log_time_sync();
	hndrte_add_timer(timer, 1000, 1);	/* Refire in one second */
}
