...
struct fwder_if; /* forward declaration */
             ...
             fwder_bypass_fn_t bypass_fn, struct net_device * dev, void * osh)
{
	...
	self->dev_def = dev;
	self->devs_cnt = (dev != FWDER_NET_DEVICE_NULL);
	self->osh = osh;
 ...
 * from the nvram wlandevs to determine eligibility for HW switching.
 *
