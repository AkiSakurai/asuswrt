...
wlc_ht_build_cap_ie(wlc_bsscfg_t *cfg, ht_cap_ie_t *cap_ie, uint8 *supp_mcs, bool is2G)
{
	...
	wlc_prot_n_cap_upd(wlc->prot_n, cfg, &wlc->ht_cap);
	...
	if (TXBF_ENAB(wlc->pub))
		wlc_txbf_upd_ht_bfr_bfe_cap(wlc->txbf, cfg, &cap_ie->txbf_cap);
#endif
...
#ifdef WLP2P
static bool
wlc_vndr_non_p2p_ie_filter(wlc_bsscfg_t *cfg, vndr_ie_t *ie)
{
	uint8 *parse;
	uint parse_len;
...
static bool
wlc_vndr_non_p2p_ie_write_filter(wlc_bsscfg_t *cfg, uint type, vndr_ie_t *ie)
{
	return wlc_vndr_non_p2p_ie_filter(cfg, ie);
}
 ...
 */
static bool
wlc_InTIM(wlc_bsscfg_t *cfg, bcm_tlv_t *tim_ie, uint tim_ie_len, bool *bcmc, uint *dtim_count)
{
	uint pvboff, AIDbyte;
	uint pvblen;
...
wlc_bcn_parse_tim_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	intim = wlc_InTIM(cfg, (bcm_tlv_t *)data->ie, data->ie_len, &bcmc, &dtim_count);

	WL_NONE(("wl%d: %s: intim:%d bcmc:%d dtim_count:%d\n",
	         wlc->pub->unit, __FUNCTION__, intim, bcmc, dtim_count));

#ifdef WLMCHAN
	if (MCHAN_ENAB(wlc->pub)) {
		/* fetch dtim count from beacon */
		cfg->sw_dtim_cnt = (uint16)dtim_count;
		WL_NONE(("wl%d.%d: dtim_count = %d\n",
		         wlc->pub->unit, WLC_BSSCFG_IDX(cfg), dtim_count));
	}
#endif
	...
	if (intim) {
		...
		if (pm->PM == PM_FAST) {
			 ...
			 */
			if (pm->pm2_bcn_sleep_ret_time)
				pm->pm2_sleep_ret_time_left =
					pm->pm2_bcn_sleep_ret_time;
	...
	if (SCAN_IN_PROGRESS(wlc->scan) &&
		...
		if (wlc_lazy_roam_scan_sync_dtim(wlc, cfg) &&
		    (dtim_count == 1)) {
			pm->pm2_sleep_ret_time_left +=
				cfg->current_bss->beacon_period;
			WL_SRSCAN(("Extend background roam scan for next DTIM"));
			WL_ASSOC(("Extend background roam scan for next DTIM"));
		}
		wlc_pm2_sleep_ret_timer_start(cfg);
	}
