 ...
 *
 * $Id: wlc_scb.c 493996 2014-07-30 03:13:02Z $
 */
...
#ifdef DONGLEBUILD
		/* Make sure free_mem never gets below 8K due to scb_allocs */
		   /* Each SCB and associated data structures take up about 4.5 KB */
		      (OSL_MEM_AVAIL() > (13*1024)) &&
#endif
		1) {
#ifdef SCBFREELIST
		/* If not found on freelist then allocate a new one */
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (scb->mac_address_handle &&
			...
			if (BSSCFG_AP(scb->bsscfg)) {
				ret = wlc_apps_pvb_chkupdate_from_host(wlc, scb);
				if (!ret) {
					ta_bmp = (ta_bmp & ~(0x1 << tid));
					SCB_PROPTXTSTATUS_SETTIM(scb, ta_bmp);
				}
	...
	uint16 flowid, uint8 tid)
{
	...
	while ((pkt = pktq_deq(&wlc->active_queue->q, &prec))) {
	...
	while ((pkt = pktq_deq(&tmp_q, &prec))) {
		pktq_penq(&wlc->active_queue->q, prec, pkt);
	}
}
