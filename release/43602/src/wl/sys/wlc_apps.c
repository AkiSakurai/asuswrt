 ...
 *
 * $Id: wlc_apps.c 524318 2015-01-06 14:00:39Z $
 */
...
wlc_apps_psq(wlc_info_t *wlc, void *pkt, int prec)
{
	...
	if (AIBSS_ENAB(wlc->pub))
		wlc_aibss_notify_ps_pkt(wlc->aibss_info, SCB_BSSCFG(scb), scb, pkt);
#endif /* WLAIBSS */
...
bool
wlc_apps_pvb_chkupdate_from_host(wlc_info_t *wlc, struct scb *scb)
{
	if (PROP_TXSTATUS_ENAB(wlc->pub) && BCMPCIEDEV_ENAB(wlc->pub)) {
		struct apps_wlc_psinfo *wlc_psinfo;
		struct apps_scb_psinfo *scb_psinfo;
...
wlc_apps_pvb_update(wlc_info_t *wlc, struct scb *scb)
{
	...
	*/
	if (ps_count == 0 && PROP_TXSTATUS_ENAB(wlc->pub) &&
		...
		if (scb->apsd.ac_delv == AC_BITMAP_NONE ||
			...
			ps_count = ((SCB_PROPTXTSTATUS_TIM(scb) &
				(~scb->apsd.ac_delv & AC_BITMAP_ALL)) ? 1 : 0);
		}
	}
#endif /* PROP_TXSTATUS */
	...
	if (wlc_taf_enabled(wlc->taf_handle)) {
		if (!ps_count && scb->traffic_active) {
			ps_count = 1;
		}
	}
#endif
...
wlc_apps_suppr_frame_enq(wlc_info_t *wlc, void *pkt, tx_status_t *txs, bool last_frag)
{
	...
	if (SCB_PS(scb) && !SCB_ISMULTI(scb) && !(WLPKTTAG(pkt)->flags & WLF_PSDONTQ)) {
		...
		WL_ERROR(("wl%d.%d: %s: "MACF" ps suppr pkt discarded\n", wlc->pub->unit,
