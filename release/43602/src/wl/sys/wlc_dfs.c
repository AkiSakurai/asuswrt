...
struct wlc_dfs_info {
	...
	bool radar_detected;		/* is radar detected */
};
...
wlc_dfs_chanspec(wlc_dfs_info_t *dfs)
{
	...
	for (chan = 0; chan < MAXCHANNEL; chan++) {
		chspec = CH20MHZ_CHSPEC(chan);
		if (wlc_valid_ap_chanspec(dfs, chspec)) {
			if (support_non_radar_chan && wlc_radar_chanspec(wlc->cmi, chspec) &&
				dfs->radar_detected)
				continue;
			else {
	 ...
	 */
	if (N_ENAB(wlc->pub) &&
	    CHSPEC_BW_GE(wlc->default_bss->chanspec, WL_CHANSPEC_BW_40) &&
	    !(wlc_channel_locale_flags(wlc->cmi) & WLC_NO_40MHZ)) {
		/* walk the channels looking for good 40MHz channels */
		for (chan = 0; chan < MAXCHANNEL; chan++) {
			chspec = CHBW_CHSPEC(WL_CHANSPEC_BW_40, chan);
			if (wlc_valid_ap_chanspec(dfs, chspec)) {
				if (support_non_radar_chan && wlc_radar_chanspec(wlc->cmi, chspec) &&
					dfs->radar_detected)
					continue;
...
#ifdef CLIENT_CSA
		if (BSSCFG_STA(wlc->cfg) && (wlc->cfg->_dwds)) {
