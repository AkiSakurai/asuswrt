 ...
 *
 * $Id: wlc_assoc.c 495046 2014-08-05 09:06:04Z $
 */
...
static uint32
wlc_bss_pref_score_rssi(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi, int16 rssi)
{
#ifdef WLWNM
	uint32 score;
	...
	return WLCMAXVAL(WLC_JOIN_PREF_BITS_RSSI) + rssi;
}

static uint32
wlc_bss_pref_score(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi, bool band_rssi_boost)
{
	wlc_info_t *wlc = cfg->wlc;
	...
	for (j = 0, weight = 0; j < (int)join_pref->fields; j ++) {
		...
		case WL_JOIN_PREF_RSSI:
			value = wlc_bss_pref_score_rssi(cfg, bi, rssi);
			break;
	...
	if (join_pref->fields == 0) {
		weight = wlc_bss_pref_score_rssi(cfg, bi, rssi);
...
wlc_cook_join_targets(wlc_bsscfg_t *cfg, bool for_roam, int cur_rssi)
{
	...
	for (i = 0; i < (int)wlc->join_targets->count; i++) {
		if ((roam->reason == WLC_E_REASON_BSSTRANS_REQ) &&
			...
			join_pref_score[i] = wlc_bss_pref_score(cfg, bip[i], TRUE);
		}
	...
	if (for_roam &&
		...
		if (wlc_bss_connected(cfg) && roam->reason == WLC_E_REASON_LOW_RSSI) {
			...
			WL_ASSOC(("  curr: bssid=%02x:%02x ch=%d rssi=%d\n",
				current_bss->BSSID.octet[4], current_bss->BSSID.octet[5],
				wf_chspec_ctlchan(current_bss->chanspec), cur_rssi));
			cur_pref_score = wlc_bss_pref_score(cfg, current_bss, FALSE);
		...
		WL_SRSCAN(("  criteria: score=%d roam_delta=%d",
			cur_pref_score, roam_delta));
		WL_ASSOC(("  criteria: score=%d roam_delta=%d\n",
			cur_pref_score, roam_delta));
		...
		for (j = 0; i < (int)wlc->join_targets->count; i++) {
			...
			if (wlc_bss_connected(cfg) &&
			   ...
			   (wlc_bss_pref_score(cfg, bip[i], FALSE) <=
				(cur_pref_score + roam_delta)))
				continue;
			...
			WL_ROAM(("ROAM:%d %s %d(%d) Q%d\n", j,
				bcm_ether_ntoa(&bip[j]->BSSID, eabuf), join_pref_score[j],
				(int)join_pref_score[j] - 255, bip[j]->qbss_load_chan_free));
			j++;
		}
	...
	if (CCX_ENAB(wlc->pub) && wlc->join_targets_last > 1) {
		...
		max = join_pref_score[wlc->join_targets_last - 1];
		...
		for (i = 0; i < (int)wlc->join_targets_last; i++) {
			if (join_pref_score[i] >= threshold) {
				break;
			}
		}
...
wlc_assoc_scan_complete(void *arg, int status, wlc_bsscfg_t *cfg)
{
	...
	if ((PSTA_IS_REPEATER(wlc) || DWDS_ENAB(cfg)) && (wlc->join_targets->count > 0)) {
		struct scb *scb = NULL;
		FOREACH_BSS(wlc, idx, valid_cfg) {
			scb = wlc_scbfind(wlc, valid_cfg,
				(struct ether_addr *)&wlc->join_targets->ptrs[0]->BSSID);
...
wlc_join_wsec_filter(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (WSEC_AES_ENABLED(cfg->wsec)) {
		...
		if ((UCAST_NONE(rsn) && MCAST_AES(rsn)) || (MCAST_AES(rsn) && UCAST_AES(rsn)) ||
			...
			/* check for the mixed mode */
			if (WSEC_TKIP_ENABLED(cfg->wsec)) {
				if ((cfg->tk_cm_bt == 0) && ((UCAST_NONE(rsn) && MCAST_TKIP(rsn)) ||
					(!MCAST_NONE(rsn) && UCAST_TKIP(rsn))))
					prune = FALSE;
				else {
					WL_ASSOC(("wl%d: JOIN: BSSID %s mixed mode(AES+TKIP): "
					"no mixed mode (AES+TKIP) support  or TKIP cm bt %d\n",
						WLCWLUNIT(wlc),	bcm_ether_ntoa(&bi->BSSID, eabuf),
						cfg->tk_cm_bt));
					wlc_bss_mac_event(wlc, cfg, WLC_E_PRUNE, &bi->BSSID, 0,
...
wlc_join_BSS(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	/* N.B.: bss_type passed through auth_type event field */
	if (WLEXTSTA_ENAB(wlc->pub)) {
		wlc_bss_mac_event(wlc, cfg, WLC_E_ASSOC_START, &bi->BSSID, 0,
			(as->type != AS_ASSOCIATION ? roam->reason : 0),
			WLC_DOT11_BSSTYPE(bi->infra), NULL, 0);
	}
#endif /* EXT_STA */
...
wlc_sta_assoc_upd(wlc_bsscfg_t *cfg, bool state)
{
	...
	if (WLC_MODESW_ENAB(wlc->pub))
		wlc_bss_assoc_state_notif(wlc, cfg, cfg->assoc->state, cfg->assoc->type);
}
...
do_event:
	if (!BSSCFG_ROAMOFFL(cfg->roamoffl) || !cfg->roam->active || status != WLC_E_STATUS_SUCCESS)
...
wlc_roam_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
	...
	if (WLWNM_ENAB(wlc->pub)) {
		...
		*/
		wlc_wnm_bsstrans_roamscan_complete(wlc, DOT11_BSSTRANS_RESP_STATUS_REJECT, NULL);
	}
#endif /* WLWNM */
#ifdef EXT_STA
	if (WLEXTSTA_ENAB(wlc->pub)) {
...
do_event:
	...
	/* N.B.: bss_type passed through auth_type event field */
	if (!BSSCFG_ROAMOFFL(cfg->roamoffl) && status == WLC_E_STATUS_SUCCESS)
	...
	if (WLFBT_ENAB(wlc->pub) && (cfg->WPA_auth & WPA2_AUTH_FT)) {
		...
		wlc_bss_mac_event(wlc, cfg, WLC_E_ROAM, addr, status, roam->reason, bss_type,
			ft_key, sizeof(ft_key));
	} else
#endif /* WLFBT */
...
void wlc_roam_prof_update(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool reset)
{
	...
	if (wlc->band->roam_prof) {
		...
		if (NBANDS(wlc) > 1) {
			...
			cfg->join_pref_rssi_delta.band =
				(uint8)wlc->bandstate[OTHERBANDUNIT(wlc)]->bandtype;
#else
		...
		if ((NBANDS(wlc) > 1) && reset) {
			...
			if (band->roam_prof) {
				band->roam_trigger = band->roam_prof->roam_trigger;
				band->roam_delta = band->roam_prof->roam_delta;
			}
#else
...
#ifdef NO_ROAMOFFL_SUPPORT
		WL_SRSCAN(("ROAM prof[%d]: trigger=%d delta=%d\n",
			idx, wlc->band->roam_trigger, wlc->band->roam_delta));
		WL_ASSOC(("ROAM prof[%d]: trigger=%d delta=%d\n",
			idx, wlc->band->roam_trigger, wlc->band->roam_delta));
#else
