...
#ifdef EMBED_IMAGE_4354PCI
#include <rtecdc_4354pci.h>
#endif /* EMBED_IMAGE_4354PCI */
...
static olmsg_rssi_init rssi_init_prev;
...
static int wlc_ol_watchdog(void *context);
...
static void wlc_ol_key_update(ol_sec_info *dkey, wsec_key_t *skey);
...
wlc_ol_cap(wlc_info_t *wlc)
{
	...
	if (wlc->ol == NULL) {
		if (wlc->pub->sih->chip == BCM43569_CHIP_ID ||
			wlc->pub->sih->chip == BCM43570_CHIP_ID) {
			WL_ERROR(("%s: Offload support temporarily disabled for 43569/43570\n",
				__FUNCTION__));
			return FALSE;
		}
		...
		if (wlc->pub->sih->chip == BCM43457_CHIP_ID) {
			WL_ERROR(("%s: Offload support temporarily disabled for 4345\n",
				__FUNCTION__));
			return FALSE;
		}
#endif
		if ((wlc->pub->sih->chip == BCM4335_CHIP_ID) && D11REV_IS(wlc->pub->corerev, 46)) {
			WL_ERROR(("%s: Offload support temporarily disabled for 43162\n",
				__FUNCTION__));
			return FALSE;
		}
	...
	uint32 event_time, uint32 event_data)
{
	switch (event_type) {
		...
		case WLC_EL_BEACON_IE_TX_STOP:
			bcm_bprintf(b, "IE %d changed, TX stopped\n", event_data);
			break;
	}
}
...
BCMATTACHFN(wlc_ol_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_module_register(wlc->pub, ol_iovars, "offloads", ol, wlc_ol_doiovar,
	                        watchdog_fn, wlc_ol_up, wlc_ol_down)) {
	...
	wlc_eventlog_register_print_handler(ol, WLC_EL_BEACON_IE_CHANGED,
		wlc_beacon_eventlog_handler);
	wlc_eventlog_register_print_handler(ol, WLC_EL_BEACON_IE_TX_STOP,
		wlc_beacon_eventlog_handler);
...
#ifdef EMBED_IMAGE_4354PCI
		case BCM4354_CHIP_ID:
			ol->dlarray = dlarray_4354pci;
			ol->dlarray_len = sizeof(dlarray_4354pci);
			break;
#endif /* EMBED_IMAGE_4354PCI */
#ifdef EMBED_IMAGE_43602PCI
		case BCM43602_CHIP_ID:
...
wlc_ol_watchdog(void *context)
{
	...
	if (curr_cntr == ol->prev_cntr)
	{
		...
		{
			WL_ERROR(("%s: ARM heartbeat stuck at %d.", __FUNCTION__, curr_cntr));
...
wlc_ol_stats(void *context, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\nPacket dropped due to TX Stop: %u\n",
		ltoh32(shared_info->stats.rxoe_tx_stopcnt));
...
static int wlc_ol_go(wlc_ol_info_t *ol)
{
	...
	ol->shared_info->dngl_watchdog_cntr = 0;

	if ((CHIPID(ol->sih->chip) == BCM43602_CHIP_ID) ||
	    (CHIPID(ol->sih->chip) == BCM43462_CHIP_ID)) {
		...
		switch (hdr->type) {
			...
			case BCM_OL_LOW_RSSI_NOTIFICATION: {
				...
				!cfg->roam->off) {
					...
					WL_ERROR(("%s: Offload Engine RSSI NOTIFICATION: %d\n",
					__FUNCTION__, rssi));
					...
					if (rssi < ol->wlc->band->roam_trigger) {
						/* Start scan if rssi is below trigger point */
...
static void
wlc_ol_key_sw_wowl_update(wlc_info_t *wlc,
	wsec_key_t *key, ol_sec_info *skey)
{
	int i;
	WL_INFORM(("%s\n", __FUNCTION__));

	/* For WEP, skip copying rxiv */
	if (key->algo != CRYPTO_ALGO_WEP1 && key->algo != CRYPTO_ALGO_WEP128) {
		ASSERT(key->rxiv != NULL);

		for (i = 0; i < MIN(wlc->pub->tunables->num_rxivs, NUMRXIVS); i++) {
			key->rxiv[i].hi = skey->rxiv[i].hi;
			key->rxiv[i].lo = skey->rxiv[i].lo;
			WL_INFORM(("%s: prio:%d rxiv:high %d rxiv:low %d \n", __FUNCTION__,
				i, key->rxiv[i].hi, key->rxiv[i].lo));
		}
	}

	if (key->algo == CRYPTO_ALGO_TKIP) {
		bcopy((uint8 *)(skey->tkip_rx.phase1),
			(uint8 *)(key->tkip_rx.phase1), TKHASH_P1_KEY_SIZE);
	}
	bcopy((uint8 *)(skey->data), (uint8 *)(key->data), DOT11_MAX_KEY_SIZE);
}

static int
wlc_ol_key_sw_wowl_create(wlc_info_t *wlc, int rcmta_index, ol_sec_info *skey)
{
	wsec_iv_t key_iv;
	wsec_iv_t *initial_iv = NULL;
	WL_INFORM(("%s key_id:%d\n", __FUNCTION__, skey->id));
	memset(&key_iv, 0, sizeof(wsec_iv_t));
	if (skey->iv_initialized) {
		key_iv.hi = skey->rxiv[rcmta_index].hi;
		key_iv.lo = skey->rxiv[rcmta_index].lo;
		initial_iv = &key_iv;
	}
	return wlc_key_insert(wlc, wlc->cfg, skey->len, skey->id, skey->flags,
	                      skey->algo, skey->data, &skey->ea, initial_iv, NULL);
}

static void wlc_ol_wowl_key_update(wlc_ol_info_t *ol, void *msg)
{
	wlc_info_t *wlc = ol->wlc;
	int idx;
	...
	WL_INFORM(("func %s \n", __FUNCTION__));
	/* Read the TSC/PN information and update scb_key and default keys */
	for (idx = 0; idx < WLC_DEFAULT_KEYS; idx++) {
		/* update default key if non-empty. If it was empty in software
		 * but non-empty now because a key rotation may have happened, then
		 * recreate the key from information from Shared memory
		 */
		 WL_INFORM(("idx:%d\n", idx));
		if (wlc->wsec_def_keys[idx]->len) {
			WL_INFORM(("wl%d: wlc_ol_wowl_key_update: Updating key %d\n",
			          wlc->pub->unit, idx));
			wlc_ol_key_sw_wowl_update(wlc, wlc->wsec_def_keys[idx],
				(ol_sec_info *)&txdone_msg->txinfo.defaultkeys[idx]);
		} else if (txdone_msg->txinfo.key_rot_indx == idx) {
			WL_INFORM(("wlc_ol_wowl_key_update: Updating key %d rot_index:%d aidx:%d\n",
			          idx, txdone_msg->txinfo.key_rot_indx, idx));
			if (wlc_ol_key_sw_wowl_create(wlc, idx,
				(ol_sec_info *)&txdone_msg->txinfo.defaultkeys[idx]) == 0)
				wlc_ol_key_sw_wowl_update(wlc, wlc->wsec_def_keys[idx],
				(ol_sec_info *)&txdone_msg->txinfo.defaultkeys[idx]);
			else
				WL_INFORM(("wl%d: wlc_ol_wowl_key_update: Creating key %d failed\n",
				          wlc->pub->unit, idx));
		}
	}
...
void wlc_ol_armtxdone(wlc_ol_info_t *ol, void *msg)
{
	...
	if (key) {
		if (key->algo == CRYPTO_ALGO_TKIP) {
			bcopy((uint8 *)(txdone_msg->txinfo.key.tkip_tx.phase1),
				(uint8 *)(key->tkip_tx.phase1), TKHASH_P1_KEY_SIZE);
			if (ol->ol_flags & OL_WOWL_ENAB) {
				bcopy((uint8 *)(txdone_msg->txinfo.key.tkip_rx.phase1),
					(uint8 *)(key->tkip_rx.phase1), TKHASH_P1_KEY_SIZE);
				/* copy rxiv */
				ASSERT(key->rxiv);
				for (i = 0; i < MIN(wlc->pub->tunables->num_rxivs, NUMRXIVS); i++) {
					key->rxiv[i].hi = txdone_msg->txinfo.key.rxiv[i].hi;
					key->rxiv[i].lo = txdone_msg->txinfo.key.rxiv[i].lo;
					WL_INFORM(("%s: unicast prio:%d: rxiv:high \n"
						"%d rxiv:low %d\n",
						__FUNCTION__,
						i, key->rxiv[i].hi, key->rxiv[i].lo));
				}
			...
			/* AES and anyone else */
			if (ol->ol_flags & OL_WOWL_ENAB) {
				key->txiv.hi = txdone_msg->txinfo.key.txiv.hi;
				key->txiv.lo = txdone_msg->txinfo.key.txiv.lo;
			}
		}
	}

	if (ol->ol_flags & OL_WOWL_ENAB && (WSEC_ENABLED(cfg->wsec))) {
		if (scb->key) {
			WL_INFORM(("unicast key update \n"));
			wlc_ol_key_sw_wowl_update(wlc, scb->key,
				(ol_sec_info *)&txdone_msg->txinfo.key);
		}
		wlc_ol_wowl_key_update(ol, txdone_msg);
	}
#ifdef MFP
	if (WLC_MFP_ENAB(wlc->pub) && SCB_MFP(scb))
		/* ol_sec_igtk_info is defined to be same as
		 * wlc_mfp_igtk_info_t
		 */
		wlc_mfp_set_igtk(wlc->mfp, cfg, (wlc_mfp_igtk_info_t*)
			&txdone_msg->txinfo.igtk);
#endif /* MFP */
...
wlc_ol_scan_params(wlc_ol_info_t *ol)
{
	...
	if (nssid > 0)
		size += sizeof(wlc_ssid_t) * (nssid - 1);
	size += sizeof(chanspec_t) * nchan;
	msg = (olmsg_scanparams *)MALLOC(ol->osh, size);
	if (msg == NULL)
		return BCME_NOMEM;
	bzero(msg, size);
...
/* Safe enable/disable of offloads; arbitrate multiple components that may request enab/disab */
/* enables only when no disable requests are current */
...
static void wlc_ol_set_default_iemask(wlc_ol_info_t *ol)
{
	...
	SET_ID(ol->ie_info.iemask, DOT11_MNG_EXT_RATES_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_ERP_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_NONERP_ID);

	/* Channel Switch group */
	SET_ID(ol->ie_info.iemask, DOT11_MNG_CHANNEL_SWITCH_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_EXT_CSA_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_EXT_CHANNEL_OFFSET);

	/* 802.11 Compliance Group */
	SET_ID(ol->ie_info.iemask, DOT11_MNG_QUIET_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_PWR_CONSTRAINT_ID);

	/* HT/VHT Group */
	SET_ID(ol->ie_info.iemask, DOT11_MNG_HT_CAP);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_HT_ADD);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_HT_OBSS_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_VHT_CAP_ID);
	SET_ID(ol->ie_info.iemask, DOT11_MNG_VHT_OPERATION_ID);
...
wlc_ol_bcn_enable(wlc_ol_info_t *ol, wlc_bsscfg_t *cfg)
{
	...
	p_bcn_enb->ctrl_channel = wf_chspec_ctlchan(current_bss->chanspec);
...
wlc_ol_arp_enable(wlc_ol_info_t *ol, wlc_bsscfg_t *cfg, struct ipv4_addr *host_ip)
{
	...
	arp_enable.wsec = cfg->wsec;
	/* update default keys  */
	if (WSEC_ENABLED(cfg->wsec)) {
		for (i = 0; i < WLC_DEFAULT_KEYS; i++) {
			if (cfg->bss_def_keys[i] != NULL)
				wlc_ol_key_update((ol_sec_info *)&arp_enable.defaultkeys[i],
				cfg->bss_def_keys[i]);
		}
	}
...
wlc_ol_arp_disable(wlc_ol_info_t *ol)
{
	...
	if (ol->ol_flags & OL_ARP_ENAB) {
		...
		/* Both ARP and ND disabled.  Hence disable ARM TX */
		if (!(ol->ol_flags & OL_ND_ENAB))
...
wlc_ol_nd_enable(wlc_ol_info_t *ol, wlc_bsscfg_t *cfg)
{
	...
	nd_enable.wsec = cfg->wsec;
	/* update default keys  */
	if (WSEC_ENABLED(cfg->wsec)) {
		for (i = 0; i < WLC_DEFAULT_KEYS; i++) {
			if (cfg->bss_def_keys[i] != NULL)
				wlc_ol_key_update((ol_sec_info *)&nd_enable.defaultkeys[i],
				cfg->bss_def_keys[i]);
		}
	}
...
wlc_ol_pkt_filter_add(wlc_ol_info_t *ol, wl_wowl_pattern_t *wowl_pattern, uint wowl_pattern_len)
{
	...
	if (ol->ol_flags & OL_PKT_FILTER_ENAB) {
		pkt_filter_add = (olmsg_pkt_filter_add *)MALLOC(ol->osh, msg_len);
	...
	uint wowl_enable_len)
{
	 ...
	 * Note: This is called from wlc_wowl_enable and cfg->up is not 1.
	 */
	if (!ol || !ol->ol_up || !cfg || !BSSCFG_STA(cfg) || !cfg->BSS ||
...
void wlc_ol_enable(wlc_ol_info_t *ol, wlc_bsscfg_t *cfg)
{
	...
	/* Offload is enabled turn off MI_BG_NOISE interrupt */
	WL_TRACE(("%s: Offloads enabled turning off MI_BG_NOISE n", __FUNCTION__));
...
void wlc_ol_disable(wlc_ol_info_t *ol, wlc_bsscfg_t *cfg)
{
	...
	if (ol->ol_flags == 0) {
		/* Offload is disabled, turn on MI_BG_NOISE interrupt */
		WL_TRACE(("%s: Offloads disabled turning on MI_BG_NOISE\n", __FUNCTION__));
		wlc_bmac_set_defmacintmask(ol->wlc->hw, MI_BG_NOISE, MI_BG_NOISE);
...
void wlc_ol_key_update(ol_sec_info *dkey, wsec_key_t *skey)
{
	int i = 0;
	if (dkey && skey) {
		dkey->iv_len = skey->iv_len;
		dkey->icv_len = skey->icv_len;
		dkey->idx = skey->idx;
		dkey->id = skey->id;
		dkey->algo = skey->algo;
		dkey->algo_hw = skey->algo_hw;
		dkey->len = skey->len;
		bcopy(&skey->data, &dkey->data, DOT11_MAX_KEY_SIZE);
		for (i = 0; i < ARRAYSIZE(dkey->rxiv); i++) {
			if (skey->rxiv) {
				dkey->rxiv[i].hi = skey->rxiv[i].hi;
				dkey->rxiv[i].lo = skey->rxiv[i].lo;
			} else {
				dkey->rxiv[i].hi = 0;
				dkey->rxiv[i].lo = 0;
			}
			WL_INFORM(("%s: prio:%d rxiv:high %d rxiv:low %d \n", __FUNCTION__,
				i, dkey->rxiv[i].hi, dkey->rxiv[i].lo));
		}

		if (dkey->algo == CRYPTO_ALGO_TKIP) {
			bcopy((uint8 *)&(skey->tkip_rx.phase1[0]),
				(uint8 *)&(dkey->tkip_rx.phase1[0]), TKHASH_P1_KEY_SIZE);
			bcopy((uint8 *)&(skey->tkip_tx.phase1[0]),
				(uint8 *)&(dkey->tkip_tx.phase1[0]), TKHASH_P1_KEY_SIZE);

		}
		/* AES and anybody else */
		dkey->txiv.hi = skey->txiv.hi;
		dkey->txiv.lo = skey->txiv.lo;

		WL_INFORM(("%s: Algo %d, key id %d, txiv hi:0x%x lo:0x%x iv_len:%d\n", __FUNCTION__,
			dkey->algo, dkey->id, dkey->txiv.hi, dkey->txiv.lo, dkey->iv_len));
...
void wlc_ol_armtx(wlc_ol_info_t *ol, uint8 txEnable)
{
	wlc_info_t *wlc;
	struct scb *scb = NULL;
	...
	if ((ol == NULL) || (ol->wlc == NULL) || (ol->ol_up == FALSE)) {
		return;
	}
	...
	if (!scb) {
		WL_ERROR(("can't enable arm tx ps:%d armtxstate:%d scb %p\n",
		txEnable, ol->ol_arm_txenable, scb));
		return;
	}


	WL_TRACE(("%s current ARM tx enable:  %d new ps: %d\n",
	...
	if (txEnable) {
		...
		if (wlc->wowl) {
			i = EAPOL_KEY_REPLAY_LEN;
			wlc_wowl_get_replay_counter(wlc->wowl,
				&armtx_msg.txinfo.replay_counter, &i);
		}
#endif

		if (WSEC_ENABLED(cfg->wsec) && (key != NULL)) {
			armtx_msg.txinfo.hwmic = WSEC_HW_TKMIC_KEY(wlc, key, cfg);
			WL_TRACE(("hwmic %d\n", armtx_msg.txinfo.hwmic));
			wlc_ol_key_update(&armtx_msg.txinfo.key, key);
			for (i = 0; i < WLC_DEFAULT_KEYS; i++) {
				if (cfg->bss_def_keys[i] != NULL) {
					wlc_ol_key_update(
					(ol_sec_info *)&armtx_msg.txinfo.defaultkeys[i],
					cfg->bss_def_keys[i]);
				}
			}
		}
#ifdef MFP
		if (WLC_MFP_ENAB(wlc->pub) && SCB_MFP(scb)) {
			wlc_mfp_igtk_ipn(wlc->mfp, cfg,
				&armtx_msg.txinfo.igtk.ipn_lo,
				&armtx_msg.txinfo.igtk.ipn_hi);
			armtx_msg.txinfo.igtk.id =
				wlc_mfp_igtk_id(wlc->mfp, cfg);
			wlc_mfp_igtk_key(wlc->mfp, cfg,
				(uint8*)&armtx_msg.txinfo.igtk.key);
			armtx_msg.txinfo.igtk.key_len = AES_TK_LEN;
		}
#endif /* MFP */
	}
...
void wlc_ol_groupkey_update(wlc_ol_info_t *ol, wlc_bsscfg_t *cfg)
{
	...
	if (!ol || !cfg || !BSSCFG_STA(cfg) || !cfg->BSS || !cfg->up ||
		cfg != ol->wlc->cfg || !cfg->associated || !ol->num_bsscfg_allow ||
		!(ol->ol_up)) {
		WL_TRACE(("%s: Invalid params/state\n",
			__FUNCTION__));
		return;
	}

	if (!wlc_ol_arp_nd_enabled(ol))
		return;

	bzero(&gtkupd, sizeof(olmsg_groupkeyupd));
	gtkupd.hdr.type = BCM_OL_GTK_UPD;
	gtkupd.hdr.seq = 0;
	gtkupd.hdr.len = sizeof(olmsg_groupkeyupd) - sizeof(olmsg_header);
	for (i = 0; i < WLC_DEFAULT_KEYS; i++) {
		if (cfg->bss_def_keys[i] != NULL) {
			wlc_ol_key_update((ol_sec_info *)&gtkupd.defaultkeys[i],
				cfg->bss_def_keys[i]);
		}
	}
	wlc_ol_msg_send(ol, (uint8 *)&gtkupd, sizeof(olmsg_groupkeyupd));
	return;
}
	...
	rsn_rekey_params *rkey, struct scb *scb, int wpaauth)
{
	...
	gtk_enable.WPA_auth = (uint16)wpaauth;
	bcopy(rkey, &gtk_enable.rekey, sizeof(rsn_rekey_params));

	gtk_enable.m_seckindxalgo_blk = wlc->pub->m_seckindxalgo_blk;
	gtk_enable.m_seckindxalgo_blk_sz = wlc->pub->m_seckindxalgo_blk_sz;
	gtk_enable.seckeys = wlc->seckeys;
	gtk_enable.tkmickeys = wlc->tkmickeys;
	gtk_enable.tx_tkmic_offset = wlc_wsec_tx_tkmic_offset(wlc->pub, wlc->cfg, scb);
	gtk_enable.rx_tkmic_offset = wlc_wsec_rx_tkmic_offset(wlc->pub, wlc->cfg, scb);
	gtk_enable.flags = scb->flags;
...
wlc_ol_ltr(wlc_ol_info_t *ol, wlc_ltr_info_t *ltr_info)
{
	...
	ltr.ltr_sw_en = ltr_info->ltr_sw_en;
	ltr.ltr_hw_en = ltr_info->ltr_hw_en;
...
void wlc_update_seq_iv(wlc_info_t *wlc, bool frmShm, struct scb *scb)
{
	...
	if (scb != NULL) {
		if (frmShm) {
			SCB_SEQNUM(scb, PRIO_8021D_BK)
				= wlc_read_shm(wlc, M_SEQNUM_TID);
			/* ucode's ivs store the last frame's IVs.
			* So driver needs to store the
			* next expected ones
			*/
			if (WSEC_SCB_KEY_VALID(scb)) {
				key = scb->key;
				if ((key->algo == CRYPTO_ALGO_TKIP) ||
					...
					uint32 hi = 0;
					uint16 lo = 0;
					bzero((uchar *)&iv_rc, sizeof(iv_rc));
					wlc_copyfrom_shm(wlc, M_REPCNT_TID,
						iv_rc, sizeof(iv_rc));
					lo = iv_rc[0];
					lo =  (lo + (iv_rc[1] << 8));
					key->txiv.lo = lo;
			...
			if (WSEC_SCB_KEY_VALID(scb)) {
				...
				if ((key->algo == CRYPTO_ALGO_TKIP) ||
					(key->algo == CRYPTO_ALGO_AES_CCM)) {
					bzero((uchar *)&iv_rc, sizeof(iv_rc));
					iv_rc[0] = key->txiv.lo & 0xff;
					iv_rc[1] = (key->txiv.lo >> 8) & 0xff;
					iv_rc[2] = key->txiv.hi & 0xff;
					iv_rc[3] = (key->txiv.hi >>  8) & 0xff;
					iv_rc[4] = (key->txiv.hi >> 16) & 0xff;
					iv_rc[5] = (key->txiv.hi >> 24) & 0xff;
					wlc_copyto_shm(wlc, M_REPCNT_TID,
						iv_rc, (EAPOL_KEY_REPLAY_LEN - 2));
				}
			}
		}
	}
