 ...
 *
 * $Id: wlc_ccx.c 473070 2014-04-27 05:20:34Z $
 */
...
wlc_ccx_rm_pathloss_begin(wlc_ccx_info_t *ci, wlc_rm_req_t* req)
{
	...
	wlc_phy_txpower_get(ci->wlc->band->pi, &ccx_rm->cur_txpower, &ccx_rm->cur_txpoweroverride);
...
wlc_ccx_rm_pathloss_complete(ccx_t *ccxh, bool force_done)
{
	...
	ppr_apply_max(txpwr, WLC_TXPWR_MAX);
	/* restore tx power */
	wlc_phy_txpower_set(ci->wlc->band->pi, ccx_rm->cur_txpower, ccx_rm->cur_txpoweroverride,
		txpwr);
	ppr_delete(ci->wlc->osh, txpwr);
