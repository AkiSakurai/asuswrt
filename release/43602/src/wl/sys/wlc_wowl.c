 ...
 *
 * $Id: wlc_wowl.c 467527 2014-04-03 13:48:29Z $
 */
...
static int wlc_wowl_upd_pattern_list(wowl_info_t *wowl, wl_wowl_pattern_t *wl_pattern,
	uint size, char *arg);
static int wlc_wowl_watchdog(void *arg);
...
static void wlc_wowl_key_update(
		    struct wowl_info *wowl,
		    wsec_key_t *scb_key,
		    struct scb *scb,
		    uint32 key_rot_index);
...
BCMATTACHFN(wlc_wowl_attach)(wlc_info_t *wlc)
{
	...
	if (!(wowl = (wowl_info_t *)MALLOC(wlc->osh, sizeof(wowl_info_t)))) {
	...
	bzero((char *)wowl, sizeof(wowl_info_t));
	wowl->wlc = wlc;
...
wlc_wowl_dump(wowl_info_t *wowl, struct bcmstrbuf *b)
{
	...
	if ((wakeind & WL_WOWL_MIC_FAIL) == WL_WOWL_MIC_FAIL)
		bcm_bprintf(b, "\t\tWake TKIP MIC Countermeasures\n");
                 ...
                 void *p, uint plen, void *arg, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_WOWL):
		wowl_bits = (WL_WOWL_MAGIC | WL_WOWL_NET | WL_WOWL_DIS | WL_WOWL_GTK_FAILURE |
			...
			WL_WOWL_KEYROT | WL_WOWL_FW_HALT | WL_WOWL_MIC_FAIL);
	...
	case IOV_SVAL(IOV_WOWL_OS):
		wowl_bits = (WL_WOWL_MAGIC | WL_WOWL_NET | WL_WOWL_DIS | WL_WOWL_GTK_FAILURE |
			...
			WL_WOWL_EAPID | WL_WOWL_ARPOFFLOAD | WL_WOWL_TCPKEEP_DATA |
			WL_WOWL_TCPKEEP_TIME | WL_WOWL_MDNS_CONFLICT | WL_WOWL_MDNS_SERVICE |
			WL_WOWL_KEYROT | WL_WOWL_FW_HALT | WL_WOWL_EXTMAGPAT | WL_WOWL_MIC_FAIL);
		if ((int_val & ~wowl_bits) != 0) {
			err = BCME_BADARG;
			break;
		}
	...
	uint size, char *arg)
{
	...
	if (!strncmp(arg, "clr", 3)) {
		 ...
		 */
		 for (prev = &wowl->pattern_list; *prev != NULL; prev = &(*prev)->next) {
#if defined(NDIS) && (NDISVER >= 0x0620)
			if (WIN7_AND_UP_OS(wlc->pub) && (wl_pattern->id != 0)) {
...
wlc_wowl_write_offload_ctxt(wowl_info_t *wowl, void *pkt, struct scb *scb, bool is_4gtk)
{
	...
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		...
		plcp  = (vhtHdr->RateInfo[0].plcp);  /* pick up the primate rate */
	}
	...
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		...
		ctxt.PhyTxControlWord = vhtHdr->RateInfo[0].PhyTxControlWord_0;
		ctxt.PhyTxControlWord_1 = vhtHdr->RateInfo[0].PhyTxControlWord_1;
		ctxt.mac_frmtype = SCB_QOS(scb)? htol16(FC_QOS_DATA): htol16(FC_DATA);
	...
	if (WSEC_ENABLED(bsscfg->wsec) && !BSSCFG_SAFEMODE(bsscfg)) {
		...
		if (scb->key) {
			...
			WL_ERROR(("wl%d.%d: %s: no key for encription\n",
				wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
		}
	}
	...
	if (key) {
		...
		if (D11REV_GE(wlc->pub->corerev, 42)) {
			uint16 tmp = 0;
			uint8 idx = 0;
			idx = WSEC_KEY_INDEX(wlc, key);
			tmp |= idx;
			tmp <<= 8;
			tmp = tmp | (key->algo_hw << D11AC_ENCRYPT_ALG_SHIFT);
		...
		ctxt.payload_wordoffset += (uint16)(key->iv_len);
	}
...
/* Program the shm after loading new ucode */
static
void wlc_wowl_secalgo_shm_program(struct wowl_info *wowl, wsec_key_t *scb_key, struct scb *scb)
{
	wlc_info_t *wlc = wowl->wlc;
	uint16 secsuite = 0;

	/* If pair-wise key does not exist, then this is WEP */
	if (!scb_key) {
		wsec_key_t *def_key = WSEC_BSS_DEFAULT_KEY(SCB_BSSCFG(scb));
		ASSERT(def_key);
		wlc_key_hw_wowl_init(wlc, def_key, WSEC_KEY_INDEX(wlc, def_key), SCB_BSSCFG(scb));
		secsuite = def_key->algo_hw;
	} else {
		int idx;

		for (idx = 0; idx < WLC_DEFAULT_KEYS; idx++) {
			/* Program default key if non-empty */
			if (wlc->wsec_def_keys[idx]->len) {
				WL_WOWL(("wl%d: Programming key 0x%x\n", wlc->pub->unit, idx));
				wlc_key_hw_wowl_init(wlc, wlc->wsec_def_keys[idx], idx,
				                     SCB_BSSCFG(scb));
				secsuite = wlc->wsec_def_keys[idx]->algo_hw;
			} else {
				WL_WOWL(("wl%d: Skipping 0-length key 0x%x\n",
				    wlc->pub->unit, idx));
			}
		}

		/* Program pairwise key in the last slot */
		wlc_key_hw_wowl_init(wlc, scb_key, WSEC_MAX_WOWL_KEYS - 1, SCB_BSSCFG(scb));

		secsuite |= scb_key->algo_hw << 8;
	}

	wlc_write_shm(wlc, M_WOWL_SECSUITE, secsuite);
	/* For decoding bcast keys, WOWL ucode requires this to be always set */
	wlc_mhf(wlc, MHF1, MHF1_DEFKEYVALID, MHF1_DEFKEYVALID, WLC_BAND_ALL);
}

/* Update SW state from the shm after waking up */
static
void wlc_wowl_key_update(struct wowl_info *wowl, wsec_key_t *scb_key, struct scb *scb,
                         uint32 key_rot_index)
{
	wlc_info_t *wlc = wowl->wlc;
	int idx;
	...
	/* Driver will refix it if needed */
	...
	for (idx = 0; idx < WLC_DEFAULT_KEYS; idx++) {
		 ...
		 */
		if (wlc->wsec_def_keys[idx]->len) {
			WL_WOWL(("wl%d: wlc_wowl_key_update: Updating key %d\n",
			          wlc->pub->unit, idx));
			wlc_key_sw_wowl_update(wlc, wlc->wsec_def_keys[idx], idx, SCB_BSSCFG(scb),
			                       (wowl->flags_current & WOWL_KEYROT) != 0, FALSE);
		} else if ((wowl->flags_current & WOWL_KEYROT) &&
		           ((key_rot_index & (1 << idx)) != 0)) {
			if (wlc_key_sw_wowl_create(wlc, idx, SCB_BSSCFG(scb)) == 0)
				wlc_key_sw_wowl_update(wlc, wlc->wsec_def_keys[idx], idx,
				                       SCB_BSSCFG(scb),
				                       TRUE, FALSE);
			else
				WL_WOWL(("wl%d: wlc_wowl_key_update: Creating key %d failed\n",
				          wlc->pub->unit, idx));
		}
	}

	/* Program pairwise key in the last slot */
	wlc_key_sw_wowl_update(wlc, scb_key, idx, SCB_BSSCFG(scb), FALSE, TRUE);
}
...
wlc_wowl_wsec_setup(wowl_info_t *wowl, struct scb *scb, uint32 *wowl_flags)
{
	...
	wlc_wowl_secalgo_shm_program(wowl, scb->key, scb);
...
wlc_wowl_ucode_init(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (scb && WSEC_SCB_KEY_VALID(scb) &&
		((D11REV_GE(wlc->pub->corerev, 40) && (scb->key->algo_hw == WSEC_ALGO_AES)) ||
		(D11REV_LT(wlc->pub->corerev, 40) &&
		(scb->key->algo_hw == D11_PRE40_WSEC_ALGO_AES)))) {
		if (D11REV_IS(wlc->pub->corerev, 12))
		...
		else if (D11REV_IS(wlc->pub->corerev, 37) && WLCISLCN40PHY(wlc->band))
			ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11aeswakeucode37_lcn40,
			d11aeswakeucode37_lcn40sz, d11wakelcn406initvals37,
			d11wakelcn406bsinitvals37);
		else if (D11REV_IS(wlc->pub->corerev, 37) && WLCISNPHY(wlc->band))
			ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11aeswakeucode31_mimo,
			d11aeswakeucode31_mimosz, d11waken22initvals31,
			d11waken22bsinitvals31);
		...
		if (D11REV_IS(wlc->pub->corerev, 12))
			...
			ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11ucode_wowl37_lcn40,
				d11ucode_wowl37_lcn40sz, d11wakelcn406initvals37,
				d11wakelcn406bsinitvals37);
		else if (D11REV_IS(wlc->pub->corerev, 37) && WLCISNPHY(wlc->band))
			ret = wlc_wowl_dnld_ucode_inits(wlc_hw, d11ucode_wowl31_mimo,
				d11ucode_wowl31_mimosz, d11waken22initvals31,
				d11waken22bsinitvals31);
		else if (D11REV_GE(wlc->pub->corerev, 42) && WLCISACPHY(wlc->band)) {
...
wlc_wowl_update_pm_mode(wowl_info_t *wowl)
{
	...
	if (wowl->pmstate_prev != pmstate_new) {
		WL_WOWL(("%s :Switching from %d to %d for wowl.\n",
			__FUNCTION__, wowl->pmstate_prev, pmstate_new));
	...
	if (wowl->wowl_test == 0) {
		...
		if (wlc_bss_connected(cfg)) {
			...
			if (!WSEC_SCB_KEY_VALID(scb) &&
				...
				WSEC_BSS_DEFAULT_KEY(SCB_BSSCFG(scb)) == NULL)))
			{
			...
			if (scb && (scb->WPA_auth & (WPA2_AUTH_PSK | WPA2_AUTH_UNSPECIFIED)))
...
#ifdef WLOFFLD
	if (WLOFFLD_CAP(wlc)) {
		wlc_update_seq_iv(wlc, FALSE, NULL);
...
enable:
	...
	bzero(wlc->core->macstat_snapshot, sizeof(macstat_t));
#endif
...
enable:
	 ...
	 */
	wlc_ol_wowl_enable_complete(wlc->ol);
...
wlc_wowl_reassoc(wowl_info_t *wowl)
{
	...
	*/
	if (!ASSOC_RECREATE_ENAB(wlc->pub) ||
		...
		WL_WOWL_SCANOL | WL_WOWL_LINKDOWN)))) {
			wlc_bsscfg_disable(wlc, cfg);
			wlc_link(wlc, FALSE, &cfg->BSSID, cfg,
				WLC_E_LINK_ASSOC_REC);
			/* clear link down bit, it is not part of wake reason */
			if (wowl->wakeind & WL_WOWL_LINKDOWN)
				wowl->wakeind &= ~WL_WOWL_LINKDOWN;
	} else {
		if (wowl->wakeind & WL_WOWL_MIC_FAIL) {
			/* Taken from wlc_wsec_miccheck() */
			wlc_bsscfg_t *bsscfg = cfg;
			struct pktq *q = &bsscfg->wlcif->qi->q;
			bool is_group = FALSE;
			wsec_key_t *key = NULL; /* Only for #ifdef EXT_STA */

			WL_ERROR(("%s: Woke for tkip countermeasures.\n",
			__FUNCTION__));
			...
			*/
			...
			/* signal 2 MIC failures */
			wlc_decrypt_error(wlc, &bsscfg->current_bss->BSSID,
				WLC_BSSCFG_IDX(bsscfg), WLC_E_MIC_ERROR, is_group, key,
...
wlc_wowl_clear(wowl_info_t *wowl)
{
	...
	if (WOWL_ACTIVE(wlc->pub)) {
		...
		si_setup_curmap(wlc->pub->osh, wlc->pub->sih);
		si_setup_backplanetimeout(wlc->pub->osh, wlc->pub->sih, 0x3f0);
#endif
	...
	if (!si_deviceremoved(wlc->pub->sih)) {
		...
		if (WLOFFLD_CAP(wlc)) {
			...
			wlc_update_seq_iv(wlc, TRUE, NULL);
		}
		else
#endif /* WLOFFLD */
			wowl->wakeind = wlc_wowl_clear_ucode(wowl);
...
wlc_wowl_clear_ucode(wowl_info_t *wowl)
{
	...
	if (wowl->wowl_test == 0) {
		...
		if ((scb = wlc_scbfind(wlc, wlc->cfg, &wlc->cfg->BSSID)) != NULL) {
			...
			/* Update s/w information that might have
			 * changed during sleep state
			 */
			if (WSEC_SCB_KEY_VALID(scb))
				wlc_wowl_key_update(wowl, scb->key, scb, key_rot_indx);
...
void wlc_wowl_disable_completed(wowl_info_t *wowl, void *wowl_host_info)
{
	...
	if (wlc_wowl_is_pkt_wake_ind(pwowl_host_info->wake_reason) &&
		pwowl_host_info->pkt_len < ARRAYSIZE(wowl->wowl_host_inf.pkt)) {
...
wlc_wowl_process_scan_wake_pkt(wowl_info_t *wowl)
{
	 ...
	 */
	if ((bi = MALLOC(wlc->osh, sizeof(wlc_bss_info_t))) == NULL) {
		WL_ERROR(("%s: failed to allocate wlc_bss_info_t\n", __FUNCTION__));
		return BCME_NOMEM;
	}
	bzero(bi, sizeof(wlc_bss_info_t));
...
wlc_alloc_wowl_pkt(wowl_info_t *wowl, uint32 len)
{
	...
	WLPKTTAG(p)->flags2 |= WLF3_DATA_WOWL_PKT;
...
wlc_wowl_setup_offloads(wowl_info_t *wowl, struct scb *scb)
{
	...
	if (WSEC_SCB_KEY_VALID(scb) || WSEC_WEP_ENABLED(SCB_BSSCFG(scb)->wsec)) {
		new_flags = 0;
		...
		struct ether_addr *host_mac) 	/* For ICMPV6 (NA) header options */
{
	...
	na_res = (struct nd_msg *)(((uint8*)na_ip_hdr) + sizeof(struct ipv6_hdr));
	na_res_opt = (struct nd_msg_opt *)((uint8 *)na_res + sizeof(struct nd_msg));
	...
	na_ip_hdr->payload_len = hton16(sizeof(struct nd_msg) + sizeof(struct nd_msg_opt));
	...
	wl_wowl_pattern_t **wowl_pattern, uint *len)
{
	...
	*/
	...
	uint32 pat_size = sizeof(struct ipv6_hdr) + sizeof(struct nd_msg);
	struct ipv6_hdr *ns_ip_hdr;
	struct nd_msg *ns_req;
	...
	*wowl_pattern = MALLOC(wlc->pub->osh, wowl_pattern_len);
	...
	bzero(*wowl_pattern, wowl_pattern_len);
...
static void*
wlc_wowl_prepeapol_gmsg2(wowl_info_t *wowl, wsec_key_t *key, uint8 *kck)
{
	wlc_info_t *wlc = wowl->wlc;
	...
	flags = (key->algo == CRYPTO_ALGO_AES_CCM) ? WPA_KEY_DESC_V2 : WPA_KEY_DESC_V1;
	...
	key_desc = (key->algo == CRYPTO_ALGO_AES_CCM) ? WPA_KEY_DESC_V2 : WPA_KEY_DESC_V1;
...
wlc_wowl_hw_init(wowl_info_t *wowl, struct scb *scb)
{
	...
	*((uint32 *)(uc_rc)) =
	...
	gtkp = wlc_wowl_prepeapol_gmsg2(wowl, scb->key, rekey->kck);
