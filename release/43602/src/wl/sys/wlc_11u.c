...
typedef struct {
	...
	uint8           *rx_qos_ie;     /* no longer used but avoids ROM invalidation */
...
BCMATTACHFN(wlc_11u_attach)(wlc_info_t *wlc)
{
	...
	/* bcn/prbrsp/assocreq/reassocreq/prbreq */
	...
	case IOV_GVAL(IOV_INTERWORKING): {
		...
		if (WL11U_ENAB(wlc)) {
			wlc_11u_bsscfg_cubby_t *cubby_11u =
				WLC_11U_BSSCFG_CUBBY(m11u, bsscfg);
...
wlc_11u_set_pkt_prio(wlc_11u_info_t *m11u, wlc_bsscfg_t *cfg, void *pkt)
{
	...
	/* set priority only if interworking enabled */
	if (cubby_11u->interworking) {
