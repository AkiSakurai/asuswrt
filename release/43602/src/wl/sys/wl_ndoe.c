 ...
 *
 * $Id: wl_ndoe.c 473031 2014-04-26 12:18:49Z $
 */
...
static int na_reply_peer(wl_nd_info_t *ndi, struct nd_msg *ns_req,
...
BCMATTACHFN(wl_nd_attach)(wlc_info_t *wlc)
{
	...
	ndi = MALLOC(wlc->osh, sizeof(wl_nd_info_t));
	if (!ndi) {
...
wl_ns_parse(wl_nd_info_t *ndi, void *sdu, bool sentby_host)
{
	...
	if (WLEXTSTA_ENAB(ndi->wlc->pub))
		ns_pktlen = (DOT11_MAC_HDR_LEN + SNAP_HDR_LEN + ETHER_TYPE_LEN +
		            sizeof(struct nd_msg)+ sizeof(struct ipv6_hdr));
	else
#endif /* EXT_STA */
		ns_pktlen = (ETHER_HDR_LEN + sizeof(struct nd_msg)+ sizeof(struct ipv6_hdr));
#if defined(EXT_STA)
	if (WLEXTSTA_ENAB(ndi->wlc->pub)) {
		if (ndi->req_count == 0) {
			return ret;
	...
	if ((ntoh16(eth->ether_type) != ETHER_TYPE_IPV6) ||
		...
		return ret;
			...
			RXOEINC(rxoe_totalndcnt);
#endif

	if (length >= (ns_pktlen + sizeof(struct nd_msg_opt))) {
		ns_req_opt = (struct nd_msg_opt *)(pt + sizeof(struct nd_msg));
	}
...
static int
na_reply_peer(wl_nd_info_t *ndi, struct nd_msg *ns_req, struct nd_msg_opt *options,
	struct ether_header *eth_hdr, struct ipv6_hdr *ip_hdr, bool snap, uint8 req_index)
{
	void *pkt;
	uint8 *frame;
	uint16 ns_pktlen = (ETHER_HDR_LEN + sizeof(struct nd_msg)+ sizeof(struct nd_msg_opt) +
...
wl_nd_alloc_ifndi(wl_nd_info_t *ndi_p, wlc_if_t *wlcif)
{
	...
	ndi = MALLOC(wlc->osh, sizeof(wl_nd_info_t));
	if (!ndi) {
		WL_ERROR(("wl%d: wl_nd_alloc_ifndi: MALLOC failed; total mallocs %d bytes\n",
		          wlc->pub->unit, MALLOCED(wlc->osh)));
		return NULL;
	}
...
void wl_nd_proc_msg(wlc_dngl_ol_info_t *wlc_dngl_ol, wl_nd_info_t *ndi, void *buf)
{
	...
	switch (msg_hdr->type) {
		...
		case BCM_OL_ND_ENABLE:
			...
			wlc_dngl_ol->txinfo.wsec = nd_enable->wsec;
			bcopy(&(nd_enable->defaultkeys),
				&(wlc_dngl_ol->txinfo.defaultkeys),
				DEFAULT_KEYS * sizeof(ol_sec_info));
			ndi->nd_enabled = TRUE;
			break;
...
wl_nd_ra_filter_init(wl_nd_info_t *ndi)
{
	...
	/* TBD/TODOJJ whether explicit zeroing is required */
