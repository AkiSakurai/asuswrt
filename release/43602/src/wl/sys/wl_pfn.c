 ...
 *
 * $Id: wl_pfn.c 466939 2014-04-02 00:31:21Z $
 *
 */
...
static void wl_pfn_gps_cb(wl_pfn_info_t * pfn_info, char *buf, int len);
#endif /* defined(WL_SHIF) */
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_PFN_MACADDR):
		...
		break;
#if defined(WL_SHIF)
	case IOV_SVAL(IOV_PFN_GPS):
		if (SHIF_ENAB(pfn_info->wlc->pub)) {
			int gps;

			bcopy(arg, &gps, sizeof(gps));
			if (wl_pfn_gps_scan(pfn_info, gps) != BCME_OK)
				err = BCME_ERROR;
			else
				err = BCME_OK;
		} else {
...
#if defined(WL_SHIF)
	if (SHIF_ENAB(wlc->pub) && (pfn_info->gpspfn == TRUE))
...
#if defined(WL_SHIF)
		(SHIF_ENAB(pfn_info->wlc->pub) && pfn_info->scan_retry > PFN_GPS_SCAN_RETRY) &&
#endif /* defined(WL_SHIF) */
		1) {
...
#if defined(WL_SHIF)
	if (SHIF_ENAB(pfn_info->wlc->pub) && (status != WLC_E_STATUS_SUCCESS)) {
		wl_del_timer(pfn_info->wlc->wl, pfn_info->p_pfn_timer);
		wl_add_timer(pfn_info->wlc->wl, pfn_info->p_pfn_timer,
			PFN_GPS_SCAN_RETRY_INTERVAL, TRUE);
		pfn_info->pfn_scan_state = PFN_SCAN_IDLE;
		pfn_info->scan_retry++;
		goto reschedule;
	}
...
wl_pfn_start_scan(wl_pfn_info_t * pfn_info, wlc_ssid_t	* ssid, int nssid, int bss_type)
{
	...
	if ((rc == BCME_OK) && (pfn_info->intflag & (ENABLE << PFN_MACADDR_BIT)))
...
wl_pfn_macaddr_set(wl_pfn_info_t * pfn_info, void * buf, int len)
{
	...
	if (macp->version != WL_PFN_MACADDR_CFG_VER)
		return BCME_VERSION;

	if (!(isnull = ETHER_ISNULLADDR(&macp->macaddr))) {
		if (ETHER_ISMULTI(&macp->macaddr) || !ETHER_IS_LOCALADDR(&macp->macaddr))
			return BCME_BADARG;
	}
...
wl_pfn_macaddr_get(wl_pfn_info_t * pfn_info, void * buf, int len, void * inp, uint inp_len)
{
	...
	macp = buf;
	macp->version = WL_PFN_MACADDR_CFG_VER;
	macp->reserved = 0;
...
#if defined(WL_SHIF)
static void
wl_pfn_gps_cb(wl_pfn_info_t * pfn_info, char *buf, int len)
{
	if ((len == 3) && !strcmp(buf, "0:\n")) {
...
#else
	bcm_nfcif_send_to_nfc(gci_buf, len, NON_SLIP_PKT);
	MFREE(wlc->osh, gci_buf, len);
#endif /* HNDGCI */
}
#endif /* defined(WL_SHIF) */
