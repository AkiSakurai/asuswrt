 ...
 *
 * $Id: wlc_tdls.c,v 1.1.2.10 2011-02-08 19:19:26 $
 */
...
#include <wlc_ht.h>	/* for WLC_HT_FEATURES_PROPRATES_FORCE */
...
#define TDLS_SWITCH_TIMEOUT_DEFAULT	25000	/* us */
...
static uint8 *wlc_tdls_write_rsn_ie(tdls_info_t *tdls, uint8 *cp, int buflen, uint16 WPA_auth,
...
static int wlc_tdls_watchdog(void *hdl);
...
BCMATTACHFN(wlc_tdls_attach)(wlc_info_t *wlc)
{
	...
	if (!(tdls = (tdls_info_t *)MALLOC(wlc->osh, sizeof(tdls_info_t)))) {
		WL_ERROR(("wl%d: wlc_tdls_attach: out of mem, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
		return NULL;
	}

	bzero((char *)tdls, sizeof(tdls_info_t));

	if (!(tdls->lookaside = (tdls_lookaside_t *)MALLOC(wlc->osh,
	...
	bzero((char *)tdls->lookaside, sizeof(tdls_lookaside_t) * TDLS_LOOKASIDE_SIZE);

#ifdef WLCNT
	if (!(tdls->cnt = (wlc_tdls_cnt_t *)MALLOC(wlc->osh, sizeof(wlc_tdls_cnt_t)))) {
		WL_ERROR(("wl%d: wlc_tdls_attach: out of mem, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
		goto fail;
	}
	bzero((char *)tdls->cnt, sizeof(wlc_tdls_cnt_t));
#endif /* WLCNT */
	...
	switch (info->mode) {
		...
		case TDLS_MANUAL_EP_CHSW: {
			...
			if (info->chanspec) {
				...
				if (info->chanspec == scb_tdls->base_chanspec)
				...
				if (scb_tdls->target_chanspec != scb_tdls->base_chanspec) {
					chanspec_t target_ch_s = scb_tdls->target_chanspec;
					if (!tdls->manual_chsw) {
						scb_tdls->target_chanspec =
					...
					WL_TDLS(("wl%d:%s(): Channel switch disabled, "
						...
						tdls->pub->unit, __FUNCTION__,
						scb_tdls->target_chanspec, scb_tdls->flags));
...
wlc_tdls_peer_timer(void *arg)
{
	...
	timer_arg = (tdls_timer_arg_t *)arg;
	tdls = (tdls_info_t *)timer_arg->hdl;
	wlc = tdls->wlc;
...
wlc_tdls_set(tdls_info_t *tdls, bool on)
{
	...
	if (on) {
		if (DPT_ENAB(tdls->pub)) {
			WL_ERROR(("wl%d: DPT is enalbed, turn off DPT first.\n",
				tdls->pub->unit));
...
static uint8 *
wlc_tdls_write_rsn_ie(tdls_info_t *tdls, uint8 *cp, int buflen, uint16 WPA_auth,
	uint32 wsec, rsn_parms_t *rsn, bool pmkid_cnt)
{
...
wlc_tdls_link_add_ht_op_ie(tdls_info_t *tdls, scb_t	*scb)
{
	...
	if (BSS_N_ENAB(wlc, cfg) && SCB_HT_CAP(scb)) {
		if (TDLS_WB_CAP(wlc->tdls) && TDLS_SCB_WB_CAP(SCB_TDLS(tdls, scb)) &&
			...
			if (cspec) {
				cfg->current_bss->chanspec = cspec;
				return TRUE;
			}
		}
...
wlc_tdls_link_add_vht_op_ie(tdls_info_t *tdls, scb_t	*scb)
{
	...
	opmode = wlc_vht_get_scb_opermode(wlc->vhti, scb);
	...
	if (BSS_VHT_ENAB(wlc, cfg) && SCB_VHT_CAP(scb)) {
		if (TDLS_WB_CAP(wlc->tdls) &&
			...
			TDLS_SCB_OP_MODE_NOTIF(SCB_TDLS(tdls, scb)) &&
			DOT11_OPER_MODE_CHANNEL_WIDTH_80MHZ(opmode) &&
...
wlc_tdls_action_frame_tx_complete(wlc_info_t *wlc, uint32 txstatus, void *arg)
{
	...
	if (scb_tdls->flags & TDLS_SCB_SENT_TEARDOWN) {
		...
		for (qi = wlc->tx_queues; qi != NULL; qi = qi->next)
			wlc_pktq_scb_free(wlc, &qi->q, scb);
		scb_tdls->free_me = FALSE;
		wlc_tdls_free_scb(wlc->tdls, scb);
		return;
	}
	...
	if (scb_tdls->flags == TDLS_SCB_SENT_SETUP_CFM) {
		...
		chanspec_t cur_chanspec;
#ifdef WLMCHAN
		cur_chanspec = cfg->chan_context->chanspec;
#else	/* WLMCHAN */
		...
		if (CHSPEC_BW(cfg->current_bss->chanspec) > CHSPEC_BW(cur_chanspec)) {
			wlc_update_bandwidth(wlc, scb->bsscfg, cfg->current_bss->chanspec);
#ifdef WLMCHAN
			scb_tdls->base_chanspec = scb_tdls->cur_chanspec =
				cfg->chanspec;
			scb_tdls->target_chanspec = cfg->chanspec;
#endif /* WLMCHAN */
		}
...
wlc_tdls_bsscfg_create(wlc_info_t *wlc, wlc_bsscfg_t *parent, bool initiator)
{
	...
	if (wlc_bsscfg_tdls_init(wlc, bsscfg, initiator) != BCME_OK) {
...
wlc_tdls_return_to_base_ch(tdls_info_t *tdls, struct scb *scb)
{
	...
	if (scb_tdls->cur_chanspec == scb_tdls->base_chanspec) {
		WL_TDLS(("wl%d:%s(): already at base channel %d\n",
		tdls->pub->unit, __FUNCTION__, CHSPEC_CHANNEL(scb_tdls->base_chanspec)));
		return BCME_OK;
	}
	...
	int len)
{
	...
	if (!wlc_rate_hwrs_filter_sort_validate(&target_bss->rateset, &wlc->band->hw_rateset,
	...
	if (bss_ht) {
		...
		if (bss_vht) {
			...
			if (BAND_2G(wlc->band->bandtype) || WLC_VHT_FEATURES_RATES(wlc->pub)) {
				...
				parse, len, &vht_ratemask, &brcm_prop_tlv_len);
			}
	...
	if (MCHAN_ENAB(tdls->pub)) {
		...
		if (!(SCB_BSSCFG(scb)->chan_context))
			WL_ERROR(("wl%d: wlc_tdls_join:failed to create chan_context!\n",
				tdls->pub->unit));
		scb_tdls->base_chanspec = scb_tdls->cur_chanspec = scb_tdls->target_chanspec =
		cur_bss->chanspec;
	}
#endif /* WLMCHAN */
	...
	struct ether_addr *ea, struct ether_addr *bssid, bool initiator)
{
	...
	scb_tdls = (scb_tdls_t *)MALLOC(tdls->osh, sizeof(scb_tdls_t));
	if (!scb_tdls) {
		WL_ERROR(("wl%d: cannot malloc scb_tdls\n", tdls->pub->unit));
		goto fail;
	}

	bzero((char *)scb_tdls, sizeof(scb_tdls_t));
...
#ifdef DONGLEBUILD
		if (wlc_sendnulldata(wlc, cfg, &scb->ea, 0, 0, PRIO_8021D_BE) == NULL) {
#else
		if (!wlc_sendnulldata(wlc, cfg, &scb->ea, 0, 0, PRIO_8021D_BE))
		{
#endif /* DONGLEBUILD */
	...
	struct ether_addr *da, void *p, bool direct)
{
	...
	uchar *payload;
	struct scb* scb = wlc_tdls_scbfindband_all(wlc, da,
		CHSPEC_WLCBANDUNIT(parent->current_bss->chanspec));
	...
	*payload = TDLS_PAYLOAD_TYPE;
	...
	WLPKTTAG(p)->flags |= (WLF_DPT_TYPE | (direct ? 0 : WLF_TDLS_APPATH));
...
wlc_tdls_qosinfo_update(tdls_info_t *tdls, struct scb *scb, wlc_bss_info_t *bi)
{
	...
	if (BSS_WME_ENAB(tdls->wlc, scb_tdls->parent)) {
		...
		if (bi->flags & WLC_BSS_WME) {
			...
			if (bi->wme_qosinfo) {
				wlc_qosinfo_update(scb, bi->wme_qosinfo);
	...
	uint32 cipher, struct ether_addr *ea)
{
	...
	if (!(key = MALLOC(tdls->osh, sizeof(wl_wsec_key_t)))) {
...
#ifdef WL_BEAMFORMING
	else if (TXBF_ENAB(wlc->pub))
		wlc_txbf_update_amt_secindxblk(wlc->txbf, bsscfg, key->index);
#endif /* WL_BEAMFORMING */
...
wlc_tdls_process_setup_req(tdls_info_t *tdls, struct scb *scb, struct wlc_frminfo *f, uint offset)
{
	...
	if ((bi = MALLOC(tdls->osh, sizeof(wlc_bss_info_t))) == NULL)
		return BCME_ERROR;
...
#ifdef WLMCHAN
		scb_peer->bsscfg->chanspec = bi->chanspec;
		scb_tdls->base_chanspec = scb_tdls->cur_chanspec =
			scb_peer->bsscfg->chanspec;
		scb_tdls->target_chanspec = scb_peer->bsscfg->chanspec;
#endif	/* WLMCHAN */
	}
...
exit:
	...
	scb_tdls->flags |= TDLS_SCB_RCVD_TEARDOWN;
	for (qi = wlc->tx_queues; qi != NULL; qi = qi->next)
		wlc_pktq_scb_free(wlc, &qi->q, scb_peer);
	wlc_tdls_disconnect(tdls, scb_peer, TRUE);
...
wlc_tdls_process_pti_resp(tdls_info_t *tdls, struct scb *scb, struct wlc_frminfo *f, uint offset)
{
	...
	WL_TDLS(("wl%d.%d:%s(): send null-data to peer buffer STA.\n",
		tdls->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
	if (!wlc_sendnulldata(wlc, bsscfg, f->sa, 0, 0, PRIO_8021D_BE)) {
		WL_ERROR(("wl%d.%d: failed to send PM null frame, "
			"fake a PM0->PM1 transition\n",
			tdls->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
...
wlc_tdls_tx_null_data_complete(wlc_info_t *wlc, uint32 txstatus, void *arg)
{
	...
	if (scb_tdls->flags & TDLS_SCB_WAIT_NULLDATA_ACK) {
		...
		if ((txstatus & TX_STATUS_MASK) == TX_STATUS_NO_ACK) {
			...
			wlc_tdls_listen_on_target_ch(wlc->tdls, scb_tdls);
		}
		else {
			WL_TDLS(("wl%d.%d:%s(): flags = 0x%08x, rcv ACK from %s, successfully "
				"switch to target channel %d\n",
...
wlc_tdls_rcv_data_frame(tdls_info_t *tdls, struct scb *scb, d11rxhdr_t *rxhdr)
{
	...
	if (CHSPEC_CHANNEL(scb_tdls->cur_chanspec) != WLC_RX_CHANNEL(rxhdr)) {
		WL_ERROR(("wl%d:%s():rcv pkt on channel %d, not in the current channel %d\n",
		         tdls->pub->unit, __FUNCTION__, WLC_RX_CHANNEL(rxhdr),
		         CHSPEC_CHANNEL(scb_tdls->cur_chanspec)));
		return;
	}
	...
	if (wlc_tdls_rcv_on_target_ch(tdls, rxhdr, scb_tdls->target_chanspec)) {
		...
		WL_TDLS(("wl%d:%s(): rcv data frame from %s, successfully switch to "
			...
			CHSPEC_CHANNEL(scb_tdls->target_chanspec)));
		scb_tdls->flags &= ~(TDLS_SCB_LISTEN_ON_TARGET_CH | TDLS_SCB_WAIT_TX_ON_TARGET_CH);
		wl_del_timer(tdls->wlc->wl, scb_tdls->peer_addr->timer);
		scb_tdls->timer_start = FALSE;
		scb_tdls->flags &= ~(TDLS_SCB_RECV_CHSW_REQ | TDLS_SCB_WAIT_SWITCHTIME);
		...
		WL_TDLS(("wl%d:%s(): rcv data frame from %s not on target channel %d.\n",
			tdls->pub->unit, __FUNCTION__, bcm_ether_ntoa(&scb->ea, eabuf),
			CHSPEC_CHANNEL(scb_tdls->target_chanspec)));
	}
...
uint16
wlc_tdls_get_pretbtt_time(tdls_info_t *tdls, wlc_bsscfg_t *cfg)
{
	struct scb *scb;
	scb_tdls_t *scb_tdls;
	wlc_info_t *wlc = cfg->wlc;
	...
	if (!wlc || !cfg || !BSS_TDLS_ENAB(wlc, cfg)) {
		return 0;
	}
	...
	/* Use target_chanspec!=base_chanspec as active TDLS channel switching condition for now. */
	/* Need to add with TDLS channel switch start/stop logic real use case. */
	if (scb_tdls->target_chanspec != scb_tdls->base_chanspec) {
		extratime = tdls->switch_time;
	}
	...
	chanspec_t new_chanspec)
{
	...
	WL_TDLS(("%s():wl%d.%d: BEFORE: cfg->wlcif->qi = %p, old chan_context->q = %p,"
		"wlc->active_q = %p\n", __FUNCTION__, wlc->pub->unit, WLC_BSSCFG_IDX(cfg),
		cfg->wlcif->qi, cfg->chan_context->qi, wlc->active_queue));
	...
	WL_TDLS(("%s():wl%d.%d: AFTER: cfg->wlcif->qi = %p, new chan_context->q = %p,"
		"wlc->active_q = %p\n", __FUNCTION__, wlc->pub->unit, WLC_BSSCFG_IDX(cfg),
		cfg->wlcif->qi, cfg->chan_context->qi, wlc->active_queue));
...
wlc_tdls_chsw_complete(tdls_info_t *tdls, struct scb *scb)
{
	...
	if (!(scb_tdls->flags & TDLS_SCB_RECV_CHSW_REQ)) {
		...
		if (delta < (uint)scb_tdls->switch_time/1000) {
			...
			wl_add_timer(tdls->wlc->wl, scb_tdls->peer_addr->timer, 0 /* delta */, 0);
			scb_tdls->timer_start = TRUE;
		}
		else
			wlc_tdls_chsw_send_nulldata(tdls, scb, now);
	...
	if (CHSPEC_IS40(target_chanspec)) {
		...
		*/
		if ((!(tc->ht_capinfo & HT_CAP_40MHZ) ||
			!(scb->ht_capabilities & HT_CAP_40MHZ)) &&
...
static int wlc_tdls_chsw_validate_pmstate(tdls_info_t *tdls, struct scb *scb, wlc_bsscfg_t *parent)
{
	if ((!parent->pm->PMenabled || parent->pm->PMpending)) {
		parent->tdls->tdls_PMEnable = TRUE;
		wlc_tdls_pm_allowed_update(tdls, parent);
...
wlc_tdls_chsw_send_nulldata(tdls_info_t *tdls, struct scb *scb, uint32 now)
{
	...
	if (delta < (uint)scb_tdls->switch_timeout/1000) {
		...
		scb_tdls->flags |= TDLS_SCB_WAIT_NULLDATA_ACK;
#ifdef DONGLEBUILD
		p = wlc_sendnulldata(wlc, scb->bsscfg, &scb->ea, 0, 0, PRIO_8021D_BE);
		if (p) {
			wlc_pcb_fn_register(wlc->pcb, wlc_tdls_tx_null_data_complete,
				(void *)scb_tdls->peer_addr, p);
		}
#else
		wlc_sendnulldata_ex(wlc, scb->bsscfg, &scb->ea, 0, 0, PRIO_8021D_BE,
...
#ifdef DONGLEBUILD
				p = wlc_sendnulldata(wlc, scb->bsscfg, &scb->ea,
				                     0, 0, PRIO_8021D_BE);
				if (p) {
#else
				if (wlc_sendnulldata_ex(wlc, scb->bsscfg, &scb->ea,
					0, 0, PRIO_8021D_BE, wlc_tdls_sendnulldata_cb,
					(void *)scb_tdls))
#endif /* DONGLEBUILD */
					scb_tdls->flags |= TDLS_SCB_WAIT_HEARTBEAT_ACK;
#ifdef DONGLEBUILD
					wlc_pcb_fn_register(wlc->pcb,
						wlc_tdls_tx_null_data_complete,
						(void *)scb_tdls->peer_addr, p);
				}
#endif /* DONGLEBUILD */
			}
...
#endif /* BCMDBG || BCMDBG_DUMP || TDLS_TESTBED */
