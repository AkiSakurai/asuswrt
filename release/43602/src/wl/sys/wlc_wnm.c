 ...
 *
 * $Id: wlc_wnm.c 464576 2014-03-25 10:05:44Z $
 */
	...
	uint8 *reply_to_bss, bool istx)
{
	...
	uint16 ip6_icmp6_len = sizeof(struct ipv6_hdr) + sizeof(struct nd_msg);
#ifdef BCMDBG
	char ipbuf[64], eabuf[32];
#endif /* BCMDBG */
...
wlc_wnm_bss_pref_score_rssi(wlc_info_t *wlc, wlc_bss_info_t *bi, int8 bcn_rssi, uint32 *score)
{
	...
	*score = rate_500k[i] * nss * bw * bi->qbss_load_chan_free;
	WL_WNM_BSSTRANS_LOG("bssid[3:0]:%x rate_idx %d bcn_rssi %d\n",
		*(uint32 *)&bi->BSSID, i, bcn_rssi, 0);
	WL_WNM_BSSTRANS_LOG("bw (multiple of 20M) %d nss %d chan_free %d score %d\n",
...
wlc_wnm_process_join_trgts_bsstrans(wlc_info_t *wlc, wlc_bss_info_t **bip, int trgt_count)
{
	...
	for (i = 0; i < trgt_count; i++) {
		...
		if (extcap == NULL || extcap->len < DOT11_EXTCAP_LEN_BSSTRANS ||
			...
			WL_WNM_BSSTRANS_LOG("bss[3:0] %x doesn't support bsstrans\n",
				*(uint32 *)&bip[i]->BSSID, 0, 0, 0);
			use_score = FALSE;
			break;
		}
		...
		if (qbss_load_ie == NULL ||
			...
			WL_WNM_BSSTRANS_LOG("bss[3:0] %x doesn't support load ie\n",
				*(uint32 *)&bip[i]->BSSID, 0, 0, 0);
			use_score = FALSE;
			break;
		}
