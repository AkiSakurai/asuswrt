...
*
* $Id: wlc_homanager.c 463105 2014-03-19 21:34:36Z $
*
*/
#include <wlc_cfg.h>
#include <typedefs.h>
...
static void tbow_send_goinfo(tbow_info_t *tbow_info)
{
	...
	tbow_info->goinfo.opmode = TBOW_HO_MODE_GO;
	...
	if (tbow_info->goinfo.opmode == TBOW_HO_MODE_GO)
		setupmsg->opmode = TBOW_HO_MODE_GC;
	else if (tbow_info->goinfo.opmode == TBOW_HO_MODE_STA ||
		tbow_info->goinfo.opmode == TBOW_HO_MODE_GC)
		setupmsg->opmode = TBOW_HO_MODE_GO;
...
static int tbow_recv_goinfo(tbow_info_t *tbow_info, uchar *msg, int len)
{
	...
	if (tbow_info->goinfo.opmode == TBOW_HO_MODE_STA)
		memcpy(tbow_info->goinfo.macaddr,
			&tbow_info->wlc->bsscfg[0]->cur_etheraddr, ETHER_ADDR_LEN);
	else if (tbow_info->goinfo.opmode == TBOW_HO_MODE_GC) {
	...
	{	/* for debugging */
		...
		WL_TRACE(("\n"));
		if (tbow_info->goinfo.opmode == TBOW_HO_MODE_GC) {
...
static int tbow_start_ho(tbow_info_t *tbow_info)
{
	wlc_bsscfg_t *bsscfg;
