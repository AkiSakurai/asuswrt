 ...
 *
 * $Id: wlc_11h.c 467863 2014-04-04 12:32:11Z $
 */
...
wlc_11h_write_pwr_cap_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if ((srommax = ppr_create(wlc->osh, WL_TX_BW_20)) != NULL) {
		wlc_phy_txpower_sromlimit(wlc->band->pi, bi->chanspec, &min_pwr, srommax, 0);
	 ...
	 * SROM min limit.
	 */
	wlc_phy_get_txpwr_min(wlc->band->pi, &min_pwr);
	 ...
	 */
	txpwr_backoff = wlc_phy_get_txpwr_backoff(wlc->band->pi);
