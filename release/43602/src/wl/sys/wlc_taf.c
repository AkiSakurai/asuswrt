...
#if defined(WLTAF) && defined(PROP_TXSTATUS)
#error PROP_TXSTATUS and WLTAF not yet compatible
#endif
...
#define TAF_TIME_ADAPT_DEFAULT	10
...
static void wlc_taf_times_sync(wlc_taf_info_t *taf_info)
{
	...
	for (i = 0; i < ARRAYSIZE(taf_info->tid_state); ++i) {
		wlc_taf_tid_times_sync(taf_info, &taf_info->tid_state[i]);
	}

	wlc_taf_tid_times_sync(taf_info, &taf_info->unified_tid_state);
...
static int wlc_taf_dump_list(taf_method_info_t* method, struct bcmstrbuf* b)
{
	...
	if (iter) {
		...
		bcm_bprintf(b, "No assigned %s entries\n", TAF_SCHED_NAME(method->type));
	}
	...
	taf_release_context_t *context)
{
	...
	/* Update times. */
	if (fallback) {
		wlc_taf_tid_times_sync(taf_info, tid_state);
	...
	if (taf_info->enabled && !SCB_INTERNAL(scb)) {
		...
		if (scb_taf == NULL) {
			if (taf_info->scb_cnt <= TAF_SCB_CNT_MAX) {
				...
				scb_taf = MALLOCZ(taf_info->wlc->osh, sizeof(taf_scb_cubby_t));
				if (scb_taf == NULL)
					return BCME_NOMEM;
				taf_info->scb_cnt++;
                      ...
                      taf_release_context_t *context)
{
	...
	for (list = method->list; list; list = list->next) {
		...
		if (!(scb->traffic_active & (1 << context->tid))) {
