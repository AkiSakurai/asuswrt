 ...
 *
 * $Id: wlc_d0_filter.c,v 1.15.6.1 2010-09-28 00:34:59 $
 */
...
BCMATTACHFN(wlc_d0_filter_attach)(wlc_info_t *wlc)
{
	...
	info = MALLOC(wlc->osh, sizeof(wlc_d0_filter_info_t));
	if (info == NULL) {
	...
	bzero(info, sizeof(wlc_d0_filter_info_t));
	info->wlc  = wlc;
...
#if defined(EXT_STA)
	if (WLEXTSTA_ENAB(info->wlc->pub) && !PKT_DOT3(info->wlc->pub.osh, sdu)) {
