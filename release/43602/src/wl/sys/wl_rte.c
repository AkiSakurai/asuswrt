 ...
 *
 * $Id: wl_rte.c 496805 2014-08-14 11:44:17Z $
 */
...
#ifdef BCMPCIEDEV
static uint16 wl_flowring_update(hndrte_dev_t *dev, uint16 flowid, uint8 op, uint8 * sa,
	uint8 *da, uint8 tid);
#endif /* BCMPCIEDEV */
...
wl_alloc_if(wl_info_t *wl, int iftype, uint subunit, struct wlc_if *wlcif)
{
	...
	if (iftype == WL_IFTYPE_WDS) {
		snprintf(dev->name, HNDRTE_DEV_NAME_MAX, "wds%d.%d", wl->pub->unit, subunit);
	} else {
...
wl_sw_encrypt_mic(wl_info_t *wl, struct wlc_if *wlcif, wlc_bsscfg_t *bsscfg, struct lbuf *lb)
{
	...
	if (scb) {
		...
		if (key == NULL || key->algo == CRYPTO_ALGO_TKIP ||
			(WSEC_SOFTKEY(wlc, key, bsscfg) || WLPKTFLAG_PMF(WLPKTTAG(lb))))
			tkip_enab = TRUE;
	}
	else
		tkip_enab = TRUE;
...
wl_send(hndrte_dev_t *src, hndrte_dev_t *dev, struct lbuf *lb)
{
	...
	if (BCMPCIEDEV_ENAB(wl->pub) && WSEC_ENABLED(bsscfg->wsec) &&
		PKTISTXFRAG(wl->pub->osh, lb) && (PKTFRAGTOTLEN(wl->pub->osh, lb) > 0))
		pktfetch = TRUE;
	...
	while (lb != NULL) {
		 ...
		 */
		if (pktfetch && wl_sw_encrypt_mic(wl, wlcif, bsscfg, lb)) {
			wl_tx_pktfetch(wl, lb, src, dev);
			goto nextlb;
		}
...
void wlfc_push_pkt_txstatus(struct wl_info *wl, void* p, void *txs, uint32 sz)
{
	...
	if (PKTISTXFRAG(wl->pub->osh, p) && PKTFRAGMETADATALEN(wl->pub->osh, p)) {
...
void
wl_chain_rxcompletions_amsdu(osl_t *osh, void *p)
{
#ifdef BCMPCIEDEV_ENABLED
	void *p1;
	uint16 head,  current;
