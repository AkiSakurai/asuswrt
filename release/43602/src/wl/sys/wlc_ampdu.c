 ...
 *
 * $Id: wlc_ampdu.c 527132 2015-01-16 03:42:44Z $
 */
...
#ifdef WLAMSDU_TX
		     (AMSDU_TX_ENAB(wlc->pub) && amsdu_in_ampdu &&
#ifdef WLOVERTHRUSTER
	    ...
	    force || AMPDU_HW_ENAB(ampdu_tx->wlc->pub))
	{
		...
		if (taf) {
			...
			if (new_qlen == 0) {
				...
				scb_ampdu->scb->traffic_active &= ~(1 << (ini->tid));
	...
	scb_ampdu_tid_ini_t *ini, uint16 release, uint16 q_avail, taf_scheduler_public_t* taf)
{
	...
	if (taf) {
		...
		uint32 pkt_tag;
		uint16 pktbytes = (uint16)PKTLEN(osh, p);
		...
		if ((taf->released_units + taf->total_released_units) >=
			...
			/* Try to roundup released packets number */
			uint8 max_release = scb_ampdu->release;
			if (chunk_pkts >= 2 * max_release) {
				if ((chunk_pkts % max_release) == 0) {
					release = 0;
				}
			} else if (chunk_pkts >= max_release) {
				if (2 * chunk_pkts == 3 * max_release) {
					release = 0;
				}
			} else {
				int i;
	...
	if (txs->status.was_acked) {
		...
		if (AIBSS_ENAB(ampdu_tx->wlc->pub) && ini->scb && !SCB_ISMULTI(ini->scb)) {
			...
			if (cfg && BSSCFG_IBSS(cfg)) {
				...
				aibss_scb_info->tx_noack_count = 0;
			}
		}
#endif
	} else {
...
ampdu_cleanup_tid_ini(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid, bool force)
{
	...
	if (ini->bar_ackpending) { /* Cancel all pkt callbacks referencing ini */
	...
	bool override)
{
	...
	if (!scb_ampdu->ini[tid]) {
		ini = MALLOC(wlc->osh, sizeof(scb_ampdu_tid_ini_t));
