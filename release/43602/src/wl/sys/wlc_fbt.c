...
wlc_fbt_bsscfg_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (fbt_bss_priv->use_sup_wpa == FALSE) {
		...
		if (!fbt_bss_priv->wpa) {
			fbt_bss_priv->wpa = MALLOC(fbt_priv->osh, sizeof(wpapsk_t));
		...
		bzero(fbt_bss_priv->wpa, sizeof(wpapsk_t));

		/* Check if wpa-info already exists */
		if (!fbt_bss_priv->wpa_info) {
			fbt_bss_priv->wpa_info = MALLOC(fbt_priv->osh, sizeof(wpapsk_info_t));
		...
		bzero(fbt_bss_priv->wpa_info, sizeof(wpapsk_info_t));
	}
	return;
err:
...
void wlc_fbt_sup_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if ((fbt_bss != NULL) && (fbt_bss_priv->use_sup_wpa == TRUE)) {
