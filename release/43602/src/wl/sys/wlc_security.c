 ...
 *
 * $Id: wlc_security.c 492293 2014-07-21 09:18:58Z $
 */
...
static void
wlc_wsec_update_tw(wlc_info_t *wlc, struct wlc_frminfo *f)
{
...
static bool wlc_wsec_inrange(wlc_info_t *wlc, struct scb *scb,
	wsec_key_t *key, uint ividx, uint32 iv32, uint16 iv16)
{
	wlc_bsscfg_t *bsscfg;
	uint16 iv16_upper = key->rxiv[ividx].lo + WSEC_RXIV_MAXRANGE;
	bool inrange = TRUE;
	...
	if (key->rxiv[ividx].lo > iv16_upper) {
		/* wrapped case */
		if (iv16 >= key->rxiv[ividx].lo &&
		    iv32 != key->rxiv[ividx].hi)
			inrange = FALSE;
		else if (iv16 < key->rxiv[ividx].lo &&
			iv32 != key->rxiv[ividx].hi &&
			(iv16 > iv16_upper ||
			iv32 != (key->rxiv[ividx].hi + 1)))
			inrange = FALSE;
	} else {
		if (iv16 > iv16_upper ||
		    iv32 != key->rxiv[ividx].hi)
	...
	if (!inrange) {
		WL_ERROR(("wl%d.%d: %s (%s) iv16 0x%04x iv32 0x%08x lo 0x%04x hi 0x%08x\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
			(key->algo == CRYPTO_ALGO_TKIP)?"TKIP":"AES",
			iv16, iv32, key->rxiv[ividx].lo, key->rxiv[ividx].hi));
	}
	...
	struct wlc_frminfo *f, uint8 prio)
{
	...
	case CRYPTO_ALGO_TKIP:
		...
		if ((f->WPA_auth & WPA_AUTH_NONE) == 0 &&
		    !wlc_wsec_inrange(wlc, scb, f->key, ividx, iv32, iv16)) {
			return FALSE;
		}
#endif /* defined(BCM47XX_CA9) && defined(WL_PL310_WAR) */
		if ((f->WPA_auth & WPA_AUTH_NONE) == 0 &&
	...
	if (((f->key->algo == CRYPTO_ALGO_TKIP) || (f->key->algo == CRYPTO_ALGO_AES_CCM)) &&
	    ((f->WPA_auth & WPA_AUTH_NONE) == 0))
		wlc_wsec_update_tw(wlc, f);
#endif
	return TRUE;
}
