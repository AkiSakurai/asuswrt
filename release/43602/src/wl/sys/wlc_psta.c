 ...
 *
 * $Id: wlc_psta.c 485764 2014-06-17 10:51:53Z $
 */
...
wlc_psta_prim_associated(wlc_psta_info_t *psta)
{
	...
	return (wlc_psta_is_datapath_available(psta) &&		/* The datapath is available, */
		(primary_bss->roam->time_since_bcn < 2));	/* and beacons are received. */
                 ...
                 void *p, uint plen, void *a, int32 alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_PSTA):
		if ((int_val < PSTA_MODE_DISABLED) || (int_val > PSTA_MODE_REPEATER))
			 ...
			 */
			if (DWDS_ENAB(cfg))
				cfg->saved_psta_mode = PSTA_MODE_DISABLED;
#endif
		}
		break;
...
/* For Dongle memory optimization we redefine PSTA_MAX_ASSOC number here
* rather than in wlc_psta.h that can save 912 bytes.
*/
#ifdef MAXVSLAVEDEVS
#if MAXVSLAVEDEVS > 0 && MAXVSLAVEDEVS < (RCMTA_SIZE / 2)
#if MAXVSLAVEDEVS < 2
#error "MAXVSLAVEDEVS is too small for PSTA/PSTAR!"
#endif
#undef PSTA_MAX_ASSOC
#define PSTA_MAX_ASSOC(wlc)	(PSTA_IS_REPEATER(wlc) ? (MAXVSLAVEDEVS - 1) : MAXVSLAVEDEVS)
#endif /* MAXVSLAVEDEVS > 0 && MAXVSLAVEDEVS < (RCMTA_SIZE / 2) */
#endif /* MAXVSLAVEDEVS */
                ...
                wlc_bsscfg_t *pcfg)
{
	...
	chanspec_t chanspec;
	psta_bsscfg_cubby_t *psta_cfg;
	wl_psta_create_event_t psta_create_e;
	...
	/* Notify host a PSTA creation event */
	wlc_eventq_set_ind(wlc->eventq, WLC_E_PSTA_CREATE_IND, TRUE);
	memcpy(&psta_create_e.ds_ea, &psta_cfg->psa->ds_ea, ETHER_ADDR_LEN);
	memcpy(&psta_create_e.psta_ea, ea, ETHER_ADDR_LEN);
	wlc_bss_mac_event(pcfg->wlc, cfg, WLC_E_PSTA_CREATE_IND, NULL, 0, 0, 0,
		&psta_create_e, sizeof(wl_psta_create_event_t));
                   ...
                   wlc_bsscfg_t **bsscfg)
{
	...
	WL_PSTA(("wl%d: Sending up the frame to cfg %d\n",
	         psta->pub->unit, WLC_BSSCFG_IDX(cfg)));
