 ...
 *
 * $Id: wlc_bmac.c 519478 2014-12-07 04:14:38Z $
 */
...
wlc_bmac_update_slot_timing(wlc_hw_info_t *wlc_hw, bool shortslot)
{
    ...
    if (shortslot) {
        ...
        if (region_group == REGION_EU) {
            W_REG(osh, &regs->u.d11regs.ifs_slot, 0x0009);  /*
                                                               APHY_SLOT_TIME */
        }
        else {
            W_REG(osh, &regs->u.d11regs.ifs_slot, 0x0207);        /*
                                                                     APHY_SLOT_TIME */
        }
        ...
        if (region_group == REGION_EU) {
            W_REG(osh, &regs->u.d11regs.ifs_slot, 0x0014);  /*
                                                               BPHY_SLOT_TIME */
        }
        else {
            W_REG(osh, &regs->u.d11regs.ifs_slot, 0x0212);  /*
                                                               BPHY_SLOT_TIME */
        }
	...
	bool piomode, osl_t *osh, void *regsva, uint bustype, void *btparam)
{
	...
	if ((BCM4350_CHIP(wlc_hw->sih->chip) &&
	     ...
	     CHIPID(wlc_hw->sih->chip) == BCM43602_CHIP_ID ||
	     CHIPID(wlc_hw->sih->chip) == BCM43462_CHIP_ID) { /* 43602 is PCIe only */
...
BCMINITFN(wlc_bmac_reset)(wlc_hw_info_t *wlc_hw)
{
	...
	#define LED_GPIO (10)
	if (/*  D11REV_IS(wlc_hw->corerev, 40) ||
		D11REV_IS(wlc_hw->corerev, 42) || */
		D11REV_IS(wlc_hw->corerev, 49)) {
		/* gpio override gpio10 */
		si_ccreg(wlc_hw->sih, CHIPCTRLADDR, ~0, 0x1);
		si_ccreg(wlc_hw->sih, CHIPCTRLDATA, (0x1<<(16+LED_GPIO)), (0x1<<(16+LED_GPIO)));
	}
}
...
#ifdef BCMSDIODEV
	{1, 128, 128, 1, 1, 0, {16, 16, 16, 16, 16, 0, 128, 0, 0}};
#else
