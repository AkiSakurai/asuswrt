 ...
 *
 * $Id: wlc_ampdu_rx.c 484017 2014-06-11 22:17:13Z $
 */
...
#define AMPDU_HOST_HASPKT(resp, index, osh, p)	\
	{		\
		...
		AMPDU_CONSOLIDATE_AMSDU_RXCHAIN(osh, p);	\
...
#define AMPDU_CLEAR_HOSTPKT(resp, index)	\
	{	\
		(resp)->host_pkt_pending[index] = FALSE;	\
		(resp)->rxq[index] = 0; 	\
	}
#if defined(BCMPKTIDMAP)
#define AMPDU_CONSOLIDATE_AMSDU_RXCHAIN(osh, p)   wl_chain_rxcompletions_amsdu(osh, p)
...
#else /* BCMPKTIDMAP */
#define AMPDU_CONSOLIDATE_AMSDU_RXCHAIN(osh, p) do {} while (0)
