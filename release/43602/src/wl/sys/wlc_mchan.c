 ...
 *
 * $Id: wlc_mchan.c 472837 2014-04-25 11:07:45Z $
 */
...
wlc_mchan_create_bss_chan_context(wlc_info_t *wlc, wlc_bsscfg_t *cfg, chanspec_t chanspec)
{
	...
	if (chan_ctxt) {
		...
		else if (!wlc_mchan_context_in_use(mchan, chan_ctxt, cfg)) {
			if (new_chan_ctxt != NULL) {
				if (!BSS_TDLS_ENAB(wlc, cfg) || (BSS_TDLS_ENAB(wlc, cfg) &&
					(new_chan_ctxt != wlc->cfg->chan_context))) {
					WL_MCHAN(("context not in use, new exist, delete\n"));
					/* delete the old context on different chanspec first */
					_wlc_mchan_delete_bss_chan_context(wlc, cfg,
						&oldqi_stopped_flag);
					chan_ctxt = NULL;
				}
			}
				...
				WL_MCHAN(("context in use, new not exist, reuse\n"));
#ifdef ENABLE_FCBS
				wlc_phy_fcbs_uninit(wlc->band->pi, chan_ctxt->chanspec);
				wlc_phy_fcbs_arm(wlc->band->pi, chanspec, 0);
#endif
