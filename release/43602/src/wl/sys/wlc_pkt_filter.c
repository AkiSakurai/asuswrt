...
#if LOCAL_DEBUG
#undef WL_INFORM
#define WL_INFORM(arg)	printf arg
#endif
...
BCMATTACHFN(wlc_pkt_filter_attach)(void *context)
{
	...
	info = MALLOC(osh, sizeof(wlc_pkt_filter_info_t));
	if (info == NULL) {
#ifdef BCM_OL_DEV
	...
	bzero(info, sizeof(wlc_pkt_filter_info_t));
...
bool wlc_pkt_filter_recv_proc(wlc_pkt_filter_info_t *info, const void *sdu)
{
	...
	if (WOWLPF_ENAB(info->wlc->pub)) {
		if (match) {
			...
			wlc_wowlpf_pktfilter_cb(info->wlc, filter->id, ETHER_ISBCAST(pkt));
		}
	}
#endif
...
)
{
	...
	PF2DBG_CFG(("Comparing data_len %d with constant %d + 2*%d (%d)\n",
	            ...
	            WL_PKT_FILTER_PATTERN_FIXED_LEN + 2 * wl_pattern_filter->size_bytes));

	/* Validate filter length. */
	if (data_len != (WL_PKT_FILTER_PATTERN_FIXED_LEN + 2 * wl_pattern_filter->size_bytes)) {
		rc = BCME_BUFTOOSHORT;
		goto fail;
	}
	 ...
	 */
	mask    = &wl_pattern_filter->mask_and_pattern[0];
	pattern = &wl_pattern_filter->mask_and_pattern[wl_pattern_filter->size_bytes];
...
)
{
	...
	*/
	...
	mask    = &pattern_filter->mask_and_pattern[0];
	pattern = &pattern_filter->mask_and_pattern[pattern_filter->size_bytes];

	if (filter->type == WL_PKT_FILTER_TYPE_ENCRYPTED_PATTERN_MATCH) {
		wl_pkt_decrypter_t *decrypt_ctx = pattern_filter->decrypt_ctx;
...
wlc_pkt_filter_cache_packet(wlc_pkt_filter_info_t *info, const void *sdu, uint32 id)
{
	...
	if (!info->cached_wake_packet) {
		info->cached_wake_packet = MALLOC(osh, sizeof(pm_wake_packet_t));
		if (!info->cached_wake_packet) {
