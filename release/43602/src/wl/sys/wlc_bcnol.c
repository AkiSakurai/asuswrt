...
static int wlc_dngl_ol_bcn_channel(wlc_dngl_ol_bcn_info_t *bcn_ol, void* bcnframe, uint plen);
...
#endif
static bool wlc_dngl_ol_bcn_check_tx_stop(uint8 ie);
...
static bool
wlc_dngl_ol_bcn_check_tx_stop(uint8 ie)
{
	switch (ie) {
	...
	void *p, struct dot11_management_header *hdr, wlc_d11rxhdr_t *wrxh)
{
	...
	if (bcn_ol->beacon_ol_enable) {
...
#endif
			RXOEINC(rxoe_obssidcnt);
			return TRUE;
		}

		/*
		 * try to get it from IE TLV. If channel info in IE is present then
		 * use it else use it from RXHDR
		 */
		channel = wlc_dngl_ol_bcn_channel(bcn_ol,  bcnframe, plen);
		WL_TRACE(("%s: Channel from IE TLV: 0x%08x\n", __FUNCTION__, channel));

		if (channel == 0) {
			/*
			 * No channel info in IE now get channel info from RxHDR
			 */
			channel = wlc_recv_mgmt_rx_channel_get(wlc_dngl_ol->wlc_hw, wrxh);
			WL_TRACE(("%s: Channel from RxChan: 0x%08x\n", __FUNCTION__, channel));
		}

		/*
		 * See if channel we got is same as channel we are supposed to get beacon on
		 * if they are different and we are not in WoWl mode then send beacon to host
		 * so that host can take further action. Also skip beacon count increament code.
		 */
		if ((channel !=  pbcnmsg->ctrl_channel) &&
		    (wlc_dngl_ol->wowl_cfg.wowl_enabled == FALSE)) {
			/* Send beacon to host so that it can start roaming */
			WL_ERROR(("%s: channel change! got beacon on channel: %d instead of: %d\n",
				__FUNCTION__, channel, pbcnmsg->ctrl_channel));
			goto release;
		}
...
#ifdef BCMDBG
		if (ppcie_shared->stats.rxoe_bcncnt % 100 == 0) {
			wlc_dngl_ol_eventlog_write(wlc_dngl_ol->eventlog_ol,
				WLC_EL_TEST, ppcie_shared->stats.rxoe_bcncnt);
		}
#endif /* BCMDBG */
		...
		if (bcn_ol->ie_ol_enable) {
			...
			if (GET_IE(bcn_ol->iemask, i)) {
				...
				case DOT11_MNG_TIM_ID:
					if (wlc_dngl_ol_bcn_tim(bcn_ol, bcnframe, plen)) {
						bcn_to_host = TRUE;
						RXOEINCIE(rxoe_iechanged, i);
						WL_TRACE(("Found Beacon with TIM set\n"));
#ifdef BCMDBG
						wlc_dngl_ol_eventlog_write(wlc_dngl_ol->eventlog_ol,
							WLC_EL_BEACON_IE_CHANGED, i);
#endif
					}
					break;
				case DOT11_MNG_VS_ID:
					if (wlc_dngl_ol_bcn_vndrie(bcn_ol, bcnframe, plen)) {
						bcn_to_host = TRUE;
						RXOEINCIE(rxoe_iechanged, i);
						WL_TRACE(("Found Beacon with VS_ID set\n"));
#ifdef BCMDBG
						wlc_dngl_ol_eventlog_write(wlc_dngl_ol->eventlog_ol,
							WLC_EL_BEACON_IE_CHANGED, i);
#endif
					...
					break;
				default:
					if (wlc_dngl_ol_bcn_compare(bcn_ol, i, bcnframe, plen)) {
					    bcn_to_host = TRUE;
					    RXOEINCIE(rxoe_iechanged, i);
#ifdef BCMDBG
					    wlc_dngl_ol_eventlog_write(wlc_dngl_ol->eventlog_ol,
					    ...
					    if (wlc_dngl_ol->TX &&
					        ...
					        wlc_dngl_ol_bcn_check_tx_stop(i)) {

						    WL_ERROR(("ALERT: Changed IE %d in beacon"
							" causing TX to stop\n", i));

						    wlc_dngl_ol->TX = FALSE;
						    wlc_dngl_ol_eventlog_write(
							wlc_dngl_ol->eventlog_ol,
							WLC_EL_BEACON_IE_TX_STOP, i);
					   }
					}
					break;
			    } /* switch */
		...
		if (bcn_to_host && (wlc_dngl_ol->wowl_cfg.wowl_enabled == FALSE)) {
			goto release;
		}
		if (!bcn_to_host)
			return TRUE;
...
wlc_dngl_ol_find_vndrie(vndriemask_info *ouidata, void *iedata, uint plen)
{
	...
	while ((ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_VS_ID))) {
		...
		if (memcmp((uint8*)oui_orig, (uint8*)ouidata,
			sizeof(vndriemask_info)) == 0) {
	...
	*/
	...
	for (i = 0; i < maxie; i++) {
		if (SET_VNDRIE(pbcnmsg->vndriemask[i].oui.mask)) {
			...
			/* not found in beacon send up */
			if (!bcn_ie) {
				WL_TRACE(("oui not in BCN\n"));
				return TRUE;
			}
				/* is IE/OUI/Subtype in orig */
			orig_ie = wlc_dngl_ol_find_vndrie(&pbcnmsg->vndriemask[i],
				(uint8 *)pbcnmsg->iedata, pbcnmsg->iedatalen);
			/* Mask has but iedata doesnt so sendup */
			if (!orig_ie) {
				WL_TRACE(("oui not in orig\n"));
				return TRUE;
			}

			/* compare both ie */
			if (orig_ie->len != bcn_ie->len) {
				WL_TRACE(("oui found ie length mismatch\n"));
				return TRUE;
			}
			if (memcmp(bcn_ie, orig_ie, orig_ie->len)) {
				WL_TRACE(("oui mismatch reset deferral\n"));
				return TRUE;
			}
...
static int wlc_dngl_ol_bcn_channel(wlc_dngl_ol_bcn_info_t *bcn_ol, void* bcnframe, uint plen)
{
	...
	if ((ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_HT_ADD)) != NULL) {
		return (ie->data[0]);
	}
	if ((ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_VHT_OPERATION_ID)) != NULL) {
		return (ie->data[0]);
	}
	return 0;
}
...
static void wlc_dngl_ol_bcn_enable_dump(wlc_dngl_ol_bcn_info_t *bcn_ol, olmsg_bcn_enable *pbcnmsg)
{
	WL_TRACE(("defcnt:%d bcn_length:%d bi:%d\n"
			"ctrl_channel:%d aid:%d iemask:0x%x iedatalen:%d\n", pbcnmsg->defcnt,
			pbcnmsg->bcn_length, pbcnmsg->bi,
			pbcnmsg->ctrl_channel, pbcnmsg->aid, bcn_ol->iemask[0],
			pbcnmsg->iedatalen));
	WL_TRACE(("%p %p\n", bcn_ol, pbcnmsg));
