 ...
 *
 * $Id: wlc_pdsvc.c 467163 2014-04-02 17:41:32Z $
 */
...
static int
wlc_proxd_bsscfg_init(wlc_pdsvc_info_t *pdsvc)
{
	wlc_info_t *wlc = pdsvc->wlc;
	wlc_bsscfg_t *bsscfg = pdsvc->bsscfg;
	...
	/* Mark that this bsscfg is for proximity detection */
	bsscfg->flags2 |= WLC_BSSCFG_FL2_PROXD;
...
wlc_proxd_ifadd(wlc_pdsvc_info_t *pdsvc, struct ether_addr *addr)
{
	...
	if (wlc_proxd_bsscfg_init(pdsvc) != BCME_OK) {
...
BCMATTACHFN(wlc_proxd_attach)(wlc_info_t *wlc)
{
	 ...
	 */
	if (wlc_module_register(wlc->pub, wlc_proxd_iovars, "proxd", (void *)pdsvc,
		wlc_proxd_doiovar, NULL, NULL, NULL) != BCME_OK) {
		WL_ERROR(("wl%d: %s: wlc_module_register() failed\n", wlc->pub->unit,
			__FUNCTION__));
		goto fail;
	}

	if (si_pmu_fvco_pllreg(wlc->hw->sih, &pdsvc->fvco, &pdsvc->pllreg) != BCME_OK) {
		WL_ERROR(("wl%d: %s: get fvco failed\n", wlc->pub->unit, __FUNCTION__));
...
int
wlc_proxd_recv_action_frames(wlc_info_t *wlc, struct dot11_management_header *hdr, uint8 *body,
	int body_len, wlc_d11rxhdr_t *wrxh, uint32 rspec)
{
...
void wlc_proxd_tx_conf(wlc_info_t *wlc, uint16 *phyctl, uint16 *mch, wlc_pkttag_t *pkttag)
{
	...
	{
		if ((*phyctl & D11AC_PHY_TXC_CORE_MASK) & ((*phyctl & D11AC_PHY_TXC_CORE_MASK)-1))
		{
			/* measurement packet using one antenna to tx */
			uint16 mask = (1 << D11AC_PHY_TXC_CORE_SHIFT);
			while (mask & D11AC_PHY_TXC_CORE_MASK) {
				if (*phyctl & mask)
					break;
				mask <<= 1;
			}
			if (mask & D11AC_PHY_TXC_CORE_MASK)
...
static int wlc_pdsvc_func(wlc_info_t *wlc, uint8 action, chanspec_t chanspec,
	struct ether_addr *addr, uint32 flags)
{
	wlc_pdsvc_info_t* pdsvc;
...
pdsvc_func_t wlc_pdsvc_register(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, notifypd notify,
	void *notifyptr, uint8 fmtcnt, struct ether_addr *allow_addr, bool setonly, uint32 flags)
{
	wlc_pdsvc_info_t* pdsvc;
	wl_proxd_params_tof_method_t tofparam;
	wl_proxd_params_tof_tune_t toftune;
