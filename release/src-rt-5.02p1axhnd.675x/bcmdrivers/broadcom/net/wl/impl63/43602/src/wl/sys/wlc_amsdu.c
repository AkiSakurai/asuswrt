 ...
 *
 * $Id: wlc_amsdu.c 695189 2017-04-19 06:53:34Z $
 */
...
#ifndef AMSDU_REMOVE_OBSOLETE_FIELDS
	uint8	pad[8];						/* Pad for ROM library */
#endif /* AMSDU_REMOVE_OBSOLETE_FIELDS */
...
void
wlc_amsdu_pkt_freed(wlc_info_t *wlc, void *pkt, uint s)
{
...
BCMATTACHFN(wlc_amsdu_attach)(wlc_info_t *wlc)
{
	...
	if (!(ami->amsdu_deagg = (amsdu_deagg_t *)MALLOC(wlc->osh,
		sizeof(amsdu_deagg_t) * RX_FIFO_NUMBER))) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto fail;
	}
	bzero((char *)ami->amsdu_deagg, sizeof(amsdu_deagg_t) * RX_FIFO_NUMBER);
...
#endif /* BCMDBG) || defined(BCMDBG_DUMP) */
...
BCMATTACHFN(wlc_amsdu_tx_attach)(amsdu_info_t *ami, wlc_info_t *wlc)
{
	...
	err = wlc_pcb_fn_set(ami->wlc->pcb, 0, WLF2_PCB1_AMSDU, wlc_amsdu_pkt_freed);
	if (err != BCME_OK) {
...
wlc_amsdu_set(amsdu_info_t *ami, bool on)
{
	...
	if (AMPDU_AQM_ENAB(wlc->pub))
		wlc->pub->txmaxpkts = MAXTXPKTS_AMPDUAQM_DFT;
	...
	void *p, uint plen, void *a, int alen, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_RX_AMSDU_IN_AMPDU):
		err = wlc_set_rx_amsdu_in_ampdu(wlc, bool_val);
 ...
 */
void * BCMFASTPATH
wlc_amsdu_pktc_agg(amsdu_info_t *ami, wlc_bsscfg_t *bsscfg, struct scb *scb,
	void *p, void *n, uint8 tid, uint32 lifetime)
{
	wlc_info_t *wlc;
	int32 pad, totlen = 0;
...
#if defined(PROP_TXSTATUS) && defined(BCMPCIEDEV)
	else if (BCMPCIEDEV_ENAB(wlc->pub) && PROP_TXSTATUS_ENAB(wlc->pub) &&
		WLFC_GET_REUSESEQ(wlfc_query_mode(wlc->wl))) {
		/*
	...
	if (PKTISTXFRAG(wlc->osh, p)) {
		...
		/* use ether header 14B space in D3_BUFFER to save the
		 * amsdu header for first p.
		 */
	...
	eacopy(SCBTAG_EA(scb), eh->ether_dhost);
	eacopy(&bsscfg->cur_etheraddr, eh->ether_shost);
	WLPKTTAG(p)->flags |= WLF_AMSDU;

#if defined(BCM_DHDHDR)
	/* for second msdu */
	if (PKTISTXFRAG(wlc->osh, n)) {
		/* Before we free second msdu D3_BUFFER we have to include the
		...
		/* free the second msdu D3_BUFFER, we don't need it anymore */
...
wlc_amsdu_agg(void *ctx, struct scb *scb, void *p, uint prec)
{
	...
	/* check existing AMSDU (if there is one) has same WLF3_FAVORED attribute as
	 * new packet
	 */
	if (scb_ami->prio[tid].amsdu_agg_p) {
...
passthrough:
	...
	WL_AMSDU(("%s: passthrough scb %p txpending %d\n", __FUNCTION__, scb,
		scb_ami->prio[tid].amsdu_agg_txpending));
	WLF2_PCB1_REG(p, WLF2_PCB1_AMSDU);
...
wlc_amsdu_agg_close(amsdu_info_t *ami, struct scb *scb, uint tid)
{
	...
	amsduprio->amsdu_agg_bytes = 0;
	amsduprio->headroom_pad_need = 0;
	WLF2_PCB1_REG(ptid, WLF2_PCB1_AMSDU);
...
wlc_recvamsdu(amsdu_info_t *ami, wlc_d11rxhdr_t *wrxh, void *p, uint16 *padp, bool chained_sendup)
{
	...
	case RXS_AMSDU_FIRST:
		...
		if (!wlc_amsdu_deagg_open(ami, fifo, p)) {
			goto abort;
		}
...
#ifdef ASSERT
		/* intermediate frames should hav 2 byte padding if wlc->hwrxoff is aligned
		* on mod 4 address
		*/
...
#ifdef ASSERT
		/* last frame should hav 2 byte padding if wlc->hwrxoff is aligned
		* on mod 4 address
		*/
		if ((ami->wlc->hwrxoff % 4) == 0)
			ASSERT((wrxh->rxhdr.RxStatus1 & RXS_PBPRES));
	...
	WL_AMSDU(("wlc_recvamsdu: add one more A-MSDU buffer %d bytes, accumulated %d bytes\n",
		PKTLEN(osh, p), pkttotlen(osh, deagg->amsdu_deagg_p)));
...
static bool BCMFASTPATH
wlc_amsdu_deagg_open(amsdu_info_t *ami, int fifo, void *p)
{
                        ...
                        bool *chained_sendup, struct scb *scb, uint16 sec_offset)
{
	...
	if (WLEXTSTA_ENAB(wlc->pub)) {
		ASSERT((sizeof(hdr_copy) + sec_offset) < PKTHEADROOM(osh, newpkt));
	...
	while (newpkt != NULL) {
		...
		if (PKTNEXT(osh, newpkt) == NULL)
			PKTFRAG_TRIM_TAILBYTES(osh, newpkt, DOT11_FCS_LEN);
