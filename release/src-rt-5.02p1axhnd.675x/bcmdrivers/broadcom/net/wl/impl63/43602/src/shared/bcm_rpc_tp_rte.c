 ...
 *
 * $Id: bcm_rpc_tp_rte.c 546914 2015-04-07 01:15:55Z $
 */
...
BCMATTACHFN(bcm_rpc_tp_attach)(osl_t * osh, void *bus)
{
	...
	rpc_th->tp_dngl_agg_sframes_limit = BCM_RPC_TP_DNGL_AGG_DEFAULT_SFRAME;
	rpc_th->tp_dngl_agg_bytes_max = BCM_RPC_TP_DNGL_AGG_DEFAULT_BYTE;
...
{
	rpc_buf_t *lb = pktlast(rpcb->osh, bb);
	if (lb) {
		uint8 *buf = bcm_rpc_buf_data(rpcb, lb);
		uint32 pktlen = bcm_rpc_buf_len_get(rpcb, lb);
		uint32 rpc_len = *((uint32 *)buf);

#ifdef BCM_FD_AGGR
		/* length bit map: 31~24: pad head; 23~16: pad tail; low 16: real RPC payload; */
		rpc_len = (rpc_len & 0xFFFF) +
			((rpc_len >> BCM_RPC_TP_PADHEAD_SHIFT) & 0xFF) +
			((rpc_len >> BCM_RPC_TP_PADEND_SHIFT) & 0xFF);
#endif /* BCM_FD_AGGR */

		BCM_REFERENCE(rpc_len);
		ASSERT(rpc_len + BCM_RPC_TP_ENCAP_LEN == pktlen);
		ASSERT((padbytes % BCM_RPC_TP_ENCAP_LEN) == 0);

		if ((uint)PKTTAILROOM(rpcb->osh, lb) >= padbytes) {
			memset(buf + pktlen, 0, padbytes);
			pktlen += padbytes;
			bcm_rpc_buf_len_set(rpcb, lb, pktlen);
		} else {
			rpc_buf_t *lbn = bcm_rpc_tp_buf_alloc(rpcb, padbytes);
			if (lbn) {
				buf = bcm_rpc_buf_data(rpcb, lbn);
				memset(buf, 0, padbytes);
				PKTSETNEXT(rpcb->osh, lbn, NULL);
				PKTSETNEXT(rpcb->osh, lb, lbn);
			} else {
				RPC_TP_ERR(("failed to allocate padding pkt\n"));
			}
		}
	}
}
...
static void
bcm_rpc_tp_agg_limit_set_internal(rpc_tp_info_t *rpc_th, uint8 sf, uint16 bytes)
{
	rpc_th->tp_dngl_agg_sframes_limit = MIN(sf, BCM_RPC_TP_DNGL_AGG_MAX_SFRAME);
	rpc_th->tp_dngl_agg_bytes_max = MIN(bytes, BCM_RPC_TP_DNGL_AGG_MAX_BYTE);
}

void
bcm_rpc_tp_agg_limit_set(rpc_tp_info_t *rpc_th, uint8 sf, uint16 bytes)
{
	uint32 val = (sf << 16) | bytes;
#ifdef BCM_FD_AGGR
	val = dngl_set_bus_agglimit((struct dngl *)rpc_th->ctx, val);
#endif /* BCM_FD_AGGR */
	bcm_rpc_tp_agg_limit_set_internal(rpc_th, (val >> 16) & 0xFF, val & 0xFFFF);
}
...
bcm_rpc_tp_agg_set(rpc_tp_info_t *rpcb, uint32 reason, bool set)
{
	...
	if (set) {
		uint32 vn, vo = (rpcb->tp_dngl_agg_sframes_limit << 16) |
			rpcb->tp_dngl_agg_bytes_max;
		RPC_TP_AGG(("%s: agg start\n", __FUNCTION__));

		mboolset(rpcb->tp_dngl_aggregation, reason);

#ifdef BCM_FD_AGGR
		vn = dngl_set_bus_agglimit((struct dngl *)rpcb->ctx, vo);
#else
		/* for BMAC, need use the chained ops ioctl to access the dngl bus setting */
		vn = vo;
#endif /* BCM_FD_AGGR */
		if (vo != vn) {
			RPC_TP_AGG(("original %d %d\n", (vo >> 16) & 0xFF, vo & 0xFFFF));
			RPC_TP_AGG(("changed to %d %d\n", (vn >> 16) & 0xFF, vn & 0xFFFF));
			bcm_rpc_tp_agg_limit_set_internal(rpcb, (vn >> 16) & 0xFF, vn & 0xFFFF);
		}
...
#ifdef BCM_FD_AGGR
		(void) dngl_set_bus_agglimit((struct dngl *)rpcb->ctx, 0);
#endif /* BCM_FD_AGGR */
	}
}
