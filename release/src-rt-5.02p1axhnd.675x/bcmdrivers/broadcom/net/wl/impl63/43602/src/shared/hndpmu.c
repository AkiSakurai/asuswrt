 ...
 *
 * $Id: hndpmu.c 666951 2016-10-25 09:44:01Z $
 */
...
static rsc_per_chip_t* si_pmu_get_rsc_positions(si_t *sih)
{
	...
	case BCM43362_CHIP_ID:
		...
		break;
#ifdef UNRELEASEDCHIP
	case BCM43457_CHIP_ID:  /* same resource defs for bits in struct rsc_per_chip_s as 4350 */
	...
	if (ISSIM_ENAB(sih)) {
		...
		case BCM43569_CHIP_ID:
			...
			pmudelay = (si_pmu_res_uptime(sih, osh, cc, rsc->ht_avail) +
				D11SCC_SLOW2FAST_TRANSITION) * ((1000000 + ilp - 1) / ilp);
#ifdef SRFAST
			if (BCM4350_CHIP(sih->chip) && CST4350_CHIPMODE_SDIOD(sih->chipst)) {
				pmudelay = pmudelay +450;
			} else {
				pmudelay = (11 * pmudelay) / 10;
			}
#else
...
BCMATTACHFN(si_pmu_res_init)(si_t *sih, osl_t *osh)
{
	...
	if (BCM43602_CHIP(sih->chip) && SR_ENAB() && sr_isenab(sih)) {
...
/*	PLL 0       PLL 1       PLL 2       PLL 3       PLL 4       */
	...
	0x000a033c, 0x080004e2, 0x0000000A, 0x0000106A,
...
/*	PLL 0       PLL 1       PLL 2       PLL 3       PLL 4       */
	0x00000000, 0x00000000, 0x000012b1, 0x00627627, 0x0c060c0c,
	0x000a033d, 0x080004e2, 0x0000000A, 0x0000106A,
	0x00000000, 0x00000000, 0x00000cb1, 0x00FD4314, 0x0c060c0c,
	0x000a033d, 0x080004e2, 0x0000000A, 0x0000106A
};

static const uint32	pmu1_pllctrl_tab_43430_984mhz[] = {
/*	PLL 0       PLL 1       PLL 2       PLL 3       PLL 4       */
	0x00000000, 0x00000000, 0x000012b1, 0x00d888ff, 0x0c060b0b,
	0x000a033c, 0x080004e2, 0x0000000A, 0x0000106A,
	0x00000000, 0x00000000, 0x00000db1, 0x004efff0, 0x0c060b0b,
	0x000a033c, 0x080004e2, 0x0000000A, 0x0000106A
};
/*  ************************  tables for 4335a0 END *********************** */
...
#ifdef UNRELEASEDCHIP
		case BCM43457_CHIP_ID:  /* Same HT_ vals as 4350 */
...
#ifdef SRFAST
	if ((sih->pmurev >= 22) &&
		(BCM4350_CHIP(sih->chip) && CST4350_CHIPMODE_SDIOD(sih->chipst)) &&
		(R_REG(osh, PMUREG(sih, pmucontrol_ext)) & (1 << PCTLEX_FTE_SHIFT))) {
		return uptime + dmax;
	} else {
...
BCMATTACHFN(si_pmu_swreg_init)(si_t *sih, osl_t *osh)
{
...
#if defined(DONGLEBUILD)
	{
		...
		if (getintvar(NULL, rstr_avs_enab)) {
			...
			if (otp_read_word(sih, OTP4345_AVS_STATUS_OFFSET, &avs_status) != BCME_OK)
				break;
