 ...
 *
 * $Id: wlc_phy_lp.c 430789 2013-10-21 10:52:43Z $
 */

/* XXX WARNING: phy structure has been changed, read this first
 *
 * This submodule is for LP PHYs only. It depends on the common submodule wlc_phy_cmn.c
 *   This file was created (split out of wlc_phy.c) on 08/17/2009.
 *   The full CVS history inherited from old wlc_phy_lpssn.c. cvs annotate -r 1.2146 wlc_phy_lpssn.c
 *   Refer to wlc_phy_cmn.c for new structure
 *
 * Future Improvement: cleanup code, restructuring to share some functions
 *              clean up ifdef, if LPCONF,
...
/* XXX To profile how long aci channel scan takes
 * This has been removed since it reaches over to access the tsf timer.
 * Add back if needed, only for debug as this should NOT be an external feature.
 */
...
wlc_lpphy_btcx_override_enable(phy_info_t *pi)
{
	if (LPREV_GE(pi->pubpi.phy_rev, 2) && (pi->sh->machwcap & MCAP_BTCX)) {
		...
		W_REG(pi->sh->osh, &pi->regs->PHYREF_BTCX_CTRL, 0x03);
		/* Force WLAN priority */
		W_REG(pi->sh->osh, &pi->regs->PHYREF_BTCX_TRANS_CTRL, 0xff);
	}
}
...
WLBANDINITFN(wlc_lpphy_rev2_baseband_init)(phy_info_t *pi)
{
	...
	if (CHIPID(pi->sh->chip) == BCM4325_CHIP_ID &&
		...
		PHY_REG_LIST_START_WLBANDINITDATA
			/*
			 * fixme: not all of these reg writes are needed b/c some
			 * values are the same as the dflt
			 */
...
BCMATTACHFN(wlc_phy_txpwr_srom_read_lpphy)(phy_info_t *pi)
{
	...
	/*
	 * japan_wide_filter is only used on channel 14 by rev1 logic (PR50114),
	 * by rev0 logic (wlc_lpphy_pr41573), or when not a 2063 radio (i.e. 2062)
	 * logic (PR46135) even though a value is always put in the pi structure for
	 * a lpphy.
	 */
...
wlc_lpphy_rc_cal_rev0(phy_info_t *pi)
{
	...
	int ddfs_tone;
	phy_info_lpphy_t *pi_lp = pi->u.pi_lpphy;
	chanspec_t save_chan = pi->radio_chanspec;
...
cleanup:
	...
	wlc_phy_chanspec_set_lpphy(pi, save_chan);
}
...
#if defined(PHYCAL_CACHING)
	txgains.index = (uint8)cache->lpphy_papd_cal_gain_index;
#else
...
wlc_radio_2063_channel_tune(phy_info_t *pi, uint8 channel)
{
	...
	/* XXX STALE:
	 * fixed qFxtal = wlc_phy_qdiv_roundup(PHY_XTALFREQ(pi->xtalfreq), PLL_2063_MHZ, 16);
	 */
