 ...
 *
 * $Id: wlc_wnm.c 776762 2019-07-09 06:27:51Z $
 */
...
typedef struct wnm_bsstrans_sta_info_mem {
	...
	wl_bsstrans_rssi_rate_map_t *rssi_rate_map;
	wnm_bsstrans_rateidx2rate500k_t *idx2rate;
	struct scb *resp_scb;
	struct wl_timer *resp_timer;
	...
	/* patching to make 43569a2 fd compilable */
	char patch[2];
...
typedef struct {
	...
	parp_entry_t *parp_table[WNM_PARP_TABLE_SIZE];	/* proxyarp entries link table */
	parp_entry_t *parp_candidate_list;		/* proxyarp candidate entry link list */
	uint8 parp_smac[ETHER_ADDR_LEN];	/* L2 SMAC from DHCP Req frame */
	uint8 parp_cmac[ETHER_ADDR_LEN];	/* Bootp Client MAC from DHCP Req frame */
#endif /* WLWNM_AP */
...
#endif /* WLWNM_AP */
#if defined(STA) && !defined(WLWNM_AP_ONLY)
...
static int wlc_wnm_watchdog(void *context);
...
#endif /* !WLWNM_NO_ACTION_FRAMES */
#if !defined(WLWNM_PARP_DISABLED)
static int wlc_wnm_parp_addentry(wlc_info_t *wlc, wnm_bsscfg_cubby_t *wnm_cfg,
	struct ether_addr *ea, uint8 *ip, uint8 ip_ver, bool cached);
static int wlc_wnm_parp_delentry(wlc_info_t *wlc, wnm_bsscfg_cubby_t *wnm_cfg,
	struct ether_addr *ea, uint8 *ip, uint8 ip_ver, bool cached);
static parp_entry_t *wlc_wnm_parp_findentry(wlc_info_t *wlc, wnm_bsscfg_cubby_t *wnm_cfg, uint8 *ip,
	uint8 ip_ver, bool cached);
static void wlc_wnm_parp_watchdog(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bool all, uint8 *del_ea,
	bool periodic);
#endif /* !WLWNM_PARP_DISABLED */
#endif /* WLWNM_AP */
...
BCMATTACHFN(wlc_wnm_attach)(wlc_info_t *wlc)
{
	...
	}
	{
		...
		wnm->bsstrans_stainfo = (wnm_bsstrans_sta_info_t *)bsi_mem;
		...
		rrm->len = sizeof(wl_bsstrans_rssi_rate_map_t);
	...
	wnm->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(wnm_scb_cubby_t),
		...
		wlc_wnm_scb_init, wlc_wnm_scb_deinit, wlc_wnm_scb_dump, (void *)wnm, 0);
#else
...
wlc_wnm_bsstrans_query_send(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg, wlc_bss_list_t *list)
{
	...
	ngb = (dot11_neighbor_rep_ie_t *) query->data;
	for (i = 0; i < bss_cnt; i++) {
		uint32 bssid_info;
		dot11_ngbr_bsstrans_pref_se_t *pref;
		wlc_bss_info_t *bi = list->ptrs[i];

		WL_WNM(("+ BssTransQuery: adding <%s>\n", bi->SSID));

		ngb->id = DOT11_MNG_NEIGHBOR_REP_ID;
		ngb->len = DOT11_NEIGHBOR_REP_IE_FIXED_LEN +
			TLV_HDR_LEN + DOT11_NGBR_BSSTRANS_PREF_SE_LEN;
		memcpy(&ngb->bssid, &bi->BSSID, ETHER_ADDR_LEN);

		bssid_info = DOT11_NGBR_BI_REACHABILTY_UNKN |
			((bi->capability & DOT11_CAP_SPECTRUM)? DOT11_NGBR_BI_CAP_SPEC_MGMT : 0) |
			((bi->capability & DOT11_CAP_QOS)? DOT11_NGBR_BI_CAP_QOS : 0) |
			((bi->capability & DOT11_CAP_APSD)? DOT11_NGBR_BI_CAP_APSD : 0) |
			((bi->capability & DOT11_CAP_RRM)? DOT11_NGBR_BI_CAP_RDIO_MSMT : 0) |
			((bi->capability & DOT11_CAP_DELAY_BA)? DOT11_NGBR_BI_CAP_DEL_BA : 0) |
			((bi->capability & DOT11_CAP_IMMEDIATE_BA)? DOT11_NGBR_BI_CAP_IMM_BA : 0);
		htol32_ua_store(bssid_info, (uint8 *)(&ngb->bssid_info));
		/* Operating class (or 802.11 epic fail)... should not be trusted */
		ngb->reg = wlc_get_regclass(wlc->cmi, bi->chanspec);
		ngb->channel = CHSPEC_CHANNEL(bi->chanspec);
		/* PHY type (or historically justified wl bug) ... should not be trusted either */
		ngb->phytype = (uint8)wlc->band->phytype;
		/* Add preference subelement */
		pref = (dot11_ngbr_bsstrans_pref_se_t*) ngb->data;
		pref->sub_id = DOT11_NGBR_BSSTRANS_PREF_SE_ID;
		pref->len = DOT11_NGBR_BSSTRANS_PREF_SE_LEN;
		/* Average preference constant for all scanned APs */
		pref->preference = 128;

		ngb = (dot11_neighbor_rep_ie_t *) (((uint8*) ngb) +
	...
	uint8 *body, int body_len, bool unicast_req, dot11_bsstrans_req_t* req)
{
	...
	if (throttle->scans_done == throttle->scans_allowed) {
	...
	uint8 *body, int body_len)
{
	...
	wlc = wnm->wlc;
	BCM_REFERENCE(wnm_cfg);
	BCM_REFERENCE(wnm_scb);
	...
	case DOT11_WNM_ACTION_BSSTRANS_REQ:
		...
		break;
#endif /* STA && !WLWNM_AP_ONLY */
#if defined(WLWNM_AP) && !defined(WLWNM_NO_ACTION_FRAMES)
...
#endif /* WLWNM_AP && !WLWNM_NO_ACTION_FRAMES */
#if defined(STA) && !defined(WLWNM_AP_ONLY)
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_WNM_BSSTRANS_RSSI_RATE_MAP): {
		...
		map_len = MIN(map->len, sizeof(wl_bsstrans_rssi_rate_map_t));
	...
	case IOV_GVAL(IOV_WNM_BSSTRANS_RSSI_RATE_MAP): {
		...
		if (p_len < (int)(sizeof(wl_bsstrans_rssi_rate_map_t)))
			return BCME_BUFTOOSHORT;

		memcpy(map, wnm->bsstrans_stainfo->rssi_rate_map,
			sizeof(wl_bsstrans_rssi_rate_map_t));
		break;
	}
...
#if defined(WLWNM_AP) && !defined(WLWNM_PARP_DISABLED)
...
#endif /* WLWNM_AP && WLWNM_PARP_DISABLED */
...
wlc_wnm_scb_dump(void *context, struct scb *scb, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "     WNM cap: 0x%x\n", wlc_wnm_get_scbcap(wnm->wlc, scb));
	if (capbuf[0] != '\0')
	...
	uint8 check_beacon, bool tsf_present)
{
	...
	wlc_iem_build_ie(wlc->iemi, cfg, FC_BEACON, DOT11_MNG_TIM_ID, &cbparm, buf, 256);
	tim_ie_len = wlc_iem_calc_ie_len(wlc->iemi, cfg, FC_BEACON, DOT11_MNG_TIM_ID, &cbparm);
...
wlc_wnm_timbc_tbtt(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, wnm_bsscfg_cubby_t *wnm_cfg)
{
	...
	if ((ielen = wlc_iem_calc_ie_len(wlc->iemi, bsscfg, FC_BEACON, DOT11_MNG_DS_PARMS_ID,
	    &cbparm)) != 0) {
		uint8 dsie[4] = {0};
		wlc_iem_build_ie(wlc->iemi, bsscfg, FC_BEACON, DOT11_MNG_DS_PARMS_ID, &cbparm,
			dsie, 4);
	...
	if ((ielen = wlc_iem_calc_ie_len(wlc->iemi, bsscfg, FC_BEACON, DOT11_MNG_HT_ADD,
		...
		wlc_iem_build_ie(wlc->iemi, bsscfg, FC_BEACON, DOT11_MNG_HT_ADD, &cbparm,
			htie, TLV_HDR_LEN + HT_ADD_IE_LEN);
	...
	if (flush_tim && tx_ratespec) {
		...
		if (BSSCFG_AP(bsscfg)) {
			if (WL11H_ENAB(wlc)) {
				if (wlc_iem_calc_ie_len(wlc->iemi, bsscfg, FC_BEACON,
				    DOT11_MNG_CHANNEL_SWITCH_ID, &cbparm))
					ie_include = TRUE;
				if (wlc_iem_calc_ie_len(wlc->iemi, bsscfg, FC_BEACON,
				    DOT11_MNG_EXT_CSA_ID, &cbparm))
					ie_include = TRUE;
			}
			if (BSS_WL11H_ENAB(wlc, bsscfg) && wlc_iem_calc_ie_len(wlc->iemi,
			    bsscfg, FC_BEACON, DOT11_MNG_CHANNEL_SWITCH_ID, &cbparm))
				ie_include = TRUE;
		}
		...
		if ((wnm_cfg->timbc_frame.p_low = wlc_frame_get_timframe(wlc, bsscfg, 0,
			...
			WL_ERROR(("%s : TIM basic rate frame allocation failed\n",
				__func__));
			return;
		}
		...
		if ((wnm_cfg->timbc_frame.p = wlc_frame_get_timframe(wlc, bsscfg, tx_ratespec,
		    wnm_cfg->timbc_frame.check_beacon, wnm_cfg->timbc_tsf)) == NULL) {
			WL_ERROR(("%s : TIM high rate frame allocation failed\n",
				__func__));
			return;
		}
	}
...
wlc_wnm_prep_keydata_ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct scb *scb, uint8 *p)
{
	...
	uint32 ipn_lo;
	uint16 ipn_hi, key_id;
	wsec_key_t *key;

	/* Get GTK key */
	key = bsscfg->bss_def_keys[bsscfg->wsec_index];
	ASSERT(key);

	/* construct GTK info */
	gtk->sub_id = DOT11_WNM_SLEEP_SUBELEM_ID_GTK;
	gtk->len = DOT11_WNM_SLEEP_SUBELEM_GTK_FIXED_LEN + key->len;
	gtk->key_length = key->len;
	/* key info */
	htol16_ua_store(bsscfg->wsec_index & WPA2_GTK_INDEX_MASK, &(gtk->key_info));
	/* RSC */
	ASSERT(bsscfg == wlc_bsscfg_primary(wlc));
	ASSERT(!(bsscfg->WPA_auth & WPA_AUTH_NONE));
	htol16_ua_store(key->txiv.lo, gtk->rsc + 0);
	htol32_ua_store(key->txiv.hi, gtk->rsc + 2);
	htol16_ua_store(0, gtk->rsc + 6);
	/* key */
	memcpy(gtk->key, key->data, key->len);
	...
	/* key id */
	key_id = wlc_mfp_igtk_id(wlc->mfp, bsscfg);
	htol16_ua_store(key_id, (uint8*)&(igtk->key_id));
	/* pn */
	wlc_mfp_igtk_ipn(wlc->mfp, bsscfg, &ipn_lo, &ipn_hi);
	htol32_ua_store(ipn_lo, igtk->pn + 0);
	htol16_ua_store(ipn_hi, igtk->pn + 4);
	/* key */
	wlc_mfp_igtk_key(wlc->mfp, bsscfg, igtk->key);
...
#if !defined(WLWNM_PARP_DISABLED)
static int
wlc_wnm_parp_addentry(wlc_info_t *wlc, wnm_bsscfg_cubby_t *wnm_cfg, struct ether_addr *ea,
	uint8 *ip, uint8 ip_ver, bool cached)
{
	...
	if (ip_ver == IP_VER_4 && !IPV4_ADDR_NULL(ip) && !IPV4_ADDR_BCAST(ip)) {
		idx = WNM_PARP_TABLE_INDEX(ip[IPV4_ADDR_LEN - 1]);
		ip_len = IPV4_ADDR_LEN;
	}
	else if (ip_ver == IP_VER_6 && !IPV6_ADDR_NULL(ip)) {
		idx = WNM_PARP_TABLE_INDEX(ip[IPV6_ADDR_LEN - 1]);
		...
		if (ip_ver == IP_VER_4)
			bcm_ip_ntoa((struct ipv4_addr *)ip, ipbuf);
		else
			bcm_ipv6_ntoa((void *)ip, ipbuf);
		WL_ERROR(("Invalid addentry argument: IPv%d %s %s\n", ip_ver, eabuf, ipbuf));
#endif /* BCMDBG */

		return BCME_ERROR;
	}

	if ((entry = MALLOCZ(wlc->osh, sizeof(parp_entry_t) + ip_len)) == NULL) {
		WL_ERROR(("Allocating new parp_entry for IPv%d failed!!\n", ip_ver));
		return BCME_NOMEM;
	}

	bcopy((void *)ea, (void *)&entry->ea, ETHER_ADDR_LEN);
	entry->used = wlc->pub->now;
	entry->ip.id = ip_ver;
	entry->ip.len = ip_len;
	bcopy(ip, entry->ip.data, ip_len);

	/* if cached, put entry into cache table.  Otherwise put it in candidate list */
	if (cached) {
		entry->next = wnm_cfg->parp_table[idx];
		wnm_cfg->parp_table[idx] = entry;
	}
	else {
		entry->next = wnm_cfg->parp_candidate_list;
		wnm_cfg->parp_candidate_list = entry;
	}
...
static int
wlc_wnm_parp_delentry(wlc_info_t *wlc, wnm_bsscfg_cubby_t *wnm_cfg, struct ether_addr *ea,
	uint8 *ip, uint8 ip_ver, bool cached)
{
	...
	if (ip_ver == IP_VER_4) {
		...
		if (ip_ver == IP_VER_4)
			...
			bcm_ipv6_ntoa((void *)ip, ipbuf);
		WL_ERROR(("Invalid delentry argument: IPv%d %s %s\n", ip_ver, eabuf, ipbuf));
#endif /* BCMDBG */
		return BCME_ERROR;
	}

	if (cached)
		entry = wnm_cfg->parp_table[idx];
	else
		entry = wnm_cfg->parp_candidate_list;
	...
	while (entry) {
		if (entry->ip.id == ip_ver &&
			...
			if (prev == NULL) {
				if (cached)
					wnm_cfg->parp_table[idx] = entry->next;
				else
					wnm_cfg->parp_candidate_list = entry->next;
			}
			else
	...
	if (entry != NULL)
		MFREE(wlc->osh, entry, sizeof(parp_entry_t) + ip_len);

	return BCME_OK;
}

static parp_entry_t *
wlc_wnm_parp_findentry(wlc_info_t *wlc, wnm_bsscfg_cubby_t *wnm_cfg, uint8 *ip, uint8 ip_ver,
	bool cached)
{
	parp_entry_t *entry;
	uint16 idx, ip_len;
	...
	if (ip_ver == IP_VER_4) {
		...
		if (ip_ver == IP_VER_4)
			...
			bcm_ipv6_ntoa((void *)ip, ipbuf);
		WL_ERROR(("Invalid findentry argument: IPv%d %s\n", ip_ver, ipbuf));
#endif /* BCMDBG */
		return NULL;
	}
...
void
wlc_wnm_parp_watchdog(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bool all, uint8 *del_ea, bool periodic)
{
	wnm_bsscfg_cubby_t *wnm_cfg = WNM_BSSCFG_CUBBY(wlc->wnm_info, bsscfg);
	...
	for (idx = 0; idx < WNM_PARP_TABLE_SIZE; idx++) {
		entry = wnm_cfg->parp_table[idx];
		while (entry) {
			/* check if the entry need to be removed */
			if (all || (periodic && WNM_PARP_IS_TIMEOUT(wlc, entry)) ||
				...
				if (ip_ver == IP_VER_4)
					...
					bcm_ipv6_ntoa((void *)ip, ipbuf);
				if (showtitle) {
					WL_ERROR(("wl%d.%d: clear %s\n",  wlc->pub->unit,
						WLC_BSSCFG_IDX(bsscfg), all? "all": periodic?
						"periodic": "specific EA"));
					showtitle = FALSE;
				}
				WL_ERROR(("idx %d:\tIPv%d %s %s\n", idx, ip_ver, eabuf, ipbuf));
#endif /* BCMDBG */
				wlc_wnm_parp_delentry(wlc, wnm_cfg, &ea, ip, ip_ver, TRUE);
			}
			else {
				entry = entry->next;
			}
	...
	while (entry) {
		/* remove candidate */
		if (all || (periodic && WNM_PARP_ANNOUNCE_WAIT_REACH(wlc, entry)) ||
		    (del_ea != NULL && !bcmp(del_ea, (uint8 *)&entry->ea, ETHER_ADDR_LEN))) {
			bool promote = (periodic && WNM_PARP_ANNOUNCE_WAIT_REACH(wlc, entry))?
				TRUE: FALSE;
			...
			if (prev == NULL)
				...
				prev->next = entry->next;
#ifdef BCMDBG
			bcm_ether_ntoa(&entry->ea, eabuf);
			if (ip_ver == IP_VER_4)
				bcm_ip_ntoa((struct ipv4_addr *)entry->ip.data, ipbuf);
			else
				bcm_ipv6_ntoa((void *)entry->ip.data, ipbuf);
#endif /* BCMDBG */

			node = wlc_wnm_parp_findentry(wlc, wnm_cfg,
				entry->ip.data, IP_VER_6, TRUE);
			if (promote && node == NULL) {
#ifdef BCMDBG
				WL_ERROR(("wl%d.%d: Add parp_entry by candidate IPv%d %s %s\n",
					wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), ip_ver, eabuf,
					ipbuf));
#endif /* BCMDBG */
				wlc_wnm_parp_addentry(wlc, wnm_cfg, &entry->ea, entry->ip.data,
					entry->ip.id, TRUE);
			}
			else {
#ifdef BCMDBG
				WL_ERROR(("wl%d.%d: withdraw candidate IPv%d %s %s\n",
					wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), ip_ver, eabuf,
					ipbuf));
#endif /* BCMDBG */
			}
			MFREE(wlc->osh, entry, sizeof(parp_entry_t) + entry->ip.len);
...
static void *
proxyarp_alloc_reply(wlc_info_t *wlc, uint32 pktlen, struct ether_addr *src_ea,
	struct ether_addr *dst_ea, uint16 ea_type, bool snap, void **p)
{
	...
	if ((pkt = PKTGET(wlc->osh, pktlen, FALSE)) == NULL) {
	...
	uint8 *reply_to_bss, bool istx)
{
	...
	if (!IPV4_ADDR_NULL(arp->src_ip) && !IPV4_ADDR_BCAST(arp->src_ip)) {
		entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, arp->src_ip, IP_VER_4, TRUE);
		if (entry == NULL) {
#ifdef BCMDBG
...
#endif /* BCMDBG */
			wlc_wnm_parp_addentry(wlc, wnm_cfg,
	...
	/* only learning ARP-Probe(DAD) in receiving path */
		...
		if (!istx && op == ARP_OPC_REQUEST) {
			entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, arp->dst_ip,
				IP_VER_4, TRUE);
			if (entry == NULL)
				entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, arp->dst_ip,
					IP_VER_4, FALSE);
	...
	if (op == ARP_OPC_REPLY) {
		entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, arp->src_ip, IP_VER_4, FALSE);
		if (entry) {
			struct ether_addr ea;
			bcopy(&entry->ea, &ea, ETHER_ADDR_LEN);
#ifdef BCMDBG
			WL_ERROR(("wl%d.%d: withdraw candidate parp_entry IPv4 %s %s\n",
				...
				bcm_ip_ntoa((struct ipv4_addr *)arp->src_ip, ipbuf)));
#endif /* BCMDBG */
			wlc_wnm_parp_delentry(wlc, wnm_cfg, &ea, arp->src_ip, IP_VER_4, FALSE);
		}
	}
	...
	if (istx) {
		...
		if (op == ARP_OPC_REQUEST) {
			...
			if ((entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, arp->dst_ip,
				IP_VER_4, TRUE)) == NULL) {
			...
			if ((*reply = proxyarp_alloc_reply(wlc, pktlen, &entry->ea,
...
wnm_proxyarp_dhcp4_handle(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, frame_proto_t *fp, bool istx)
{
	...
	if (istx) {
		...
		if (bcmp(wnm_cfg->parp_cmac, &dhcp[DHCP_CHADDR_OFFSET], ETHER_ADDR_LEN)) {
#ifdef BCMDBG
			WL_ERROR(("wl%d.%d: Unmatch DHCP Req Client MAC (%s)",
				wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
				bcm_ether_ntoa((struct ether_addr *)wnm_cfg->parp_cmac, eabuf)));
		...
		if ((scb = wlc_scbfind(wlc, bsscfg, (struct ether_addr *)wnm_cfg->parp_smac))
		    != NULL) {
			parp_entry_t *entry = wlc_wnm_parp_findentry(wlc, wnm_cfg,
...
#endif /* BCMDBG */
				wlc_wnm_parp_addentry(wlc, wnm_cfg, &scb->ea,
					&dhcp[DHCP_YIADDR_OFFSET], IP_VER_4, TRUE);
			}
			else {
		 ...
		 */
		bcopy(eh->ether_shost, wnm_cfg->parp_smac, ETHER_ADDR_LEN);
		bcopy(&dhcp[DHCP_CHADDR_OFFSET], wnm_cfg->parp_cmac, ETHER_ADDR_LEN);
	}
	...
	uint8 *reply_to_bss, bool istx)
{
	...
	if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA ||
		...
		entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, entry_ip, IP_VER_6, TRUE);
		if (entry == NULL) {
#ifdef BCMDBG
...
#endif /* BCMDBG */
			wlc_wnm_parp_addentry(wlc, wnm_cfg, entry_ea, entry_ip, IP_VER_6,
				TRUE);
		}
	}
	/* only learning Neighbor-Solicition(DAD) in receiving path */
		...
		if (!istx) {
			entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, entry_ip, IP_VER_6,
				TRUE);
			if (entry == NULL)
				entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, entry_ip,
					IP_VER_6, FALSE);
...
#endif /* BCMDBG */
				wlc_wnm_parp_addentry(wlc, wnm_cfg, entry_ea, entry_ip,
					IP_VER_6, FALSE);
			}
		}
	}
	...
	if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA) {
		entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, entry_ip, IP_VER_6, FALSE);
		if (entry) {
			struct ether_addr ea;
			bcopy(&entry->ea, &ea, ETHER_ADDR_LEN);
#ifdef BCMDBG
			WL_ERROR(("wl%d.%d: withdraw candidate parp_entry IPv6 %s %s\n",
				...
				bcm_ipv6_ntoa((void *)entry_ip, ipbuf)));
#endif /* BCMDBG */
			wlc_wnm_parp_delentry(wlc, wnm_cfg, &ea, entry_ip, IP_VER_6, FALSE);
		}
	}
	...
	if (istx) {
		...
		if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NS) {
			...
			if ((entry = wlc_wnm_parp_findentry(wlc, wnm_cfg, nd_msg->target.addr,
			    IP_VER_6, TRUE)) == NULL) {
				if (wlc->wnm_info->parp_discard)
					return FRAME_DROP;
				else
			...
			if ((*reply = proxyarp_alloc_reply(wlc, pktlen, &entry->ea, reply_mac,
	...
	if (WLC_MFP_ENAB(wnm->wlc->pub)) {
		while (len) {
			...
			if (key[0] == DOT11_WNM_SLEEP_SUBELEM_ID_GTK) {
				...
				if (gtk->len != DOT11_WNM_SLEEP_SUBELEM_GTK_FIXED_LEN +
				                gtk->key_length)
					return BCME_ERROR;
				...
				wlc_mfp_igtk_update(wlc->mfp, bsscfg,
					igtk->len - sizeof(igtk->key_id) - sizeof(igtk->pn),
					igtk->key_id, igtk->pn, igtk->key);
			}
...
wlc_wnm_bss_max_idle_ie_process(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if (WNM_MAXIDLE_ENABLED(wnm_cfg->cap) && maxidle_ie != NULL &&
		maxidle_ie->len == DOT11_BSS_MAX_IDLE_PERIOD_IE_LEN) {
...
wlc_wnm_process_join_trgts_bsstrans(wlc_info_t *wlc, wlc_bss_info_t **bip, int trgt_count)
{
	...
	for (i = 0; i < trgt_count; i++) {
		...
		if (qbss_load_ie == NULL ||
			qbss_load_ie->length != (sizeof(dot11_qbss_load_ie_t) - TLV_HDR_LEN)) {
