...
static int   wlc_taf_watchdog(void *handle);
...
BCMATTACHFN(wlc_taf_attach)(wlc_info_t * wlc)
{
	...
	*/
	if ((taf_info->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(taf_scb_cubby_t *),
	                ...
	                wlc_taf_scbcubby_init, wlc_taf_scbcubby_exit, NULL,
				taf_info, SCB_CUBBY_ID_TAF)) < 0)
#else
			wlc_taf_scbcubby_init, wlc_taf_scbcubby_exit,
				NULL, taf_info)) < 0)
#endif // endif
	{
		goto exitfail;
...
/* memory of taf_cubby have been allocated in the target space */
/* and content of original one is copied to target */
/* all the embedded SCB in the taf_cubby have been updated into the new SCB */
#if defined(BCM_HOST_MEM_RESTORE) && defined(BCM_HOST_MEM_SCB)
int
taf_cubby_dongle2host(wlc_info_t *wlc, void *context,
struct scb* scb_dongle, struct scb* scb_host)
{
	wlc_taf_info_t *scb_cubbyinfo_taf = (wlc_taf_info_t *)context;
	taf_scb_cubby_t *scb_cubby_taf_host = NULL;
	taf_scb_cubby_t *scb_cubby_taf_dongle = *SCB_TAF_CUBBY_PTR(scb_cubbyinfo_taf, scb_dongle);

	if (scb_cubby_taf_dongle != NULL) {
		scb_cubby_taf_dongle->scb = scb_host;
		scb_cubby_taf_host = (taf_scb_cubby_t *)wlc_scb_alloc_mem_get(wlc,
			SCB_CUBBY_ID_TAF, sizeof(taf_scb_cubby_t), 1);
		if (!scb_cubby_taf_host)
			return BCME_NOMEM;

		memcpy(scb_cubby_taf_host, scb_cubby_taf_dongle, sizeof(taf_scb_cubby_t));

		scb_cubby_taf_host->scb = scb_host;
		scb_cubby_taf_host->ea = scb_host->ea;
		scb_cubby_taf_host->info.type = TAF_UNDEFINED;

		scb_cubby_taf_host->next = scb_cubbyinfo_taf->head;
		scb_cubbyinfo_taf->head = scb_cubby_taf_host;

		*SCB_TAF_CUBBY_PTR(scb_cubbyinfo_taf, scb_host) = scb_cubby_taf_host;

		/* remove taf dongle cubby */
		if (scb_cubby_taf_dongle) {
			taf_scb_cubby_t *prev, *curr;
			taf_list_delete(scb_cubbyinfo_taf, scb_cubby_taf_dongle);
			prev = (taf_scb_cubby_t *)&scb_cubbyinfo_taf->head;
			for (curr = scb_cubbyinfo_taf->head; curr; prev = curr, curr = curr->next) {
				if (scb_cubby_taf_dongle == curr) {
					prev->next = curr->next;
					break;
				}
			}
		...
		scb_cubby_taf_dongle->next = scb_cubbyinfo_taf->pool;
		scb_cubbyinfo_taf->pool = scb_cubby_taf_dongle;
		scb_cubby_taf_dongle->scb = NULL;
		scb_cubby_taf_dongle->timestamp = scb_cubbyinfo_taf->wlc->pub->now;
		*SCB_TAF_CUBBY_PTR(scb_cubbyinfo_taf, scb_dongle) = NULL;
		}
		return BCME_OK;
	}
	return BCME_ERROR;
...
struct scb* scb_dongle, struct scb* scb_host)
{
	wlc_taf_info_t *taf_info = (wlc_taf_info_t *)context;
	taf_scb_cubby_t *scb_taf_dongle = NULL;
	taf_scb_cubby_t *scb_taf_host = *SCB_TAF_CUBBY_PTR(taf_info, scb_host);
	if (scb_taf_host != NULL) {
		scb_taf_host->scb = scb_dongle;
		scb_taf_dongle = MALLOCZ(wlc->osh, sizeof(taf_scb_cubby_t));
		if (scb_taf_dongle == NULL)
			return BCME_NOMEM;

		memcpy(scb_taf_dongle, scb_taf_host, sizeof(taf_scb_cubby_t));
		scb_taf_dongle->scb = scb_dongle;
		scb_taf_dongle->ea = scb_dongle->ea;
		scb_taf_dongle->info.type = TAF_UNDEFINED;
		scb_taf_dongle->next = taf_info->head;
		taf_info->head = scb_taf_dongle;

		*SCB_TAF_CUBBY_PTR(taf_info, scb_dongle) = scb_taf_dongle;
		*SCB_TAF_CUBBY_PTR(taf_info, scb_host) = NULL;
		return BCME_OK;
	}
	return BCME_ERROR;
}
