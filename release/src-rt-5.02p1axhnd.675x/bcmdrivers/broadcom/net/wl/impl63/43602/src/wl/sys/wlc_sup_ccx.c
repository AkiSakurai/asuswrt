 ...
 *
 * $Id: wlc_sup_ccx.c 330269 2012-04-30 14:42:50Z $
 */
...
wlc_cckm_reassoc_resp(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg)
{
	...
	if (ROAMOFFL_ENAB(cfg) && ccxsup_bss_priv->wpa->gtk_len == 0) {
		if (ccxsup_bss_priv->wpa->WPA_auth == WPA2_AUTH_CCKM)
			ccxsup_bss_priv->wpa->gtk_len = cckmie->gtklen - 8;
		else /* TKIP */
			ccxsup_bss_priv->wpa->gtk_len = cckmie->gtklen;
	}
...
void
wlc_ccxsup_update_krk_btk(wlc_ccxsup_info_t *ccxsup_info, wlc_bsscfg_t *cfg,
	const uint8 *krk, const uint8 *btk)
{
	bss_ccxsup_priv_t *ccxsup_bss_priv;
	bss_ccxsup_info_t *ccxsup_bss;
	...
	bcopy(krk, ccxsup_bss_priv->key_refresh_key, CCKM_KRK_LEN);
	bcopy(btk, ccxsup_bss_priv->base_transient_key, CCKM_BTK_LEN);
	WL_ROAM(("CCKM KRK: %02X%02X%02X\n", krk[0], krk[1], krk[2]));
	WL_ROAM(("CCKM BTK: %02X%02X%02X\n", btk[0], btk[1], btk[2]));
}
