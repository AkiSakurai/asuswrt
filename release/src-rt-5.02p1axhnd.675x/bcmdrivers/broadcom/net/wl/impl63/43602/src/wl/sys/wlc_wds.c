...
/* local functions */
static int wlc_wds_watchdog(void *arg);
...
wlc_wds_ioctl(void *hdl, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_GET_WDSLIST:
		...
		if (len < ((((int)wds_maclist->count - 1) * (int)sizeof(struct wds_client_info))
...
wlc_ap_wds_timeout(wlc_wds_info_t *mwds)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (SCB_LEGACY_WDS(scb)) {
			 ...
			 */
			if (((wlc->scb_activity_time && (wlc->pub->now - scb->used) >=
				wlc->scb_activity_time) || !(scb->flags & SCB_WDS_LINKUP)) &&
