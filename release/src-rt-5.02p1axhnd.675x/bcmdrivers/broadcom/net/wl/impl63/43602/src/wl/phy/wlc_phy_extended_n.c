...
wlc_phy_4324x_fds_nphy(phy_info_t *pi, bool enable)
{
	if (enable) {
		...
		phy_reg_mod(pi, NPHY_TX_freqdomainScaling, 0x1, 0x1);
	} else {
		phy_reg_mod(pi, NPHY_TX_freqdomainScaling, 0x1, 0x0);
	}

	/* set below bit so that sr engine saves/restores full 32 bits */
	phy_reg_mod(pi, NPHY_SrControl, NPHY_SrControl_UseSampPlay32_MASK,
...
wlc_phy_rfseq_nphy_rev19(phy_info_t *pi)
{
	...
	if (CHSPEC_IS5G(pi->radio_chanspec) && NREV_IS(pi->pubpi.phy_rev, LCNXN_BASEREV + 3)) {
		mod_radio_reg(pi, RADIO_20671_OVR13, 0x1 << 9, 0x1 << 9);
		mod_radio_reg(pi, RADIO_20671_OVR29, 0x1 << 9, 0x1 << 9);
		mod_radio_reg(pi, RADIO_20671_TIA_CFG1_CORE0, 0x1, 0x1);
		mod_radio_reg(pi, RADIO_20671_TIA_CFG1_CORE1, 0x1, 0x1);
	}
...
wlc_phy_oclscd_setup_cleanup_nphy(phy_info_t *pi, bool setup)
{
	...
	if (setup) {
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			phy_reg_write(pi, NPHY_pktgainSettleLen, 125);
			phy_reg_write(pi, NPHY_clip1carrierDetLen, 0x5c);
			/* bypass_ovr */
			if (NREV_IS(pi->pubpi.phy_rev, LCNXN_BASEREV + 3)) {
				phy_reg_mod(pi, NPHY_FreqGainBypass, 0x1, 0x1);
			} else {
				phy_reg_mod(pi, NPHY_FreqGainBypass, 0x1, 0x1);
			}
		} else {
			phy_reg_write(pi, NPHY_pktgainSettleLen, 135);
			phy_reg_write(pi, NPHY_clip1carrierDetLen, 0x68);
			/* bypass_ovr */
			if (NREV_IS(pi->pubpi.phy_rev, LCNXN_BASEREV + 3)) {
				phy_reg_mod(pi, NPHY_FreqGainBypass, 0x1, 0x0);
			} else {
				phy_reg_mod(pi, NPHY_FreqGainBypass, 0x1, 0x1);
			}
		...
		if (CHIP_4324_B0(pi) || CHIP_4324_B4(pi))
			phy_reg_write(pi, NPHY_pktgainSettleLen, 0x50);
		else
			phy_reg_write(pi, NPHY_pktgainSettleLen, 0x40);
...
wlc_phy_oclinit_nphy_rev19(phy_info_t *pi, uint8 ocl_en, uint8 scd_en)
{
	...
	if (CHSPEC_IS40(pi->radio_chanspec)) {
	phy_reg_mod(pi, NPHY_OCL_CoarseLength0,
	...
	/* OCL_noClipLength1 */
	phy_reg_mod(pi, NPHY_OCL_CoarseLength0, 0xff,
		0x38);

	/* OCL_noClipLength2 */
	phy_reg_mod(pi, NPHY_OCL_CoarseLength1, 0xff << 8,
		0x38 << 8);
	...
	if (NREV_IS(pi->pubpi.phy_rev, LCNXN_BASEREV + 4) ||
		...
		phy_reg_mod(pi, NPHY_OCLControl2,
	...
	if (scd_en) {
		 ...
		 */
		phy_reg_write(pi, NPHY_scd_shut_off_trig_cnt, 0x180c);

		phy_reg_mod(pi, NPHY_REV19_RxStat_Pwr_Var_muxCtrl,
		...
		if (CHSPEC_IS5G(pi->radio_chanspec)) {
			...
			PHY_REG_LIST_EXECUTE(pi);
			phy_reg_mod(pi, NPHY_SCD_dBm_Table2_3, 0xff, 0xb5);
		} else {
			PHY_REG_LIST_START
		...
		phy_reg_mod(pi, NPHY_REV19_SCD_shutOffMinSym,
	...
	phy_reg_mod(pi, NPHY_OCLControl1, NPHY_OCLControl1_ocl_core_mask_ovr_MASK,
		(pi->phy_ocl_force_core0 ? 1 : 0) << NPHY_OCLControl1_ocl_core_mask_ovr_SHIFT);
}
#endif /* Compiling out OCLSCD 4324b1/b3 - use 'oclscd' target to compile in */
...
wlc_phy_tempsense_nphy_rev19(phy_info_t *pi)
{
	...
	ocl_flag = (phy_reg_read(pi, NPHY_OCLControl1) & 0x1);
	...
	forceClk_flg = (phy_reg_read(pi, NPHY_forceClk) & 0x3ff);
	phy_reg_write(pi, NPHY_forceClk, 0x1f);

	/* save a set of radio registers */
	save_RF_reg[0] = read_radio_reg(pi, RADIO_20671_OVR13);
	save_RF_reg[1] = read_radio_reg(pi, RADIO_20671_OVR29);
	save_RF_reg[2] = read_radio_reg(pi, RADIO_20671_OVR33);
	save_RF_reg[3] = read_radio_reg(pi, RADIO_20671_OVR1);
	save_RF_reg[4] = read_radio_reg(pi, RADIO_20671_OVR17);
	save_RF_reg[5] = read_radio_reg(pi, RADIO_20671_OVR7);
	save_RF_reg[6] = read_radio_reg(pi, RADIO_20671_OVR23);
	save_RF_reg[7] = read_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE0);
	save_RF_reg[8] = read_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE1);
	save_RF_reg[9] = read_radio_reg(pi, RADIO_20671_AUXPGA_CFG1_CORE0);
	save_RF_reg[10] = read_radio_reg(pi, RADIO_20671_AUXPGA_CFG1_CORE1);
	save_RF_reg[11] = read_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0);
	save_RF_reg[12] = read_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1);
	save_RF_reg[13] = read_radio_reg(pi, RADIO_20671_OVR12);
	save_RF_reg[14] = read_radio_reg(pi, RADIO_20671_OVR28);

	/* testbuf_sel_test_port */
	mod_radio_reg(pi, RADIO_20671_OVR13, 0x1 << 15, 0x1 << 15);
	mod_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE0, 0x7 << 4, 0x1 << 4);
	mod_radio_reg(pi, RADIO_20671_OVR29, 0x1 << 15, 0x1 << 15);
	mod_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE1, 0x7 << 4, 0x1 << 4);

	/* testbuf pu */
	mod_radio_reg(pi, RADIO_20671_OVR33, 0x1 << 2, 0x1 << 2);
	mod_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE0, 0x1, 0x1);
	mod_radio_reg(pi, RADIO_20671_OVR33, 0x1, 0x1);
	mod_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE1, 0x1, 0x1);

	/* afe_auxpga pu */
	mod_radio_reg(pi, RADIO_20671_OVR1, 0x1 << 15, 0x1 << 15);
	mod_radio_reg(pi, RADIO_20671_AUXPGA_CFG1_CORE0, 0x1, 0x1);
	mod_radio_reg(pi, RADIO_20671_OVR17, 0x1 << 15, 0x1 << 15);
	mod_radio_reg(pi, RADIO_20671_AUXPGA_CFG1_CORE1, 0x1, 0x1);

	/* Power up RX buffer */
	/* set lpf_rxbuf_pu[I,Q] */
	mod_radio_reg(pi, RADIO_20671_OVR7, 0x1 << 7, 0x1 << 7);
	mod_radio_reg(pi, RADIO_20671_OVR7, 0x1 << 6, 0x1 << 6);
	mod_radio_reg(pi, RADIO_20671_OVR23, 0x1 << 7, 0x1 << 7);
	mod_radio_reg(pi, RADIO_20671_OVR23, 0x1 << 6, 0x1 << 6);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0, 0x1 << 7, 0x1 << 7);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0, 0x1 << 6, 0x1 << 6);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1, 0x1 << 7, 0x1 << 7);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1, 0x1 << 6, 0x1 << 6);

	/* sel_rx_buffer */
	mod_radio_reg(pi, RADIO_20671_OVR7, 0x1 << 3, 0x1 << 3);
	mod_radio_reg(pi, RADIO_20671_OVR23, 0x1 << 3, 0x1 << 3);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0, 0x3 << 12, 0x2 << 12);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1, 0x3 << 12, 0x2 << 12);

	/* sel_byp_rxlpf */
	mod_radio_reg(pi, RADIO_20671_OVR7, 0x1 << 5, 0x1 << 5);
	mod_radio_reg(pi, RADIO_20671_OVR23, 0x1 << 5, 0x1 << 5);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0, 0x3 << 14, 0x1 << 14);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1, 0x3 << 14, 0x1 << 14);

	/* lpf_sel_tx_rx */
	mod_radio_reg(pi, RADIO_20671_OVR7, 0x1 << 2, 0x1 << 2);
	mod_radio_reg(pi, RADIO_20671_OVR23, 0x1 << 2, 0x1 << 2);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0, 0x3 << 10, 0x3 << 10);
	mod_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1, 0x3 << 10, 0x3 << 10);

	/* ovr_tempsense_sel_Vbe_Vbg & ovr_tempsense_swap_amp */
	mod_radio_reg(pi, RADIO_20671_OVR12, 0x3, 0x3);
	mod_radio_reg(pi, RADIO_20671_OVR28, 0x3, 0x3);
	...
	TSSIMode = (uint8) (phy_reg_read(pi, NPHY_TSSIMode) & 0xf);

	phy_reg_mod(pi, NPHY_TSSIMode, NPHY_TSSIMode_tssiEn_MASK,
		0x1 << NPHY_TSSIMode_tssiEn_SHIFT);

	TSSIMaxTxFrmDlyTime = (uint8) (phy_reg_read(pi, NPHY_TSSIMaxTxFrmDlyTime) & 0xff);
	TSSIMaxTssiDlyTime = (uint8) (phy_reg_read(pi, NPHY_TSSIMaxTssiDlyTime) & 0xff);
	phy_reg_write(pi, NPHY_TSSIMaxTxFrmDlyTime, 80);
	phy_reg_write(pi, NPHY_TSSIMaxTssiDlyTime, 64);

	/* poll for RSSI */
	for (i = 0; i <= 3; i++) {
		mod_radio_reg(pi, RADIO_20671_TEMPSENSE_CFG_CORE0, 0x3 << 1, i << 1);
		mod_radio_reg(pi, RADIO_20671_TEMPSENSE_CFG_CORE1, 0x3 << 1, i << 1);
		OSL_DELAY(100);

		PHY_CAL(("ts_cfg_c0(%d) %d ts_cfg_c1(%d) %d\n",
		i, read_radio_reg(pi, RADIO_20671_TEMPSENSE_CFG_CORE0),
		i, read_radio_reg(pi, RADIO_20671_TEMPSENSE_CFG_CORE1)));

		iq_flip = (phy_reg_read(pi, NPHY_IQFlip) & 0x1);

		if (iq_flip) {
			rssi0_i = phy_reg_read(pi, NPHY_REV19_RSSIQVal1);
		} else {
			rssi0_i = phy_reg_read(pi, NPHY_REV19_RSSIIVal1);
		}

		iq_flip = (phy_reg_read(pi, NPHY_IQFlip) & (0x1 << 4)) >> 4;

		if (iq_flip) {
			rssi1_i = phy_reg_read(pi, NPHY_REV19_RSSIQVal2);
		} else {
			rssi1_i = phy_reg_read(pi, NPHY_REV19_RSSIIVal2);
		}
	...
	write_radio_reg(pi, RADIO_20671_OVR13, save_RF_reg[0]);
	write_radio_reg(pi, RADIO_20671_OVR29, save_RF_reg[1]);
	write_radio_reg(pi, RADIO_20671_OVR33, save_RF_reg[2]);
	write_radio_reg(pi, RADIO_20671_OVR1, save_RF_reg[3]);
	write_radio_reg(pi, RADIO_20671_OVR17, save_RF_reg[4]);
	write_radio_reg(pi, RADIO_20671_OVR7, save_RF_reg[5]);
	write_radio_reg(pi, RADIO_20671_OVR23, save_RF_reg[6]);
	write_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE0, save_RF_reg[7]);
	write_radio_reg(pi, RADIO_20671_TESTBUF_CFG1_CORE1, save_RF_reg[8]);
	write_radio_reg(pi, RADIO_20671_AUXPGA_CFG1_CORE0, save_RF_reg[9]);
	write_radio_reg(pi, RADIO_20671_AUXPGA_CFG1_CORE1, save_RF_reg[10]);
	write_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE0, save_RF_reg[11]);
	write_radio_reg(pi, RADIO_20671_LPF_CFG1_CORE1, save_RF_reg[12]);
	write_radio_reg(pi, RADIO_20671_OVR12, save_RF_reg[13]);
	write_radio_reg(pi, RADIO_20671_OVR28, save_RF_reg[14]);
	...
	/* forcetxgatedClksOn */
	phy_reg_mod(pi, NPHY_fineclockgatecontrol, 0x1, 0x0);

	phy_reg_write(pi, NPHY_TSSIMode, TSSIMode);
	phy_reg_write(pi, NPHY_TSSIMaxTxFrmDlyTime, TSSIMaxTxFrmDlyTime);
	phy_reg_write(pi, NPHY_TSSIMaxTssiDlyTime, TSSIMaxTssiDlyTime);
	phy_reg_write(pi, NPHY_forceClk, forceClk_flg);
...
wlc_phy_tempsense_from_statusbyte_nphy_rev19(phy_info_t *pi)
{
	...
	tempsenseval0[0] = (phy_reg_read(pi, NPHY_REV19_Core0TxPwrCtrlStatusTemp)
	...
	tempsenseval0[1] = (phy_reg_read(pi, NPHY_REV19_Core0TxPwrCtrlStatusTemp2)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	tempsenseval0[1] = (tempsenseval0[1] > 255)? (int16)(tempsenseval0[1]-512)
				: (int16)tempsenseval0[1];
	tempsenseval0[2] = (phy_reg_read(pi, NPHY_REV19_Core0TxPwrCtrlStatusTemp3)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	tempsenseval0[2] = (tempsenseval0[2] > 255)? (int16)(tempsenseval0[2]-512)
				: (int16)tempsenseval0[2];
	tempsenseval0[3] = (phy_reg_read(pi, NPHY_REV19_Core0TxPwrCtrlStatusTemp4)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	...
	tempsenseval1[0] = (phy_reg_read(pi, NPHY_REV19_Core1TxPwrCtrlStatusTemp)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	tempsenseval1[0] = (tempsenseval1[0] > 255)? (int16)(tempsenseval1[0]-512)
				: (int16)tempsenseval1[0];
	tempsenseval1[1] = (phy_reg_read(pi, NPHY_REV19_Core1TxPwrCtrlStatusTemp2)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	tempsenseval1[1] = (tempsenseval1[1] > 255)? (int16)(tempsenseval1[1]-512)
				: (int16)tempsenseval1[1];
	tempsenseval1[2] = (phy_reg_read(pi, NPHY_REV19_Core1TxPwrCtrlStatusTemp3)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	tempsenseval1[2] = (tempsenseval1[2] > 255)? (int16)(tempsenseval1[2]-512)
				: (int16)tempsenseval1[2];
	tempsenseval1[3] = (phy_reg_read(pi, NPHY_REV19_Core1TxPwrCtrlStatusTemp4)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	...
	valid_flag[0] = ((phy_reg_read(pi, NPHY_REV19_Core0TxPwrCtrlStatusTemp)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTempValid_MASK)
				>> NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTempValid_SHIFT);
	valid_flag[1] = ((phy_reg_read(pi, NPHY_REV19_Core1TxPwrCtrlStatusTemp)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTempValid_MASK)
				>> NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTempValid_SHIFT);
...
wlc_phy_vbat_from_statusbyte_nphy_rev19(phy_info_t *pi)
{
	...
	vbat[0] = (phy_reg_read(pi, NPHY_REV19_Core0TxPwrCtrlStatusVbat)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	vbat[0] = (vbat[0] > 255)? (int16)(vbat[0]-512) : (int16)vbat[0];

	vbat[1] = (phy_reg_read(pi, NPHY_REV19_Core1TxPwrCtrlStatusVbat)
				& NPHY_REV19_CoreXTxPwrCtrlStatusTemp_avgTemp_MASK);
	...
	if ((rssi_type == NPHY_LCNXN_REV3_TSSI_OFF) || (core_code == RADIO_MIMO_CORESEL_OFF)) {
		...
		if (rssi_type == NPHY_RSSI_SEL_TBD) {
			...
			auxpga_gain = (CHSPEC_IS2G(pi->radio_chanspec))?
				(uint8)(pi->sh->TssiAuxgain2g) : (uint8)(pi->sh->TssiAuxgain5g);
			auxpga_vmid = (CHSPEC_IS2G(pi->radio_chanspec))?
				(uint16)(pi->sh->TssiVmid2g) : (uint16)(pi->sh->TssiVmid5g);
...
wlc_phy_rev3_setup_rfiqcal_mux(phy_info_t *pi, uint16 attenuator, bool override, uint8 tssi0_iqcal1)
{
	...
	pi_nphy->tx_rx_cal_radio_saveregs_rev19[0] =
		read_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE0);
	pi_nphy->tx_rx_cal_radio_saveregs_rev19[1] =
		read_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE1);
	pi_nphy->tx_rx_radio_reg_save = 1;

	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		/* Enable TSSI ENV Det PAD A-band mux */
		mod_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE0, 0xf0,
			((2 + ((tssi0_iqcal1 & 0x1) << 3) + ext_env_det) << 4));
		mod_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE1, 0xf0,
			((2 + ((tssi0_iqcal1 & 0x1) << 3) + ext_env_det) << 4));
	} else {
	...
	mod_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE0, 0x4,
		ext_env_det << 2);
	mod_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE1, 0x4,
		ext_env_det << 2);

	/* PU_iqcal_core0/1 */
	 if (tssi0_iqcal1 == 1) {
		mod_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE0, 0x2,
			1 << 1);
		mod_radio_reg(pi, RADIO_20671_IQCAL_CFG1_CORE1, 0x2,
			1 << 1);
...
wlc_phy_bphy_ofdm_noise_hw_set_nphy_rev3to6(phy_info_t *pi)
{
	...
	if (pi->interf.bphy_desense_index < 0) {
	  ...
	  if (CHSPEC_IS2G(pi->radio_chanspec)) {
	    DigiGainLimit = phy_reg_read(pi, NPHY_DigiGainLimit0);
	    PeakEnergyL = phy_reg_read(pi, NPHY_TO_BPHY_OFF+BPHY_PEAK_ENERGY_LO);
	  } else {
	    DigiGainLimit = 0;
	    PeakEnergyL = 0;
	  }
	...
	if (pi->interf.ofdm_desense_index >= 0)
		...
		crsmin_value = phy_reg_read(pi, NPHY_crsminpoweru0);
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		phy_reg_mod(pi, NPHY_DigiGainLimit0, 0xffff, DigiGainLimit);
		phy_reg_mod(pi, (NPHY_TO_BPHY_OFF+BPHY_PEAK_ENERGY_LO), 0xffff, PeakEnergyL);
	}

	phy_reg_mod(pi, NPHY_crsminpoweru0, 0xff, crsmin_value);
	phy_reg_mod(pi, NPHY_crsminpowerl0, 0xff, crsmin_value);
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		PHY_ACI(("In NPHY: {DigiGainLimit, PeakEnergyL, drop_initBiq1,"
			...
			phy_reg_read(pi, NPHY_DigiGainLimit0),
		...
		PHY_ACI(("In NPHY: {Initgain_bq1, drop_initBiq1,"
				...
				phy_reg_read(pi, NPHY_Core1InitGainCodeA2056),
				drop_initBiq1, phy_reg_read(pi, NPHY_crsminpoweru0)));
	}
}
...
wlc_phy_bphy_ofdm_noise_hw_set_nphy_rev7to15(phy_info_t *pi)
{
	...
	phy_reg_mod(pi, NPHY_crsminpoweru0, 0xff, crsmin_value_u0);
	phy_reg_mod(pi, NPHY_crsminpowerl0, 0xff, crsmin_value_l0);
...
wlc_nphy_apply_cond_chg(phy_info_t *pi, ppr_t *tx_pwr_target)
{
	ppr_plus_cmn_val(tx_pwr_target,
		(pi->sh->cond_offs1 + pi->sh->cond_offs2 +
		pi->sh->cond_offs3 + pi->sh->cond_offs4));
}
...
void wlc_nphy_update_cond_backoff_boost(phy_info_t* pi)
{
	...
	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		temp_offs1 = pi->sh->temp_offs1_5g;
		temp_offs2 = pi->sh->temp_offs2_5g;
		vbat_offs1 = pi->sh->vbat_offs1_5g;
		vbat_offs2 = pi->sh->vbat_offs2_5g;
	} else {
		temp_offs1 = pi->sh->temp_offs1_2g;
		temp_offs2 = pi->sh->temp_offs2_2g;
		vbat_offs1 = pi->sh->vbat_offs1_2g;
		vbat_offs2 = pi->sh->vbat_offs2_2g;
	}

	if ((!temp_offs1) && (!temp_offs2) && (!vbat_offs1) && (!vbat_offs2))
		return;

	vbat_temp_fsm = (phy_reg_read(pi, NPHY_REV19_TxPwrCtrlNew1FeatureEnables) &
	...
	temp_offs1, temp_offs2, &(pi->sh->cond_offs1), &(pi->sh->cond_offs2),
	pi->sh->high_temp_threshold, pi->sh->low_temp_threshold, pi->sh->temp_diff);

	wlc_nphy_update_temp_vbat_backoff_boost(pi, vbat,
	vbat_offs1, vbat_offs2, &(pi->sh->cond_offs3), &(pi->sh->cond_offs4),
	pi->sh->high_vbat_threshold, pi->sh->low_vbat_threshold, pi->sh->vbat_diff);
}
...
static void wlc_nphy_txgainindex_cap_update(phy_info_t *pi, uint16 txidx_cap)
{
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		min_txidx = pi->sh->min_txpwrindex_2g;
	        pi->sh->min_txpwrindex_2g = txidx_cap;
	} else {
		min_txidx = pi->sh->min_txpwrindex_5g;
	        pi->sh->min_txpwrindex_5g = txidx_cap;
	}
...
void wlc_nphy_txgainindex_cap_adjust(phy_info_t *pi)
{
	...
	if ((pi->sh->txidxcap_high == NPHY_4324EPA_TXIDXCAP_INVALID) &&
		(pi->sh->txidxcap_low == NPHY_4324EPA_TXIDXCAP_INVALID))
		return;

	if (CHSPEC_IS2G(pi->radio_chanspec))
		nom_txidxcap = pi->sh->nom_txidxcap_2g;
	else
		nom_txidxcap = pi->sh->nom_txidxcap_5g;
	...
	if (pi->sh->txidxcap_high != NPHY_4324EPA_TXIDXCAP_INVALID) {
		if (pi_nphy->txidxcap_hi_inuse) {
			...
			if (temper < (NPHY_4324EPA_TEMPER_THRES_HI - NPHY_4324EPA_TEMPER_DIFF)) {
		        ...
		        wlc_nphy_txgainindex_cap_update(pi, (uint16)pi->sh->txidxcap_high);
		}
	}

	if (pi->sh->txidxcap_low != NPHY_4324EPA_TXIDXCAP_INVALID) {
		if (pi_nphy->txidxcap_lo_inuse) {
			...
			wlc_nphy_txgainindex_cap_update(pi, (uint16)pi->sh->txidxcap_low);
		}
	}
}
#endif /* CODE_OPT_4324 */
	...
	if (enable) {
		...
		FOREACH_CORE(pi, core) {
			...
			miscreg  = (core == 0)?  NPHY_REV19_RfctrlMiscReg9:
				NPHY_REV19_RfctrlMiscReg10;
			phy_reg_write(pi, rxgain_addr, rfmxgain);
			phy_reg_write(pi, lpfgain_addr, lpfgain);
			phy_reg_mod(pi, override_reg,
...
int wlc_nphy_tssi_read_iovar(phy_info_t *pi)
{
	...
	tssi[0] = phy_reg_read(pi, NPHY_TSSIVal1);
	tssi[1] = phy_reg_read(pi, NPHY_TSSIVal2);
...
wlc_phy_aci_noise_print_values_nphy(phy_info_t *pi)
{
	...
	if (NREV_LT(pi->pubpi.phy_rev, LCNXN_BASEREV + 4)) {
		...
		printf("0x=%x\n0x=%x\n",
			(uint16) (phy_reg_read(pi, NPHY_crsminpowerl0)),
			(uint16) (phy_reg_read(pi, NPHY_crsminpoweru0)));
		...
		printf("0x=%x\n0x=%x\n",
			read_radio_reg(pi, RADIO_2057_W2_MASTER_CORE0),
			read_radio_reg(pi, RADIO_2057_W2_MASTER_CORE1));
		printf("PRINT END *************************************************\n");
		...
		printf("hw states... CRSMINPWRs %x %x | C0: GAINCodeAB %x %x RFSeq0x106 %x |"
			...
			phy_reg_read(pi, NPHY_crsminpoweru0_core1) & 0xff,
			phy_reg_read(pi, NPHY_Core1InitGainCodeA2057),
			phy_reg_read(pi, NPHY_Core1InitGainCodeB2057),
			temp[0],
			(read_radio_reg(pi, RADIO_20671_OVR3) >> 6) & 0x1,
			(read_radio_reg(pi, RADIO_20671_OVR19) >> 6) & 0x1,
			(read_radio_reg(pi, RADIO_20671_LNA2G_CFG1_CORE0) >> 8) & 0xf,
			(read_radio_reg(pi, RADIO_20671_LNA2G_CFG1_CORE1) >> 8) & 0xf,
			(read_radio_reg(pi, RADIO_20671_OVR3) >> 2) & 0x1,
			(read_radio_reg(pi, RADIO_20671_OVR19) >> 2) & 0x1,
			(read_radio_reg(pi, RADIO_20671_LNA2G_CFG2_CORE0) >> 4) & 0xf,
			(read_radio_reg(pi, RADIO_20671_LNA2G_CFG2_CORE1) >> 4) & 0xf);
	}
...
wlc_phy_rxfe_ctrl_nphy(phy_info_t *pi)
{
	...
	phy_reg_mod(pi, NPHY_REV19_rxfeFifoCtrl,
	...
	phy_reg_mod(pi, NPHY_REV19_RxFeCtrl1,
...
wlc_phy_cal_dump_nphy(phy_info_t *pi, struct bcmstrbuf *b)
{
	...
	if (NREV_GE(pi->pubpi.phy_rev, 3)) {
		if (NREV_GE(pi->pubpi.phy_rev, LCNXN_BASEREV+3)) {
			...
			rcal_value = read_radio_reg(pi, RADIO_20671_BG_CFG1_CORE0);
	...
	crs_min_pwr[0] = (phy_reg_read(pi, NPHY_crsminpowerl0) & 0xff);

	if (NREV_GE(pi->pubpi.phy_rev, LCNXN_BASEREV + 4))
		crs_min_pwr[1] = (phy_reg_read(pi, NPHY_crsminpowerl0_core1) & 0xff);

	listen_gain_A[0] = phy_reg_read(pi, NPHY_Core1InitGainCodeA2057);
	listen_gain_B[0] = phy_reg_read(pi, NPHY_Core1InitGainCodeB2057);
	listen_gain_A[1] = phy_reg_read(pi, NPHY_Core2InitGainCodeA2057);
	listen_gain_B[1] = phy_reg_read(pi, NPHY_Core2InitGainCodeB2057);
...
wlc_phy_get_paparams_lo_nphy(phy_info_t *pi, int16 *a1, int16 *b0, int16 *b1)
{
	...
	if (curr_chan <= 14) {
		a1[0] = pi->sh->pa2gw0a0_lo;
		b0[0] = pi->sh->pa2gw1a0_lo;
		b1[0] = pi->sh->pa2gw2a0_lo;
		a1[1] = pi->sh->pa2gw0a1_lo;
		b0[1] = pi->sh->pa2gw1a1_lo;
		b1[1] = pi->sh->pa2gw2a1_lo;
	} else if (curr_chan <= 64) {
		a1[0] = pi->sh->pa5glw0a0_lo;
		b0[0] = pi->sh->pa5glw1a0_lo;
		b1[0] = pi->sh->pa5glw2a0_lo;
		a1[1] = pi->sh->pa5glw0a1_lo;
		b0[1] = pi->sh->pa5glw1a1_lo;
		b1[1] = pi->sh->pa5glw2a1_lo;
	} else if (curr_chan <= 140) {
		a1[0] = pi->sh->pa5gw0a0_lo;
		b0[0] = pi->sh->pa5gw1a0_lo;
		b1[0] = pi->sh->pa5gw2a0_lo;
		a1[1] = pi->sh->pa5gw0a1_lo;
		b0[1] = pi->sh->pa5gw1a1_lo;
		b1[1] = pi->sh->pa5gw2a1_lo;
	} else if (curr_chan <= 165) {
		a1[0] = pi->sh->pa5ghw0a0_lo;
		b0[0] = pi->sh->pa5ghw1a0_lo;
		b1[0] = pi->sh->pa5ghw2a0_lo;
		a1[1] = pi->sh->pa5ghw0a1_lo;
		b0[1] = pi->sh->pa5ghw1a1_lo;
		b1[1] = pi->sh->pa5ghw2a1_lo;
	} else {
	...
	uint16* clip_stats)
{
	...
	/* save lna1 rout ovr */
	lna1_rout[0] = read_radio_reg(pi, RADIO_20671_OVR3);
	lna1_rout[1] = read_radio_reg(pi, RADIO_20671_OVR19);

	/* remove lna1 rout ovr */
	mod_radio_reg(pi, RADIO_20671_OVR3,
		0x1 << 6, 0x0 << 6);
	mod_radio_reg(pi, RADIO_20671_OVR19,
		0x1 << 6, 0x0 << 6);
	mod_radio_reg(pi, RADIO_20671_OVR3,
		0x1 << 2, 0x0 << 2);
	mod_radio_reg(pi, RADIO_20671_OVR19,
	...
	forceClk = phy_reg_read(pi, NPHY_forceClk);

	/* modify & program */
	temp1 = (temp | 0x200000);
	wlc_si_pmu_chipcontrol_access(pi, 4, &temp1, 1);
	phy_reg_write(pi, NPHY_forceClk, 0x3f);

	/* NumSampToCol */
	phy_reg_write(pi, NPHY_ClipIqestSampleCount, num_samps);
	/* waitTime */
	phy_reg_mod(pi, NPHY_ClipIqestWaitTime, 0xff, wait_time);
	/* iqMode */
	phy_reg_mod(pi, NPHY_ClipStatAccCmd, 0x1 << 1, clip_stat_mode << 1);
	/* nbsel */
	phy_reg_mod(pi, NPHY_ClipStatAcc_muxsel, 0x3, mux_idx);
	/* w1sel */
	phy_reg_mod(pi, NPHY_ClipStatAcc_muxsel, 0x3 << 2, mux_idx << 2);
	/* w3sel */
	phy_reg_mod(pi, NPHY_ClipStatAcc_muxsel, 0x3 << 4, mux_idx << 4);
	/* iqstart */
	phy_reg_mod(pi, NPHY_ClipStatAccCmd, 0x1, 1);

	not_done = (phy_reg_read(pi, NPHY_ClipStatAccCmd) & 0x1);
	...
	if (!not_done) {
		nb_clip_cntI0 = phy_reg_read(pi, NPHY_ClipIqestNbI0);
		nb_clip_cntQ0 = phy_reg_read(pi, NPHY_ClipIqestNbQ0);
		nb_clip_cntI1 = phy_reg_read(pi, NPHY_ClipIqestNbI1);
		nb_clip_cntQ1 = phy_reg_read(pi, NPHY_ClipIqestNbQ1);
		w3_clip_cntI0 = phy_reg_read(pi, NPHY_ClipIqestW3I0);
		w3_clip_cntQ0 = phy_reg_read(pi, NPHY_ClipIqestW3Q0);
		w3_clip_cntI1 = phy_reg_read(pi, NPHY_ClipIqestW3I1);
		w3_clip_cntQ1 = phy_reg_read(pi, NPHY_ClipIqestW3Q1);
	...
	phy_reg_write(pi, NPHY_forceClk, forceClk);
	wlc_si_pmu_chipcontrol_access(pi, 4, &temp, 1);

	/* restore lna1 rout ovr */
	write_radio_reg(pi, RADIO_20671_OVR3, lna1_rout[0]);
	write_radio_reg(pi, RADIO_20671_OVR19, lna1_rout[1]);
}
...
wlc_phy_dynamic_rflo_ucode_war_nphy(phy_info_t *pi, uint8 tap_point)
{
	...
	case 1:
		...
		wlapi_bmac_write_shm(pi->sh->physhim,
			...
			read_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC1_CORE0));
		wlapi_bmac_write_shm(pi->sh->physhim,
			(shm_ptr + M_TX_MODE_0x14d) * 2,
			read_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC1_CORE1));
		wlapi_bmac_write_shm(pi->sh->physhim,
			(shm_ptr + M_TX_MODE_0xb1) * 2,
			read_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC2_CORE0));
		wlapi_bmac_write_shm(pi->sh->physhim,
			(shm_ptr + M_TX_MODE_0x14e) * 2,
			read_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC2_CORE1));
		/* For handling drv issues, taking sw copy */
		pi_nphy->TX_logen5g_idac1_core0 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC1_CORE0);
		pi_nphy->TX_logen5g_idac1_core1 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC1_CORE1);
		pi_nphy->TX_logen5g_idac2_core0 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC2_CORE0);
		pi_nphy->TX_logen5g_idac2_core1 = read_radio_reg(pi,
	...
	case 2:
		...
		wlapi_bmac_write_shm(pi->sh->physhim,
			...
			read_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE0));
		wlapi_bmac_write_shm(pi->sh->physhim,
			(shm_ptr + M_TX_MODE_0x151) * 2,
			read_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE1));
		/* For handling drv issues, taking sw copy */
		pi_nphy->TX_logen5g_tune_core0 = read_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE0);
		pi_nphy->TX_logen5g_tune_core1 = read_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE1);
		break;
	case 3:
		...
		wlapi_bmac_write_shm(pi->sh->physhim,
			...
			read_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE1));
		/* For handling drv issues, taking sw copy */
		pi_nphy->RX_logen5g_idac1_core0 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC1_CORE0);
		pi_nphy->RX_logen5g_idac1_core1 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC1_CORE1);
		pi_nphy->RX_logen5g_idac2_core0 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC2_CORE0);
		pi_nphy->RX_logen5g_idac2_core1 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_IDAC2_CORE1);
		pi_nphy->RX_logen5g_tune_core0 = read_radio_reg(pi,
			RADIO_20671_LOGEN5G_TUNE_CORE0);
		pi_nphy->RX_logen5g_tune_core1 = read_radio_reg(pi,
	...
	case 4:
		...
		write_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC1_CORE0,
			pi_nphy->TX_logen5g_idac1_core0);
		write_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC1_CORE1,
			pi_nphy->TX_logen5g_idac1_core1);
		write_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC2_CORE0,
			pi_nphy->TX_logen5g_idac2_core0);
		write_radio_reg(pi, RADIO_20671_LOGEN5G_IDAC2_CORE1,
			pi_nphy->TX_logen5g_idac2_core1);
		write_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE0,
			pi_nphy->TX_logen5g_tune_core0);
		write_radio_reg(pi, RADIO_20671_LOGEN5G_TUNE_CORE1,
...
wlc_phy_cck_acpr_war_nphy(phy_info_t *pi)
{
	...
	mod_radio_reg(pi, RADIO_20671_OVR2, 0x1 << 9, 0x1 << 9);
}
...
wlc_phy_lq_based_crsminpwr_limiting_nphy(phy_info_t *pi, uint16 *crsminpwr)
{
	...
	int8 Koffset = CHSPEC_IS2G(pi->radio_chanspec) ? pi->sh->noisecaloffset:
		pi->sh->noisecaloffset5g;
	...
	curr_gain_biq0 = ((phy_reg_read(pi, NPHY_Core1InitGainCodeB2057) >>
		NPHY_Core1InitGainCodeB2057_InitBiQ0Index_SHIFT) & 0xf);
...
void
wlc_phy_rf_swctrl_set(phy_info_t *pi, uint32 offset, uint32 value)
{
	uint8 Nmap = 0;
	uint32 mask, shifted;
	uint8 ct2 = 0;
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		*/
		...
		uint8 antswctrllut_vals0[6] = {0, 0, 0, 0, 0, 0};

		uint8 map1[] = {1, 2, 3, 17, 18, 19};
		uint8 antswctrllut_vals1[6] = {0, 0, 0, 0, 0, 0};

		/* 2G core 0 */
		Nmap = (sizeof(map0) / sizeof(map0[0]));
		for (ct2 = 0; ct2 < Nmap; ct2++) {
			offset = map0[ct2];
			wlc_phy_table_read_nphy(pi, NPHY_TBL_ID_ANTSWCTRLLUT,
				1, offset, 8,
		...
		if (core == 0) {
			Nmap = (sizeof(map0) / sizeof(map0[0]));
				for (ct2 = 0; ct2 < Nmap; ct2++) {
				offset = map0[ct2];
				val = ((antswctrllut_vals0[ct2] &~mask));
				wval = val | (shifted & mask);
				wlc_phy_table_write_nphy(pi, NPHY_TBL_ID_ANTSWCTRLLUT,
					1, offset, 8, &wval);
			}
			...
			Nmap = (sizeof(map1) / sizeof(map1[0]));
				for (ct2 = 0; ct2 < Nmap; ct2++) {
				offset = 32 + map1[ct2];
				val = ((antswctrllut_vals1[ct2] &~mask));
				wval = val | (shifted & mask);
				wlc_phy_table_write_nphy(pi, NPHY_TBL_ID_ANTSWCTRLLUT,
					1, offset, 8, &wval);
			}
		...
		*/
		...
		uint8 antswctrllut_vals0[4] = {0, 0, 0, 0};

		uint8 map1[] = {1, 2, 17, 18};
		uint8 antswctrllut_vals1[4] = {0, 0, 0, 0};
...
void
wlc_phy_rf_swctrl_reset(phy_info_t *pi)
{
	uint8 Nmap = 0;
	uint32 offset;
	uint8 ct2 = 0;
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		*/
		...
		phy_reg_write(pi, NPHY_REV19_swCtrlLUTConfig, 0x1a);
		...
		*/
		phy_reg_mod(pi, NPHY_REV19_Override_TR_rx_pu_high_gain,
