 ...
 *
 * $Id: siutils.h 590777 2015-10-06 02:51:54Z $
 */
...
struct si_pub {
	...
	bool bSeciOnSprom;	/* =1 selects SPROM_DIN/DOUT as SECI pins else GPIO_0/1 */
};
...
extern void si_eci_notify_wl_ant_info_to_bt(si_t *sih, uint32 val);
#else
#define si_eci(sih) 0
...
#define GCI_CORECTL_SECICLKSTR			25 /* SeciClkStretch */
#define GCI_CORECTL_SECICLKSTR_SHIFT		24 /* SeciClkStretch (bits 24 to 31) */
...
#ifdef SSWAR
extern uint8 si_enable_host_L12exit_war(si_t *sih, uint8 *wake_mask, uint8 *cur_status);
#endif // endif
