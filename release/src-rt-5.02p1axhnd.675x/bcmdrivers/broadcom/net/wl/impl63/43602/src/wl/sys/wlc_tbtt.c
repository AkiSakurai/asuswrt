...
/* ==== please keep these debug stuff at the bottom ==== */
	...
	/* stats */
	uint collision;			/* schedule conflict counter */
	uint pretbtt;			/* pretbtt interrupt counter */
	uint pulate;			/* pretbtt later than PU range */
	uint puearly;			/* pretbtt earlier than PU */
	uint tbtt;			/* tbtt interrupt counter */
	uint pulatemin;
	uint puearlymax;
	uint pretbttlatemax;
	uint pretbttlate;		/* pretbtt is late but still before the toss point */
	uint pretbttmiss;		/* pretbtt is so late and it is tossed */
	uint pretbttmissmax;
#endif // endif
} wlc_tbtt_ent_t;
...
#define PULATECNTINC(te)	((te)->pulate++)
#define PUEARLYCNTINC(te)	((te)->puearly++)
#define TBTTCNTINC(te)		((te)->tbtt++)
#define PRETBTTLCNTINC(te)	((te)->pretbttlate++)
#define PRETBTTMCNTINC(te)	((te)->pretbttmiss++)
#else
#define COLCNTINC(te)
#define PRETBTTCNTINC(te)
#define PULATECNTINC(te)
#define PUEARLYCNTINC(te)
#define TBTTCNTINC(te)
#define PRETBTTLCNTINC(te)
#define PRETBTTMCNTINC(te)
...
wlc_tbtt_ent_init(wlc_tbtt_info_t *ti, wlc_bsscfg_t *cfg)
{
	...
	/* PowerUp time before TBTT (preTBTT) */
	te->PU_time = wlc_pretbtt_calc(cfg);
	te->PU_time = ((te->PU_time + 31) >> 5) << 5;

	/* PowerUp margin (normal preTBTT interrupt arrival time after expected preTBTT time) */
	te->PU_tolerance = te->PU_time * 20 / 100;
	te->PU_tolerance = ((te->PU_tolerance + 31) >> 5) << 5;

	/* LatePreTBTT margin (abnormal preTBTT interrupt arrival time after expected TBTT time) */
	te->LP_tolerance = 3000;

#if defined(BCMDBG) || defined(BCMDBG_DUMP)
	te->pulatemin = te->PU_time - te->PU_tolerance;
	te->puearlymax = 0;
	te->pretbttlatemax = 0;
	te->pretbttmissmax = 0;
#endif // endif
...
wlc_tbtt_ent_dump(void *ctx, wlc_bsscfg_t *cfg, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\tcollision %u tbtt %u\n", te->collision, te->tbtt);
	bcm_bprintf(b, "\tpretbtt %u (pulate %u puearly %u) pretbtt late %u pretbtt miss %u\n",
	            te->pretbtt, te->pulate, te->puearly, te->pretbttlate, te->pretbttmiss);
	bcm_bprintf(b, "\tpulatemin %u puearlymax %u pretbttlatemax %u pretbttmissmax %u\n",
	            te->pulatemin, te->puearlymax, te->pretbttlatemax, te->pretbttmissmax);
	bcm_bprintf(b, "\tPU center %u PU margin %u LP margin %u\n",
	            te->PU_time, te->PU_tolerance, te->LP_tolerance);
}
#endif /* BCMDBG || BCMDBG_DUMP */
...
wlc_tbtt_pre_tbtt_cb(void *ctx, wlc_mcnx_intr_data_t *notif_data)
{
	 ...
	 * than it is here now so the calculation off tbtt may be off a bit...
	 */
	...
	/* XXX for an unknown reason the pretbtt comes in variable pace
	 ...
	 *                    ^     |      |     ^ |    ^  |
	 *                    |     |      |     | |    |  |
	 *                puearlymax|      |pulatemin pretbttlatemax
	 *                          |      |       |       |
	 *                          |<--4->|       |<--5-->|
	 * p. pretbtt
	 * t. tbtt
	...
	/* after PU margin... */
	if (tt_next_tbtt > 0 &&
	    tt_next_tbtt <= te->PU_time - te->PU_tolerance) {
		PULATECNTINC(te);
#if defined(BCMDBG) || defined(BCMDBG_DUMP)
		if (tt_next_tbtt < te->pulatemin)
			te->pulatemin = tt_next_tbtt;
#endif // endif
	}
	/* before pretbtt... */
	else if (tt_next_tbtt > te->PU_time &&
	         tt_next_tbtt <= te->PU_time + te->PU_tolerance) {
		PUEARLYCNTINC(te);
#if defined(BCMDBG) || defined(BCMDBG_DUMP)
		if (tt_next_tbtt > te->puearlymax)
			te->puearlymax = tt_next_tbtt;
#endif // endif
	}
	...
	/* fake a pretbtt/tbtt... */
	if (tf_last_tbtt < te->LP_tolerance) {
		wlc_tbtt_ent_data_t notif_data1;
		...
		PRETBTTLCNTINC(te);
#if defined(BCMDBG) || defined(BCMDBG_DUMP)
		if (tf_last_tbtt > te->pretbttlatemax)
			te->pretbttlatemax = tf_last_tbtt;
#endif // endif
		return;
	}
	...
	PRETBTTMCNTINC(te);
#if defined(BCMDBG) || defined(BCMDBG_DUMP)
	if (tf_last_tbtt > te->pretbttmissmax)
		te->pretbttmissmax = tf_last_tbtt;
#endif // endif
#ifdef BCMDBG
{
	wlc_mcnx_next_l_tbtt64(mcnx, cfg, tsf_h, tsf_l, &tbtt_h, &tbtt_l);
