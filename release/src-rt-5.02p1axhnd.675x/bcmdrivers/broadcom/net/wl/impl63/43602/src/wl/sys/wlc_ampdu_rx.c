 ...
 *
 * $Id: wlc_ampdu_rx.c 769206 2018-11-06 10:38:14Z $
 */
...
static const bcm_iovar_t ampdu_iovars[] = {
	...
	{"ampdu_rxaggr", IOV_AMPDU_RXAGGR, (0), IOVT_BUFFER, sizeof(struct ampdu_aggr)},
	{NULL, 0, 0, 0, 0}
};
...
#error "PCIe FD AMPDU hostreorder & forwardreorder features can't coexist at present"
#endif // endif
...
#define AMPDU_HOST_HASPKT(resp, index, osh, p)	\
	{		\
		(resp)->host_pkt_pending[index] = TRUE;		\
...
#define AMPDU_CHAIN_RXCPLID_FLUSH(a)	({	\
	wl_flush_rxreorderqeue_flow(a);	\
	AMPDU_CHAIN_RXCPLID_RESET(a);	\
})
#else /* BCMPKTIDMAP */
...
#define AMPDU_CHAIN_RXCPLID_FLUSH(a) do {} while (0)
#endif /* BCMPKTIDMAP */
#else /* WLAMPDU_HOSTREORDER */
#define AMPDU_CHECK_HOST_HASPKT(resp, index)	FALSE
#define AMPDU_HOST_HASPKT(resp, index, osh, p)		do {} while (0)
...
#define AMPDU_PKT_PENDING(wlc, resp, index)	\
	(AMPDU_RXQ_HASPKT(resp, index) || \
	 (AMPDU_HOST_REORDER_ENAB(wlc) && AMPDU_CHECK_HOST_HASPKT(resp, index)))
...
struct ampdu_rx_info {
	...
	int8 rxaggr_support;	/* Support ampdu rx aggregation */
...
static void wlc_ampdu_rx_cleanup(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
	ampdu_rx_info_t *ampdu_rx, uint16 conf_TID_bmap);
...
struct bsscfg_ampdu_rx {
	...
	int8 rx_amsdu_in_ampdu_ovrd;	/* rx amsdu in ampdu override */
};

struct ampdu_rx_bsscfg_cubby {
	bsscfg_ampdu_rx_t *bsscfg_rx_cubby;
};

#define BSSCFG_AMPDU_RX_INFO(ampdu_rx, bsscfg) (BSSCFG_CUBBY((bsscfg), (ampdu_rx)->bsscfg_handle))
#define BSSCFG_AMPDU_RX_CUBBY(ampdu_rx, bsscfg) \
	(((struct ampdu_rx_bsscfg_cubby *)BSSCFG_AMPDU_RX_INFO(ampdu_rx, bsscfg))->bsscfg_rx_cubby)
...
wlc_ampdu_release_ordered(wlc_info_t *wlc, scb_ampdu_rx_t *scb_ampdu, uint8 tid)
{
		...
		AMPDU_PKT_PENDING(wlc, resp, indx); indx = NEXT_RX_INDEX(indx))
	{
		...
		if (!AMPDU_HOST_REORDER_ENAB(wlc)) {
...
#if defined(BCMPKTIDMAP)
		ampdu_create_f(wlc, scb, &f, p, WLPKTTAG(p)->u.wrxh);
#else  /* ! BCMPKTIDMAP */
...
wlc_ampdu_release_all_ordered(wlc_info_t *wlc, scb_ampdu_rx_t *scb_ampdu, uint8 tid)
{
	...
	if (AMPDU_HOST_REORDER_ENAB(wlc)) {
		wlc_ampdu_release_n_ordered(wlc, scb_ampdu, tid,
			scb_ampdu->ampdu_rx->ba_max_rx_wsize);
		if (BCMPCIEDEV_ENAB(wlc->pub))
			AMPDU_CHAIN_RXCPLID_FLUSH(&resp->rxcpl_list);
	} else {
...
/* set optional wsize override from bmac dongle */
	...
	if (wlc->pub->ampdu_ba_rx_wsize)
		ampdu_rx->ba_rx_wsize = wlc->pub->ampdu_ba_rx_wsize;
#endif /* WLC_HIGH_ONLY */
	...
	if (D11REV_IS(wlc->pub->corerev, 42)) {
		ampdu_rx->mpdu_density = AMPDU_DENSITY_16_US;
	}
#endif // endif
	...
	if (VHT_ENAB(wlc->pub)) {
		...
		ampdu_rx->rx_factor = VHT_DEFAULT_RX_FACTOR;
	} else
#endif /* WL11AC */
	{
	...
	if ((ampdu_rx->bsscfg_handle = wlc_bsscfg_cubby_reserve(wlc,
		sizeof(struct ampdu_rx_bsscfg_cubby), bsscfg_ampdu_rx_init, bsscfg_ampdu_rx_deinit,
	...
	ampdu_rx->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct ampdu_rx_cubby),
		scb_ampdu_rx_init, scb_ampdu_rx_deinit, NULL,
		(void *)ampdu_rx, SCB_CUBBY_ID_AMPDU_RX);
#else
	ampdu_rx->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct ampdu_rx_cubby),
...
#ifdef BCM_HOST_MEM_SCB
		if (SCB_ALLOC_ENAB(ampdu_rx->wlc->pub) && SCB_HOST(scb)) {
			scb_ampdu = (scb_ampdu_rx_t *)wlc_scb_alloc_mem_get(ampdu_rx->wlc,
				SCB_CUBBY_ID_AMPDU_RX, sizeof(scb_ampdu_rx_t), 1);
		}
...
#ifdef BCM_HOST_MEM_SCB
	if (SCB_ALLOC_ENAB(ampdu_rx->wlc->pub) && SCB_HOST(scb)) {
		wlc_scb_alloc_mem_free(ampdu_rx->wlc, SCB_CUBBY_ID_AMPDU_RX, (void *)scb_ampdu);
	}
	else
#endif // endif
...
{
	ampdu_rx_info_t *ampdu_rx = (ampdu_rx_info_t *)context;
	struct ampdu_rx_bsscfg_cubby *cubby_info =
		(struct ampdu_rx_bsscfg_cubby *)BSSCFG_AMPDU_RX_INFO(ampdu_rx, bsscfg);
	bsscfg_ampdu_rx_t *bsscfg_ampdu_rx;

	ASSERT(bsscfg != NULL);
	bsscfg_ampdu_rx = MALLOC(ampdu_rx->wlc->osh, sizeof(bsscfg_ampdu_rx_t));
	if (!bsscfg_ampdu_rx)
		return BCME_NOMEM;
	bzero(bsscfg_ampdu_rx, sizeof(bsscfg_ampdu_rx_t));
	cubby_info->bsscfg_rx_cubby = bsscfg_ampdu_rx;

	bsscfg_ampdu_rx->ampdu_rx = ampdu_rx;
	bsscfg_ampdu_rx->bsscfg = bsscfg;
	if (ampdu_rx->rxaggr_support) {
		/* Enable for all TID by default */
		bsscfg_ampdu_rx->rxaggr_override = AUTO;
		bsscfg_ampdu_rx->rxaggr_TID_bmap = NBITMASK(AMPDU_MAX_SCB_TID);
	} else {
		/* AMPDU RX module does not allow rx aggregation */
		bsscfg_ampdu_rx->rxaggr_override = OFF;
		bsscfg_ampdu_rx->rxaggr_TID_bmap = 0;
	}
	bsscfg_ampdu_rx->rx_amsdu_in_ampdu_ovrd = AUTO;
...
bsscfg_ampdu_rx_deinit(void *context, wlc_bsscfg_t *bsscfg)
{
	...
	struct ampdu_rx_bsscfg_cubby *cubby_info =
		(struct ampdu_rx_bsscfg_cubby *)BSSCFG_AMPDU_RX_INFO(ampdu_rx, bsscfg);
	bsscfg_ampdu_rx_t *bsscfg_ampdu_rx = NULL;
	...
	if (cubby_info)
		bsscfg_ampdu_rx = cubby_info->bsscfg_rx_cubby;
	if (!bsscfg_ampdu_rx)
		return;

	MFREE(ampdu_rx->wlc->osh, bsscfg_ampdu_rx, sizeof(bsscfg_ampdu_rx_t));
	cubby_info->bsscfg_rx_cubby = NULL;
}
...
/** Send DELBA and flush all ampdu rx queues */
void
wlc_ampdu_rx_cleanup_all(ampdu_rx_info_t *ampdu_rx)
{
	int idx;
	wlc_bsscfg_t *cfg;
	...
	FOREACH_AS_BSS(wlc, idx, cfg) {
		wlc_ampdu_rx_cleanup(wlc, cfg, ampdu_rx, AMPDU_ALL_TID_BITMAP);
	}
}
	...
	scb_ampdu_rx_t *scb_ampdu, uint8 tid)
{
	...
	if (AMPDU_HOST_REORDER_ENAB(wlc)) {
		if (BCMPCIEDEV_ENAB(wlc->pub)) {
			AMPDU_CHAIN_RXCPLID_FLUSH(rx_list);
		}
		else if (p) {
...
static void
wlc_ampdu_rx_cleanup(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
	ampdu_rx_info_t *ampdu_rx, uint16 conf_TID_bmap)
{
	uint8 tid;
	scb_ampdu_rx_t *scb_ampdu = NULL;
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_AMPDU_RX_FACTOR):
		...
		if (VHT_ENAB(wlc->pub)) {
			...
			wlc_vht_update_ampdu_cap(wlc->vhti, ampdu_rx->rx_factor);
			wlc_ampdu_update_ie_param(ampdu_rx);
			break;
		}
#endif /* WL11AC */
	...
	case IOV_GVAL(IOV_AMPDU_RXAGGR):
	{
		...
		bsscfg_ampdu_rx_t *bsscfg_ampdu_rx;
		if (bsscfg != NULL &&
			(bsscfg_ampdu_rx = BSSCFG_AMPDU_RX_CUBBY(ampdu_rx, bsscfg)) != NULL) {
			struct ampdu_aggr *rxaggr = p;

			bzero(rxaggr, sizeof(*rxaggr));
			rxaggr->aggr_override = bsscfg_ampdu_rx->rxaggr_override;
			rxaggr->enab_TID_bmap = bsscfg_ampdu_rx->rxaggr_TID_bmap;
	...
	case IOV_SVAL(IOV_AMPDU_RXAGGR):
	{
		...
		if (bsscfg != NULL) {
			...
			uint16 enable_TID_bmap = (rxaggr->enab_TID_bmap & rxaggr->conf_TID_bmap)
				& AMPDU_ALL_TID_BITMAP;
			uint16 disable_TID_bmap = ((~rxaggr->enab_TID_bmap) & rxaggr->conf_TID_bmap)
				& AMPDU_ALL_TID_BITMAP;
			if (enable_TID_bmap)
				wlc_ampdu_agg_state_update_rx(wlc, bsscfg, ON, enable_TID_bmap);
			if (disable_TID_bmap)
				wlc_ampdu_agg_state_update_rx(wlc, bsscfg, OFF, disable_TID_bmap);
		} else {
			err = BCME_NOTFOUND;
		}
		break;
...
void
BCMATTACHFN(wlc_ampdu_agg_rxaggr_support)(ampdu_rx_info_t *ampdu_rx, int8 rxaggr)
{
	ampdu_rx->rxaggr_support = rxaggr;
}

/** called on association and for BT coexistence as well */

/* override appliled for all TIDs */
void
wlc_ampdu_agg_state_update_rx_override(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int8 rxaggr)
{
	ampdu_rx_info_t *ampdu_rx = wlc->ampdu_rx;
	bsscfg_ampdu_rx_t *bsscfg_ampdu_rx = BSSCFG_AMPDU_RX_CUBBY(ampdu_rx, bsscfg);
	...
	if (bsscfg_ampdu_rx->rxaggr_override == rxaggr)
		return;

	bsscfg_ampdu_rx->rxaggr_override = rxaggr;

	if (rxaggr == OFF)
		wlc_ampdu_rx_cleanup(wlc, bsscfg, ampdu_rx, AMPDU_ALL_TID_BITMAP);
}

void
wlc_ampdu_agg_state_update_rx(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
	bool rxaggr, uint16 conf_TID_bmap)
{
	...
	if (rxaggr == ON)
		bsscfg_ampdu_rx->rxaggr_TID_bmap |= (conf_TID_bmap & AMPDU_ALL_TID_BITMAP);
	else {
		uint16 stateChangedTID = bsscfg_ampdu_rx->rxaggr_TID_bmap;
		bsscfg_ampdu_rx->rxaggr_TID_bmap &= ((~conf_TID_bmap) & AMPDU_ALL_TID_BITMAP);
		stateChangedTID ^= bsscfg_ampdu_rx->rxaggr_TID_bmap;
		stateChangedTID &= AMPDU_ALL_TID_BITMAP;

		/* Override should have higher priority if not AUTO */
		if (bsscfg_ampdu_rx->rxaggr_override == AUTO && stateChangedTID)
			wlc_ampdu_rx_cleanup(wlc, bsscfg, ampdu_rx, stateChangedTID);
	}
}

#ifdef WLAMSDU
/** called by another module who temporarily wants to configure rx_amsdu_in_ampdu */
void
wlc_ampdu_set_rx_amsdu_override(wlc_info_t *wlc,
	wlc_bsscfg_t *bsscfg, bool ovrd)
{
	ampdu_rx_info_t *ampdu_rx = wlc->ampdu_rx;
	bsscfg_ampdu_rx_t *bsscfg_ampdu_rx = BSSCFG_AMPDU_RX_CUBBY(ampdu_rx, bsscfg);

	/* ovrd ON would mean that rx_amsdu_in_ampdu will be allowed
	 * even if wlc->_rx_amsdu_in_ampdu is OFF.
	 * As there is no requirement of such case, just treat ON as AUTO.
	 */
	if (ovrd == ON) {
		ovrd = AUTO;
	}
	...
	if (wlc->_rx_amsdu_in_ampdu &&
		bsscfg_ampdu_rx->rx_amsdu_in_ampdu_ovrd != ovrd) {
		/* rx_amsdu_in_ampdu override configuration has been changed.
		 * So send DELBA to do ADDBA handshake once again.
		 */
		wlc_ampdu_rx_cleanup(wlc, bsscfg, ampdu_rx, AMPDU_ALL_TID_BITMAP);
	}
	bsscfg_ampdu_rx->rx_amsdu_in_ampdu_ovrd = ovrd;
	...
	wlc_d11rxhdr_t *wrxh)
{
	...
	if (f->qos) {
		...
		ASSERT(f->ividx < wlc->pub->tunables->num_rxivs);
	}
...
#ifdef WLAMPDU_HOSTREORDER
	if (AMPDU_HOST_REORDER_ENAB(ampdu_rx->wlc) && resp->queued) {
		return FALSE;
	}
#endif // endif
...
wlc_ampdu_recvdata(ampdu_rx_info_t *ampdu_rx, struct scb *scb, struct wlc_frminfo *f)
{
...
#ifdef WL11AC
			(((f->rxh->PhyRxStatus_0 & PRXS0_FT_MASK) == FT_VHT) &&
	...
	if (seq == resp->exp_seq) {
		...
		if (ACMAJORREV2_THROUGHPUT_OPT_ENAB(wlc)) {
			/* When the seq num match but queue non empty */
			/* Count as a hole */
			if (resp->queued && wlc->isvhtmcs) {
				wlc->pldo.nholes++;
			}
		}
#endif // endif
		...
		WL_AMPDU_RX(("wl%d: %s: Releasing pending packets\n",
			wlc->pub->unit, __FUNCTION__));
		if (AMPDU_HOST_REORDER_ENAB(wlc) && resp->queued) {
			if (BCMPCIEDEV_ENAB(wlc->pub))
	...
	if (offset >= resp->ba_wsize) {
		...
		if (AMPDU_HOST_REORDER_ENAB(wlc) && BCMPCIEDEV_ENAB(wlc->pub)) {
			AMPDU_CHAIN_RXCPLID_RESET(&resp->rxcpl_list);
		}
		...
		if ((resp = scb_ampdu->resp[tid])) {
			if (AMPDU_HOST_REORDER_ENAB(wlc)) {
				...
				AMPDU_HOST_HASPKT(resp, indx, wlc->osh, f->p);
			}
			else {
				ASSERT(!AMPDU_RXQ_HASPKT(resp, indx)); /* rxq[indx] is free */
				ASSERT(f->p != NULL);
				AMPDU_RXQ_SETPKT(resp, indx, f->p);

#if defined(BCMPKTIDMAP)
				/* save d11rxhdr pointer in pkttag, reuse field u */
				ASSERT(WLPKTTAG(f->p)->u.wrxh == NULL);
				WLPKTTAG(f->p)->u.wrxh = f->wrxh;
#else  /* ! BCMPKTIDMAP */
...
#ifdef WLAMPDU_HOSTREORDER
	if (AMPDU_HOST_REORDER_ENAB(ampdu_rx->wlc) && !BCMPCIEDEV_ENAB(ampdu_rx->wlc->pub)) {
...
#ifdef WLAMPDU_HOSTREORDER
		if (AMPDU_HOST_REORDER_ENAB(ampdu_rx->wlc)) {
			if (BCMPCIEDEV_ENAB(ampdu_rx->wlc->pub)) {
				AMPDU_CHAIN_RXCPLID_FLUSH(&resp->rxcpl_list);
			}
	...
	dot11_addba_req_t *addba_req, int body_len)
{
	...
	if (!AMPDU_ENAB(ampdu_rx->wlc->pub) || (SCB_BSSCFG(scb)->BSS && !SCB_HT_CAP(scb))) {
		wlc_send_addba_resp(ampdu_rx->wlc, scb, DOT11_SC_DECLINED,
			addba_req->token, timeout, param_set);
	...
	if ((resp == NULL) || (AMPDU_HOST_REORDER_ENAB(ampdu_rx->wlc) &&
		!BCMPCIEDEV_ENAB(ampdu_rx->wlc->pub) && (tohost_ctrlpkt == NULL)))
#else /* WLAMPDU_HOSTREORDER */
	if (resp == NULL)
#endif /* WLAMPDU_HOSTREORDER */
	{
	...
	if (!ampdu_rx->wlc->_rx_amsdu_in_ampdu ||
		bsscfg_ampdu_rx->rx_amsdu_in_ampdu_ovrd == OFF ||
#ifdef WLAMSDU_2G_DISABLED
		(CHSPEC_IS2G(SCB_BSSCFG(scb)->current_bss->chanspec)) ||
#endif /* WLAMSDU_2G_DISABLED */
		0) {
...
wlc_ampdu_update_ie_param(ampdu_rx_info_t *ampdu_rx)
{
	...
	if (VHT_ENAB(wlc->pub)) {
		if (ampdu_rx->rx_factor > AMPDU_RX_FACTOR_64K)
			ht_rx_factor = AMPDU_RX_FACTOR_64K;
		else
			ht_rx_factor = ampdu_rx->rx_factor;
	} else {
		ht_rx_factor = ampdu_rx->rx_factor;
	}
	...
	uint16 cur_idx, bool new_hole, bool del_flow, bool flush_all)
{
	...
	*/
	/* XXX: pcie full dongle doesn't need metadata, as reorder is done on the dongle itself
...
wlc_ampdu_frwd_handle_host_reorder(ampdu_rx_info_t *ampdu_rx, void *pkt, bool forward)
{
	...
	if (ptr == NULL) {
		...
		if (WLPKTTAGSCBGET(pkt) != NULL) {
			...
			ptr->resp = wlc_ampdu_get_resp_ptr(ampdu_rx, WLPKTTAGSCBGET(pkt), pkt);
			if (ptr->resp == NULL) {
				MFREE(wlc->osh, ptr, sizeof(ampdu_reorder_info_t));
				return pkt;
			}
		...
		if (forward) {
			...
			if (ptr->new_hole_informed) {
				...
				if ((p_ahr = PKTGET(wlc->osh, TXOFF, TRUE)) != NULL) {
					...
					wlc_pkttag_info_move(wlc->pub, pkt, p_ahr);
...
/* memory of ampdu_rx_cubby have been allocated in the target */
/* space and content of original one is copied to target */
/* all the embedded SCB in the ampdu_rx_cubby have been updated into the new SCB */
#if defined(BCM_HOST_MEM_RESTORE) && defined(BCM_HOST_MEM_SCB)
int
...
{
	ampdu_rx_info_t *ampdu_rx = (ampdu_rx_info_t *)context;
	struct scb_ampdu_rx *scb_ampdu_dongle = SCB_AMPDU_RX_CUBBY(ampdu_rx, scb_dongle);
	if (scb_ampdu_dongle != NULL) {
		scb_ampdu_dongle->scb = scb_host;
		scb_ampdu_dongle->ampdu_rx = ampdu_rx;

		struct ampdu_rx_cubby *cubby_info_ampdu_rx;
		struct ampdu_rx_cubby *shadow_cubbyinfo_ampdu_rx;
		cubby_info_ampdu_rx = (struct ampdu_rx_cubby *)SCB_AMPDU_INFO(ampdu_rx, scb_dongle);
		shadow_cubbyinfo_ampdu_rx = (struct ampdu_rx_cubby *)
			SCB_AMPDU_INFO(ampdu_rx, scb_host);

		if (cubby_info_ampdu_rx != NULL) {
			scb_ampdu_rx_t *scb_ampdu_rx_host = NULL;
			scb_ampdu_rx_t *scb_ampdu_rx_dongle = cubby_info_ampdu_rx->scb_rx_cubby;
			scb_ampdu_rx_host = (scb_ampdu_rx_t *)wlc_scb_alloc_mem_get
				(ampdu_rx->wlc, SCB_CUBBY_ID_AMPDU_RX, sizeof(scb_ampdu_rx_t), 1);
			if (!scb_ampdu_rx_host)
				return BCME_NOMEM;

			memcpy(scb_ampdu_rx_host, scb_ampdu_rx_dongle, sizeof(scb_ampdu_rx_t));
			uint8 tid;
			for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
				scb_ampdu_rx_host->resp[tid] = scb_ampdu_rx_dongle->resp[tid];
			}

			shadow_cubbyinfo_ampdu_rx->scb_rx_cubby = scb_ampdu_rx_host;
			scb_ampdu_rx_host->scb = scb_host;
			scb_ampdu_rx_host->ampdu_rx = ampdu_rx;

			MFREE(ampdu_rx->wlc->osh, scb_ampdu_rx_dongle, sizeof(scb_ampdu_rx_t));
			cubby_info_ampdu_rx->scb_rx_cubby = NULL;
			return BCME_OK;
		}
	}
	return BCME_ERROR;
...
{
	ampdu_rx_info_t *ampdu_rx = wlc->ampdu_rx;
	scb_ampdu_rx_t *scb_ampdu_rx = SCB_AMPDU_RX_CUBBY(ampdu_rx, scb_host);
	if (scb_ampdu_rx != NULL) {
		scb_ampdu_rx->scb = scb_dongle;
		scb_ampdu_rx->ampdu_rx = ampdu_rx;
	}

	struct ampdu_rx_cubby *cubby_info_ampdu_rx_dongle = (struct ampdu_rx_cubby *)
		SCB_AMPDU_INFO(ampdu_rx, scb_dongle);
	struct ampdu_rx_cubby *cubby_info_ampdu_rx_host = (struct ampdu_rx_cubby *)
		SCB_AMPDU_INFO(ampdu_rx, scb_host);

	if (cubby_info_ampdu_rx_host != NULL) {
		scb_ampdu_rx_t *scb_ampdu_rx_dongle = NULL;
		scb_ampdu_rx_t *scb_ampdu_rx_host = cubby_info_ampdu_rx_host->scb_rx_cubby;

		if (scb_ampdu_rx_host != NULL) {
			scb_ampdu_rx_dongle = MALLOC(ampdu_rx->wlc->osh, sizeof(scb_ampdu_rx_t));
			if (!scb_ampdu_rx_dongle)
				return BCME_NOMEM;

			memcpy(scb_ampdu_rx_dongle, scb_ampdu_rx_host, sizeof(scb_ampdu_rx_t));
			cubby_info_ampdu_rx_dongle->scb_rx_cubby = scb_ampdu_rx_dongle;
			uint8 tid;
			for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
				scb_ampdu_rx_dongle->resp[tid] = scb_ampdu_rx_host->resp[tid];
			}
			scb_ampdu_rx_dongle->scb = scb_dongle;
			scb_ampdu_rx_dongle->ampdu_rx = ampdu_rx;

			wlc_scb_alloc_mem_free(wlc, SCB_CUBBY_ID_AMPDU_RX,
				(void *)scb_ampdu_rx_host);
			cubby_info_ampdu_rx_host->scb_rx_cubby = NULL;
			return BCME_OK;
		}
	}
	return BCME_ERROR;
