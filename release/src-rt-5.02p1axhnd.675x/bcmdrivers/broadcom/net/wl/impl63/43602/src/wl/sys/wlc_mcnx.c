	...
	uint32 ltsf_h, uint32 ltsf_l, uint32 *ltbtt_h, uint32 *ltbtt_l)
{
	...
	if (bmi->unaligned_tbtt_tsfo > 0) {
		bcn_offset = wlc_calc_tbtt_offset(bmi->bcn_prd, rtbtt_h, rtbtt_l);
		wlc_uint64_add(&rtbtt_h, &rtbtt_l, 0,
			(bmi->unaligned_tbtt_tsfo - bcn_offset) % (bmi->bcn_prd << 10));
	} else
	...
	uint32 ltsf_h, uint32 ltsf_l, uint32 *ltbtt_h, uint32 *ltbtt_l)
{
	...
	if (bmi->unaligned_tbtt_tsfo > 0) {
		...
		wlc_uint64_sub(&rtbtt_h, &rtbtt_l, 0,
			(bcn_offset - bmi->unaligned_tbtt_tsfo) % (bmi->bcn_prd << 10));
	} else {
		/* last tbtt */
		wlc_tsf64_to_last_tbtt64(bmi->bcn_prd, &rtbtt_h, &rtbtt_l);
	}
	wlc_mcnx_r2l_tsf64(mcnx, cfg, rtbtt_h, rtbtt_l, ltbtt_h, ltbtt_l);
}
...
void
wlc_mcnx_bss_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool up)
{
	wlc_info_t *wlc = mcnx->wlc;
	...
	wlc_mcnx_bss_upd_notif(mcnx, cfg, up);
}
...
_wlc_mcnx_st_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool set)
{
	...
	state = set ?
	        (P2P_GO(wlc, cfg) ? M_P2P_BSS_ST_GO :
	         ...
	         BSSCFG_NAN(cfg) ? M_P2P_BSS_ST_NAN | M_P2P_BSS_ST_GC :
	         M_P2P_BSS_ST_STA) :
	        0;
...
_wlc_mcnx_tbtt_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool set)
{
	...
	if (set) {
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_ST(bss), 0);
	}
}
...
wlc_mcnx_tbtt_adj_all(wlc_mcnx_info_t *mcnx, int32 off_h, int32 off_l)
{
	...
	FOREACH_AS_BSS(wlc, idx, cfg) {
		...
		upd = BSSCFG_STA(cfg) && (cfg->BSS || cfg != wlc->cfg) && !IBSS_HW_ENAB(cfg);
...
static void
wlc_mcnx_bss_upd_notif(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool up)
{
...
int
BCMATTACHFN(wlc_mcnx_intr_register)(wlc_mcnx_info_t *mcnx, wlc_mcnx_intr_fn_t cb, void *arg)
{
...
int
BCMATTACHFN(wlc_mcnx_intr_unregister)(wlc_mcnx_info_t *mcnx, wlc_mcnx_intr_fn_t cb, void *arg)
{
	bcm_notif_h hdl = mcnx->intr_notif_hdl;
	return bcm_notif_remove_interest(hdl, (bcm_notif_client_callback)cb, arg);
}
...
wlc_mcnx_bssid_set(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	int idx;
	int bss;
	uint16 mask = ADDR_BMP_BSSID;
	uint16 val = ADDR_BMP_BSSID;
...
/* accessors */
int
wlc_mcnx_rcmta_bssid_idx(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg)
{
	bss_mcnx_info_t *bmi;
...
_wlc_mcnx_ctw_upd(wlc_mcnx_info_t *mcnx, wlc_bsscfg_t *cfg, bool enab, uint32 ctw)
{
	...
	if (enab) {
		...
		if (!cfg->associated) {
			...
			if (offset >> 5 < ctw >> 5) {
				...
				if (wlc_mcnx_read_shm(mcnx, M_P2P_BSS_CTW(bss)) == 0) {
					uint16 ctwend = (uint16)((ltbtt_l + ctw) >> 5);
		...
		wlc_mcnx_write_shm(mcnx, M_P2P_BSS_CTW(bss), (uint16)(ctw >> 5));
