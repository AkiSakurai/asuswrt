 ...
 *
 * $Id: wlc_pmkid.c 308467 2012-01-16 08:15:52Z $
 */
...
BCMATTACHFN(wlc_pmkid_attach)(wlc_info_t *wlc)
{
	...
	if (!(pmkid_info = (wlc_pmkid_info_t *)MALLOC(wlc->osh, sizeof(wlc_pmkid_info_t)))) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
		          wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return NULL;
	}

	bzero((char *)pmkid_info, sizeof(wlc_pmkid_info_t));
	pmkid_info->wlc = wlc;
...
wlc_pmkid_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (!(bss_pmkid = (bss_pmkid_info_t *)MALLOC(pmkid_info->osh, sizeof(bss_pmkid_info_t)))) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n", UNIT(pmkid_info),
	...
	bzero(bss_pmkid, sizeof(bss_pmkid_info_t));
...
wlc_pmkid_event(wlc_pmkid_info_t *pmkid_info, wlc_bsscfg_t *cfg)
{
	...
	e->data = wlc_event_data_alloc(wlc->eventq, pmkid_info->osh,
...
#endif // endif
#ifdef MFP
	if (WLC_MFP_ENAB(pmkid_info->wlc->pub) && (cfg->wsec & MFP_CAPABLE))
