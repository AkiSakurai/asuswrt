 ...
 *
 * $Id: wlc_ccx.c 778869 2019-09-12 05:53:46Z $
 */
...
typedef struct wlc_ccx_info {
	...
	int8		*p_cur_qtxpwr;		/* tx power in qdbm before apply TPC */
	int8		*p_tpc_qtxpwr;		/* TPC in qdbm */
#else
	int8		PAD[3];
...
#ifdef CCX_SDK
#define MFP_TKIP_PRIO	0xff	/* tkip priority in MFP */
...
static int wlc_ccx_watchdog(void *hdl);
...
const uint8 ckip_llc_snap[CKIP_LLC_SNAP_LEN] = { 0xAA, 0xAA, 0x03, 0x00, 0x40, 0x96, 0x00, 0x02 };
...
BCMATTACHFN(wlc_ccx_attach)(wlc_info_t *wlc)
{
	...
	if ((ci = (wlc_ccx_info_t *)MALLOC(pub->osh, sizeof(wlc_ccx_info_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			pub->unit, __FUNCTION__, MALLOCED(pub->osh)));
		return NULL;
	}
	bzero((char *)ci, sizeof(wlc_ccx_info_t));
	...
	if ((rm->ccx = (ccx_rm_t *)MALLOC(wlc->osh, sizeof(ccx_rm_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto error;
	}
	bzero((char*)rm->ccx, sizeof(ccx_rm_t));

#ifdef WLTXPWR_PER_CORE
	/* Allocate memory for RM's power structure. */
	if ((rm->ccx->p_cur_txpower = (int8 *)MALLOCZ(wlc->osh, WL_TXPWR_CORE_MAX)) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto error;
	}
...
#ifdef WLTXPWR_PER_CORE
	/* Allocate memory for power structures. */
	if ((ci->p_cur_qtxpwr = (int8 *)MALLOCZ(wlc->osh, WL_TXPWR_CORE_MAX)) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto error;
	}

	if ((ci->p_tpc_qtxpwr = (int8 *)MALLOCZ(wlc->osh, WL_TXPWR_CORE_MAX)) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto error;
	}
...
#ifdef WLTXPWR_PER_CORE
			if (ci->rm_state->ccx->p_cur_txpower) {
				MFREE(ci->pub->osh, ci->rm_state->ccx->p_cur_txpower,
					WL_TXPWR_CORE_MAX);
				ci->rm_state->ccx->p_cur_txpower = NULL;
			}
#endif /* WLTXPWR_PER_CORE */
...
#ifdef WLTXPWR_PER_CORE
		if (ci->p_cur_qtxpwr) {
			MFREE(ci->pub->osh, ci->p_cur_qtxpwr, WL_TXPWR_CORE_MAX);
			ci->p_cur_qtxpwr = NULL;
		}
		if (ci->p_tpc_qtxpwr) {
			MFREE(ci->pub->osh, ci->p_tpc_qtxpwr, WL_TXPWR_CORE_MAX);
			ci->p_tpc_qtxpwr = NULL;
		}
#endif /* WLTXPWR_PER_CORE */
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_CCKM_KRK):
		...
		if (plen >= CCKM_KRK_LEN + CCKM_BTK_LEN) {
			/* Save BTK for the late idsup enabling */
			bcopy((uint8 *)p + CCKM_KRK_LEN, ci->base_transient_key, CCKM_BTK_LEN);
		}
		break;
...
wlc_ccx_on_leave_bss(ccx_t *ccxh)
{
	...
	if (ci->tpc) {
...
#ifdef WLTXPWR_PER_CORE
		int8 cur_qtxpwr[WL_TXPWR_CORE_MAX];
#else
		int8 cur_qtxpwr;
#endif /* WLTXPWR_PER_CORE */
		ppr_t *txpwr;
		 ...
		 */
#ifdef WLTXPWR_PER_CORE
		wlc_phy_txpower_get(ci->wlc->band->pi, cur_qtxpwr, NULL);
		if ((cur_qtxpwr[0] == ci->p_tpc_qtxpwr[0]) &&
			(cur_qtxpwr[1] == ci->p_tpc_qtxpwr[1])) {
#else
		wlc_phy_txpower_get(ci->wlc->band->pi, &cur_qtxpwr, NULL);
		if (cur_qtxpwr == ci->tpc_qtxpwr) {
			...
			/* restore tx power */
#ifdef WLTXPWR_PER_CORE
			wlc_phy_txpower_set(ci->wlc->band->pi, ci->p_cur_qtxpwr, FALSE, txpwr);
#else
			wlc_phy_txpower_set(ci->wlc->band->pi, ci->cur_qtxpwr, FALSE, txpwr);
#endif /* WLTXPWR_PER_CORE */
	...
	if (pwr_ie && !bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_PWR_CONSTRAINT_ID)) {
		...
		if (pwr_ie->len >= 5 && !bcmp(pwr_ie->oui, CISCO_AIRONET_OUI, 3) &&
...
#ifdef WLTXPWR_PER_CORE
			wlc_phy_txpower_get(ci->wlc->band->pi, cur_qtxpwr, NULL);
			if (ci->tpc && (ci->p_tpc_qtxpwr[0] == cur_qtxpwr[0]) &&
				(ci->p_tpc_qtxpwr[1] == cur_qtxpwr[1])) {
				cur_qtxpwr[0] = ci->p_cur_qtxpwr[0];
				cur_qtxpwr[1] = ci->p_cur_qtxpwr[1];
			}
#else
			wlc_phy_txpower_get(ci->wlc->band->pi, &cur_qtxpwr, NULL);
...
#ifdef WLTXPWR_PER_CORE
			/* Shouldn't we infere whole power out of the per-chain instead ?
			 */
			if (((int32)(cur_qtxpwr[0]) >
			...
			if ((int32)(cur_qtxpwr) >
...
#ifdef WLTXPWR_PER_CORE
				int i = 0;
				int8 adj_qtxpwr[WL_TXPWR_CORE_MAX];
				adj_qtxpwr[0] = adj_qtxpwr[1] =
					pwr_ie->power * WLC_TXPWR_DB_FACTOR;
#endif /* WLTXPWR_PER_CORE */
				...
				/* restore tx power */
#ifdef WLTXPWR_PER_CORE
				wlc_phy_txpower_set(ci->wlc->band->pi, adj_qtxpwr, FALSE, txpwr);
				for (i = 0; i < WL_TXPWR_CORE_MAX; i++) {
					ci->p_cur_qtxpwr[i] = cur_qtxpwr[i];
					ci->p_tpc_qtxpwr[i] =
						pwr_ie->power * WLC_TXPWR_DB_FACTOR;
				}
				ppr_delete(ci->wlc->osh, txpwr);
#else
				wlc_phy_txpower_set(ci->wlc->band->pi,
		...
		if (AIE && AIE->len > AIRONET_IE_CKIP) {
			...
			if (val & (CKIP_MIC_ENABLED | CKIP_KP_ENABLED)) {
				...
				for (i = 0; i < WSEC_MAX_DEFAULT_KEYS; i++) {
					wsec_key_t *key = WSEC_KEY(wlc, i);

					if (key)
						wlc_key_iv_init(wlc, cfg, key, NULL);
				}
			}
		}
#ifdef BCMDBG
		...
		if (ie->channel) {
			...
			if (i == channel_idx) {
				...
				if (WLRCC_ENAB(wlc->pub)) {
					WL_ASSOC(("Adding CH%d to RCC\n", ie->channel));
					/* merge the given channels to RCC only when
					 * the given channel is valid.
					 */
					if (wlc_valid_chanspec_db(wlc->cmi,
						CH20MHZ_CHSPEC(ie->channel))) {
						rcc_add_chanspec(cfg->roam, ie->channel);
					}
				}
#endif // endif
			}
...
#ifdef WLRCC
	/* WLRCC doesn't check roam list as some cases are reported
	   that the AP/WLC provides improper AP lists that never match the roam targets.
	*/
	return 0;
#else
...
wlc_ccx_rm_scan_begin(wlc_ccx_info_t *ci, int type, uint32 dur)
{
	...
	wlc->scan->bss_type = DOT11_BSSTYPE_ANY;
	bcopy(ether_bcast.octet, wlc->scan->bssid.octet, ETHER_ADDR_LEN);
	wlc->scan->usage = SCAN_ENGINE_USAGE_RM;
...
wlc_ccx_rm_pathloss_begin(wlc_ccx_info_t *ci, wlc_rm_req_t* req)
{
	...
	ppr_t *txpwr;
#ifdef WLTXPWR_PER_CORE
	int8 restored_pwr[WL_TXPWR_CORE_MAX];
#else
	int8 cur_qtxpwr;
#endif /* WLTXPWR_PER_CORE */
	...
	ccx_rm->cur_txpowerctrl = wlc_phy_txpower_hw_ctrl_get(ci->wlc->band->pi);
	wlc_phy_txpower_hw_ctrl_set(ci->wlc->band->pi, FALSE);

	/* save tx power and set the desired tx power */
#ifdef WLTXPWR_PER_CORE
	wlc_phy_txpower_get(ci->wlc->band->pi, ccx_rm->p_cur_txpower, &ccx_rm->cur_txpoweroverride);
#else
	wlc_phy_txpower_get(ci->wlc->band->pi, &cur_qtxpwr, &ccx_rm->cur_txpoweroverride);
	...
	/* restore tx power */
#ifdef WLTXPWR_PER_CORE
	restored_pwr[0] = restored_pwr[1] = pathloss_data->req_txpower * 4;
	wlc_phy_txpower_set(ci->wlc->band->pi, restored_pwr, FALSE, txpwr);
#else
	wlc_phy_txpower_set(ci->wlc->band->pi, pathloss_data->req_txpower * 4, FALSE, txpwr);
#endif /* WLTXPWR_PER_CORE */
	ppr_delete(ci->wlc->osh, txpwr);
...
wlc_ccx_rm_pathloss_complete(ccx_t *ccxh, bool force_done)
{
	...
	wlc_phy_txpower_hw_ctrl_set(ci->wlc->band->pi, ccx_rm->cur_txpowerctrl);
	...
	/* restore tx power */
#ifdef WLTXPWR_PER_CORE
	wlc_phy_txpower_set(ci->wlc->band->pi, ccx_rm->p_cur_txpower, ccx_rm->cur_txpoweroverride,
		txpwr);
#else
	wlc_phy_txpower_set(ci->wlc->band->pi, (int8)ccx_rm->cur_txpower,
...
wlc_ccx_rm_frm(ccx_t *ccxh, wlc_d11rxhdr_t *wrxh, struct dot11_header *h)
{
	...
	if (!elt) {
		...
		elt = (wlc_ccx_rm_frm_elt_t *)MALLOC(ci->pub->osh, sizeof(wlc_ccx_rm_frm_elt_t));
		if (!elt) {
			WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
				ci->pub->unit, __FUNCTION__, MALLOCED(ci->pub->osh)));
			return;
		}
		bzero(elt, sizeof(wlc_ccx_rm_frm_elt_t));
...
wlc_ccx_rm_validate(ccx_t *ccxh, chanspec_t cur_chanspec, wlc_rm_req_t *req)
{
	...
	if (req->chanspec && req->chanspec != cur_chanspec && wlc->pub->associated &&
		...
		WL_INFORM(("wl%d: wlc_rm_validate: refusing off channel "
			   ...
			   "limit %d, RM type %d token %d channel %d\n",
			   wlc->pub->unit, req->dur, ci->rm_limit,
			   req->type, req->token, CHSPEC_CHANNEL(req->chanspec)));
		req->flags |= WLC_RM_FLAG_REFUSED;
		return -1;
	}
	...
	ccx_rm_req_t* req_pkt, int req_len)
{
	...
	rm_req = (wlc_rm_req_t*)MALLOC(ci->pub->osh, rm_req_size);
	if (rm_req == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			ci->pub->unit, __FUNCTION__, MALLOCED(ci->pub->osh)));
...
wlc_ccx_rm_new_report(wlc_ccx_info_t *ci, uchar** buf, int* avail_len)
{
	...
	*buf = NULL;
	...
	new_report = MALLOC(ci->pub->osh, sizeof(wlc_ccx_rm_rept_t));
	if (new_report == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			ci->pub->unit, __FUNCTION__, MALLOCED(ci->pub->osh)));
		return NULL;
	...
	bzero(new_report, sizeof(wlc_ccx_rm_rept_t));
...
wlc_ccx_indicate_event(wlc_ccx_info_t *ci, int event, PTLV_STRUCT tlvs, uint tlv_cnt)
{
	...
	if (!(indication = (PCCX_NIC_SPECIFIC_EXTENSION)MALLOC(ci->pub->osh, len))) {
		WL_ERROR(("wl%d: %s: indication out of memory, malloced %d bytes\n",
			ci->pub->unit, __FUNCTION__, MALLOCED(ci->pub->osh)));
		return BCME_NOMEM;
	}
	bzero(indication, len);
	...
	bool append)
{
	...
	if (!ie_len)
		return;
	if ((rept_ie = (ccx_neighbor_rept_ie_t *)MALLOC(ci->pub->osh, ie_len)) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			ci->pub->unit, __FUNCTION__, MALLOCED(ci->pub->osh)));
		return;
	}
	ie = rept_ie;
	bzero((char*)ie, ie_len);
	...
	*bytes_needed = 0;
	...
	case OID_CCX_STA_STATISTICS_2: {
		...
		stat2->RSNAMgmtStatsCCMPReplays =
			WLCNTVAL(ci->pubci.mgmt_cnt.ccmpmgmtreplyerr);
	...
	*bytes_needed = 0;
	...
	case OID_CCX_ASSOC_INFO: {
		...
		if (bcmp((char*)&assoc->ccxBSSID,
			&ci->wlc->join_targets->ptrs[ci->wlc->join_targets_last]->BSSID,
			ETHER_ADDR_LEN)) {
	...
	struct dot11_management_header *hdr, void *p, struct scb *scb)
{
	...
	algo = scb->key->algo;
	ASSERT(algo == CRYPTO_ALGO_TKIP || algo == CRYPTO_ALGO_AES_CCM);

	/* verify FC field */
	fc = ltoh16(hdr->fc);
	if (!(fc & FC_WEP)) {
		if (algo == CRYPTO_ALGO_TKIP)
			WLCNTINCR(ci->pubci.mgmt_cnt.tkipmgmtnoencrypterr);
		else
			WLCNTINCR(ci->pubci.mgmt_cnt.ccmpmgmtnoenypterr);
		WL_ERROR(("wl%d: %s: unencrypt unicast management frame from %s\n",
			ci->pub->unit, __FUNCTION__,
	...
	prio = (algo == CRYPTO_ALGO_TKIP) ? MFP_TKIP_PRIO : 0;
	...
	/* decrypt pkt */
	if (!wlc_wsec_recvdata(ci->wlc, ci->pub->osh, scb, &f, prio)) {
		WL_ERROR(("wl%d: %s: decrypt packet failed\n",
			ci->pub->unit, __FUNCTION__));
		return BCME_ERROR;
	}
	...
	if (algo == CRYPTO_ALGO_TKIP) {
		...
		if (mhdr->id != DOT11_MNG_PROPR_ID ||
			...
			WL_ERROR(("wl%d: %s: MHDR ie check failed\n",
				ci->pub->unit, __FUNCTION__));
			return BCME_ERROR;
		}
	}
	...
	f.eh = (struct ether_header *)PKTDATA(ci->pub->osh, f.p);
	/* check mic */
	if (!wlc_wsec_miccheck(ci->wlc, ci->pub->osh, scb, &f)) {
		WL_ERROR(("wl%d: %s: MIC check failed\n",
			ci->pub->unit, __FUNCTION__));
		return BCME_ERROR;
	}

	/* update rx iv */
	wlc_wsec_rxiv_update(ci->wlc, &f);
	...
	/* remove 802.11 hdr and TKIP iv header or CCMP header */
	PKTPULL(ci->pub->osh, p, offset + scb->key->iv_len);
...
wlc_ccx_mfp_tx(ccx_t *ccxh, void *p, struct scb *scb)
{
	...
	ASSERT(key->algo == CRYPTO_ALGO_TKIP || key->algo == CRYPTO_ALGO_AES_CCM);
	...
	if (key->algo == CRYPTO_ALGO_TKIP) {
		...
		*(uint16*)mhdr->fc = hdr->fc;
		...
		offset = DOT11_MGMT_HDR_LEN + key->iv_len - ETHER_HDR_LEN;
		...
		/* calculate MIC */
		wlc_dofrag_tkip(ci->pub, p, 0, 1, ci->pub->osh, SCB_BSSCFG(scb), scb, eh,
			key, MFP_TKIP_PRIO, PKTLEN(ci->pub->osh, p));
	...
	wlc_key_iv_update(ci->wlc, SCB_BSSCFG(scb), key, (uint8*)(hdr + 1), TRUE, FALSE);

	/* encrypt frame */
	if (!wlc_wsec_sw_encrypt_data(ci->wlc, ci->pub->osh, p, SCB_BSSCFG(scb),
		scb, key)) {
		WL_ERROR(("wl%d: %s: encrypting mgmt pkt fail\n",
			ci->pub->unit, __FUNCTION__));
		status = BCME_ERROR;
	}
...
wlc_ccx_mfp_set_key(ccx_t *ccxh, wl_wsec_key_t *key)
{
	...
	if (!key->len) {
		scb->flags2 &= ~SCB2_CCX_MFP;
		/* wlc_key_remove()=>wlc_key_delete()=>wlc_key_reset() will clean
		 * up WL_MFP_KEY in key->flags along with all other flags
		 */
	...
	/* must have key */
	ASSERT(scb->key);
	/* key must already be installed successfully */
	if (!scb->key) {
		WL_ERROR(("wl%d: %s: no key set for MFP\n", ci->pub->unit, __FUNCTION__));
		return;
	}
	...
	if (scb->key->algo != CRYPTO_ALGO_TKIP &&
		...
		WL_ERROR(("wl%d: %s: unsupported key algorithm for MFP\n",
			ci->pub->unit, __FUNCTION__));
		return;
	}
	...
	WLCNTSET(ci->pubci.mgmt_cnt.ccmpmgmtreplyerr, 0);
	...
	int code, bool is_status, void *arg, int arg_len)
{
	...
	if (row_number != 1) {
		...
		entry = (wlc_ccx_status_row_t*)MALLOC(ci->pub->osh, sizeof(wlc_ccx_status_row_t));
		if (entry == NULL) {
			WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			ci->pub->unit, __FUNCTION__, MALLOCED(ci->pub->osh)));
			return;
		}
		bzero(entry, sizeof(wlc_ccx_status_row_t));
...
#endif /* CCX_SDK */
void
wlc_ccx_update_krk_btk(wlc_bsscfg_t *cfg)
{
	wlc_ccx_info_t *ci = (wlc_ccx_info_t *)cfg->wlc->ccx;
	wlc_ccxsup_update_krk_btk(cfg->wlc->ccxsup, cfg, ci->key_refresh_key,
		ci->base_transient_key);
}
#endif /* BCMCCX */
