 ...
 *
 * $Id: bcmrobo.c 462666 2014-03-18 12:45:17Z $
 */
...
#ifdef PLC
	/* XXX WAR to age arl entry faster. When a STA roams to a remote
	 * WDS-AP and returns, the corresponding arl entry in the switch
	 * is not updated with latest received port until the entry ages
	 * out. Default aging timeout is 5 min. Instead of fixing smaller
	 * aging timeout ideally we would like to fast age the arl entry
	 * when the roam event occurs.
	 */
	if (robo->plc_hw) {
		val32 = 0x100002;
