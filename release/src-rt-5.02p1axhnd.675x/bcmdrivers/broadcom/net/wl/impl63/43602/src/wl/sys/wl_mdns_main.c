 ...
 */
/*
 * $Id: wl_mdns_main.c 375469 2012-12-18 22:51:38Z $
 *
 * mdns_sd_main.c
 *
...
void mDNSPlatformInit(wlc_dngl_ol_mdns_info_t *mdns, proxy_services_t *proxy_services)
{
	...
	memcpy(mdns->NicMacAddr, &mdns->wlc_dngl_ol->txinfo.cur_etheraddr, ETHER_ADDR_LEN);
...
wl_mDNS_event_handler(wlc_dngl_ol_mdns_info_t *mdnsi, uint32 event, void *event_data)
{
	...
	case BCM_OL_E_WOWL_COMPLETE:
		...
		wl_mDNS_Init(mdnsi, (uint8 *)ppcie_shared->mdns_dbase, ppcie_shared->mdns_len);
		break;
...
wlc_dngl_ol_mdns_attach(wlc_dngl_ol_info_t *wlc_dngl_ol)
{
	...
	ppcie_shared->mdns_dbase[0] = ppcie_shared->mdns_len = 0;
