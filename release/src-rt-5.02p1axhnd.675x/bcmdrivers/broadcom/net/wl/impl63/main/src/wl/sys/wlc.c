 ...
 *
 * $Id: wlc.c 789184 2020-07-21 19:39:50Z $
 */
...
enum wlc_iov {
	...
	IOV_FIPS_MUTE = 170,
	IOV_FIPS_CMVP_TEST = 171,
	IOV_ATF_PERC_RESET = 172,
	IOV_CLM_DATA_VER = 173,
...
static uint16 wlc_num_chanspecs_2g(chanspec_t chanspec);

static uint16 wlc_num_chanspecs_5g(chanspec_t chanspec);

static uint16 wlc_num_chanspecs_6g(chanspec_t chanspec);
...
extern void wlc_phy_bypass_idletssi_init(wlc_phy_t *ppi, bool force);
...
static uint16
wlc_num_chanspecs_2g(chanspec_t chanspec)
{
	uint16 num_chanspecs = 0;
	if (CHSPEC_IS20(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_2G_20;
	} else if (CHSPEC_IS40(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_2G_40;
	}
	return num_chanspecs;
}

static uint16
wlc_num_chanspecs_5g(chanspec_t chanspec)
{
	uint16 num_chanspecs = 0;
	if (CHSPEC_IS20(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_5G_20;
	} else if (CHSPEC_IS40(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_5G_40;
	} else if (CHSPEC_IS80(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_5G_80;
	} else if (CHSPEC_IS8080(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_5G_8080;
	} else if (CHSPEC_IS160(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_5G_160;
	}
	return num_chanspecs;
}

static uint16
wlc_num_chanspecs_6g(chanspec_t chanspec)
{
	uint16 num_chanspecs = 0;
	if (CHSPEC_IS20(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_6G_20;
	} else if (CHSPEC_IS40(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_6G_40;
	} else if (CHSPEC_IS80(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_6G_80;
	} else if (CHSPEC_IS160(chanspec)) {
		num_chanspecs = WL_NUMCHANSPECS_6G_160;
	}
	return num_chanspecs;
}
...
wlc_rsdb_fixup_init_chanspec(wlc_info_t *wlc, chanspec_t chanspec)
{
	...
	*/
	...
	CHSPEC_BAND(other_wlc->chanspec) == CHSPEC_BAND(chanspec)) {
		chanspec_t chspec = CH20MHZ_CHSPEC2(0, WL_CHANSPEC_BAND_2G);
		if (!CHSPEC_IS2G(other_wlc->chanspec)) {
			chspec |= 15; /* so next ch will not be in the 2g band */
		}
		wlc_next_chanspec(wlc->cmi, chspec, CHAN_TYPE_ANY, TRUE);
	}
	return chanspec;
}
#endif /* WLRSDB */
...
void BCMINITFN(wlc_init)(wlc_info_t *wlc)
{
	...
	wlc_csimon_init(wlc, wlc_csimon_m2m_dd_done_cb);
#endif // endif
...
BCMATTACHFN(wlc_info_init)(wlc_info_t *wlc, int unit)
{
	...
	if ((wlc->phy_cap & PHY_PREATTACH_CAP_SUP_5G) &&
		...
		wlc->chanspec = CH20MHZ_CHSPEC2(36, WLC_BAND_5G);
	} else {
		wlc->chanspec = CH20MHZ_CHSPEC2(1, WLC_BAND_2G);
	}
...
wlc_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_CHANNEL: {
		/* TODO:6GHZ: add support for setting 6g channel */
		int chspec = (WL_CHANSPEC_BW_20 | WL_CHANNEL_BAND(val) | val);
	...
	case WLC_GET_VALID_CHANNELS: {
		...
		FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
			...
			for (i = band->first_ch; i <= band->last_ch && count < MAXCHANNEL; i++) {
				if (wlc_valid_chanspec_db(wlc->cmi,
					CH20MHZ_CHSPEC2(i, band->bandtype))) {
	...
	*/
...
#endif /* defined(WLATF) && defined(WLATF_PERC) */
		}
		wlc->atf = int_val;
#if defined(WLATF) && defined(WLATF_PERC)
	...
	case IOV_GVAL(IOV_CHANSPECS): {
		...
		if (chanspec == 0) {
			...
			if (CHSPEC_IS2G(chanspec)) {
				max_num_chanspecs = wlc_num_chanspecs_2g(chanspec);
			} else if (CHSPEC_IS5G(chanspec)) {
				max_num_chanspecs = wlc_num_chanspecs_5g(chanspec);
			} else if (CHSPEC_IS6G(chanspec)) {
				max_num_chanspecs = wlc_num_chanspecs_6g(chanspec);
			}
		}
...
wlc_pkttag_info_frag_cpy(wlc_info_t *wlc, void *pkt_from, void *pkt_to)
{
	...
	/* following fields are cleared (using RX side fields) to clear possible TAF/ATF info */
...
wlc_do_chanswitch(wlc_bsscfg_t *cfg, chanspec_t newchspec)
{
	...
	if (bw_chg) {
		if (!wlc->any_sta_in_160mhz && !wlc->num_160mhz_assocs)
                     ...
                     uint16 *buf, int *len)
{
	...
	wlc_bcn_prb_body(wlc, type, cfg, body, &body_len);
...
wlc_bss_update_brcm_ie(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	uint8 assoc;
	brcm_ie_t *brcm_ie;
	volatile uint8 flags = 0, flags1 = 0, flags2 = 0;
	...
	if (brcm_ie->flags2)
		...
		brcm_ie->len = BRCM_IE_LEN - TLV_HDR_LEN - 1;
...
wlc_nulldata_template(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct ether_addr *da)
{
	...
	ASSERT(wlc_valid_rate(wlc, rspec,
	                      CHSPEC_BANDTYPE(wlc->cfg->current_bss->chanspec), TRUE));
	...
	struct ether_addr *sa, uint16 reason_code)
{
	...
	reason = (uint16 *)pbody;
	reason[0] = htol16(reason_code);
	if (reason_code == DOT11_RC_INACTIVITY) {
		/* Sta might be in PS or PPS:
	...
	/* Explicitly passing scb pointer as NULL, so that deauth will be sent even if scb is
	 * being (or going to be) deleted. In order not to break MFP, passing the key from the scb.
	 */
	if (wlc_queue_80211_frag(wlc, pkt, cfg->wlcif->qi, NULL,
		NULL, FALSE, key,
		(scb ? WLC_LOWEST_SCB_RSPEC(scb) : WLC_LOWEST_BAND_RSPEC(wlc->band)))) {
		if (BSS_SMFS_ENAB(wlc, cfg)) {
			(void)wlc_smfs_update(wlc->smfs, cfg, SMFS_TYPE_DEAUTH_TX, reason_code);
		}
		WL_ASSOC(("wl%d.%d: %s: station "MACF" has been notified of deauthentication \n",
			wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__,
			ETHERP_TO_MACF(SCB_EA(scb))));
		return pkt;
	}
