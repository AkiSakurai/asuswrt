...
#ifdef DWDS
	{"dwds", IOV_DWDS, (IOVF_SET_DOWN), 0, IOVT_UINT8, 0},
...
wlc_wds_create(wlc_info_t *wlc, struct scb *scb, uint flags)
{
	...
	} else if (DWDS_GENAB(scb->bsscfg) && BSSCFG_AP(scb->bsscfg)) {
		wlc_wds_wpa_role_set(wlc->mwds, scb, WL_WDS_WPA_ROLE_AUTH);
	} else {
		wlc_wds_wpa_role_set(wlc->mwds, scb, WL_WDS_WPA_ROLE_AUTO);
	}
#endif /* AP */
