 ...
 *
 * $Id: threadx.c 779823 2019-10-08 11:09:11Z $
 */
...
{
	hnd_dpc_t *dpc = THREAD_DPC(thread);
	...
	if (dpc != NULL) {
#ifdef HND_INTSTATS
		hnd_stats_t *stats = dpc->stats;
		printf("dpc [%s] p=0x%p, fn=0x%p stats=%p\n",
			stats->id ? stats->id : "unknown",
			dpc, dpc->dpc_fn, stats);
#else
		printf("dpc p=0x%p, fn=0x%p\n", dpc, dpc->dpc_fn);
#endif /* HND_INTSTATS */
	}
}
