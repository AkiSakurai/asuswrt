...
_rpsnoa_update_cmnparams_from_band(wlc_rpsnoa_info_t *rpsi, uint band)
{
	...
	FOREACH_WLC(wlc_cmn, idx, current_wlc) {
		...
		if (ap_cfg) {
			bandunit = BSSCFG_BANDUNIT(ap_cfg);
			if (bandunit == band) {
				_rpsnoa_set_cmnparams_to_wlc(current_wlc, bandunit, pending);
				break;
			}
		}
	}
...
_rpsnoa_update_cmnparams_to_wlc(wlc_info_t *wlc, wlc_rpsnoa_info_t *rpsi)
{
	...
	if (ap_cfg) {
		bandunit = BSSCFG_BANDUNIT(ap_cfg);
		_rpsnoa_set_cmnparams_to_wlc(wlc, bandunit, pending);
	}
}
...
_rpsnoa_get_rps_status(wlc_rpsnoa_info_t *rpsi, uint band)
{
	...
	FOREACH_WLC(wlc_cmn, idx, current_wlc) {
		FOREACH_UP_AP(current_wlc, bss_idx, bsscfg) {
			...
			bandunit = BSSCFG_BANDUNIT(bsscfg);
			if (band == bandunit) {
				res = (rpsnoa_status_t)
		...
		uint16 *result_len, bool set)
{
	...
	band_type = in->data->band;

	if (band_type > WLC_BAND_6G || band_type < WLC_BAND_INVALID) {
		res = BCME_BADARG;
		goto exit;
	}
	...
	if (set) {
		...
		FOREACH_WLC_BAND_ORDERED(rpsi->wlc, bandunit) {
			if (band_type == WLC_BAND_ALL ||
			    (bandunit == BAND_2G_INDEX && band_type == WLC_BAND_2G) ||
			    (bandunit == BAND_5G_INDEX && band_type == WLC_BAND_5G) ||
			    (bandunit == BAND_6G_INDEX && band_type == WLC_BAND_6G)) {
				if (params_cmn->enable[bandunit] != enable) {
					params_cmn->enable[bandunit] = enable;
					_rpsnoa_update_cmnparams_from_band(rpsi, bandunit);
				}
			}
		}
	} else {
		...
		FOREACH_WLC_BAND_ORDERED(rpsi->wlc, bandunit) {
			if (band_type == WLC_BAND_ALL ||
			    (bandunit == BAND_2G_INDEX && band_type == WLC_BAND_2G) ||
			    (bandunit == BAND_6G_INDEX && band_type == WLC_BAND_6G)) {
				out->data[idx].band = wlc_bandunit2bandtype(bandunit);
				out->data[idx].value = params_cmn->enable[bandunit];
				idx++;
			}
		}
		out->hdr.cnt = idx;
	...
	void *result, uint16 *result_len, bool set)
{
	...
	FOREACH_WLC_BAND_ORDERED(rpsi->wlc, bandunit) {
		if (band_type == WLC_BAND_ALL ||
		    ...
		    (bandunit == BAND_6G_INDEX && band_type == WLC_BAND_6G)) {
			out->data[idx].band = wlc_bandunit2bandtype(bandunit);
			out->data[idx].value =	_rpsnoa_get_rps_status(rpsi, bandunit);
			idx++;
		}
	}
	...
	if (set) {
		...
		FOREACH_WLC_BAND_ORDERED(rpsi->wlc, bandunit) {
			if (band_type == WLC_BAND_ALL ||
			    ...
			    (bandunit == BAND_5G_INDEX && band_type == WLC_BAND_5G) ||
			    (bandunit == BAND_6G_INDEX && band_type == WLC_BAND_6G)) {
				_rpsnoa_cmnparams_set(CMD_FLAG_SET, bandunit,
					in->param, params_cmn);
				_rpsnoa_update_cmnparams_from_band(rpsi, bandunit);
			}
		}
		...
		FOREACH_WLC_BAND_ORDERED(rpsi->wlc, bandunit) {
			if (band_type == WLC_BAND_ALL ||
			    ...
			    (bandunit == BAND_5G_INDEX && band_type == WLC_BAND_5G) ||
			    (bandunit == BAND_6G_INDEX && band_type == WLC_BAND_6G)) {
				out->param[idx].band = wlc_bandunit2bandtype(bandunit);
				_rpsnoa_cmnparams_set(CMD_FLAG_GET, bandunit,
...
wlc_rpsnoa_bsscfgs_condition_in_wlc(wlc_info_t *wlc, bool* pending)
{
	...
	if (ap_cnt == 1 && up_bsscfg_cnt == 0) {
		...
		if (pending)
			*pending = (params_cmn->enable[BSSCFG_BANDUNIT(ap_cfg)]) ? TRUE : FALSE;
	} else {
		ap_cfg = NULL;
		if (pending)
			*pending = FALSE;
...
wlc_rpsnoa_schedule(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	brpsi = BSS_RPSNOA_INFO(wlc->rpsnoa, cfg);
	level = params_cmn->level[BSSCFG_BANDUNIT(cfg)];
