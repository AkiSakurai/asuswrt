...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(DUMP_MUTX)
...
#endif /* BCMDBG || BCMDBG_DUMP || DUMP_MUTX */
...
wlc_mutx_scb_state_upd(void *ctx, scb_state_upd_data_t *notif_data)
{
	...
	if ((oldstate & ASSOCIATED) && !SCB_ASSOCIATED(scb)) {
		...
		tp_idx = (SCB_HE_CAP(scb) && HE_MMU_ENAB(wlc->pub)) ?
