 ...
 */
/*
 * $Id: wl_mdns_main.c 781150 2019-11-13 07:11:32Z $
 *
 * mdns_sd_main.c
 *
...
*/
static bonjourProxyIF_t g_bonjourProxyInfo =
{
	BONJOUR_PROXY_SIG,
...
static bonjourProxyIF_t *
BCMRAMFN(get_bonjourProxyInfo)(void)
{
	return &g_bonjourProxyInfo;
}
...
send_packet(wlc_info_t *wlc, uchar *params, uint p_len)
{
	...
	pkt = PKTGET(osh, p_len + wlc->txhroff, TRUE);
	if (pkt == NULL) {
...
wl_mDNS_Init(wlc_mdns_info_t *mdnsi, uint8 *dbase, uint32 dbase_len)
{
	...
	bonjourProxyIF_t *bonjourProxyInfo = get_bonjourProxyInfo();
...
mDNSAddr *
BCMRAMFN(get_AllDNSLinkGroup_v4)(void)
{
	return &AllDNSLinkGroup_v4;
}

mDNSAddr *
BCMRAMFN(get_AllDNSLinkGroup_v6)(void)
{
	return &AllDNSLinkGroup_v6;
}

static bool
BCMRAMFN(is_multi)(u8 *addr, int type)
{
		...
		if (m->omsg->h.numAnswers > 0 || m->omsg->h.numAdditionals)
		{
			if (m->IPv4Available) {
				...
				mDNSSendDNSMessage(mdnsi, m->omsg, responseptr, MulicastV4EthAddr.b,
				                   get_AllDNSLinkGroup_v4(), MulticastDNSPort);
			} else {
			...
			if (m->IPv6Available) {
				...
				mDNSSendDNSMessage(mdnsi, m->omsg, responseptr, MulicastV6EthAddr.b,
				                   get_AllDNSLinkGroup_v6(), MulticastDNSPort);
			} else {
	...
	u8 upper_protocol, u16 frameId, u16 pktLen)
{
	...
	if (upper_protocol == PROTOCOL_UDP) {
		uint16 len = ntoh16(pkt->txp.udp.len);
		pkt->ip.tot_len = hton16(sizeof(ipHdr_t) + len);
	} else {
