 ...
 *
 * $Id: wlc_macdbg.c 780438 2019-10-25 02:31:52Z $
 */
...
do { \
	W_REG(wlc->osh, regname ## _l, ((params) & 0xFFFF)); \
	W_REG(wlc->osh, regname ## _h, (((params) >> 16) & 0xFFFF)); \
	if (use_psm) { \
...
do { \
	W_REG(wlc->osh, regname, ((params) & 0xFFFF)); \
	if (use_psm) { \
		if (PSMX_ENAB(wlc->pub)) \
...
wlc_print_d11reg(wlc_info_t *wlc, int idx, int type, uint16 addr, struct bcmstrbuf *b, uint8 **p)
{
	...
	case D11REG_TYPE_TPL:
		W_REG(osh, D11_XMT_TEMPLATE_RW_PTR(wlc), addr);
	...
	bool w_en, uint32 w_val)
{
	...
	lcnt = pregs->cnt;
	while (lbmp || lcnt) {
		WL_INFORM(("idx %d bitmap %#x cnt %d addr %#x\n",
			idx, lbmp, lcnt, addr));
...
wlc_dump_peruser(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	d11regs_list_t reglist[] = {
		...
		{D11REG_TYPE_IHR16, 0xb14, 0x00000229, 2, 0},
	...
	for (i = 0; i < MAX_NUSR(pub->corerev); i++) {
		W_REG(osh, D11_PSM_REG_MUX(wlc), 0xff00 | i);
		...
		W_REG(wlc, D11_AQMCSB_BASEL(wlc), 0);
		W_REG(wlc, D11_AQMCSB_REQ(wlc), (csb_req & 0xff80) | qmap);
		for (j = 0; j <= 5; j++) {
			if (j == 1 || j == 4) {
				continue;
			}
			csb_req = R_REG(osh, D11_AQMCSB_REQ(wlc));
			W_REG(wlc, D11_AQMCSB_REQ(wlc), (csb_req & 0xfcff) | (2 << 8));
			csb_req = R_REG(osh, D11_AQMCSB_REQ(wlc));
			W_REG(wlc, D11_AQMCSB_REQ(wlc), (csb_req & 0xc3ff) | (j << 10));
...
wlc_dump_sctpl(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	if (D11REV_GE(pub->corerev, 56)) {
		...
		W_REG(osh, smpl_ctrl_reg, scpctl & ~SC_STRT);
		if (use_psm) {
		...
		W_REG(osh, D11_PSM_PHY_CTL(wlc), scpctl & ~PHYCTL_SC_STRT);
	}
...
wlc_macdbg_smpl_capture_optns(wlc_info_t *wlc, wl_maccapture_params_t *params, bool set)
{
	...
	if (params->smpl_ctrl && set) {
		...
		W_REG(wlc->osh, smpl_ctrl_reg, params->smpl_ctrl
...
wlc_macdbg_smpl_capture_set(wlc_info_t *wlc, wl_maccapture_params_t *params)
{
	...
	if (params->cmd == WL_MACCAPT_STOP) {
		if (D11REV_GE(wlc->pub->corerev, 56)) {
			...
			/* Clear start bit */
			AND_REG(wlc->osh, D11_PSM_PHY_CTL(wlc), ~PHYCTL_SC_STRT);
		}
		...
		if (D11REV_GE(wlc->pub->corerev, 56)) {
			...
			W_REG(wlc->osh, D11_PSM_PHY_CTL(wlc), 0);
		}
		return err;
	}
	...
	if (params->la_mode) {
		...
		W_REG(wlc->osh, D11_PSM_GPIOEN(wlc), params->s_mask);
	}
	...
	W_REG(wlc->osh, D11_PSMCoreCtlStat(wlc), d11_regvalue | PSM_CORE_CTL_SS);
	...
	if (D11REV_GE(wlc->pub->corerev, 56)) {
		...
		d11_regvalue |= (PHYCTL_PHYCLKEN | PHYCTL_SC_STRT | PHYCTL_SC_SRC_LB |
				PHYCTL_SC_TRANS_EN);
		W_REG(wlc->osh, D11_PSM_PHY_CTL(wlc), d11_regvalue);
...
#ifdef WLCNT
static void
wlc_dump_muagg_histo(wlc_info_t *wlc, struct bcmstrbuf *b)
{
		muagg_histo_t histo;
...
wlc_dump_d11cnts(wlc_info_t *wlc, struct bcmstrbuf * b)
{
	...
	if (macdbg->pd11print == NULL || macdbg->d11print_sz == 0) {
		return BCME_UNSUPPORTED;
	}
...
wlc_dump_ucode_fatal_phydebug(wlc_info_t *wlc)
{
	...
	if (WLCISACPHY(wlc->band) && (ACREV_GE(wlc->band->phyrev, 32))) {
		...
		for (i = 0; i < gpiosel; i ++) {
			...
			W_REG(osh, D11_PHY_REG_DATA(wlc), i);
...
wlc_dump_ucode_fatal(wlc_info_t *wlc, uint reason)
{
	...
	for (k = 0; k < 8; k++) {
		W_REG(osh, D11_SubrStkRdPtr(wlc), k);
...
#define PHYREG_TXERRLOGCTL 0x11c9
	if (D11REV_GE(corerev, 128)) {
		...
		W_REG(wlc->osh, D11_PHY_REG_ADDR(wlc), PHYREG_TXERRLOGCTL);
