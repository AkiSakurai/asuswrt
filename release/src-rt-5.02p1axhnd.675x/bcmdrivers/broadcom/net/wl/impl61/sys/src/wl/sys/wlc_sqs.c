	...
	taf_scheduler_public_t* taf)
{
	...
	ASSERT(scb_cfp);
	ASSERT(!SCB_DWDS((struct scb*)SCB_CFP_SCB(scb_cfp)));
	...
	if (taf_pkt_units_to_fill < 0) {
		WL_ERROR(("%s: taf_pkt_units_to_fill = %d, actual.released_units = %u, "
			"virtual.released_units = %u, total.released_units %u\n",
			__FUNCTION__, taf_pkt_units_to_fill, taf->ias.actual.released_units,
			taf->ias.virtual.released_units, taf->ias.total.released_units));
		ASSERT(0);
#ifdef BCMDBG
	...
	max_virtual_release = taf->ias.margin + WLC_AMPDU_MPDUS_TO_PKTS(WLC_SQS_WLC(sqs_info),
		(struct scb*)SCB_CFP_SCB(scb_cfp), prio, max_virtual_release);

	real_pkts = wlc_sqs_ampdu_n_pkts(WLC_SQS_WLC(sqs_info), scb_cfp, prio);

	if (max_virtual_release > real_pkts) {
		max_virtual_release -= real_pkts;
	} else {
		uint32 new_release = taf->ias.margin ? taf->ias.margin : 1;

		WL_TAFF(WLC_SQS_WLC(sqs_info), "enough real packets exist (have %u, require %u) "
			"- ask %u pkt\n", real_pkts, max_virtual_release, new_release);
		/* ask some pkts to kick the TAF state machine triggers the same */
		max_virtual_release = new_release;
	}
	virtual_release = wlc_ampdu_pull_packets(WLC_SQS_WLC(sqs_info), SCB_CFP_SCB(scb_cfp),
		prio, (uint16)max_virtual_release, taf);

	if (virtual_release) {
		uint32 pkts_release = WLC_AMPDU_PKTS_TO_MPDUS(WLC_SQS_WLC(sqs_info),
			(struct scb*)SCB_CFP_SCB(scb_cfp), prio, virtual_release);

		taf->ias.virtual.release += virtual_release;
		taf->ias.virtual.released_units += taf_pkt_time_units * pkts_release;

		//WL_TAFF(WLC_SQS_WLC(sqs_info), "pktbyte %u, pkt_time_units %u, units to fill %u, "
		//	"max count %u, (available vpkts %u), virtual release %u, pkt rel %u\n",
		//	pktbytes, taf_pkt_time_units, taf_pkt_units_to_fill, max_virtual_release,
		//	taf->ias.traffic_count_available, virtual_release, pkts_release);
		...
		//WL_TAFF(WLC_SQS_WLC(sqs_info), "no sqs rel, %u requested\n", max_virtual_release);
#ifdef BCMDBG
		taf->complete = TAF_REL_COMPLETE_RESTRICTED;
#endif // endif
		return FALSE;
...
#ifdef PKTQ_LOG
struct pktq * wlc_sqs_taf_get_scb_pktq(void *scbh)
{
	scb_cfp_t * scb_cfp = (scb_cfp_t *)scbh;
	struct scb* scb = SCB_CFP_SCB(scb_cfp);

	return scb_ampdu_prec_pktq(WLC_SQS_WLC(WLC_SQS_G), scb);
}
#endif /* PKTQ_LOG */
#endif /* WLTAF */
