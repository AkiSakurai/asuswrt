 ...
 *
 * $Id: wbd_master_control.c 782279 2019-12-16 10:19:29Z $
 */
...
wbd_master_is_bsta_steering_possible(i5_dm_clients_type *i5_sta)
{
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		if (!I5_IS_MULTIAP_AGENT(i5_device->flags) || !I5_HAS_BH_BSS(i5_device->flags) ||
			...
			WBD_DEBUG("Self Device["MACDBG"] STA["MACDBG"] Skip "
				"Current Device["MACDBG"] Flags[%x]\n",
				MAC2STRDBG(i5_self_device->DeviceId), MAC2STRDBG(i5_sta->mac),
				MAC2STRDBG(i5_device->DeviceId), i5_device->flags);
			continue;
		}
		...
		if (i5_sta_ifr_device == i5_device) {
			WBD_DEBUG("Current Device["MACDBG"] STA["MACDBG"] is backhaul STA and its "
				"from same device so, skip\n",
		...
		while (i5_parent_device && (i5_parent_device != i5_self_device) &&
			...
			WBD_DEBUG("Self Device["MACDBG"] Current Device["MACDBG"] Parent["MACDBG"] "
				"STA IFR Device["MACDBG"]\n",
				MAC2STRDBG(i5_self_device->DeviceId),
				MAC2STRDBG(i5_device->DeviceId),
		...
		if (i5_parent_device == NULL) {
			is_possible = 1;
			WBD_INFO("Current Device["MACDBG"] STA["MACDBG"] Possible\n",
				MAC2STRDBG(i5_device->DeviceId), MAC2STRDBG(i5_sta->mac));
			break;
		} else {
			WBD_INFO("Current Device["MACDBG"] STA["MACDBG"] Parent["MACDBG"] "
				"Not possible to move STA as it forms a loop with same device\n",
				MAC2STRDBG(i5_device->DeviceId), MAC2STRDBG(i5_sta->mac),
				MAC2STRDBG(i5_parent_device->DeviceId));
		}
	}
...
find_next_sta:
	...
	if (!wbd_master_is_bsta_steering_possible(i5_sta)) {
		WBD_INFO("BHSTA["MACDBG"] steering is not possible as it can form a loop. Try for "
			"other STAs\n",
			MAC2STRDBG(i5_sta->mac));
		assoc_sta->flags |= WBD_FLAGS_ASSOC_ITEM_BH_OPT_DONE;
		i5_sta = NULL;
	...
	if ((wbd_master->bounce_cfg_bh.cnt == 0) ||
		...
		WBD_INFO("BKTID[%d] BHSTA["MACDBG"] is bouncing STA till %d "
			"seconds\n",
	...
	if (b_bss_count <= 1) {
		WBD_INFO("For BHSTA["MACDBG"], Number of Backhaul BSS[%d]. Cannot manage backhaul "
