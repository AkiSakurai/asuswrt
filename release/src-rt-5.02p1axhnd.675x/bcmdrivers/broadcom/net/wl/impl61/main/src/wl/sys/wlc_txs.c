 ...
 *
 * $Id: wlc_txs.c 781432 2019-11-20 16:21:26Z $
 */
...
wlc_print_trigtxstatus(wlc_info_t *wlc, tx_status_t *txs, ratesel_tgtxs_t *tgtxs)
{
	...
	if ((txsid & 0xff) == 0) {
		...
		printf("Tgtxs: txsid uidx / nusr uaid rspbmp | lfifo gfcs qnull agglen qosctl | "
		       "ruidx rate rssi uph rtidx txcnt | tgsts tbcnt trcnt isLast\n");
	}
	printf("wl%d: tgtxs %d %d / %d %d %04x | rxs: %2d %2d %d %6d 0x%04x "
	       "| rate: %3d 0x%02x %s 0x%02x %d %d | sts: %d %d %d%s\n",
	       /* section: group */
	       wlc->pub->unit, txsid, uidx,
	       TGTXS_NUSR(txs->status.s1), uaid,
	       TGTXS_GDRSP(txs->status.s2),
	       /* section: rxs */
...
wlc_dotrigtxstatus(wlc_info_t *wlc, tx_status_t *txs)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if ((scb->aid & HE_STAID_MASK) == TGTXS_AID(txs->status.s4)) {
			...
			wlc_musched_upd_ul_stats(wlc->musched, scb, txs);
			wlc_musched_upd_ul_rustats(wlc->musched, scb, txs);
#endif // endif
...
wlc_dotxstatus(wlc_info_t *wlc, tx_status_t *txs, uint32 frm_tx2)
{
...
#if defined(WL_MU_TX) && defined(WL11AX)
	if (HE_ULMU_ENAB(wlc->pub) && (TGTXS_TGFID(txs->status.s1) == TX_STATUS128_TRIGGERTP)) {
	...
	if (pkttag == NULL) {
		...
		if (mbuf_type(m) == MBUF_TYPE_FREE) {
			osl_panic("wlc_dotxstatus: no pkttag, mbuf %p is a free mbuf status:0x%x\n",
				OSL_OBFUSCATE_BUF(m), txs->status.raw_bits);
	...
	if (was_acked) {
		...
		prec_pktlen = pkttotlen(wlc->osh, p) - D11_TXH_LEN_EX(wlc) -
...
fatal:
	if (p != NULL) {
		...
		wlc_txfifo_complete(wlc, queue, pktcnt);
	}
