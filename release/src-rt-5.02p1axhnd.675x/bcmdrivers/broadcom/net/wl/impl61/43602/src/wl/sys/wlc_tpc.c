 ...
 *
 * $Id: wlc_tpc.c 766080 2018-07-24 10:09:01Z $
 */
	...
	wl_chanspec_txpwr_max_t *arg, wl_chanspec_txpwr_max_t *params)
{
	...
	for (cnt = 0; cnt < list->count; cnt++) {
		...
		wlc_phy_txpower_sromlimit(wlc->band->pi, chanspec, (uint8*)&min_srom, srommax, 0);
...
wlc_tpc_rep_build(wlc_info_t *wlc, int8 rssi, ratespec_t rspec, dot11_tpc_rep_t *tpc_rep)
{
	...
	txpwr_backoff = wlc_phy_get_txpwr_backoff((wlc_phy_t *)wlc->band->pi);
...
wlc_tpc_write_brcm_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (WL11K_ENAB(wlc->pub)) {
		uint8 target_pwr = wlc_phy_txpower_get_target_max((wlc_phy_t *)wlc->band->pi);
		uint8 *cp = data->buf;
...
wlc_tpc_alloc_txpower_data(wlc_tpc_info_t *tpc, phy_tx_power_t **power, ppr_t **reg_limits)
{
	...
	if (!(phy_txpwr = (phy_tx_power_t*)MALLOC(wlc->osh, sizeof(phy_tx_power_t)))) {
		err = BCME_NOMEM;
		goto done;
	}

	bzero(phy_txpwr, sizeof(phy_tx_power_t));
...
wlc_tpc_get_current(wlc_tpc_info_t *tpc, void *pwr, uint len, wlc_bsscfg_t *bsscfg)
{
...
#else
	wlc_phy_txpower_get(wlc->band->pi, &user_target, &override);
	...
	if (old_power) {
...
#ifdef WLTXPWR_PER_CORE
			old_power->txpwr_band_max[r] = (uint8)pwr_to_wl->user_target[0];
#else
		...
		wlc_phy_get_tssi_sens_min(wlc->band->pi, tssisen_min);
		wlc_phy_get_tssi_sens_min(wlc->band->pi, tssisen_min);
		...
		if (!hwtable_txpwr)
		{
			...
			/* Check if CCK rate is overridden to SISO
			 * match the target core with antenna diversity selection
			 */
			if (get_cck_override_diversity(wlc, pwr_to_wl->last_tx_ratespec)) {
				for (idx = 0; idx < ARRAYSIZE(power->est_Pout); idx++) {
					if (power->est_Pout[idx]) {
		...
		if ((err = ppr_serialize(power->ppr_board_limits, pserbuf,
			pwr_to_wl->ppr_len, &serlen)) != BCME_OK) {
			err = BCME_ERROR;
			goto free_power;
		}
		...
		pserbuf += pwr_to_wl->ppr_len;
		if ((err = ppr_serialize(reg_limits, pserbuf,
			pwr_to_wl->ppr_len, &serlen)) != BCME_OK) {
			err = BCME_ERROR;
			goto free_power;
		}
