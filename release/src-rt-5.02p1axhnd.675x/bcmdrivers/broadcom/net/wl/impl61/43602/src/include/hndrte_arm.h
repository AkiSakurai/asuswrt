 ...
 *
 * $Id: hndrte_arm.h 443645 2013-12-17 07:00:27Z $
 */
...
#define	hndrte_reg_map(pa, size)	({BCM_REFERENCE(size); (void *)(pa);})
 ...
 */
#define	hndrte_dma_map(va, size)	({BCM_REFERENCE(size); ((uint32)va & ~MEMORY_REMAP);})
#else
#define	hndrte_dma_map(va, size)	({BCM_REFERENCE(size); va;})
#endif /* CONFIG_XIP */
#define	hndrte_dma_unmap(pa, size)	({BCM_REFERENCE(pa); BCM_REFERENCE(size);})

/* Cache support (or lack thereof) */
#if defined(__ARM_ARCH_7R__) && defined(HNDRTE_CACHED)
extern void caches_on(void);
#else
static inline void caches_on(void) { return; }
#endif /* defined(__ARM_ARCH_7R__) && defined(HNDRTE_CACHED) */
