...
#define	DATA_XFER	3
	...
	hndrte_timer_t *timer;	/* to track inactivity */
	...
	hndrte_timer_t *tx_timer; /* zero timer */
...
static void hndgci_uart_timer_handler(hndrte_timer_t *t);
static void hndgci_uart_tx_timer_handler(hndrte_timer_t *t);
...
_gci_write_uart_data(si_t *sih, gci_t *gu)
{
	...
	/* stop timer */
	hndrte_del_timer(gu->uart.timer);
	...
	if (!gu->uart.tx_count) {
		...
		hndrte_del_timer(gu->uart.tx_timer);
	}

	/* start timer */
	hndrte_add_timer(gu->uart.timer, gu->uart.timeout, FALSE);
}
 ...
 */
int
hndgci_uart_config_rx_complete(char delim, int len, int timeout, gci_rx_cb_t rx_cb, void *ctx)
{
	gci_t *gu = gci;
...
hndgci_uart_tx(si_t *sih, void *buf, int count)
{
	...
	if (gci->uart.wake_state == SLEEP) {
		...
		/* start idle timer */
		hndrte_add_timer(gci->uart.timer, gci->uart.timeout, FALSE);
	}

	/* start tx timer */
	if (gci->uart.wake_state == DATA_XFER)
		hndrte_add_timer(gci->uart.tx_timer, 0, TRUE);
...
hndgci_uart_intr_handler(si_t *sih, uint32 intstatus)
{
	...
	if (intstatus & (GCI_INTSTATUS_SRFAF |
		...
		hndrte_del_timer(gci->uart.timer);
	...
	if (intstatus & GCI_INTSTATUS_STFF) {
		...
		hndrte_add_timer(gu->uart.timer, gu->uart.timeout, FALSE);
		if (gu->uart.tx_count)
			hndrte_add_timer(gu->uart.tx_timer, 0, TRUE);
	}
}
...
hndgci_uart_init(si_t *sih, uint8 baudrate_idx)
{
	...
	if (!gci->uart.timer)
		gci->uart.timer = hndrte_init_timer((void *)sih, gci,
	...
	if (!gci->uart.tx_timer)
		gci->uart.tx_timer = hndrte_init_timer((void *)sih, gci,
