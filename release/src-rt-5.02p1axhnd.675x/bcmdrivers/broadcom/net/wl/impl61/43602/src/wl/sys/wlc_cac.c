 ...
 *
 * $Id: wlc_cac.c 696026 2017-04-25 06:03:27Z $
 */
...
BCMATTACHFN(wlc_cac_attach)(wlc_info_t *wlc)
{
	...
	cac->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct cac_scb_acinfo),
		...
		(scb_cubby_dump_t) wlc_cac_scb_acinfo_dump, (void *)wlc, 0);
#else
...
wlc_ccx_ts_rateset_ie(wlc_cac_t *cac, uint8 *ie, int ac)
{
	...
	wlc_rateset_filter(&rateset, &scb->rateset, FALSE, WLC_RATES_CCK_OFDM,
	                   RATE_MASK, wlc_get_mcsallow(wlc, NULL));
	/* filter-out unsupported rates */
	wlc_rate_hwrs_filter_sort_validate(&rateset, &wlc->band->hw_rateset, FALSE,
		wlc->stf->txstreams);
...
#if defined(BCMCCX) || defined(WLFBT)
	if (CCX_ENAB(wlc->pub) || (WLFBT_ENAB(wlc->pub) && (bsscfg->WPA_auth & WPA2_AUTH_FT))) {
#if defined(BCMCCX)
		cac->roamed = FALSE;
#endif /* BCMCCX */
