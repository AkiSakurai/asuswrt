 ...
 *
 * $Id: wlc_lq.c 721118 2017-09-13 09:40:48Z $
 */
...
static const bcm_iovar_t wlc_lq_iovars[] = {
	...
	{"lq_rssi_pen_level", IOV_LQ_RSSI_PENALTY_LEVEL,
	(0), IOVT_INT32, 0
	},
#endif /* STA */
...
extern void BCMATTACHFN(wlc_chanim_mfree)(osl_t *osh, chanim_info_t *c_info);
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(BCMDBG_DUMP_RSSI) || \
	defined(WLTEST)
	...
	void *p, uint plen, void *a, int alen, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_RSSI_DELTA):
		bsscfg->link->rssi_delta = (uint8)int_val;
		break;

	case IOV_GVAL(IOV_RSSI_DELTA):
		*ret_int_ptr = (int32)bsscfg->link->rssi_delta;
		break;

	case IOV_SVAL(IOV_SNR_DELTA):
		bsscfg->link->snr_delta = (uint8)int_val;
		break;

	case IOV_GVAL(IOV_SNR_DELTA):
		*ret_int_ptr = (int32)bsscfg->link->snr_delta;
		break;

	case IOV_SVAL(IOV_LQ_MAX_BCN_LOSS):
		bsscfg->link->max_lq_bcn_loss = (uint32)int_val;	/* in ms */
#ifdef EVENT_LOG_COMPILE
		event_log_tag_start(EVENT_LOG_TAG_BEACON_LOG, EVENT_LOG_SET_WL,
			(bsscfg->link->max_lq_bcn_loss == 0) ? EVENT_LOG_TAG_FLAG_NONE :
			EVENT_LOG_TAG_FLAG_LOG);
#endif /* EVENT_LOG_COMPILE */
		break;

	case IOV_GVAL(IOV_LQ_MAX_BCN_LOSS):
		*ret_int_ptr = (int32)bsscfg->link->max_lq_bcn_loss;	/* in ms */
		break;

	case IOV_SVAL(IOV_LQ_RSSI_PENALTY_LEVEL):
		bsscfg->link->lq_rssi_penalty_level = (int8)int_val;	/* in dBm */
		break;

	case IOV_GVAL(IOV_LQ_RSSI_PENALTY_LEVEL):
		*ret_int_ptr = (int32)bsscfg->link->lq_rssi_penalty_level;
		break;
#endif /* STA */
...
wlc_lq_rssi_snr_noise_reset_ma(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int rssi, int snr, int noise)
{
	...
	link->rssi_qdb = 0;
	link->snr = snr;
	link->lq_last_bcn_time = OSL_SYSUPTIME();
	wlc->noise_lvl = noise;
...
void wlc_lq_update_tbtt(wlc_bsscfg_t *cfg)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_link_qual_t *link = cfg->link;
	...
	else if (CHSPEC_IS2G(cfg->current_bss->chanspec) &&
		!wlc_bmac_coex_antsel(wlc)) {
		/* Skip beacon loss checking if WLAN is not the owner of ant */
#ifdef EVENT_LOG_COMPILE
		EVENT_LOG(EVENT_LOG_TAG_BEACON_LOG, "Beacon(tbtt): COEX");
#endif // endif
		goto skip;
	}
#endif /* !WLC_HIGH_ONLY */
	...
	if ((link->max_lq_bcn_loss == 0) ||
	    ((int)(now - link->lq_last_bcn_time) < (int)link->max_lq_bcn_loss))
		return;

	/* Check for RSSI level. The penalty only kicks in for low RSSI case */
	if ((link->lq_rssi_penalty_level != 0) &&
	    (link->lq_rssi_penalty_level < link->rssi))
		return;

	/* By now, we have missed enough beacons. Need to discount RSSI/SNR */
	if ((link->rssi_delta != 0) && (link->rssi != WLC_RSSI_INVALID)) {
		rssi = link->rssi - link->rssi_delta;

		/* Do not reduce it beyond WLC_RSSI_MINVAL or noise floor */
		if (rssi < WLC_RSSI_MINVAL)
			rssi = WLC_RSSI_MINVAL;
		if ((wlc->noise_lvl != WLC_NOISE_INVALID) && (rssi < wlc->noise_lvl))
			rssi = wlc->noise_lvl;

		if (RSSI_PKT_WIN_DEBUG(link->rssi_pkt_win_sz))
			WL_PRINT(("Apply rssi_delta: last beacon = %08x now = %08x\n",
				link->lq_last_bcn_time, now));
		wlc_lq_rssi_update_ma(cfg, rssi, 0, FALSE);
		cfg->current_bss->RSSI = (int16)link->rssi;
	}

	if ((link->snr_delta != 0) && (link->snr != WLC_SNR_INVALID)) {
		snr = link->snr - link->snr_delta;
		if (snr < WLC_SNR_INVALID)
			snr = WLC_SNR_INVALID;

		if (RSSI_PKT_WIN_DEBUG(link->snr_pkt_win_sz))
			WL_PRINT(("Apply snr_delta: last beacon = %08x now = %08x\n",
				link->lq_last_bcn_time, now));
		wlc_lq_snr_update_ma(cfg, snr, FALSE);
...
skip:
	 ...
	 */
	link->lq_last_bcn_time += cfg->current_bss->beacon_period;
	if ((int)(link->lq_last_bcn_time - now) > 0)
		link->lq_last_bcn_time = now;
}
...
wlc_lq_chanim_us_accum(chanim_info_t* c_info, chanim_cnt_us_t *cur_cnt_us, chanim_acc_us_t *acc_us)
{
	...
	ccastats_us_delta = cur_cnt_us->tx_tm - last_cnt_us->tx_tm;
	acc_us->tx_tm += ccastats_us_delta;
	last_cnt_us->tx_tm =  cur_cnt_us->tx_tm;
...
#ifdef WLCHANIM_US
			chanim_cnt_us->ccastats_cnt[i] = chanim_cnt->ccastats_cnt[i];
			if (i > 0 && i <= 4) {
	...
	chanim_acc_us_t *acc_us)
{
	...
	if (last_cnt->timestamp) {
		if (c_info->chanswitch_overhead) {
			interval = (cur_time - last_cnt->timestamp) -
				(1 + (c_info->chanswitch_overhead/1000));
	...
	wlc_chanim_stats_us_t *cur_stats_us, bool chan_switch)
{
	...
	cur_stats_us->chanim_stats_us.tx_tm = acc_us->tx_tm;
	cur_stats_us->chanim_stats_us.rx_obss = acc_us->ccastats_us[2];
	cur_stats_us->chanim_stats_us.rx_bss = acc_us->ccastats_us[1];
...
wlc_lq_chanim_if_info_setup(chanim_info_t *c_info, chanspec_t chanspec, bool bsscfg)
{
 ...
 */
static void
wlc_lq_chanim_if_info_release(chanim_info_t *c_info, chanspec_t chanspec, bool bsscfg)
{
	chanim_interfaces_t *ifaces = c_info->ifs;
	chanim_interface_info_t *if_info = NULL;
	int i;
                                 ...
                                 chanspec_t prev_chanspec)
{
...
#endif /* WLCHANIM_US */
	if (if_info == NULL) {
		WL_ERROR(("wl%d: %s: if_info is NULL chanspec=%x prev_chanspec=%x\n",
		          wlc->pub->unit, __FUNCTION__, chanspec, prev_chanspec));
		return;
	}
#ifdef WLCHANIM_US
	last_cnt_us = &c_info->last_cnt_us;
...
#ifdef BCMDBG
	wlc_lq_chanim_display(wlc, prev_chanspec, if_info->stats);
#endif // endif
	...
	if_info->acc_us->tx_tm = if_info->stats_us->chanim_stats_us.tx_tm;
	...
	if_info->acc_us->ccastats_us[2] = if_info->stats_us->chanim_stats_us.rx_obss;
	if_info->acc_us->ccastats_us[1] = if_info->stats_us->chanim_stats_us.rx_bss;
...
#ifdef BCMDBG
					wlc_lq_chanim_display(wlc, if_info->chanspec,
					                      if_info->stats);
#endif // endif
				}
			}
...
#ifdef WLCHANIM_US
static int
wlc_lq_chanim_get_stats_us(chanim_info_t *c_info, wl_chanim_stats_us_t* iob, int *len, int cnt)
{
	uint32 count = 0;
	uint32 datalen;
	wlc_chanim_stats_us_t* stats_us = NULL;
