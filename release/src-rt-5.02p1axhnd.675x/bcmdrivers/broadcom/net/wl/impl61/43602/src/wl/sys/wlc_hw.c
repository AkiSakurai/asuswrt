...
BCMATTACHFN(wlc_hw_attach)(wlc_info_t *wlc, osl_t *osh, uint unit, uint *err)
{
	...
	if ((wlc_hw->ltecx_hw = (wlc_hw_ltecx_info_t*)
	     MALLOCZ(osh, sizeof(wlc_hw_ltecx_info_t))) == NULL) {
		*err = 10111;
		goto fail;
	}
...
#if defined(BCMLTECOEX) && !defined(BCMLTECOEX_DISABLED)
	if (wlc_hw->ltecx_hw != NULL)
		MFREE(osh, wlc_hw->ltecx_hw, sizeof(wlc_hw_ltecx_info_t));
#endif /* defined(BCMLTECOEX) && !defined(BCMLTECOEX_DISABLED) */
...
wlc_ap_mute(wlc_info_t *wlc, bool mute, wlc_bsscfg_t *cfg, uint32 user)
{
	...
	else ap = AP_ACTIVE(wlc) && wlc_hw->mute_ap == 0;
