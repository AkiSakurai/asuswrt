 ...
 *
 * $Id: wlc_clm.c 515328 2014-11-14 03:57:29Z $
 */
...
#else
			/* To avoid extension of this field - just modulo comparison) */
			if (blob_num_rates &&
				(blob_num_rates !=
...
extern clm_result_t clm_country_lookup(const ccode_t cc, unsigned int rev, clm_country_t *country)
{
	int ds_idx;
	clm_cc_rev_t cc_rev;
	if (!cc || !country) {
	...
	for (base_ht_idx = 0; base_ht_idx < (int)ARRAYSIZE(base_ht_loc_data); ++base_ht_idx) {
		...
		do {
			...
			if (pg_bw == CLM_BW_80_80) {
				/* This is a BUG! Shall be 'tx_rec +=...'. It is preserved
				 * (in a hope that this ClmAPI would not be used on targets that
				 * support 80+80MHz operation) to avoid ROM invalidation
				 */
...
extern clm_result_t clm_agg_map_iter(const clm_agg_country_t agg, clm_agg_map_t *map, ccode_t cc,
	unsigned int *rev)
{
...
static const  char* clm_get_app_version_string_shim(data_source_id_t ds_id)
{
	return get_data_sources(ds_id)->data ? clm_get_app_version_string(ds_id) : NULL;
}

#define clm_get_app_version_string clm_get_app_version_string_shim
...
static clm_result_t remap_regrev(const ccode_t cc, unsigned *rev, MY_BOOL map_16_to_8,
	clm_result_t result)
{
	...
	for (cc_remap_end = cc_remap + regrev_remap_set->num; cc_remap < cc_remap_end;
			++cc_remap)
	{
	...
	for (num_regrevs = cc_remap[1].index - cc_remap->index; num_regrevs--; ++regrev_remap) {
...
extern clm_result_t clm_country_iter_shim(clm_country_t *country, ccode_t cc, unsigned int *rev);
clm_result_t clm_country_iter_shim(clm_country_t *country, ccode_t cc, unsigned int *rev)
{
	clm_result_t ret = clm_country_iter(country, cc, rev);
	return remap_regrev(cc, rev, FALSE, ret);
}

extern clm_result_t clm_country_lookup_shim(const ccode_t cc, unsigned int rev,
		clm_country_t *country);
clm_result_t clm_country_lookup_shim(const ccode_t cc, unsigned int rev,
		clm_country_t *country)
{
...
extern clm_result_t clm_agg_country_iter_shim(clm_agg_country_t *agg, ccode_t cc,
		unsigned int *rev);
clm_result_t clm_agg_country_iter_shim(clm_agg_country_t *agg, ccode_t cc,
		unsigned int *rev)
{
...
extern clm_result_t clm_agg_map_iter_shim(const clm_agg_country_t agg, clm_agg_map_t *map,
		ccode_t cc, unsigned int *rev);
clm_result_t clm_agg_map_iter_shim(const clm_agg_country_t agg, clm_agg_map_t *map,
		ccode_t cc, unsigned int *rev)
{
...
extern clm_result_t clm_agg_country_lookup_shim(const ccode_t cc, unsigned int rev,
		clm_agg_country_t *agg);
clm_result_t clm_agg_country_lookup_shim(const ccode_t cc, unsigned int rev,
		clm_agg_country_t *agg)
{
	remap_regrev(cc, &rev, TRUE, CLM_RESULT_OK);
...
extern clm_result_t clm_agg_country_map_lookup_shim(const clm_agg_country_t agg,
		const ccode_t target_cc, unsigned int *rev);
clm_result_t clm_agg_country_map_lookup_shim(const clm_agg_country_t agg,
		const ccode_t target_cc, unsigned int *rev)
{
...
#ifdef WL11DFSTW
extern clm_result_t clm_country_flags_shim(const clm_country_locales_t *locales, clm_band_t band,
		unsigned long *ret_flags);
clm_result_t clm_country_flags_shim(const clm_country_locales_t *locales, clm_band_t band,
		unsigned long *ret_flags)
{
	clm_result_t ret = clm_country_flags(locales, band, ret_flags);
	if (ret == CLM_RESULT_OK) {
		locale_data_t locale_data;
		get_loc_def(locales,
			(band == CLM_BAND_2G) ? CLM_LOC_IDX_BASE_2G : CLM_LOC_IDX_BASE_5G,
			&locale_data);
		if (locale_data.def_ptr && ((locale_data.def_ptr[CLM_LOC_DSC_FLAGS_IDX] &
			CLM_DATA_FLAG_DFS_MASK) == CLM_DATA_FLAG_DFS_TW))
		{
