...
#define PARAM_NOT_CONFIGURED		(0xffffffff)
#else
...
add_ssid_llist(wlc_info_t *wlc, ssid_llist_t **head, uint32 ssid_len, uchar *ssid)
{
	...
	item = MALLOC(wlc->osh, sizeof(ssid_llist_t));
	if (item == 0)
		return 0;
...
BCMATTACHFN(wl_anqpo_attach)(wlc_info_t *wlc, wl_gas_info_t *gas)
{
	...
	anqpo = MALLOC(wlc->osh, sizeof(wl_anqpo_info_t));
	if (!anqpo) {
		WL_ERROR(("wl%d: %s: MALLOC failed; total mallocs %d bytes\n",
	...
	bzero(anqpo, sizeof(wl_anqpo_info_t));
	anqpo->wlc = wlc;
	wlc->anqpo = anqpo;
	anqpo->gasi = gas;
