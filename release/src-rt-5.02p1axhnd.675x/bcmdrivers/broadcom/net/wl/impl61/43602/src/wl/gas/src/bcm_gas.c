...
#if defined(BCM_ANQPO_ROM_COMPAT)
#define wl_gas_malloc(drv, size)	malloc(size)
#define wl_gas_free(drv, addr)		free(addr)
#endif // endif
...
#ifdef BCM_ANQPO_ROM_COMPAT
#define BCMGAS_PKT_ID(data)		((uint32)(data))
#define find_gas_entry(packetIdPtr)	((bcm_gas_t *)(*(uint32 *)packetIdPtr))
#else
...
static void query_request_notification(bcm_gas_t *gas, uint16 reqLen, uint8 *req)
{
	...
	if (p->fn) {
		...
		p->fn(p->context, gas, &event);
#else /* BCM_ANQPO_ROM_COMPAT */
