 ...
 *
 * $Id: wlc_pdtof.c 493233 2014-07-25 17:04:23Z $
 */
...
static void wlc_pdtof_hw(pdtof_obj_t *tofobj, bool enter, bool tx)
{
	...
	gTofSeqCnt = 0;
#endif // endif
	wlc_phy_tof(wlc->band->pi, enter, tx, tofobj->tunep->hw_adj, tofobj->seq_en,
	...
	int32 *gd, int32 *adj, bool initiator)
{
	...
	*gd = *adj = 0;
	...
	if (seq_en) {
		...
		if (wlc_phy_seq_ts(wlc->band->pi, nbuffer, tbl_value,
...
static void wlc_pdtof_measure(pdtof_obj_t *tofobj, int cmd)
{
	...
	if (wlc_pdtof_cmd(wlc, tofobj->shmemptr, tof_cmd)) {
		wlc_phy_tof_cmd(wlc->band->pi, tofobj->seq_en);
		if (cmd == TOF_RX) {
			tof_cmd |= TOF_RX;
	...
	int32 *gd, int32 *adj, uint16 id, bool acked, int8 *rssip, bool *discard, bool initiator)
{
	...
	if ((rspcmd & TOF_RSP_MASK) == TOF_SUCCESS) {
		...
		if (wlc_phy_tof_info(wlc->band->pi, &frame_type, &frame_bw, &cfo, &rssi)
			== BCME_OK) {
			if (rssip)
				*rssip = rssi;
			...
			if (event == TOF_EVENT_WAKEUP) {
				if (sm->tof_mode == WL_PROXD_MODE_INITIATOR) {
					...
					if (tofobj->tunep->seq_en &&
					    CHSPEC_IS80(tofobj->paramp->chanspec)) {
					  tofobj->seq_en = TRUE;
					} else {
					  tofobj->seq_en = FALSE;
					}
...
int wlc_tof_seq_iov(pdmthd_if_t* svcif, uint32 flags, int* p_result)
{
	...
	if (p_result) {
		*p_result = 0;
		...
		return wlc_phy_tof_dbg_acphy(wlc->band->pi, (int)flags);
	}
}
#endif // endif
