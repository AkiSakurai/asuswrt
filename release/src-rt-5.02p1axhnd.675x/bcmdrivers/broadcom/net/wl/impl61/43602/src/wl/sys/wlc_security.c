 ...
 *
 * $Id: wlc_security.c 670298 2016-11-15 08:28:09Z $
 */
...
enum {
    KEY_HW_RXI_HWDEC            = 1,
    KEY_HW_RXI_PKTC_FIRST       = 2,
    KEY_HW_RXI_PKTC_LAST        = 4
};
...
#ifdef BRCMAPIVTW
/* XXX
 * Block packets with true replay IVs and allow packets with out-of-order IVs to pass.
 * Broadcom APs without BRF1_PSIVO bit set in their Broadcom IE all have a potential issue
 * where the encrypted packets may go out of order due to a bug in the firmware when the
 * associated STAs go in and out PM mode too often and too fast. To workaround a trace
 * window is used. Packets with IVs smaller than the lower bound of the window are replay
 * packets. Packets with IVs falling in the window and traces show that such packets have
 * been seen before are replay packets. Otherwise packets are good to accept.
 */
...
static bool
wlc_wsec_isreplay(wlc_bsscfg_t *bsscfg, wsec_key_t *key, uint ividx, uint32 iv32, uint16 iv16,
	bool ismulti)
{
	uint16 off_tw;
	bool in_tw;
	...
	if (!AMPDU_HOST_REORDER_ENAB(wlc)) {
		if (!(wlc->brcm_ap_iv_tw_override == AUTO && bsscfg->brcm_ap_iv_tw) &&
...
/* Update the trace window bounds and initialize the trace entries */
static void
wlc_wsec_update_tw(wlc_info_t *wlc, struct wlc_frminfo *f, bool chained)
{
	...
	/* for chained, mark the packets up to the beginning of the chain as recvd */
	if (chained) {
		while (off_tw-- > 0) {
			uint8 *bp = &key->iv_tw[ividx].bmp[off_tw >> 3];
			uint8 bmask = (1 << (off_tw & 7));
			if (*bp & bmask)
				break;
			*bp |= bmask;
		}
	...
	bool ismulti)
{
	...
	WL_NONE(("wl%d: wlc_wsec_isreplay: ividx %d, (iv32, iv16)0x%08x 0x%04x"
		 ...
		 bsscfg->wlc->pub->unit, ividx, iv32, iv16,
		 key->rxiv[ividx].hi, key->rxiv[ividx].lo));
	ret = _wlc_wsec_isreplay(iv32, iv16, (key)->rxiv[ividx].hi, (key)->rxiv[ividx].lo);

#ifdef GTK_RESET
	/* XXX PR80077 WAR: Only allow first multicast packet after gtk is plumbed
	 * to have replay counter mismatch.
	 * All other MC/BC packets needs to have valid replay count
	 */
	if (key->gtk_plumbed && ismulti && ret) {
		WL_WSEC(("wl%d: wlc_wsec_isreplay: replay detected, sync up replay: ividx %d,"
			...
			bsscfg->wlc->pub->unit, ividx, key->rxiv[ividx].hi,
			key->rxiv[ividx].lo, iv32, iv16));
...
#if defined(WLFIPS) || defined(BCMCCMP)
...
wlc_wsec_recvdata_enc_toss(wlc_info_t *wlc, struct wlc_frminfo *f, struct scb *scb)
{
	...
	if (!f->ismulti && BSSCFG_AP(bsscfg) && SCB_LEGACY_WDS(scb) &&
		...
		if (!(scb->flags & SCB_DEAUTH)) {
			 ...
			 */
			wlc_senddeauth(wlc, bsscfg, scb, &scb->ea, &bsscfg->cur_etheraddr,
...
#endif /* AP */
#ifdef WLDPT
	if (DPT_ENAB(wlc->pub) && BSSCFG_IS_DPT(bsscfg) &&
	    (!f->ismulti) && (f->WPA_auth != WPA_AUTH_DISABLED)) {
		/* pairwise key is out of sync with peer, send deauth */
		if (!(scb->flags & SCB_DEAUTH)) {
			wlc_senddeauth(wlc, bsscfg, scb, &scb->ea, &bsscfg->cur_etheraddr,
			               &bsscfg->cur_etheraddr, DOT11_RC_AUTH_INVAL);
...
#define WSEC_RXIV_MAXRANGE	2048

static bool is_cache_issue_detected(wlc_info_t *wlc, struct scb *scb,
	wsec_key_t *key, uint ividx, uint32 iv32, uint16 iv16)
{
	wlc_bsscfg_t *bsscfg;
	bool inrange = TRUE;
	uint64 new_iv = (iv16 | (iv32<<16));
	uint64 cur_iv, last_iv;

	cur_iv = (key->rxiv[ividx].hi << 16) | key->rxiv[ividx].lo;
	last_iv = (key->rxiv[key->last_rxiv].hi << 16) | key->rxiv[key->last_rxiv].lo;
	if ((new_iv > (cur_iv + WSEC_RXIV_MAXRANGE)) &&
		(new_iv > (last_iv + WSEC_RXIV_MAXRANGE))) {
		inrange = FALSE;
	}
	bsscfg = SCB_BSSCFG(scb);
	ASSERT(bsscfg != NULL);
	...
	if (!inrange) {
		WL_ERROR(("wl%d.%d: %s (%s) iv16 0x%04x iv32 0x%08x"
			", (expected) lo 0x%04x hi 0x%08x (ividx %d)\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
			(key->algo == CRYPTO_ALGO_TKIP)?"TKIP":"AES",
			iv16, iv32, key->rxiv[ividx].lo, key->rxiv[ividx].hi, ividx));
	}
	...
	struct wlc_frminfo *f, uint8 prio)
{
	...
	hw_rxi_flags |= (KEY_HW_RXI_PKTC_FIRST | KEY_HW_RXI_PKTC_LAST);
	if ((WLPKTTAG(f->p)->flags) & WLF_RX_PKTC_NOTFIRST)
		hw_rxi_flags &= ~KEY_HW_RXI_PKTC_FIRST;
	if ((WLPKTTAG(f->p)->flags) & WLF_RX_PKTC_NOTLAST)
		hw_rxi_flags &= ~KEY_HW_RXI_PKTC_LAST;

	WLPKTTAG(f->p)->flags &= ~(WLF_RX_PKTC_NOTFIRST | WLF_RX_PKTC_NOTLAST);

	/*
	 * XXX: Fix this
	 * store a flag as part of the bssconfig to figure out the group cyper
	 * All cryptos except WAPI/SMS4 use Byte 3 of the IV to figure out the group key
	 * but WAPI uses Byte 0..bit 0 of the IV to figure out the index
	 */
	/* Move the checks inside wlc_key_lookup here to skip the function call
	 * and save some cpu cycles during fast data path.
	 */
	if (!f->ismulti && WSEC_SCB_KEY_VALID(scb) &&
		scb->key->algo == CRYPTO_ALGO_AES_CCM) {
...
#ifdef BCMWAPI_WPI
	/*
	 * XXX: sms4 encrypted broadcast packets are not handled by the ucode properly
	 * Please refer to PR80838 for details.
	 * Although ucode sets this flag saying DEC attempted, it is not really done
	 * this workaround makes the packets go thorugh software decryption
	 */
...
#endif // endif
	/* Check for replays unless using WPA_AUTH_NONE
	 * WPA FIXME: how do we tell if iv32 has wrapped, and do we care?
	 * Wrap will look like a replay here.
	 */
	switch (f->key->algo) {
	case CRYPTO_ALGO_TKIP:
		...
		if ((f->WPA_auth & WPA_AUTH_NONE) == 0 &&
		    is_cache_issue_detected(wlc, scb, f->key, ividx, iv32, iv16)) {
			return FALSE;
		}
#endif /* defined(BCM47XX_CA9) && defined(WL_PL310_WAR) */
		if ((f->WPA_auth & WPA_AUTH_NONE) == 0 &&
		    wlc_wsec_isreplay(bsscfg, f->key, ividx, iv32, iv16, f->ismulti)) {
			...
			if (BSSCFG_STA(bsscfg)) {
				...
				if (CCX_ENAB(wlc->pub) && WLPKTFLAG_PMF(WLPKTTAG(f->p)))
					WLCNTINCR(wlc->ccx->mgmt_cnt.tkipmgmtreplayerr);
				else
#endif // endif
				{
	...
	case CRYPTO_ALGO_AES_CCM:
		...
		if (!(BSSCFG_IBSS(bsscfg) && (bsscfg->WPA_auth & WPA_AUTH_NONE)) &&
			...
			WLPKTTAG(f->p)->flags |= WLF_RX_PKTC_ISREPLAY;
...
#if defined(BCMCCX) && defined(CCX_SDK)
				if (CCX_ENAB(wlc->pub) && WLPKTFLAG_PMF(WLPKTTAG(f->p)))
					WLCNTINCR(wlc->ccx->mgmt_cnt.ccmpmgmtreplyerr);
				else
#endif // endif
				{
	...
	if (WLC_SW_KEYS(wlc, bsscfg) ||
		...
		/* decrypt */
		/* WPA FIXME: need to better differentiate between "Init Vector"
		 * and IV len (fig. 46 in 802.11 spec
		 */
		switch (f->key->algo) {
		case CRYPTO_ALGO_TKIP:
...
#ifndef LINUX_CRYPTO
#ifdef BCMCCX
			if (CCX_ENAB(wlc->pub) && WSEC_CKIP_KP_ENABLED(bsscfg->wsec)) {
				uint8 toDsFromDs = (f->fc & (FC_TODS|FC_FROMDS)) >> FC_TODS_SHIFT;
				uint8 kp[CKIP_KEY_SIZE];

				WL_WSEC((" CKIP\n"));

				/* pbody currently points to the IV */
				CKIP_key_permute(kp, f->key->data, toDsFromDs, f->pbody);
				prepare_key(kp, CKIP_KEY_SIZE, &ks);
				rc4(f->pbody + f->key->iv_len, f->body_len - f->key->iv_len, &ks);

				/* check ICV */
				if (CRC32_GOOD_VALUE == hndcrc32(f->pbody + f->key->iv_len,
					f->body_len - f->key->iv_len, CRC32_INIT_VALUE)) {
					f->rxh->RxStatus1 &= ~RXS_DECERR;
					WL_WSEC((" SW CKIP checking ICV OK\n"));
				} else {
					f->rxh->RxStatus1 |= RXS_DECERR;
					WL_WSEC((" SW CKIP checking ICV error\n"));
				}
				break;
			}
#endif /* BCMCCX */
...
#if defined BCMCCMP || defined WLFIPS
		case CRYPTO_ALGO_AES_CCM:
			if (!(f->pbody[3] & DOT11_EXT_IV_FLAG)) {
...
#ifdef WLFIPS
			/* if use FIPS, pass packet to FIPS decryption function */
			if (FIPS_ENAB(wlc)) {
				WL_WSEC((" FIPS decryption\n"));
				nonce_1st_byte = wlc_get_nonce_1st_byte(scb, f->h, f->p, FALSE);
				if (wl_fips_decrypt_pkt(wlc->fips, f->key->id, f->h, f->len,
					nonce_1st_byte)) {
					WL_ERROR((" FIPS CCMP decryption failed\n"));
#if defined(BCMCCX) && defined(CCX_SDK)
					if (CCX_ENAB(wlc->pub) && WLPKTFLAG_PMF(WLPKTTAG(f->p)))
						WLCNTINCR(wlc->ccx->mgmt_cnt.ccmpmgmtdecrypterr);
					else
#endif // endif
					{
						WLCNTINCR(wlc->pub->_cnt->rxundec);
...
#if defined(BCMCCX) && defined(CCX_SDK)
				if (CCX_ENAB(wlc->pub) && WLPKTFLAG_PMF(WLPKTTAG(f->p)))
					WLCNTINCR(wlc->ccx->mgmt_cnt.tkipmgmticverr);
				else
#endif // endif
				{
	...
	if (f->key && !ext_tkip) {
		if (!f->isamsdu) {
			/* SPLIT RX: integrity check sum could be in host and not in TCM */
			/* So update host frag len */
			PKTFRAG_TRIM_TAILBYTES(osh, f->p, f->key->icv_len);
			if (!PKTFRAGUSEDLEN(osh, f->p)) {
				f->len -= f->key->icv_len;
			}
		} else {
			void *pt = pktlast(osh, f->p);
			PKTFRAG_TRIM_TAILBYTES(osh, pt, f->key->icv_len);
		}
...
#ifdef BRCMAPIVTW
	/* update trace window, if needed */
	if (((f->key->algo == CRYPTO_ALGO_AES_CCM) || (f->key->algo == CRYPTO_ALGO_TKIP)) &&
	    ((f->WPA_auth & WPA_AUTH_NONE) == 0)) {
		bool chained;
		 ...
		 */
		chained = (hw_rxi_flags & KEY_HW_RXI_PKTC_LAST) &&
			!(hw_rxi_flags & KEY_HW_RXI_PKTC_FIRST);
		if (hw_rxi_flags & (KEY_HW_RXI_PKTC_FIRST | KEY_HW_RXI_PKTC_LAST))
			wlc_wsec_update_tw(wlc, f, chained);
	}
#endif // endif
	return TRUE;
}
	...
	struct scb *scb, wsec_key_t *key)
{
...
#ifdef BCMCCX
		if (CCX_ENAB(wlc->pub) && WSEC_CKIP_KP_ENABLED(cfg->wsec)) {
			uint8 toDsFromDs = (h->fc & (FC_TODS|FC_FROMDS)) >> FC_TODS_SHIFT;
			uint8 kp[CKIP_KEY_SIZE];
			WL_WSEC(("wl%d: wlc_wsec_sw_encrydata: CKIP\n", wlc->pub->unit));
			CKIP_key_permute(kp, key->data, toDsFromDs, piv);
	...
	case CRYPTO_ALGO_AES_CCM:
		PKTSETLEN(osh, p, PKTLEN(osh, p) + key->icv_len);
#ifdef WLFIPS
		/* if use FIPS, pass packet to FIPS encryption function */
		if (FIPS_ENAB(wlc)) {
			WL_WSEC(("wl%d: wlc_wsec_sw_encrydata: FIPS encryption\n",
				wlc->pub->unit));
			nonce_1st_byte = wlc_get_nonce_1st_byte(scb, h, p, FALSE);
			if (wl_fips_encrypt_pkt(wlc->fips, key->id, h, len,
				nonce_1st_byte)) {
				WL_ERROR(("wl%d: wlc_wsec_sw_encrydata: FIPS CCMP encryption"
					" failed\n", wlc->pub->unit));
				return FALSE;
			}
		}
...
#if defined(BCM47XX_CA9) && defined(WL_PL310_WAR)
	/* update latest received iv index. */
	f->key->last_rxiv  = f->ividx;
#endif // endif
}
...
wlc_wsec_miccheck(wlc_info_t *wlc, osl_t *osh, struct scb *scb, struct wlc_frminfo *f)
{
	...
	case CRYPTO_ALGO_TKIP: {
		 ...
		 */
		/* SPLIT RX: MIC is in host rather than in TCM */
		if (PKTFRAGUSEDLEN(osh, f->p)) {
			f->len = len = PKTLEN(osh, f->p);
			f->body_len = body_len;
		} else
		{
		...
		if (D11REV_GE(wlc->pub->corerev, 13) && (f->rxh->RxStatus2 & RXS_TKMICATMPT) &&
			...
			/* SPLIT RX: MIC is in host rather than in TCM */
			/* So update host frag len */
			PKTFRAG_TRIM_TAILBYTES(osh, f->p, TKIP_MIC_SIZE);
		} else {
			uint32 l, r, rxl, rxr;
			uint body_eom_len;
			uchar *micp;
		...
		if (mic_error) {
			if (BSSCFG_STA(bsscfg)) {
...
#if defined(BCMCCX) && defined(CCX_SDK)
				if (CCX_ENAB(wlc->pub) && WLPKTFLAG_PMF(WLPKTTAG(f->p)))
					WLCNTINCR(wlc->ccx->mgmt_cnt.tkipmgmtlocalmicfail);
				else
#endif // endif
				{
				...
				if (bsscfg->tkip_countermeasures) {
					/* already in countermeasure quiet period */
					return FALSE;
				}

				if (bsscfg->tk_cm_dt == 0) {
					WL_WSEC(("wl%d: wlc_recvdata: TKIP countermeasures:"
						" indicating MIC failure...starting detect"
						" timer\n",
						wlc->pub->unit));

					WLCNTINCR_MIN(wlc->pub->_cnt->tkipcntrmsr);
					/* start detect timer */
					bsscfg->tk_cm_dt = WPA_TKIP_CM_DETECT;
				} else {
					WL_WSEC(("wl%d: wlc_recvdata: TKIP countermeasures:"
					" indicating 2nd MIC failure before detect"
						" timer timeout...activate"
						" countermeasures\n",
						wlc->pub->unit));

					WLCNTINCR_MIN(wlc->pub->_cnt->tkipcntrmsr);
					/* flush the common txq */
					/* TODO : which queue[s] must be flushed ????
					* pktq_flush(wlc->pub->osh, &wlc->txq, TRUE);
					*/
	...
	case CRYPTO_ALGO_WEP1:
	case CRYPTO_ALGO_WEP128:
		if (CCX_ENAB(wlc->pub) && WSEC_CKIP_MIC_ENABLED(bsscfg->wsec)) {
			uint32 mic;
			uint32 rxSEQ;
			int MMH_encap;
			int key_index = f->key_index;
			bool rxseq_check;

			WL_WSEC(("wl%d: %s: CKIP MIC check\n", wlc->pub->unit, __FUNCTION__));

			/* check CKIP MIC (pbody start from CKIP_LLC_SNAP) */
			MMH_encap = wsec_ckip_mic_check(key->data, (uint8 *)(f->da),
				(uint8 *)(f->sa), pbody, body_len, (uint8 *)&mic);
			if (!MMH_encap) {
				wsec_key_t *prev_key = WSEC_KEY(wlc, key_index ^ 1);

				WL_ERROR((" CKIP MIC failure using key %d, got MIC=0x%08x,"
					" expected 0x%08x from %s\n", key_index,
					*(uint32 *)(pbody + 8), mic,
					bcm_ether_ntoa(&h->a2, addrstr)));

				/* check with previous key, if available */
				if (!prev_key) {
					WL_ERROR((" no previous key...CKIP MIC failure\n"));
					return FALSE;
				}
				if (!wsec_ckip_mic_check(prev_key->data, (uint8 *)(f->da),
					(uint8 *)(f->sa), pbody, body_len, (uint8 *)&mic)) {
					WL_ERROR((" check with previous key %d, got MIC=0x%08x,"
						"expected 0x%08x, CKIP MIC failure\n",
						key_index ^ 1, *(uint32 *)(pbody + 8), mic));
					return FALSE;
				}
				WL_ERROR((" check with previous key %d...CKIP MIC success\n",
					key_index ^ 1));
			}

			if (MMH_encap == -1) {
				WL_WSEC((" IAPP frame: no CKIP MIC\n"));
				break;
			}

			WL_WSEC((" CKIP MIC success, check SEQ \n"));

			/* check CKIP SEQ */
			rxSEQ = ntoh32(*(uint32 *)(pbody + CKIP_LLC_SNAP_LEN + CKIP_MIC_SIZE));
			if (f->ismulti)
				rxseq_check = bcm_ckip_rxseq_check(rxSEQ,
					&key->mrxseq_base, &key->mrxseq_bitmap);
			else
				rxseq_check = bcm_ckip_rxseq_check(rxSEQ,
					&key->rxseq_base, &key->rxseq_bitmap);

			if (!rxseq_check) {
				WL_ERROR((" CKIP SEQ failure\n"));
				return FALSE;
			}
			WL_WSEC((" CKIP SEQ success\n"));
		}
		break;
#endif	/* BCMCCX */
	default:
	...
	uint32 event_type, bool group, wsec_key_t *key, bool flush_txq)
{
	...
	if (WLEXTSTA_ENAB(wlc->pub)) {
		...
		if (key) {
			e->event.auth_type = WSEC_KEY_INDEX(wlc, key);
			e->event.flags = (e->event.auth_type < WSEC_MAX_DEFAULT_KEYS)
				? WLC_EVENT_MSG_GROUP : 0;
		} else {
			/* XXX - error without key case. key index is not used in event
			 * handling anyway
			 */
			e->event.auth_type = 0;
			e->event.flags = 0;
...
#ifdef EXT_STA
		if (WLEXTSTA_ENAB(wlp) && BSSCFG_HAS_NATIVEIF(cfg)) {
...
wlc_wsec_rx_tkmic_offset(wlc_pub_t *wlp, wlc_bsscfg_t *cfg, struct scb *scb)
{
	if ((BSSCFG_AP(cfg) &&
#ifdef EXT_STA
		!(WLEXTSTA_ENAB(wlp) && BSSCFG_HAS_NATIVEIF(cfg)) &&
#endif /* EXT_STA */
	...
	uint8 prio, uint frag_length)
{
	...
	if (to_full > 0) {
		/* WPA FIXME: need to track overall length of fragments or
		 * recalculate them, and pass that length to mic_eom, since the
		 * overall frame length (not just this frag len) needs to be
...
#endif /* WLFIPS || BCMCCMP */

/* Functions moved from wlc.c as part of pktfetch for 802.1x frames in splitrx mode
 * since wlc.c is not accomodating any new functions
*/
#ifdef BCMSPLITRX
/* Probably need to move this definition out of here */
bool
wlc_sw_decrypt_pktfetch_required(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
struct scb *scb, struct wlc_frminfo *f)
{
	wsec_key_t *key = NULL;
	int key_index;
...
#ifdef BCMWAPI_WPI
	if (scb->key && scb->key->algo == CRYPTO_ALGO_SMS4)
		key_index = f->pbody[0] & 0x01;
	else
#endif /* BCMWAPI_WPI */
		key_index = f->pbody[3] >> DOT11_KEY_INDEX_SHIFT;
	key = wlc_key_lookup(wlc, scb, bsscfg, key_index, f->ismulti);

	/* Do nothing if we don't have key */
	if (!key)
		return FALSE;
	 ...
	 */
	if (f->rxh->RxStatus1 & RXS_DECATMPT) {
		if (key && (key->algo == CRYPTO_ALGO_SMS4) && wlc->pr80838_war &&
			(WSEC_KEY_INDEX(wlc, key) < WSEC_MAX_DEFAULT_KEYS))
			f->rxh->RxStatus1 &= ~RXS_DECATMPT;
	}
#endif /* BCMWAPI_WPI */
...
void
wlc_recvdata_schedule_pktfetch(wlc_info_t *wlc, struct scb *scb, wlc_frminfo_t *f,
	bool promisc_frame)
{
	wlc_eapol_pktfetch_ctx_t *ctx = NULL;
	struct pktfetch_info *pinfo = NULL;

	pinfo = MALLOC(wlc->osh, sizeof(struct pktfetch_info));
	if (!pinfo) {
		WL_ERROR(("%s: Out of mem: Unable to alloc pktfetch_info!\n", __FUNCTION__));
		goto error;
	}

	ctx = MALLOC(wlc->osh, sizeof(wlc_eapol_pktfetch_ctx_t));
	if (!ctx) {
	...
	/* Reset pktfetch flags */
	WLPKTTAG(f->p)->flags   &= ~WLF_RXPKTFETCH_CHECK_DONE;
	WLPKTTAG(f->p)->flags   &= ~WLF_NEED_RXPKTFETCH;
