 ...
 *
 * $Id: wlc_apps.c 766795 2018-08-14 12:26:58Z $
 */

/**
 * @file
 * @brief
 * XXX Twiki: [WlDriverPowerSave]
 */
...
/*
 * XXX: PSQ pktq let needs to handle large no of ampdus released and requeued.
 */
#define PSQ_MAX_QUEUE_SIZE	512
#ifdef SHARED_FRAG_POOL_LEN
#define PSQ_PKTQ_LEN_DEFAULT	(MIN(PSQ_MAX_QUEUE_SIZE, SHARED_FRAG_POOL_LEN))
#elif defined(SHARED_POOL_LEN)
#define PSQ_PKTQ_LEN_DEFAULT	(MIN(PSQ_MAX_QUEUE_SIZE, SHARED_POOL_LEN))
#else
#define PSQ_PKTQ_LEN_DEFAULT	(PSQ_MAX_QUEUE_SIZE)
#endif // endif
#define PSQ_PKTQ_LEN_BCMC	(PSQ_PKTQ_LEN_DEFAULT/2)
...
struct apps_scb_psinfo {
	...
	bool		ext_qos_null;	/* Send extra QoS NULL frame to indicate EOSP
					 ...
					 */
	bool		in_ps_off;	/* in the middle turning ps off */
#ifdef PROP_TXSTATUS
...
/* Accessor function of above array for ROMmability */
static uint BCMRAMFN(wlc_acbitmap_to_precbitmap)(uint ab)
{
	return wlc_acbitmap2precbitmap[ab & 0xf];
}

/* Map AC bitmap to precedence bitmap */
#define WLC_ACBITMAP_TO_PRECBITMAP(ab)  wlc_acbitmap_to_precbitmap(ab)
...
{
	struct wlc_pps_info*  pps_info = MALLOC(wlc->osh, sizeof(*pps_info));
	...
	bzero((char *)pps_info, sizeof(*pps_info));
...
BCMATTACHFN(wlc_apps_attach)(wlc_info_t *wlc)
{
	...
	wlc_psinfo->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct apps_scb_psinfo),
		...
		wlc_apps_scb_psinfo_dump, (void *)wlc, 0);
#else
...
#endif // endif
	/* PS state init */
	pktq_init(&scb_psinfo->psq, WLC_PREC_COUNT,
		(SCB_ISMULTI(scb)) ?
			PSQ_PKTQ_LEN_BCMC :
			PSQ_PKTQ_LEN_DEFAULT);
	return 0;
}
...
wlc_apps_scb_ps_off(wlc_info_t *wlc, struct scb *scb, bool discard)
{
	 ...
	 * just return.
	 */
	if (scb_psinfo->in_ps_off)
		return;
	scb_psinfo->in_ps_off = TRUE;
	...
	if (bss_info->ps_nodes == 0 && !(bsscfg->flags & WLC_BSSCFG_NOBCMC)) {
		...
		} else if (!BSSCFG_IBSS(bsscfg) || !AIBSS_ENAB(wlc->pub)) {
			bcmc_scb->PS = FALSE;
#ifdef PSPRETEND
...
wlc_apps_scb_ps_on(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (PROP_TXSTATUS_ENAB(wlc->pub)) {
		...
		if (wlc->wlfc_vqdepth > 0 && CREDIT_INFO_UPDATE_ENAB(wlc->pub))
			wlfc_psmode_request(wlc->wl, scb->mac_address_handle,
			((vqdepth == 0) ? wlc->wlfc_vqdepth : vqdepth),
...
#endif /* MBSS */
		/* IBSS BCMC SCB is always in PS state, uCode takes
		 * care of blocking/un blocking BCMC FIFO after ATIM window
		 */
...
wlc_apps_process_pend_ps(wlc_info_t *wlc)
{
	...
	/* XXX PMQ todo : we should keep track of pkt pending for each scb and wait for
...
static void
wlc_apps_ps_free_pkt(wlc_info_t *wlc, struct scb *scb, void *pkt)
{
	/*
	 * Just packet freeing is not sufficient, packets in wlc queue might be
	 * tagged with ampdu. Let's notify AMPDU module about freeing.
	 */
#ifdef WLAMPDU
...
/* Free pending PS packets for this flowid */
void
wlc_apps_ps_flush_flowid(wlc_info_t *wlc, struct scb *scb, uint16 flowid)
{
	...
	WL_PS(("wl%d.%d: wlc_apps_ps_flush: flushing %d packets for %s aid %d flowid %d\n",
	...
	PKTQ_PREC_ITER(q, prec) {
		...
		while (pkt) {
			if (!SCB_ISMULTI(scb)) {
				UNSIGNEDDEC(wlc_psinfo->ps_deferred);
			}
...
wlc_apps_ps_flush_mchan(wlc_info_t *wlc, struct scb *scb)
{
	...
	/* Don't care about dequeue precedence */
	 while ((pkt = pktq_deq(&scb_psinfo->psq, &prec))) {
		...
		wlc_apps_ps_free_pkt(wlc, scb, pkt);
...
wlc_apps_ps_send(wlc_info_t *wlc, struct scb *scb, uint prec_bmp, uint32 extra_flags)
{
	 ...
	 */
	/*
	 * XXX: It's possible that hardware resources may be available for
...
wlc_apps_ps_enq(void *ctx, struct scb *scb, void *pkt, uint prec)
{
	...
	if (PROP_TXSTATUS_ENAB(wlc->pub) && BCMPCIEDEV_ENAB(wlc->pub)) {
		struct apps_wlc_psinfo *wlc_psinfo;
		struct apps_scb_psinfo *scb_psinfo;
		ASSERT(wlc);
		ASSERT(scb);
...
static void wlc_apps_move_to_psq(wlc_info_t *wlc, struct pktq *txq, struct scb* scb)
{
	...
	PKTQ_PREC_ITER(txq, prec) {
		...
		while (pktq_ppeek(txq, prec) != head_pkt) {
			...
			/* XXX: enqueueing at the same prec may create a remote
...
bool
wlc_apps_pvb_chkupdate_from_host(wlc_info_t *wlc, struct scb *scb, bool op)
{
...
#ifdef DONGLEBUILD
				if ((pkt = wlc_sendnulldata(wlc, scb->bsscfg, &scb->ea, 0,
						WLF_PSDONTQ, PRIO_8021D_BE)) == NULL) {
#else
				if (wlc_sendnulldata_ex(wlc, scb->bsscfg, &scb->ea, 0,
...
wlc_bss_apps_tbtt_update(wlc_bsscfg_t *cfg)
{
	...
	/* XXX: if touching all the PS scbs is too inefficient then we
...
wlc_apps_tbtt_update(wlc_info_t *wlc)
{
	...
	/* XXX: if touching all the PS scbs multiple times is too inefficient
...
wlc_apps_ps_prep_mpdu(wlc_info_t *wlc, void *pkt)
{
	...
	if (ETHER_ISMULTI(txh_info.TxFrameRA)) {
		...
		/* XXX - There is a hack to send uni-cast P2P_PROBE_RESP frames using bsscfg's
	...
	*/
	...
	if (pktq_empty(&scb_psinfo->psq) || wlc_apps_apsd_ndelv_count(wlc, scb) == 0) {
		...
		if ((pkt = wlc_sendnulldata(wlc, scb->bsscfg, &scb->ea, 0, WLF_PSDONTQ,
		                            PRIO_8021D_BE)) == NULL) {
#else
		if (wlc_sendnulldata_ex(wlc, scb->bsscfg, &scb->ea, 0, WLF_PSDONTQ,
...
wlc_apps_tim_len(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	wlc_psinfo = wlc->psinfo;
	bss_psinfo = &wlc_psinfo->bss_info[WLC_BSSCFG_IDX(cfg)];
...
#else
	if (wlc_sendnulldata(wlc, scb->bsscfg, &scb->ea, 0,
		(WLF_PSDONTQ | WLF_APSD), prio) == FALSE) {
#endif /* DONGLEBUILD */
                      ...
                      struct dot11_header *h, bool last_frag)
{
	...
	if (last_frag &&
		...
		/* XXX JQL: should we keep going in case there are on-the-fly
...
#ifdef WLTDLS
		/* XXX Trigger frames are delivered in PTI interval, because
...
void
wlc_apps_bss_wd_ps_check(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	struct apps_wlc_psinfo *wlc_psinfo = wlc->psinfo;
	struct scb *bcmc_scb;
	apps_bss_info_t *bss_info;
	...
	if (!(bsscfg->flags & WLC_BSSCFG_NOBCMC)) {
		...
		ASSERT(bcmc_scb->bsscfg == bsscfg);
		bss_info = &wlc_psinfo->bss_info[WLC_BSSCFG_IDX(bsscfg)];
		scb_psinfo = SCB_PSINFO(wlc_psinfo, bcmc_scb);

		if ((bss_info->ps_nodes == 0) && (bcmc_scb->PS == TRUE)) {
			if (MBSS_ENAB(wlc->pub) &&
				(scb_psinfo->bcmc_stuck_fid != bsscfg->bcmc_fid_shm)) {
				scb_psinfo->bcmc_stuck_fid = bsscfg->bcmc_fid_shm;
				scb_psinfo->bcmc_stuck_cnt = 1;
			} else {
				scb_psinfo->bcmc_stuck_cnt++;
			}
		} else  {
			scb_psinfo->bcmc_stuck_fid = INVALIDFID;
			scb_psinfo->bcmc_stuck_cnt = 0;
		}

		if (scb_psinfo->bcmc_stuck_cnt > BCMC_STUCK_THRESHOLD) {
			scb_psinfo->bcmc_stuck_fid = INVALIDFID;
			scb_psinfo->bcmc_stuck_cnt = 0;

			WL_ERROR(("%s:bcmc_fid = 0x%x  bcmc_fid_shm = 0x%x  mc_fifo_pkts = %d. "
				"Flushing BCMC queue and releasing PM mode for it\n",
				__FUNCTION__, bsscfg->bcmc_fid, bsscfg->bcmc_fid_shm,
				bsscfg->mc_fifo_pkts));
#ifdef WL_MULTIQUEUE
			wlc->txfifo_detach_pending = TRUE;
			wlc_bmac_tx_fifo_sync(wlc->hw, (1 << TX_BCMC_FIFO), FLUSHFIFO);
#endif /* WL_MULTIQUEUE */
			wlc_apps_ps_flush(wlc, bcmc_scb);
		}
	}
}
...
wlc_apps_process_pspretend_status(wlc_info_t *wlc, struct scb *scb, bool pps_recvd_ack)
{
	...
	* So in that case, set scb->PS_pend to False, so the scb will get out of PM
	* in wlc_apps_process_pend_ps() after txfifo draining is done.
	*/
...
#if defined(PROP_TXSTATUS)
	if (PROP_TXSTATUS_ENAB(wlc->pub)) {
		bcm_bprintf(b, ", MAC_handle_bitmap:0x%08x",
			((wlfc_mac_desc_handle_map_t*)wlc->wlfc_data)->bitmap);
	}
#endif /* PROP_TXSTATUS */
