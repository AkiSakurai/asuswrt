...
static void *
pciedngl_probe(hndrte_dev_t *rtedev, void *regs, uint bus, uint16 device, uint coreid, uint unit);
static int pciedngl_open(hndrte_dev_t *drv);
static int pciedngl_close(hndrte_dev_t *drv);
static int
pciedngl_send(hndrte_dev_t *src, hndrte_dev_t *drv, struct lbuf *lb);
static int
pciedngl_ioctl(hndrte_dev_t *drv, uint32 cmd, void *buf, int len, int *used, int *needed, int set);
static void pciedngl_isr(hndrte_dev_t *drv);
int pciedngl_bus_binddev(void *bus, void *dev);
...
static hndrte_devfuncs_t pciedngl_funcs = {
...
hndrte_dev_t pciedngl_dev = {
	name:		"pciedngldev",
	funcs:		&pciedngl_funcs
};
...
static int
pciedngl_open(hndrte_dev_t *drv)
{
...
static int
pciedngl_close(hndrte_dev_t *drv)
{
	struct dngl_bus *pciedngl = (struct dngl_bus *)drv->softc;
	printf("%s called\n", __FUNCTION__);
...
pciedngl_probe(hndrte_dev_t *rtedev, void *regs, uint bus, uint16 device, uint coreid, uint unit)
{
	...
	if (hndrte_add_isr(0, coreid, unit, (isr_fun_t)pciedngl_isr, rtedev, bus)) {
			printf("pcidongle_probe:hndrte_add_isr failed\n");
			goto fail;
	}
#endif // endif
...
int
pciedngl_bus_binddev(void *bus, void *dev)
{
	struct dngl_bus *pciedngl = ((hndrte_dev_t *)bus)->softc;
	return dngl_binddev(pciedngl->dngl, bus, dev);
}
