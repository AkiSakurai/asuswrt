 ...
 *
 * $Id: bcmotp.c 687024 2017-02-27 06:01:22Z $
 */
...
BCMNMIATTACHFN(ipxotp40n_read2x)(void *oh, chipcregs_t *cc, uint off)
{
	...
	if (oi->sih->ccrev >= 49) {
		otpcontrol1 = R_REG(oi->osh, &cc->otpcontrol1);
		otpcontrol1 &= (OTPC1_CLK_EN_MASK | OTPC1_CLK_DIV_MASK);
	}

	W_REG(oi->osh, &cc->otpcontrol1, otpcontrol1 |
	...
	if (!rewrite) {
		...
		case OTP_HW_RGN:
			...
			if (0 ||
				...
				((CHIPID(oi->sih->chip) == BCM43457_CHIP_ID) &&
				CST4345_CHIPMODE_SDIOD(oi->sih->chipst)) ||
				CHIPID(oi->sih->chip) == BCM43455_CHIP_ID ||
#endif /* UNRELEASEDCHIP */
				((BCM4350_CHIP(oi->sih->chip)) &&
