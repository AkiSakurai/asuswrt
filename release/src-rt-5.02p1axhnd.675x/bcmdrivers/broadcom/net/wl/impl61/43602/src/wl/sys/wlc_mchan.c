 ...
 *
 * $Id: wlc_mchan.c 539964 2015-03-10 12:43:01Z $
 */
...
#define MCHAN_MIN_PRE_INTERFACE_CLOSE_TIME       6000 /* min  time to pre-closing interface */
#define MCHAN_MAX_PRE_INTERFACE_CLOSE_TIME       9000 /* max time to pre -closing interface */
#define MCHAN_INC_PRE_INTERFACE_CLOSE_TIME       1000 /* inc time to pre -closing interface */
#define MCHAN_DEC_PRE_INTERFACE_CLOSE_TIME       200  /* dec time to pre -closing interface */
...
#define MCHAN_GO_STA_DEF_PM_WAIT_TO 8500 /* Default timeout val for GO+STA */
#define MCHAN_GO_STA_PROC_TIME_RED 1500 /* Factor for reducing proc_time in GO+STA */
...
struct mchan_info {
	...
	/* Keep all these stuff at the end */
#ifdef BCMDBG
...
#if defined(WLMCHANPRECLOSE) && !defined(WLMCHANPRECLOSE_ROM_COMPAT)
...
#endif /* WLMCHANPRECLOSE && !WLMCHANPRECLOSE_ROM_COMPAT */
	...
	bool fake_pm_acked; /* track fake PM transition when PM=1 is not ack'd */
};
...
#if defined(WL_PROT_DYNBW) && defined(WL_MODESW)
static void wlc_mchan_notif_cb_notif(mchan_info_t *mchan, wlc_bsscfg_t *cfg,
	int status, int signal);
#endif // endif

/* N channel concurrency */
static void wlc_mchan_pm_cleanup(mchan_info_t *mchan, wlc_bsscfg_t *cfg,
	uint8 requestor);
...
wlc_mchan_reset_params(mchan_info_t * mchan)
{
	...
	mchan->timer_running = FALSE;
		...
		mchan->pm_wait_to = 0;
#else /* WLMCHANPRECLOSE */
		mchan->pre_interface_close_time = MCHAN_MIN_PRE_INTERFACE_CLOSE_TIME;
		mchan->fake_pm_acked = FALSE;
#endif /* !WLMCHANPRECLOSE */
}

#ifdef WLMCHANPRECLOSE
/* increase or decrease pre close time as per PM=1 ACK status */
static void
wlc_mchan_update_pm_interface_close_time(mchan_info_t *mchan)
{
	if (mchan->fake_pm_acked) {
		mchan->fake_pm_acked = FALSE;
		mchan->pre_interface_close_time += MCHAN_INC_PRE_INTERFACE_CLOSE_TIME;
	}
	else {
		mchan->pre_interface_close_time -= MCHAN_DEC_PRE_INTERFACE_CLOSE_TIME;
	}
	if (mchan->pre_interface_close_time > MCHAN_MAX_PRE_INTERFACE_CLOSE_TIME) {
		mchan->pre_interface_close_time = MCHAN_MAX_PRE_INTERFACE_CLOSE_TIME;
	}
	else if (mchan->pre_interface_close_time < MCHAN_MIN_PRE_INTERFACE_CLOSE_TIME) {
		mchan->pre_interface_close_time = MCHAN_MIN_PRE_INTERFACE_CLOSE_TIME;
	}
}
#endif /* WLMCHANPRECLOSE */
...
static void
wlc_mchan_pm_cleanup(mchan_info_t * mchan, wlc_bsscfg_t * cfg, uint8 requestor)
{
	if (MCHAN_ACTIVE(mchan->wlc->pub))
		return;
	mboolclr(cfg->pm->PMblocked, requestor);
...
BCMATTACHFN(wlc_mchan_attach)(wlc_info_t *wlc)
{
...
#if !defined(MCHAN_PM_WAIT_TO_NOPROC) || defined(WLMCHANPRECLOSE)
	mchan->pm_wait_to = 0; /* default value */
#else
	mchan->pm_wait_to = MCHAN_PM_WAIT_TO_NOPROC; /* default value */
#endif // endif
	...
	if ((wlc->pub->sih->boardvendor == VENDOR_APPLE && wlc->pub->sih->boardtype == 0x0093)) {
		...
		mchan->proc_time_us = WLC_MCHAN_PROC_TIME_BMAC_US(mchan);
#else
#if !defined(MCHAN_PM_WAIT_TO_SAMEPROC) && !defined(WLMCHANPRECLOSE)
		mchan->proc_time_us = WLC_MCHAN_PROC_TIME_US(mchan) + mchan->pm_wait_to;
#else
		mchan->proc_time_us = WLC_MCHAN_PROC_TIME_US(mchan);
#endif /* MCHAN_PM_WAIT_TO_SAMEPROC && !WLMCHANPRECLOSE */
#endif /* WLC_HIGH_ONLY */
	}
...
wlc_mchan_prepare_pm_mode(mchan_info_t *mchan, wlc_bsscfg_t *target_cfg, uint32 tsf_l)
{
	...
	if (WL_INFORM_ON()) {
		...
		for (i = 0; wlc->bsscfg[i]; i++) {
			if (wlc->bsscfg[i] == target_cfg &&
		...
		if (!cfg->pm->PMenabled) {
			if (cfg->in_abs) {
				WL_ERROR(("wl%d.%d %s: in abs period, skip pmstate set!!!\n",
				          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__));
				/* we can't go to PS now as already absence is hit. There is no
				* point in falsely claiming that mchan is in pm pending state.
				* opportunity should be given to other cfg. silently exit.
				*/
				continue;
			} else {
				/* If p2p go is present, it will switch to active/presence period.
				 * p2p tx/rx happens during this period, don't let pcie
				 * link go to L1.2; keep sending ltr_active.
				 */
				FOREACH_BSS(wlc, i, other_cfg) {
					if (P2P_GO(wlc, other_cfg))
						wlc_cfg_set_pmstate_upd(other_cfg, FALSE);
				}
	...
	FOREACH_AS_STA(wlc, idx, cfg) {
		...
		if ((cfg->pm->PM == PM_OFF || cfg->pm->PM == PM_FAST || cfg->pm->WME_PM_blocked ||
			...
			if (!(MCHAN_ACTIVE(wlc->pub) && BSS_P2P_ENAB(wlc, cfg) && cfg->in_abs)) {
				...
				/* If p2p go is present, it will switch to abs period;
				 * it is ok to send ltr_sleep now wrt p2p.
				 */
				FOREACH_BSS(wlc, i, other_cfg) {
					if (P2P_GO(wlc, other_cfg))
						wlc_cfg_set_pmstate_upd(other_cfg, TRUE);
				}
				if (cfg->pm->PM == PM_FAST) {
...
wlc_mchan_pm_pending_complete(mchan_info_t *mchan)
{
	...
	ASSERT(mchan->pm_pending_ctx != NULL);
		...
		wlc_mchan_stop_timer(mchan, MCHAN_TO_WAITPMTIMEOUT);
#else /* WLMCHANPRECLOSE */
		/* Dynamically update pre close time as per PM=1 ACK status */
		wlc_mchan_update_pm_interface_close_time(mchan);
#endif /* !WLMCHANPRECLOSE */
...
#ifdef WLMCHANPRECLOSE
	if (BSSCFG_AP(WLC_BSSCFG(wlc, mchan->trigger_cfg_idx))) {
		wlc_mchan_timer_wait((void*)wlc);
		wlc_mchan_adopt_bss_chan_context(wlc, bsscfg->chan_context, FORCE_ADOPT);
		wlc_mchan_pm_block_all(mchan, WLC_PM_BLOCK_MCHAN_CHANSW, FALSE);
	}
	else
#endif /* WLMCHANPRECLOSE */
...
{
	wlc_info_t *wlc = (wlc_info_t *)arg;
	wlc_bsscfg_t *bsscfg_target = NULL;
	wlc_bsscfg_t *bsscfg = wlc_mchan_get_cfg_frm_q(wlc, wlc->primary_queue);
	...
	if (!BSSCFG_AP(bsscfg)) {
		bsscfg_target = wlc_mchan_get_other_cfg_frm_q(wlc, wlc->primary_queue);
		wlc_mchan_prepare_pm_mode(wlc->mchan, bsscfg_target, 0);
	}
}
#endif /* WLMCHANPRECLOSE */
	...
	switch (type) {
		...
		case MCHAN_TO_REGULAR:
			...
			if (MCHAN_PRECLOSE_ENAB(mchan)) {
				...
				if (to < mchan->pre_interface_close_time)
					to = mchan->pre_interface_close_time;
...
wlc_mchan_create_bss_chan_context(wlc_info_t *wlc, wlc_bsscfg_t *cfg, chanspec_t chanspec)
{
	...
	prev_chanspec = cfg->chanspec;

	wlc_lq_chanim_create_bss_chan_context(wlc, chanspec, prev_chanspec);
	...
	if ((mchan->other_cfg_idx != MCHAN_CFG_IDX_INVALID) &&
		(mchan->trigger_cfg_idx != MCHAN_CFG_IDX_INVALID)) {
		if (BSSCFG_AP(WLC_BSSCFG(wlc, mchan->trigger_cfg_idx))) {
			/* Enabling PM timeout for GO+STA cases */
#if !defined(WLMCHANPRECLOSE)
			mchan->pm_wait_to = MCHAN_GO_STA_DEF_PM_WAIT_TO;
#endif /* !WLMCHANPRECLOSE */
#ifdef WLC_HIGH_ONLY
			mchan->proc_time_us = WLC_MCHAN_PROC_TIME_BMAC_US(mchan) +
				mchan->pm_wait_to;
			...
			mchan->proc_time_us = WLC_MCHAN_PROC_TIME_US(mchan) + mchan->pm_wait_to;
#endif /* WLC_HIGH_ONLY */
#if !defined(WLMCHANPRECLOSE) && !defined(WLC_HIGH_ONLY)
			mchan->proc_time_us -= MCHAN_GO_STA_PROC_TIME_RED;
#endif /* !WLMCHANPRECLOSE && !WLC_HIGH_ONLY */
			mchan->proc_time_us =
				MCHAN_TIMING_ALIGN(mchan->proc_time_us);
		}
	}
...
_wlc_mchan_delete_bss_chan_context(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint *oldqi_info)
{
	...
	wlc_lq_chanim_delete_bss_chan_context(wlc, cfg->chanspec);
	...
	if (other_bsscfg) {
		if (P2P_CLIENT(wlc, other_bsscfg)) {
			if (wlc_p2p_noa_valid(wlc->p2p, other_bsscfg)) {
				wlc_mchan_pm_cleanup(mchan, other_bsscfg, WLC_PM_BLOCK_MCHAN_ABS);
			}
		}
		else {
			wlc_mchan_pm_cleanup(mchan, other_bsscfg, WLC_PM_BLOCK_MCHAN_CHANSW);
		}
	...
	if (mchan->trigger_cfg_idx == cfg->_idx) {
		...
		FOREACH_BSS(wlc, bssidx, bsscfg) {
			...
			if (bsscfg->chan_context == cfg->chan_context) {
				selected_idx = bsscfg->_idx;
				break;
			}
                                 ...
                                 wlc_mchan_adopt_type adopt_type)
{
	...
	switch_cfg = wlc_mchan_get_cfg_frm_q(wlc, qi);
	UNUSED_PARAMETER(switch_cfg);

	if (home_chanspec != current_chanspec) {
		/* Tell CHANIM that we're about to switch channels */
		wlc_lq_chanim_adopt_bss_chan_context(wlc, home_chanspec, current_chanspec);
	}
	...
	|| defined(MCHAN_MINIDUMP))
		...
		if (wlc->primary_queue == qi &&
			...
			if (PROP_TXSTATUS_ENAB(wlc->pub)) {
				 ...
				 */
				if (!switch_cfg->in_abs) {
					wlc_wlfc_mchan_interface_state_update(wlc,
						switch_cfg, WLFC_CTL_TYPE_INTERFACE_OPEN,
						FALSE);
				}
			}
#endif /* PROP_TXSTATUS */
			...
			if (current_chanspec != home_chanspec) {
				...
				if (WLC_PROT_DYNBW_ENAB(wlc->pub)) {
					if (BSSCFG_STA(bsscfg) && bw_update) {
						bw_update = wlc_modesw_update_bandwidth(wlc,
							bsscfg, chan_ctxt->chanspec);
					}
				}
				if (!bw_update)
#endif /* WL_PROT_DYNBW */
			...
			if (wlc_btc_mode_get(wlc) != WL_BTC_DISABLE) {
				/*
...
wlc_mchan_ap_pretbtt_proc(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint32 tsf_l, uint32 tsf_h)
{
	...
	/* Soft AP case. There is no NOA. Use 802.11g QUITE IE */
		...
		now = R_REG(wlc->osh, &wlc->regs->tsf_timerlow);
		/* FIXME: VSDB TODO .. tbtt_l is now part of mcnx cubby !
		 * offset = now + ((ap_bcn_per >>10) -
		 *			((now - bsscfg->tbtt_l) % (ap_bcn_per >>10)));
		*/
		offset = now + (ap_bcn_per >>10);
...
/* Not using DTIM based logic as of now */
	...
	/* mark the bsscfg that was skipped this time */
		...
		mchan->si_algo_last_switch_time = 0;
	} else if ((mchan->switch_algo == MCHAN_BANDWIDTH_ALGO) ||
...
wlc_mchan_abs_proc(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint32 tsf_l)
{
	...
	/* return if not mchan not active */
	if (!(MCHAN_ENAB(wlc->pub) && MCHAN_ACTIVE(wlc->pub))) {
		return;
	}

	other_cfg = WLC_BSSCFG(wlc, (cfg->_idx == wlc->mchan->trigger_cfg_idx ?
		wlc->mchan->other_cfg_idx : wlc->mchan->trigger_cfg_idx));

	ASSERT(wlc->mchan->other_cfg_idx != MCHAN_CFG_IDX_INVALID);

	if (BSSCFG_AP(cfg)) {
		if (mchan->noa_dur) {
			/* Make sure we have no unserviced schedule elements.
			*If so, remove them.
			 */
			if (mchan->mchan_sched) {
				WL_MCHAN(("something is wrong, mchan_sched not empty!\n"));
		...
		wlc_mchan_prepare_pm_mode(wlc->mchan, other_cfg, tsf_l);
	}
	else {
		uint32 go_bcn_per = cfg->current_bss->beacon_period << 10;
		uint32 cur_time;
		uint32 abs_dur;
		...
		*/
		...
		abs_dur = wlc_mcnx_read_shm(wlc->mcnx,
			M_P2P_BSS_NOA_DUR(wlc_mcnx_BSS_idx(wlc->mcnx, cfg)));
		abs_dur <<= P2P_UCODE_TIME_SHIFT;

		end_time = ((abs_dur + go_bcn_per) - (cur_time - tsf_l)) -
			mchan->proc_time_us;

		/* add timer to schedule channel switch for end of abs period */
		wlc_mchan_start_timer(mchan, end_time, MCHAN_TO_REGULAR);
...
wlc_mchan_psc_proc(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint32 tsf_l)
{
	...
	if (BSSCFG_AP(cfg) && wlc->mchan->special_bss_idx == MCHAN_CFG_IDX_INVALID) {
		...
		* out of blocking mode. simple return in that case.
		*/
		if (mchan->blocking_bsscfg != other_cfg) {
			wlc_mchan_adopt_bss_chan_context(wlc, cfg->chan_context, FORCE_ADOPT);
		}
		else {
			wlc_mchan_prepare_pm_mode(wlc->mchan, other_cfg, tsf_l);
			return;
		...
		end_time = (next_abs_time - cur_time - MCHAN_TIMER_DELAY_US - proc_time);
		wlc_mchan_start_timer(mchan, end_time, MCHAN_TO_REGULAR);
		wlc_mchan_sched_add(mchan, other_cfg->_idx, 0, end_time);
		mchan->timer_running = TRUE;
	}
#ifdef WLMCHANPRECLOSE
	if (BSSCFG_AP(cfg)) {
...
#if defined(BCM4354) || defined(BCM4356) || defined(BCM4358)
	default_proc_time += 2500;
#endif /* BCM4354 || BCM4356 || BCM4358 */
#if defined(BCM43569)
#ifdef WLMCHANPRECLOSE
	/* 10000 - wlc_mchan_pretbtt_time_us(8000 for DONGLEBUILD) + 1000 = 3 ms */
	default_proc_time += 1000;
#elif !defined(MCHAN_PM_WAIT_TO_NOPROC)
	default_proc_time += 3000;
#else
	default_proc_time += 5000;
#endif // endif
#endif /* BCM43569 */
