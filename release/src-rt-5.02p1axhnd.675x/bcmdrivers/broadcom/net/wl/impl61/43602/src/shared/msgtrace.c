 ...
 *
 * $Id: msgtrace.c 456077 2014-02-17 20:17:52Z $
 */
...
static msgtrace_t *msgtrace = NULL;
...
msgtrace_stop(void)
{
	...
	if (msgtrace->event_trace_enabled) {
		...
		hndrte_del_timer(msgtrace->timer);
...
msgtrace_start(void)
{
	...
	if (!msgtrace->event_trace_enabled) {
		...
		hndrte_add_timer(msgtrace->timer, SEND_TIMEOUT_VALUE, FALSE);
...
static void
msgtrace_timeout(hndrte_timer_t *t)
{
	msgtrace_hdr_t hdr;
	uint8 *trace;
	uint16 len;
	...
	if ((len = msgtrace_get(&trace)) > 0) {
		...
		if (msgtrace->event_trace_enabled) {
			...
			hndrte_add_timer(msgtrace->timer, RETRY_TIMEOUT_VALUE, FALSE);
		}
	}
}
...
void
BCMATTACHFN(msgtrace_init)(osl_t *osh, void *hdl1, void *hdl2, msgtrace_func_send_t func_send)
{
	...
	msgtrace = (msgtrace_t *) MALLOC(osh, sizeof(msgtrace_t));
	if (!msgtrace)
		return;
	bzero(msgtrace, sizeof(msgtrace_t));
	...
	msgtrace->timer  = hndrte_init_timer(NULL, NULL, msgtrace_timeout,  NULL);
