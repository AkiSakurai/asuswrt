 ...
 * HNDRTE specific constructs should be used in this file).
 *
 ...
 *
 * $Id: hndarm.c 515975 2014-11-18 07:45:09Z $
 */
...
static uint
BCMATTACHFN(si_setirq_oob)(si_t *sih, uint coreid)
{
	...
	(void) si_setcore(sih, ARM_CORE_ID, _armcr4_idx);
...
BCMATTACHFN(si_setirq)(si_t *sih, uint irq, uint coreid, uint coreunit)
{
	...
	#else
	core_idx = si_setirq_oob(sih, coreid);
	#endif /* REROUTE_OOBINT */
...
BCMATTACHFN(si_arm_init)(si_t *sih)
{
	...
	if (sr != NULL) {
		...
		/* PR47848 WAR: Disable SOCRAM standby.
		 * PR50125 WAR: SOCRAM standby is enabled by default in corerev 1 & 2,
		 * disable SOCRAM standby.
		 */
		else if (rev == 1 || rev == 2) {
			while (bank--) {
				W_REG(osh, &sr->bankidx, bank);
	...
	hndarm_armr = si_setcore(sih, ARM_CORE_ID, get_armcr4_idx());
	printf("hndarm_armr addr: 0x%x, cr4_idx: %d\n", (uint)hndarm_armr, get_armcr4_idx());
	ASSERT(hndarm_armr);
...
#endif /* HT_POWERSAVE */
	if (sih->ccrev >= 40)
		OR_REG(osh, ARMREG(hndarm_armr, clk_ctl_st), CCS_HQCLKREQ);
	...
	case 1:
		/* PR47507 & PR47174 WAR: HW issue with cdp sleep instruction.
		 * Run ARM with whatever clock available on the backplane when
		 * idling (ARM will stop when no clock is on the backplane).
		 */
#ifdef HNDRTE_TEST
		arm7s_wait = arm_wfi;
#else
...
#ifdef	__ARM_ARCH_7R__
	/* Enable WFIClkStop */
	OR_REG(osh, ARMREG(hndarm_armr, corecontrol), (ACC_WFICLKSTOP));
#endif /* __ARM_ARCH_7R__ */
#endif /* !defined(BCMDBG_LOADAVG) */
...
hnd_cpu_loadavg_timer(si_t *sih, uint32 val)
{
	...
	intstatus = R_REG(hndrte_osh, ARMREG(regs, intstatus));
	BCM_REFERENCE(intstatus);
	si_setcoreidx(sih, idx);
}
#else
...
uint32
si_arm_disable_deepsleep(bool disable)
{
	if (disable)
		OR_REG(hndrte_osh, ARMREG(hndarm_armr, clk_ctl_st), CCS_HTAREQ);
	else
		AND_REG(hndrte_osh, ARMREG(hndarm_armr, clk_ctl_st), ~CCS_HTAREQ);
	return (R_REG(hndrte_osh, ARMREG(hndarm_armr, clk_ctl_st)));
}
 ...
 *
*/
int cr4_mpu_set_region(uint32 region, uint32 base_address, uint32 size_index, uint32 control)
{
	...
	if (base_address & ((1 << (size_index+1))-1)) {
		return BCME_ERROR;
	}

	mpu_set_region(region, base_address, size_index<<1, control);
