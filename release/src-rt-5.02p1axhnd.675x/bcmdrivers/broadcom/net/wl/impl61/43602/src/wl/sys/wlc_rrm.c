 ...
 *
 * $Id: wlc_rrm.c 769941 2018-11-30 05:08:47Z $
 */
...
BCMATTACHFN(wlc_rrm_attach)(wlc_info_t *wlc)
{
	...
	rrm_info->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct rrm_scb_cubby),
		wlc_rrm_scb_init, wlc_rrm_scb_deinit,
		wlc_rrm_scb_dump, (void *)rrm_info, SCB_CUBBY_ID_RRM);
#else
	rrm_info->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct rrm_scb_cubby),
...
wlc_rrm_noise_sample_request(wlc_info_t *wlc)
{
	...
	noise_dbm = wlc_phy_noise_avg((wlc_phy_t *)wlc->band->pi);
...
wlc_rrm_send_noiserep(wlc_rrm_info_t *rrm_info)
{
	...
	rmrep_noise->anpi = wlc_phy_noise_avg((wlc_phy_t *)wlc->band->pi);
...
static void wlc_rrm_send_locid
	dot11_rm_action_t *rm_rep;
	dot11_rm_ie_t *rmrep_ie;
	struct scb *scb = NULL;
	uint8 *rmrep_locid;
...
wlc_rrm_framerep_add(wlc_rrm_info_t *rrm_info, uint8 *bufptr, uint cnt)
{
	...
	wlc_stf_phy_txant_upd(wlc);
	noise_avg = wlc_phy_noise_avg((wlc_phy_t *)wlc->band->pi);
...
static uint8
wlc_rrm_get_wb_chanwidth(chanspec_t chanspec)
{
...
static void
wlc_rrm_get_wb_ie_from_chanspec(dot11_wide_bw_chan_ie_t *wbc_ie,
	chanspec_t chanspec)
{
	if (wbc_ie == NULL) {
		return;
	...
	wbc_ie->channel_width = wlc_rrm_get_wb_chanwidth(chanspec);
	...
	int addtype)
{
	...
	for (i = 0; i < rep_count; i++) {
		...
		if (chspec) {
			wlc_rrm_get_wb_ie_from_chanspec(wbc_ie, chspec);
		}
	...
	int body_len, int8 rssi, ratespec_t rspec)
{
	...
	wlc_phy_stf_chain_get(wlc->band->pi, &tx_antidx, &rcv_antidx);
...
#ifdef BCM_HOST_MEM_SCB
	if (!SCB_INTERNAL(scb) && SCB_ALLOC_ENAB(rrm_info->wlc->pub) && SCB_HOST(scb)) {
		scb_info = (rrm_scb_info_t *)wlc_scb_alloc_mem_get(rrm_info->wlc,
			SCB_CUBBY_ID_RRM, sizeof(rrm_scb_info_t), 1);
	}
	if (!scb_info)
#endif // endif
...
static void wlc_rrm_scb_deinit(void *ctx, struct scb *scb)
{
	...
	if (scb_info) {
#ifdef BCM_HOST_MEM_SCB
		if (SCB_ALLOC_ENAB(rrm_info->wlc->pub) && SCB_HOST(scb)) {
			wlc_scb_alloc_mem_free(rrm_info->wlc, SCB_CUBBY_ID_RRM, (void *)scb_info);
		}
		else
#endif // endif
...
/* memory of rrm_cubby have been allocated in the target space */
/* and content of the original rrm_cubby is copied to target */
#if defined(BCM_HOST_MEM_RESTORE) && defined(BCM_HOST_MEM_SCB)
int
rrm_cubby_host2dongle(wlc_info_t *wlc, void *context,
struct scb* scb_dongle, struct scb* scb_host)
{
	wlc_rrm_info_t *rrm_info = (wlc_rrm_info_t *)context;
	rrm_scb_cubby_t *scb_cubby_rrm_dongle = RRM_SCB_CUBBY(rrm_info, scb_dongle);
	rrm_scb_cubby_t *scb_cubby_rrm_host = RRM_SCB_CUBBY(rrm_info, scb_host);

	if (scb_cubby_rrm_host != NULL) {
		rrm_scb_info_t *scb_info_rrm_dongle = NULL;
		rrm_scb_info_t *scb_info_rrm_host = scb_cubby_rrm_host->scb_info;

		if (scb_info_rrm_host != NULL) {
			scb_info_rrm_dongle = (rrm_scb_info_t *)
				MALLOC(wlc->osh, sizeof(rrm_scb_info_t));
			if (!scb_info_rrm_dongle) {
				return BCME_NOMEM;
			}

			memcpy(scb_info_rrm_dongle, scb_info_rrm_host, sizeof(rrm_scb_info_t));
			scb_cubby_rrm_dongle->scb_info = scb_info_rrm_dongle;

			wlc_scb_alloc_mem_free(rrm_info->wlc,
				SCB_CUBBY_ID_RRM, (void *)scb_info_rrm_host);
			scb_cubby_rrm_host->scb_info = NULL;
			return BCME_OK;
		}
	}
	return BCME_ERROR;
...
int
rrm_cubby_dongle2host(wlc_info_t *wlc, void *context,
struct scb* scb_dongle, struct scb* scb_host)
{
	wlc_rrm_info_t *rrm_info = (wlc_rrm_info_t *)context;
	rrm_scb_cubby_t *scb_cubbyinfo_rrm = RRM_SCB_CUBBY(rrm_info, scb_dongle);
	rrm_scb_cubby_t *shadow_cubbyinfo_rrm = RRM_SCB_CUBBY(rrm_info, scb_host);
	if (scb_cubbyinfo_rrm != NULL) {
		rrm_scb_info_t* scb_cubby_rrm_host = NULL;
		rrm_scb_info_t* scb_cubby_rrm_dongle = scb_cubbyinfo_rrm->scb_info;
		if (scb_cubby_rrm_dongle != NULL) {
			scb_cubby_rrm_host = (rrm_scb_info_t *)wlc_scb_alloc_mem_get(rrm_info->wlc,
				SCB_CUBBY_ID_RRM, sizeof(rrm_scb_info_t), 1);
			if (!scb_cubby_rrm_host) {
				return BCME_NOMEM;
			}
			memcpy(scb_cubby_rrm_host, scb_cubby_rrm_dongle, sizeof(rrm_scb_info_t));

			shadow_cubbyinfo_rrm->scb_info = scb_cubby_rrm_host;
			MFREE(wlc->osh, scb_cubby_rrm_dongle, sizeof(rrm_scb_info_t));
			scb_cubbyinfo_rrm->scb_info = NULL;
			return BCME_OK;
		}
	}
	return BCME_ERROR;
