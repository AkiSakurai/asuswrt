...
static const bcm_iovar_t staprio_iovars[] = {
	...
	{"bsscfg_class", IOV_BSSCFG_CLASS, (0), IOVT_INT32, 0 },
	{NULL, 0, 0, 0, 0 }
};
...
typedef struct staprio_bsscfg_cubby {
	uint32 class;	/* bsscfg flag: 0 for private, 1 for public, 2 for video, etc */
} staprio_bsscfg_cubby_t;
...
#define STAPRIO_BSSCFG_CUBBY(staprio, cfg) \
	((staprio_bsscfg_cubby_t *)BSSCFG_CUBBY(cfg, (staprio)->cfgh))
...
static int wlc_staprio_watchdog(void *cntxt);
static int wlc_staprio_dump(wlc_staprio_info_t *staprio, struct bcmstrbuf *b);
static int wlc_proc_staprio(wlc_staprio_info_t *staprio, wlc_bsscfg_t *bsscfg,
...
/* staprio bsscfg cubby_init function */
static int staprio_bsscfg_init(void *ctx, wlc_bsscfg_t *bsscfg)
{
	wlc_staprio_info_t *staprio = (wlc_staprio_info_t *)ctx;
	staprio_bsscfg_cubby_t *staprio_bsscfg_cubby = STAPRIO_BSSCFG_CUBBY(staprio, bsscfg);

	staprio_bsscfg_cubby->class = STAPRIO_BSSCFG_PRIVATE;

	return BCME_OK;
}

/* staprio bsscfg cubby_dump fucntion */
static void staprio_bsscfg_dump(void *ctx, wlc_bsscfg_t *bsscfg, struct bcmstrbuf *b)
{
	wlc_staprio_info_t *staprio = (wlc_staprio_info_t *)ctx;
	staprio_bsscfg_cubby_t *staprio_bsscfg_cubby = STAPRIO_BSSCFG_CUBBY(staprio, bsscfg);

	bcm_bprintf(b, "BSSCFG flag: 0x%x\n", staprio_bsscfg_cubby->class);
}
...
BCMATTACHFN(wlc_staprio_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_module_register(wlc->pub, staprio_iovars, "staprio",
		staprio, wlc_staprio_doiovar, wlc_staprio_watchdog, NULL, NULL)) {
	...
	if ((staprio->cfgh = wlc_bsscfg_cubby_reserve(wlc, sizeof(staprio_bsscfg_cubby_t),
		staprio_bsscfg_init, NULL, staprio_bsscfg_dump, (void *)staprio)) < 0) {
		WL_ERROR(("wl%d: %s: wlc_bsscfg_cubby_reserve() failed\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}

	if ((staprio->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(struct staprio_scb_cubby),
#if defined(BCM_HOST_MEM_RESTORE) && defined(BCM_HOST_MEM_SCB)
		staprio_scb_init, staprio_scb_deinit, staprio_scb_dump, (void *)staprio, 0)) < 0) {
#else
		staprio_scb_init, staprio_scb_deinit, staprio_scb_dump, (void *)staprio)) < 0) {
#endif // endif
	...
	struct wlc_if       *wlcif)
{
	...
	wlc_bsscfg_t	*bsscfg;
	staprio_bsscfg_cubby_t *staprio_bsscfg_cubby;
	...
	staprio_bsscfg_cubby = STAPRIO_BSSCFG_CUBBY(staprio, bsscfg);
	ASSERT(staprio_bsscfg_cubby != NULL);
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_BSSCFG_CLASS):
			*ret_uint_ptr = staprio_bsscfg_cubby->class;
			break;

		case IOV_SVAL(IOV_BSSCFG_CLASS):
			if (uint_val <= STAPRIO_BSSCFG_VIDEO)
				staprio_bsscfg_cubby->class = uint_val;
			else
				err = BCME_RANGE;
			break;
		default:
...
/* module Watchdog timer - do we need this? */
static int
wlc_staprio_watchdog(void *ctx)
{
	BCM_REFERENCE(ctx);
...
/* Read bsscfg class */
uint32 wlc_get_bsscfg_class(wlc_staprio_info_t *staprio, wlc_bsscfg_t *bsscfg)
{
	staprio_bsscfg_cubby_t *staprio_bsscfg_cubby;

	if (!staprio || !bsscfg)
		return STAPRIO_BSSCFG_PRIVATE;

	staprio_bsscfg_cubby = STAPRIO_BSSCFG_CUBBY(staprio, bsscfg);

	return (staprio_bsscfg_cubby->class);
}
