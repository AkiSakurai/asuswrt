...
static lcb_t *
wlc_scb_lpc_get_cubby(wlc_lpc_info_t *wlpci, struct scb *scb, uint16 frameid)
{
	ASSERT(wlpci);
	ASSERT(scb);

	ASSERT(FID2AC(wlpci->pub, frameid) < AC_COUNT);
	return (SCB_LPC_CUBBY(wlpci, scb, FID2AC(wlpci->pub, frameid)));
}
...
uint8
wlc_scb_lpc_getcurrpwr(wlc_lpc_info_t *wlpci, struct scb *scb, uint16 frameid)
{
	lcb_t *state = NULL;
	...
	state = wlc_scb_lpc_get_cubby(wlpci, scb, frameid);
	return wlc_lpc_getcurrpwr(state);
}

void
wlc_scb_lpc_update_pwr(wlc_lpc_info_t *wlpci, struct scb *scb, uint16 frameid,
	uint16 PhyTxControlWord0, uint16 PhyTxControlWord1)
{
	...
	state = wlc_scb_lpc_get_cubby(wlpci, scb, frameid);
	wlc_lpc_update_pwr(state, frameid, phy_ctrl_word);
	return;
}

void
wlc_scb_lpc_store_pcw(wlc_lpc_info_t *wlpci, struct scb *scb, uint16 frameid,
	uint16 phy_ctrl_word)
{
	lcb_t *state = NULL;
