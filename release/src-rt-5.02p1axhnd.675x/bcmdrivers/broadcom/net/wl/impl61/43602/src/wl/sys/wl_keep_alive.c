 ...
 *
 * $Id: wl_keep_alive.c 454103 2014-02-07 15:58:00Z $
 *
 */
...
BCMATTACHFN(wl_keep_alive_attach)(wlc_info_t *wlc)
{
	...
	info = MALLOC(wlc->pub->osh, MKEEP_ALIVE_INFOSIZE(keep_alive_count));
	if (info == NULL) {
	...
	bzero(info, MKEEP_ALIVE_INFOSIZE(keep_alive_count));
