 ...
 *
 * $Id: wlc_csa.c 766613 2018-08-08 06:59:54Z $
 */
...
#endif /* STA */
static void wlc_csa_prot_obss_notif_cb_notif(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg,
...
wlc_csa_scb_init(void *ctx, struct scb *scb)
{
	...
	if ((csa_scb = MALLOC(wlc->osh, sizeof(wlc_csa_scb_cubby_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return BCME_NOMEM;
	}

	bzero(csa_scb, sizeof(wlc_csa_scb_cubby_t));
	*pcsa_scb = csa_scb;
...
{
#ifdef DONGLEBUILD
	return (wlc_sendnulldata(wlc, cfg, da, 0, 0, PRIO_8021D_VO) != NULL);
#else
	return wlc_sendnulldata(wlc, cfg, da, 0, 0, PRIO_8021D_VO);
#endif /* DONGLEBUILD */
}
static void
...
wlc_csa_timeout(void *arg)
{
	...
	wlc_csa_prot_obss_notif_cb_notif(csam, cfg,
...
wlc_csa_process_channel_switch(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	*/
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, csa->csa.chspec)) {
		WL_REGULATORY(("wl%d: %s: Received invalid channel: %d\n",
	...
	WL_REGULATORY(("wl%d.%d: %s: Recved CSA: mode %d, channel %d, count %d, secs %d\n",
	               wlc->pub->unit, cfg->_idx, __FUNCTION__,
	               csa->csa.mode, CHSPEC_CHANNEL(csa->csa.chspec), csa->csa.count, secs));
	wlc_csa_prot_obss_notif_cb_notif(csam, cfg,
		BCME_OK, CSA_CHANNEL_CHANGE_START, csa->csa.chspec);

	if (PSTA_ENAB(wlc->pub) || (BSSCFG_STA(cfg) && (DWDS_ENAB(cfg) || MAP_ENAB(cfg)))) {
	...
	uint8 *body, int body_len)
{
	...
	if (body_len > (int)(sizeof(struct dot11y_action_ext_csa))) {
		...
		wide_bw_chan_switch_ie = (dot11_wide_bw_chan_switch_ie_t*)
	        bcm_parse_tlvs(ies, ies_len, DOT11_MNG_WIDE_BW_CHANNEL_SWITCH_ID);

		if (wide_bw_chan_switch_ie == NULL || wide_bw_chan_switch_ie->len <
		...
		if (tag->len >= DOT11_EXT_CSA_IE_LEN) {
			...
			if (tag != NULL) {
				wide_bw_ie = (dot11_wide_bw_chan_switch_ie_t *)
				  bcm_parse_tlvs(tag->data, tag->len,
				                 DOT11_MNG_WIDE_BW_CHANNEL_SWITCH_ID);
			}
			 ...
			 */
			if (wide_bw_ie != NULL && wide_bw_ie->len >= DOT11_WIDE_BW_SWITCH_IE_LEN) {
				bw = channel_width_to_bw(wide_bw_ie->channel_width);
				csa->chspec = wlc_get_vht_chanspec(wlc, csa_ie->b.channel, bw);
			}
			else
#endif /* WL11AC */
			{
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	if (VHT_ENAB(wlc->pub)) {
		...
		wide_bw_ie = bcm_parse_tlvs(ies, ies_len, DOT11_MNG_WIDE_BW_CHANNEL_SWITCH_ID);
		if (wide_bw_ie != NULL &&
		    wide_bw_ie->len == DOT11_WIDE_BW_SWITCH_IE_LEN) {
	...
	const struct ether_addr *dst, wl_chan_switch_t *csa, uint8 action_id)
{
	...
	if (action_id == DOT11_SM_ACTION_CHANNEL_SWITCH) {
		...
		if ((N_ENAB(wlc->pub) && (CHSPEC_IS40(csa->chspec))) ||
		    (VHT_ENAB(wlc->pub) && (CHSPEC_IS80(csa->chspec)))) {
		...
		FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
			...
			if (!SCB_ISMULTI(scb)) {
				...
				if (!SCB_PS_PRETEND(scb)) {
					/* invalidate txc */
					if (WLC_TXC_ENAB(wlc)) {
						uint *inv = wlc_txc_inv_ptr(wlc->txc, scb);
						if (inv) {
							*inv = 0;
						}
					}
...
int
wlc_csa_prot_obss_notif_cb_register(wlc_csa_info_t *csam,
...
int
wlc_csa_prot_obss_notif_cb_unregister(wlc_csa_info_t *csam,
	wlc_csa_notif_cb_fn_t cb, void *arg)
{
	bcm_notif_h hdl = csam->csa_notif_hdl;
...
static void
wlc_csa_prot_obss_notif_cb_notif(wlc_csa_info_t *csam,
