...
BCMATTACHFN(wlc_lpc_attach)(wlc_info_t *wlc)
{
	...
	if (!(lpci = (lpc_info_t *)MALLOC(wlc->osh, sizeof(lpc_info_t)))) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return NULL;
	}

	bzero((char *)lpci, sizeof(lpc_info_t));
	lpci->wlc = wlc;
	lpci->osh = wlc->osh;
	lpci->pub = wlc->pub;
	...
	void *p, uint plen, void *a, int alen, int val_size, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		case IOV_SVAL(IOV_LPC):
		{
			...
			wlc_phy_lpc_algo_set(wlc->band->pi, bool_val);
		}
		break;

		case IOV_GVAL(IOV_LPC_PARAMS):
		{
			if (wlc->pub->_lpc_algo) {
...
{
	uint8 min_idx = wlc_phy_lpc_getminidx(state->lpci->wlc->band->pi);
...
{
	uint8 *lpc_pwr_level = wlc_phy_lpc_get_pwrlevelptr(wlc->band->pi);
...
wlc_lpc_getcurrpwr(lcb_t *state)
{
	...
	if ((NOW(wlc) - state->last_update_time) < lpci->lpc_params.lpc_exp_time) {
		...
		pwr_offset = wlc_phy_lpc_getoffset(wlc->band->pi, state->curr_pwr_level_idx);
	} else
...
void
wlc_lpc_update_pwr(lcb_t *state, uint16 frameid, uint16 phy_ctrl_word)
{
	...
	wlc_scb_ratesel_get_info(wrsi, scb, frameid, lpc_params->rate_stab_thresh,
...
exit:
	...
	if (pwridx_in_cubby != state->curr_pwr_level_idx) {
		...
		wlc_scb_ratesel_clr_cache(wrsi, scb, frameid);
	}
