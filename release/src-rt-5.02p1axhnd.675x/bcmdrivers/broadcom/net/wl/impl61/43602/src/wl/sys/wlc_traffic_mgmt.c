 ...
 *
 * $Id: wlc_traffic_mgmt.c 658773 2016-09-09 14:51:51Z $
 */
...
#ifdef TRAFFIC_MGMT_RSSI_POLICY
static int trf_mgmt_set_rssi_policy(wlc_trf_mgmt_ctxt_t *tm, uint32 policy);
#endif /* TRAFFIC_MGMT_RSSI_POLICY */
...
wlc_intfer_scb_init(void *ctx, struct scb *scb)
{
	...
	scb_intfer_stats = (void *)MALLOC(WLCOSH(trf_mgmt_ctxt), sizeof(wlc_scb_intfer_stats_t));
	if (!scb_intfer_stats)
		return BCME_ERROR;
...
#if defined(BCM_HOST_MEM_RESTORE) && defined(BCM_HOST_MEM_SCB)
		wlc_intfer_scb_dump, trf_mgmt_ctxt, 0)) < 0) {
#else
...
wlc_trf_mgmt_allocate_info_block(wlc_trf_mgmt_ctxt_t *trf_mgmt_ctxt, wlc_bsscfg_t *bsscfg)
{
...
#ifdef WLINTFERSTAT
	trf_mgmt_info->intfer_params = MALLOC(wlc->osh, sizeof(wl_intfer_params_t));
	...
	memset(trf_mgmt_info->intfer_params, 0, sizeof(wl_intfer_params_t));
		...
		wlc_trf_mgmt_info_t *trf_mgmt_info)
{
	...
	dwm_info = MALLOC(WLCOSH(trf_mgmt_ctxt), sizeof(trf_mgmt_dwm_info_t));
	if (!dwm_info) {
	...
	bzero(dwm_info, sizeof(trf_mgmt_dwm_info_t));

	/* Allocate DWM filters table */
	dwm_info->tm_dwm_tbl = MALLOC(WLCOSH(trf_mgmt_ctxt), sizeof(trf_mgmt_dwm_tbl_t));
	...
	bzero(dwm_info->tm_dwm_tbl, sizeof(trf_mgmt_dwm_tbl_t));
...
queue_it:
	...
	struct scb* scb = WLPKTTAGSCBGET(scb);
