...
*$Id: wlc_ota_test.c 348425 2012-08-01 22:12:24Z $
...
BCMATTACHFN(wlc_ota_test_attach)(wlc_info_t *wlc)
{
	...
	/* ota test info */
	ota_test_info = (ota_test_info_t *)MALLOC(wlc->osh, sizeof(ota_test_info_t));
	if (ota_test_info == NULL) {
		WL_ERROR(("wl%d: %s: MALLOC failed, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return NULL;
	}
	bzero(ota_test_info, sizeof(ota_test_info_t));

	/* attach ota test vector */
	ota_test_info->test_vctr = (wl_ota_test_vector_t *)MALLOC(wlc->osh,
	...
	bzero(ota_test_info->test_vctr, sizeof(wl_ota_test_vector_t));
...
wlc_ota_test_init_seq(wlc_info_t *wlc)
{
	...
	/* wl country ALL */
	wlc_iovar_op(wlc, "country", NULL, 0, cntry, sizeof(WLC_CNTRY_BUF_SZ), IOV_SET, NULL);
...
wlc_ota_test_wait_for_sync_pkt(wlc_info_t *wlc)
{
	...
	wlc_ctrupd(wlc, OFFSETOF(macstat_t, pktengrxducast),
		MACSTATOFF(pktengrxducast));
	pktengrxducast_start = WLCNTVAL(wlc->pub->_cnt->pktengrxducast);
	...
	pktengrxducast_end = WLCNTVAL(wlc->pub->_cnt->pktengrxducast);
...
wlc_ota_test_wait_for_sync(void *arg)
{
	...
	if (max_no_cores >= 2) {
	    /* Current test phase */
	    cnt = wlc->ota_info->test_phase;
	    cores = test_vctr->test_arg[cnt].txant;
	    coremask[1] |= cores;
	    coremask[1] |= cores << 8;
	    mcs_mask[streams-1] = ((cores & 0xf) << 4) | (streams & 0xf);
	    coremask[0] |= mcs_mask[0] << 0;
