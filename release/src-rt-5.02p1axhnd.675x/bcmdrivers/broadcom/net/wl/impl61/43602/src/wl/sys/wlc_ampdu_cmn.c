 ...
 *
 * $Id: wlc_ampdu_cmn.c 537921 2015-02-28 01:23:49Z $
 */
...
wlc_scb_ampdu_enable(wlc_info_t *wlc, struct scb *scb)
{
	...
	wlc_ampdu_agg_state_update_rx(wlc, SCB_BSSCFG(scb), ON, AMPDU_ALL_TID_BITMAP);
}
	...
	uint16 cf_policy, bool enq_only, bool *blocked)
{
	...
	*blocked = FALSE;
	...
	WL_INFORM(("wl%d.%d: %s: seq 0x%x tid %d\n", wlc->pub->unit,
...
void
BCMATTACHFN(wlc_ampdu_set_aggr_support)(wlc_info_t *wlc, bool txaggr, int8 rxaggr)
{
	wlc_ampdu_tx_set_txaggr_support(wlc->ampdu_tx, txaggr);
	wlc_ampdu_agg_rxaggr_support(wlc->ampdu_rx, rxaggr);
}

void
wlc_ampdu_agg_state_upd_ovrd_band(wlc_info_t *wlc, bool aggr, int band, int txrx)
{
	int idx;
	wlc_bsscfg_t *cfg;
	...
	if (txrx < AMPDU_UPD_ALL || txrx > AMPDU_UPD_RX_ONLY)
		return;

	FOREACH_BSS(wlc, idx, cfg) {
		if ((band == WLC_BAND_5G && CHSPEC_IS2G(cfg->current_bss->chanspec)) ||
			(band == WLC_BAND_2G && CHSPEC_IS5G(cfg->current_bss->chanspec)))
			/* Skip undesired band */
			continue;

		if (txrx != AMPDU_UPD_TX_ONLY)
			wlc_ampdu_agg_state_update_rx_override(wlc, cfg, aggr);
		if (txrx != AMPDU_UPD_RX_ONLY)
