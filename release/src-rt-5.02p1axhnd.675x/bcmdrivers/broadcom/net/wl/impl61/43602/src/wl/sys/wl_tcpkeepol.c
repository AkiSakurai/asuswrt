 ...
 *
 * $Id: wl_tcpkeepol.c 376474 2012-12-27 07:27:11Z $
 */
 ...
 */
typedef struct tcp_keep_alive_conn_info_t {
	struct	ether_addr src_mac;		/* Source Mac */
	...
	uint32  ack_num;			/* TCP Ack Number */
	uint16  tcpwin;				/* TCP window */
	uint16  period;				/* Interval between Pks send */
	uint16	timeout;			/* Timeout value to wait for ack */
	uint16	keep_alive_sec_tics;		/* Flag for decideding keep alive send */
	uint16	ack_timeout_sec_tics;		/* Flag for deciding timeout  */
	bool	ka_session_active;		/* Is kep alive session active */
	uint16	ka_sess_id;			/* id for the session */
	struct tcp_keep_alive_conn_info_t *next; /* Linked list */
} tcp_keep_conn_info_t;
...
#define TCP_KEEP_ALIVE_MAX_SESSION		4
#define TCP_KEEP_ALIVE_ONE_SEC_TICK		1000
...
struct tcp_keep_info {
	...
	tcp_keep_conn_info_t	*tcp_keep_conn_info;
...
void wl_tcp_setup_keep_pkt(wl_tcp_keep_info_t *tcpkeepi,
    tcp_keep_conn_info_t *tcp_keepalive_connection, void *keep_pkt);
...
/* IOVar table */
enum {
	IOV_TCPKA_CONN_ADD,		/*  TCP Keepalive ADD Session  */
	IOV_TCPKA_ENABLE,		/*  TCP Keepalive ADD Session  */
	IOV_TCPKA_CONN_DEL		/*  TCP Keepalive DEL Session  */
};

static const bcm_iovar_t tcp_keepalive_iovars[] = {
	{"tcpka_conn_add", IOV_TCPKA_CONN_ADD,
	(0), IOVT_BUFFER, 0
	},
	{"tcpka_conn_enable", IOV_TCPKA_ENABLE,
	(0), IOVT_BUFFER, 0
	},
	{"tcpka_conn_del", IOV_TCPKA_CONN_DEL,
	(0), IOVT_UINT16, 0
	},
	{NULL, 0, 0, 0, 0 }
};
...
#ifndef BCM_OL_DEV
	wlc_mac_event(tcp_keep_info->wlc, WLC_E_WAKE_EVENT, NULL, WLC_E_STATUS_SUCCESS,
		wake_reason, 0, 0, 0);
#endif // endif
}
...
{
	tcp_keep_conn_info_t *tcpka_info;

	tcpka_info = info->tcp_keep_conn_info;

	info->tcp_keep_enabled = 0;	/* we are waking up host */
	info->tcp_wowl_pme_asserted = TRUE;

	while (tcpka_info) {
		tcpka_info->keep_alive_sec_tics = 0;
		tcpka_info->ack_timeout_sec_tics = 0;
		tcpka_info->ka_session_active = FALSE;
		tcpka_info = tcpka_info->next;
	}
}

/* Send Keep alive packet if there is added session */
static void
wl_send_tcp_keep_alive(wl_tcp_keep_info_t *info)
{
	tcp_keep_conn_info_t *tcpka_info;
	...
	while (tcpka_info) {
		...
		if (tcpka_info->ka_session_active == TRUE) {
			/* Ready to send if reaches the interval/preriod time */
			if (tcpka_info->keep_alive_sec_tics == tcpka_info->period) {
				wl_tcp_setup_keep_pkt(info, tcpka_info, info->keep_pkt);
				tcp_keep_send_pkt(info, info->keep_pkt);
				tcpka_info->keep_alive_sec_tics = 0;
				tcpka_info->ack_timeout_sec_tics = 0;
			}
			else
				tcpka_info->keep_alive_sec_tics++;
		}
...
tcp_keepalive_timer_cb(void *arg)
{
	...
	while (tcpka_info) {
		...
		if (tcpka_info->ka_session_active == TRUE) {
			/* Ready to send if reaches the interval/preriod time */
			if (tcpka_info->ack_timeout_sec_tics == tcpka_info->timeout) {
				WL_ERROR(("%s: KEEP-ALIVE retry fail, wake host\n", __FUNCTION__));
				tcp_keepalive_make_reset(info, info->keep_pkt, NULL);
				tcp_wowl_assert_pme(info, WL_WOWL_TCPKEEP_TIME, NULL, TRUE);
				return;
			}
			else
				tcpka_info->ack_timeout_sec_tics++;
		}
			tcpka_info = tcpka_info->next;
	}

	wl_send_tcp_keep_alive(info);
	wl_add_timer(info->wlc->wl, info->tcp_keepalive_timer, TCP_KEEP_ALIVE_ONE_SEC_TICK, FALSE);
}

/**
 * Initialize TCP private context.
 * Returns a pointer to the TCP private context, NULL on failure.
 */
wl_tcp_keep_info_t *
BCMATTACHFN(wl_tcp_keep_attach)(wlc_info_t *wlc)
{
	...
	info = MALLOC(wlc->osh, sizeof(wl_tcp_keep_info_t));
	if (info == NULL) {
	...
	/* Keep Alive Info structure is allocated here for MAX sessions */
	info->tcp_keep_conn_info = MALLOC(wlc->osh, sizeof(tcp_keep_conn_info_t));
	if (info->tcp_keep_conn_info == NULL) {
		WL_ERROR(("wl%d: %s: MALLOC failed; total mallocs %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto fail;
	}

	bzero(info->tcp_keep_conn_info, sizeof(*(info->tcp_keep_conn_info)));

	tcpka_info = info->tcp_keep_conn_info;

	for (num_sess = 0; num_sess < TCP_KEEP_ALIVE_MAX_SESSION; num_sess ++) {
		info->tcp_keep_conn_info->next = MALLOC(wlc->osh, sizeof(tcp_keep_conn_info_t));
		if (info->tcp_keep_conn_info->next == NULL) {
			WL_ERROR(("wl%d: %s: MALLOC failed; total mallocs %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			goto fail;
		}

		info->tcp_keep_conn_info = info->tcp_keep_conn_info->next;
		bzero(info->tcp_keep_conn_info, sizeof(*(info->tcp_keep_conn_info)));
	}
	info->tcp_keep_conn_info = tcpka_info;
...
/* Attach Keepalive functionalities */
void
BCMATTACHFN(wl_tcp_keep_detach)(wl_tcp_keep_info_t *tcp_keep_info)
{
	...
	if (tcp_keep_info->keep_pkt != NULL) {
		...
		tcp_keep_info->keep_pkt = NULL;
	}
	if (tcp_keep_info->wake_pkt != NULL) {
		PKTFREE(WLCOSHK(tcp_keep_info), tcp_keep_info->wake_pkt, FALSE);
		tcp_keep_info->wake_pkt = NULL;
	}
	if (tcp_keep_info->tcp_keepalive_timer != NULL)
		wl_free_timer(tcp_keep_info->wlc->wl, tcp_keep_info->tcp_keepalive_timer);
	while (tcp_keep_info->tcp_keep_conn_info) {
		tcpka_info = tcp_keep_info->tcp_keep_conn_info;
		tcp_keep_info->tcp_keep_conn_info = tcp_keep_info->tcp_keep_conn_info->next;
		if (tcpka_info != NULL)
			MFREE(WLCOSHK(tcp_keep_info), tcpka_info, sizeof(tcp_keep_conn_info_t));
	}
...
/* IOVAR functions */
static int
tcp_keepalive_doiovar(void *hdl, const bcm_iovar_t *vi, uint32 actionid, const char *name,
            void *params, uint plen, void *arg, int alen, int vsize, struct wlc_if *wlcif)
{
	wl_tcp_keep_info_t *info = hdl;
	int err = 0;
	int32 int_val = 0;
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_TCPKA_CONN_ADD): {
			tcp_keep_conn_info_t *tcpka_info;
			bool bFound = FALSE;
			uint id_pos = 0;
			struct tcpka_conn *tcpka_in = (struct tcpka_conn *)arg;
			tcpka_conn_t	tcpka;

			/* Fill the Keep Alive info */
			bcopy((uint8*)params, (uint8 *) &tcpka, sizeof(struct tcpka_conn));

			/* Max sessions added */
			if (info->num_conn_recs >= TCP_KEEP_ALIVE_MAX_SESSION) {
				err = BCME_ERROR;
				break;
			}
			...
			id_pos = 1;
			while (tcpka_info) {
				if (!(tcpka_info->ka_sess_id)) {
					/* Fill Tcp info */
					tcpka_info->period = tcpka.period;
					tcpka_info->timeout = tcpka.timeout;
					bcopy(&tcpka.dst_mac, &tcpka_info->dst_mac, ETHER_ADDR_LEN);
					bcopy(&info->wlc->pub->cur_etheraddr,
						&tcpka_info->src_mac, ETHER_ADDR_LEN);
					bcopy(&tcpka.src_ip, &tcpka_info->src_ip, IPV4_ADDR_LEN);
					bcopy(&tcpka.dst_ip, &tcpka_info->dst_ip, IPV4_ADDR_LEN);
					tcpka_info->ipid = tcpka.ipid;
					tcpka_info->src_port = tcpka.srcport;
					tcpka_info->dst_port = tcpka.dstport;
					tcpka_info->seq_num = tcpka.seq;
					tcpka_info->keep_alive_sec_tics = 0;
					tcpka_info->ack_timeout_sec_tics = 0xFFFF;
					tcpka_info->ka_sess_id = id_pos;
					tcpka_in->sess_id = id_pos;
					bFound = TRUE;
					break; /* Add entry and break from for loop */
				}
		...
		case IOV_SVAL(IOV_TCPKA_ENABLE): {

			tcp_keep_conn_info_t *tcpka_info;
			bool bFound = FALSE;
			tcpka_conn_sess_t tcpka;

			bcopy((uint8*)params, (uint8 *) &tcpka, sizeof(struct tcpka_conn_sess));

			tcpka_info = info->tcp_keep_conn_info;

			while (tcpka_info) {
				if (tcpka_info->ka_sess_id == tcpka.sess_id) {
					tcpka_info->ka_session_active = tcpka.flag;
					tcpka_info->keep_alive_sec_tics = 0;
					tcpka_info->ack_timeout_sec_tics = 0xFFFF;
					bFound = TRUE;
					break; /* Enable Active flag */
				}
				tcpka_info = tcpka_info->next;
			}
			...
			if (tcpka.flag == TRUE) {
				/* Keep alive functionality enabled if it was not */
				if (!info->tcp_keep_enabled) {
					info->tcp_keep_enabled = TRUE;
					wl_add_timer(info->wlc->wl, info->tcp_keepalive_timer,
					TCP_KEEP_ALIVE_ONE_SEC_TICK, FALSE);
				}
			} else {
				if (!info->tcp_keep_enabled)
					wl_del_timer(info->wlc->wl, info->tcp_keepalive_timer);
			}
			break;
		}

		case IOV_SVAL(IOV_TCPKA_CONN_DEL): {

			tcp_keep_conn_info_t *tcpka_info;
			bool bFound = FALSE;
			uint16 sess_id = 0;

			/* Get the session id and Delete the entry */
			sess_id = (uint16)int_val;

			/* Do we have sessions? */
			if (info->num_conn_recs <= 0) {
				err = BCME_NOTFOUND;
			break;
			}
			...
			while (tcpka_info) {
				if (tcpka_info->ka_sess_id == sess_id) {
					tcpka_info->ka_session_active = FALSE;
					tcpka_info->ka_sess_id = 0;
					bFound = TRUE;
					break; /* Del entry is done break from loop */
				}
				tcpka_info = tcpka_info->next;
			}
...
wl_tcpkeep_parse(wl_tcp_keep_info_t *tcp_keep_info, void *sdu)
{
	...
	uint32 psum;
	char tcp_flags[0];
	tcp_keep_conn_info_t *tcp_keepalive_connection;
	tcp_keep_conn_info_t *tcpka_info;
	...
	while (tcpka_info) {
		/* Make sure session is active */
		if (tcpka_info->ka_session_active == TRUE) {
			tcp_keepalive_connection = tcpka_info;
			if (bcmp(&tcp_keepalive_connection->dst_ip,
				ipv4_in->src_ip, IPV4_ADDR_LEN) != 0) {
				WL_TRACE(("%s: Dropping, Came from wrong machine\n", __FUNCTION__));
				return -1;
			}
			if (bcmp(&tcp_keepalive_connection->src_ip,
				ipv4_in->dst_ip, IPV4_ADDR_LEN) != 0) {
				WL_TRACE(("%s: Dropping, Heading to wrong machine\n",
					__FUNCTION__));
				return -1;
			}
			if (ntoh16(tcp_in->dst_port) != tcp_keepalive_connection->src_port) {
				WL_TRACE(("%s: Dropping, Heading to wrong port\n",
					__FUNCTION__));
				return -1;
			}
			if (ntoh16(tcp_in->src_port) != tcp_keepalive_connection->dst_port) {
			...
			if ((tcp_flags[1] & 0x3f) != TCP_FLAG_ACK) {
				WL_ERROR(("wl_tcpkeep_parse: flags not just ACK: %x\n",
					tcp_flags[1]));
				tcp_keepalive_make_reset(tcp_keep_info,
					tcp_keep_info->keep_pkt, NULL);
				tcp_wowl_assert_pme(tcp_keep_info, WL_WOWL_TCPKEEP_DATA,
					sdu, FALSE);
				return -1;
			}
			...
			if ((tcp_keepalive_connection->ack_num == ntoh32(tcp_in->seq_num)) &&
				((tcp_keepalive_connection->seq_num + 1)
					== ntoh32(tcp_in->ack_num))) {
				...
				tcp_keepalive_connection->ack_timeout_sec_tics = 0;
				return 0;
			}
		}
    ...
    tcp_keep_conn_info_t *tcp_keepalive_connection, void *keep_pkt)
{
	...
	bcopy(&tcp_keepalive_connection->dst_mac, eth->ether_dhost, ETHER_ADDR_LEN);
	bcopy(&tcp_keepalive_connection->src_mac, eth->ether_shost, ETHER_ADDR_LEN);
	eth->ether_type = hton16(ETHER_TYPE_IP);
	ip = (struct ipv4_hdr *)(keep_pkt + sizeof(struct ether_header));
	bcopy(&tcp_keepalive_connection->dst_ip, ip->dst_ip, IPV4_ADDR_LEN);
	bcopy(&tcp_keepalive_connection->src_ip, ip->src_ip, IPV4_ADDR_LEN);

	/* Fill IP */
	ip->version_ihl = 0x45;		/* standard value */
	ip->tos = 0;
	ip->tot_len = hton16(sizeof(struct ipv4_hdr) + sizeof(struct bcmtcp_hdr));
	ip->id = hton16(tcp_keepalive_connection->ipid);
	ip->frag = 0;
	...
	tcp->src_port = hton16(tcp_keepalive_connection->src_port);
	tcp->dst_port = hton16(tcp_keepalive_connection->dst_port);
	tcp->seq_num = hton32(tcp_keepalive_connection->seq_num);
	tcp->ack_num = hton32(tcp_keepalive_connection->ack_num);
	tcp->tcpwin = hton16(tcp_keepalive_connection->tcpwin);
	tcp->chksum = 0;
	tcp->urg_ptr = 0;
	tcp->hdrlen_rsvd_flags = ((sizeof(struct bcmtcp_hdr) >> 2) << TCP_HDRLEN_SHIFT);
    ...
    void *buf)
{
	...
	tcpka_info = tcpkeepi->tcp_keep_conn_info;
	...
	switch (msg_hdr->type) {
		...
		case BCM_OL_TCP_KEEP_CONN:
			...
			wl_tcp_setup_keep_pkt(tcpkeepi, tcpka_info,
			    tcpkeepi->keep_pkt);
...
BCMATTACHFN(wl_icmp_attach)(wlc_info_t *wlc)
{
	...
	icmpi = MALLOC(wlc->osh, sizeof(wl_icmp_info_t));
	if (!icmpi) {
		WL_ERROR(("wl%d: %s: MALLOC failed; total mallocs %d bytes\n",
		          wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return NULL;
	...
	if (ethertype == ETHER_TYPE_IP) {
		...
		switch (icmpv4_in->type) {
			case ICMP_TYPE_ECHO_REQUEST:
				...
				icmp_send_echo_reply(icmpi, sdu, snap,
		...
		switch (icmpv6_in->icmp6_type) {
			case ICMP6_ECHO_REQUEST:
				...
				icmpv6_send_echo_reply(icmpi, sdu, snap, icmp_len +
