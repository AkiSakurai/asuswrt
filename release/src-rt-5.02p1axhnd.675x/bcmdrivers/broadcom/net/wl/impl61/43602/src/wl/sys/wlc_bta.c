 ...
 *
 * $Id: wlc_bta.c 593242 2015-10-15 20:31:28Z $
 */
...
BCMATTACHFN(wlc_bta_attach)(wlc_info_t *wlc)
{
	...
	if ((bta = (bta_info_t *)MALLOC(wlc->osh, sizeof(bta_info_t))) == NULL) {
	...
	bta->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(scb_bta_t),
		...
		scb_bta_init, scb_bta_deinit, NULL, (void *)bta, 0);
#else
...
wlc_bta_alloc_hcievent(bta_info_t *bta, uint8 ecode, uint8 plen)
{
	...
	if ((buf = MALLOC(bta->wlc->osh, buf_size)) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
...
wlc_bta_doconnect(bta_info_t *bta, int plidx, chanspec_t chanspec)
{
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, chanspec)) {
		...
		WL_ERROR(("wl%d: %s: invalid channel (%d) to build local AMP_ASSOC"
			" for physical link %d\n", wlc->pub->unit, __FUNCTION__,
			CHSPEC_CHANNEL(chanspec), plh));
	...
	if (bta->bsscfg_creator == NULL) {
		...
		else if (wlc_bsscfg_bta_init(wlc, bsscfg)) {
...
wlc_bta_cs_complete(void *arg, int st)
{
	...
	if (st == WLC_E_STATUS_SUCCESS) {
		status = wlc_bta_doconnect(bta, plidx, wlc->chanspec_selected);
		if (status == HCI_SUCCESS)
			return;
	...
	for (i = 0; i < req->count; i ++) {
		...
		for (j = 0; j < local->count; j ++) {
			...
			if (chan == 0 &&
				...
				continue;
			if (c != CHSPEC_CHANNEL(local->element[j]))
				continue;
	...
	chanspec_t *chanspec_sel)
{
	...
	if (local_non_country) {
		...
		for (i = (uint8)cur->count; i >= 1; i --)
			if (CHSPEC_CHANNEL(cur->element[i - 1]) > 11)
				cur->count --;
	}
#ifdef BAND5G
	...
	if (bta->pl[plidx].flags & BTA_PL_CREATOR) {
		...
		ret = wlc_cs_scan_start(wlc->cfg, req, FALSE, TRUE, band, APCS_BTA,
	...
	void *p, uint plen, void *a, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_BT_AMP_CHAN):
		*((uint32 *)a) = CHSPEC_CHANNEL(bta->chanspec_user);
		break;
