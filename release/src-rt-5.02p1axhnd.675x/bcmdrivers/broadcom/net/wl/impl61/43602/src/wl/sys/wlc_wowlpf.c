 ...
 *
 * $Id: wlc_wowlpf.c 300516 2011-12-04 17:39:44Z $
 */
...
static wowlpf_info_t *wowlpf;

#ifdef USB_XDCI
extern void xdc_dongle_sleep(void);
#endif // endif
...
wlc_wowlpf_enable(wowlpf_info_t *wowl)
{
	...
	*/
	...
	if (!(wowl->flags_user & WL_WOWL_UNASSOC)) {
		...
		if (BSSCFG_STA(wlc->cfg) && !WSEC_SCB_KEY_VALID(scb) &&
...
#ifdef USB_XDCI
		if ((wowl->dngldown) == DNGLDN_WLCMD_ENABLE) {
			xdc_dongle_sleep();
		}
#endif // endif
...
bool
wlc_wowlpf_wlcmd_wakeup(void)
{
	return ((wowlpf->dngldown == DNGLDN_WLCMD_ENABLE)? TRUE : FALSE);
}
#endif // endif
