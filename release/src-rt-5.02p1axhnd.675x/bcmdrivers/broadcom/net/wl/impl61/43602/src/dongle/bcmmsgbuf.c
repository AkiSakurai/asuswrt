...
void *proto_attach(osl_t *osh, struct dngl *dngl, struct dngl_bus *bus,
...
void BCMATTACHFN(proto_detach)(void *proto)
{
...
void proto_ctrldispatch(void *proto, void *p, uchar *ioct_resp_buf)
{
	dngl_proto_info_t *dngl_proto = (dngl_proto_info_t *)proto;
	...
	if (flags & MSGBUF_IOC_ACTION_MASK) {
		/* if inlen is 0 then set buf to NULL */
		if (inlen == 0) {
			buf = NULL;
		}
		...
		/* if outlen is 0 then set buf to NULL */
		if (outlen == 0) {
			buf = NULL;
		}
...
void *proto_pkt_header_push(void *proto, void *p)
{
	dngl_proto_info_t *dngl_proto = (dngl_proto_info_t *)proto;
	cmn_msg_hdr_t *msg = NULL;
	uint16 hdrlen;
	...
	if (PKTHEADROOM(osh, p) < hdrlen) {
		...
		if (PKTISRXFRAG(osh, p)) {
			...
			PKTSETFRAGDATA_LO(osh, (p1), 1,
					(PKTFRAGDATA_LO(osh, (p), 1)));
			PKTSETFRAGDATA_HI(osh, (p1), 1,
					(PKTFRAGDATA_HI(osh, (p), 1)));
			PKTSETFRAGLEN(osh, (p1), 1, (PKTFRAGLEN(osh, (p), 1)));
			...
			/* Alloc a packet that will fit all the data; chaining
			 * the header won't work
			 */
...
int proto_pkt_header_pull(void *proto, void *p)
{
	return 0;

	/* Nothing to be populated here */
}
void proto_dev_event(void *proto, void *data)
{
