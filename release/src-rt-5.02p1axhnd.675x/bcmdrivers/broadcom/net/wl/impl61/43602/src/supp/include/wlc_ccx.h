...
/* exported externs */
#ifdef BCMCCX
extern ccx_t *wlc_ccx_attach(wlc_pub_t *pub, wlc_info_t *wlc);
...
extern int wlc_ccx_parse_ie(ccx_t *ccx, uint8 *tlvs, int tlvs_len, wlc_bss_info_t *bi);
extern int wlc_ccx_chk_iapp_frm(ccx_t *ccxh, struct wlc_frminfo *f, void *rx_ctxt);
extern void wlc_ccx_parse_assoc_resp_ie(ccx_t *ccx, uint8 *ies, uint ies_len,
	wlc_bsscfg_t *bsscfg);
extern void wlc_ccx_update_BSS(ccx_t *ccx, uint16 rxTSFtime, uint rx_phy,
	wlc_bss_info_t *BSS);
extern void wlc_ccx_process_bcn(ccx_t *ccx, uint8 *tag_params, int params_len);
...
extern uint8 *wlc_ccx_append_ies(ccx_t *ccxh, uint8 *pbody, wlc_bss_info_t *bi, bool reassoc,
	uint16 wpa_auth, struct ether_addr *curea, wpa_ie_fixed_t* wpa_ie);
#ifdef CCX_SDK
...
extern int wlc_ccx_mfp_rx(ccx_t *ccx, d11rxhdr_t *rxh,
...
extern void wlc_ccx_rm_frm(ccx_t *ccx, d11rxhdr_t *rxh, struct dot11_header *h, uint len);
