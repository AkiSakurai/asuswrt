...
#include <bcmparams.h> /* BCM4709ACDCRH_CPUMAP */
...
int
fwder_affinity(int fwder_unit)
{
	fwder_cpumap_t * cpumap;

	FWDER_TRACE(("%s fwder_unit<%d>\n", __FUNCTION__, fwder_unit));
	FWDER_ASSERT((fwder_unit < FWDER_MAX_RADIO));

	cpumap = &fwder_cpumap_g[fwder_unit];

	/* Setup IRQ affinity */
	irq_set_affinity(cpumap->irq, cpumask_of(cpumap->cpu));
}
             ...
             fwder_bypass_fn_t bypass_fn, struct net_device * dev, void * osh)
{
	...
	{	/* Check if 3 GMAC based HW switching is enabled. */
		const char * fwder_enabled_nvar;
		int fwder_enabled = 0;
		fwder_enabled_nvar = getvar(NULL, FWDER_ENABLED_NVAR);
		if (fwder_enabled_nvar)
			fwder_enabled = bcm_atoi(fwder_enabled_nvar);
		if (fwder_enabled == 0)
			return FWDER_NULL;
	}
