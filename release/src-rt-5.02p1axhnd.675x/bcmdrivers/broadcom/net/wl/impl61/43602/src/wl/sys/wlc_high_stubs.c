 ...
 *
 * $Id: wlc_high_stubs.c 649365 2016-07-15 19:54:06Z $
 */
...
#define HANDLE_BOUNDARY(b, p, prev_p)						\
	...
	case WLRPC_WLC_BMAC_BTC_PERIOD_GET_ID: {
		...
		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb,
			3 * sizeof(uint32), WLRPC_WLC_BMAC_BTC_PERIOD_GET_ID);
		ASSERT(rpc_buf != NULL);
		...
		err = bcm_xdr_pack_int32(&retb, (uint32)agg_off_bm);
		ASSERT(!err);
	...
	case WLRPC_WLC_BMAC_DUMP_ID: {
		...
		init_len = 2*sizeof(int32) + ROUNDUP(len, sizeof(uint32));
		...
		bcm_binit(&strb, (char*)retb.buf + sizeof(uint32) + sizeof(uint32), len);

		/* wlc_bmac_dump() will pack the string directly into rpc_buf */

		wlc_bmac_dump(wlc_hw, &strb, dump_id, &offset);
		err = bcm_xdr_pack_int32(&retb, offset);
		ASSERT(!err);
		/* only need to pack the real len used */
		actual_len = strb.origsize - strb.size;
		err = bcm_xdr_pack_uint32(&retb, actual_len);
		ASSERT(!err);
		final_len = 2*sizeof(int32) + ROUNDUP(actual_len, sizeof(uint32));
...
#ifdef WL_WOWL_MEDIA
		wl_wowl_dngldown(wlc->wl, USB_SHUTDOWN);
#endif /* WL_WOWL_MEDIA */
		break;
	}
	...
	case WLRPC_WLC_PHY_CALC_PPR_PWR_CAP_ID: {
		...
		limit_val = wlc_phy_calc_ppr_pwr_cap(
			((wlc_hw_info_t*)wlc_hw)->band->pi, (uint8)ret);

		err = bcm_xdr_pack_uint32(&retb, limit_val);
		ASSERT(!err);
	...
	case WLRPC_WLC_PHY_SET_LOCALE: {
		...
		wlc_phy_set_locale(((wlc_hw_info_t *)wlc_hw)->band->pi, region_group);
		break;
	}
