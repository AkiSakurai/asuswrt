 ...
 * $Id: bcmstdlib.h 676784 2016-12-24 16:10:17Z $:
 */
...
/* printf hook: 3 arguments:
 *  1. buffer
 *  2. size
 *  3. Any additional info
 *  return value > 0 => all arguments handled. No need to execute default printf()
 *  <= 0 => default printf() need to run
 *  return value depends on what the hook does. In case of event logs, it returns number
 *  of bytes "printed" to the event log buffer
 */
typedef int (*post_printf_hook)(char* buf, int size, void* info);
extern post_printf_hook get_current_post_printf_hook(void);
extern void register_post_printf_hook(post_printf_hook fn);
extern void unregister_post_printf_hook(void);
#endif // endif
