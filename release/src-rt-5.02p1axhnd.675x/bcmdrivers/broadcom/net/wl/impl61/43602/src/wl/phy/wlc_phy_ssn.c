 ...
 *
 * $Id: wlc_phy_ssn.c 430789 2013-10-21 10:52:43Z $
 */

/* XXX this file supports all phy_type(a/b/g/n/lp/ssn/qn) and radio(20xx)
 *  it is organized as following major sections
 *         inter-module connection
 *         macros
 *         typedef, enum, struct, global variable
 *         local function prototype
 *         function implementation
 *  , where each section contains common part first, then phy_type specific part
 *
 *  The function naming convention is
 *    wlc_phy_submodule_noun_verb_phy-type
 *    e.g. wlc_phy_txpwrctrl_srom_read_nphy
 *         wlc_phy_txpwrctrl_srom_read_gphy
 *    the phy-type postfix is used in the hope that it will be eliminated once we
 *    merge/share phy-type specific functions
 *
 *  To reduce code size,
 *   - #if xxx should be used. e.g.
 *        #if NCONF will prevent the code from going into legacy build
 *        internal testing code should be under #if BCMINTERNAL so customer won't get it
 *        #if WLTEST is needed for manufacturing test support
 *        reclaimable function(not needed after driver up) should be inside BCMINITFN()
 *   - share/reuse macros, struct, function as much as possible
 *
 *  BMAC: to remove radio_disabled
 *        make radar detection a mode, move pub->radar out
 *        move WL11H_EN and radar_chanspec out
*/
#include <wlc_cfg.h>
...
wlc_sslpnphy_btcx_override_enable(phy_info_t *pi)
{
	...
	if (CHSPEC_IS2G(pi->radio_chanspec) && (wlc_hw->machwcap & MCAP_BTCX)) {
		...
		OR_REG(GENERIC_PHY_INFO(pi)->osh, &pi->regs->PHYREF_BTCX_CTRL, BTCX_CTRL_EN | BTCX_CTRL_SW);

		/* Set BT priority & antenna to allow A2DP to catchup */
		AND_REG(GENERIC_PHY_INFO(pi)->osh,
			&pi->regs->PHYREF_BTCX_TRANS_CTRL, ~(BTCX_TRANS_TXCONF | BTCX_TRANS_ANTSEL));

		if (wlapi_is_eci_coex_enabled(pi->sh->physhim)) {
			/* Wait for A2DP to flush all pending data */
			W_REG(GENERIC_PHY_INFO(pi)->osh, &pi->regs->PHYREF_BTCX_ECI_ADDR, 3);
			for (delay_val = 0, eci_busy_cnt = 0;
				delay_val < BTCX_FLUSH_WAIT_MAX_MS * 10; delay_val++) {
				/* Make sure ECI update is not in progress */
				if ((eci_m = R_REG(GENERIC_PHY_INFO(pi)->osh, &pi->regs->PHYREF_BTCX_ECI_DATA))
					& 0x4000) {
					if (!(a2dp = (eci_m & 0xf))) {
...
pri_wlan:
		/* Set WLAN priority */
		OR_REG(GENERIC_PHY_INFO(pi)->osh, &pi->regs->PHYREF_BTCX_TRANS_CTRL, BTCX_TRANS_TXCONF);

		/* Wait for BT activity to finish */
		delay_val = 0;
		while (R_REG(GENERIC_PHY_INFO(pi)->osh, &pi->regs->PHYREF_BTCX_STAT) & BTCX_STAT_RA) {
		...
		OR_REG(GENERIC_PHY_INFO(pi)->osh,
			&pi->regs->PHYREF_BTCX_TRANS_CTRL, BTCX_TRANS_ANTSEL | BTCX_TRANS_TXCONF);
	}
...
/* XXX MOGRIFY REVISIT  This doesn't mogrify away WLTEST and BCMXX correctly.
 *
 * #if 0 || \
 *   defined(WLTEST) || defined(BCMCCX)
 *
 * is the result.
 */
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMCCX) || defined(WLCURPOWER) || \
	!defined(ROMTERMPHY)
void
	...
	uint32 *maxpwr, bool *measurement_valid)
{
	...
	*maxpwr = 0;
	...
	WL_WRITE_SHM(pi, (2 * (sslpnphy_shm_ptr +
		M_55f_REG_VAL)), 0);
	W_REG(GENERIC_PHY_INFO(pi)->osh, &pi->regs->maccommand, MCMD_BG_NOISE);
...
wlc_sslpnphy_radio_2063_channel_tune(phy_info_t *pi, uint8 channel)
{
	...
	/* XXX STALE:
	 * fixed qFxtal = wlc_lpphy_qdiv_roundup(PHY_XTALFREQ(pi->xtalfreq), PLL_2063_MHZ, 16);
	 */
...
#endif /* WLMEDIA_APCS */
void
BCMATTACHFN(wlc_phy_interference_mode_attach_sslpnphy)(phy_info_t *pi)
{
	if (SSLPNREV_IS(pi->pubpi.phy_rev, 3)) {
