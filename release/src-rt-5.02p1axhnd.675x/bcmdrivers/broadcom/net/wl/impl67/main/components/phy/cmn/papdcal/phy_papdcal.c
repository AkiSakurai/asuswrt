 ...
 *
 * $Id: phy_papdcal.c 787578 2020-06-04 01:28:31Z $
 */
...
int
phy_papdcal_get_ipaoff(phy_info_t *pi, int32* ipaofftime)
{
	phy_type_papdcal_fns_t *fns = pi->papdcali->priv->fns;

	if (fns->get_ipaoff != NULL) {
		return (fns->get_ipaoff)(fns->ctx, ipaofftime);
	} else {
		return BCME_UNSUPPORTED;
	}
}
...
int
phy_papdcal_set_ipaoff(phy_info_t *pi, int32 ipaofftime)
{
	phy_type_papdcal_fns_t *fns = pi->papdcali->priv->fns;

	if (fns->set_ipaoff != NULL) {
		return (fns->set_ipaoff)(fns->ctx, ipaofftime);
	} else {
		return BCME_UNSUPPORTED;
	}
}
...
int
phy_papdcal_get_cts2self(phy_info_t *pi, int32* cts2selftime)
{
	phy_type_papdcal_fns_t *fns = pi->papdcali->priv->fns;

	if (fns->get_cts2self != NULL) {
		return (fns->get_cts2self)(fns->ctx, cts2selftime);
	} else {
		return BCME_UNSUPPORTED;
	}
}
...
int
phy_papdcal_set_cts2self(phy_info_t *pi, int32 cts2selftime)
{
	phy_type_papdcal_fns_t *fns = pi->papdcali->priv->fns;

	if (fns->set_cts2self != NULL) {
		return (fns->set_cts2self)(fns->ctx, cts2selftime);
	} else {
		return BCME_UNSUPPORTED;
	}
}
