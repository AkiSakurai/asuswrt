 ...
 *
 *	$Id: acs_channels.c 788834 2020-07-13 11:52:13Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define CHANNEL_6G_BAND_GROUP(c) \
	(c < 17) ? 1 : ((c < 33) ? 2 : ((c < 49) ? 3 : ((c < 65) ? 4 : ((c < 81) ? 5 : ((c < 97) ? 6 : ((c < 113) ? 7 : \
	((c < 129) ? 8 : ((c < 145) ? 9 : ((c < 161) ? 10 : ((c < 177) ? 11 : ((c < 193) ? 12 : ((c < 209) ? 13 : \
	((c < 225) ? 14 : 15)))))))))))))
struct tc {
	char *prefix;
	int index;
...
#if defined(GTAXE11000)
double acs_asus_adjust_txpwr_score(BandType bt, int chan, int is_dfs, int is_160m)
{
	double score_pwr = 0, score_dfs = 0, score_bw160=0, score_adj = 0;
	int ch_5g_band_grp;
	int ch_6g_band_grp;
#if 0
	ACSD_PRINT("band[%d],chan[%d] is_dfs[%d], is_160m[%d] => ",
			bt, chan, is_dfs, is_160m);
#endif
	switch(bt) {
	    case BAND_TYPE_2G:
		if(chan < 2 || chan > 10)
	    ...
	    case BAND_TYPE_5G:
		ch_5g_band_grp = CHANNEL_5G_BAND_GROUP(chan);
		if(ch_5g_band_grp == 1) {
			score_pwr = 17 + (chan - 36)/4;
			score_dfs = 20;
			score_bw160 = is_160m ? 10 : 0;
		}
		else if(ch_5g_band_grp == 2 || ch_5g_band_grp == 3 || ch_5g_band_grp == 4 || ch_5g_band_grp == 5) {
			score_pwr = 15;
			score_dfs = is_dfs ? 10 : 0;
			score_bw160 = is_160m ? (is_dfs ? (ch_5g_band_grp == 5 ? 0 : 10) : 0) : 0;
		}
		else if(ch_5g_band_grp == 6) {
			score_pwr = 22;
			score_dfs = 20;
			score_bw160 = 0;
		}
	    ...
	    case BAND_TYPE_6G:
		ch_6g_band_grp = CHANNEL_6G_BAND_GROUP(chan);
		if (ch_6g_band_grp < 3)
			score_pwr = 16;
		else if (ch_6g_band_grp == 3)
			score_pwr = 15;
		else if (ch_6g_band_grp == 4)
			score_pwr = 14;
		else if (ch_6g_band_grp == 5 || ch_6g_band_grp == 6)
			score_pwr = 13;
		else if (ch_6g_band_grp == 7 || ch_6g_band_grp == 8)
			score_pwr = 12;
		else if (ch_6g_band_grp == 9 || ch_6g_band_grp == 10)
			score_pwr = 11;
		else if (ch_6g_band_grp == 11 || ch_6g_band_grp == 12)
			score_pwr = 10;
		else if (ch_6g_band_grp == 13 || ch_6g_band_grp == 14)
			score_pwr = 9;
		else
			score_pwr = 8;

		score_dfs = 0;
		score_bw160 = (ch_6g_band_grp == 15) ? 0 : (is_160m ? 2 : 1);
		break;
	}

	score_adj = -(score_pwr + score_dfs + score_bw160);
#if 0
	ACSD_PRINT("%.1f + %.1f + %.1f = %.1f\n", score_pwr, score_dfs, score_bw160, score_pwr + score_dfs + score_bw160);
#endif
	return score_adj;
}
#endif
...
txpwr_count_one:
			...
			for (j = 0; j < c_info->txpwr_params->count; j++) {
				...
				if (tc_index != -1) {
					...
					snprintf(prefix, sizeof(prefix), "wl%d_", c_info->unit);
					is_bw160 = nvram_get_int(strcat_r(prefix, "bw_160", tmp));
					...
					if(BAND_2G(c_info->rs_info.band_type)) {
						...
						band = BAND_TYPE_5G;
						is_dfs = nvram_get_int("acs_dfs") | nvram_get_int("acs_band3");
					}
					else if(BAND_6G(c_info->rs_info.band_type)) {
						band = BAND_TYPE_6G;
						is_dfs = 0;
					}
					else {
						candi[i].chscore[CH_SCORE_TXPWR].score = 0;
						return;
					}

					candi[i].chscore[CH_SCORE_TXPWR].score = acs_asus_adjust_txpwr_score(band, ctrlchan, is_dfs, is_bw160);
...
acs_candidate_score_cns(ch_candidate_t *candi, acs_chaninfo_t* c_info)
{
	 ...
	 */
	if (!hits) {
		ACSD_ERROR("%s: knoise check failed for chanspec: 0x%4x (%s)\n", c_info->name, chspec, wf_chspec_ntoa(chspec, chanspecbuf));
		return;
	}
...
acs_candidate_score_txop(ch_candidate_t *candi, acs_chaninfo_t* c_info)
{
	...
	if (done) {
		...
		ACSD_ERROR("%s: txop check failed for chanspec: 0x%4x (%s)\n", c_info->name, chspec, wf_chspec_ntoa(chspec, chanspecbuf));
		return;
	}
