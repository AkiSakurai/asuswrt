 ...
 *
 * $Id: phy_tof_api.h 786259 2020-04-22 16:27:58Z $
 */
...
int phy_tof_chan_freq_response_api(phy_info_t *pi,
	int len, int nbits, int32* Hr, int32* Hi, uint32* Hraw, const bool single_core,
	uint8 num_sts, bool collect_offset, bool tdcs_en);
...
int wlc_phy_kvalue(phy_info_t *pi, chanspec_t chanspec, uint32 rspecidx, uint32 *kip,
	uint32 *ktp, uint8 seq_en);
#else
int wlc_phy_tof(phy_info_t *pi, bool enter, bool hw_adj);
int wlc_phy_tof_info(phy_info_t *pi, int* p_frame_type, int* p_frame_bw, int8* p_rssi);
int wlc_phy_tof_kvalue(phy_info_t *pi, chanspec_t chanspec, uint32 *kip, uint32 *ktp);
#endif /* WL_PROXD_SEQ */
