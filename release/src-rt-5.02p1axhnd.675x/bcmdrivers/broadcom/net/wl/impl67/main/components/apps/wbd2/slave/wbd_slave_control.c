	...
	uint8 is_retry)
{
	...
	if (!i5DmIsMacWildCard(bss_info->bssid)) {
		...
		band = ieee1905_get_band_from_channel(ioctl_data->rclass, bss_info->target_channel);

		switch (band) {
			case BAND_2G:
			{
				ioctl_data->chanspec = wf_channel2chspec(bss_info->target_channel,
					bw, WL_CHANSPEC_BAND_2G);
			}
			break;
			case BAND_5GL:
			case BAND_5GH:
			...
			{
				ioctl_data->chanspec = wf_channel2chspec(bss_info->target_channel,
					bw, WL_CHANSPEC_BAND_5G);
			}
			break;
			case BAND_6G:
			{
				ioctl_data->chanspec = wf_channel2chspec(bss_info->target_channel,
					bw, WL_CHANSPEC_BAND_6G);
			}
			break;
		}
...
wbd_slave_process_bh_steer_request_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	if (WBD_IS_HAPD_ENABLED(info->flags)) {
		...
		band = ieee1905_get_band_from_channel(bh_steer_req->opclass,
				bh_steer_req->channel);

		switch (band) {
			case BAND_2G:
			{
				join_params->params.chanspec_list[0] =
					wf_channel2chspec(bh_steer_req->channel, bw,
						WL_CHANSPEC_BAND_2G);
			}
			break;
			case BAND_5GL:
	...
	wbd_best_bsta_wght_t *wght_cfg)
{
	...
	if (wght_cfg->flags & WBD_BEST_BSTA_WGHT_FLAG_MAX_RATE) {
		...
		blanket_get_max_rate(i5_ifr->ifname, NULL, &phyrate);
