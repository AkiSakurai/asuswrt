...
#define NUMBER_OF_REMOTES_6715			6
...
#define MIN_VLIMIT_16NM				700		/* Minimum operating voltage (mV) */
#define MAX_VLIMIT_28NM				1100		/* Maximum operating voltage (mV) */
#define MAX_VLIMIT_16NM				1100		/* Maximum operating voltage (mV) */
...
#define DEFAULT_VMARGIN_L_6715			40		/* Margin given in FF corner (mV) [BOARD] */
#define DEFAULT_VMARGIN_H_6715			40		/* Margin given in SS corner (mV) [BOARD] */
#define DEFAULT_VLIMIT_L_6715			700		/* Minimum operating voltage (mV) [BOARD] */
#define DEFAULT_VLIMIT_H_6715			910		/* Maximum operating voltage (mV) [BOARD] */
...
#define NOMINAL_VOLTAGE_16NM			800		/* Nominal voltage (mV) */
...
#define NUMBER_OF_OSCILLATORS(sih)		(NUMBER_OF_CENTRALS(sih) + 2*NUMBER_OF_REMOTES(sih))
#define NUMBER_OF_OSCILLATORS_MAX		(NUMBER_OF_CENTRALS_MAX + 2*NUMBER_OF_REMOTES_MAX)
...
static const osc_freq_t oscillator_thresholds_6710[] = {
 ...
 /* 6715 central oscillator frequency thresholds at 0.92V, SS, 125C condition, units are MHz */
	...
	switch (CHIPID(avs->sih->chip)) {
		CASE_BCM6715_CHIP:
			...
			if (osc_index < NUMBER_OF_CENTRALS(avs->sih)) {
				...
				freq = (osc_index % 2) ?
					oscillator_thresholds_6715[3] :		/* Remote HVT ROSC is a copy of Central ROSC 3 */
					oscillator_thresholds_6715[1];		/* Remote SVT ROSC is a copy of Central ROSC 1 */
			}
			break;
		CASE_BCM6710_CHIP:
			ASSERT(NUMBER_OF_CENTRALS(avs->sih) <=
...
read_remote_rosc_count(avs_context_t *avs, uint osc_num, osc_count_t *svt_count, osc_count_t *hvt_count)
{
	...
	*svt_count = *hvt_count = INVALID_OSCILLATOR_COUNT;
	...
	while(i--) {
		...
		reg = read_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL(avs->sih) + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
		if ((reg & REMOTE_ROSC_CONTROL_VALID) == REMOTE_ROSC_CONTROL_VALID) {
			/* Read counters */
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			reg = read_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT(avs->sih) + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
			sleep(PMB_REMOTE_ROSC_DELAY_US);
...
initialize_oscillators(avs_context_t *avs)
{
	...
	for(i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
		...
		write_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL(avs->sih) + (i * BCHP_AVS_PMB_OFFSET(avs->sih)),
...
reset_measurements(avs_context_t *avs, reset_t reset)
{
	...
	if (reset & RESET_REMOTE) {
		...
		for (i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
			...
			read_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT(avs->sih) + (i * BCHP_AVS_PMB_OFFSET(avs->sih)));
			sleep(PMB_REMOTE_ROSC_DELAY_US);
		}
	}
}
...
find_nominal_voltage(avs_context_t *avs, dac_code_t initial_dac_code)
{
	...
	AVS_INFORM(("%s: v=%umV initial_dac=%u\n", __FUNCTION__, NOMINAL_VOLTAGE(avs->sih), dac_code));
	...
	while(iterations--) {
		...
		voltage_delta = (int)(voltage - MV_TO_INT(NOMINAL_VOLTAGE(avs->sih)));
...
initialize_avs(avs_context_t *avs)
{
	...
	avs->local_fit_dac_offset = (dac_code_t)((MV_TO_INT(LOCAL_FIT_DAC_OFFSET) * 1000 /* mV */) / avs->dac_min_vstep);
	ASSERT(avs->local_fit_dac_offset > 0);

	AVS_INFORM(("local fit: dac_offset=%u (%umV)\n", avs->local_fit_dac_offset, LOCAL_FIT_DAC_OFFSET));
