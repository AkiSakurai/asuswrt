 ...
 *
 * $Id: wlc.c 791272 2020-09-22 09:25:29Z $
 */
...
static void wlc_init_tpe_regulatory(wlc_info_t *wlc);
...
#if BAND6G || defined(WL_OCE_AP)
	wlc->nbr_discovery_cap = 0; /* TODO:6GHZ: WLC_6G_CAP_NO_WILDCARD_PRBREQ */
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	*/
...
#ifdef WLOLPC
		if (OLPC_ENAB(wlc) && (qdbm < wlc_phy_tssivisible_thresh(pi))) {
	...
	{
		if (IS_WLC_6G_CAP_BOTH_FD_BCAST_PRB_RSP_BIT_SET((uint32)int_val)) {
		...
		/* 6G_CAP_HW_MANAGED_UPR_FD part is not tested, will be tested once
		 * ucode support is available
		 */
		if (isset(&(wlc->nbr_discovery_cap), WLC_6G_CAP_HW_MANAGED_UPR_FD)) {
			if (isset(&(wlc->nbr_discovery_cap), WLC_6G_CAP_20TU_BCAST_PRB_RSP)) {
				wlc_ap_update_bcast_prb_rsp_template(wlc, TRUE);
			}

			if (isset(&(wlc->nbr_discovery_cap), WLC_6G_CAP_20TU_FILS_DISCOVERY)) {
				wlc_ap_update_fd_frame_template(wlc, TRUE);
			}
			break;
		}
...
wlc_bss_membership_get(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_AP(cfg) && wlc_akm_is_sae_h2e_enabled(cfg)) {
...
static void
wlc_init_tpe_regulatory(wlc_info_t *wlc)
{
	int i;

	wlc->reg_eirp.txpwr_20 = 24;
	wlc->reg_eirp.txpwr_40 = 30;
	wlc->reg_eirp.txpwr_80 = 36;
	wlc->reg_eirp.txpwr_160 = 42;
	for (i = 0; i < TPE_PSD_COUNT; i++) {
		wlc->reg_eirp_psd.txpwr_psd[i] = -2;
	}
}
