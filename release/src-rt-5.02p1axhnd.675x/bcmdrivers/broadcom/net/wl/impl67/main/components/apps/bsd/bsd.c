		...
		if (bsd_log_fd != NULL) {
			...
			*/
			...
			if (reason & (BSD_STEERING_RESULT_SUCC | BSD_STEERING_RESULT_FAIL)) {
				fprintf(bsd_log_fd, "%9lu %s steerexecImplCmnHandleAssocUpdateBTM Steering MAC="MACF" FROM_BAND=%d TO_BAND=%d REASON=%d %s\n",
					(unsigned long)(time(NULL)), date_str,
					ETHERP_TO_MACF(addr),
				...
				fprintf(bsd_log_fd, "%9lu %s steerexecImplCmnStartSteer RDKB_LOG_STEERED MAC="MACF" FROM_BAND=%d TO_BAND=%d REASON=%d steering_in_progress\n",
					(unsigned long)(time(NULL)), date_str,
					ETHERP_TO_MACF(addr),
					(bssinfo->intf_info->band == BSD_BAND_2G) ? 0 : 1,
					(to_bssinfo->intf_info->band == BSD_BAND_2G) ? 0 : 1,
