...
wlc_fifo_sta_mu_bitmap(wlc_fifo_info_t *fifo_info, struct scb *scb, void *fifo_bitmap)
{
	...
	for (ac = 0; ac < AC_COUNT; ac++)
	{
		ASSERT(fifo_scb->fifo_idx[ac] < WLC_HW_NFIFO_INUSE(fifo_info->wlc));
		if (wlc_fifo_scb_inflt_ac(fifo_info->wlc, scb, ac)) {
			setbit(fifo_bitmap, fifo_scb->fifo_idx[ac] - fifo_info->mu_fifo_start_idx);
		}
	}
}
...
wlc_fifo_sta_ulo_bitmap(wlc_fifo_info_t *fifo_info, struct scb *scb, uint16 *fifo_bitmap)
{
	...
	ASSERT(fifo_info->module_state == FIFO_STATE_MU);
	ASSERT(fifo_info->fifo_global_max > 0);
	block = (fifo_scb->fifo_idx[0] - fifo_info->mu_fifo_start_idx) / NUM_BITS_U16;
	base = block * NUM_BITS_U16;
	for (ac = 0; ac < AC_COUNT; ac++) {
		ASSERT(block == (fifo_scb->fifo_idx[ac]
			- fifo_info->mu_fifo_start_idx)/NUM_BITS_U16);
		*fifo_bitmap |= 1 << ((fifo_scb->fifo_idx[ac] - fifo_info->mu_fifo_start_idx)
				- base);
	}
	return block;
}
...
wlc_fifo_is_ulofdma(wlc_fifo_info_t *fifo_info, struct scb *scb)
{
	...
	int fifo_idx;
	fifo_scb_cubby_t *fifo_scb = FIFO_SCB_CUBBY(fifo_info, scb);
	fifo_idx = fifo_scb->fifo_idx[AC_BE] - fifo_info->mu_fifo_start_idx;
