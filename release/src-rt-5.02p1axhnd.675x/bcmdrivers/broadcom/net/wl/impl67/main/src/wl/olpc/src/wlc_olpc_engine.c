...
static void wlc_olpc_bsscfg_updn(void *ctx, bsscfg_up_down_event_data_t *evt)
{
	...
	wlc = olpc->wlc;
	/* if primary bsscfg just went up, then force calibrate on channel */
	if ((evt->up) && (evt->bsscfg == wlc->primary_bsscfg)) {
	...
	} else if (!(evt->up) && !wlc_olpc_find_other_up_on_chan(olpc->wlc, evt->bsscfg)) {
