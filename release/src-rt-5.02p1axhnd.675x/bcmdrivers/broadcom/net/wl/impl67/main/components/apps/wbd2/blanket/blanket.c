 ...
 *
 * $Id: blanket.c 791103 2020-09-16 11:11:08Z $
 *
 *  @author
 *  @bug No known bugs.
 */
...
static uint16 rc_map[] =
{
	...
	/* 6G, 20 */
	131, 60,
	(0x5002), (0x5001), (0x5005), (0x5009), (0x500d), (0x5011), (0x5015),
...
/* Fetches center channel for 80 and 160.
 * Fetches control channel for 40.
 * Fetches channel for 20 (control and center are the same here).
 * Trying to match channel representation in Annex E of IEEE 802.11 spec.
 */
unsigned char
blanket_chspec_to_channel(chanspec_t in_chspec)
{
	unsigned char channel = 0;

	if (CHSPEC_IS80(in_chspec) || CHSPEC_IS160(in_chspec)) {
		channel = CHSPEC_CHANNEL(in_chspec);
	} else {
		channel = wf_chspec_ctlchan(in_chspec);
	}
	...
	ieee1905_glist_t *list_excl)
{
	...
	if (rc <= REGCLASS_40MHZ_LAST) {
		uint bw = blanket_get_bw_from_rc(rc, &bw);
		blanket_add_chspec_to_list(list_excl, wf_channel2chspec(channel, bw,
			WL_CHANNEL_2G5G_BAND(channel)));
		...
		while (num_of_entries) {
			if (channel == blanket_chspec_to_channel(rc_chspec_map[idx])) {
	...
	ieee1905_radio_caps_type *in_radio_caps, ieee1905_glist_t *list_excl)
{
	...
	uint8 rcaps_rclist[REGCLASS_MAX_COUNT] = {0};
	uint8 *ptr = NULL;
	...
	for (iter_rcmap = 0; iter_rcmap < rc_max;) {
		...
		while (num_chspec > 0) {
			if (band & (ieee1905_get_band_from_channel(curr_opclass,
				blanket_chspec_to_channel(rc_map[iter_rcmap])))) {
			...
			*/
			...
			BKT_DEBUG("opclass[%d] Tot_chans[%d] supported[%s] "
				...
				wf_channel2chspec(opcls->chan_list[idx_chan], WL_CHANSPEC_BW_20,
					WL_CHANNEL_2G5G_BAND(opcls->chan_list[idx_chan])));
