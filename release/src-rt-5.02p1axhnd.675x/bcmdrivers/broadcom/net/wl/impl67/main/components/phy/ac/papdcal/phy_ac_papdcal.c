 ...
 *
 * $Id: phy_ac_papdcal.c 790261 2020-08-20 01:35:19Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
static int phy_ac_papdcal_get_ipaoff(phy_type_papdcal_ctx_t *ctx, int32* papdipaoff);
static int phy_ac_papdcal_set_ipaoff(phy_type_papdcal_ctx_t *ctx, int32 papdipaoff);
static int phy_ac_papdcal_get_cts2self(phy_type_papdcal_ctx_t *ctx, int32* papdcts2self);
static int phy_ac_papdcal_set_cts2self(phy_type_papdcal_ctx_t *ctx, int32 papdcts2self);
#endif // endif
	...
	phy_papdcal_info_t *cmn_info)
{
	...
	fns.get_ipaoff = phy_ac_papdcal_get_ipaoff;
	fns.set_ipaoff = phy_ac_papdcal_set_ipaoff;
	fns.get_cts2self = phy_ac_papdcal_get_cts2self;
	fns.set_cts2self = phy_ac_papdcal_set_cts2self;
#endif // endif
#if defined(WLTEST) || defined(DBG_PHY_IOV) || defined(WFD_PHY_LL_DEBUG)
#ifndef ATE_BUILD
...
static int
phy_ac_papdcal_get_ipaoff(phy_type_papdcal_ctx_t *ctx, int32* ipaoff)
{
	phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
	*ipaoff = (int32)info->papdipaoff;
	return BCME_OK;
}

static int
phy_ac_papdcal_set_ipaoff(phy_type_papdcal_ctx_t *ctx, int32 ipaoff)
{
	phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
	if (ipaoff < 0 || ipaoff > 4095) {
		return BCME_ERROR;
	} else {
		info->papdipaoff = ipaoff;
		return BCME_OK;
	}
}

static int
phy_ac_papdcal_get_cts2self(phy_type_papdcal_ctx_t *ctx, int32* cts2self)
{
	phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
	*cts2self = (int32)info->papdcts2self;
	return BCME_OK;
}

static int
phy_ac_papdcal_set_cts2self(phy_type_papdcal_ctx_t *ctx, int32 cts2self)
{
	phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
	if (cts2self < 0 || cts2self > 65535) {
		return BCME_ERROR;
	} else {
		info->papdcts2self = cts2self;
		return BCME_OK;
	}
}
#endif // endif
...
BCMATTACHFN(phy_ac_papdcal_nvram_attach_old)(phy_ac_papdcal_info_t *papdcal_info)
{
	...
	papdcal_info->papdipaoff = 0;
	papdcal_info->papdcts2self = 0;
...
phy_ac_papd_cal_mode0_1(phy_info_t *pi, acphy_papdCalParams_t *calParams,  uint8 *calmode)
{
	...
	if (TINY_RADIO(pi)) {
		...
		if (ACMAJORREV_1(pi->pubpi->phy_rev)) {
			...
			/* Continuous PAPD PAoff = 0smps, in BW20 1smp = 25ns */
			MOD_PHYREG(pi, PapdIpaOffCorr, papd_calIpaOffCorr, papdi->papdipaoff);
			MOD_PHYREGCEE(pi, PapdCalShifts, core, papdLambda_I, 0xa);
			MOD_PHYREGCEE(pi, PapdCalShifts, core, papdLambda_Q, 0xa);
			MOD_PHYREGCEE(pi, EpsilonTableAdjust, core, epsilonScalar, 0x8);
			phy_ac_papd_phy_core_setup_majorrev51_128_129_131(pi, core);
			if (papdi->papdcts2self != 0 && pi->sh->up) {
				wlc_phy_cts2self(pi, papdi->papdcts2self);
			}
		} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
			MOD_PHYREG(pi, PapdEpsilonUpdateIterations,
				epsilonUpdateIterations, num_iter);
