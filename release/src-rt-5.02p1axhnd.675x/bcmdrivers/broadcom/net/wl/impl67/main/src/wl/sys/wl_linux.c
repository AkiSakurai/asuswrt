 ...
 *
 * $Id: wl_linux.c 790740 2020-09-01 21:44:50Z $
 */
...
#include <bcm_intr.h>
	...
	|| (defined(BCMHWA) && defined(HWA_TXSTAT_BUILD))
...
wl_client_get_info(struct net_device *net, char *mac, int priority, wlan_client_info_t *info_p)
{
	...
	{
	struct scb * scb = wlc_scbfind_from_wlcif(wl->wlc, wlif->wlcif, mac);
...
#if defined(BCM_PKTFWD)
void
wl_set_dwds_client(struct wl_if *wlif, bool set)
{
	struct d3fwd_wlif *d3fwd_wlif;

	if ((wlif == NULL) || (wlif->d3fwd_wlif == NULL))
		return;

	d3fwd_wlif = wlif->d3fwd_wlif;

	if (set == TRUE) {
		BCM_REFERENCE(d3fwd_wlif);
#if defined(netdev_wlan_set_dwds_client)
		netdev_wlan_set_dwds_client(d3fwd_wlif);
#endif /* netdev_wlan_unset_dwds_client */
	} else {
		BCM_REFERENCE(d3fwd_wlif);
#if defined(netdev_wlan_unset_dwds_client)
		netdev_wlan_unset_dwds_client(d3fwd_wlif);
#endif /* netdev_wlan_unset_dwds_client */
	}
}
#endif /* BCM_PKTFWD */
...
wl_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (pdev->vendor == PCI_VENDOR_ID_BROADCOM && (
		...
		pdev->device == BCM43684_D11AX6G_ID ||
		pdev->device == BCM43684_D11AX2G6G_ID ||
		pdev->device == BCM43684_D11AX5G6G_ID ||
		pdev->device == BCM43684_D11AX2G5G6G_ID)) {
...
wl_free(wl_info_t *wl)
{
	...
	defined(STS_XFER_M2M_INTR) || defined(BCM_CSIMON))
