 ...
 *
 * $Id: wbd_slave_com_hdlr.c 791210 2020-09-21 05:08:57Z $
 */
...
wbd_slave_process_event_escan(i5_dm_interface_type *evt_ifr, char *ifname, bcm_event_t *pvt_data)
{
	...
	if (escan_event_status == WLC_E_STATUS_PARTIAL) {
		...
		blanket_get_global_rclass(CH20MHZ_CHSPEC(bi->ctl_ch), &bi_opClass);
	...
	/* Escan finished : Success / Aborted */
		...
		if (!scanstate_running && !send_indep_scan) {
			...
			WBD_DEBUG("Channel scan is not Triggered by SmartMesh or Independent Scan "
	...
	wl_chan_change_reason_t reason, i5_dm_interface_type *interface)
{
	...
	if ((reason != WL_CHAN_REASON_CSA) &&
		...
		FOREACH_20_SB(chspec, channel) {
			uint bitmap_sb = 0x00;
			blanket_get_chan_info(interface->ifname, channel, &bitmap_sb);
	...
	com_hndl = wbd_com_init(wbd_get_ginfo()->hdl,
		INVALID_SOCKET, sock_options, NULL, NULL, wbd_get_ginfo());
...
{
	int ret = WBDE_OK, fbt_generate_local = WBD_FBT_DEF_GEN_LOCAL;
	...
	if (rc_flags) {
		...
		if (peer_triggered && !fbt_generate_local) {
...
wbd_slave_process_fbt_config_resp_cmd(unsigned char *neighbor_al_mac,
	unsigned char *tlv_data, unsigned short tlv_data_len)
{
	int ret = WBDE_OK, peer_triggered = 1;
	...
	ieee1905_unassoc_sta_link_metric_query *query)
{
	...
	for (i = 0; i < query->chCount; i++) {
		...
		/* TODO:6GHZ: modify lines below to be compatible with 6GHZ */
		chspec = wf_channel2chspec(channel, bw, WL_CHANNEL_2G5G_BAND(channel));
		...
		for (count = 0; count < n_sta; count++) {
			 ...
			 * with matching band
			 */
			WBD_DS_FIND_I5_BSS_IN_DEVICE_FOR_BAND_AND_SSID(self_dev, band, i5_bss,
				ssid, &ret);
...
wbd_slave_send_operating_chan_report(i5_dm_interface_type *sifr)
{
	...
	int tx_pwr;
	int ret = WBDE_OK;
	i5_dm_chan_rc_map_type *g_chan_rc_map;
	uint32 g_chan_rc_map_count = 0;
	int i, j, idx = 0;
	WBD_ENTER();

	memset(&chan_rpt, 0, sizeof(chan_rpt));
	chan_rpt.list =	(operating_rpt_opclass_chan_list *)wbd_malloc(
			sizeof(operating_rpt_opclass_chan_list) * I5_MAX_CHANRCS, &ret);
	WBD_ASSERT();
	...
	if (opclass >= 131) {
		/* temporary workaround to complete 6G onboarding.
		 * TODO: make it inline with other bands
		 */
		chan_rpt.n_op_class = 1;
		chan_rpt.list[0].op_class = opclass;
		chan_rpt.list[0].chan = channel;
		goto send;
	}

	if (opclass == 84) {
		chan_rpt.list[1].op_class = 81;
		chan_rpt.list[0].op_class = 84;
		chan_rpt.list[1].chan = chan_rpt.list[0].chan = channel;
		chan_rpt.n_op_class = 2;
		goto send;
	}

	g_chan_rc_map = i5DmGetChannelRCMap(&g_chan_rc_map_count);
	for (i = 0; i < g_chan_rc_map_count; i++) {
		if (g_chan_rc_map[i].channel == channel) {
			break;
		}
	}
	if (i == g_chan_rc_map_count) {
		WBD_ERROR("channel [%d] not supported. "
			"Skip sending operating channel report", channel);
		goto end;
	}

	for (j = (g_chan_rc_map[i].count - 1); j >= 0; j--) {
		if (g_chan_rc_map[i].regclass[j] > opclass) {
			continue;
		}
		chan_rpt.list[idx].op_class = g_chan_rc_map[i].regclass[j];
		if (chan_rpt.list[idx].op_class > REGCLASS_40MHZ_LAST) {
			chan_rpt.list[idx].chan = CHSPEC_CHANNEL(sifr->chanspec);
		} else {
	...
	unsigned short tlv_data_len)
{
	...
	chanspec = wf_channel2chspec(zwdfs_msg.cntrl_chan, bw,
			WL_CHANNEL_2G5G_BAND(zwdfs_msg.cntrl_chan));
	...
	unsigned short tlv_data_len)
{
	...
	/* TODO:6GHZ: modify lines below to be compatible with 6GHZ */
	chanspec = wf_channel2chspec(msg->cntrl_chan, bw, WL_CHANNEL_2G5G_BAND(msg->cntrl_chan));
...
wbd_slave_process_cac_request(wbd_info_t *info, ieee1905_cac_rqst_list_t *pmsg)
{
	...
	for (i = 0; i < pmsg->count; i++) {
		...
		blanket_get_bw_from_rc(cac_rqst[i].opclass, &bw);
		if (cac_rqst[i].opclass <= REGCLASS_40MHZ_LAST) {
			chanspec = wf_channel2chspec(cac_rqst[i].chan, bw,
				WL_CHANNEL_2G5G_BAND(cac_rqst[i].chan));
		} else {
			/* 80MHZ and above use center channel and bandwidth */
			chanspec = CHBW_CHSPEC(bw, cac_rqst[i].chan);
		}
		...
		if (!wf_chspec_valid(chanspec) || !cac_method_valid || !cac_action_valid) {
			WBD_ERROR("Invalid CAC request parameter(s) for Interface ["MACF"]. "
				"%svalid chanspec[0x%x] (opclass[%d] channel[%d] bw[0x%x]), "
				"%svalid cac method (supported flag [0x%x] request [%d]), "
				"%svalid cac action [%d]\n", ETHERP_TO_MACF(cac_rqst[i].mac),
				wf_chspec_valid(chanspec) ? "" : "in", chanspec,
				cac_rqst[i].opclass, cac_rqst[i].chan, bw,
		...
		FOREACH_20_SB(chanspec, channel) {
			uint bitmap_sb = 0x00;
			ret = blanket_get_chan_info(pdmif->ifname, channel, &bitmap_sb);
			if (ret < 0) {
		...
		if (cac_method == MAP_CAC_METHOD_MIMO_DIMENSION_REDUCED_CAC) {
			...
			FOREACH_20_SB(pdmif->chanspec, channel) {
				uint bitmap_sb = 0x00;
				blanket_get_chan_info(pdmif->ifname, channel, &bitmap_sb);
				if (bitmap_sb & WL_CHAN_RADAR) {
					is_radar = 1;
					break;
				}
...
wbd_slave_process_cac_terminate_rqst(wbd_info_t *info, ieee1905_cac_termination_list_t *pmsg)
{
	...
	for (i = 0; i < pmsg->count; i++) {
		...
		blanket_get_bw_from_rc(cac_term[i].opclass, &bw);
		if (cac_term[i].opclass <= REGCLASS_40MHZ_LAST) {
			chanspec = wf_channel2chspec(cac_term[i].chan, bw,
					WL_CHANNEL_2G5G_BAND(cac_term[i].chan));
		} else {
			/* 80MHZ and above use center channel and bandwidth */
			chanspec = CHBW_CHSPEC(bw, cac_term[i].chan);
		}
		if (ifr_vndr_data->cac_rqst_chspec != chanspec) {
	...
	uint8 *pbuf, uint16 *payload_len, uint32 is_edcrs_eu, bool weather, uint8 method)
{
	...
	for (rclass_cnt = 0; rclass_cnt < REGCLASS_MAX_COUNT; rclass_cnt++) {
		...
		if (rc_chan_map[rclass_cnt].regclass < rc_first ||
			(rc_chan_map[rclass_cnt].regclass > rc_last && (pdmif->band == BAND_2G ||
			rc_chan_map[rclass_cnt].regclass <= REGCLASS_40MHZ_LAST))) {
			continue;
		}
		...
		for (chan_cnt = 0; chan_cnt < rc_chan_map[rclass_cnt].count; chan_cnt++) {
			...
			if (rc_chan_map[rclass_cnt].regclass > REGCLASS_40MHZ_LAST &&
			...
			blanket_get_bw_from_rc(rc_chan_map[rclass_cnt].regclass, &bw);
			if (rc_chan_map[rclass_cnt].regclass <= REGCLASS_40MHZ_LAST) {
				chanspec = wf_channel2chspec(chan, bw, WL_CHANNEL_2G5G_BAND(chan));
			} else {
				chanspec = CHBW_CHSPEC(bw, chan);
			}
...
wbd_slave_prepare_cac_complete_tlv_payload(uint8 **pbuf, uint32 *payload_len)
{
	...
	foreach_i5glist_item(sifr, i5_dm_interface_type, sdev->interface_list) {
		...
		if (cac_compl->opclass > REGCLASS_40MHZ_LAST) {
		...
		if (ifr_vndr_data->cac_compl_status == MAP_CAC_STATUS_RADAR_DETECTED) {
			...
			FOREACH_20_SB(ifr_vndr_data->cac_rqst_chspec, channel) {
				...
				blanket_get_chan_info(sifr->ifname, channel, &bitmap_sb);
				if (!(bitmap_sb & WL_CHAN_RADAR)) {
					continue;
				}
				blanket_get_global_rclass(CH20MHZ_CHSPEC(channel),
...
wbd_slave_prepare_cac_status_payload(uint8 **pbuf, uint16 *payload_len)
{
	...
	foreach_i5glist_item(sifr, i5_dm_interface_type, sdev->interface_list) {
		wbd_slave_valid_chan_info_list_t valid_chan_info[100];
		uint16 filled_len = 0;
	...
	wbd_slave_valid_chan_info_list_t *chan_info)
{
	...
	for (i = 0; i < g_rc_chan_map_count; i++) {
		...
		for (j = 0; j < g_rc_chan_map[i].count; j++) {
			...
			if (g_rc_chan_map[i].regclass <= REGCLASS_40MHZ_LAST) {
				chanspec = wf_channel2chspec(g_rc_chan_map[i].channel[j], bw,
					WL_CHANNEL_2G5G_BAND(g_rc_chan_map[i].channel[j]));
			} else {
				chanspec = CHBW_CHSPEC(bw, g_rc_chan_map[i].channel[j]);
			}
...
wbd_slave_prepare_ongoing_cac_chan_list(i5_dm_interface_type *pdmif, uint8 *pbuf)
{
	...
	if (is_edcrs_eu) {
		FOREACH_20_SB(sub->chanspec, sub_channel) {
			uint bitmap_sb = 0x00;
			blanket_get_chan_info(pdmif->ifname, sub_channel, &bitmap_sb);
	...
	if (chan_info->opclass > REGCLASS_40MHZ_LAST) {
