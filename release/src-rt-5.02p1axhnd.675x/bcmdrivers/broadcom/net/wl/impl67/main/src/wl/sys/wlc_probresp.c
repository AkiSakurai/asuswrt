 ...
 *
 * $Id: wlc_probresp.c 790619 2020-08-28 09:39:57Z $
 */
	...
	uint8 *body, int body_len)
{
	...
	if (BSS_OCE_ENAB(cfg->wlc, cfg) &&
		isset(&(cfg->wlc->nbr_discovery_cap), WLC_6G_CAP_PRB_RSP_CANCEL) &&
	...
	uint8 *plcp, struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	if (wlc->band->bandunit == BAND_6G_INDEX && wildcard_ssid == TRUE &&
	    ...
	    isset(&(wlc->nbr_discovery_cap), WLC_6G_CAP_NO_WILDCARD_PRBREQ)) {
