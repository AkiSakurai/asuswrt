 ...
 *
 * $Id: wbd_master_com_hdlr.c 791168 2020-09-18 06:14:13Z $
 */
...
/* Master process WNM request for Non preferred channel report */
static int wbd_master_process_wnm_mbo_np_chan_list(wbd_assoc_sta_item_t *sta, uint8* ibuf,
	uint8 ibuf_len, bool attr);
/* Read Non preferred channel report and create entry list used for tbss selection avoidance */
static int wbd_master_process_wnm_update_mbo_np_chan_list(wbd_assoc_sta_item_t* sta, uint8* ibuf,
	uint8 ibuf_len, bool attr, uint8 chan_list_len);
/* Get Non preferred subelement channel list length */
static int wbd_master_process_wnm_mbo_get_count_chan_from_non_pref_list(uint8* ibuf,
	uint8 ibuf_len, bool attr);
#endif /* MULTIAPR2 */
/* ------------------------------------ Static Declarations ------------------------------------ */
	...
	ieee1905_operating_chan_report *chan_report)
{
	...
	switch (agent_if_band) {
		case BAND_2G:
			chspec = wf_channel2chspec(prim_channel, highest_bw, WL_CHANSPEC_BAND_2G);
		break;
		case BAND_5GL:
		case BAND_5GH:
		case (BAND_5GL | BAND_5GH):
			chspec = wf_channel2chspec(prim_channel, highest_bw, WL_CHANSPEC_BAND_5G);
		break;
		case BAND_6G:
			chspec = wf_channel2chspec(prim_channel, highest_bw, WL_CHANSPEC_BAND_6G);
		break;
	}
	...
	wl_chan_change_reason_t reason, uint8 cntrl_chan, uint8 opclass)
{
	...
	if (opclass <= REGCLASS_40MHZ_LAST) {
...
#if defined(MULTIAPR2)
static int
wbd_master_process_wnm_mbo_get_count_chan_from_non_pref_list(uint8* ibuf, uint8 ibuf_len, bool attr)
{
	uint8 *ptr = ibuf;
...
static int
wbd_master_process_wnm_update_mbo_np_chan_list(wbd_assoc_sta_item_t *sta, uint8* ibuf,
	uint8 ibuf_len, bool attr, uint8 chan_list_len)
{
...
static int
wbd_master_process_wnm_mbo_np_chan_list(wbd_assoc_sta_item_t *sta, uint8* ibuf,
	uint8 ibuf_len, bool attr)
{
	...
	chan_list_len = wbd_master_process_wnm_mbo_get_count_chan_from_non_pref_list(ibuf,
		ibuf_len, attr);
	if (chan_list_len <= 0) {
	...
	ret = wbd_master_process_wnm_update_mbo_np_chan_list(sta, ibuf, ibuf_len,
		attr, chan_list_len);
end:
	WBD_EXIT();
	return ret;
	...
	uint8 *data, uint8 body_len)
{
	...
	while ((body_len - bytes_rd) > (WBD_MBO_WNM_NOTIFICATION_MIN_SUBELEMENT_LEN
		...
		if (ptr[oui_type_offset] == WBD_MBO_NP_SUBELEMENT_CHAN_OUI_TYPE) {
			...
			ret = wbd_master_process_wnm_mbo_np_chan_list(assoc_sta, ptr,
				ptr_len, FALSE);
			if (ret != WBDE_OK) {
				goto end;
			}
		...
		foreach_iglist_item(emt_p, ieee1905_chscan_result_item,
			...
			foreach_iglist_item(emt_nbr_p, ieee1905_chscan_result_nbr_item,
				...
				cur_chspec = wf_channel2chspec(emt_p->channel,
					...
					WL_CHANNEL_2G5G_BAND(emt_p->channel));
	...
	foreach(name, ifname_list, next) {
		...
		foreach_iglist_item(emt_p, ieee1905_chscan_result_item,
			...
			if (strcmp(emt_p->ifname, name) == 0) {
				chan_list[chan_count] = CH20MHZ_CHSPEC(emt_p->channel);
				chan_count++;
			}
		}
	...
	ieee1905_chscan_report_msg *chscan_rpt)
{
	...
	if (device_vndr->flags & WBD_DEV_FLAG_ONBOOT_CHSCAN_REQ_SENT) {
		...
		/* Processes Onboot Channel Scan Report by Controller */
		wbd_master_process_onboot_channel_scan_rpt(src_al_mac, ts_chscan_rpt, chscan_rpt);

	/* It is Requested/Indipendent ChScan Report sent by the device */
	} else {
	}
