 ...
 *
 * $Id: wlc_akm_ie.c 790889 2020-09-08 01:54:35Z $
 */
...
bool
wlc_akm_is_sae_h2e_enabled(wlc_bsscfg_t *cfg)
{
	rsnxe_ie_info_t *rsnxe_info = NULL;
	...
	rsnxe_info = RSNXE_INFO_GET(cfg);

	/* verifying with TLV data and data len passed from hostapd */
	if (!rsnxe_info || !rsnxe_info->rsnxe_len || !(rsnxe_info->rsnxe)) {
		return FALSE;
	}
	if (rsnxe_info->rsnxe_cap & DOT11_CAP_SAE_HASH_TO_ELEMENT) {
		WL_INFORM(("wl%d:%d: sae_h2e enabled\n", cfg->wlc->pub->unit,
			WLC_BSSCFG_IDX(cfg)));
		return TRUE;
	}
	return FALSE;
