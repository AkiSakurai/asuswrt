 ...
 *
 * $Id: phy_ac_chanmgr.c 790797 2020-09-03 06:36:23Z $
 */
...
static const uint8 avvmid_set129[7][5][8] = {
	{
	/* http://confluence.broadcom.com/pages/viewpage.action?pageId=431104865 */
		...
		{2, 2, 2, 0, 120, 120, 120, 0}},	  /* pdet_id = 5 MCM_SQ */
	{
		...
		{2, 2, 2, 0, 90, 90, 90, 0},
		{2, 2, 2, 0, 90, 90, 90, 0},
		{2, 2, 2, 0, 90, 90, 90, 0}},  /* pdet_id = 6, MCM6_S, MCM6_Q */
};
...
wlc_phy_set_regtbl_on_bw_change_acphy(phy_info_t *pi)
{
	...
	if (ACMAJORREV_GE47(pi->pubpi->phy_rev) && !ACMAJORREV_128(pi->pubpi->phy_rev)) {
		uint16 he_rx_tx, he_nav, nonhe_rx_tx, nonhe_nav;
		...
		if (!ACMAJORREV_129(pi->pubpi->phy_rev)) {
			if (CHSPEC_IS20(pi->radio_chanspec)) {
				...
				PHY_ERROR(("wl%d: %s: Unsupported BW!\n",
				        pi->sh->unit, __FUNCTION__));
				ASSERT(0);
				// To prevent 'may be used uninitialized' error
				he_rx_tx = 0;
				he_nav = 0;
				nonhe_rx_tx = 0;
				nonhe_nav = 0;
		...
		W_REG(pi->sh->osh, D11_IFS_EDCAPRI(pi), (uint16)0x14e4);
		W_REG(pi->sh->osh, D11_IFS_SIFS_RX_TX_TX(pi), he_rx_tx);
		W_REG(pi->sh->osh, D11_IFS_SIFS_NAV_TX(pi), he_nav);
		W_REG(pi->sh->osh, D11_IFS_EDCAPRI(pi), (uint16)0x4e4);
		W_REG(pi->sh->osh, D11_IFS_SIFS_RX_TX_TX(pi), nonhe_rx_tx);
		W_REG(pi->sh->osh, D11_IFS_SIFS_NAV_TX(pi), nonhe_nav);
	}
