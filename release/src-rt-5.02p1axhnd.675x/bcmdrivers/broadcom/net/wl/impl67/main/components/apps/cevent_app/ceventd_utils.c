 ...
 *
 *
 * $Id: ceventd_utils.c 788707 2020-07-09 08:48:15Z $
 */
...
{
	CA_DBG("Checking for d2c by ce subtype %u\n", ce->subtype);
	switch (ce->subtype)  {
		case CEVENT_D2C_ST_BTM_REQ:
			CA_MSG("ce subtype: %u\n", ce->subtype);
			break;
		case CEVENT_D2C_ST_BTM_RESP:
			if (reason < CEVENT_BTM_RESP_REASON_LAST) {
				CA_DBG("%s\n", ca_btm_resp_str[reason]);
				snprintf(pheader, hsz, "%s", ca_btm_resp_str[reason]);
				return BCME_OK;
			}
		case CEVENT_D2C_ST_BTM_QUERY:
			CA_MSG("ce subtype: %u\n", ce->subtype);
			break;
		default:
			CA_MSG("Default. ce subtype: %u\n", ce->subtype);
