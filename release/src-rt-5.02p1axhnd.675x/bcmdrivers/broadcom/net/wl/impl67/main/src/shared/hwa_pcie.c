...
hwa_rxpath_dma_reclaim(struct hwa_dev *dev)
{
	...
	for (core = 0; core < HWA_RX_CORES; core++) {
		...
		// NOTE: Rev132 should have an option to recycle D11B:rd to D11B:wr_dir
		...
		if (loop_count == HWA_FSM_IDLE_POLLLOOP) {
			HWA_ERROR(("%s recycle_done is not done <%u:%u>\n", __FUNCTION__,
				u32, recycle_done));
		} else {
			u32 = HWA_RD_REG_NAME(HWA1x, dev->regs, rx_core[core], recycle_status);
			recycle_wr_index = BCM_GBF(u32, HWA_RX_RECYCLE_STATUS_RECYCLE_WR_INDEX);
			HWA_ERROR(("%s recycle_done is done <%u:%u> recycle_wr_index<%u>\n",
		...
		// 9. Update recycle_wr_index, recycle_trans_id only for 6715Bx
		// 6715A0 has bug, need to use SW WAR.
		if (HWAREV_GE(dev->corerev, 132) || !dev->d11b_recycle_war) {
...
#define HWA_BPR_FIFO_REG(b, CORE, NAME) \
	...
	for (core = 0; core < HWA_RX_CORES; core++) {
		...
		HWA_PKTPGR_EXPR(HWA_BPR_REG(b, rx_core[core], debug_pagein_cnt));
	} // for core
	...
	uint16 rd_idx, uint32 transfer_count)
{
	...
	// XXX Do I need to take care of tx_bm.avail_sw ?

	if (txpost->pktpgr_trans_id[txpost->schedcmd_id] !=
		HWA_TXPOST_PKTPGR_TRANS_ID_INVALID) {
		HWA_PRINT("%s schedcmd_id<%u> process is not finish %d\n",
			__FUNCTION__, txpost->schedcmd_id,
			txpost->pktpgr_trans_id[txpost->schedcmd_id]);
		goto done;
	}
	...
	*/
	...
	for (pkt = 0; pkt < pkt_count - 1; ++pkt) {
		...
		if (txlbuf->context.control.data != (txlbuf->data_buffer +
			HWA_TXPOST_DATA_OFFSET_BYTES)) {
			HWA_ERROR(("%s(%d): INV-DATA\n", __FUNCTION__, __LINE__));
			HWA_PKT_DUMP_EXPR(_hwa_txpost_dump_pkt(txlbuf, NULL,
				"INV-DATA", 0, TRUE, TRUE));
		}

		txlbuf = (hwa_pp_lbuf_t*)PKTLINK(txlbuf);
		if (PKTNEXT(dev->osh, txlbuf) != NULL) {
			HWA_ERROR(("%s(%d): INV-NEXT\n", __FUNCTION__, __LINE__));
			HWA_PKT_DUMP_EXPR(_hwa_txpost_dump_pkt(txlbuf, NULL,
				"INV-NEXT", pkt, TRUE, TRUE));
			HWA_ASSERT(0);
		}
	}
	...
	if (txlbuf->context.control.data != (txlbuf->data_buffer +
			HWA_TXPOST_DATA_OFFSET_BYTES)) {
		HWA_ERROR(("%s(%d): TAIL-INV-DATA\n", __FUNCTION__, __LINE__));
		HWA_PKT_DUMP_EXPR(_hwa_txpost_dump_pkt(txlbuf, NULL,
			"TAIL-INV-DATA", 0, TRUE, TRUE));
	}
	...
	if (trans_id != txpost->pktchain_id) {
		...
		HWA_ASSERT(txpost->wi_count[txpost->pktchain_id] == 0);
		// Ensure that HWA3a has not skipped a schedcmd
		HWA_DEBUG_EXPR(HWA_ASSERT(trans_id == txpost->pktchain_id+1));
	...
	if (pkt_count == 1) {
		...
		if (flowid == HWA_TXPOST_FLOWID_NULL ||
			...
			HWA_ERROR((" %s: flowid_override <%u> is invalid update to <%u>\n",
