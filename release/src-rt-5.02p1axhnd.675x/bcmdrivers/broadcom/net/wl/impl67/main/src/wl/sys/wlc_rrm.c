 ...
 *
 * $Id: wlc_rrm.c 790319 2020-08-21 11:26:17Z $
 */
...
wlc_rrm_add_tbtt_info_for_nbr(uint8 *pbuf, wlc_bsscfg_t *bsscfg, nbr_rpt_elem_t *nbr_elt)
{
	...
	memcpy(pbuf, (uint8*)&tbtt_info, TBTT_INFO_FIELD_HDR_LEN);
	return BCME_OK;
}
	...
	uint8 *len_copied)
{
	...
	uint8 nbr_count, chan, rclass, nbytes_rd;
	int8 index;
	...
	while (nbr_rep) {
		...
		if (index == -1) {
			...
			ptraverse += rnr_info[index].count * TBTT_INFO_FIELD_HDR_LEN;
...
wlc_write_neighbor_ap_info_field(uint8 *pbuffer, wlc_rnr_info_t *nbr_rnr_info, uint8 *len_copied)
{
	...
	memcpy(pbuffer, nbr_rnr_info->pbuf, nbr_rnr_info->count * TBTT_INFO_FIELD_HDR_LEN);
	pbuffer += nbr_rnr_info->count * TBTT_INFO_FIELD_HDR_LEN;
	*len_copied = (nbr_rnr_info->count * TBTT_INFO_FIELD_HDR_LEN) +
