 ...
 *
 * $Id: dhd_msgbuf.c 789673 2020-08-04 20:37:18Z $
 */
...
dhd_prot_event_process(dhd_pub_t *dhd, void *msg)
{
	...
	if (buflen > DHD_RX_EVENT_BUFSZ) {
		DHD_ERROR(("%s: event size too big<%u>\n", __FUNCTION__, buflen));
	}
...
dhd_prot_ioctl(dhd_pub_t *dhd, int ifidx, wl_ioctl_t *ioc, void *buf, int len)
{
	...
	if (ret >= 0) {
		...
		if (ioc->cmd == WLC_GET_VAR && buf != NULL) {
			DHD_ERROR(("%s: status ret value is %d iov=%s\n",
				__FUNCTION__, ret, (char*)buf));
		} else {
			DHD_ERROR(("%s: status ret value is %d cmd=%d\n",
