		...
		int tid, taf_tech_type_t tech, uint32 units)
{
	...
	if (method->ordering == TAF_LIST_SCORE_MINIMUM && TAF_TECH_MASK_IS_MUMIMO(1 << tech)) {
		...
		if ((taf_info->mu_boost == TAF_MUBOOST_FACTOR ||
			...
			if (ias_cfg(method).mu_boost_compensate > 0) {
				boost = (boost * ias_cfg(method).mu_boost_compensate) >> 10;
			}

			if (boost < ias_cfg(method).mu_boost_limit) {
				boost = ias_cfg(method).mu_boost_limit;
			}
		...
		WL_TAFM4(method, MACF" boost: score orig %u, clients %u, group %u, boost %u, "
			...
			scb_stats->global.rdata.mu_clients_count),
			ias_cfg(method).mu_boost_limit, boost, score);
	}
	...
	struct bcmstrbuf* b)
{
	...
	if (!strcmp(cmd, "mu_boost_limit")) {
		uint32 boost_limit = ias_cfg(method).mu_boost_limit ?
			(1024 * 1024) / ias_cfg(method).mu_boost_limit : 1024;

		int err = wlc_taf_param(&local_cmd, &boost_limit, 1024, 32 * 1024, b);

		TAF_ASSERT(boost_limit > 0);
		result->misc = boost_limit;
		ias_cfg(method).mu_boost_limit = (1024 * 1024) / boost_limit;
		bcm_bprintf(b, "reciprocal boost limit %u\n", ias_cfg(method).mu_boost_limit);
		return err;
	}
	if (!strcmp(cmd, "mu_boost_compensate")) {
		int err = wlc_taf_param(&local_cmd, &ias_cfg(method).mu_boost_compensate,
			64, 4096, b);

		result->misc = ias_cfg(method).mu_boost_compensate;
		return err;
	}
#endif /* TAF_ENABLE_MU_BOOST */
	if (!strcmp(cmd, "opt")) {
