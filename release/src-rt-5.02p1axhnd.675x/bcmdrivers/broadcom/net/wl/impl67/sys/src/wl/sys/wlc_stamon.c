 ...
 *
 * $Id: wlc_stamon.c 789902 2020-08-11 10:58:23Z $
 */
...
wlc_stamon_offchan_timer(void *arg)
{
	...
	do {
		...
		if (ctxt->stacfg[i].chanspec != phy_utils_get_chanspec(WLC_PI(wlc)) &&
			...
			wlc_mac_request_entry(wlc, NULL, WLC_ACTION_SCAN, 0) == BCME_OK) {
...
void
wlc_stamon_get_reserved_amt_index(wlc_info_t *wlc, uint16 *amt_start_idx,
	uint16 *amt_max_idx)
{
	wlc_stamon_info_t *ctxt = NULL;
	...
	*amt_start_idx = ctxt->amt_start_idx;
	*amt_max_idx = ctxt->amt_max_idx;
}
