 ...
 *
 * $Id: wlc_rx.c 791290 2020-09-22 14:21:16Z $
 *
 */
#include <wlc_cfg.h>
#include <typedefs.h>
...
#ifdef WLAMPDU
static void
wlc_recv_bcn_ibss_config_txmod(wlc_info_t *wlc, struct scb *scb, ht_cap_ie_t *cap_ie,
	...
	int len, bool update_roam)
{
	 ...
	 */
	if (OCE_ENAB(wlc->pub) && (wlc->pub->now % 120 == 0)) {
		wlc_bss_info_t bi;
		...
		if (!ETHER_ISMULTI(&h->a1) && !(scb && (scb->state & PENDING_AUTH)) &&
			...
			if (!(/* Avoid sending deauth till AP has a txqueue */
				...
				(void)wlc_senddeauth(wlc, bsscfg, scb,
					&h->a2, bssid, cur_etheraddr, rc, FALSE);
...
#ifdef WLAMPDU
/**
 * In IBSS mode, there are no assoc messages. Still, the AMPDU transmit path has to be enabled if
 * the remote node supports that. This function is called when a previously unknown remote node
 * 'joins' our IBSS.
 */
static void
wlc_recv_bcn_ibss_config_txmod(wlc_info_t *wlc, struct scb *scb,
...
#ifdef WLAMPDU
				wlc_recv_bcn_ibss_config_txmod(wlc, scb, cap_ie, he_6g_caps_ie);
#endif /* WLAMPDU */
			}
		}
	}
