 ...
 *
 * $Id: wlc_mbss.c 790619 2020-08-28 09:39:57Z $
 */
...
typedef struct {
	...
	wlc_spt_t	*upr_fd_template; /* bcast Probe response or Fils discovery DMA template */
...
static const bcm_iovar_t mbss_iovars[] = {
	...
	{"ucode_mbssid", IOV_UCODE_MBSSID,
	IOVF_SET_DOWN, 0, IOVT_BOOL, 0}, /* enable/disable ucode changes for mbssid */
	{NULL, 0, 0, 0, 0, 0}
};
	...
	void *params, uint plen, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
#ifdef WL_MBSSID
	case IOV_GVAL(IOV_UCODE_MBSSID):
		*ret_uint_ptr = wlc->pub->_ucode_mbssid;
		break;

	case IOV_SVAL(IOV_UCODE_MBSSID):
		wlc->pub->_ucode_mbssid = bool_val;
		break;
#endif /* WL_MBSSID */
...
mbss_bsscfg_up(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
...
#ifdef UCODE_SUPPORT_UPR_FD_AVAILABLE
	/* Allocate DMA space for UPR/FD software template */
	result = wlc_spt_init(wlc, bmi->upr_fd_template, BCN_TEMPLATE_COUNT, bcn_tmpl_len);
	if (result != BCME_OK) {
		WL_ERROR(("wl%d.%d: %s: unable to allocate beacon templates",
			wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__));
		return result;
	...
	/* Set the BSSCFG index in the packet tag for beacons */
	for (idx = 0; idx < BCN_TEMPLATE_COUNT; idx++) {
		WLPKTTAGBSSCFGSET(bmi->upr_fd_template->pkts[idx], WLC_BSSCFG_IDX(cfg));
	}
#endif /* UCODE_SUPPORT_UPR_FD_AVAILABLE */
...
#ifdef UCODE_SUPPORT_UPR_FD_AVAILABLE
		wlc_spt_deinit(wlc, bmi->upr_fd_template, FALSE);
#endif /* UCODE_SUPPORT_UPR_FD_AVAILABLE */
...
#ifdef UCODE_SUPPORT_UPR_FD_AVAILABLE
/* Testing Pending ..., just putting framework in place for ucode support for
 * broadcast probe response or Fils discovery frame.
 *
 * TODO:
 * - Call from wlc_bss_update_probe_resp along with wlc_mbss_update_probe_resp.
 * - Move bcast_prb_rsp specific data upr_fd_template to wlc->upr_fd_info as only
 *   single cfg is sufficient.
 */
void
wlc_mbss_update_bcast_prb_rsp(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	uint16 *bcn;
	int idx;
	wlc_pkt_t pkt;
	...
	if ((idx = SPT_PAIR_AVAIL(bmi->upr_fd_template)) == -1) {
		ASSERT(!"Beacon template not available");
		return;
	}

	pkt = bmi->upr_fd_template->pkts[idx];
	ASSERT(pkt != NULL);
	...
	wlc_bcn_prb_template(wlc, FC_PROBE_RESP, bcn_rspec, cfg, bcn, &len);
	...
	bmi->upr_fd_template->latest_idx = idx;
	bmi->upr_fdtemplate->bcn_modified = TRUE;
}
#endif /* UCODE_SUPPORT_UPR_FD_AVAILABLE */
void
