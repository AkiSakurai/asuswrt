 ...
 *
 * $Id: wlc_ap.c 791272 2020-09-22 09:25:29Z $
 */
...
BCMATTACHFN(wlc_ap_attach)(wlc_info_t *wlc)
{
	...
	wlc->upr_fd_info->timer = wl_init_timer((struct wl_info*)wlc->wl,
		wlc_ap_upr_fd_timer_cb, wlc, "upr_fd_timer");

	if (wlc->upr_fd_info->timer == NULL) {
		WL_ERROR(("wl%d: %s: wl_init_timer for unsolicited bcast prb rsp failed\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
...
wlc_ap_wlc_down(void *ctx)
{
	...
	if (wlc->upr_fd_info) {
		...
		if (WLC_IS_UPR_FD_TIMER_RUNNING(wlc->upr_fd_info)) {
			UPR_FD_TX_DEL_TIMER(wlc);
			WLC_UPR_FD_STOP_TIMER(wlc->upr_fd_info);
		}
	}
		...
		if (SCB_IS_UNUSABLE(scb)) {
			if (SCB_AUTHENTICATED(scb)) {
				...
				(void)wlc_senddeauth(wlc, cfg, scb, &scb->ea, &cfg->BSSID,
					&cfg->cur_etheraddr, DOT11_RC_INACTIVITY, FALSE);
...
wlc_rnr_update_timer(void *ctx)
{
	...
	if (!wlc->rnr_update_period || !isset(&wlc->nbr_discovery_cap, WLC_6G_CAP_RNR_IE)) {
		WL_INFORM(("wl%d: Skip adding RNR IE as rnr_update_period[%d] OR"
...
wlc_rnr_element_calc_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (!BSSCFG_AP(cfg) || !cfg->rnr_nbr_ap_info_size ||
		!isset(&cfg->wlc->nbr_discovery_cap, WLC_6G_CAP_RNR_IE)) {
		return 0;
	}
...
wlc_ap_upr_fd_timer_cb(void *ctx)
{
	...
	wlc_bsscfg_t *bsscfg;
	uint32 now, pending, next_bcn_time;
	bool send_bcast_prb_rsp = FALSE;
	bool send_fd = FALSE;

	upr_fd_info = wlc->upr_fd_info;

	if (isset(&(wlc->nbr_discovery_cap), WLC_6G_CAP_20TU_BCAST_PRB_RSP)) {
		send_bcast_prb_rsp = TRUE;
	}

	if (isset(&(wlc->nbr_discovery_cap), WLC_6G_CAP_20TU_FILS_DISCOVERY)) {
		send_fd = TRUE;
	}
	...
	if ((!send_fd && !send_bcast_prb_rsp) || !upr_fd_info->period) {
		...
		UPR_FD_TX_DEL_TIMER(wlc);
		WLC_UPR_FD_STOP_TIMER(upr_fd_info);
		return;
	}
	...
	now = hnd_get_reftime_ms();
#elif defined(OSL_SYSUPTIME_SUPPORT)
	now = OSL_SYSUPTIME();
#endif /* BCMPCIEDEV && PROP_TXSTATUS */
	next_bcn_time = upr_fd_info->next_bcn_time;
	pending = next_bcn_time - now;

	if (now > next_bcn_time) {
		/* beacon is scheduled, skip probe response */
		UPR_FD_TX_DEL_TIMER(wlc);
		return;
	}
	if (pending < WL_PRB_RSP_CANCEL_GRACE_PERIOD) {
		WL_INFORM(("wl%d beacon is scheduled, skip probe response, between beacon"
			" interval transmitted [%d] BCAST probe response \n", wlc->pub->unit,
			upr_fd_info->n_pkts_xmit));
...
wlc_ap_start_upr_fd_timer(wlc_info_t *wlc)
{
	...
	/* start repeatative timer */
	UPR_FD_TX_ADD_TIMER(wlc);
	WLC_UPR_FD_START_TIMER(upr_fd_info);
	WL_INFORM(("wl%d: %d TU timer started\n", wlc->pub->unit, upr_fd_info->period));

#if defined(BCMPCIEDEV) && defined(PROP_TXSTATUS)
	upr_fd_info->next_bcn_time = hnd_get_reftime_ms() + apcfg->current_bss->beacon_period;
#elif defined(OSL_SYSUPTIME_SUPPORT)
	upr_fd_info->next_bcn_time = OSL_SYSUPTIME() + apcfg->current_bss->beacon_period;
#endif /* BCMPCIEDEV && PROP_TXSTATUS */
}
#endif /* BAND6G || (WL_OCE && WL_OCE_AP) */

#ifdef UCODE_SUPPORT_UPR_FD_AVAILABLE
void
wlc_ap_update_bcast_prb_rsp_template(wlc_info_t *wlc, bool suspend_mac)
{
	wlc_bsscfg_t *cfg = NULL;
	int idx;

	/* first up AP interface is sufficient to send for all interfaces in
	 * radio
	 */
	FOREACH_UP_AP(wlc, idx, cfg) {
		break;
	...
	if (!cfg) {
		WL_INFORM(("wl%d: No AP interface UP, exit bcast_prb_rsp\n"));
		return;
	}
...
#ifdef MBSS
	if (MBSS_BCN_ENAB(wlc, cfg)) {
		wlc_mbss_update_bcast_prb_rsp(wlc, cfg);
	} else
#endif /* MBSS */
	if (HWBCN_ENAB(cfg)) { /* Hardware beaconing for this config */
		...
		wlc_bcn_prb_template(wlc, FC_PROBE_RESP, wlc->bcn_rspec, cfg, bcn, &len);
...
void
wlc_ap_update_fd_frame_template(wlc_info_t *wlc, bool suspend_mac)
{
...
#endif /* UCODE_SUPPORT_UPR_FD_AVAILABLE */
	...
	chanspec_t chspec, uint8 *cp, int buflen)
{
	...
	wlc_txpwr_eirp_t      temp_eirp;
	wlc_txpwr_eirp_psd_t  temp_eirp_psd;

	memcpy(&temp_eirp, &wlc->reg_eirp, sizeof(wlc_txpwr_eirp_t));
	memcpy(&temp_eirp_psd, &wlc->reg_eirp_psd, sizeof(wlc_txpwr_eirp_psd_t));

	wlc_check_override_clm(wlc, &temp_eirp, &temp_eirp_psd);

	wlc_write_client_eirp(&cp, &temp_eirp, TRANS_PWR_REG_EIRP,
		TRANS_PWR_CAT_DEFAULT, chspec, buflen);
	wlc_write_client_eirp(&cp, &temp_eirp, TRANS_PWR_LOCAL_EIRP,
		TRANS_PWR_CAT_DEFAULT, chspec, buflen);
	if (regulatory_info == 0) {
		 wlc_write_client_eirp_psd(&cp, &temp_eirp_psd, TRANS_PWR_REG_EIRP_PSD,
		 TRANS_PWR_CAT_DEFAULT, chspec, buflen);
		 wlc_write_client_eirp_psd(&cp, &temp_eirp_psd, TRANS_PWR_REG_EIRP_PSD,
...
wlc_csw_calc_pwr_env_ie_len(void *ctx, wlc_iem_calc_data_t *calc)
{
	...
	if (!calc->cbparm->vht && !calc->cbparm->he)
		return BCME_OK;
...
wlc_csw_write_pwr_env_ie(void *ctx, wlc_iem_build_data_t *build)
{
	...
	chanspec_t chspec = ftcbparm->csw.chspec;
	if (!build->cbparm->vht && !build->cbparm->he)
		return BCME_OK;

	if (BAND_2G(wlc->band->bandtype))
		return BCME_OK;

	wlc_write_transmit_power_envelope_ie(wlc, chspec, build->buf, build->buf_len);
