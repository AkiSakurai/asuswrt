	...
	wl_phyextract_t  *phy_extract)
{
	...
	if (corerev < 128) {
		sts.antenna = (*D11PHYRXSTS_ACCESS_REF_D(&wrxh->rxhdr, corerev, PhyRxStatus_0)
	...
	if (corerev < 128) {
		sts.sq = ((*D11PHYRXSTS_ACCESS_REF_D(&wrxh->rxhdr, corerev, PhyRxStatus_1)
	...
	if (RSPEC_ISCCK(rspec)) {
		...
		if (D11REV_GE(corerev, 128)) {
			...
			sts.preamble = (*D11PHYRXSTS_ACCESS_REF_D(&wrxh->rxhdr,
				corerev, PhyRxStatus_0)
				& PRXS0_SHORTH) ?
