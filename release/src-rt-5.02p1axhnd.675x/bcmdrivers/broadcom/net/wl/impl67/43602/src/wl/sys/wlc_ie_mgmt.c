	...
	wlc_iem_calc_fn_t calc_fn, wlc_iem_build_fn_t build_fn, void *ctx)
{
	...
	/* Note: on dongle, _wlc_iem_add_cbe and some tables
	 * may not be available after wl init sequence...
	 */
	if (iem->init)
		return BCME_UNSUPPORTED;
...
{
	return wlc_iem_calc_ie_len_ex(iem, cfg, ft, tag, NULL, cbparm);
}

uint
wlc_iem_calc_ie_len_ex(wlc_iem_info_t *iem, wlc_bsscfg_t *cfg, uint16 ft,
...
{
	return wlc_iem_build_ie_ex(iem, cfg, ft, tag, NULL, cbparm, buf, buf_len);

}

int
wlc_iem_build_ie_ex(wlc_iem_info_t *iem, wlc_bsscfg_t *cfg, uint16 ft,
