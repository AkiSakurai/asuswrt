...
BCMATTACHFN(wlc_stats_attach)(wlc_info_t *wlc)
{
	...
	if (!(stats_info = (wlc_stats_info_t *)MALLOC(wlc->osh, sizeof(wlc_stats_info_t)))) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return NULL;
	}
	bzero((char *)stats_info, sizeof(wlc_stats_info_t));
	stats_info->wlc = wlc;
	...
	stats_info->bcn_len_hist = (wlc_bcn_len_hist_t *)MALLOC(wlc->osh,
