 ...
 *
 * $Id: wlc_key.c 781623 2019-11-26 09:22:14Z $
 */
...
#ifdef BCMWAPI_WPI
static uint8 wlc_key_get_algo(wlc_info_t *wlc);
#endif /* BCMWAPI_WPI */
...
wlc_key_allocate_ex(wlc_info_t *wlc, int idx)
{
	...
	if ((key = MALLOCZ(wlc->osh, sizeof(wsec_key_t))) != NULL) {
		key->idx = (uint8)idx;
#ifdef WLMCNX
		key->rcmta = (uint8)wlc->pub->max_addrma_idx;
#endif // endif
#if defined(WLAMPDU_HOSTREORDER) || defined(BRCMAPIVTW)
		if ((key->iv_tw = (iv_tw_t*)MALLOCZ(wlc->pub->osh,
			sizeof(iv_tw_t) * wlc->pub->tunables->num_rxivs)) == NULL) {
...
#ifdef ACKSUPR_MAC_FILTER
static int
wlc_key_allocate_acksupr(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int start,
	struct ether_addr *key_ea)
{
	int amt_idx = -1;
	int wsec_idx = BCME_NOTFOUND;
	uint16 attr;
	...
	if ((amt_idx = wlc_macfltr_find_and_add_addrmatch(wlc, cfg,
		key_ea, attr)) == BCME_NOTFOUND)
		return BCME_NOTFOUND;

	wsec_idx = start + amt_idx;
	if (wlc->wsec_keys[wsec_idx] == NULL &&
		wlc_key_allocate_ex(wlc, wsec_idx) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
		          wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		return BCME_NOMEM;
	}
...
wlc_key_move(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wsec_key_t *src, wsec_key_t *dst)
{
	...
	if (src->len > 0) {
		...
		/* remove rxiv ptr from src or it will get freed in wlc_key_delete */
		src->rxiv = NULL;
...
/* XXX mSTA: Use RCMTA for BSSID match, use the index of the BSSID entry + 4
 * as the index into SECINDXALGO for key index, use the key index in the
 * SECINDXALGO entry for key location in shared memory. 3 contiguous key
 * slots are preserved in the shared memory for each STA (pairwise/ucast
 * key first and group keys follow). See twiki 'UcodeMultiSTA' for details.
 * To share the RCMTA for BSSID match and for real key use the key management
 * infrastructure. It is not optimal since it will waste some slots in the
 * SECINDXALGO area and in the shared memoey if the bsscfg doesn't enable
 * the security but it is simple (also the P2P or BTAMP will enable security
 * by default anyway so it is not a real concern!).
 */

/* XXX P2P: Use higher index entries in RCMTA for BSSID/TA (used by the
 * associated AP) and RA matches (used as MAC), and use the lower index
 * entries for TA (used by the associated STAs) match.
 * There is a new M_ADDR_BMP_BLK block in shared memory to describe RCMTA
 * entries in higher indices, see twiki 'Multi-MAC' for details.
 */
...
{
	uint grpkeys = (cfg != wlc->cfg && BSSCFG_STA(cfg) && !BSSCFG_AWDL(wlc, cfg) &&
...
wlc_get_new_key(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint32 key_flags, struct ether_addr *key_ea)
{
	...
	if ((BSSCFG_AP(cfg) && WLC_ACKSUPR(wlc))) {
		if ((wsec_idx = wlc_key_allocate_acksupr(wlc, cfg, start, key_ea)) >= start) {
			WL_WSEC(("wl%d.%d: %s: wlc_key_allocate_acksupr alloc wsec_idx %d\n",
			          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__, wsec_idx));
			goto key_alloc_done;
		}
		/*
		* if not found, means AMT table is full, just override
		* an exited one that is not really associated
		*/
		if (wsec_idx != BCME_NOTFOUND)
			return wsec_idx;
	}
#endif /* ACKSUPR_MAC_FILTER */
#ifdef PSTA
	...
	WL_WSEC(("wl%d.%d: allocated %d keys starting at slot %d for %s\n",
	         wlc->pub->unit, WLC_BSSCFG_IDX(cfg), 1 + grpkeys, wsec_idx,
	         key_ea?bcm_ether_ntoa(key_ea, eabuf):"NULL"));
#ifdef ACKSUPR_MAC_FILTER
key_alloc_done:
#endif /* ACKSUPR_MAC_FILTER */
...
/* update the pairwise key address for a hardware key. Takes into account
 * the difference between wsec & h/w indices and always adds VALID and
 * A2 attributes to the input AMT attributes.
 */
static void
wlc_key_write_addrmatch(wlc_info_t *wlc, int wsec_idx,
	const struct ether_addr *ea, uint16 attr)
{
	WL_WSEC(("wl%d: %s key %d\n", wlc->pub->unit, __FUNCTION__, wsec_idx));

	ASSERT(wsec_idx >= WSEC_MAX_DEFAULT_KEYS);
	ASSERT((uint)wsec_idx < (wlc->pub->max_addrma_idx + WSEC_MAX_DEFAULT_KEYS));
	ASSERT(!attr || (attr & AMT_ATTR_VALID));

	attr |= AMT_ATTR_VALID | AMT_ATTR_A2;
	wlc_set_addrmatch(wlc, (wsec_idx - WSEC_MAX_DEFAULT_KEYS), ea, attr);
}

/* XXX when key->len > 0 (which means we have security key) use wlc_key_update()
 * or wlc_key_delete() to add/remove key and RCMTA entry; otherwise use
 * wlc_key_write_addrmatch() to update the RCMTA/AMT entry (WSEC_KEY()
 * returns NULL when key->len == 0 which causes
 * wlc_key_update()/wlc_key_delete() to return immediately)
 */

int
wlc_key_add_bssid(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
...
hw_upd:
	...
	/* used by BSSID match only */
		...
		wlc_key_write_addrmatch(wlc, wsec_idx, &key->ea, 0);
...
void
wlc_key_del_bssid(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wsec_key_t *key;
	uint i;
	wlc_bsscfg_t *bc;
...
#ifdef WLAWDL
	ASSERT(cfg != wlc->cfg && BSSCFG_STA(cfg) && (cfg->BSS || BSSCFG_AWDL(wlc, cfg)));
#else
	ASSERT(cfg != wlc->cfg && BSSCFG_STA(cfg) && cfg->BSS);
#endif // endif
	...
	wsec_iv_t *initial_iv, wsec_key_t **pkey_ptr)
{
	...
	if (ETHER_ISNULLADDR(key_ea)) {
		...
		if (bsscfg == wlc->cfg) {
			...
			/* XXX Send key 0 and 1 to slot 0 and key 2 and 3 to slot 1.
			 * This is to "compress" 4 group keys into 2 slots given
			 * that WFA requires to use key 1 and 2. When using static
			 * WEP key it is unfortunate that key 0 and 1, 2 and 3 can't
			 * be plumbed at the same time.
			 */
			...
			if (bsscfg->bss_def_keys[def_idx] != NULL) {
				...
				/* Get a new key. */
				if ((wsec_idx = wlc_get_new_key(wlc,
					bsscfg, key_flags, key_ea)) < 0)
					return wsec_idx;
	...
	/* set a per station key */
		...
		if (bsscfg->associated) {
			...
			if (!(scb = wlc_scbfindband(wlc, bsscfg, key_ea, assoc_bandunit))) {
				if (!(scb =
				      wlc_scblookupband(wlc, bsscfg, key_ea, assoc_bandunit))) {
		...
		if (scb->key) {
			...
			new_key = TRUE;
#endif /* PSTA */
		/* XXX mSTA: hand the key struct used by BSSID match over to scb
...
#endif // endif
					rcmta < (int)wlc->pub->max_addrma_idx)
	...
	/* update the key */
	if (key->rxiv)
		MFREE(wlc->pub->osh, key->rxiv,
		      sizeof(wsec_iv_t) * wlc->pub->tunables->num_rxivs);
	/* The following line can't use WSEC_KEY macro, since the len might be 0. */
	{
		...
		key->iv_tw = iv_tw;
		if (key->iv_tw)
			bzero(key->iv_tw, sizeof(iv_tw_t) * wlc->pub->tunables->num_rxivs);
#endif /* BRCMAPIVTW */
	}
	...
	if (IBSS_PEER_GROUP_KEY_ENAB(wlc->pub)) {
		 ...
		 * if not already done
		 */
		if ((bsscfg->brcm_ap_iv_tw) && (new_key) &&
		    BSSCFG_STA(bsscfg) && (key->iv_tw == NULL)) {
			if ((key->iv_tw = (iv_tw_t*)MALLOC(wlc->pub->osh,
			              sizeof(iv_tw_t) * wlc->pub->tunables->num_rxivs)) != NULL)
				bzero(key->iv_tw, sizeof(iv_tw_t) * wlc->pub->tunables->num_rxivs);
		}
#endif /* BRCMAPIVTW */
	...
	case WEP1_KEY_SIZE:
#ifdef BCMCCX
		if (key_flags & (WL_CKIP_KP | WL_CKIP_MMH)) {
			WL_WSEC(("wl%d.%d: wlc_key_insert: CKIP/MMH\n",
			         wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		} else if (key_flags & WL_CKIP_KP) {
			WL_WSEC(("wl%d.%d: wlc_key_insert: CKIP\n",
			         wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		} else if (key_flags & WL_CKIP_MMH) {
			WL_WSEC(("wl%d.%d: wlc_key_insert: WEP/MMH\n",
			         wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		} else
#endif /* BCMCCX */
...
#ifdef BCMCCX
		/* CKIP Key Expansion, from 5 bytes to 16 bytes, just in case... */
		WL_WSEC(("wl%d.%d: wlc_key_insert: expand 40-bit WEP key to 16-byte CKIP key\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		bcopy((char*)key->data, ((char*)key->data) + WEP1_KEY_SIZE, WEP1_KEY_SIZE);
		bcopy((char*)key->data, ((char*)key->data) + 2*WEP1_KEY_SIZE, WEP1_KEY_SIZE);
		key->data[3*WEP1_KEY_SIZE] = key->data[0];
#endif /* BCMCCX */
		break;
	case WEP128_KEY_SIZE:
		...
		key->icv_len = DOT11_ICV_LEN;
#ifdef BCMCCX
		/* CKIP Key Expansion, from 13 bytes to 16 bytes, just in case... */
		WL_WSEC(("wl%d.%d: wlc_key_insert: expand 128-bit WEP key to 16-byte CKIP key\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		key->data[WEP128_KEY_SIZE] = key->data[0];
		key->data[WEP128_KEY_SIZE + 1] = key->data[1];
		key->data[WEP128_KEY_SIZE + 2] = key->data[2];
#endif /* BCMCCX */
		break;
	case TKIP_KEY_SIZE:
#ifdef BCMWAPI_WPI
		...
		if (key_algo == CRYPTO_ALGO_SMS4) {
			...
			key->algo_hw = wlc_key_get_algo(wlc);
...
#ifdef GTK_RESET
#ifdef STA
			/* XXX PR80077 WAR for a buggy AP where the initial IV in
			 * the rekey message don't match to what AP replay counter
			 * used in the MC/BC packet
			 */
	...
	case AES_KEY_SIZE:
		...
		case CRYPTO_ALGO_CKIP:
			if (key_flags & WL_CKIP_MMH)
				WL_WSEC(("wl%d.%d: wlc_key_insert: CKIP/MMH\n",
				         wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
			else
				WL_WSEC(("wl%d.%d: wlc_key_insert: CKIP\n",
				         wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
			if (bsscfg->WPA_auth != WPA_AUTH_CCKM) {
				WL_ERROR(("wl%d.%d: %s: non-CCKM CKIP not supported\n",
					wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
				return BCME_RANGE;
			}
			key->algo = CRYPTO_ALGO_WEP128;
...
#ifdef STA
			/* XXX PR80077 WAR for a buggy AP where the initial IV in
			 * the rekey message doesn't match the AP replay counter
			 * used in the MC/BC packet
			 */
			if (!scb && BSSCFG_STA(bsscfg) && bsscfg->wsec_portopen) {
				WL_WSEC(("wl%d.%d: wlc_key_insert: gtk plumbed,"
	...
	if (ETHER_ISNULLADDR(key_ea)) {
		if (key_flags & WL_PRIMARY_KEY) {
			...
			if ((WLC_MFP_ENAB(wlc->pub)) && WLWNM_ENAB(wlc->pub))
				wlc_wnm_sleep_key_update(wlc, bsscfg);
#endif // endif
...

	{
		...
		if ((ret = wlc_key_iv_init(wlc, bsscfg, key, initial_iv)) != BCME_OK) {
...
#ifdef STA
	/* XXX mSTA: update the unicast key again in order to reflect
	 * the bcmc algo field in the unicast key's SECINDXALGO entry.
	 */
	if (bsscfg != wlc->cfg && !BSSCFG_PSTA(bsscfg) && BSSCFG_STA(bsscfg) && bsscfg->BSS &&
		...
		if (MCNX_ENAB(wlc->pub)) {
			if ((scb = wlc_scbfindband(wlc, bsscfg, &bsscfg->BSSID,
				CHSPEC_WLCBANDUNIT(bsscfg->current_bss->chanspec))) != NULL &&
				scb->key != NULL)
	...
	if (BSSCFG_STA(bsscfg) &&
		...
		if ((key_algo & SEC_ALGO_KEY_EXCH) && (wlc->cfg == bsscfg)) {
			...
			if (WLWNM_ENAB(wlc->pub))
				wlc_wnm_check_dms_req(wlc, &bsscfg->current_bss->BSSID);
#endif /* WLWNM */
		}
...
#endif // endif
		/* XXX mSTA: clear the BSSID match to prevent a stale pointer to
		 * the key struct
		 */
		if (key == bsscfg->rcmta) {
...
wlc_key_reset(wlc_info_t *wlc, wsec_key_t *key)
{
	...
	if (key->rxiv)
		MFREE(wlc->pub->osh, key->rxiv, sizeof(wsec_iv_t) * wlc->pub->tunables->num_rxivs);
	key->rxiv = NULL;
...
wlc_key_scb_delete(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (BSSCFG_AIBSS(wlc, bsscfg)) {
	/* for aibss, don't delete bsscfg->bss_def_keys[0], null scb's ref to it */
		WL_WSEC(("wl%d: %s: bsscfg is AIBSS, don't delete default key\n",
		wlc->pub->unit, __FUNCTION__));
		scb->key = NULL;
#if defined(IBSS_PEER_GROUP_KEY)
		scb->ibss_grp_keys[0] = NULL;
#endif // endif
		return;
	}
...
wlc_key_set_error(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int kflag, wl_wsec_key_t *key_param)
{
	...
	/* If B/MCAST ether addr, use default key; or, look up by ether addr
	 * if one was furnished; otherwise, if key index is in range use that.
	 */
	if (ETHER_ISMULTI(&key_param->ea)) {
		wlc_bsscfg_t *bsscfg = wlc_bsscfg_primary(wlc);
		key = WSEC_BSS_DEFAULT_KEY(bsscfg);

	} else if (!ETHER_ISNULLADDR(&key_param->ea)) {
		if ((scb = wlc_scbfind(wlc, cfg, &key_param->ea)) == NULL)
			return BCME_NOTFOUND;
		key = scb->key;

	} else if (key_param->index < (uint32)WLC_MAX_WSEC_KEYS(wlc)) {
		key = WSEC_KEY(wlc, key_param->index);
	}
...
#endif /* defined(BCMDBG) || defined(MFP_TEST) */
...
wlc_key_iv_init(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wsec_key_t *key, wsec_iv_t *initial_iv)
{
	...
	case CRYPTO_ALGO_WEP128:
		...
		if (wlc->pub->up) {
			...
			r1 = (uint16)(wlc->pub->now);
			r2 = (uint16)(wlc->pub->now >> 16);
		}
		...
		key->mrxseq_base = 0;
		key->mrxseq_bitmap = 0;
		WL_WSEC(("wl%d: wlc_key_iv_init: reset CKIP rxSEQ, bitmap to 0\n", wlc->pub->unit));
#endif /* BCMCCX */
		break;
	...
	case CRYPTO_ALGO_TKIP:
		...
		bzero(key->rxiv, sizeof(wsec_iv_t) * wlc->pub->tunables->num_rxivs);
		break;
#else
		if (initial_iv)
	...
	case CRYPTO_ALGO_AES_CCM:
		...
		bzero(key->rxiv, sizeof(wsec_iv_t) * wlc->pub->tunables->num_rxivs);
		for (i = 0; i < wlc->pub->tunables->num_rxivs; i ++) {
			if (initial_iv != NULL) {
...
void
wlc_key_iv_update(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
	wsec_key_t *key, uchar *buf, bool update, bool uc_seq)
{
	int indx;
	uint32 iv_tmp = 0;
	wsec_iv_t *txiv;
	...
	case CRYPTO_ALGO_AES_CCM:
		...
		/* check that iv does not rollover - if so, disable the bsscfg. This is needed
		 * to implement good security practices and NIST certification of AES. In the
		 * uncommon rollover case, corrupt the key and transmit. Note that this would affect
		 * only the traffic using this key and not tx to other peers (e.g. on AP)
		 */
		if (!txiv->lo && !txiv->hi) {
			/* corrupt the key and reinit h/w key state */
			wlc_getrand(wlc, key->data, sizeof(key->data));
			wlc_key_hw_init(wlc, WSEC_KEY_INDEX(wlc, key), bsscfg);
		}
...
#endif // endif
		if (WL_WSEC_DUMP_ON())
			prhex("  wlc_key_iv_update: updated AES iv", buf, DOT11_IV_TKIP_LEN);
...
#ifdef BCMCCX
			/* just update iv */
			if (WSEC_CKIP_KP_ENABLED(bsscfg->wsec)) {
				txiv->hi++;
			}
			else
#endif /* BCMCCX */
...
void
wlc_dwds_hw_tid_war(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, wsec_key_t *key,
	struct dot11_header *h, uint8 prio)
{
	...
	qos = (FC_TYPE(fc) == FC_TYPE_DATA && FC_SUBTYPE_ANY_QOS(FC_SUBTYPE(fc)));

	if (a4 && qos) {
		iv_data = (uchar *)h + DOT11_A3_HDR_LEN + ETHER_ADDR_LEN + DOT11_QOS_LEN;

		indx = key->id;

		old_lo = txiv->lo;
		/* take care of the fact that we already incremented
		 * iv once before so just add 15.
		 */
		txiv->lo = old_lo + 15;
		if (txiv->lo < old_lo)
			txiv->hi++;
		...
		/* Add in control flags and properly format */
		iv_data[0] = txiv->lo & 0xff;
		/* if doing the dwds workaround, make sure the lower nibble is equal to the
		 * proper tid,
		 */
		/* mask off any potential stray bits in the nibble that may have
		 * been placed there by shared tsc between STA and DWDS vap.
		 */
		iv_data[0] &= 0xF0;
		/* OR the tid into the right place of the IV */
		iv_data[0] |= (prio & QOS_TID_MASK);

		iv_data[1] = (txiv->lo >> 8) & 0xff;
		iv_data[2] = 0;
		iv_data[3] = (uint8)(((indx & 3) << DOT11_KEY_INDEX_SHIFT)
			| WLC_AES_EXTENDED_PACKET);
		iv_data[4] = txiv->hi & 0xff;
		iv_data[5] = (txiv->hi >>  8) & 0xff;
		iv_data[6] = (txiv->hi >> 16) & 0xff;
		iv_data[7] = (txiv->hi >> 24) & 0xff;

		if (WL_WSEC_DUMP_ON()) {
			prhex("  wlc_key_iv_update: updated AES iv", iv_data, DOT11_IV_TKIP_LEN);
		}
	}
...
wlc_key_hw_init_all(wlc_info_t *wlc)
{
	...
	for (i = 0; i < hw_keys; i++) {
		...
		/* PR100597: Need to take care of APSTA mode, in which offsets of TX/RX
		 * TKIP MIC keys of STA are different from AP's. The primary bsscfg only
		 * applies to STA(eth1) in APSTA mode. Hw keys should be set
		 * with corresponding bsscfgs to avoid the TKIP countermeasures.
		 */
...
wlc_key_hw_clear_all(wlc_info_t *wlc)
{
	...
	for (i = 0; i < (int)wlc->pub->wsec_max_rcmta_keys - WSEC_MAX_DEFAULT_KEYS; i++)
		wlc_clear_addrmatch(wlc, i);

	/* update the rcmta_size reg with the max RCMTA_SIZE */
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		W_REG(wlc->osh, &wlc->regs->u_rcv.d11regs.rcmta_size, RCMTA_SIZE);
	}
...
#ifdef WLOFFLD
	if (WLOFFLD_CAP(wlc)) {
		wlc_ol_groupkey_update(wlc->ol, bsscfg);
	}
#endif /* WLOFFLD */
}
...
wlc_key_hw_wowl_init(wlc_info_t *wlc, wsec_key_t *key, int rcmta_index, wlc_bsscfg_t *bsscfg)
{
	...
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		...
		wowl_seckindx_algo_blk = D11AC_M_WOWL_SECKINDXALGO_BLK;
		wowl_tkip_tsc_ttak_blk = D11AC_M_WOWL_TKIP_TSC_TTAK;
		wowl_tscpn_blk = D11AC_M_WOWL_TSCPN_BLK;
	}
	...
	if (rcmta_index >= WSEC_MAX_DEFAULT_KEYS)
		amt_attr = wlc_clear_addrmatch(wlc, rcmta_index - WSEC_MAX_DEFAULT_KEYS);
	...
	if (key->algo == CRYPTO_ALGO_TKIP) {
		...
		for (j = 0; j < TKHASH_P1_KEY_SIZE; j += 2) {
			...
			wlc_write_shm(wlc, wowl_tkip_tsc_ttak_blk + offset + j, v);
		}
	...
	if (rcmta_index >= WSEC_MAX_DEFAULT_KEYS)
		wlc_key_write_addrmatch(wlc, rcmta_index, &key->ea, amt_attr);
}
                       ...
                       bool restore_tsc_pn)
{
	...
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		...
		wowl_tscpn_blk = D11AC_M_WOWL_TSCPN_BLK;
		wowl_offload_ctx_blk = D11AC_M_WOWL_OFFLOAD_CTX;
	}
...
wlc_key_hw_init(wlc_info_t *wlc, int indx, wlc_bsscfg_t *bsscfg)
{
	    ...
	    rcmta_index < wlc->pub->max_addrma_idx + WSEC_MAX_DEFAULT_KEYS)
	{
	    ...
	    if (WLC_ACKSUPR(wlc)) {
			...
			wlc_get_addrmatch(wlc, rcmta_index - WSEC_MAX_DEFAULT_KEYS, &ea, &attr);
			FOREACH_AP(wlc, idx, tmp_cfg) {
				if (BSSCFG_ACKSUPR(tmp_cfg) && (wlc_macfltr_addr_match(wlc->macfltr,
					tmp_cfg, &ea) != WLC_MACFLTR_ADDR_ALLOW)) {
					amt_attr = wlc_clear_addrmatch(wlc,
...
#endif /* defined(IBSS_PEER_GROUP_KEY) */
	{
		...
		/* XXX mSTA: updating keys for non-primary STA requires special attention.
		 * unicast key: update the bcmc key algo field in the unicast key's
		 * SECINDXALGO entry, skip TKIP whose bcmc frame decryption is done in s/w
		 */
		if (!PSTA_ENAB(wlc->pub) &&
			...
			/* XXX bsscfg->wsec_index is invalid in case of STA hence
			 * we have to look for bcmc key algo by other means...
			 * finding the first valid key and using the algo of it should
			 * be fine as long as all bcmc keys are using the same algo...
			 * maybe is bad assumption but I'd think it is ok for non
			 * primary bsscfgs which are created, used, and deleted and
			 * everytime a group key is plumbed we should have a fresh
			 * copy of bsscfg and there's no any keys leftover...
			 */
			for (i = 0; i < ARRAYSIZE(bsscfg->bss_def_keys); i ++) {
	...
	if (key && key->algo_hw != WSEC_ALGO_OFF) {
		...
		/* Program a random value if no key instead of zeroing out key data.
		* It is to prevent packets from being sent out and encrypted with
		* all-zero key after key deletion.
		*/
		char line[2*sizeof(wlc->rand_keydata) + 1];
		wlc_getrand(wlc, wlc->rand_keydata, sizeof(wlc->rand_keydata));

		BCM_REFERENCE(line);
		bcm_format_hex(line, wlc->rand_keydata, sizeof(wlc->rand_keydata));
		WL_WSEC(("wl%d: init random key value: %s\n", wlc->pub->unit, line));

		wlc_copyto_shm(wlc, wlc->seckeys + offset, wlc->rand_keydata, D11_MAX_KEY_SIZE);
	}
	...
	if (key &&
		 ...
		 */
		wlc_key_write_addrmatch(wlc, rcmta_index,
			((PSTA_ENAB(wlc->pub) && BSSCFG_STA(bsscfg)) ?
			&bsscfg->cur_etheraddr : &key->ea), amt_attr);
	}
...
#ifdef BCMWAPI_WPI
static uint8
wlc_key_get_algo(wlc_info_t *wlc)
{
	uint8 algo_hw = D11_PRE40_WSEC_ALGO_SMS4; /* default */

	if (D11REV_GE(wlc->pub->corerev, 40))
		algo_hw = WSEC_ALGO_SMS4_DFT_2005_09_07;
	return algo_hw;
}
#endif /* BCMWAPI_WPI */

void wlc_key_update_secindxblk(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
	int wsec_index, uint8 amt_index)
{
	uint8 key_index = (uint8)wsec_index;
	wsec_key_t *key;

	if ((wsec_index < 0) || (key_index > WLC_MAX_WSEC_KEYS(wlc))) {
		WL_WSEC(("wl%d: %s: wsec key index not valid (%i)\n",
			wlc->pub->unit, __FUNCTION__, wsec_index));
		return;
	}
	key = WSEC_KEY(wlc, key_index);
	if (!key) {
		WL_WSEC(("wl%d: %s: wsec key NULL at index (%d)\n",
		         wlc->pub->unit, __FUNCTION__, key_index));
		return;
	}
	wlc_write_shm(wlc, (M_SECKINDXALGO_BLK + (amt_index + WSEC_MAX_DEFAULT_KEYS) * 2),
		key->algo_hw | (key_index << SKL_INDEX_SHIFT));

	WL_WSEC(("wl%d: %s: updated key index %d, amt index %d\n",
		wlc->pub->unit, __FUNCTION__, key_index, amt_index));
	return;
}
	...
	int8 match_index, int8 skl_index, int8 key_index)
{
	...
	if (match_index != -1) {
		uint16 attr;
		wlc_get_addrmatch(wlc, match_index, &ea, &attr);
	}
