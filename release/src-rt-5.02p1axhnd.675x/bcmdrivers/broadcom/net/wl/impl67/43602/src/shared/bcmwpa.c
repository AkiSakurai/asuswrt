 ...
 *
 * $Id: bcmwpa.c 784961 2020-03-10 06:53:38Z $
 */
...
#endif	/* BCMCCX */
#ifdef WLDPT
	if (!bcmp((char *)suite->oui, BRCM_OUI, DOT11_OUI_LEN)) {
		switch (suite->type) {
...
{
#ifdef WL_SAE
	uint32 auth = wpa_auth;
#else
	uint16 auth = wpa_auth;
#endif /* WL_SAE */
...
bool
bcmwpa_is_rsn_auth(uint32 wpa_auth)
{
	uint32 auth = wpa_auth;
...
bool
bcmwpa_includes_rsn_auth(uint32 wpa_auth)
{
	uint32 auth = wpa_auth;
...
{
	return ((auth & WPA3_AUTH_SAE_PSK) != 0);
}
#endif /* WL_SAE */
