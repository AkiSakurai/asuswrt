 ...
 *
 * $Id: bcmsrom.c 666951 2016-10-25 09:44:01Z $
 */
...
#ifdef BCMPCIEDEV
	if ((BUSTYPE(bustype) == SI_BUS) && BCM43602_CHIP(sih->chip)) {
#else
	...
	bool check_crc)
{
	...
	/* XXX This CRC check bypass is used for 4350 X14
	* where CRC may not be programmed properly
	* JIRA:SWWLAN-40922
	*/
		...
		*/
		...
		if ((si_is_sprom_available(sih) && srom[0] == 0xffff) ||
			...
			if (defvarslen == 0) {
				...
				else if (CHIPID(sih->chip) == BCM43455_CHIP_ID) {
					defvarslen = srom_vars_len(defaultsromvars_4335);
					bcopy(defaultsromvars_4335, vp, defvarslen);
				}
#endif // endif
...
#ifdef UNRELEASEDCHIP
				} else if (CHIPID(sih->chip) == BCM43457_CHIP_ID) {
					defvarslen = srom_vars_len(defaultsromvars_4345);
					bcopy(defaultsromvars_4345, vp, defvarslen);
...
#ifdef UNRELEASEDCHIP
	case BCM43457_CHIP_ID: ciss = 1; defcis = defcis4335; hdrsz = 4; break;
	case BCM43455_CHIP_ID: ciss = 1; defcis = defcis4335; hdrsz = 4; break;
#endif /* UNRELEASEDCHIP */
	case BCM4350_CHIP_ID:
	case BCM4354_CHIP_ID:
	case BCM4356_CHIP_ID:
