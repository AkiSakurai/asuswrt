 ...
 *
 * $Id: wlc_bmac_stubs.c 649365 2016-07-15 19:54:06Z $
 */
...
wlc_phy_calc_ppr_pwr_cap(wlc_phy_t *ppi, uint8 core)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(uint32), WLRPC_WLC_PHY_CAL_CACHE_RESTORE_ID);
	ASSERT(rpc_buf != NULL);
...
wlc_phy_ioctl(wlc_phy_t *ppi, int cmd, int len, void *arg, bool *ta_ok)
{
	...
	/* XXX All the references to this so far are 32-bit bufs, need to do it case by
	 * case if there're other types of input/output buffers
	 */
...
wlc_bmac_dump(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *strb, wlc_bmac_dump_id_t dump_id)
{
	...
	do {
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, 3*sizeof(uint32), WLRPC_WLC_BMAC_DUMP_ID);
	...
	err = bcm_xdr_unpack_int32(&retb, &offset);
	ASSERT(!err);
...
#if defined(WLMCHAN) || defined(WL_MODESW)
void
...
void
wlc_phy_set_locale(wlc_phy_t *pi, uint8 region_group)
{
	bcm_xdr_buf_t b;
	rpc_buf_t *rpc_buf;
	int err;
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, sizeof(uint32), WLRPC_WLC_PHY_SET_LOCALE);
	ASSERT(rpc_buf != NULL);
