 ...
 *
 * $Id: wlc_sup.c 784961 2020-03-10 06:53:38Z $
 */
...
wlc_wpa_sup_sendeapol(wlc_sup_info_t *sup_info, wlc_bsscfg_t *cfg, uint16 flags, wpa_msg_t msg)
{
	...
	if (p != NULL) {
#ifdef WLDPT
		if (sup->wpa->WPA_auth == BRCM_AUTH_DPT)
...
wlc_wpa_sup_eapol(supplicant_t *sup, eapol_header_t *eapol, bool encrypted)
{
	...
	if (key_info & WPA_KEY_PAIRWISE) {
		...
		if (!(key_info & WPA_KEY_MIC)) {
			...
			if (wpa->WPA_auth == BRCM_AUTH_DPT) {
				wpa->state = WPA_SUP_KEYUPDATE;
				wlc_dpt_port_open(sup->wlc->dpt, &sup->peer_ea);
			} else
#endif // endif
			if (bcmwpa_is_rsn_auth(wpa->WPA_auth) &&
#ifdef WLBTAMP
...
#endif /* BCMSUP_PSK */
#ifdef WLDPT
		if (sup->cfg->WPA_auth == BRCM_AUTH_DPT) {
			wlc_dpt_wpa_passhash_done(sup->wlc->dpt, &sup->peer_ea);
			return;
		}
#endif // endif
	...
	if (supbsspub->sup_type == SUP_WPAPSK && eapol_hdr->type == EAP_PACKET) {
		...
		if (eap_hdr->type == EAP_IDENTITY &&
			...
			if (sup->wpa->WPA_auth & WPA_AUTH_UNSPECIFIED ||
				...
				if (eacmp(&cfg->last_upd_bssid, &cfg->BSSID) &&
					BSSCFG_ROAMOFFL(cfg, &cfg->BSSID)) {
					/* Send up WLC_E_BSSID
					 * No need to send up FT key info here
					 * because a new full auth is about to happen now
					 */
					uint bss_type = cfg->BSS ? DOT11_BSSTYPE_INFRASTRUCTURE :
						DOT11_BSSTYPE_INDEPENDENT;
					WL_ROAM(("WLC_E_BSSID @ EAPOL start\n"));
					bcopy(&cfg->BSSID, &cfg->last_upd_bssid, ETHER_ADDR_LEN);
					wlc_bss_mac_event(cfg->wlc, cfg, WLC_E_BSSID, &cfg->BSSID,
						0, WLC_E_STATUS_SUCCESS, bss_type, 0, 0);
				}
...
#ifdef MFP
	scb = wlc_scbfind(sup->wlc, cfg, &cfg->BSSID);

	if (scb == NULL) {
		return BCME_ERROR;
	}
	if (WLC_MFP_ENAB(sup->wlc->pub) && (scb->flags2 & SCB2_MFP))
...
#ifdef BCMSUP_PSK
int
wlc_sup_get_state(wlc_sup_info_t *sup_info, wlc_bsscfg_t *cfg)
{
	supplicant_t *sup = SUP_BSSCFG_CUBBY_PRIV(sup_info, cfg);
	wpapsk_t *wpa = sup->wpa;
	return wpa->state;
...
#ifndef NO_ROAMOFFL_SUPPORT
int
wlc_sup_late_enable(wlc_sup_info_t *sup_info, wlc_bsscfg_t *cfg)
{
	int err = 0;
	supplicant_t *sup = SUP_BSSCFG_CUBBY_PRIV(sup_info, cfg);

	wlc_sup_handle_joinstart(sup_info, cfg);
	wlc_sup_handle_joindone(sup_info, cfg);
#if defined(BCMCCX) && defined(BCMINTSUP) && defined(BCMSUP_PSK)
	err = wlc_sup_set_pmk(sup_info, cfg, &cfg->pmk, cfg->associated);
	wlc_ccx_update_krk_btk(cfg);
#endif /* BCMCCX && BCMINTSUP && BCMSUP_PSK */
