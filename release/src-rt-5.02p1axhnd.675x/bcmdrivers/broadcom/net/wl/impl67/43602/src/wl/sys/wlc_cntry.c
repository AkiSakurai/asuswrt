 ...
 *
 * $Id: wlc_cntry.c 766613 2018-08-08 06:59:54Z $
 */
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_COUNTRY): {
		...
		strncpy(country_abbrev_host, country_abbrev, WLC_CNTRY_BUF_SZ-1);
...
wlc_populate_ccode_info(wlc_info_t *wlc, void *arg, uint len)
{
	...
	for (band = 0, ce = &ci->ccodelist[0], count = 0; band < MAXBANDS; band ++) {
		...
		if (WLC_CNTRY_DEFAULT_ENAB(wlc)) {
			hd = wlc_cntry_get_default(wlc->cntry);
			if (hd) {
				strncpy(ce->ccode, hd, sizeof(ccode_t));
				ce->band = band;
