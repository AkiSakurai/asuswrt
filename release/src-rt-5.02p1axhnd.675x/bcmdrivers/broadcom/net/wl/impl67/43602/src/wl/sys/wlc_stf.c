 ...
 *
 * $Id: wlc_stf.c 786782 2020-05-07 19:58:27Z $
 */
...
#endif /* WL11AC */
	IOV_CCK_TX_MODE, 	/* on/off single chain TX for 11b rates (1,2, 5,5,11 mbps) */
	IOV_STF_LAST
};
	...
	switch (actionid) {
		...
		case IOV_SVAL(IOV_STF_RXCHAIN):
			...
			if (err == BCME_OK) {
...
#if defined(WLPROPRIETARY_11N_RATES) /* avoids ROM abandon of this function */
				...
				/* update stf coremask bits in shmem that */
				/* may depend on num of rxchains (e.g. cck txdiv) */
		...
		case IOV_GVAL(IOV_STF_TXCORE):
		{
			...
			if (wlc->clk) {
			/* get current state if cck tx diversity */
				cck_txdiv = (wlc_read_shm(wlc, M_COREMASK_BPHY) &
					BPHY_ONE_CORE_TX) != 0;
			} else {
				/* no clock, return value to be applied on Radio up */
				cck_txdiv = wlc->stf->cck_onecore_tx &&
					(wlc->stf->txstreams > 1) && (wlc->stf->rxstreams > 1);
			}
			/* wl expects the bit in core[1]:23 */
			core[1] |=  (uint32)cck_txdiv << 23;
		...
		case IOV_SVAL(IOV_STF_TXCORE):
		{
			...
			if (core[1]) {
				...
				/* keep cck tx diversity setting in stf structure */
				wlc->stf->cck_onecore_tx =
					((core[1] >> 16) & CCK_TX_DIVERSITY_BIT) != 0;
		...
		case IOV_GVAL(IOV_CCK_TX_MODE):
		{
			uint16 coremask_bphy;
			coremask_bphy = wlc_read_shm(wlc, M_COREMASK_BPHY);
			*ret_int_ptr = (coremask_bphy & BPHY_ONE_CORE_TX) != 0;
			break;
		}

		case IOV_SVAL(IOV_CCK_TX_MODE):
		{
			uint8 ccx_txmode = (uint8)int_val;
			uint16 coremask_bphy;

			coremask_bphy = wlc_read_shm(wlc, M_COREMASK_BPHY);

			if (ccx_txmode) {
				coremask_bphy |= BPHY_ONE_CORE_TX;
			} else {
				coremask_bphy &= ~(BPHY_ONE_CORE_TX);
			}
			wlc_write_shm(wlc, M_COREMASK_BPHY, coremask_bphy);
			break;
		}
...
{
	uint i, j;
	uint n = PHYTYPE_HT_CAP(wlc->band) ? WL_NUM_3x3_ELEMENTS : WL_NUM_2x2_ELEMENTS;
	...
	bcm_bprintf(b, "Power/Rate Dump (in 1/4dB): Channel %d\n", CHSPEC_CHANNEL(wlc->chanspec));
#else
	...
	for (i = 0; i < n; i++) {
		...
		if (bprint) {
			bcm_bprintf(b, "\n%s", str);
#ifdef WL11AC
			BPRINT_PPR_RATE_LOOP(b, j, WL_RATESET_SZ_VHT_MCS, ptr);
#else
...
wlc_stf_txcore_select(wlc_info_t *wlc, uint8 *txcore) /* C_CHECK */
{
	...
	for (idx = 0; idx < WL_NUM_RATES_CCK; idx++) {
		if (dsss1x1_pwrs.pwr[idx] == WL_RATE_DISABLED)
			dsss1x1_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
		if (dsss1x2_pwrs.pwr[idx] == WL_RATE_DISABLED)
			dsss1x2_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
		if (nstreams == 3)
			if (dsss1x3_pwrs.pwr[idx] == WL_RATE_DISABLED)
				dsss1x3_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
	}
...
ofdm_cal:
	...
	for (idx = 0; idx < WL_NUM_RATES_OFDM; idx++) {
		if (ofdm1x1_pwrs.pwr[idx] == WL_RATE_DISABLED)
			ofdm1x1_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
		if (ofdm1x2_pwrs.pwr[idx] == WL_RATE_DISABLED)
			ofdm1x2_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
		if (nstreams == 3)
			if (ofdm1x3_pwrs.pwr[idx] == WL_RATE_DISABLED)
				ofdm1x3_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
	}
...
mcs_cal:
...
#ifdef WL11AC
	if (isbw80) {
		ppr_get_vht_mcs(txpwr, WL_TX_BW_80, WL_TX_NSS_1, WL_TX_MODE_NONE, WL_TX_CHAINS_1,
			&mcsx1_pwrs);
		ppr_get_vht_mcs(txpwr, WL_TX_BW_80, WL_TX_NSS_1, WL_TX_MODE_CDD, WL_TX_CHAINS_2,
			&mcsx2_pwrs);
		if (nstreams == 3)
	...
	for (idx = 0; idx < NUM_MCS_RATES; idx++) {
		if (mcsx1_pwrs.pwr[idx] == WL_RATE_DISABLED)
			mcsx1_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
		if (mcsx2_pwrs.pwr[idx] == WL_RATE_DISABLED)
			mcsx2_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
		if (nstreams == 3)
			if (mcsx3_pwrs.pwr[idx] == WL_RATE_DISABLED)
				mcsx3_pwrs.pwr[idx] = (int8)wlc->stf->max_offset;
	}
	...
	if (wlc->stf->txchain) {
		if (wlc->stf->txcore_override[OFDM_IDX]) {
			if ((wlc->stf->txcore_override[OFDM_IDX] & wlc->stf->txchain) !=
				...
				wlc->stf->prev_txcore_override[OFDM_IDX] =
					wlc->stf->txcore_override[OFDM_IDX];
			...
			if ((wlc->stf->prev_txcore_override[OFDM_IDX]) &&
				txchain_diff) {
				wlc->stf->txcore_override[OFDM_IDX] =
					wlc->stf->prev_txcore_override[OFDM_IDX];
			}
		...
		if (wlc->stf->txcore_override[CCK_IDX]) {
			if ((wlc->stf->txcore_override[CCK_IDX] & wlc->stf->txchain) !=
				...
				WL_NONE(("cck override removed chain=%x val=%x\n",
					wlc->stf->txcore_override[CCK_IDX], wlc->stf->txchain));
				wlc->stf->prev_txcore_override[CCK_IDX] =
					wlc->stf->txcore_override[CCK_IDX];
			...
			if ((wlc->stf->prev_txcore_override[CCK_IDX]) &&
				txchain_diff) {
				wlc->stf->txcore_override[CCK_IDX] =
					wlc->stf->prev_txcore_override[CCK_IDX];
			}
...
wlc_stf_rxchain_set(wlc_info_t* wlc, int32 int_val, bool update)
{
	...
	if (AIBSS_ENAB(wlc->pub)) {
		...
		if (old_rxchain_cnt == 1) {
			wlc_phy_stf_chain_set(wlc->band->pi, wlc->stf->txchain, rxchain);
		}

		/* Init action state */
		mimops_mode = (rxchain_cnt == 1) ? HT_CAP_MIMO_PS_ON : HT_CAP_MIMO_PS_OFF;
		cfg->mimops_ActionPM = mimops_mode;
		cfg->mimops_ActionRetry = 0;
...
wlc_stf_txcore_shmem_write(wlc_info_t *wlc, bool forcewr)
{
	...
	for (offset = 0, idx = 0; offset < max_coremask_blk; offset++, idx++) {
		...
		/* cck tx diversity feature requires two or more rx & tx chains */
		if ((idx == CCK_IDX) && wlc->stf->cck_onecore_tx &&
			(wlc->stf->txstreams > 1) && (wlc->stf->rxstreams > 1)) {
			/*  shmem bit 15 enables onecore tx with diversity for b rates */
			map |= BPHY_ONE_CORE_TX;
		}
...
BCMATTACHFN(wlc_stf_attach)(wlc_info_t* wlc)
{
	...
	wlc->stf->cck_onecore_tx = (bool)getintvar(wlc->pub->vars, rstr_cck_onecore_tx);
	...
	if ((wlc->stf->scb_handle = wlc_scb_cubby_reserve(wlc,
		...
		sizeof(stf_scb_info_t), NULL, NULL, NULL, NULL, 0)) < 0) {
#else
...
BCMATTACHFN(wlc_stf_phy_chain_calc)(wlc_info_t *wlc)
{
	...
	wlc->stf->hw_txchain = (uint8)getintvar(wlc->pub->vars, rstr_txchain);
	wlc->stf->hw_rxchain = (uint8)getintvar(wlc->pub->vars, rstr_rxchain);
	...
	if ((CHIPID(wlc->pub->sih->chip) == BCM4352_CHIP_ID) ||
		(wlc->pub->sih->chip == BCM43522_CHIP_ID) /* can't use CHIPID macro here */) {
		if (wlc->stf->hw_txchain == 0 || wlc->stf->hw_txchain >= 7) {
			WL_ERROR(("wl%d: %s: wrong txchain setting %x for %d. Correct it to 3\n",
				wlc->pub->unit, __FUNCTION__, wlc->stf->hw_txchain, wlc->pub->sih->chip));
			wlc->stf->hw_txchain = 3;
		}

		if (wlc->stf->hw_rxchain == 0 || wlc->stf->hw_rxchain >= 7) {
			WL_ERROR(("wl%d: %s: wrong rxchain setting %x for %d. Correct it to 3\n",
				wlc->pub->unit, __FUNCTION__, wlc->stf->hw_rxchain, wlc->pub->sih->chip));
...
wlc_stf_get_204080_pwrs(wlc_info_t *wlc, ratespec_t rspec, txpwr204080_t* pwrs)
{
	 ...
	 */
	wl_tx_chains_t txbf_chains = wlc->stf->txstreams;
	uint i, j;
	...
	} else if (((int)chains > (int)nss) && !((chains == 3) && (nss == 2)))
		mode = WL_TX_MODE_CDD;
...
bool
get_cck_override_diversity(wlc_info_t *wlc, ratespec_t rspec)
{
	bool ret = FALSE;
	if (RATE_ISCCK(rspec) && (wlc_ratespec_ntx(rspec) == 1)) {
		ret = (wlc->stf->txcore_override[CCK_IDX] & CCK_TX_DIVERSITY_BIT);
	}
	return ret;
}
