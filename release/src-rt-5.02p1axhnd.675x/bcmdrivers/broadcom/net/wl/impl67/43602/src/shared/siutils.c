 ...
 *
 * $Id: siutils.c 666951 2016-10-25 09:44:01Z $
 */
...
static const si_mux_uartopt_t BCMATTACHDATA(mux4350_uartopt)[] = {
		...
		{CC4350_PIN_GPIO_07, CC4350_PIN_GPIO_01}
};
...
BCMATTACHFN(si_muxenab)(si_t *sih, uint32 w)
{
		...
		if (w & MUXENAB4350_UART_MASK) {
			...
			if (uartopt_idx < uartopt_size) {
				...
				if ((CHIPREV(sih->chiprev) >= 3) ||
					...
					/*
					 * apply MISC2 function to configure GPIO7
					 * as dbg UART_TX
					 */
					fnsel = (uart_tx == CC4350_PIN_GPIO_07) ?
						CC4350_FNSEL_MISC2 : CC4350C_FNSEL_UART;
					si_gci_set_functionsel(sih, uart_rx, CC4350C_FNSEL_UART);
					si_gci_set_functionsel(sih, uart_tx, fnsel);
				}
				else {
	...
	case BCM43430_CHIP_ID:
		if (w & MUXENAB43430_UART_MASK) {
			si_gci_set_functionsel(sih, CC43430_PIN_GPIO_00, CC43430_FNSEL_UART);
...
/* Enable pinmux (chipControl24) for SECI over GCI configuration (x87) */
	chipcontrol = si_corereg(sih, SI_CC_IDX, OFFSETOF(chipcregs_t, chipcontrol), 0, 0);
	if(sih->bSeciOnSprom) {
		/* Select SECI on DPROM_IN/OUT pins */
		si_corereg(sih, SI_CC_IDX, OFFSETOF(chipcregs_t, chipcontrol),
			~0, chipcontrol | CCTRL4360_SECI_MODE);
	} else {
		/* Select SECI on GPIO_0/1 pins */
		si_corereg(sih, SI_CC_IDX, OFFSETOF(chipcregs_t, chipcontrol),
			~0, chipcontrol | CCTRL4360_SECI_ON_GPIO01);
	}

	/* enable SECI mode */
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_corectrl), ALLONES_32,
	             (GCI_CORECTL_SECICLKSTR << GCI_CORECTL_SECICLKSTR_SHIFT)
	             | GCI_CORECTRL_MODE_SECI | GCI_CORECTRL_SECI_EN);
...
/* baudrate:4Mbps at 40MHz xtal, escseq:0xdb, high baudrate, enable seci_tx/rx */
	...
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secibauddiv), ALLONES_32, 0xF6);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_baudadj), ALLONES_32, 0xFF);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secifcr), ALLONES_32, 0x00);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secimcr), ALLONES_32, 0x09);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secilcr), ALLONES_32, 0xA8);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_uartescval), ALLONES_32, 0xDB);

	/* WLAN rx offset assignment */
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_secif0rx_offset), ALLONES_32, 0x13121110);
	...
	/* Enable nibble 0 to 11 as format 0 */
	si_gci_indirect(sih, 0,  OFFSETOF(chipcregs_t, gci_seciusef0tx_reg),
			ALLONES_32, 0xFFF);

	/* TX offset assignment (wlan to bt) */
	si_gci_indirect(sih, 0, OFFSETOF(chipcregs_t, gci_secif0tx_offset),
			ALLONES_32, 0x76543210);
	si_gci_indirect(sih, 1, OFFSETOF(chipcregs_t, gci_secif0tx_offset),
			ALLONES_32, 0x0000ba98);
}
...
si_ercx_init(si_t *sih, uint32 ltecx_mux)
{
	...
	if (CHIPID(sih->chip) == BCM4334_CHIP_ID) {
		...
		CHIPID(sih->chip) == BCM43455_CHIP_ID ||
...
si_wci2_init(si_t *sih, uint baudrate, uint32 ltecx_mux)
{
	...
	/* enable interrupts only on pos-edge of FrmSync */
	si_gci_indirect(sih, 0x00010, OFFSETOF(chipcregs_t, gci_level_polreg),
		GCI_FROMLTE_FRAMESYNC, GCI_FROMLTE_FRAMESYNC);
                       ...
                       uint bustype, void *sdh, char **vars, uint *varsz)
{
	...
	*/
	...
	if ((sih->chip == BCM4356_CHIP_ID) ||
		(sih->chip == BCM4358_CHIP_ID) ||
		(sih->chip == BCM43566_CHIP_ID) ||
		(sih->chip == BCM43567_CHIP_ID)) {
		si_chipid_fixup(sih);
	}
	...
	if (getvar(pvars, "seci_on_sprom"))
		sii->pub.bSeciOnSprom = (bool) getintvar(pvars, "seci_on_sprom");
	else
		sii->pub.bSeciOnSprom = (bool) 0;
...
void si_setup_curmap(osl_t *osh, si_t *sih)
{
	...
	sii = SI_INFO(sih);
#if defined(MACOSX)
/* XXX : Setup the  APB Wrapper for 4360/4352
**. Refer to http://hwnbu-twiki.broadcom.com/bin/view/Mwgroup/BackplaneArchitecture4360 **
** look for IDM wrapper register regions on APB 1 and then look for the address map **
** for APB Bridge 0 slave wrapper register region, ie 0x18108000 **
*/
...
#if defined(MACOSX)
/* XXX
**	Refer to ArmDocumentation for configuring the
** errorLogging , PL368_TRM.pdf
*/
...
void
si_eci_notify_wl_ant_info_to_bt(si_t *sih, uint32 val)
{
	if (sih->cccaps_ext & CC_CAP_EXT_GCI_PRESENT) {
		/* GCI Mode */
		si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_control_1),
			GCI_WL_ANT_BIT_MASK, GCI_WL_ANT_BIT_MASK);
		si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_output[1]),
			GCI_WL_ANT_BIT_MASK, val);
	}
}
...
BCMINITFN(si_gci_init)(si_t *sih)
{
	...
	if (sih->cccaps_ext & CC_CAP_EXT_GCI_PRESENT)
	{
		...
		/* Set GCI Control bits 36, 40-47, 56 to be SW Controlled. These bits
		   contain Mchan indication, WL channel info, and reduce Txpwr indication to BT.
		*/
		si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_control_1),
				(GCI_WL_CHN_INFO_MASK |
				GCI_WL_MCHAN_BIT_MASK |
				GCI_WL2BT_REDUCE_BT_POWER_MASK),
				(GCI_WL_CHN_INFO_MASK |
				GCI_WL_MCHAN_BIT_MASK |
				GCI_WL2BT_REDUCE_BT_POWER_MASK));
	}
...
si_pcie_set_ctrlreg(si_t *sih, uint32 mask, uint32 val)
{
	...
	if (((CHIPID(sih->chip) != BCM4335_CHIP_ID) || (BUSTYPE(sih->bustype) != PCI_BUS)))
		return 0;
#ifdef UNRELEASEDCHIP
	if (((CHIPID(sih->chip) != BCM43455_CHIP_ID) || (BUSTYPE(sih->bustype) != PCI_BUS)))
		return 0;
#endif // endif
...
#ifdef SSWAR
static const char BCMATTACHDATA(rstr_host_L12exit_war_opt)[] = "host_L12exit_war_opt";
uint8
BCMATTACHFN(si_enable_host_L12exit_war)(si_t *sih, uint8 *wake_mask, uint8 *cur_status)
{
	uint8  gci_gpio = CC_GCI_GPIO_INVALID;

	/* parse the L1 exit war opt from nvram */
	/* decode what that means for specific chip */
	/* apply the right gci config */
	/* enable the internal interrupts */
	/* assume: caller already registered handler for that GCI int */
	if (getvar(NULL, rstr_host_L12exit_war_opt) == NULL)
		return gci_gpio;

	gci_gpio = (uint8)(getintvar(NULL, rstr_host_L12exit_war_opt) & 0xff);
	...
	switch (CHIPID(sih->chip)) {
		...
		break;
		default:
			SI_ERROR(("L1.2 exit WAR is not supported for 0x%04x yet\n",
				CHIPID(sih->chip)));
		break;
	}
	return gci_gpio;
