 ...
 *
 * $Id: wlc_channel.c 770726 2018-12-31 07:02:15Z $
 */
...
static int wlc_channel_init_ccode(wlc_cm_info_t *wlc_cm, char* country_abbrev, int ca_len);
...
static bool wlc_channel_clm_chanspec_valid(wlc_cm_info_t *wlc_cm, chanspec_t chspec);
...
static const rcinfo_t rcinfo_global_20 = {
	...
	{124, 121}, {128, 121}, {132, 121}, {136, 121}, {140, 121}, {149, 125}, {153, 125},
	{157, 125}, {161, 125}, {165, 125}
	}
};
#endif /* BAND5G */
...
static const rcinfo_t rcinfo_global_center_80 = {
	6,
	{
	{42, 128}, {58, 128}, {106, 128}, {122, 128}, {138, 128}, {155, 128}, {0, 0}, {0, 0},
...

		{
			...
			for (i = 0; i < wbi->segment_info->num_segments; i++) {
				WL_NONE(("  segment %3d - "
					...
					i,
					wbi->segment_info->segments[i].type,
					wbi->segment_info->segments[i].offset,
					wbi->segment_info->segments[i].length,
					wbi->segment_info->segments[i].crc32,
					wbi->segment_info->segments[i].flags));
			}
		}
	}
...
#else
	/* Now 7.35 has not been verified by SVT, we just release for 43162 */
	if (CHIPID(pub->sih->chip) != BCM4335_CHIP_ID) {
...
/* These two legacy, unused declarations that have to remaing because of references from ROM
 * cause them to be in the shared RAM section whose layout must stay the same.
 */
static struct clm_data_header* download_dataptr;
static int download_data_len;
...
BCMATTACHFN(wlc_channel_mgr_attach)(wlc_info_t *wlc)
{
	...
	/* Two legacy references that must remain.  See comment at declaration for more information.
	 * Note the variables can't be just BCM_REFERENCE().  Even though this avoids the compiler
	 * unused variable warning (which we treat as an error), there is no code generated to
	 * actually use the variable. The linker will then remove the variables since we use per
	...
	if ((wlc_cm->clmload_wbi = wlc_blob_attach(wlc)) == NULL) {
		MFREE(pub->osh, wlc_cm, sizeof(wlc_cm_info_t));
		return NULL;
	}

	if ((wlc_cm->valid_rcvec.vec = (uint8 *) MALLOC(pub->osh, MAXRCVEC)) == NULL) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes", pub->unit,
			__FUNCTION__, MALLOCED(pub->osh)));
		goto fail;
	}
	bzero(wlc_cm->valid_rcvec.vec, MAXRCVEC);
...
static dload_error_status_t
wlc_handle_clm_dload(wlc_cm_info_t *wlc_cm, segment_t *segments, uint32 segment_count)
{
	...
	if (clm_dataptr != NULL) {
		WL_NONE(("wl%d: Pointing API at new base data: v%s\n",
			wlc_cm->pub->unit, clm_dataptr->clm_version));
		...
		if (clm_result != CLM_RESULT_OK) {
			...
			MFREE(wlc_cm->pub->osh, clm_dataptr,
				clm_data_len);
		}
	}
	...
	wlc_channel_init_ccode(wlc_cm, country_abbrev, sizeof(country_abbrev));
...
BCMATTACHFN(wlc_channel_mgr_detach)(wlc_cm_info_t *wlc_cm)
{
	...
	if (wlc_cm) {
		...
		if (wlc_cm->valid_rcvec.vec != NULL) {
			MFREE(wlc_cm->pub->osh, wlc_cm->valid_rcvec.vec, MAXRCVEC);
		}
...
const char* wlc_channel_sromccode(wlc_cm_info_t *wlc_cm)
{
	return wlc_cm->srom_ccode;
}
...
uint16 wlc_channel_locale_flags(wlc_cm_info_t *wlc_cm)
{
	wlc_info_t *wlc = wlc_cm->wlc;
...
uint16 wlc_channel_locale_flags_in_band(wlc_cm_info_t *wlc_cm, uint bandunit)
{
...
wlc_channels_init(wlc_cm_info_t *wlc_cm, clm_country_t country)
{
	...
	wlc_update_rcinfo(wlc_cm, switch_to_global_opclass);
...
wlc_channel_set_chanspec(wlc_cm_info_t *wlc_cm, chanspec_t chanspec)
{
	...
	uint8 cur_idx_save = wlc->pldo.cur_idx;
	wlc_temp_evm_war_init(wlc);
	/* When channel change, keep the xtalldo cur_idx status */
	wlc->pldo.cur_idx = cur_idx_save;
	...
	/* need to wait until reg limits are set before retrieving constraint */
...
wlc_regclass_vec_init(wlc_cm_info_t *wlc_cm)
{
	...
	bool saved_cap_80;
#endif	/* WL11AC */
	uint8 *rcvec = wlc_cm->valid_rcvec.vec;
	...
	for (i = 0; i < MAXCHANNEL; i++) {
		...
		if (VHT_ENAB(wlc->pub)) {
			...
			if (wlc_valid_chanspec(wlc_cm, chanspec)) {
				if ((idx = wlc_get_regclass(wlc_cm, chanspec)))
					setbit((uint8 *)rcvec, idx);
			}
		}
	...
	chanspec_t chspec, bool ie_order)
{
	...
	for (i = 0; i < MAXREGCLASS && idx < lsize; i++) {
		if (isset(wlc_cm->valid_rcvec.vec, i))
			rclist[idx++] = i;
	}
...
{
	if (CHSPEC_IS80_UNCOND(chspec)) {
		clrbit(wlc_cm->restricted_channels.vec, LL_20_SB(CHSPEC_CHANNEL(chspec)));
		clrbit(wlc_cm->restricted_channels.vec, LU_20_SB(CHSPEC_CHANNEL(chspec)));
		clrbit(wlc_cm->restricted_channels.vec, UL_20_SB(CHSPEC_CHANNEL(chspec)));
		clrbit(wlc_cm->restricted_channels.vec, UU_20_SB(CHSPEC_CHANNEL(chspec)));
	} else if (CHSPEC_IS40_UNCOND(chspec)) {
...
wlc_channel_dump_srom_ppr(void *handle, struct bcmstrbuf *b)
{
	...
	if ((srompwr = ppr_create(wlc_cm->pub->osh, ppr_get_max_bw())) == NULL) {
		return BCME_ERROR;
	}
...
void
wlc_update_rcinfo(wlc_cm_info_t *wlc_cm, bool switch_to_global_opclass)
{
	wlc_pub_t *pub = wlc_cm->pub;
...
bool
wlc_sta_supports_global_rclass(wlc_info_t *wlc, uint8 *setBitBuff, uint8 *pflag)
{
	bool global_opclass_support = FALSE;

	/* MAXRCLIST = 3 for 20, 40 lower and 40 upper RCINFO
	 * MACRCLIST_REG = 1 for 80 RCINFO
	 */
	 ...
	 */
	/* check for 2G band */
	if (isset(setBitBuff, WLC_REGCLASS_GLOBAL_2G_20MHZ) ||
		isset(setBitBuff, WLC_REGCLASS_GLOBAL_2G_20MHZ_CH14)) {
		global_opclass_support = TRUE;
	}
#ifdef BAND5G
	rcinfo[0] =  &rcinfo_global_center_80;
	rcinfo[1] =  &rcinfo_global_40upper;
	rcinfo[2] =  &rcinfo_global_40lower;
	rcinfo[3] =  &rcinfo_global_20;

	if ((wlc_chk_rclass_support_5g(rcinfo[0], setBitBuff)) ||
		(wlc_chk_rclass_support_5g(rcinfo[1], setBitBuff)) ||
		(wlc_chk_rclass_support_5g(rcinfo[2], setBitBuff)) ||
		(wlc_chk_rclass_support_5g(rcinfo[3], setBitBuff))) {
