 ...
 *
 * $Id: wlc_intr.c 676722 2016-12-23 10:47:06Z $
 */
	...
	if (macintstatus & MI_DMAINT) {
		if (D11REV_IS(wlc_hw->corerev, 4)) {
			...
			if (BCMSPLITRX_ENAB()) {
				...
				W_REG(osh, &regs->intctrlregs[RX_FIFO1].intstatus, DEF_RXINTMASK);
...
wlc_intrsoff(wlc_info_t *wlc)
{
	...
	SET_MACINTMASK(wlc_hw->osh, wlc_hw->regs, 0);
	(void)GET_MACINTMASK(wlc_hw->osh, wlc_hw->regs);	/* sync readback */
