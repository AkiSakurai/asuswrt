 ...
 *
 * $Id: wlc_auth.c 784961 2020-03-10 06:53:38Z $
 */
...
#define KEY_DESC(auth, scb) (SCB_SHA256(scb) ? WPA_KEY_DESC_V3 : (\
	!WSEC_TKIP_ENABLED(auth->bsscfg->wsec) ? WPA_KEY_DESC_V2 :  \
		WPA_KEY_DESC_V1))
	...
	uint eapol_ver)
{
	...
	scrbuf = MALLOC(auth->wlc->osh, MAX(WPA_KEY_DATA_LEN_256, WPA_MIC_KEY_LEN*2));
...
err:
	if (scrbuf)
		MFREE(auth->wlc->osh, scrbuf, MAX(WPA_KEY_DATA_LEN_256, WPA_MIC_KEY_LEN*2));
	if (rc4key)
...
static int
wlc_auth_plumb_gtk(authenticator_t *auth)
{
...
BCMATTACHFN(wlc_auth_attach)(wlc_info_t *wlc)
{
	...
	auth_info->scb_handle =
	        ...
	        wlc_scb_cubby_reserve(wlc, sizeof(struct auth_cubby), wlc_auth_scb_init,
	                              wlc_auth_scb_deinit, NULL, (void*) auth_info, 0);
#else
		wlc_scb_cubby_reserve(wlc, sizeof(struct auth_cubby), wlc_auth_scb_init,
	...
	if (wlc_module_register(wlc->pub, auth_iovars, "auth", auth_info, wlc_auth_doiovar,
	                        wlc_auth_watchdog, NULL, wlc_auth_down)) {
...
#ifdef MFP
	if (WLC_MFP_ENAB(auth->wlc->pub))
		wlc_mfp_reset_igtk(AUTHMFP(auth), auth->bsscfg);
#endif // endif
}
                       ...
                       struct scb *scb)
{
	...
	case WPA_AUTH_PTKINITDONE:
		...
		if (wpa->WPA_auth == BRCM_AUTH_DPT)
			wlc_dpt_port_open(wlc->dpt, &scb->ea);
#endif /* WLDPT */
...
#ifdef MFP
			if (WLC_MFP_ENAB(auth->wlc->pub)) {
				buf_len += wlc_mfp_igtk_len(AUTHMFP(auth), auth->bsscfg);
...
void wlc_auth_tkip_micerr_handle(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
		...
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			...
			bsscfg->tk_cm_bt = WPA_TKIP_CM_BLOCK;
			bsscfg->tk_cm_bt_tmstmp = OSL_SYSUPTIME();
			bsscfg->tk_cm_activate = FALSE;
			bsscfg->tkip_countermeasures = TRUE;
		}
}
#endif /* BCMAUTH_PSK */
