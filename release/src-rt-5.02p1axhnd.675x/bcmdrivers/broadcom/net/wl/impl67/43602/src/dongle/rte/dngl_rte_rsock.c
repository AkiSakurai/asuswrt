 ...
 *
 * $Id: dngl_rte_rsock.c 464956 2014-03-26 08:59:40Z $
 */
	...
	uint32 cmd, void *buf, int len, int *used, int *needed, bool set)
{
	...
	ret = dngl->slave->funcs->ioctl(dngl->slave, cmd, buf, len, used, needed, set);
...
dngl_sendslave(struct dngl *dngl, void *p)
{
	...
	if (dngl->slave->funcs->xmit(dngl->rtedev, dngl->slave, lb)) {
...
int
BCMINITFN(dngl_binddev)(struct dngl *dngl, void *bus, void *dev)
{
	...
	if (dngl->rtedev == NULL && dngl->slave == NULL) {
		...
		err("dngl %p %p bus %p dev %p inconsistent", dngl->rtedev, dngl->slave, bus, dev);
		return -1;
	}
...
dngl_unbinddev(struct dngl *dngl, void *bus, void *dev)
{
	...
	ASSERT(dngl->slave == dev);
	ASSERT((hndrte_dev_t *)bus->chained == dev);
	ASSERT((hndrte_dev_t *)dev->chained == bus);

	((hndrte_dev_t *)bus)->chained = NULL;
...
dngl_opendev(struct dngl *dngl)
{
	...
	ASSERT(dngl->slave);
	dngl->slave->funcs->open(dngl->slave);
	dngl->devopen = TRUE;
...
dngl_init(struct dngl *dngl)
{
	...
	hndrte_cons_addcmd("reboot", (cons_fun_t)dngl_reboot, 0);
#endif /* BCMDBG */
	dngl->up = TRUE;
}
