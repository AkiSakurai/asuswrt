...
# $Id: wl.mk 784961 2020-03-10 06:53:38Z $
...
ifeq ($(WL),1)
	...
	ifneq ($(MINIAP),1)
		...
		WLFILES_SRC_LO += src/wl/phy/wlc_phytbl_lcn20.c
	endif

	WLFILES_SRC_HI += src/wl/sys/wlc.c
	WLFILES_SRC_HI += src/wl/sys/wlc_ldo.c
...
ifeq ($(ATE),1)
	...
	ifeq ($(WL_PROT_DYNBW),1)
		ifeq ($(WL_PROT_OBSS_DISABLED),1)
			WLFILES_SRC_HI += src/wl/sys/wlc_prot_obss.c
		endif
		WLFLAGS += -DWL_PROT_DYNBW
	endif
	ifeq ($(WL_MODESW),1)
		WLFLAGS += -DWL_MODESW
...
ifeq ($(WLNDIS),1)
	ifeq ($(WLXP), 1)
		...
		WLFILES_SRC_HI += src/wl/sys/wlc_hostoid.c
	endif
...
ifeq ($(WL_BTCDYN),1)
	...
	ifeq ($(WLCLMINC),1)
		WLFLAGS += -DWLCLMINC
		CLM_INC_FILE_SUFFIX ?= _inc
		WLFILES_SRC_HI += src/wl/clm/src/wlc_clm_data$(CLM_INC_FILE_SUFFIX).c
	endif
...
ifneq ($(BCM_STA_CFG80211)$(BCM_STA_CFG80211_HIGH),)
...
ifeq ($(BCM_HOST_MEM_SCB),1)
	WLFLAGS += -DBCM_HOST_MEM_SCB
	WLFILES_SRC += src/wl/sys/wlc_scb_alloc.c
endif

ifeq ($(BCM_HOST_MEM_RESTORE),1)
        WLFLAGS += -DBCM_HOST_MEM_RESTORE
endif
...
ifeq ($(WLFBT),1)
	...
	ifneq ($(WLVISTA)$(WLWIN7)$(WLWIN8),)
		WLFLAGS += -DEXT_STA
		WLFLAGS += -DWLFBT_OVER_DS_DISABLED
		WLFLAGS += -DWL_MONITOR
...
# LCN20CONF -- 0 is remove from code, else bit mask of supported lcn20phy revs
ifneq ($(LCN20CONF),)
	WLFLAGS += -DLCN20CONF=$(LCN20CONF)
endif
...
# ifdef HOST_CLK_SYNC
ifeq ($(WL_HOST_CLK_SYNC),1)
	WLFILES_SRC += src/wl/sys/wlc_hostclksync.c

	ifeq ($(WL_HOST_CLK_SYNC_NIC),1)
		WLFLAGS += -DWL_HOST_CLK_SYNC -DWL_HOST_CLK_SYNC_NIC
	else
		WLFLAGS += -DWL_HOST_CLK_SYNC -DWL_PROTO_ASTP
	endif
endif
# endif HOST_CLK_SYNC
...
ifeq ($(WL_ASSOC_MGR),1)
	...
	WLFILES_SRC_HI += src/wl/sys/wlc_assoc_mgr.c
endif
...
ifeq ($(WLFIPS),1)
	WLFLAGS += -DWLFIPS
	WLFILES_SRC_HI += src/wl/sys/wl_ndfips_5.c
	WLFILES_SRC_HI += src/bcmcrypto/aes.c
endif
#endif // endif
...
ifeq ($(WLDPT),1)
	WLFLAGS += -DWLDPT
	WLFILES_SRC_HI += src/wl/sys/wlc_dpt.c
endif
#endif // endif
...
ifeq ($(BCMBTCXGCI48BITS),1)
    WLFLAGS += -DBCMBTCXGCI48BITS
endif
#endif // endif
...
# Work-arounds for ROM compatibility in anqpo modules.
ifeq ($(BCM_ANQPO_ROM_COMPAT),1)
	EXTRA_DFLAGS += -DBCM_ANQPO_ROM_COMPAT
endif

# Work-arounds for ROM compatibility in apivtw modules.
ifeq ($(BCM_APIVTW_ROM_COMPAT),1)
        EXTRA_DFLAGS += -DBCM_APIVTW_ROM_COMPAT
endif
...
ifeq ($(WL_SHIF),1)
	WLFLAGS += -DWL_SHIF
	WLFILES_SRC += src/wl/sys/wlc_shub.c
endif
#endif // endif
...
ifeq ($(BCM_SHIF),1)
	WLFLAGS += -DBCM_SHIF_ENABLED
	WLFLAGS += -DDISABLE_CONSOLE_UART_OUTPUT
	WLFILES_SRC_LO += src/shared/bcm_shif.c
endif
...
# TX power setting per band.
ifeq ($(WLTXPWR_PER_BAND),1)
    WLFLAGS += -DWLTXPWR_PER_BAND
endif
...
# ROM compatibility work-around for lbuf struct layout for ROMs that excluded BCMPKTIDMAP.
