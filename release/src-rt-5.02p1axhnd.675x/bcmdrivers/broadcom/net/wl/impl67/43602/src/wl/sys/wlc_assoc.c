 ...
 *
 * $Id: wlc_assoc.c 784961 2020-03-10 06:53:38Z $
 */
...
#include <wlc_hs20.h>
#include <wlc_ht.h> 	/* for WLC_HT_FEATURES_PROPRATES_FORCE definition */
#ifdef WLNDOE
#include <wl_ndoe.h>
#endif // endif
...
#endif // endif
#ifdef WL_SAE
static int wlc_assoc_send_extauth(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
...
#if AS_LAST_STATE != 25 /* don't change this without adding to the table below!!! */
...
/* determine if the current association state can yield to other requests */
#define AS_CAN_YIELD(bss, st)	wlc_assoc_can_yield((bss), (st))

static bool
wlc_assoc_can_yield(bool infra, uint state)
{
	return ((infra && state == AS_WAIT_RCV_BCN) ||
	        state == AS_SYNC_RCV_BCN ||
	        state == AS_RECREATE_WAIT_RCV_BCN);
}
...
wlc_join_start_prep(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	*/
	...
	err = wlc_bsscfg_enable(wlc, cfg);
	if (err != BCME_OK) {
		/* Function "wlc_bsscfg_enable" error handling already included error messages  */
		return err;
	}
	...
	wl_join_assoc_params_t *assoc_params)
{
	...
	if (wlc_join_start_prep(wlc, cfg) != BCME_OK)
		return;
...
/* If bss-transition scoring is not enabled, rssi is used for scoring.
 * Else, bss-transition score is used along with score_delta and rssi is used with roam_delta.
...
wlc_populate_join_pref_score(wlc_bsscfg_t *cfg, bool for_roam, join_pref_t *join_pref)
{
	...
	wlc_bss_info_t **bip = wlc->join_targets->ptrs;
	int i, j;
#if defined(BCMDBG) || defined(WLMSG_ASSOC)
	char eabuf[ETHER_ADDR_STR_LEN];
#endif /* BCMDBG || WLMSG_ASSOC */
...
wlc_cook_join_targets(wlc_bsscfg_t *cfg, bool for_roam, int cur_rssi)
{
...
#ifdef WLWNM
	if (WLWNM_ENAB(cfg->wlc->pub)) {
		wlc_wnm_process_join_trgts_bsstrans(wlc, bip, wlc->join_targets->count);
	}
#endif /* WLWNM */
	...
	if (for_roam &&
...
#endif /* WLWNM */
				cur_pref_score = cur_pref_rssi = 0;
			WL_ASSOC(("wl%d: ROAM: %s: cur_pref_rssi forced to 0x%x for roam_type "
			      "0x%x\n", WLCWLUNIT(wlc), __FUNCTION__, cur_pref_rssi,
			      roam->reason));
		}
#endif /* BCMCCX */
		...
		(int)cur_pref_rssi-255, wlc->band->roam_trigger, roam_delta));

#ifdef WLWNM
		if (WLWNM_ENAB(wlc->pub) && wlc_wnm_bsstrans_is_product_policy(wlc)) {
			/* Zero score for trgts not meeting roam_delta improvement */
			for (i = 0; i < (int)wlc->join_targets->count; i++) {
				WL_WNM_BSSTRANS_LOG("bssid %02x:%02x rssi: cur:%d trgt:%d\n",
					bip[i]->BSSID.octet[4], bip[i]->BSSID.octet[5],
					cur_pref_rssi, join_pref[i].rssi);
				if (join_pref[i].rssi < (cur_pref_rssi + roam_delta)) {
					wlc_zero_pref(&join_pref[i]);
				}
			}
		 ...
		 * already sorted in increasing order of join_pref.score
		 */
		for (i = 0; i < (int)wlc->join_targets->count; i++) {
		 ...
		 */
		for (j = 0; i < (int)wlc->join_targets->count; i++) {
			/* one last check:
		...
		WL_SRSCAN(("  result: %d target(s)", wlc->join_targets_last));
		WL_ASSOC(("  result: %d target(s)\n", wlc->join_targets_last));
	 ...
	 */
	if (CCX_ENAB(wlc->pub) && wlc->join_targets_last > 1) {
		uint32 max, threshold;
		max = join_pref[wlc->join_targets_last - 1].score;
		...
		for (i = 0; i < (int)wlc->join_targets_last; i++) {
		...
		for (j = (int)wlc->join_targets_last; --j >= 0;) {
			int k;
			bool done = TRUE;
			for (k = i; k < j; k++) {
	...
	if (memcmp(cfg->join_bssid.octet, BSSID_INVALID, sizeof(cfg->join_bssid.octet)) != 0) {
		...
		*  connected to.  check for same BSSID and same chanspec.
		*/
		if ((!memcmp(cfg->join_bssid.octet, bip[j]->BSSID.octet, ETHER_ADDR_LEN) ||
			(!memcmp(cfg->join_bssid.octet, BSSID_BROADCAST, ETHER_ADDR_LEN) &&
			!memcmp(cfg->current_bss->BSSID.octet, bip[j]->BSSID.octet,
			ETHER_ADDR_LEN))) && (wf_chspec_ctlchan(cfg->current_bss->chanspec) ==
			wf_chspec_ctlchan(bip[j]->chanspec))) {
			WL_ASSOC(("wl%d.%d:%s(): prune the same AP from candidate lists.\n",
				wlc->pub->unit, cfg->_idx, __FUNCTION__));
			wlc->join_targets_last = 0;
		}
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int channel_num)
{
...
#if defined(BCMCCX) && !defined(WLRCC)
		/* AP-assisted roaming
	...
	if (ASSOC_CACHE_ASSIST_ENAB(wlc) && wlc->scan_results->count > 0) {
		...
		if (WLRCC_ENAB(wlc->pub)) {
			if ((chanspec_list == NULL) && roam->rcc_valid &&
				!roam->roam_fullscan) {
		...
		*/
		...
		err = wlc_scan(wlc->scan, bss_type, bssid, 1, &ssid, scan_type, nprobes,
			...
			FALSE, scan_flag, cfg, SCAN_ENGINE_USAGE_NORM, NULL, NULL);
...
wlc_assoc_done(wlc_bsscfg_t *cfg, int status)
{
	...
	if (type == AS_ASSOCIATION) {
		...
		/* send any deferred link down if appropriate */
		if (cfg->bcn_to_dly_flag &&
		    (status != WLC_E_STATUS_SUCCESS) && cfg->roam->bcns_lost) {
			cfg->bcn_to_dly_flag = FALSE;
			wlc_link(cfg->wlc, FALSE, &cfg->prev_BSSID, cfg, WLC_E_LINK_BCN_LOSS);
		}
...
wlc_assoc_scan_complete(void *arg, int status, wlc_bsscfg_t *cfg)
{
	...
	if ((cfg->WPA_auth & WPA2_AUTH_UNSPECIFIED) &&
		...
		(!BSSCFG_IS_FBT(cfg, wlc->pub) || !(cfg->WPA_auth & WPA2_AUTH_FT)) &&
	...
	if (wlc->join_targets->count > 0) {
		...
		for (i = 0; i < wlc->join_targets->count; i++) {
	...
	if (WLRCC_ENAB(wlc->pub)) {
		if (roam->roam_scan_started) {
			...
			if (wlc->join_targets_last == 0 && (roam->rcc_mode != RCC_MODE_FORCE)) {
				if (roam->fullscan_count == roam->nfullscans ||
					wlc->join_targets->count == 0) {
			...
			if (for_roam && roam_active && (cache_valid == FALSE)) {
				...
				if (roam->rcc_mode != RCC_MODE_FORCE) {
					...
					rcc_update_from_join_targets(roam, wlc->join_targets);
				}
			}
		}
	}
...
wlc_roam_scan(wlc_bsscfg_t *cfg, uint reason, chanspec_t *list, uint32 channum)
{
	 ...
	 */
	if (cfg->roam_scan_params->active_time) {
		wlc_assoc_scan_prep(cfg, cfg->roam_scan_params, NULL, list, channum);
	} else {
	/* original */
...
wlc_join_wsec_filter(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (WSEC_WEP_ENABLED(common_wsec)) {
		if ((cfg->tk_cm_bt == 0) && (cfg->WPA_auth == WPA_AUTH_CCKM) &&
			...
			WL_ASSOC(("wl%d: JOIN: BSSID %s WEP: no WEP support or TKIP cm bt %d\n",
				WLCWLUNIT(wlc), bcm_ether_ntoa(&bi->BSSID, eabuf), cfg->tk_cm_bt));
			wlc_bss_mac_event(wlc, cfg, WLC_E_PRUNE, &bi->BSSID, 0,
				WLC_E_PRUNE_ENCR_MISMATCH,
				CHSPEC_CHANNEL(bi->chanspec), 0, 0);
...
wlc_join_attempt(wlc_bsscfg_t *cfg)
{
	...
	for (; wlc->join_targets_last > 0; wlc->join_targets_last--) {
		...
		if (WLCAUTOWPA(cfg)) {
			...
			/* WPA_auth */
#ifdef BCMCCX
			(void)bcmwpa_akm2WPAauth(join_pref->wpa[bi->wpacfg].akm, &WPA_auth,
				!(bi->flags & WLC_BSS_WPA2));
#else
			(void)bcmwpa_akm2WPAauth(join_pref->wpa[bi->wpacfg].akm, &WPA_auth, FALSE);
#endif // endif
	...
	if (wlc->join_targets_last > 0) {
...
#endif /* BCMCCX && CCX_SDK */
		{
			...
			if (WLWNM_ENAB(wlc->pub)) {
				...
				bssid = &wlc->join_targets->ptrs[wlc->join_targets_last]->BSSID;
				BCM_REFERENCE(bssid);
	...
	wlc_bss_info_t *bi, bool associated)
{
	...
	if (WSEC_AES_ENABLED(wsec_cfg) && (UCAST_AES(&bi->wpa) || UCAST_AES(&bi->wpa2)))
...
#ifdef BCMCCX
	if (CCX_ENAB(wlc->pub) && bi->ccx_version < 4) {
	...
	*/
	if (!(cfg->WPA_auth & WPA2_AUTH_FT))
		allow_reassoc = TRUE;
	else if (BSSCFG_IS_FBT(cfg, wlc->pub) && !ETHER_ISNULLADDR(&cfg->current_bss->BSSID) &&
...
wlc_join_BSS(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (bi == NULL) {
		...
		WL_ASSOC(("wl%d: %s: JOIN: BSS info == NULL, join_targets_last[%d]\n",
			WLCWLUNIT(wlc), __FUNCTION__, wlc->join_targets_last));
		return;
	}
#endif // endif
	...
	if ((bi->flags & WLC_BSS_HT)||(bi->flags2 & WLC_BSS_VHT)) {
	...
	*/
	...
	if (((WLC_BAND_PI_RADIO_CHANSPEC != chanspec) &&
		...
		((as->type != AS_ROAM) ||
		  ...
		  * is not reachable.
		  */
		 (BSSCFG_IS_FBT(cfg, wlc->pub) && (target_bss->flags2 & WLC_BSS_OVERDS_FBT) &&
...
#if defined(WLFBT)
		(BSSCFG_IS_FBT(cfg, wlc->pub) && ft_band_changed) &&
#endif /* WLFBT */
	...
	if (bi->infra == 0) {
		 ...
		 */
		{
			...
			if (bss_vht) {
				...
				if (BAND_2G(target_bss_band) || WLC_VHT_FEATURES_RATES(wlc->pub)) {
					prop_tlv = wlc_read_vht_features_ie(wlc->vhti,
		...
		if (!(scb->state & (PENDING_AUTH | PENDING_ASSOC))) {
			 ...
			 * a fast transition.
			 */
			if (BSSCFG_IS_FBT(cfg, wlc->pub) && (as->type == AS_ROAM) &&
			 ...
			 */
			if (BSSCFG_IS_FBT(cfg, wlc->pub) && (as->type == AS_ROAM) &&
				cfg->associated	&& (auth == DOT11_FAST_BSS) &&
...
#endif /* WLFBT */
#endif /* WLFBT_OVER_DS_DISABLED */
			{
#ifdef WL_SAE
				if (BSSCFG_EXT_AUTH(cfg) && auth == DOT11_SAE) {
					wlc_assoc_send_extauth(wlc, cfg, &bi->BSSID, auth, AS_SENT_AUTH_1);
				} else
#endif /* WL_SAE */
				{
					wlc_assoc_change_state(cfg, AS_SENT_AUTH_1);
...
#ifdef BCMLTECOEX
				/* LTECX: indicate assoc is in progress to ucode */
				if (BCMLTECOEX_ENAB(wlc->pub)) {
					wlc_ltecx_assoc_in_prog(wlc, TRUE);
				}
#endif /* BCMLTECOEX */
...
#endif /* WLP2P && BCMDBG */
			if (pkt != NULL)
				wlc_pcb_fn_register(wlc->pcb, wlc_auth_tx_complete,
				                    (void *)(uintptr)cfg->ID, pkt);
			else
				wl_add_timer(wlc->wl, as->timer, WECA_ASSOC_TIMEOUT + 10, 0);
		} else
			/*
			 * We have pending auth/assoc still on scb, lets timeout now as its not
			 * expected; This might have happened if band got changed in middle of
			 * assoc/auth and scb->state not got updated, while reusing same scb for
			 * next retry.
			 */
			wl_add_timer(wlc->wl, as->timer, 0, 0);
	}
        ...
        struct ether_addr *bssid, int auth_alg, uint state)
{
	...
	if (BSSCFG_AP(cfg)) {
		return BCME_NOTSTA;
	} else if ((auth_alg == DOT11_SAE) && (state == AS_SENT_AUTH_1)) {
...
wlc_assoc_timeout(void *arg)
{
	...
	WL_ASSOC(("wl%d: wlc_timer assoc_timeout\n", wlc->pub->unit));

	if (!wlc->pub->up)
		return;

#ifdef BCMLTECOEX
	/* LTECX: indicate assoc completed to ucode */
	if (BCMLTECOEX_ENAB(wlc->pub)) {
		wlc_ltecx_assoc_in_prog(wlc, FALSE);
	}
#endif /* BCMLTECOEX */
...
wlc_assoc_recreate_timeout(wlc_bsscfg_t *cfg)
{
	...
	if ((err = wlc_mac_request_entry(wlc, cfg, WLC_ACTION_RECREATE_ROAM)) != BCME_OK) {
		WL_ERROR(("wl%d.%d: RECREATE request failed, err %d\n",
	...
	if (bsscfg->BSS && scb != NULL) {
		...
		if (scb->WPA_auth != bsscfg->WPA_auth)
			WL_ERROR(("wl%d: %s: scb->WPA_auth 0x%x "
				...
				WLCWLUNIT(wlc), __FUNCTION__, scb->WPA_auth,
				bsscfg->WPA_auth));
...
wlc_sta_assoc_upd(wlc_bsscfg_t *cfg, bool state)
{
	...
	if (COEX_ENAB(wlc->pub) && !cfg->associated) {
		/* reinit OBSS parameters which might have been changed by AP */
		wlc_ht_obss_scanparam_init(&cfg->obss->params);
	}
...
#ifdef WLMCHAN
	/* Create mchan chan_context if MCHAN is enabled for the bss */
	if (BSS_MCHAN_ENAB(wlc, cfg)) {
		if (state == FALSE) {
			wlc_mchan_delete_bss_chan_context(wlc, cfg);
		}
...
#if defined(WLFBT)
	if (BSSCFG_IS_FBT(cfg, wlc->pub) && (bcmwpa_is_rsn_auth(cfg->WPA_auth)) &&
	...
	struct dot11_bcn_prb *bcn, int bcn_len)
{
	...
	if (cfg->BSS) {
		 ...
		 * associated.
		 */
		if (!wlc_sendnulldata(wlc, cfg, &cfg->current_bss->BSSID, 0, 0, PRIO_8021D_BE))
...
#endif // endif
#if defined(WLNDOE) && !defined(NO_WLNDOE_RA_SUPPORT)
	/* clear the cache once we join new AP */
	if (NDOE_ENAB(wlc->pub))
		wlc_iovar_op(wlc, "nd_ra_filter_cache_clear", NULL, 0,
		   NULL, 0,
		   IOV_SET, cfg->wlcif);
#endif // endif
}
	...
	struct dot11_management_header *hdr, uint8 *body, uint body_len)
{
	...
	WL_SRSCAN(("rcvd assoc: status %d: bssid %02x:%02x",
		ltoh16(assoc->status), hdr->bssid.octet[4], hdr->bssid.octet[5]));
	WL_ASSOC(("rcvd assoc: status %d: bssid %02x:%02x\n",
		ltoh16(assoc->status), hdr->bssid.octet[4], hdr->bssid.octet[5]));
	...
	uint8 *body, uint body_len, bool short_preamble)
{
	...
	if (cfg->auth == DOT11_SAE && auth_alg == DOT11_SAE) {

		if (auth_status == DOT11_SC_SUCCESS) {
			if (auth_seq == WL_SAE_COMMIT)
				wlc_assoc_change_state(cfg, AS_SENT_AUTH_2);
			else if (auth_seq == WL_SAE_CONFIRM)
				wlc_auth_complete(cfg, status, &hdr->sa, auth_status, auth_alg);
		}
		wlc_assoc_timer_del(wlc, cfg);
		wl_add_timer(wlc->wl, cfg->assoc->timer, WECA_AUTH_TIMEOUT + 10, 0);
...
wlc_join_start_ibss(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	/* save the default_bss configuration */
...
#endif // endif
	err = wlc_join_start_prep(wlc, cfg);
	if (err != BCME_OK)
		return err;
	...
	if (!wlc->pub->up) {
		WL_ERROR(("wl%d: %s: unable to start IBSS while driver is down\n",
		          wlc->pub->unit, __FUNCTION__));
		/* restore the saved default_bss configuration */
...
#ifdef WLDPT
	/* cleanup the DPT peers which require an association */
	if (DPT_ENAB(wlc->pub))
		wlc_dpt_cleanup(wlc->dpt, cfg);
#endif /* WLDPT */
...
wlc_assoc_success(wlc_bsscfg_t *cfg, struct scb *scb)
{
	...
	if (cfg->BSS && cfg->associated) {
		chanspec_t prev_chspec = cfg->current_bss->chanspec;

		prev_scb = wlc_scbfindband(wlc, cfg, &cfg->prev_BSSID,
			CHSPEC_WLCBANDUNIT(prev_chspec));
		if (prev_scb) {
			 ...
			 */
			if ((WL11H_ENAB(wlc) &&
				wlc_radar_chanspec(wlc->cmi, prev_chspec)) ||
				wlc_restricted_chanspec(wlc->cmi, prev_chspec)) {
				wlc_set_quiet_chanspec(wlc->cmi, prev_chspec);
			}
	...
	if (WLC_MFP_ENAB(wlc->pub)) {
		...
		if ((cfg->wsec & MFP_CAPABLE) && (cfg->target_bss->wpa2.flags & RSN_FLAGS_MFPC))
			scb->flags2 |= SCB2_MFP;
	...
	if (WL11K_ENAB(wlc->pub) &&
		!(BSSCFG_IS_FBT(cfg, wlc->pub) && (cfg->auth_atmptd == DOT11_FAST_BSS))) {
...
wlc_assoc_continue(wlc_bsscfg_t *cfg, struct ether_addr* addr)
{
	...
	do {
		...
		/* For now, only support the continuance from a sent auth state
		* Can add more later, but right now, only this is supported
		*/
		if (cfg->assoc->state != AS_SENT_AUTH_1) {
			WL_ASSOC(("%s: trying to continue assoc; wrong state %d, not %d\n"));
			bcmerr = BCME_NOTREADY;
			break;
		}
	...
	uint auth_status, uint auth_type)
{
	...
	if (status == WLC_E_STATUS_SUCCESS) {
		 ...
		 */
		if (BSSCFG_IS_FBT(cfg, wlc->pub) && (target_bss->flags2 & WLC_BSS_OVERDS_FBT) &&
		    (as->state == AS_SENT_FTREQ)) {
			wlc_join_BSS(cfg, wlc->join_targets->ptrs[wlc->join_targets_last]);
			return;
		}
...
#ifdef WLP2P
		WL_ERROR(("wl%d: JOIN: authentication failure, no ack\n",
			WLCWLUNIT(wlc)));
		if (BSS_P2P_ENAB(wlc, cfg)) {
			if (as->auth_tx_retry++ < 4) {
				wlc_bss_info_t* bi =
					wlc->join_targets->ptrs[wlc->join_targets_last];

				/* XXX WAR: currently auth frame is not sent at GO's presence period
				 * So now try to send auth frame again to sync the time
				 */
				OSL_DELAY(7*1000);
				wlc_assoc_change_state(cfg, AS_SENT_AUTH_1);
				pkt = wlc_sendauth(cfg, &scb->ea, &bi->BSSID, scb,
					cfg->auth, 1, DOT11_SC_SUCCESS, NULL, NULL,
					((bi->capability & DOT11_CAP_SHORT) != 0));
...
do_event:
	if (!cfg->roam->active || status != WLC_E_STATUS_SUCCESS || !BSSCFG_ROAMOFFL(cfg, addr))
		wlc_bss_mac_event(wlc, cfg, WLC_E_AUTH, addr, status, auth_status, auth_type, 0, 0);
	else
		WL_ASSOC(("wl%d: ROAM offload: discard WLC_E_AUTH\n", WLCWLUNIT(wlc)));
	...
	uint assoc_status, bool reassoc, uint bss_type)
{
	...
	if (status == WLC_E_STATUS_SUCCESS) {
...
#ifdef WL_HOST_CLK_SYNC
		if (HOSTCLKSYNC_ENAB(wlc->pub))
			wlc_host_clk_sync_reset(wlc->hostclksync_info);
#endif // endif
		if (!reassoc) {
			wlc_bsscfg_up(wlc, cfg);
			if (addr) {
				bcopy(addr, &cfg->last_upd_bssid, ETHER_ADDR_LEN);
			}
		} else {
			cfg->roamed = TRUE;
			if (WLRCC_ENAB(wlc->pub)) {
		...
		if (BSSCFG_IS_FBT(cfg, wlc->pub) && SUP_ENAB(wlc->pub) &&
			(cfg->WPA_auth & WPA2_AUTH_FT) && reassoc)
...
wlc_set_ssid_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
	...
	if (status != WLC_E_STATUS_SUCCESS) {
		...
		*/
		...
		} else if (cfg->enable && (assoc_params != NULL) && assoc_params->bssid_cnt) {
			int chidx = assoc_params->chanspec_num;
			int bccnt = assoc_params->bssid_cnt;
...
wlc_roam_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
	...
	if (WLWNM_ENAB(wlc->pub)) {
		...
		*/
		ret = wlc_wnm_bsstrans_roamscan_complete(wlc,
			DOT11_BSSTRANS_RESP_STATUS_REJECT, NULL);
	}
#endif /* WLWNM */
#ifdef EXT_STA
	if (WLEXTSTA_ENAB(wlc->pub)) {
...
#ifndef WLRCC
			/* As WLRCC covers CCX fast roam,
			 * no need to try another roam scan
			 */
...
do_event:
	...
	/* N.B.: bss_type passed through auth_type event field */
	if (status == WLC_E_STATUS_SUCCESS && !BSSCFG_ROAMOFFL(cfg, addr)) {
#ifdef WLFBT
...
#ifdef BCMCCX
	if (CCX_ENAB(wlc->pub) && wlc_ccx_is_droam(wlc->ccx))
		return;
#endif // endif
	/* Check for motion */
	if (!roam->motion &&
...
#ifdef BCMSUP_PSK
#define AS_INITIAL_KEYING(wlc, cfg, req) wlc_assoc_is_keying(wlc, cfg, req)
static bool
wlc_assoc_is_keying(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req)
{
	return FALSE;
}
#else /* BCMSUP_PSK */
#define AS_INITIAL_KEYING(wlc, cfg, req) FALSE
#endif /* BCMSUP_PSK */
...
wlc_mac_request_roam(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req)
{
	...
	if (AS_IN_PROGRESS(wlc) || AS_INITIAL_KEYING(wlc, cfg, req)) {
...
wlc_mac_request_scan(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req)
{
	...
	if (req == WLC_ACTION_SCAN || req == WLC_ACTION_ISCAN || req == WLC_ACTION_ESCAN) {
		...
		if (!WLC_APSTA_ON_RADAR_CHANNEL(wlc) && AP_ACTIVE(wlc) && WL11H_AP_ENAB(wlc) &&
			...
			WL_ERROR(("wl%d: %s: On radar channel (not ETSI), WLC_SCAN ignored\n",
				wlc->pub->unit,	__FUNCTION__));
			err = BCME_SCANREJECT;
			return err;
		}
	...
	else if (AS_IN_PROGRESS(wlc) || AS_INITIAL_KEYING(wlc, cfg, req)) {
		wlc_bsscfg_t *bc = wlc->assoc_req[0];
		wlc_assoc_t *as;
...
bool wlc_roam_scan_islazy(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool roam_scan_isactive)
{
	...
	/* More checks */
	if ((cfg->pm->PM != PM_FAST) || !cfg->associated || !wlc->band->roam_prof)
		return FALSE;

	return (wlc->band->roam_prof[cfg->roam->roam_prof_idx].roam_flags & WL_ROAM_PROF_LAZY);
}
...
bool wlc_lazy_roam_scan_suspend(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	return (wlc->band->roam_prof[cfg->roam->roam_prof_idx].roam_flags & WL_ROAM_PROF_SUSPEND);
}
...
{
	ASSERT(wlc_roam_scan_islazy(wlc, cfg, FALSE));
	return (wlc->band->roam_prof[cfg->roam->roam_prof_idx].roam_flags & WL_ROAM_PROF_SYNC_DTIM);
}
...
#ifdef WL_SAE
int
wlc_mgmt_tx_complete(wlc_info_t *wlc, wlc_bsscfg_t *cfg, void *pkt,
		void *arg)
{
