 ...
 *
 * $Id: nicpci.c 531106 2015-02-02 08:44:00Z $
 */
...
pcie_war_pmebits(pcicore_info_t *pi)
{
	...
	if (!PCIE_GEN2(pi->sih) &&
	    (pi->sih->buscorerev != 18 && pi->sih->buscorerev != 19))
		return;
...
pciedev_prep_D3(void *pch, bool enter_D3)
{
	...
	/* corerev6 = 4350Bx, corerev7 = 4350C0, corerev11 = 4350C1  */
	if ((sih->buscorerev == 7) || (sih->buscorerev == 11)) {
		/* XXX NIC mode (offloads), Survive PERST should not be
		 * enabled.  4350C2 have it enable by default.
		 */
		if (sih->buscorerev == 11)
			AND_REG(pi->osh, &pcieregs->control, ~PCIE_SPERST);
		if (enter_D3) {
...
static void
pcie_war_upd(void *pch, bool enter_D3)
{
	pcicore_info_t *pi = (pcicore_info_t *)pch;
...
BCMATTACHFN(pcicore_attach)(void *pch, char *pvars, int state)
{
	...
	if (!PCIE_GEN1(sih)) {
		if ((BCM4360_CHIP_ID == CHIPID(sih->chip)) ||
			...
			(BCM43455_CHIP_ID == CHIPID(sih->chip)) ||
#endif // endif
			0)
			pi->pcie_reqsize = PCIE_CAP_DEVCTRL_MRRS_1024B;
...
pcicore_pmeen(void *pch)
{
	...
	/* don't enable PME from driver, let the OS take care of it */
	/* corerev7 = 4350C0, corerev9 = 43602, corerev11 = 4350C1 */
	if (PCIE_GEN2(pi->sih)) {
		if (pi->sih->buscorerev == 7 ||
		    pi->sih->buscorerev == 9 ||
		    pi->sih->buscorerev == 11)
			return;
	}
