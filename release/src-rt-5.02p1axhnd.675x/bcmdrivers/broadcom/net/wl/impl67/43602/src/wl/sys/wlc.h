 ...
 *
 * $Id: wlc.h 781623 2019-11-26 09:22:14Z $
 */
...
#define SYNCFIFO						(0)
#define FLUSHFIFO						(1)
#define SYNCAWDLFIFO						(2)
#define FLUSHFIFO_FLUSHID					(3)
...
#define ACMAJORREV2_THROUGHPUT_OPT_ENAB(wlc) \
	(wlc->pub->associated && wlc_phy_get_2069_xtalldowar_status(wlc->band->pi))
#else
#define ACMAJORREV2_THROUGHPUT_OPT_ENAB(wlc) FALSE
#endif /* ACMAJORREV2_THROUGHPUT_OPT */

typedef struct wlc_per_ldo_info {
	uint8 cur_idx; /* = 0 : default, = 1 : alternative ldo setting */
	uint32 psr;
	uint32 nholes;
	uint32 nrxfrm;
	uint32 rxper;
	int count;
	uint32 ldo_time_out;
	uint32 glacial_monitor;
	uint32 long_ldo_time_out;
} wlc_per_ldo_t;
...
#define WLC_HT_PROP_RATES_CAP_PHY(wlc) \
	((wlc_phy_cap_get((wlc)->band->pi) & PHY_CAP_HT_PROP_RATES) != 0)
 ...
 *
 * XXX Create a new struct (instead of adding to modulecb_t) to avoid ROM invalidation for functions
 *     that iterate the 'modulecb' array (and therefore depend on the sizeof the 'modulecb_t'
 *     struct).
 */
...
typedef struct {
...
#endif // endif
#if defined(WLC_CHANIM_STATS_ROM_COMPAT) || defined(WLC_CHANIM_STATS_ROM_COMPAT2)
	uint8 accum_cnt_obsolete[120]; /* Unused. Keep for ROM compatibility. */
#endif // endif
	...
	wlc_info_t *wlc;
	chanim_interfaces_t *ifs; /* interface details and its corresponding stats */
...
/* defined for forcePMenabled requestor */
/* The value of forcePMenabled is to identify the module who did force bsscfg to
 * doze state (i.e., PMenabled FALSE -> TRUE).
 * Only the module who did force bsscfg to doze state should be allowed to
 * bring bsscfg to wake up state (i.e., PMenabled TRUE -> FALSE).
*/
#define WLC_PM_FORCE_PMENABLED_SCAN	1	/* to indicate forcePMenabled was by scan */
...
#if defined(PKTHIST_DBG) && defined(PROP_TXSTATUS)
#define MAX_PKTS_RECORDED 50
/* For recording per PKT stats */
...
typedef struct pkt_hist {
	pkt_stats_t pkt_stats_hist[MAX_PKTS_RECORDED];	/* Store stats of last n PKTs */
	uint8 pkt_stat_cnt;			/* Count of no. of PKTs fo which stats are stored */
} pkt_hist_t;
#endif /* PKTHIST_DBG && PROP_TXSTATUS */
...
#endif // endif
#if defined(WLTAF) && !defined(WLTAF_ROM_COMPAT)
...
#if defined(WET) || defined(WET_ROM_COMPAT)
	void		*weth;			/* pointer to wet specific information */
#endif /* WET || WET_ROM_COMPAT */
#if defined(WET_TUNNEL) || defined(WET_TUNNEL_ROM_COMPAT)
	void		*wetth;			/* pointer to wet tunnel specific information */
#endif /* WET_TUNNEL || WET_TUNNEL_ROM_COMPAT */
#ifdef BCMCCX
...
#define WLC_INFO_WL_OKC_INFO_FIELD \
...
#if defined(CCA_STATS_IN_ROM)
	cca_info_t *cca_info;

#if defined(ISID_STATS_IN_ROM)
	itfr_info_t *itfr_info;
#endif /* ISID_STATS_IN_ROM */

	cca_chan_qual_t *cca_chan_qual;
#endif /* CCA_STATS_IN_ROM  */
	...
	void    *hostclksync_info;  /* pointer to TSF sync info */
	wlc_modesw_info_t *modesw; /* modesw Wlc structure pointer */

#if defined(WLC_INFO_RELOC_WLCHANIM) && defined(WLCHANIM)
	chanim_info_t *chanim_info;
#endif	/* WLC_INFO_RELOC_WLCHANIM && WLCHANIM */

/* Leave debug members at the end of struct */

#if defined(PKTHIST_DBG) && defined(PROP_TXSTATUS)
	pkt_hist_t *pkt_hist_enq_txfifo;	/* history of PKTs enq'd to TXFIFO */
	pkt_hist_t *pkt_hist_sup_txfifo;	/* history of PKTs suppressed from TXFIFO */
#endif /* PKTHIST_DBG && PROP_TXSTATUS */
...
#endif // endif
	wlc_nan_info_t	*nan;		/*  bcm NAN innstance */
	bool urgent_tx_mode;		/* Flag to determine if urgent TX mode is enabled */
	uint32  last_lpas_phy_wd_ms; /* Last time when phy_wd was invoked in LPAS mode */
	bool	block_datafifo_D3;	/* prohibit host frames to go out */
	wlc_shub_info_t *shub_info;
	...
	int8		*p_txpwr_qdbm_2g;	/* txpwr value forced using iovar */
	int8		*p_txpwr_qdbm_5g;
#else
	int8		txpwr_qdbm_2g;	/* txpwr value forced using iovar */
	int8		txpwr_qdbm_5g;
#endif /* WLTXPWR_PER_CORE */
	bool		txpwr_2g_over_flag; /* Flag for checking if txpwr override is used */
	bool		txpwr_5g_over_flag;
#ifdef WL_ASSOC_MGR
	wlc_assoc_mgr_info_t	*assoc_mgr; /* for fine grained handling of association */
#endif /* WL_ASSOC_MGR */
	uint32	fifoflush_id;
	...
	wlc_scb_alloc_info_t *scb_alloc_info;
	...
	cca_chan_qual_t *cca_chan_qual;
#endif /* CCA_STATS && !CCA_STATS_IN_ROM  */
	uint8 rand_keydata[D11_MAX_KEY_SIZE];
};
...
#ifdef MBSS
extern void wlc_tx_fifo_sync_bcmc_pkt(wlc_info_t *wlc, void *pkt);
#endif /* MBSS */
#endif /* WL_MULTIQUEUE */
...
extern void wlc_ht_upd_txbf_cap(wlc_bsscfg_t *cfg, bool bfr, bool bfe, uint32 *cap);
...
extern int wlc_prep_sdu_fast(wlc_info_t *wlc, wlc_if_t *wlcif,
	wlc_bsscfg_t *bsscfg, struct scb *scb, uint32 scb_flags, wsec_key_t *key,
	void *sdu, uint pktlen, uint *fifop);
...
/* Enqueue at tail, with no overflow evict lower prec, PKTQ_LOG etc. */
extern void wlc_prec_enq_fast(wlc_info_t *wlc, struct pktq *q, void *pkt, int prec);
#endif // endif
...
extern void BCMFASTPATH wlc_recvdata_sendpkt(wlc_info_t *wlc, void *p, wlc_if_t *wlcif);
...
extern void wlc_bcn_ts_tsf_calc(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh, void *plcp, uint32 *tsf_h,
	uint32 *tsf_l);
...
extern void wlc_frameburst_limit_ext(wlc_info_t *wlc);
...
#endif /* EXT_STA && !EXT_STA_DISABLED */
...
wlc_get_llc_snap_header(uchar *pbody, uint32 wsec, bool protected)
{
	if (protected) {
		if (WSEC_WEP_ENABLED(wsec))
			...
			return (struct dot11_llc_snap_header*)(pbody + SMS4_WPI_CBC_MAC_LEN);
#endif // endif
		else
...
#define EAPOL_PKTFETCH_REQUIRED(lsh) \
	(lsh->type == hton16(ETHER_TYPE_802_1X) && \
...
#define NDOE_PKTFETCH_REQUIRED(wlc, lsh, f) \
	(lsh->type == hton16(ETHER_TYPE_IPV6) && \
	LLC_SNAP_HEADER_CHECK(lsh) && \
	f->body_len >= (((uint8 *)lsh - (uint8 *)f->pbody) + ICMP6_MIN_BODYLEN) && \
...
#define SW_TKIP_PKTFETCH_REQUIRED(scb, rxh, h) \
	((scb->key) && ((scb->key)->algo == CRYPTO_ALGO_TKIP) && \
...
#ifdef BCMCCX
#define CCX_IAPP_PKTFETCH_REQUIRED(wlc, bsscfg, f, lsh)	\
...
extern bool wlc_sw_decrypt_pktfetch_required(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
...
extern void wlc_recvdata_pktfetch_cb(void *lbuf, void *orig_lfrag, void *ctx, bool cancelled);
...
extern uint32 wlc_link_txflow_scb(wlc_info_t *wlc, struct wlc_if *wlcif, uint16 flowid, uint8 op,
...
#ifdef WLAMSDU
extern int wlc_set_rx_amsdu_in_ampdu(wlc_info_t *wlc, bool on);
#endif /* WLAMSDU */

#ifdef ACMAJORREV2_THROUGHPUT_OPT
void wlc_temp_evm_war_init(wlc_info_t *wlc);
void wlc_temp_evm_war(wlc_info_t *wlc, wlc_txh_info_t* txh_info,
	tx_status_t *txs, uint supr_status);
void wlc_ldo_war_rx_status(wlc_info_t *wlc, ratespec_t rspec, bool is_amsdu,
	uint8 isretry, uint16 aggtype, uint8 plcp_valid);
...
extern uint8 wlc_template_plcp_offset_ext(wlc_info_t *wlc, ratespec_t rspec);
