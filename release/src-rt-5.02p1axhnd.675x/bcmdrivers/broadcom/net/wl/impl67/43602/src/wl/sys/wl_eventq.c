 ...
 *
 * $Id: wl_eventq.c 335384 2012-05-26 00:09:37Z $
 *
 */
...
wl_eventq_process_event(void *arg)
{
	...
	if (wlevtq->head) {
		...
		if (wlevtq->head) {
			...
			/* XXX There should be a del here, but it is missing in the ROM version,
			 * and this doesn't seem to cause a problem in hndrte.
			 */
			wl_del_timer(wlevtq->wlc->wl, wlevtq->timer);
