	...
	int len, int val_size, struct wlc_if *wlcif)
{
			...
			if (SCB_ASSOCIATED(scb)) {
				if (flag != MFP_TEST_FLAG_NORMAL) {
					if (flag == MFP_TEST_FLAG_ANY_KEY) {
						...
						memcpy((char *)&key_param.ea, (char *)&scb->ea,
							sizeof(struct ether_addr));
						wlc_key_set_error(wlc, bsscfg,
							WSEC_GEN_MFP_ACT_ERROR, &key_param);
					}
	...
	case IOV_SVAL(IOV_MFP_TEST_BIP):
		...
		break;
	case IOV_SVAL(IOV_MFP_TEST_SHA256):
		if ((bsscfg->wsec & MFP_REQUIRED) && !flag) {
			WL_WSEC(("wl%d: %s: MFP required mandates SHA256\n",
				WLCWLUNIT(wlc), __FUNCTION__));
			err = BCME_BADARG;
			break;
		}
		if (flag)
			...
			bsscfg->wsec &= ~MFP_SHA256;
		break;
	case IOV_GVAL(IOV_MFP_TEST_SHA256):
		*ret_int_ptr = (int)bsscfg->wsec & MFP_SHA256 ? TRUE : FALSE;
		break;
	default:
		err = BCME_UNSUPPORTED;
		break;
	...
	struct scb *scb, uint16 fc, uint16 reason_code, int flag)
{
	...
	if (BSSCFG_AP(bsscfg) && ETHER_ISMULTI(da)) {
		/* only include the length for reason code here. The mmic_ie len */
		/* would be accomodated inside wlc_mfp_frame_get_mgmt - tail_len */
		uint body_len = 2;
		if (flag)
		...
		if (flag == 1) {
			wl_wsec_key_t key_param;
			memcpy(&key_param.ea, da, sizeof(struct ether_addr));
			wlc_key_set_error(wlc, bsscfg,
				fc == FC_DISASSOC?
	...
	if (flag && BSSCFG_AP(bsscfg) && ETHER_ISMULTI(da)) {
		/* mmic_ie mic would be populated based on the tail_len */
		wlc_mfp_tx_mcast(wlc->mfp, p, bsscfg, pbody, 2);
		if (flag == 1) {
			/* send frame with bad mic by flipping first byte */
			mmic_ie_t *ie;
			ie = (mmic_ie_t *) (pbody + 2);
			ie->mic[0] = ~ie->mic[0];
		}
	}
...
mfp_bip_test(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int flag)
{
	...
	/* check mic for frame */
	mfp_test_bip_mic(hdr, (void *)(frame + sizeof(*hdr)), 20, igtk, mic);
