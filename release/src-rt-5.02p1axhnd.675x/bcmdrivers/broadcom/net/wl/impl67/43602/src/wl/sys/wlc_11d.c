 ...
 *
 * $Id: wlc_11d.c 474117 2014-04-30 06:57:13Z $
 */
...
struct wlc_11d_info {
	...
	/* compile-time debug fields need to be at the bottom of structure to avoid ROM conflicts */
                         ...
                         uint8 *bcn_prb, uint bcn_prb_len)
{
	...
	if (bi->infra == 0 || !bcn_prb || (bcn_prb_len <= DOT11_BCN_PRB_LEN))
		return;
...
wlc_11d_scan_complete(wlc_11d_info_t *m11d, int status)
{
	...
	wlc_set_countrycode(wlc->cmi, m11d->best_abbrev);
	m11d->awaiting_cntry_info = FALSE;
	wlc_11d_set_autocountry_scan_learned(wlc->m11d, m11d->best_abbrev);
