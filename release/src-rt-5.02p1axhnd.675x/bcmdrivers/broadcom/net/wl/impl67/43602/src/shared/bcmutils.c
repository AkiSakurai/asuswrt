 ...
 * $Id: bcmutils.c 781623 2019-11-26 09:22:14Z $
 */
...
lfbufpool_swap_d11_buf(osl_t *osh, void *p)
{
	...
	lfbuf = (lfrag_buf_t*)((uchar*)d3buf_head - LFBUFSZ);
	if (lfbuf->lfbufp != d3_lfrag_buf_pool) {
		BCM_ERROR(("CAN'T SWAP FROM D3 BUFFER, p:%p, lfbufpool: %p, d3buf_pool:%p,"
			" d11bufpool:%p\n",
			p, lfbuf->lfbufp, d3_lfrag_buf_pool, d11_lfrag_buf_pool));
		ASSERT(0);
	}
...
pktpool_emptycb_disable(pktpool_t *pktp, bool disable)
{
	...
	if ((!disable) && (pktp->cbcnt)) {
		if ((pktp->empty == FALSE) && (pktp->emptycb_disable == EMPTYCB_SKIPPED)) {
			pktpool_avail_notify(pktp);
		}
	}
	pktp->emptycb_disable = disable ? 1 : 0;
}
 ...
 */
uint32
bcm_count_zeros_sequence(const uint32 *bmap,
	const uint32 pos, const uint32 numbits, const uint32 maxbits)
{
	...
	if (numbits == 1) { /* test a single bit */
		count = ((bmap[s32] & (1 << (pos % NUM_BITS_U32))) == 0);
		return count;
	}
	...
	if ((e32 == s32) && (numbits <= NUM_BITS_U32)) {
		...
		count = _bcm_count_zeros_sequence_u32(bmap[s32],
			pos % NUM_BITS_U32, numbits);
	} else { /* pos and epos span multiple uint32 */
		count = _bcm_count_zeros_sequence_m32(bmap, s32, e32,
