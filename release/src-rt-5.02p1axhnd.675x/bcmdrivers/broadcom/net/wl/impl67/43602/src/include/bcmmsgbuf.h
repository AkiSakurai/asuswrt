...
typedef struct bcmpcie_soft_doorbell {
	...
	addr64_t haddr; /* address, eg thread wakeup register address */
...
typedef struct ioctl_req_msg {
	...
	/* as per new spec reqs, all structs need to have same length */
	uint32		rsvd1[2];
} ioctl_req_msg_t;
...
typedef struct ioctl_resp_evt_buf_post_msg {
	...
	/* all structs of specific ring type need to have same length. */
	uint32		rsvd[4];
...
/* Olympic is defining the header format for this information
 * for now make this a fixed 4 bytes
*/
