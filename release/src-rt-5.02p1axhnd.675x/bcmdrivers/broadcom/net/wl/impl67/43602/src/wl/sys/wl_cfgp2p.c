 ...
 *
 * $Id: wl_cfgp2p.c 465194 2014-03-26 23:00:33Z $
 *
 */
#include <typedefs.h>
#include <linuxver.h>
...
#ifdef CUSTOMER_HW4
u32 wl_cfgp2p_dbg_level = (WL_CFGP2P_DBG_ERROR | WL_CFGP2P_DBG_ACTION);
#else
#ifdef BCMDEBUG
u32 wl_cfgp2p_dbg_level = (WL_CFGP2P_DBG_ERROR | WL_CFGP2P_DBG_INFO |
	WL_CFGP2P_DBG_TRACE | WL_CFGP2P_DBG_ACTION | WL_CFGP2P_DBG_DBG);
#else
u32 wl_cfgp2p_dbg_level = (WL_CFGP2P_DBG_ERROR | WL_CFGP2P_DBG_INFO);
...
bool wl_cfgp2p_is_pub_action(void *frame, u32 frame_len)
{
	...
	CFGP2P_TRACE(("Started... frame = 0x%lx, frame_len = %d \n",
		(unsigned long)frame, frame_len));
...
bool wl_cfgp2p_find_gas_subtype(u8 subtype, u8* data, u32 len)
{
	...
	CFGP2P_TRACE(("Started... subtype = %d, len = %d \n", subtype, len));
...
void wl_cfgp2p_print_actframe(bool tx, void *frame, u32 frame_len, u32 channel)
{
	...
	CFGP2P_TRACE(("Started... tx = %d, frame_len = %d, channel = %d \n",
		tx, frame_len, channel));
...
{
	CFGP2P_TRACE(("Started... cfg = 0x%lx \n", (unsigned long)cfg));
...
wl_cfgp2p_set_firm_p2p(struct bcm_cfg80211 *cfg)
{
	...
	if (val == 0) {
		val = 1;
		CFGP2P_DBG(("Call 'wldev_ioctl()' for WLC_DOWN \n"));
		...
		CFGP2P_DBG(("Call 'wldev_ioctl()' for WLC_UP \n"));
            ...
            chanspec_t chspec)
{
	...
	CFGP2P_TRACE(("Started... cfg = 0x%lx, if_type = %d \n", (unsigned long)cfg, if_type));
	...
	if (unlikely(err < 0))
		...
		CFGP2P_DBG(("Call 'wldev_ioctl()' for WLC_SET_SCB_TIMEOUT \n"));
...
wl_cfgp2p_set_discovery(struct bcm_cfg80211 *cfg, s32 on)
{
	...
	CFGP2P_TRACE(("Started... cfg = 0x%lx, on = %d \n", (unsigned long)cfg, on));
...
wl_cfgp2p_set_p2p_mode(struct bcm_cfg80211 *cfg, u8 mode, u32 channel, u16 listen_ms, int bssidx)
{
	...
	CFGP2P_TRACE(("Started... cfg=0x%lx, mode=%d, channel=%d, listen_ms=%d, bssidx=%d \n",
		(unsigned long)cfg, mode, channel, listen_ms, bssidx));
...
set_ie:
	if (ie_len) {
		ret = wl_cfgp2p_set_management_ie(cfg, dev, bssidx,
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_p2p_status(cfg, LISTEN_EXPIRED) == 0) {
		...
		if (wl_get_drv_status_all(cfg, REMAINING_ON_CHANNEL) ||
			...
			CFGP2P_DBG(("Listen DONE for ramain on channel expired\n"));
...
wl_cfgp2p_listen_expired(unsigned long data)
{
	...
	CFGP2P_TRACE(("Started... data = 0x%lx \n", data));
	...
	wl_af_params_t *af_params, s32 bssidx)
{
	...
	if ((ret = wl_cfg80211_apply_eventbuffer(bcmcfg_to_prmry_ndev(cfg), cfg, &buf)) < 0)
		return ret;
...
wl_cfgp2p_bss_isup(struct net_device *ndev, int bsscfg_idx)
{
	...
	*(int*)getbuf = -1;
	...
	if (result != 0) {
		CFGP2P_ERR(("wldev_iovar_getbuf_bsscfg() failed. bsscfg_idx = %d, result = %d \n",
			bsscfg_idx, result));
...
wl_cfgp2p_set_p2p_ps(struct bcm_cfg80211 *cfg, struct net_device *ndev, char* buf, int len)
{
	...
	if (cfg->p2p && cfg->p2p->vif_created) {
		...
		if ((legacy_ps != -1) && ((legacy_ps == PM_MAX) || (legacy_ps == PM_OFF))) {
			CFGP2P_DBG(("Call 'wldev_ioctl()' for WLC_SET_PM \n"));
			ret = wldev_ioctl(dev, WLC_SET_PM, &legacy_ps, sizeof(legacy_ps), true);
			if (unlikely(ret))
				CFGP2P_ERR(("error (%d)\n", ret));
...
static int wl_cfgp2p_do_ioctl(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	CFGP2P_TRACE(("Started... net = 0x%lx, cmd = %d \n", (unsigned long)net, cmd));
	...
	if (cmd == SIOCDEVPRIVATE+1) {
		...
		ret = wl_cfgp2p_priv_ioctl(ndev, ifr, cmd);
#else
		(void)ndev;
#endif // endif
...
static int wl_cfgp2p_if_open(struct net_device *net)
{
	...
	CFGP2P_TRACE(("Started... net = 0x%lx \n", (unsigned long)net));
...
typedef struct wl_cfgp2p_priv_cmd {
	char *buf;
	int used_len;
	int total_len;
} wl_cfgp2p_priv_cmd;

int wl_cfgp2p_priv_ioctl(struct net_device *net, struct ifreq *ifr, int cmd)
{
	int ret = 0;
	char *command = NULL;
	int bytes_written = 0;
	wl_cfgp2p_priv_cmd priv_cmd;
	struct wireless_dev *wdev;
	struct bcm_cfg80211 *cfg;

	CFGP2P_TRACE(("Started... cmd = %d \n", cmd));
	...
	if (copy_from_user(&priv_cmd, ifr->ifr_data, sizeof(wl_cfgp2p_priv_cmd))) {
		ret = -EFAULT;
		goto exit;
	}
	...
	if (!command)
	{
		CFGP2P_ERR(("%s: failed to allocate memory\n", __FUNCTION__));
		ret = -ENOMEM;
		goto exit;
	}
	...
	CFGP2P_INFO(("%s: Android private cmd \"%s\" on %s\n",
		__FUNCTION__, command, ifr->ifr_name));

	if (strnicmp(command, CMD_START, strlen(CMD_START)) == 0) {
		CFGP2P_INFO(("%s, Received regular START command\n", __FUNCTION__));
		/* bytes_written = wl_android_wifi_on(net); */
		CFGP2P_ERR(("CMD_START\n"));

	}
	else if (strnicmp(command, CMD_SETFWPATH, strlen(CMD_SETFWPATH)) == 0) {
		/* bytes_written = wl_android_set_fwpath(net, command, priv_cmd.total_len); */
		CFGP2P_ERR(("CMD_SETFWPATH\n"));
	}
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		CFGP2P_ERR(("CMD_LINKSPEED\n"));
	}
	else if (strnicmp(command, CMD_RXFILTER_START, strlen(CMD_RXFILTER_START)) == 0) {
		/* bytes_written = net_os_set_packet_filter(net, 1); */
		CFGP2P_ERR(("CMD_RXFILTER_START\n"));
	}
	else if (strnicmp(command, CMD_RXFILTER_STOP, strlen(CMD_RXFILTER_STOP)) == 0) {
		/* bytes_written = net_os_set_packet_filter(net, 0); */
		CFGP2P_ERR(("CMD_RXFILTER_STOP\n"));
	}
	else if (strnicmp(command, CMD_RXFILTER_ADD, strlen(CMD_RXFILTER_ADD)) == 0) {
		/* int filter_num = *(command + strlen(CMD_RXFILTER_ADD) + 1) - '0'; */
		/* bytes_written = net_os_rxfilter_add_remove(net, TRUE, filter_num); */
		CFGP2P_ERR(("CMD_RXFILTER_ADD\n"));
	}
	else if (strnicmp(command, CMD_RXFILTER_REMOVE, strlen(CMD_RXFILTER_REMOVE)) == 0) {
		/* int filter_num = *(command + strlen(CMD_RXFILTER_REMOVE) + 1) - '0'; */
		/* bytes_written = net_os_rxfilter_add_remove(net, FALSE, filter_num); */
		CFGP2P_ERR(("CMD_RXFILTER_REMOVE\n"));
	}
		...
		CFGP2P_ERR(("CMD_BTCOEXMODE\n"));

	}
	else if (strnicmp(command, CMD_SETSUSPENDOPT, strlen(CMD_SETSUSPENDOPT)) == 0) {
		/* bytes_written = wl_android_set_suspendopt(net, command, priv_cmd.total_len); */
		CFGP2P_ERR(("CMD_SETSUSPENDOPT\n"));
	}
		...
		CFGP2P_ERR(("CMD_PNOSSIDCLR_SET\n"));
	}
	else if (strnicmp(command, CMD_PNOSETUP_SET, strlen(CMD_PNOSETUP_SET)) == 0) {
		/* bytes_written = wl_android_set_pno_setup(net, command, priv_cmd.total_len); */
		CFGP2P_ERR(("CMD_PNOSETUP_SET\n"));
	}
	else if (strnicmp(command, CMD_PNOENABLE_SET, strlen(CMD_PNOENABLE_SET)) == 0) {
		uint pfn_enabled = *(command + strlen(CMD_PNOENABLE_SET) + 1) - '0';
		/* bytes_written = dhd_dev_pno_enable(net, pfn_enabled); */
		CFGP2P_ERR(("CMD_PNOENABLE_SET\n"));
	}
#endif /* #ifdef PNO_SUPPORT */
	else if (strnicmp(command, CMD_P2P_DEV_ADDR, strlen(CMD_P2P_DEV_ADDR)) == 0) {
		/* bytes_written = wl_android_get_p2p_dev_addr(net, command, priv_cmd.total_len); */
		CFGP2P_ERR(("CMD_P2P_DEV_ADDR\n"));
	}
		...
		CFGP2P_ERR(("Unknown PRIVATE command %s - ignored\n", command));
	...
	if (bytes_written >= 0) {
		...
		if (copy_to_user(priv_cmd.buf, command, bytes_written)) {
			CFGP2P_ERR(("%s: failed to copy data to user buffer\n", __FUNCTION__));
			ret = -EFAULT;
		}
	}
	else {
...
#endif /* #if defined(OEM_ANDROID) || defined(WLC_HIGH_ONLY) */
