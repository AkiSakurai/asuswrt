 ...
 *
 * $Id: wlc_fbt.c 330269 2012-04-30 14:42:50Z $
 */
...
#define WLC_FT_PARAM_OVERRIDE_ENAB(priv) \
	((priv) != NULL && (priv)->fbt_bss_overrides != NULL)
...
typedef struct bss_fbt_priv {
	...
	/* the below are used only when FT is via OS supplicant
	* and sec params are cfg-ed via iovar
	*/
...
#define FT_MSG_LVL_IMPORTANT 1
#define FT_MSG_LVL_CHATTY 2
#define FT_MSG_LVL_ENTRY 4
...
static void wlc_fbt_dump_fbt_keys(bsis_fbt_priv_t *fbt_bss_priv, uchar *pmkr0, uchar *pmkr1);
#endif /* STA && FBT_STA */
#ifdef AP
...
/* RSNIE */
static uint
wlc_fbt_write_overridable_ie(void *ctx, wlc_iem_build_data_t *data,
	uint16 param_type, wlc_iem_build_fn_t build_fn_def, uint8 *out_data);

static uint
wlc_fbt_calc_overridable_ie_len(void *ctx, wlc_iem_calc_data_t *data,
	uint16 param_type, wlc_iem_calc_fn_t calc_fn_def);
static uint wlc_fbt_auth_calc_rsn_ie_len_ex(void *ctx, wlc_iem_calc_data_t *data);
static uint wlc_fbt_auth_calc_rsn_ie_len(void *ctx, wlc_iem_calc_data_t *data);
static int wlc_fbt_auth_write_rsn_ie_ex(void *ctx, wlc_iem_build_data_t *data);
...
static uint wlc_fbt_calc_md_ie_len_ex(void *ctx, wlc_iem_calc_data_t *data);
static int wlc_fbt_write_md_ie_ex(void *ctx, wlc_iem_build_data_t *data);
...
static uint wlc_fbt_calc_ft_ie_len_ex(void *ctx, wlc_iem_calc_data_t *data);
static int wlc_fbt_write_ft_ie_ex(void *ctx, wlc_iem_build_data_t *data);
...
static const bcm_iovar_t fbt_iovars[] = {
	{"fbtoverds", IOV_ALLOW_FBTOVERDS, IOVF_SET_DOWN, IOVT_UINT32, 0},
	{"fbt_cap", IOV_FBT_CAP, (IOVF_OPEN_ALLOW), IOVT_UINT32, 0},
	{"fbt", IOV_FBT, IOVF_SET_DOWN, IOVT_INT32, 0},
	{"fbt_ap", IOV_FBT_AP, IOVF_SET_DOWN|IOVF_BSSCFG_AP_ONLY, IOVT_INT32, 0},
	{"fbt_mdid", IOV_FBT_MDID, IOVF_SET_DOWN|IOVF_BSSCFG_AP_ONLY, IOVT_UINT16, 0},
	{"fbt_r0kh_id", IOV_FBT_R0KH_ID, IOVF_SET_DOWN|IOVF_BSSCFG_AP_ONLY, IOVT_BUFFER, 0},
	{"fbt_r1kh_id", IOV_FBT_R1KH_ID, IOVF_SET_DOWN|IOVF_BSSCFG_AP_ONLY, IOVT_BUFFER, 0},
	{"fbt_reassoc_time", IOV_FBT_REASSOC_TIME, IOVF_BSSCFG_AP_ONLY, IOVT_UINT32, 0},
	{"fbt_auth_resp", IOV_FBT_AUTH_RESP, (0), IOVT_BUFFER, 0},
...
BCMATTACHFN(wlc_fbt_attach)(wlc_info_t *wlc)
{
	...
	if (!(fbt_info = (wlc_fbt_info_t *)MALLOC(wlc->osh, sizeof(wlc_fbt_info_t)))) {
	...
	if (wlc_iem_add_build_fn_mft(wlc->iemi, rsniefstbmp, DOT11_MNG_RSN_ID,
	      wlc_fbt_auth_calc_rsn_ie_len_ex, wlc_fbt_auth_write_rsn_ie_ex, fbt_info) != BCME_OK) {
	...
	if (wlc_iem_add_build_fn_mft(wlc->iemi, mdiefstbmp, DOT11_MNG_MDIE_ID,
	      wlc_fbt_calc_md_ie_len_ex, wlc_fbt_write_md_ie_ex, fbt_info) != BCME_OK) {
	...
	if (wlc_iem_add_build_fn_mft(wlc->iemi, ftiefstbmp, DOT11_MNG_FTIE_ID,
	      wlc_fbt_calc_ft_ie_len_ex, wlc_fbt_write_ft_ie_ex, fbt_info) != BCME_OK) {
	...
	wlc_bsscfg_t *bsscfg)
{
	...
	do {
		...
		if (!WLC_FT_PARAM_OVERRIDE_ENAB(cubby)) {
		...
		wlc_ft_override_get_buf(fbt_info, bsscfg,
	...
	if (bcmerr != BCME_OK) {
		WL_ERROR(("wl%d:%s: Returning error (%s)\n",
			WLC_FBT_PRIV_INFO(fbt_info)->pub->unit, __FUNCTION__,
			bcmerrorstr(bcmerr)));
	}
...
{
	int bcmerr = BCME_OK;
	bss_fbt_priv_t *cubby = wlc_fbt_get_bsscfg(fbt_info, bsscfg);
	...
	void *params, uint plen, void *arg, int alen, int vsize, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_FBT):
		{
			*ret_int_ptr =  WLFBT_ENAB(wlc->pub) && BSSCFG_IS_FBT(bsscfg, wlc->pub);
			break;
		}
	...
	case IOV_GVAL(IOV_FBT_AP):
		{
			if (WLFBT_ENAB(wlc->pub)) {
				*ret_int_ptr = BSSCFG_IS_FBT(bsscfg, wlc->pub);
			} else {
				err = BCME_UNSUPPORTED;
			}
			break;
	...
	case IOV_SVAL(IOV_FBT_DS_ADD_STA):
		{
			if (WLFBT_ENAB(wlc->pub)) {
				wlc_fbt_auth_resp_t *fbt_auth_resp;
				fbt_auth_resp = (wlc_fbt_auth_resp_t *)arg;
				err = wlc_fbtap_process_ds_auth_resp(wlc, fbt_info,
					bsscfg, fbt_auth_resp);
			} else {
				err = BCME_UNSUPPORTED;
			}
			break;
	...
	case IOV_SVAL(IOV_FBT_ACT_RESP):
		{
			if (WLFBT_ENAB(wlc->pub)) {
				...
				err = wlc_fbtap_process_action_resp(wlc, fbt_info,
					bsscfg, fbt_act_resp);
			} else {
				err = BCME_UNSUPPORTED;
			}
...
{
	bss_fbt_priv_t *fbt_bss_priv;
	uint8 *to_len_ptr = NULL;
	uint8 **to_ptr_ptr = NULL;
	wlc_bss_fbt_overrides_t *fbt_bss_overrides = NULL;
	*to_ptr = NULL;
	...
	do {
		...
		if (!WLC_FT_PARAM_OVERRIDE_ENAB(fbt_bss_priv)) {
			break;
		}
		...
		wlc_ft_param_type_to_pointers(fbt_bss_overrides,
			param_type, &to_ptr_ptr, &to_len_ptr);
...
wlc_fbt_bsscfg_cubby_init(wlc_fbt_info_t *fbt_info, wlc_bsscfg_t *cfg)
{
	...
	if (!(fbt_bss = (bss_fbt_info_t *)MALLOC(fbt_priv->osh, sizeof(bss_fbt_info_t)))) {
		WL_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n",
			UNIT(fbt_priv), __FUNCTION__, MALLOCED(fbt_priv->osh)));
	}
	bzero(fbt_bss, sizeof(bss_fbt_info_t));
...
wlc_fbt_bsscfg_free_overrides(bss_fbt_priv_t *fbt_bss_priv)
{
	...
	if (fbt_bss_overrides) {

		for (tbl_idx = 0; tbl_idx < WL_FBT_PARAM_TYPE_FIRST_INVALID;
			tbl_idx++) {

			if (fbt_bss_overrides->override_ptrs[tbl_idx]) {
	...
	struct scb *scb, wsec_key_t *scb_key, bool short_preamble)
{
	...
	if (key) {
		wlc_key_iv_update(wlc, cfg, key, pbody, 0, 0);
	...
	if (wlc_queue_80211_frag(wlc, p, qi, scb, scb->bsscfg,
		short_preamble, key, 0))
		return p;
#endif /* !defined(WLFBT_OVER_DS_DISABLED) || defined(FBT_FDAP) */
	...
	uint16 param_type, wlc_iem_calc_fn_t calc_fn_def)
{
	...
	do {
		...
		if (WLFBT_ENAB(fbt_priv->wlc->pub)) {
			...
			if (wlc_ft_override_get_len(fbt_info, cfg,
				param_type, &ie_len) != NULL) {
				/* use length of override */
				break;
			} else {
				ie_len = (uint8)calc_fn_def(ctx, data);
			}
		}
	} while (0);
	...
	uint16 param_type, wlc_iem_build_fn_t build_fn_def, uint8 *out_data)
{
	...
	do {
		...
		if (WLFBT_ENAB(fbt_priv->wlc->pub)) {
			...
			if (wlc_ft_override_get_buf(fbt_info, cfg,
				WL_FBT_PARAM_TYPE_RSNIE, out_data,
				BCM_TLV_MAX_DATA_SIZE)) {
				break;
			} else {
				bcmerr = build_fn_def(ctx, data);
			}
		}
	} while (0);
	return bcmerr;
}
/* for handling overrides */
static uint
wlc_fbt_auth_calc_rsn_ie_len_ex(void *ctx, wlc_iem_calc_data_t *data)
{
	return wlc_fbt_calc_overridable_ie_len(ctx, data, WL_FBT_PARAM_TYPE_RSNIE,
		wlc_fbt_auth_calc_rsn_ie_len);
}

static uint
wlc_fbt_calc_md_ie_len_ex(void *ctx, wlc_iem_calc_data_t *data)
{
	return wlc_fbt_calc_overridable_ie_len(ctx, data, WL_FBT_PARAM_TYPE_MDE,
		wlc_fbt_calc_md_ie_len);
}

static uint
wlc_fbt_calc_ft_ie_len_ex(void *ctx, wlc_iem_calc_data_t *data)
{
	return wlc_fbt_calc_overridable_ie_len(ctx, data, WL_FBT_PARAM_TYPE_FTIE,
		wlc_fbt_calc_ft_ie_len);
}

static int
wlc_fbt_auth_write_rsn_ie_ex(void *ctx, wlc_iem_build_data_t *data)
{
	bss_fbt_priv_t *fbt_bss_priv =
		wlc_fbt_param_get_bss_priv((wlc_fbt_info_t *)ctx, data->cfg);

	/* populate pmkr0name override if there is one */
	if (fbt_bss_priv) {
		wlc_ft_override_get_buf(ctx, data->cfg, WL_FBT_PARAM_TYPE_PMK_R0_NAME,
			fbt_bss_priv->pmkr0name, sizeof(fbt_bss_priv->pmkr0name));
	}

	return wlc_fbt_write_overridable_ie(ctx, data, WL_FBT_PARAM_TYPE_RSNIE,
		wlc_fbt_auth_write_rsn_ie, data->buf);
}
static int
wlc_fbt_write_md_ie_ex(void *ctx, wlc_iem_build_data_t *data)
{
	return wlc_fbt_write_overridable_ie(ctx, data, WL_FBT_PARAM_TYPE_MDE,
		wlc_fbt_write_md_ie, data->buf);
}

static int
wlc_fbt_write_ft_ie_ex(void *ctx, wlc_iem_build_data_t *data)
{
	bss_fbt_priv_t *fbt_bss_priv =
		wlc_fbt_param_get_bss_priv((wlc_fbt_info_t *)ctx, data->cfg);
	wlc_bss_fbt_overrides_t *fbt_bss_overrides;
	...
	if (WLC_FT_PARAM_OVERRIDE_ENAB(fbt_bss_priv)) {
		...
		if (fbt_bss_overrides->override_ptrs[WL_FBT_PARAM_TYPE_R0_KHID]) {
			fbt_bss_priv->r0khid = (bcm_tlv_t*)
				fbt_bss_overrides->override_ptrs[WL_FBT_PARAM_TYPE_R0_KHID];
		}

		if (fbt_bss_overrides->override_ptrs[WL_FBT_PARAM_TYPE_R1_KHID]) {
			fbt_bss_priv->r1khid = (bcm_tlv_t*)
				fbt_bss_overrides->override_ptrs[WL_FBT_PARAM_TYPE_R1_KHID];
		}
	...
	return wlc_fbt_write_overridable_ie(ctx, data, WL_FBT_PARAM_TYPE_FTIE,
		wlc_fbt_write_ft_ie, data->buf);
}
		...
		case FC_REASSOC_REQ:
			{
				...
				if (bi->wpa2.flags & RSN_FLAGS_FBT && bi->bcn_prb != NULL) {
					...
					/* look for an MD IE in saved bcn/prb, and verify
					 * the length
					 */
...
wlc_fbt_scan_parse_ccx_ext_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	 ...
	 * CCX Ext Cap IE.
	 */
	if (!BSSCFG_IS_FBT(cfg, wlc->pub) || (bi->wpa2.flags & RSN_FLAGS_FBT))
		return BCME_OK;
		...
		if (BSSCFG_IS_FBT(cfg, wlc->pub) && (cfg->WPA_auth & WPA2_AUTH_FT)) {
			...
			for (i = 0; i < bi->wpa2.acount && (akm_match == FALSE); i++) {
				if (((bi->wpa2.auth[i] == RSN_AKM_UNSPECIFIED) &&
					...
					(((bi->wpa2.auth[i] == RSN_AKM_MFP_1X) &&
					(cfg->WPA_auth & WPA2_AUTH_UNSPECIFIED)) ||
					((bi->wpa2.auth[i] == RSN_AKM_MFP_PSK) &&
					(cfg->WPA_auth & WPA2_AUTH_PSK)))) {
...
#ifdef AP
	if (BSSCFG_AP(cfg)) {
		return BSSCFG_IS_FBT(cfg, fbt_info->mod_priv.wlc->pub);
	}
#endif /* AP */
		...
		if (offsets[RSNE_OFFSETS_PMKID_INDEX]) {
			...
			if (pmk->count.low != 0 || pmk->count.high != 0) {
				if (memcmp(pmk->list, fbt_scb->pmk_r1_name, WPA2_PMKID_LEN) != 0) {
					...
					/* Send BCME_IE_NOTFOUND instead of BCME_BAD_IE_DATA to
					 * save memory
					 */
					return BCME_IE_NOTFOUND;
				}
...
wlc_fbtap_reassociation_timer(void *arg)
{
	...
	FOREACH_UP_AP(wlc, i, bsscfg)
	{
		...
		if (!fbt_bsscfg || !BSSCFG_IS_FBT(bsscfg, fbt_priv->wlc->pub))
			continue;
		reassoc_deadline = ((fbt_bsscfg->reassoc_time) * DOT11_TU_TO_US)/1000;
