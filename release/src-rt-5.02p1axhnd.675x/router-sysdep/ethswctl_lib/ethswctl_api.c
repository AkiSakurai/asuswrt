...
static inline int ethswctl_init(struct ifreq *p_ifr)
{
    ...
    if (ioctl(skfd, SIOCGIFINDEX, p_ifr) < 0 ) {
        ...
        if (ioctl(skfd, SIOCGIFINDEX, p_ifr) < 0 ) {
            close(skfd);
            fprintf(stderr, "neither bcmsw nor eth0 exist\n");
            return -1;
        }
    }
#endif
...
int ethswctl_mibdump_x(int unit, int port, int priority, int type)
{
    ...
    if (((port & ~BCM_EXT_6829) < 0) || ((port & ~BCM_EXT_6829) > 8)) {
        fprintf(stderr, "Invalid port. Valid values are 0 to 8. \n");
        err = -1;
        goto out;
    }
    if (type < 0 || type > 2) {
...
int bcm_enet_map_oam_idx_to_unit_port(int oamIdx, int *unit, bcm_port_t *port)
{
    ...
    *unit = e->unit;
    *port = e->port;
    DBG(fprintf(stderr, "oamIdx %d is: unit %d port %d", oamIdx, *unit, *port););
    close(skfd);
    return err;
}
...
int bcm_enet_map_oam_idx_to_rdpaif(int oamIdx)
{
    ...
    DBG(fprintf(stderr, "oamIdx %d is: rdpa_if %d", oamIdx, *rdpa_if););
    close(skfd);
...
int bcm_enet_map_oam_idx_to_phys_port(bcm_port_t oam_idx)
{
    ...
    /*if error raised, return -1*/
    if (err){
        fprintf(stderr, "Get unit port failed, oam_idx=%d\n", oam_idx);
        return -1;
    }
    return port;
}
...
int bcm_enet_map_phys_port_to_rdpa_if(int unit, bcm_port_t port)
{
    ...
    if((err = ETHSW_IOCTL_WRAPPER(skfd, SIOCETHSWCTLOPS, &ifr))) {
        ...
        DBG(fprintf(stderr, "unit %d port %d --> rdpa_if %#x", e->unit, e->port, e->val););
        close(skfd);
        return e->val;
    }
}
...
int bcm_enet_get_rdpa_if_from_if_name(const char* ifname, int* rdpaIf_p)
{
    ...
    if((err = ETHSW_IOCTL_WRAPPER(skfd, SIOCETHSWCTLOPS, &ifr))) {
        ...
        else if (errsv != ENOTTY) {
            fprintf(stderr, "ioctl command return error %d\n", errsv);
        }
        close(skfd);
        return err;
    } else {
        DBG(fprintf(stderr, "ifname - %s rdpa_if %d", e->ifname, e->val););
        close(skfd);
        *rdpaIf_p = e->val;
        return err;
    }
...
int bcm_spi_write(unsigned int addr, char *data, int len)
{
    ...
    DBG(fprintf(stderr, "len = %d; data = ", len););
    for (i=len; i>=0; i--) {
...
int bcm_ethsw_kernel_poll(struct mdk_kernel_poll *mdk_kernel_poll)
{
    ...
    e->mdk_kernel_poll.link_change = mdk_kernel_poll->link_change;
 ...
 *    unit - unit
 *    port - port.
 *    enable - enable or disable, -1 stands for not support
 * Returns:
 *    BCM_E_NONE - Success.
 */
...
static int fake_ethSwCtlOps(void *data)
{
    ...
    switch (e->op)
    {
        case ETHSWOAMIDXMAPPING:
            if (e->type == TYPE_GET)
            {
                if (e->oam_idx_str.map_sub_type == OAM_MAP_SUB_TYPE_TO_UNIT_PORT)
                {
                    e->unit = 0;
                    e->port = e->oam_idx_str.oam_idx;
                }
                else if (e->oam_idx_str.map_sub_type == OAM_MAP_SUB_TYPE_FROM_UNIT_PORT)
                {
                    e->oam_idx_str.oam_idx = e->port;
                }
                else
                {
                    ret = -EOPNOTSUPP;
                    fprintf(stderr, "Unsupported sub type %u\n",
                      e->oam_idx_str.map_sub_type);
                }
            }
            break;
        case ETHSWGETIFNAME:
...
void getIntfStats(const char *devName, struct IntfStats_s *s)
{
...
int ethswctl_gpon_mcast_gem_set(int mcast_idx)
{
    ...
    ifr.ifr_data = (void *)&ethswctl;
    ethswctl.op = ETHPORTMCASTGEMSET;
    err = ETHSW_IOCTL_WRAPPER(skfd, SIOCETHSWCTLOPS, &ifr);
    if (err < 0 )
