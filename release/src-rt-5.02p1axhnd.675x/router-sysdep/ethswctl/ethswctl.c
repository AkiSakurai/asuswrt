...
static int debug_handler(int argc __attribute__((unused)), cmd_t *cmd __attribute__((unused)), char** argv)
{
    int level, ret_val = 0;
...
static int cmd_partial_match(char *cmd, const char *msg)
{
    static char *delimit = " \t";
    static char cbuf[64];
    char *msg_cmd = cbuf;

    strncpy(cbuf, msg, sizeof(cbuf)-1);
    msg_cmd = strtok(cbuf, delimit);
    msg_cmd = strtok(NULL, delimit);
    msg_cmd = strtok(NULL, delimit);
    return !strncmp(msg_cmd, cmd,1);
}
...
void print_usage(char *cmd)
{
    ...
    for(i=0; i<MAX_NUM_COMMANDS; i++) {
        if (!cmd || cmd_partial_match(cmd, usage_msgs[i])) {
            char firstline[200] = {};                           // only print 1st line of cmd help msg
            memcpy(firstline, usage_msgs[i], (int)(strchr(usage_msgs[i], '\n')-usage_msgs[i])+1);
            fprintf(stderr, "%s", firstline);
        }
    }
    command_helpall();
}
...
#endif
{
    ...
    const char *msg;
    int port = 0, page = 0, exclusive = 0,  priority = 0, que = 0, type = 0, mode = -1, enable = -1;
...
#define SF2_QOS_TC_SRC_SEL_VAL_MASK 3
    ...
    unit = SF2_ETHSWCTL_UNIT;
#endif

#if defined(HAS_SF2_CFP)
    memset(&cfpArg, 0, sizeof(cfpArg));
#endif
    while ((c = getopt_long(argc, argv, "abc:d:e:f:hi:l:m:n:o:p:P:qrst:T:u:v:V:wx:y:z:", longopts, &longIdx)) != -1) {
        ...
        switch (c) {
            ...
            case 'd': opt_d++; regdata = (unsigned long long)(strtoll(optarg, NULL, 0)); 
                        que = strtol(optarg, NULL, 0); break;
            ...
            case 'p': opt_p++; 
                        ...
                        if ((str = strtok(NULL, "."))) sub_port = (int) strtol(str, NULL, 0);
                        break;

            case 'P': opt_P++; page = (int)(strtol(optarg, NULL, 0)); break;

            case 'q': opt_q++; get_opt_optarg(argc, argv, &opt_q, (unsigned int*)&priority);
#if defined(HAS_SF2_CFP)
            ...
            case 's': opt_s++; get_opt_optarg(argc, argv, &opt_s, &speed); break;
    ...
    if (strcmp(name, "hw-switching") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[HWSWITCHING_MSG]);
            return 0;
        }
        if (opt_o) {
        ...
        if (opt_h || opt_p == 0) {
            fprintf(stderr, "%s", usage_msgs[MIBDUMP_MSG]);
            return 0;
        }
        ...
        if (opt_h || opt_P == 0) {
            fprintf(stderr, "%s", usage_msgs[PAGEDUMP_MSG]);
            return 0;
        }
        if (opt_s) {
            page |= BCM_EXT_6829;
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[IUDMADUMP_MSG]);
            return 0;
        }
        ...
        if (!opt_p) {
            fprintf(stderr, "%s", usage_msgs[IUDMASPLIT_MSG]);
            return 0;
        }
        if(!opt_q)
            iudma = -1;
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWCTRL_MSG]);
            return 0;
        }
        if (!opt_t) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWPRIOCTRL_MSG]);
            return 0;
        }
        ...
        if (opt_v || opt_m || opt_l) {
            ret_val = ethswctl_control_priority_op(unit, port, priority, type, &val, &mode, &lval, &que, 0);
        } else {
            ret_val = ethswctl_control_priority_op(unit, port, priority, type, &val, &mode, &lval, &que, 1);
        }
    }
#if defined(HAS_SF2)
    else if (strcmp(name, "swflowctrl") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWFLOWCTRL_MSG]);
            return 0;
        }
        if (!opt_t) {
            fprintf(stderr, "Type (-t option) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWQUEMAP_MSG]);
            return 0;
        }
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWQUEMON_MSG]);
            return 0;
        }
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWACB_MSG]);
            return 0;
        }
        if (!opt_t) {
            fprintf(stderr, "Type (-t option) should be specified\n");
    ...
    else if (strcmp(name, "maclmt") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWMACLMT_MSG]);
            return 0;
        }
        ...
        ret_val = ethswctl_maclmt_op(unit, port, type, (int*)&val);
    }
#if defined(HAS_SF2_CFP)
    else if (strcmp(name, "cfp") == 0) {
        if (opt_h || cfpArg.argFlag == 0) {
            fprintf(stderr, "%s", usage_msgs[SWCFPTCAM_MSG]);
            return 0;
        }
        ...
        if ((cfpArg.argFlag & CFP_ARG_L3_FRAMING_M) == 0) {
            ...
            if (!(cfpArg.argFlag & CFP_ARG_IP_FLAGS)) {
                if ((cfpArg.argFlag & CFP_ARG_ETYPE_SAP_M)) {
                    if (cfpArg.etype_sap == 0x0800) {
                        cfpArg.l3_framing = CfpL3Ipv4;
                        cfpArg.argFlag &= ~CFP_ARG_ETYPE_SAP_M;
                    }
                    else
        ...
        if (((cfpArg.argFlag & CFP_ARG_CHG_FPMAP_IB_M)>0) ^ ((cfpArg.argFlag & CFP_ARG_FPMAP_IB_M)>0)) {
        ...
        else
        {
            ...
            switch (cfpArg.rc)
            {
                ...
                case CFP_RC_NON_EXISTING_INDEX:
                    if (cfpArg.op != CFPOP_DELETE_ALL){ fprintf(stderr, "Failed: requested entry of the index does not exist\n"); }
                        ret_val = -1;
                    break;
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[RXSCHED_MSG]);
            return 0;
        }
        if (opt_v) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[WRRPARAMS_MSG]);
            return 0;
        }
        if (opt_v) {
            if (!opt_t) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[VLAN_MSG]);
            return 0;
        }
        if (!opt_v) {
    ...
    if (opt_f && opt_u) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[GETRXCNTRS_MSG]);
            return 0;
        }
        ret_val = bcm_switch_getrxcntrs();
    } else if (strcmp(name, "resetrxcntrs") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[RESETRXCNTRS_MSG]);
            return 0;
        }
        ret_val = bcm_switch_resetrxcntrs();
    } else if (strcmp(name, "pbvlanmap") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PBVLANMAP_MSG]);
            return 0;
        }
    if (opt_v) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[GETSTAT_MSG]);
            return 0;
        }
        if (!opt_t || !opt_p) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[GETSTAT32_MSG]);
            return 0;
        }
        if (!opt_t || !opt_p) {
            fprintf(stderr, "Type (-t) and port(-p) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PAUSE_MSG]);
            return 0;
        }
        if (!opt_p) {
        ...
        if (opt_v) {
            if (val > PAUSE_FLOW_PFC_RX)
            {
                fprintf(stderr, "invalid value %d specified\n", val);
                fprintf(stderr, "%s", usage_msgs[PAUSE_MSG]);
                return 0;
            }
            ret_val = bcm_port_pause_capability_set(unit, port, val);
        } else {
            ret_val = bcm_port_pause_capability_get2(unit, port, (int*)&val, pfc_timer);
            if (ret_val) {
                goto close_n_ret;
            }
            val8 = val & 0xff;
            ...
            switch (val8) {
                ...
                case PAUSE_FLOW_CTRL_RX:
                    ...
                    break;
                case PAUSE_FLOW_PFC_TX:
                    printf("Tx PFC Enabled\n");
                    printf("Current Run Time PFC timers:\n");
                    for (i=0; i<8; i++)
                        printf("    Class %d: 0x%x\n", i, pfc_timer[i]);
                    break;
                case PAUSE_FLOW_PFC_RX:
                    printf("Rx PFC Enabled\n");
                    break;
                case PAUSE_FLOW_PFC_BOTH:
                    printf("Both Tx and Rx PFC Enabled\n");
                    break;
                default:
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[COSQ_MSG]);
            return 0;
        }
        if (opt_p && opt_q) {
            if (opt_v) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[COSTXQ_MSG]);
            return 0;
        }
        if (opt_v) {
            if (opt_q) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[BYTERATECFG_MSG]);
            return 0;
        }
        if (!opt_v) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[BYTERATE_MSG]);
            return 0;
        }
        if (!opt_v) {
            fprintf(stderr, "Channel (-v option) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[DEFTXQEN_MSG]);
            return 0;
        }
        if (!opt_i) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[DEFTXQ_MSG]);
            return 0;
        }
        if (!opt_i) {
            fprintf(stderr, "The if_name (-i option) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[TEST_MSG]);
            return 0;
        }
        if (!opt_t) {
            fprintf(stderr, "Type (-t option) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PKTRATECFG_MSG]);
            return 0;
        }
        if (!opt_v) {
            fprintf(stderr, "Channel (-v option) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PKTRATE_MSG]);
            return 0;
        }
        if (!opt_v) {
            fprintf(stderr, "Channel (-v option) should be specified\n");
        ...
        bcm_vlan_t vid = val;
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[ARL_MSG]);
            return 0;
        }
        if (!opt_m) {
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[ARLDUMP_MSG]);
            return 0;
        }
        ret_val = bcm_arl_dump(unit);
    } else if (strcmp(name, "arlflush") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[ARLFLUSH_MSG]);
            return 0;
        }
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[UNTAGGEDPRIO_MSG]);
            return 0;
        }
        if (!opt_p) {
            fprintf(stderr, "Port(-p) should be specified\n");
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[COSQPRIOMETHOD_MSG]);
            return 0;
        }
...
#if defined(HAS_SF2)
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[COSQSCHED_MSG]);
            return 0;
        }
        if (opt_v) { //SET
            // validate type 1..4
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PORTCTRL_MSG]);
            return 0;
        }
        if (opt_v) {
        ...
        if (opt_h || opt_p == 0) {
            fprintf(stderr, "%s", usage_msgs[PORTLOOPBACK_MSG]);
            return 0;
        }

        if (bcm_get_phyid(unit, port, &sub_port) < 0) {
            fprintf(stderr, "%s", usage_msgs[PORTLOOPBACK_MSG]);
            return 0;
        }

        if (opt_v) {
            ret_val = bcm_port_subport_loopback_set(unit, port, sub_port, speed, val);
            if (ret_val) {
                fprintf(stderr, "Loopback set failed.\n");
                goto close_n_ret;
            }
        } else {
            ret_val = bcm_port_subport_loopback_get(unit, port, sub_port, (int *)&speed, (int *)&param1);
            if (ret_val) {
                fprintf(stderr, "Loopback get failed.\n");
                goto close_n_ret;
            }
            if (param1) {
                printf("Unit %d Port %d loopback is enabled at speed %dMbps\n", unit, port, speed);
            } else {
                printf("Unit %d Port %d loopback is disabled\n", unit, port);
            }
        }
    } else if (strcmp(name, "phymode") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PHYMODE_MSG]);
            return 0;
        }
    ...
    } else if (strcmp(name, "createport") == 0) {
        ...
        if (if_name)
            strncpy(net_port.ifname, if_name, IFNAMSIZ);

        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[CREATEPORT_MSG]);
            return 0;
        }

        if (opt_p) {
            ret_val = bcm_port_create(&net_port);
            if (ret_val) {
                goto close_n_ret;
            }
            printf("port %d created as %s with name %s\n", port, param4 ? "WAN" : "LAN", net_port.ifname);
        }
        else {
            fprintf(stderr, "%s", usage_msgs[CREATEPORT_MSG]);
            return 0;
        }
    } else if (strcmp(name, "deleteport") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[DELETEPORT_MSG]);
            return 0;
        }

        if (opt_p) {
            ret_val = bcm_port_delete(port);
            if (ret_val) {
                goto close_n_ret;
            }
            printf("port %d deleted\n", port);
        }
        else {
            fprintf(stderr, "%s", usage_msgs[DELETEPORT_MSG]);
            return 0;
        }
    } else if (strcmp(name, "gponmcastgemset") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[GPONMCASTGEMSET_MSG]);
            return 0;
        }

        if (opt_p) {
            ret_val = ethswctl_gpon_mcast_gem_set(port);
            if (ret_val) {
                goto close_n_ret;
            }
            printf("port %d set as gpon mcast gem idx\n", port);
        }
        else {
            fprintf(stderr, "%s", usage_msgs[GPONMCASTGEMSET_MSG]);
            return 0;
        ...
        if (opt_h || opt_p == 0) {
            fprintf(stderr, "%s", usage_msgs[JUMBO_MSG]);
            return 0;
        }
        ...
        if (opt_h || opt_v == 0) {
            fprintf(stderr, "%s", usage_msgs[DSCP2PRIO_MSG]);
            return 0;
        }
        ...
        if (opt_h || opt_v == 0) {
            fprintf(stderr, "%s", usage_msgs[PCP2PRIO_MSG]);
            return 0;
        }
        ...
        if (opt_h || opt_p == 0) {
            fprintf(stderr, "%s", usage_msgs[PID2PRIO_MSG]);
            return 0;
        }
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[REGACCESS_MSG]);
            return 0;
        }
        ...
        if (opt_h ) {
            fprintf(stderr, "%s", usage_msgs[DOSCTRLCTRL_MSG]);
            return 0;
        }
        if (opt_v) {
            if (val & (1<<0)) dosCtrl.da_eq_sa_drop_en =1;
            ...
            printf(" DA_SA=%d; IP_LAN=%d; TCP_BLAT=%d; UDP_BLAT=%d; TCP_NULL=%d;\n",(int)dosCtrl.da_eq_sa_drop_en, (int)dosCtrl.ip_lan_drop_en,
        ...
        unsigned long long data64 = 0;
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SPIACCESS_MSG]);
            return 0;
        }
        if (opt_d) {
            data64 = cpu_to_le64(regdata);
        ...
        unsigned long long data64 = 0;
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[PSEUDOMDIOACCESS_MSG]);
            return 0;
        }
        if (opt_d) {
            data64 = cpu_to_le64(regdata);
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SETLINKSTATUS_MSG]);
            return 0;
        }
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SWITCHINFO_MSG]);
            return 0;
        }
        if (opt_v) {
        ...
        if (opt_h || (!opt_i ^ !opt_o)) {
            fprintf(stderr, "%s", usage_msgs[WAN_MSG]);
            return 0;
        }
        if (opt_o) {
            if (strcmp(operation, "enable") == 0) {
        ...
        if (opt_h || (opt_p==0) || ((opt_x && !opt_y) || (!opt_x && opt_y))) {
            fprintf(stderr, "%s", usage_msgs[PORTRXRATECTRL_MSG]);
            return 0;
        }
        ...
        if (opt_h || (opt_p==0) || ((opt_x && !opt_y) || (!opt_x && opt_y))) {
            fprintf(stderr, "%s", usage_msgs[PORTTXRATECTRL_MSG]);
            return 0;
        }
        if (opt_x) { /* Optional Parameter present ==> Set */
            int is_pkt_mode;
        ...
        if (opt_h || (opt_p == 0) || (opt_t == 0) || (opt_v == 0)) {
            fprintf(stderr, "%s", usage_msgs[PORTTXRATECFG_MSG]);
            return 0;
        }
        priority = opt_q? priority: -1;
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[SOFTSWITCH_MSG]);
            return 0;
        }

        if ( opt_o && opt_i ) {
            if (strcmp(operation, "enable") == 0) {
               ret_val = bcm_enet_driver_enable_soft_switching_port(1, if_name);
            }
            else if (strcmp(operation, "disable") == 0) {
               ret_val = bcm_enet_driver_enable_soft_switching_port(0, if_name);
            }
            else {
               fprintf(stderr, "%s", usage_msgs[SOFTSWITCH_MSG]);
               return 0;
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[HWSTP_MSG]);
            return 0;
        }
    ...
    else if (strcmp(name, "ifstp") == 0) {
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[IFSTP_MSG]);
            return 0;
        }

        if (opt_o && opt_i) {
            if (strcasecmp(operation, "disabled") == 0) {
                ret_val = bcm_enet_driver_if_stp_set(if_name, STP_DISABLED);
            }
            else if (strcasecmp(operation, "blocking") == 0) {
               ret_val = bcm_enet_driver_if_stp_set(if_name, STP_BLOCKING);
            }
            else if (strcasecmp(operation, "discarding") == 0) {
               ret_val = bcm_enet_driver_if_stp_set(if_name, STP_BLOCKING);
            }
            else if (strcasecmp(operation, "listening") == 0) {
               ret_val = bcm_enet_driver_if_stp_set(if_name, STP_LISTENING);
            }
            else if (strcasecmp(operation, "learning") == 0) {
               ret_val = bcm_enet_driver_if_stp_set(if_name, STP_LEARNING);
            }
            else if (strcasecmp(operation, "forwarding") == 0) {
               ret_val = bcm_enet_driver_if_stp_set(if_name, STP_FORWARDING);
            }
            else {
               fprintf(stderr, "%s", usage_msgs[IFSTP_MSG]);
               return 0;
           ...
           ret_val = bcm_enet_driver_if_stp_get(if_name, &param1);
           if (!ret_val) printf("%s STP state: %s\n", if_name, 
                param1==STP_BLOCKING?"Blocking":
                param1==STP_FORWARDING?"Forwarding":
                param1==STP_LEARNING?"Learning":
                param1==STP_LISTENING?"Listening":"Disabled");
        }
        else {
            fprintf(stderr, "%s", usage_msgs[IFSTP_MSG]);
            return 0;
        ...
        if (opt_h || (opt_o && (strcmp(operation, "enable") == 0) && ((!opt_p || !opt_x || !opt_y)))) {
            fprintf(stderr, "%s", usage_msgs[PORT_MIRROR_MSG]);
            return 0;
        }
...
#else
                   ,port,param1,param2, (param3?"Yes":"No"));
#endif
        }
        else {
        ...
        int enbl = 0;
        if (opt_h ) {
            fprintf(stderr, "%s", usage_msgs[PORT_TRUNK_MSG]);
            return 0;
        }
        if (opt_o) { /* Optional Parameter present ==> Set */
          ...
          if (!opt_p) {
              ...
              printf("%s", usage_msgs[PORT_STORM_CTRL_MSG]);
              return 0;
          }
        ...
        if (opt_h || (opt_v == 0) ||(opt_t == 0)) {
            fprintf(stderr, "%s", usage_msgs[OAMIDX_MSG]);
            return 0;
        }
        ...
        if (opt_h) {
            fprintf(stderr, "%s", usage_msgs[KEEPALIVE_MSG]);
            return 0;
        }
        else
        {
            unsigned int tries;
            unsigned int iter = param3;
            ...
            printf("Starting runner keepalive with timeout of %us, %u retries",
                   param1, param2);
            if (param3)
            {
                printf(", %u iterations\n", param3);
            ...
            while (iter || !param3)
            {
                ...
                {
                    tries--;
                    ret_val = bcm_KeepAlive(param1);
                    if (ret_val == BCM_E_NONE)
                    {
                        successes++;
                        break;
                    }
                    else if (ret_val == -ETIME)
                    {
                        if (!tries && !param3)
                        {
                            printf("runner keepalive: runner is unresponsive! (%d attempts)\n",
                                   param2 - tries + 1);
                        }
                        ret_val = BCM_E_NONE;
                    }
                ...
                if (ret_val < 0)
                {
                    if (ret_val == -ENETDOWN)
                    {
                        printf("runner keepalive interface bcmswlpbk0 is down\n");
                    }
                    else if (ret_val == -ENOLINK)
                    {
                        printf("runner keepalive interface bcmswlpbk0 link is down\n");
                    }
                    else
                    {
                        printf("runner keepalive error %d: %s\n",
            ...
            if (param3)
            {
                printf("--- Runner keep alive statistics ---\n");
                printf("%d iterations, %d successes\n",
                       param3, successes);
            }
        }
    }
...
#else
static int ethswctl_control_priority_op_single(int unit, int port __attribute__((unused)), 
        int priority, int type, int *val, int get)
#endif
{
    int ret_val;
    ...
    switch (type) {
        ...
        case TXQ_MAX_STREAMS:
            ...
            break;
        case TXQ_CUR_STREAMS:
            type = bcmSwitchCurStreams;
            break;
        case TXQ_ACT_QUE_PER_PORT:
            type = bcmSwitchActQuePerPort;
            break;
#else
...
static int get_switch_pmap(int unit)
{
    ...
    int i8, i4;
    unsigned int u1 = 0, u2 = 0, u3 = 0, u4 = 0, u5 = 0, u7 = 0;

    if(pmap[unit] == (unsigned int)-1) {
        pmap[unit] = 0;
        bcm_get_switch_info(unit, &u1, &u2, &u3, &i4, &u4, &u5, &pmap[unit], &u7, &i8);
    }
...
int ethswctl_control_priority_op(int unit, int port,
    int queue, int type, unsigned int *val, int *threConfMode, int *maxStr, int *actQues, int get)
{
    int ret_val = 0, j, p, q, t, pmap, threPortConfMode, totalPorts = 0, 
        lanUpPorts = 0, wanUpPorts = 0, curStreams = 0, setMode;

    pmap = get_switch_pmap(unit) | (1<<SF2_IMP0_PORT) | (1<<SF2_WAN_IMP1_PORT);
    threPortConfMode = getThreConfMode(unit);

    if (!threPortConfMode && port != -1 && port != SF2_IMP0_PORT && port != SF2_WAN_IMP1_PORT) {
        fprintf(stderr, "Error: Global Flow Threshold mode; no port number other than IMP0(8)/IMP1(5) should be specified\n"
                "    Or use ethswctl -c swflowctrl -t 0 -v 1 to set Port Flow Threshold Mode first.\n");
        return -1;
    }

    if (!get) { // For set 
        setMode = *threConfMode;
        ...
        if (setMode != -1)
            ret_val = ethswctl_control_priority_set_single(unit, 0, 0, TXQ_THRED_CONFIG_MODE, setMode);

        if (*maxStr != -1)
            ret_val |= ethswctl_control_priority_set_single(unit, 0, 0, TXQ_MAX_STREAMS, *maxStr);

        if (*actQues != -1)
            ret_val |= ethswctl_control_priority_set_single(unit, 0, 0, TXQ_ACT_QUE_PER_PORT, *actQues);
        ...
        ethswctl_control_priority_get_single(unit, 0, 0, TXQ_CUR_STREAMS, (void *)&curStreams);
        ethswctl_control_priority_get_single(unit, 0, 0, TXQ_ACT_QUE_PER_PORT, (void *)actQues);
        ...
        printf("Cur Stream Number   : %d\n", curStreams);
        printf("Act Queu Per Port   : %d\n", *actQues);
    }

    printf("%s Flow Threshold Mode.\n", threPortConfMode? "Port": "Global");
    for( j=0, p = port==-1? 0: port; p <= SF2_IMP0_PORT; p++, j++) {
        int allPortModeNextPort[3] = {SF2_WAN_IMP1_PORT-1, SF2_IMP0_PORT-1, 255};
        ...
        if (port != -1) break;
        if (port == -1 && !threPortConfMode)
            p = allPortModeNextPort[j];
        printf("==========================================================================\n");
    }
    return ret_val;
}
...
void queue_map_print(int queWan, int queRemap)
{
    int i, q;
    ...
    {
        printf("WAN Queues: ");
        for(i=0, q=0; q<FC_LAN_TXQ_QUEUES; q++)
        {
            if (queWan & (1<<q))
            {
                printf("%s%d", i? ", ": "", q);
        ...
        printf("Ethernet Queue Remapping:\n");
        for(i=0, q=0; q<FC_LAN_TXQ_QUEUES; q++)
        {
            printf("  Ethernet Queue %d -> Switch Queue %d\n", q, (queRemap>>(q*4)) & 0xf);
        }
    }
}
...
int ethswctl_quemap(int unit, int queWan, int queRemap)
{
    ...
    if (queWan == -1 && queRemap == -1)
    {
        queue_map_print(curWan, curRemap);
        return rc;
    }
    else if (queWan == -1)
    {
    ...
    queue_map_print(queWan, queRemap);
...
#if defined(HAS_SF2)
int ethswctl_maclmt_op(int unit, int port, int type, int *val)
{
    int ret_val = 0, p = 0, pmap;

    static char *maclmt_action[] = {
        "0=incOvrLmtPktCnt + forward", "1=incOvrLmtPktCnt + drop", 
        "2=incOvrLmtPktCnt + copy to CPU", "3=incOvrLmtPktCnt + redirect to CPU"
    };
    pmap = get_switch_pmap(unit) | (1<<SF2_IMP0_PORT) | (1<<SF2_WAN_IMP1_PORT);

    if (PORT_LIMIT_EN <= type  && type < MACLMT_MAX_TYPES) {    // set operation
        ret_val = bcm_port_maclmt_set(unit, port, type, val);
    } else if ( type == -1) {                                   // get operation
        int enable, limit, action, learned, pkt_cnt;
        for (p = (port==-1) ? 0 : port; p <= SF2_IMP0_PORT; p++) {
            if ((pmap & 1<< p) == 0) continue;
            if ((ret_val = bcm_port_maclmt_get(unit, p, PORT_LIMIT_EN, &enable)) != 0 ||
                (ret_val = bcm_port_maclmt_get(unit, p, PORT_LIMIT, &limit)) != 0 ||
                (ret_val = bcm_port_maclmt_get(unit, p, PORT_ACTION, &action)) != 0 ||
                (ret_val = bcm_port_maclmt_get(unit, p, PORT_LEARNED_COUNT, &learned)) != 0 ||
                (ret_val = bcm_port_maclmt_get(unit, p, PORT_OVER_LIMIT_PKT_COUNT, &pkt_cnt)) != 0) {
                printf("Error getting port attribute %d\n", ret_val);
                return ret_val;
            }
            printf("port %d %s limit=[%d MACs] ovrLmtAction=[%s] learned=[%d MACs] ovrLmtCnt=[%d pkt]\n", p, enable? "enable ":"disable", limit, maclmt_action[action], learned, pkt_cnt);
            if (port !=-1) break;
        }
        if ((ret_val = bcm_port_maclmt_get(unit, port, GLOBAL_LIMIT, &limit)) !=0 ||
            (ret_val = bcm_port_maclmt_get(unit, port, GLOBAL_LEARNED_COUNT, &learned)) !=0) {
            printf("Error getting global attribute %d\n", ret_val);
            return ret_val;
        }
        printf(" global limit=[%d MACs] learned=[%d MACs]\n", limit, learned);
    }
...
int ethswctl_flow_control_get(int unit, int type __attribute__((unused)), unsigned int *val)
{
    int ret_val = 0;
