...
#if defined(RTAX58U) || defined(TUFAX3000) || defined(RTAX82U)
    memcpy(&tail, imagePtr + imageSize - MAX_TAIL_LEN, MAX_TAIL_LEN);
#if 0
    for (i = 0; i < sizeof(tail); i++)
        printf("%02x ", ((uint8_t *)&tail)[i]);
    printf("\n");
#endif
    /* safe strip trailing spaces */
    for (i = 0; i < MAX_PID_LEN && tail.pid[i] != '\0'; i++);
    for (i--; i >= 0 && (tail.pid[i] == '\x20' || tail.pid[i] == '\xff'); i--)
        tail.pid[i] = '\0';
#if 0
    printf("productid field in image: %s\n", tail.pid);
#endif

#if defined(RTAX58U)
    if (strncmp("RT-AX58U", tail.pid, MAX_PID_LEN))
#elif defined(TUFAX3000)
    if (strncmp("TUF-AX3000", tail.pid, MAX_PID_LEN))
#elif defined(RTAX82U)
    if (strncmp("RT-AX82U", tail.pid, MAX_PID_LEN))
#endif
    {
        if (strlen(tail.pid))
        {
            printf("firmware check error!\n");
            goto err_out;
        }
    }
#endif
